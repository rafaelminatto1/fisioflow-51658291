...
serve(async (req) => {
  ...
  try {
    // Rate limiting
    const rateLimitResult = await checkRateLimit(req, 'create-voucher-checkout');
    if (!rateLimitResult.allowed) {
      console.warn(`Rate limit excedido para create-voucher-checkout: ${rateLimitResult.current_count}/${rateLimitResult.limit}`);
