...
serve(async (req) => {
  ...
  try {
    // Rate limiting
    const rateLimitResult = await checkRateLimit(req, 'intelligent-reports');
    if (!rateLimitResult.allowed) {
      console.warn(`Rate limit excedido para intelligent-reports: ${rateLimitResult.current_count}/${rateLimitResult.limit}`);
