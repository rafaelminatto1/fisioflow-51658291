...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'staff_performance_metrics' AND schemaname = 'public') THEN
    ...
    EXECUTE 'DROP POLICY IF EXISTS "Admins veem métricas de performance" ON public.staff_performance_metrics';
    ...
    EXECUTE 'DROP POLICY IF EXISTS "Profissionais veem próprias métricas" ON public.staff_performance_metrics';
    ...
    EXECUTE 'DROP POLICY IF EXISTS "Sistema cria métricas" ON public.staff_performance_metrics';
