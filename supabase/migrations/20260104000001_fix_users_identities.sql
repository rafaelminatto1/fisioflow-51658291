...
BEGIN
  ...
  SELECT id INTO rafael_user_id FROM auth.users WHERE email = 'REDACTED_EMAIL' LIMIT 1;
  ...
  IF rafael_user_id IS NOT NULL AND NOT EXISTS (
    ...
    VALUES (
      gen_random_uuid(),
      rafael_user_id,
      jsonb_build_object('email', 'REDACTED_EMAIL', 'email_verified', true, 'sub', rafael_user_id::text),
      'email',
      now(),
      now(),
      now()
