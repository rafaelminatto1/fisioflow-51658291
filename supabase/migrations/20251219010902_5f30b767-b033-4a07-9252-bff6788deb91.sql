...
DROP POLICY IF EXISTS "Admins e fisios gerenciam prescrições" ON public.exercise_prescriptions;
...
DROP POLICY IF EXISTS "Acesso público via QR code" ON public.exercise_prescriptions;
...
DROP POLICY IF EXISTS "Pacientes podem marcar exercícios concluídos" ON public.exercise_prescriptions;
...
DROP TRIGGER IF EXISTS update_exercise_prescriptions_updated_at ON public.exercise_prescriptions;
...
-- Índices
CREATE INDEX IF NOT EXISTS idx_prescriptions_patient ON public.exercise_prescriptions(patient_id);
CREATE INDEX IF NOT EXISTS idx_prescriptions_qr_code ON public.exercise_prescriptions(qr_code);
CREATE INDEX IF NOT EXISTS idx_prescriptions_status ON public.exercise_prescriptions(status);