...
-- 2. Daily Quest Definitions (Insert if not exists logic is trickier in SQL pure insert without unique constraint on title)
-- We will check by title to avoid duplicates or assume this is a fresh run/safe to add.
-- Best to use ON CONFLICT if we had a unique code, but we don't.
-- We'll delete existing daily quests to reset them (safe for definitions) or just insert.
-- Let's check if 'category' + 'title' can be unique.
-- For now, let's just insert standard ones if they don't exist.
