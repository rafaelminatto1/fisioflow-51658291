...
const firestore = () => (0, firestore_1.getFirestore)((0, app_1.getApp)());
...
async function getProfile(userId) {
    ...
    try {
       ...
       WHERE user_id = $1`, [userId]);
        ...
        logger_1.logger.info('[Auth Middleware] PostgreSQL query failed, trying Firestore:', error);
    }
    ...
    try {
        ...
        logger_1.logger.info('[Auth Middleware] Firestore query failed:', firestoreError);
    }
    ...
    try {
        ...
        try {
           ...
           ON CONFLICT (id) DO UPDATE SET slug = EXCLUDED.slug`, [organizationId, orgSlug, 'admin@fisioflow.com.br']);
            logger_1.logger.info(`[Auth Middleware] Created/updated organization: ${organizationId}`);
        }
        catch (orgError) {
