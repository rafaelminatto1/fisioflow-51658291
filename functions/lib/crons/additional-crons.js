...
exports.expiringVouchers = (0, scheduler_1.onSchedule)({
    ...
    try {
        ...
        // Send reminders
        let remindersSent = 0;
        const results = await Promise.all(vouchersSnapshot.docs.map(async (doc) => {
            ...
            try {
                ...
                if (patient.email) {
                    firebase_functions_1.logger.info('[expiringVouchers] Enviando lembrete por email', {
                ...
                if (patient.phone) {
                    firebase_functions_1.logger.info('[expiringVouchers] Enviando lembrete por WhatsApp', {
                        voucherId: voucher.id,
                        patientId: patient.id,
        ...
        firebase_functions_1.logger.info('[expiringVouchers] Concluído', {
            remindersSent,
        ...
        firebase_functions_1.logger.error('[expiringVouchers] Erro fatal', { error });
        throw error;
    }
});
// Helper function to fetch documents and return a map
async function fetchDocumentsMap(collectionPath, ids) {
    const db = (0, init_1.getAdminDb)();
    const map = new Map();
    if (ids.length === 0)
        return map;
...
exports.birthdays = (0, scheduler_1.onSchedule)({
    ...
    try {
        ...
        const orgIds = [
            ...new Set(birthdayPatients.map((p) => p.organization_id).filter(Boolean)),
        ];
        ...
        const results = await Promise.all(birthdayPatients.map(async (patient) => {
            const org = orgsMap.get(patient.organization_id);
            const preferredChannel = patient.settings?.notification_channel ||
            ...
            try {
                ...
                if (whatsappEnabled && preferredChannel === 'whatsapp' && patient.phone) {
                    firebase_functions_1.logger.info('[birthdays] Enviando mensagem WhatsApp', {
                        patientId: patient.id,
                        patientPhone: patient.phone,
                    });
                    messagesQueued++;
                ...
                if (emailEnabled && patient.email) {
                    firebase_functions_1.logger.info('[birthdays] Enviando email', {
                        patientId: patient.id,
                        patientEmail: patient.email,
                    });
                    messagesQueued++;
        ...
        firebase_functions_1.logger.error('[birthdays] Erro fatal', { error });
        throw error;
    }
});
// ============================================================================
...
exports.cleanup = (0, scheduler_1.onSchedule)({
    ...
    try {
        ...
        try {
            ...
            result.notificationHistory = await (0, init_1.deleteByQuery)('notification_history', 'created_at', '<', ninetyDaysAgo.toISOString(), { maxDeletes: 10000 });
        ...
        try {
            ...
            result.passwordResetTokens = await (0, init_1.deleteByQuery)('password_reset_tokens', 'created_at', '<', twentyFourHoursAgo.toISOString(), { maxDeletes: 5000 });
        ...
        try {
            ...
            result.systemHealthLogs = await (0, init_1.deleteByQuery)('system_health_logs', 'created_at', '<', thirtyDaysAgo.toISOString(), { maxDeletes: 5000 });
        ...
        if (errors.length > 0) {
            firebase_functions_1.logger.warn('[cleanup] Erros durante limpeza', { errors });
        }
        firebase_functions_1.logger.info('[cleanup] Limpeza concluída', { result });
        void {
            success: true,
            timestamp: new Date().toISOString(),
            ...result,
        ...
        firebase_functions_1.logger.error('[cleanup] Erro fatal', { error });
        throw error;
    }
});
// ============================================================================
    ...
    try {
        ...
        for (const patientId of uniquePatientIds) {
            ...
            if (!patientRef.exists) {
                const count = appointmentsSnapshot.docs.filter(doc => doc.data().patient_id === patientId).length;
                orphanedAppointmentsCount += count;
            }
        }
        if (orphanedAppointmentsCount > 0) {
            issues.push(`${orphanedAppointmentsCount} agendamentos com pacientes inválidos`);
        }
        ...
        if (!sessionsSnapshot.empty) {
            ...
            for (const appointmentId of uniqueAppointmentIds) {
                ...
                if (!appointmentRef.exists) {
                    const count = sessionsSnapshot.docs.filter(doc => doc.data().appointment_id === appointmentId).length;
                    orphanedSessionsCount += count;
                }
            }
        }
        if (orphanedSessionsCount > 0) {
            issues.push(`${orphanedSessionsCount} sessões com agendamentos inválidos`);
        }
        ...
        if (!paymentsSnapshot.empty) {
            ...
            for (const appointmentId of uniqueAppointmentIds) {
                const appointmentRef = await db
                    ...
                    .get();
                if (!appointmentRef.exists) {
                    const count = paymentsSnapshot.docs.filter(doc => doc.data().appointment_id === appointmentId).length;
                    orphanedPaymentsCount += count;
                }
            }
        }
        if (orphanedPaymentsCount > 0) {
            issues.push(`${orphanedPaymentsCount} pagamentos com agendamentos inválidos`);
        }
        ...
        for (const orgId of uniqueOrgIds) {
            ...
            if (!orgRef.exists) {
                const count = patientsSnapshot.docs.filter(doc => doc.data().organization_id === orgId).length;
                orphanedPatientsCount += count;
            }
        }
        if (orphanedPatientsCount > 0) {
            issues.push(`${orphanedPatientsCount} pacientes com organizações inválidas`);
        }
        // Log results
        if (issues.length > 0) {
        ...
        firebase_functions_1.logger.error('[dataIntegrity] Erro fatal', { error });
        throw error;
    }
});
//# sourceMappingURL=additional-crons.js.map