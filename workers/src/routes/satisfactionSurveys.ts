...
app.get('/', requireAuth, async (c) => {
  ...
  if (therapistId) { sql += ` AND therapist_id = $${idx++}`; params.push(therapistId); }
  if (startDate) { sql += ` AND sent_at >= $${idx++}`; params.push(startDate); }
  if (endDate) { sql += ` AND sent_at <= $${idx++}`; params.push(endDate); }
  if (responded === 'true') { sql += ` AND responded_at IS NOT NULL`; }
  else if (responded === 'false') { sql += ` AND responded_at IS NULL`; }
...
app.post('/', requireAuth, async (c) => {
  ...
  const result = await db.query(
    ...
    [
      user.organizationId, body.patient_id, body.appointment_id ?? null, body.therapist_id ?? null,
      body.nps_score ?? null, body.q_care_quality ?? null, body.q_professionalism ?? null,
      body.q_facility_cleanliness ?? null, body.q_scheduling_ease ?? null, body.q_communication ?? null,
      body.comments ?? null, body.suggestions ?? null,
    ]
  );
