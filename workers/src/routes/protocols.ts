...
app.get('/', async (c) => {
  ...
  if (evidenceLevel) {
    conditions.push(
      eq(exerciseProtocols.evidenceLevel, evidenceLevel as 'A' | 'B' | 'C' | 'D'),
    );
  }
    ...
    db
      ...
      .where(
        ...
        sql`${protocolExercises.protocolId} = (
          SELECT id FROM exercise_protocols WHERE ${isUuid ? sql`id = ${id}` : sql`slug = ${id}`
          } LIMIT 1
        )`,
      )
...
app.put('/:id', requireAuth, async (c) => {
  ...
  const allowedFields = [
    'name', 'conditionName', 'protocolType', 'evidenceLevel',
    'description', 'objectives', 'contraindications', 'weeksTotal',
    'phases', 'milestones', 'restrictions', 'progressionCriteria',
    'references', 'icd10Codes', 'tags', 'clinicalTests', 'isPublic',
  ];
