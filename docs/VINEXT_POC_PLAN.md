...
4.  **Autenticação:** Neon Auth.

### Impacto de uma Migração Completa para Vinext
Se fôssemos migrar o aplicativo principal (`apps/web` ou o SPA principal):
- **O que ganharíamos:** Renderização no Servidor (SSR) e Server Components (RSC) nativos. Isso melhoraria potencialmente o First Contentful Paint (FCP) de rotas pesadas.
...
2. Criar uma rota dinâmica (ex: `app/financeiro/fluxo-caixa/page.tsx`) para testar o roteamento.

### Fase 4: Integração com a Infraestrutura FisioFlow
1. **DB Direto (RSC):** Na `page.tsx` (como Server Component), conectar diretamente ao Neon via `@neondatabase/serverless` + Drizzle ORM, usando o schema compartilhado `@fisioflow/db`.
2. **Neon Auth:** Ler o JWT de cookies HttpOnly (`f-auth-token`) via `cookies()` do vinext/server e validar via JWKS.
