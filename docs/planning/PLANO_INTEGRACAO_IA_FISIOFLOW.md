...
| **4**            | **Feature de alto valor: resumo de evolução Observação Livre** | - Quando uma sessão é fechada (`POST /api/sessions/:id/close`), disparar uma fila (Queue) que chama o gateway com prompt: “Resuma a evolução Observação Livre abaixo em 2‑3 frases para o paciente: …”. <br> - Salvar o texto retornado em nova tabela `session_summaries`. <br> - Enviar notificação via WhatsApp (usando Twilio ou similar) com o resumo. <br> - Validar latência (<2 s) e qualidade com usuários internos.                 |
...
_Fim do documento._ Salve este conteúdo como, por exemplo, `PLANO_INTEGRACAO_IA_FISIOFLOW.md` no diretório raiz do repositório.
