...
- **Transition Status:** The project migration from Firebase to Neon DB (PostgreSQL) + Drizzle ORM + Cloudflare Workers (v4.0.0 architecture) is **COMPLETED**.
- **API Consolidation:** The legacy API (`cloudflare-worker/fisioflow-api.ts`) has been purged. The canonical API is `apps/api/src/index.ts`.
