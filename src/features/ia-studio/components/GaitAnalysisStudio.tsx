				...
				>
					...
					<header className="p-6 border-b border-white/5 flex items-center justify-between bg-black/40 backdrop-blur-md">
						<div className="flex items-center gap-4">
							...
							<div>
								<h2 className="text-xl font-black text-white tracking-tight uppercase italic">GAIT<span className="text-blue-500">STUDIO</span></h2>
								<p className="text-[10px] text-blue-400 uppercase font-black tracking-widest">Advanced Kinematics Engine</p>
							</div>
						</div>
						<div className="flex items-center gap-3">
							<Button variant="outline" className="border-white/10 text-white rounded-xl h-10 gap-2 bg-white/5 hover:bg-white/10" onClick={() => toast.info("Exportando relatório PDF...")}>
							...
							{!videoFile ? (
								<div className="absolute inset-0 flex flex-col items-center justify-center text-center p-12 space-y-8">
									...
									<div className="space-y-3">
										<h3 className="text-2xl font-black text-white uppercase tracking-tighter">Análise de Ciclo de Marcha</h3>
										<p className="text-slate-500 max-w-sm font-medium">Importe um vídeo para iniciar a detecção automática de fases da marcha e oscilação.</p>
									</div>
											...
											>
												<div className="w-full max-w-md space-y-6">
													<div className="flex justify-between items-end">
														...
														<span className="text-2xl font-black text-white tabular-nums">{analysisProgress}%</span>
													</div>
													<Progress value={analysisProgress} className="h-2 bg-white/10" />
													<div className="flex gap-2">
														<Badge variant="outline" className="bg-white/5 border-white/10 text-white/50 text-[9px]">33 LANDMARKS</Badge>
														<Badge variant="outline" className="bg-white/5 border-white/10 text-white/50 text-[9px]">TEMPORAL MAPPING</Badge>
													</div>
												</div>
											</motion.div>
										)}
						...
						<aside className="w-full lg:w-[380px] space-y-6 flex flex-col h-full overflow-y-auto pr-2 custom-scrollbar">
							<Card className="bg-slate-900/50 border-white/5 rounded-[32px] overflow-hidden shrink-0">
								<CardContent className="p-6 space-y-6">
									<div className="flex items-center justify-between">
										<h3 className="text-sm font-black uppercase text-slate-400 tracking-widest">IA Overlays</h3>
							...
							<div className="flex-1 space-y-4">
								<div className="flex items-center gap-2 px-2">
									...
									<h4 className="text-xs font-black uppercase text-slate-400 tracking-widest">Biomecânica em Tempo Real</h4>
								</div>
								
								{[
									{ label: "Oscilação Lateral", value: metrics.trunkSway, status: "Alerta", color: "text-amber-500", bg: "bg-amber-500/10" },
									{ label: "Simetria de Passo", value: metrics.symmetry, status: "Ideal", color: "text-emerald-500", bg: "bg-emerald-500/10" },
									{ label: "Cadência", value: metrics.cadence, status: "Análise", color: "text-blue-400", bg: "bg-blue-400/10" },
									{ label: "Carga de Impacto", value: metrics.impact, status: "Foco", color: "text-red-500", bg: "bg-red-500/10" }
								].map((m, i) => (
