				...
				<div className="space-y-6">
					<div className="flex justify-between items-end">
						...
						<div className="text-right">
							<span className="text-sm font-black text-emerald-500">{p.progressPercentage}% concluído</span>
						</div>
					</div>

					<div className="relative">
						<Progress value={p.progressPercentage} className="h-4 rounded-full bg-slate-100 dark:bg-slate-800 shadow-inner" />
						<motion.div 
							initial={{ width: 0 }}
					...
					<div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-100 dark:border-slate-800">
						<div>
							<span className="text-[10px] uppercase font-black text-slate-400 tracking-widest block mb-1">Total Estimado</span>
							<div className="flex items-center gap-2 font-bold text-slate-700 dark:text-slate-300">
								<Target className="w-3.5 h-3.5" /> {p.predictedTotal} Sessões
							</div>
						</div>
						<div>
							<span className="text-[10px] uppercase font-black text-slate-400 tracking-widest block mb-1">Fatores IA</span>
							<div className="flex items-center gap-2 font-bold text-slate-700 dark:text-slate-300">
								<Calendar className="w-3.5 h-3.5" /> Alta em {Math.ceil(p.remainingSessions / 2)} sem
							</div>
						</div>
					</div>
				</div>
