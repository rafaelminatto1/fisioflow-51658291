				...
				>
					...
					<div className="absolute top-0 left-0 right-0 p-6 flex items-center justify-between z-50 bg-gradient-to-b from-black/80 to-transparent">
						<div className="flex items-center gap-4">
							...
							<div>
								<h2 className="text-xl font-black text-white tracking-tight italic">FISIO<span className="text-blue-500">ADM</span></h2>
								<p className="text-[10px] text-blue-400 uppercase font-black tracking-widest">Bio-Vision Engine v2.0</p>
							</div>
						</div>
					...
					<div className="w-full h-full max-w-7xl grid grid-cols-1 lg:grid-cols-4 gap-8 mt-16">
								...
								<div>
									...
									<div className="grid grid-cols-1 gap-3">
										{JOINTS.map((joint) => (
											...
											>
												{joint.label}
												{activeJoint === joint.id ? <Zap className="w-4 h-4 fill-white" /> : <ArrowRight className="w-4 h-4 opacity-0 group-hover:opacity-100" />}
											</Button>
										))}
									</div>
								</div>
								...
								{activeJoint && (
									...
									>
										<div className="text-center space-y-1">
											<span className="text-[10px] font-black text-blue-400 uppercase tracking-[0.2em]">Ângulo de Pico</span>
										...
										<div className="flex gap-3">
											<Button variant="outline" className="flex-1 h-12 rounded-xl border-white/10 hover:bg-white/5 text-white" onClick={handleReset}>
												<RefreshCw className="w-4 h-4 mr-2" /> Reset
											</Button>
											<Button className="flex-1 h-12 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold" onClick={handleSave}>
						...
						<div className="lg:col-span-3 relative rounded-[40px] overflow-hidden bg-slate-950 border border-white/5 shadow-2xl">
							{!activeJoint ? (
								<div className="absolute inset-0 flex flex-col items-center justify-center text-center p-12 space-y-6 z-10">
									...
									<div className="space-y-2">
										...
										<p className="text-slate-500 max-w-md">Posicione o paciente de forma que a articulação selecionada esteja visível no enquadramento.</p>
									</div>
								</div>
							) : (
								<>
							...
							<div className="absolute bottom-8 left-8 flex items-center gap-4 z-40">
								<Badge variant="outline" className="bg-black/50 backdrop-blur-md border-white/10 text-white/70 py-2 px-4 rounded-full font-bold">
