...
const BODY_PARTS = [
  "Joelho", "Coluna", "Ombro", "Quadril", "Tornozelo", "Mão/Punho", "Pé", "Geral"
];
...
export function ClinicalImportIA() {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_WORKERS_API_URL}/api/ai/ingest`, {
        method: "POST",
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="space-y-2">
            ...
            <Select onValueChange={(v) => setMetadata(prev => ({ ...prev, specialty: v }))}>
              <SelectTrigger>
          ...
          <div className="space-y-2">
            <Label className="text-xs font-bold uppercase text-muted-foreground">Parte do Corpo</Label>
            <Select onValueChange={(v) => setMetadata(prev => ({ ...prev, body_part: v }))}>
              <SelectTrigger>
                <SelectValue placeholder="Selecione..." />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            ...
            <Select onValueChange={(v) => setMetadata(prev => ({ ...prev, content_type: v }))}>
              <SelectTrigger>
                <SelectValue placeholder="Selecione..." />
              </SelectTrigger>
              <SelectContent>
        ...
        <div className="space-y-2">
          <Label className="text-xs font-bold uppercase text-muted-foreground">Conteúdo Clínico (Texto ou Resumo)</Label>
          <Textarea 
        ...
        <div className="flex items-center justify-between pt-2">
          <div className="flex items-center gap-2 text-[10px] text-muted-foreground font-medium italic">
            ...
            Este conteúdo será processado via Cloudflare AI Gateway e salvo no banco vetorial.
          </div>
          <Button 
            ...
            className="gap-2"
          >
            {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : success ? <CheckCircle2 className="h-4 w-4" /> : <Upload className="h-4 w-4" />}
            {loading ? "Processando..." : success ? "Importado!" : "Alimentar IA"}
          </Button>
        </div>
      </CardContent>
    </Card>
