...
function normalizeFisioBrainResult(value: unknown): FisioBrainResult {
  const raw =
    value && typeof value === "object"
      ? (value as Partial<FisioBrainResult>)
      : {};
...
function FisioBrainChat() {
  ...
  async function handleSearch() {
    ...
    try {
      ...
      const data = normalizeFisioBrainResult(
        await request<unknown>(`/api/fisiobrain/search?${params.toString()}`),
      );
      setResult(data);
      setHistory((prev) =>
        [
          { query: query.trim(), result: data, timestamp: new Date() },
          ...prev,
        ].slice(0, 10),
      );
    } catch {
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        {result ? (
              ...
              <Card className="bg-white border-blue-100 rounded-2xl shadow-md overflow-hidden">
                <CardHeader className="pb-3 border-b border-blue-50 bg-blue-50/30">
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-base font-bold font-display text-blue-900">
                      Resposta do FisioBrain
                    </CardTitle>
                  </div>
                </CardHeader>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {result.sources.map((source) => {
                    const badge =
                      SOURCE_BADGES[source.source] ?? SOURCE_BADGES.wiki;
                    const Icon = badge.Icon;
                    return (
                      <Card
                        key={source.id}
                        className="bg-slate-50/30 border-slate-200/60 rounded-xl hover:shadow-md transition-all cursor-pointer group/source"
                      >
                        <CardContent className="p-4 space-y-2.5">
                          <div className="flex items-start justify-between gap-3">
                            <p className="text-sm font-bold text-slate-900 group-hover/source:text-blue-600 transition-colors line-clamp-1">
                              {source.title}
                            </p>
                            <Badge
                              className={cn(
                                "shrink-0 text-[9px] font-black uppercase tracking-wider gap-1 rounded-md border-transparent",
                                badge.className,
                              )}
                            >
          ...
          <div className="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-3xl p-16 text-center text-slate-400 bg-slate-50/20">
            ...
            <p className="font-bold text-slate-600 font-display text-lg">
              Faça uma pergunta clínica ao FisioBrain
            </p>
            <p className="text-sm mt-2 max-w-sm font-medium">
              O FisioBrain busca em artigos científicos, protocolos, exercícios
              e páginas Wiki indexadas para fornecer a melhor evidência.
            </p>
          </div>
        )}
      </div>
  ...
  const handleSimulation = async () => {
    ...
    try {
      const res = await fetch(
        `${getWorkersApiUrl()}/api/agents/simulator/chat`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="fisiobrain" className="w-full">
        <TabsList className="bg-slate-100/50 p-1.5 rounded-2xl h-auto w-fit mb-8 border border-slate-200/50">
          <TabsTrigger
            value="fisiobrain"
            className="gap-2 rounded-xl px-5 py-2.5 font-bold data-[state=active]:bg-white data-[state=active]:shadow-md data-[state=active]:text-blue-600 transition-all"
          >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="rounded-2xl border-slate-200 shadow-sm bg-white overflow-hidden">
              <CardHeader className="border-b border-slate-50 bg-slate-50/30">
                <CardTitle className="text-lg font-bold font-display text-slate-900">
                  Revisor de Notas SOAP
                </CardTitle>
                <CardDescription className="font-medium">
                  Otimize sua documentação clínica com feedback em tempo real da
                  IA.
                </CardDescription>
              </CardHeader>
            ...
            <div className="space-y-6">
              {soapResult ? (
                <Card className="border-blue-200 bg-blue-50/30 rounded-2xl shadow-md overflow-hidden">
                  <CardHeader className="pb-3 border-b border-blue-100 bg-blue-100/20">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge
                        variant={
                          soapResult.score > 70 ? "default" : "destructive"
                        }
                  ...
                  <CardContent className="space-y-5 pt-5 p-6">
                    <ul className="space-y-2.5">
                      {asStringArray(soapResult.suggestions).map(
                        (s: string, i: number) => (
                          <li
                            key={i}
                            className="flex items-start gap-3 text-sm text-slate-700 font-medium leading-relaxed"
                          >
                ...
                <div className="h-full flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-2xl p-12 text-center text-slate-400 bg-slate-50/20">
                  ...
                  <p className="font-bold text-slate-600 font-display">
                    Insira uma nota SOAP
                  </p>
                  <p className="text-xs mt-1 font-medium">
                    O agente analisará clareza, objetividade e conformidade
                    clínica.
                  </p>
                </div>
              )}
            ...
            <div>
              {simResult ? (
                  ...
                  <CardHeader className="pb-3 border-b border-blue-100 bg-blue-100/20">
                    <div className="flex items-center justify-between">
                      ...
                      {simResult.safetyTriggered && (
                        <Badge
                          variant="destructive"
                          className="animate-pulse rounded-lg font-black uppercase tracking-widest text-[9px]"
                        >
                ...
                <div className="h-full flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-2xl p-12 text-center text-slate-400 bg-slate-50/20">
                  ...
                  <p className="text-xs mt-1 font-medium">
                    Configure o perfil e clique em simular para ver a reação do
                    paciente.
                  </p>
                </div>
              )}
