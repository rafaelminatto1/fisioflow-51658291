...
function mapArtifact(raw: Record<string, unknown>): KnowledgeArtifact {
  ...
  return {
    ...
    evidenceLevel: String(raw.evidenceLevel ?? raw.evidence ?? 'Consensus') as KnowledgeArtifact['evidenceLevel'],
    ...
    clinicalImplications: Array.isArray(raw.clinicalImplications) ? raw.clinicalImplications.map(String) : [],
    ...
    metadata: {
      ...
      authors: Array.isArray(metadata.authors) ? metadata.authors as KnowledgeArtifact['metadata']['authors'] : [],
      journal: metadata.journal ? String(metadata.journal) : (raw.source ? String(raw.source) : undefined),
...
export const knowledgeService = {
  ...
  async listArtifacts(organizationId: string, group?: string): Promise<KnowledgeArtifact[]> {
    try {
      const artifacts = ((await knowledgeApi.listArticles()).data ?? []).map((row) => mapArtifact(row as unknown as Record<string, unknown>));
      const filtered = group && group !== 'Todas' ? artifacts.filter((artifact) => artifact.group === group) : artifacts;
  ...
  async addNote(note: Omit<KnowledgeNote, 'id' | 'createdAt'>): Promise<string> {
    const result = await knowledgeApi.addNote(note.artifactId, note as unknown as Record<string, unknown>);
    return String(result.data.id);
  }
};
