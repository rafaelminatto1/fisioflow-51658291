...
function mapArtifact(raw: Record<string, unknown>): KnowledgeArtifact {
	...
	return {
		...
		citationCount:
			raw.citationCount == null ? undefined : Number(raw.citationCount),
...
function mapNote(raw: Record<string, unknown>): KnowledgeNote {
	return {
		...
		highlightColor: raw.highlight_color
			? String(raw.highlight_color)
			: undefined,
...
export const knowledgeService = {
	...
	async listArtifacts(
		...
		try {
			const artifacts = ((await knowledgeApi.listArticles()).data ?? []).map(
				(row) => mapArtifact(row as unknown as Record<string, unknown>),
			);
			const filtered =
				group && group !== "Todas"
	...
	async updateArtifact(
		id: string,
		data: Partial<KnowledgeArtifact>,
	): Promise<void> {
		await knowledgeApi.updateArticle(
			id,
			data as unknown as Record<string, unknown>,
		);
	},
	...
	async addNote(
		note: Omit<KnowledgeNote, "id" | "createdAt">,
	): Promise<string> {
