...
export default function TemplateAnalyticsPage() {
	...
	const { data: pages = [] } = useQuery({
		...
		queryFn: () =>
			user?.organizationId
				? wikiService.listPages(user.organizationId)
				: Promise.resolve([]),
	...
	const rows = useMemo<TemplateRow[]>(() => {
		...
		return Array.from(byTemplate.entries())
			.map(([templateId, list]) => {
				const donePages = list.filter(
					(page) => getTriageStatus(page) === "done",
				);
				...
				return {
					...
					avgLeadTimeDays:
						done > 0 ? Number((leadTimeTotal / done).toFixed(1)) : 0,
					movesLast14d,
				};
			})
	...
	const totals = useMemo(() => {
		...
		const avgCompletion =
			totalPages > 0 ? Number(((totalDone / totalPages) * 100).toFixed(1)) : 0;
			...
			<div className="grid gap-6 lg:grid-cols-2">
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Percentual de sucesso por tipo de template.
						</CardDescription>
					</CardHeader>
					<CardContent className="h-[300px] mt-4">
						<ResponsiveContainer width="100%" height="100%">
