...
export function sortByTriageOrder(a: WikiPage, b: WikiPage): number {
	const orderA =
		typeof a.triage_order === "number"
			? a.triage_order
			: Number.MAX_SAFE_INTEGER;
	const orderB =
		typeof b.triage_order === "number"
			? b.triage_order
			: Number.MAX_SAFE_INTEGER;
...
export function reorderTriagePages(
	list: WikiPage[],
	startIndex: number,
	endIndex: number,
): WikiPage[] {
...
export function filterTriagePages(
	pages: WikiPage[],
	...
	return pages.filter((page) => {
		if (
			...
			(page.template_id || "manual") !== filters.templateId
		)
			return false;
		if (filters.ownerId !== "all" && page.created_by !== filters.ownerId)
			return false;

		if (tagQuery) {
			const matchesTag = page.tags.some((tag) =>
...
export function calculateAverageAgeDays(
	pages: WikiPage[],
	status: TriageStatus,
	now = new Date(),
): number {
...
export function calculateAverageTimeInColumnDays(
	...
	const total = scoped.reduce((acc, page) => {
		const lastStatusUpdateAt =
			toMillis(page.updated_at) || toMillis(page.created_at);
...
export function buildTriageDropPlan(
	input: TriageDropPlanInput,
): TriageUpdate[] {
	const {
		draggableId,
		sourceStatus,
	...
	if (sourceStatus === destinationStatus) {
		const reordered = reorderTriagePages(
			sourceList,
			sourceIndex,
			destinationIndex,
		);
