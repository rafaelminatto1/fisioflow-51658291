...
function makePage(partial: Partial<WikiPage>): WikiPage {
	return {
		...
		created_at: (partial.created_at as any) || {
			toDate: () => new Date("2026-02-20T00:00:00Z"),
		},
		updated_at: (partial.updated_at as any) || {
			toDate: () => new Date("2026-02-24T00:00:00Z"),
		},
...
describe("triageUtils unit", () => {
	it("getTriageStatus resolves from tags", () => {
		expect(
			getTriageStatus(makePage({ tags: ["triage", "triage-backlog"] })),
		).toBe("backlog");
		expect(
			getTriageStatus(makePage({ tags: ["triage", "triage-in-progress"] })),
		).toBe("in-progress");
		expect(getTriageStatus(makePage({ tags: ["triage", "triage-done"] }))).toBe(
			"done",
		);
	});

	it("applyTriageStatus keeps non-status tags and updates status tag", () => {
		const next = applyTriageStatus(
			["triage", "incident", "triage-backlog"],
			"done",
		);
	...
	it("filterTriagePages applies template/owner/tag/text filters", () => {
		const pages = [
			makePage({
				id: "1",
				title: "Incidente API",
				template_id: "incident-postmortem-v1",
				created_by: "u1",
				tags: ["triage", "incident", "triage-backlog"],
			}),
			makePage({
				id: "2",
				title: "Ata semanal",
				template_id: "meeting-notes-v1",
				created_by: "u2",
				tags: ["triage", "ata", "triage-backlog"],
			}),
		];
