...
function getOrderIndex(slug: string): number {
  const index = EVIDENCE_PAGE_ORDER.indexOf(slug as (typeof EVIDENCE_PAGE_ORDER)[number]);
...
export function getEvidenceTree(pages: WikiPage[]): {
  ...
  const trailPages = sortEvidencePages(
    pages.filter((page) =>
      EVIDENCE_TRAIL_ORDER.includes(page.slug as (typeof EVIDENCE_TRAIL_ORDER)[number]),
    ),
  );
