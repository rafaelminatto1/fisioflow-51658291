...
export const PROTOCOL_DETAILS: Record<
  string,
  {
    description: string;
    objectives: string[];
...
> = {
  "Reconstrução do LCA": {
    description:
      "Protocolo de reabilitação pós-operatória para reconstrução do Ligamento Cruzado Anterior, baseado em evidências científicas atualizadas e guidelines internacionais.",
    objectives: [
  ...
  "Tendinopatia do Manguito Rotador": {
    description:
      "Protocolo de reabilitação conservadora para tendinopatia do manguito rotador, focando em exercícios progressivos de fortalecimento e controle motor.",
    objectives: [
...
export function getProtocolCategory(conditionName: string): string {
  ...
  if (
    lower.includes("joelho") ||
    lower.includes("lca") ||
    lower.includes("lcp") ||
    lower.includes("menisco") ||
    lower.includes("patelar")
  )
    return "joelho";
  if (
    lower.includes("ombro") ||
    lower.includes("manguito") ||
    lower.includes("rotador") ||
    lower.includes("glenoumeral")
  )
    return "ombro";
  if (lower.includes("cotovelo") || lower.includes("epicondilite")) return "cotovelo";
  if (lower.includes("punho") || lower.includes("mao") || lower.includes("carpo"))
    return "punho_mao";
  if (
    lower.includes("coluna") ||
    lower.includes("lombar") ||
    lower.includes("cervical") ||
    lower.includes("lombalgia") ||
    lower.includes("cervicalgia")
  )
    return "coluna";
  if (
    lower.includes("tornozelo") ||
    lower.includes("fascite") ||
    lower.includes("plantar") ||
    lower.includes("pe")
  )
    return "tornozelo";
  if (lower.includes("quadril") || lower.includes("protese") || lower.includes("fetal"))
    return "quadril";
  return "all";
}
...
export const getQuickTemplateCounts = (
  ...
  const posOperatorioCount = protocols.filter((p) => p.protocol_type === "pos_operatorio").length;
  const esportivoCount = protocols.filter((p) => p.protocol_type === "esportivo").length;
  const conservadorCount = protocols.filter(
    (p) => p.protocol_type === "patologia" || p.protocol_type === "conservador",
  ).length;
  const geriatriaCount = protocols.filter((p) => p.protocol_type === "geriatria").length;
