...
export function DoctorAutocomplete({
    ...
    const showDropdown = open && (isLoading || doctors.length > 0 || shouldShowCreateOption || (searchTerm.length >= 2));
                                        ...
                                        >
                                            <div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/20 flex items-center justify-center shrink-0 border border-blue-200 dark:border-blue-800/50">
                                                <span className="text-[11px] font-bold text-blue-700 dark:text-blue-300">
                                                    {(doctor.name.startsWith('Dr') ? doctor.name.substring(3) : doctor.name).charAt(0).toUpperCase()}
                                                </span>
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[11px] text-muted-foreground">
                                                    {doctor.specialty && <span className="font-medium text-blue-600/70 dark:text-blue-400/70 uppercase tracking-wider">{doctor.specialty}</span>}
                                                    {doctor.crm && <span>• CRM {doctor.crm}{doctor.crm_state && `/${doctor.crm_state}`}</span>}
                            ...
                            {shouldShowCreateOption && (
                                ...
                                >
                                    ...
                                    <div className="flex flex-col">
                                        <span className="font-bold text-sm">Cadastrar novo médico</span>
                                        <span className="text-xs opacity-80 italic">"{normalizedSearchTerm}"</span>
                                    </div>
                                </button>
                            )}
                        </div>
