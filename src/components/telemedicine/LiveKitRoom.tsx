...
export function LiveKitRoom({
	...
	const [livekitData, setLivekitData] = useState<
		LiveKitTokenResponse["data"] | null
	>(null);
	...
	const connect = async () => {
		...
		setError(null);
		try {
			const res = await request<LiveKitTokenResponse>(
				"/api/telemedicine/livekit-token",
				{
					method: "POST",
					body: JSON.stringify({
						room_id: roomId,
					...
					<div className="bg-muted rounded-lg p-4 text-sm space-y-2">
						...
						<div className="flex gap-2 flex-wrap">
							...
							<Badge variant="outline">
								{role === "therapist" ? "Profissional" : "Paciente"}
							</Badge>
				...
				<CardContent className="p-6 flex flex-col items-center gap-3 text-center">
					...
					{error?.includes("não configurado") && (
						<p className="text-xs text-muted-foreground bg-muted p-2 rounded">
							Configure LIVEKIT_API_KEY e LIVEKIT_API_SECRET via{" "}
