    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Busque exercícios no banco open-source wger. O sistema buscará evidências científicas
            (PubMed) e a IA estimará a dificuldade automaticamente.
          </DialogDescription>
        </DialogHeader>
          ...
          {isEnriching ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-500 gap-3">
              ...
              <p className="text-sm font-medium animate-pulse">
                A IA está enriquecendo os dados clínicos...
              </p>
            </div>
          ) : results.length > 0 ? (
            <div className="space-y-2">
              {results.map((ex) => (
                <div
                  key={ex.id}
                  className="p-3 border rounded-lg bg-white flex justify-between items-center dark:bg-slate-800"
                >
                  <div className="space-y-1">
                    <p className="font-semibold text-sm">{ex.name}</p>
                    <div className="flex gap-2 text-[10px] text-slate-500">
                      <span className="bg-slate-100 px-1 rounded">
                        {ex.category?.name || "Geral"}
                      </span>
