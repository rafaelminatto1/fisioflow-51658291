...
export const NewProtocolModal: React.FC<NewProtocolModalProps> = ({
		...
		if (protocol && open) {
			setFormData({
				...
				milestones: Array.isArray(protocol.milestones)
					? (protocol.milestones as any)
					: [],
				restrictions: Array.isArray(protocol.restrictions)
					? (protocol.restrictions as any)
					: [],
	...
	const addMilestone = () => {
		setFormData((prev) => ({
			...
			milestones: [
				...prev.milestones,
				{ week: prev.milestones.length + 1, description: "" },
			],
		}));
	};
											...
											{formData.restrictions.map((r, idx) => (
												...
												>
													<div className="flex gap-1 w-32">
														...
														/>
														<span className="text-muted-foreground/30 self-center">
															/
														</span>
														<Input
															type="number"
						...
						<div className="flex gap-3">
							{step < 3 ? (
								<Button
									onClick={nextStep}
									disabled={
										step === 1 && (!formData.name || !formData.condition_name)
									}
