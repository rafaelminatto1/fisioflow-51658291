...
export function WaitlistHorizontal({
  ...
  return (
    <div
      className={cn(
        "w-full bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 flex flex-col transition-all duration-300",
        className,
      )}
    >
          ...
          {hasConnectionBanner ? (
            <div className="flex items-center gap-2">
              ...
              {onRefresh && (
                ...
                >
                  <RefreshCw
                    className={cn("h-3 w-3", (isChecking || isReconnecting) && "animate-spin")}
                  />
                  Atualizar
                </Button>
              )}
          ...
          <ScrollArea className="w-full whitespace-nowrap p-4">
            <div className="flex w-max space-x-4 pb-4">
              ...
              {waitlist.map((entry) => (
                <div
                  key={entry.id}
                  className="w-[280px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-3 hover:shadow-md transition-all relative group"
                >
                  ...
                  <div className="pl-3">
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-6 w-6 -mr-2 -mt-1 text-slate-500 opacity-0 group-hover:opacity-100 transition-opacity"
                          >
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-3">
                      {entry.preferred_days?.slice(0, 3).map((day) => (
                        <div
                          key={day}
                          className="text-[10px] px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-md"
                        >
                          {DAY_LABELS[day]}
                        </div>
                      ))}
