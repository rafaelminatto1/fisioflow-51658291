			...
			>
				...
				>
					{[
						...
						<div
							key={day}
							className="p-3 xs:p-4 text-center text-xs xs:text-sm font-semibold border-r border-border/50 last:border-r-0"
							role="columnheader"
						>
				...
				<div
					className="flex-1 min-h-0 overflow-auto calendar-scroll"
					role="grid"
				>
					<div
						className="grid min-h-full"
						style={{
							gridTemplateRows: `repeat(${weeks.length}, minmax(9.5rem, 1fr))`,
						}}
					>
						{weeks.map((week, weekIndex) => (
							<div
								key={weekIndex}
								className="grid grid-cols-7 border-b border-border/50 last:border-b-0"
								role="row"
							>
								{week.map((day) => {
									...
									return (
										<div
											...
											className={cn(
												...
												isToday(day) &&
													"bg-primary/5 ring-1 ring-inset ring-primary/15 shadow-sm",
												isCurrentMonth &&
													!isToday(day) &&
										...
										>
											...
											>
												{dayAppointments
													...
													.map((apt) => (
														<AppointmentContextMenu
															...
															onMoveToToday={() => {
																/* Optionally implement move to today from month view */
															}}
														>
															...
															>
																<div
																	className={cn(
																		...
																		isOverCapacity(apt) &&
																			"animate-pulse ring-1 ring-amber-400",
																	)}
												...
												{dayAppointments.length > MAX_VISIBLE_APPOINTMENTS && (
													<div
														className="more-appointments-indicator mt-auto rounded-lg border border-dashed border-primary/25 bg-primary/5 px-2 py-1 text-[11px] font-medium text-primary"
														role="status"
														aria-label={`${dayAppointments.length - MAX_VISIBLE_APPOINTMENTS} agendamentos adicionais`}
													>
														+{dayAppointments.length - MAX_VISIBLE_APPOINTMENTS}{" "}
