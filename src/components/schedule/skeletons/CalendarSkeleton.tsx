  ...
  return (
    <div className={cn("w-full h-full bg-white dark:bg-slate-950", className)}>
      ...
      <div className="border-b border-slate-200 dark:border-slate-800 p-4">
        ...
        {/* Day headers */}
        <div className={cn("grid gap-px", viewType === "day" ? "grid-cols-1" : "grid-cols-7")}>
          {Array.from({ length: columns }).map((_, i) => (
            <div key={i} className="h-12 bg-slate-100 dark:bg-slate-900 rounded-md animate-pulse" />
          ))}
        </div>
      </div>
      ...
      <div className="p-4 overflow-hidden">
        <div className={cn("grid gap-2", viewType === "day" ? "grid-cols-1" : "grid-cols-7")}>
