			...
			<div className="sticky top-0 z-20 bg-background border-b">
				<div className="flex">
					...
					<div className="w-12 flex-shrink-0 border-r border-border/30">
						<div
							style={{
								height: `${((endHour - WEEK_CONFIG.startHour) * 4 + 1) * slotHeight}px`,
							}}
			...
			<div style={style}>
				<div className="flex h-full min-h-[60px] border-b border-border/20">
					...
					{Array.from({ length: 7 }, (_, dayIndex) => {
						...
						const isSelected =
							selectedSlot &&
							isSameDay(selectedSlot, slotDate) &&
...
export const VirtualizedWeekView = memo(
	({
			...
			if (isCurrentWeek) {
				...
				if (currentHour >= startHour && currentHour <= endHour) {
					...
					const currentSlotIndex =
						((currentHour - startHour) * 60 + currentMinute) / slotDuration;
		...
		const scrollToSlot = useCallback(
			(slotDate: Date) => {
				const slotDayIndex = Math.floor(
					(slotDate.getTime() - weekStart.getTime()) / (1000 * 60 * 60 * 24),
				);
				...
				if (slotDayIndex >= 0 && slotDayIndex < 7) {
					...
					if (slotHour >= startHour && slotHour <= endHour) {
						const slotIndex =
							((slotHour - startHour) * 60 + slotMinute) / slotDuration;
							...
							<List
								...
								initialScrollOffset={
									isToday(weekStart)
										? Math.max(
												0,
												(new Date().getHours() - startHour) * slotHeight * 4 -
													height / 2,
											)
										: 0
								}
