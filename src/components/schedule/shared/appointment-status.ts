...
export const APPOINTMENT_STATUS_CONFIG: Record<string, AppointmentStatusConfig> = {
  agendado: {
    ...
    bg: "bg-amber-50/90 dark:bg-amber-500/20",
    hoverBg: "hover:bg-amber-100/90 dark:hover:bg-amber-500/30",
  ...
  atendido: {
    ...
    bg: "bg-emerald-50/90 dark:bg-emerald-500/20",
    hoverBg: "hover:bg-emerald-100/90 dark:hover:bg-emerald-500/30",
...
export function normalizeStatus(status: string): string {
  ...
  if (s === "no_show" || s === "paciente_faltou" || s === "faltou" || s === "falta")
    return "faltou";
  if (s === "rescheduled" || s === "remarcado" || s === "reagendado" || s === "remarcar")
    return "remarcar";
  if (
    s === "in_progress" ||
    s === "completed" ||
