...
export function WaitlistIndicator({
	onSchedulePatient,
	className,
}: WaitlistIndicatorProps) {
	...
	if (isLoading) {
		return (
			<div
				className={cn(
					"flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 opacity-50",
					className,
				)}
			>
							...
							<DropdownMenuItem
								...
								onClick={() => {
									const name = entry.patient
										? PatientHelpers.getName(entry.patient)
										: "Paciente";
