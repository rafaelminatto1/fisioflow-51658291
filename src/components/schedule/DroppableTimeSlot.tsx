...
export const DroppableTimeSlot = ({
	...
	const isSaturdayBlockedAfterCutoff =
		isBlocked &&
		day.getDay() === 6 &&
		(() => {
	...
	const isInvalidDrop =
		(isDropTarget || isDraggingOver) && (isBlocked || isClosed);
	...
	const getAriaLabel = () => {
		...
		if (isClosed)
			return `Slot ${time} do dia ${formattedDate}, clínica fechada`;
		if (isBlocked)
			return `Slot ${time} do dia ${formattedDate}, horário bloqueado`;
		if (showPreview) {
	...
	return (
		<div
			...
			className={cn(
				...
				!isClosed &&
					!isBlocked &&
					!isTargetActive &&
					"hover:bg-primary/5 dark:hover:bg-primary/10 cursor-pointer group/cell",
				isBlocked && "bg-red-50/60 dark:bg-red-950/30 cursor-not-allowed",
				// @dnd-kit isOver provides real-time hover state during drag
				isOver &&
					!isClosed &&
					!isBlocked &&
					"bg-primary/10 dark:bg-primary/20 ring-2 ring-inset ring-primary/40 dark:ring-primary/50 shadow-inner animate-in fade-in duration-100",
				// Drop inválido (bloqueado/fechado com drag sobre): feedback explícito
				isInvalidDrop &&
					"bg-red-100/80 dark:bg-red-900/30 ring-2 ring-inset ring-red-400/50 animate-in fade-in duration-100",
				isTargetActive && "z-40",
			)}
		...
		>
			...
			{(isDraggingOver || isTargetActive) && !isInvalidDrop && (
				<div
					className="absolute inset-0 flex flex-col items-center justify-center gap-1.5 text-primary font-medium animate-in fade-in duration-150"
					aria-hidden="true"
				>
			...
			{!isBlocked &&
				...
				!showPreview && (
					<div
						className="absolute inset-x-0 mx-auto w-fit -top-2.5 z-20 opacity-0 group-hover/cell:opacity-100 transition-opacity duration-200 pointer-events-none"
						aria-hidden="true"
					>
