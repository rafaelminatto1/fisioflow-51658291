...
export const useAppointmentModalLogic = ({
	...
	const checkPatientHasPreviousSessions = useCallback(
	...
	useEffect(() => {
		if (
			...
			const hasPreviousSessions =
				checkPatientHasPreviousSessions(watchedPatientId);
		...
		appointment,
		currentMode,
		checkPatientHasPreviousSessions,
		setValue,
	]);
	...
	useEffect(() => {
		...
		const apptTime =
			defaultTime ||
			(appointment?.time ? String(appointment.time).substring(0, 5) : "09:00");
		setRecurringConfig({
	...
	const watchedDate = useMemo(() => {
		...
		return typeof watchedDateStr === "string"
			? parseISO(watchedDateStr)
			: watchedDateStr;
	}, [watchedDateStr]);

	const { timeSlots: slotInfo, isDayClosed } =
		useAvailableTimeSlots(watchedDate);
	...
	const selectedPatientName = useMemo(() => {
		...
		const selectedPatient = activePatients?.find(
			(patient) => patient.id === watchedPatientId,
		);
		return (
		...
		if (watchedPatientId) {
			const patientHistory = appointments.filter((a) => {
				...
				return (
					...
					!["cancelado", "remarcar", "canceled"].includes(s)
				);
			});
