...
export const useAppointmentForm = ({
  ...
  const persistAppointment = async (appointmentData: AppointmentFormData, recurringConfig: RecurringConfig, ignoreCapacity: boolean = false) => {
    ...
    if (appointmentData.is_recurring && !appointmentId && recurringConfig.days.length > 0) {
      ...
      for (const occ of occurrences) {
        ...
        try {
          ...
          logger.error('Erro ao criar agendamento recorrente', err, 'useAppointmentForm');
        }
      }
  ...
  const handleSave = async (data: AppointmentFormData, recurringConfig: RecurringConfig) => {
    ...
    try {
      ...
      if (isAppointmentConflictError(error)) {
        onOpenCapacityDialog(normalizedData, freshConflictCheck);
        ErrorHandler.handle(error, 'useAppointmentForm:handleSave', { showNotification: false });
      } else {
