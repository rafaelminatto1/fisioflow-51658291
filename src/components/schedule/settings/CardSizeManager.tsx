      ...
      <div className="rounded-lg border bg-muted/20 overflow-hidden">
        {sampleAppts.map((apt) => (
          ...
          >
            ...
            <div
              className={cn(
                "absolute left-12 right-2 top-1 bottom-1 rounded flex items-center px-2 text-white text-[10px] font-medium",
                apt.color,
              )}
            >
      ...
      <div className="rounded-lg border bg-blue-500 overflow-hidden p-2.5 text-white">
        <div className="flex items-center gap-1.5 mb-0.5">
          <span
            ...
            style={{ fontSize: `${baseTime * (fontPercentage / 100)}px` }}
          >
            08:00
          </span>
          <span
            className="font-medium truncate transition-all"
            style={{ fontSize: `${baseName * (fontPercentage / 100)}px` }}
          >
            Maria Santos
          </span>
        </div>
        <span
          className="text-white/80 text-[9px] transition-all"
          style={{ fontSize: `${9 * (fontPercentage / 100)}px` }}
        >
  ...
  const handleSlotHeightChange = useCallback(
    (newHeight: number) => {
      const newScale = Math.round(
        ((newHeight - MIN_SLOT_HEIGHT) / (MAX_SLOT_HEIGHT - MIN_SLOT_HEIGHT)) * 10,
      );
