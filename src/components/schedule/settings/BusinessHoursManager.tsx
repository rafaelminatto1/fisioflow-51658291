		...
		setHours((prev) =>
			prev.map((h) => {
				...
				return {
					...h,
					break_start: enabled
						? h.break_start || DEFAULT_BREAK_START
						: undefined,
	...
	const invalidOpenDays = useMemo(
		() =>
			hours.filter((h) => {
				...
				if (
					toMinutes(h.open_time || "07:00") >=
					toMinutes(h.close_time || "21:00")
				)
					return true;
		...
		<div className="space-y-3">
			...
			{invalidOpenDays.length > 0 && (
				<div className="p-2.5 rounded-lg border border-red-200 bg-red-50 text-xs text-red-700 dark:border-red-900/50 dark:bg-red-950/20 dark:text-red-300">
					Horários inválidos nos dias destacados. Verifique abertura/fechamento
					e intervalos.
				</div>
			)}
			...
			<div className="space-y-0.5">
				{daysOfWeek?.map((day) => {
					const hour =
						hours.find((h) => h.day_of_week === day.value) ||
						DEFAULT_HOURS[day.value];
					const isOpen = !!hour?.is_open;
					const hasBreak = !!(hour?.break_start && hour?.break_end);
					const hasError =
						isOpen &&
						toMinutes(hour?.open_time || "07:00") >=
							toMinutes(hour?.close_time || "21:00");
					const hasBreakError =
						isOpen &&
						hasBreak &&
						(() => {
					...
					const totalMinutes = isOpen
						? (() => {
								...
								if (hasBreak) {
									diff -=
										toMinutes(hour.break_end!) - toMinutes(hour.break_start!);
								}
								...
								{isOpen ? (
									<>
										<Input
											...
											onChange={(e) =>
												updateHour(day.value, "open_time", e.target.value)
											}
											className={cn(
										...
										<Input
											...
											onChange={(e) =>
												updateHour(day.value, "close_time", e.target.value)
											}
											className={cn(
												"h-8 text-xs text-center px-1",
												(hasError || hasBreakError) &&
							...
							{isOpen && (
								<div className="ml-[2rem] pl-3 pr-1 py-1">
									<button
										type="button"
										onClick={() =>
											setExpandedBreak(isExpanded ? null : day.value)
										}
										className={cn(
									...
									>
										...
										{hasBreak ? (
											<span className="font-medium">
												Pausa: {hour.break_start?.slice(0, 5)} -{" "}
												{hour.break_end?.slice(0, 5)}
											</span>
										) : (
										...
										<div className="mt-2 p-3 rounded-lg border bg-muted/20 space-y-2.5 animate-in slide-in-from-top-2 duration-200">
											...
											{hasBreakError && (
												<p className="text-[10px] text-red-600 dark:text-red-400">
													O intervalo deve estar entre o horário de abertura e
													fechamento.
												</p>
											)}
										</div>
