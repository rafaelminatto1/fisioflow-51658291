...
function NewStatusForm({
	...
	return (
		<form
			...
			className="border rounded-lg p-4 bg-slate-50 dark:bg-slate-800 space-y-3"
		>
...
export function StatusColorSettingsModal({
	open,
	onOpenChange,
}: StatusColorSettingsModalProps) {
	const {
		statusConfig,
		updateStatusColor,
		createStatus,
											...
											{statusIds.map((statusId) => {
												...
												return (
													<StatusColorEditor
														...
														onColorChange={(color) =>
															handleColorChange(statusId, color)
														}
								...
								{customStatuses.length === 0 && !showNewForm && (
									<div className="text-center py-8 text-muted-foreground">
										...
										<p className="text-xs mt-1">
											Clique no botão acima para criar um novo status.
										</p>
									</div>
								)}
