...
import { ChevronDown, ChevronUp, Minus, Plus, Trash2, Copy, Check } from "lucide-react";
...
function BufferBadge({ before, after }: { before: number; after: number }) {
    ...
    >
      <button
        ...
        className={cn(
          ...
          isExpanded ? "bg-slate-50 dark:bg-slate-900/50 border-b" : "bg-card hover:bg-muted/30",
        )}
      ...
      >
        ...
        <span className="flex items-center gap-3">
          ...
          <span className="hidden sm:inline">
            <BufferBadge before={type.bufferBeforeMinutes} after={type.bufferAfterMinutes} />
          </span>
          <Switch
              ...
              <div className="flex items-center border rounded-lg mt-1 h-[42px] overflow-hidden">
                <Button
                  ...
                  onClick={() => {
                    ...
                    else if (type.maxPerDay > 1) onUpdate({ maxPerDay: type.maxPerDay - 1 });
                  }}
                >
                ...
                <Button
                  ...
                  onClick={() =>
                    onUpdate({
                      maxPerDay: type.maxPerDay === null ? 1 : type.maxPerDay + 1,
                    })
                  }
                >
          ...
          <div>
            ...
            <div className="flex gap-2">
              {COLOR_OPTIONS.map((c) => (
                ...
                >
                  {type.color === c && <Check className="h-4 w-4 text-white" />}
                </button>
              ))}
            </div>
          </div>
