...
function formatDaysLabel(days: number[]): string {
  ...
  return runs
    .map((r) =>
      r.length >= 2 ? `${DAY_SHORT[r[0]]}-${DAY_SHORT[r[r.length - 1]]}` : DAY_SHORT[r[0]],
    )
    .join(", ");
}
...
export function WeeklyHeatmapGrid({ groups }: WeeklyHeatmapGridProps) {
  ...
  return (
    <div className="bg-card rounded-xl p-6 shadow-sm border border-border/60">
      <h3 className="text-sm font-medium text-foreground mb-4">Disponibilidade Semanal</h3>
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  cap > 0 ? getIntensityClass(ratio) : "bg-muted text-muted-foreground",
                )}
              >
                {cap > 0 ? cap : "—"}
              </div>
