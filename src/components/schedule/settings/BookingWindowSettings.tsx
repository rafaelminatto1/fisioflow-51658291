...
export function BookingWindowSettings() {
  ...
  const update = useCallback(
    <K extends keyof BookingWindowLocalState>(key: K, value: BookingWindowLocalState[K]) => {
      ...
      <div className="space-y-6">
        <div>
          <div className="flex justify-between items-end mb-2">
            <Label className="text-sm font-medium">Agendamento mínimo antecipado</Label>
        ...
        <div>
          <div className="flex justify-between items-end mb-2">
            <Label className="text-sm font-medium">Agendamento máximo antecipado</Label>
            <span className="text-sm font-semibold text-indigo-600 dark:text-indigo-400">
          ...
          <input
            ...
            onChange={(e) => update("maxAdvanceDays", parseInt(e.target.value, 10))}
            className="w-full h-2 bg-muted rounded-lg appearance-none cursor-pointer accent-indigo-500"
          />
          <div className="flex justify-between mt-1 px-1">
        ...
        <div className="space-y-3 pt-4 border-t">
          <div className="flex items-start justify-between gap-3">
            <div>
              <Label className="text-sm font-medium">Permitir agendamento do mesmo dia</Label>
              {state.allowSameDay && (
                <p className="text-xs text-muted-foreground mt-0.5">até {state.sameDayCutoff}</p>
              )}
            </div>
            <Switch
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <Label className="text-sm font-medium">Permitir agendamento online</Label>
            </div>
            <Switch
