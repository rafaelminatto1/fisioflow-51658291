...
const StatCard = memo(
	({ icon, value, label, trend, _variant = "default" }: StatCardProps) => {
		return (
			<div
				className="stat-card group cursor-default min-w-[120px] xs:min-w-[140px]"
				role="article"
			...
			>
				...
				<p className="text-white/70 text-[10px] xs:text-xs truncate font-medium">
					{label}
				</p>
				{trend !== undefined && (
					<div
...
const ScheduleStatsCards = memo(
	({ appointments, _currentDate, className }: ScheduleStatsCardsProps) => {
		...
		return (
			<div
				...
				role="group"
				aria-label="Estatísticas da agenda"
			>
				{/* Today's Total */}
				<StatCard
					icon={
				...
				{todayStats.nextAppointment ? (
					...
					<StatCard
						icon={
							<User
								className="h-4 w-4 xs:h-5 xs:w-5 text-white/70"
								strokeWidth={2}
							/>
						}
						value="--:--"
