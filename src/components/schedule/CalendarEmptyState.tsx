...
const getEmptyStateContent = (viewType: "day" | "week" | "month", currentDate: Date) => {
...
export const CalendarEmptyState: React.FC<CalendarEmptyStateProps> = ({
  ...
  const iconAnimate = reducedMotion
    ? { rotate: 0, scale: 1 }
    : { rotate: [0, 5, -5, 0], scale: [1, 1.05, 1] };
  const iconTransitionLoop = reducedMotion
    ? { duration: 0 }
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="absolute inset-0 bg-primary/5 dark:bg-primary/10 rounded-full blur-3xl scale-150"
          aria-hidden
        />
      ...
      >
        ...
        <p className="text-sm text-slate-600 dark:text-gray-500 mb-6">{content.description}</p>
        ...
        >
          <div className="flex items-start gap-3 text-left">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-slate-700 dark:text-slate-300">Dica</p>
              <p className="text-xs text-slate-600 dark:text-gray-500 mt-1">
                {viewType === "day" ? (
                  ...
                  <>
                    ...
                    <br />
                    Ou clique no agendamento e use <strong>Editar</strong> para mudar data e horário
                    (sem arrastar).
                  </>
                ) : (
