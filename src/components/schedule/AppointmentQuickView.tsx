...
export const AppointmentQuickView: React.FC<AppointmentQuickViewProps> = ({
	...
	const linkedPackage = useMemo(() => {
		...
		return (
			patientPackages.find((p) => p.id === appointment.session_package_id) ??
			null
		);
	...
	const isPackagePayment =
		Boolean(appointment.session_package_id) ||
		appointment.payment_method === "package";

	const time =
		appointment.time && appointment.time.trim() ? appointment.time : "00:00";
	const startHour = parseInt(time.split(":")[0] || "0");
	const startMinute = parseInt(time.split(":")[1] || "0");
	const endMinutes =
		startHour * 60 + startMinute + (appointment.duration || 60);
		...
		<div className="flex flex-col h-full min-w-0 max-w-full overflow-hidden">
			...
			{hasWaitlistInterest && (
				...
				>
					...
					<Bell
						className="h-3.5 w-3.5 text-amber-600 ml-auto animate-pulse"
						aria-hidden="true"
					/>
				</motion.div>
			)}
				...
				<div className="flex items-start justify-between gap-3">
					...
					{appointment.phone && (
						<Button
							...
							onClick={() =>
								window.open(
									`https://wa.me/55${appointment.phone?.replace(/\D/g, "")}`,
									"_blank",
								)
							}
						>
				...
				<div className="space-y-4">
					<div className="space-y-1.5">
						...
						<Select
							...
							onValueChange={(v) =>
								handleTherapistChange(v === THERAPIST_SELECT_NONE ? "" : v)
							}
						>
							<SelectTrigger className="h-9 w-full bg-slate-50/50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors text-sm">
								<SelectValue>
									{getTherapistById(therapists, localTherapistId)?.name ??
	...
	return (
		<>
				...
				<Drawer open={open} onOpenChange={onOpenChange}>
					<DrawerContent className="max-h-[90vh]" data-week-appointment="true">
						<DrawerHeader className="text-left border-b pb-4 hidden">
							...
							<DrawerDescription>
								Visualizar e editar detalhes do agendamento
							</DrawerDescription>
						</DrawerHeader>
				...
				<Popover open={open} onOpenChange={onOpenChange}>
					<PopoverAnchor asChild>
						{React.isValidElement(children) ? (
							...
							<div data-appointment-popover-anchor={appointment.id}>
								{children}
							</div>
						)}
					</PopoverAnchor>
			...
			<AlertDialog
				open={showNoShowConfirmDialog}
				onOpenChange={setShowNoShowConfirmDialog}
			>
				<AlertDialogContent
