...
export function NewPackagePopover({
    ...
    const { watch, setValue, setError, clearErrors, handleSubmit, reset, formState: { errors } } = form;
        ...
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                <Button
                    ...
                    title={!patientId ? "Selecione um paciente primeiro" : (triggerLabel ?? "Vender novo pacote")}
                >
                ...
                <div className="space-y-4">
                    <div className="space-y-1">
                        ...
                        <p className="text-xs text-muted-foreground">
                            {presetPackage?.description ?? 'Configure os detalhes da venda para este paciente.'}
                        </p>
                    </div>
                        ...
                        {isPresetMode ? (
                            <div className="rounded-xl border border-primary/15 bg-primary/5 p-3">
                                <div className="flex items-start justify-between gap-3">
                                    <div>
                                        <p className="text-xs font-semibold text-foreground">{presetPackage?.name}</p>
                                        <p className="mt-1 text-[10px] text-muted-foreground">
                                            Pacote padrão lançado direto para o paciente já escolhido no modal.
                                        </p>
                                    </div>
