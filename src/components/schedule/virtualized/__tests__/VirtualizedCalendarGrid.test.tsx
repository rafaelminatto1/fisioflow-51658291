...
describe("VirtualizedCalendarGrid", () => {
  ...
  const defaultProps = {
    ...
    slotHeight: 60,
    containerHeight: 600,
    renderSlot: (time: string, index: number) => <div data-testid={`slot-${index}`}>{time}</div>,
  };
  ...
  it("should apply virtualization when slot count exceeds 50", () => {
    ...
    render(<VirtualizedCalendarGrid {...defaultProps} timeSlots={manySlots} />);
  ...
  it("should call onScrollChange when scrolling", () => {
    ...
    const { container } = render(
      <VirtualizedCalendarGrid {...defaultProps} onScrollChange={onScrollChange} />,
    );
  ...
  it("should apply custom className", () => {
    const { container } = render(
      <VirtualizedCalendarGrid {...defaultProps} className="custom-class" />,
    );
  ...
  it("should use custom overscan value", () => {
    ...
    render(<VirtualizedCalendarGrid {...defaultProps} timeSlots={manySlots} overscan={5} />);
