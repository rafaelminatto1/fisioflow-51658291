...
import { DEFAULT_APPOINTMENT_DURATION_MINUTES, getOverlapStackPosition } from "@/lib/calendar";
import {
  ...
  ({
    ...
    const renderSlot = React.useCallback(
      (time: string, _rowIndex: number) => {
        ...
        const isDropTarget =
          dropTarget && isSameDay(dropTarget.date, currentDate) && dropTarget.time === time;
