...
const CalendarDayView = memo(
	({
		...
		const preferredSlotHeight = calculateSlotHeightFromCardSize(
			cardSize,
			heightScale,
		);

		const gridViewportRef = React.useRef<HTMLDivElement | null>(null);
		const [gridViewportHeight, setGridViewportHeight] =
			React.useState<number>(600);
		...
		const currentTimePositionPx = React.useMemo(() => {
			...
			const [endH] = (timeSlots[timeSlots.length - 1] || "21:00")
				.split(":")
				.map(Number);
		...
		const appointmentsByTime = React.useMemo(() => {
			...
			dayAppointments.forEach((apt) => {
				const time =
					apt.time && apt.time.trim() ? apt.time.substring(0, 5) : "09:00";
		...
		const renderSlot = React.useCallback(
			(time: string, rowIndex: number) => {
				...
				const previewCards = showPreview
					? [...targetAppointments, dragState.appointment!]
					: [];
				...
				const appointmentsStartingInThisSlot =
					...
					<div className="flex w-full" style={{ height: slotHeightMobile }}>
						...
						<div className="flex-1 relative border-b border-border/50">
									...
									<TooltipTrigger asChild>
										<div
											...
											className={cn(
												...
												blocked &&
													"blocked bg-red-50/60 dark:bg-red-950/30 ring-2 ring-inset ring-red-400/30 cursor-not-allowed",
											...
											onDragOver={(e) =>
												!blocked && handleDragOver(e, currentDate, time)
											}
											onDragLeave={handleDragLeave}
											onDrop={(e) =>
												!blocked && handleDrop(e, currentDate, time)
											}
										>
											{blocked ? (
												<span
							...
							{appointmentsStartingInThisSlot.map((apt) => {
								const aptTime =
									apt.time && apt.time.trim() ? apt.time : "09:00";
								...
								const { index: stackIndex, count: stackCount } =
									getOverlapStackPosition(dayAppointments, apt);
								const hasOverlap = stackCount > 1;

								const widthPercent = hasOverlap ? 96 / stackCount : 94;
								const leftOffset = hasOverlap
									? stackIndex * (96 / stackCount) + 2
									: 3;

								const duration =
									apt.duration ?? DEFAULT_APPOINTMENT_DURATION_MINUTES;
								...
								const isDraggingThis =
									dragState.isDragging && dragState.appointment?.id === apt.id;
						...
						<div className="h-16 md:h-20 border-b bg-gradient-to-r from-primary/10 to-primary/5 p-2 md:p-4 backdrop-blur-sm z-30 shadow-sm flex-shrink-0">
							<div className="font-semibold text-center flex items-center justify-center gap-2 text-sm md:text-base">
								...
								<span className="sm:hidden">
									{format(currentDate, "EEE, d/M", { locale: ptBR })}
								</span>
							</div>
						</div>
