...
import { Clock, Search, ExternalLink, Sun, Sunset, Moon, MoreVertical } from "lucide-react";
...
export function WaitlistSidebar({ onSchedulePatient, className }: WaitlistSidebarProps) {
  ...
  return (
    <div
      className={cn(
        "flex flex-col h-full bg-white dark:bg-slate-950 border-l border-slate-200 dark:border-slate-800 w-80",
        className,
      )}
    >
      {/* Header */}
      <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between sticky top-0 bg-white/50 backdrop-blur-sm z-10">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-semibold text-sm text-slate-900 dark:text-slate-100 leading-none">
              Lista de Espera
            </h3>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center p-8 text-center">
            ...
            <p className="text-xs text-slate-500 mt-1 max-w-[180px]">
              {searchQuery
                ...
                : "Não há pacientes na lista de espera no momento."}
            </p>
          </div>
        ) : (
          <div className="p-3 space-y-3">
            {filteredWaitlist.map((entry) => (
              <div
                key={entry.id}
                className="group relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-3 hover:shadow-md hover:border-indigo-200 dark:hover:border-indigo-900 transition-all cursor-default"
              >
                ...
                <div className="pl-3">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex flex-col">
                      <span
                        className="text-sm font-semibold text-slate-800 dark:text-slate-100 line-clamp-1"
                        title={entry.patient?.name}
                      >
                        {entry.patient?.name}
                      </span>
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-6 w-6 -mr-2 -mt-1 text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity"
                        >
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    {entry.preferred_days?.slice(0, 3).map((day) => (
                      <div
                        key={day}
                        className="text-[10px] px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-gray-500 rounded-md"
                      >
                        {DAY_LABELS[day]}
                      </div>
                    ))}
                    {entry.preferred_periods?.map((period) => {
                      ...
                      return (
                        <div
                          key={period}
                          className="text-[10px] px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-gray-500 rounded-md flex items-center gap-1"
                        >
