...
export function GoogleCalendarButton({
  ...
  const { isConnected, email, isLoading, connect, disconnect } = useGoogleCalendarSync();
      ...
      <AlertDialog open={showDisconnectDialog} onOpenChange={setShowDisconnectDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Isso interromperá a sincronização automática com o Google Calendar. Você pode
              reconectar sua conta a qualquer momento.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
export function GoogleCalendarBadge({ className }: GoogleCalendarBadgeProps) {
  ...
  return (
    <div
      className={cn(
        "flex items-center gap-2 px-2 py-1 bg-green-50 dark:bg-green-900/20 rounded-md",
        className,
      )}
    >
      <CheckCircle2 className="w-3.5 h-3.5 text-green-600 dark:text-green-400" />
      <span className="text-xs font-medium text-green-700 dark:text-green-300">{email}</span>
    </div>
  );
}
