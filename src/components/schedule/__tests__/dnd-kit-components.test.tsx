...
vi.mock("@/lib/calendar/dragPreview", () => ({
	...
	calculateCardOffsetPercent: (index: number, count: number) =>
		(index * 100) / count,
	shouldShowText: () => true,
...
vi.mock("../DropTargetPreviewCard", () => ({
	DropTargetPreviewCard: ({
		...
		<div
			key={`preview-${appointment.id}-${++previewKeyCounter}`}
			data-testid={`preview-card-${appointment.id}`}
			data-dragged={isDraggedCard}
		>
...
describe("CalendarDragOverlay", () => {
	...
	describe("Estilos de Status", () => {
		it.each([
			...
			expect(
				container.querySelector(`.${expectedClass.split(" ")[0]}`),
			).toBeInTheDocument();
		});
	});
});
...
describe("DroppableTimeSlot", () => {
	...
	describe("Renderização Básica", () => {
		...
		it("deve ter data-testid correto", () => {
			...
			const cell = container.querySelector(
				'[data-testid="time-slot-2026-02-05-09:00"]',
			);
			expect(cell).toBeInTheDocument();
		});
	});
	...
	describe("Edge Cases - Múltiplos Appointments", () => {
		it("deve renderizar múltiplos preview cards com chaves únicas", () => {
			...
			const apt3 = createMockAppointment({
				id: "apt-3",
			...
			expect(
				container.querySelector('[data-testid="preview-card-apt-1"]'),
			).toBeInTheDocument();
			expect(
				container.querySelector('[data-testid="preview-card-apt-2"]'),
			).toBeInTheDocument();
			expect(
				container.querySelector('[data-testid="preview-card-apt-3"]'),
			).toBeInTheDocument();
		});
	});
});
