...
describe("MiniCalendar", () => {
  ...
  const appointmentDates = [new Date(2025, 0, 10), new Date(2025, 0, 15), new Date(2025, 0, 20)];

  it("deve renderizar o mês atual", () => {
    render(<MiniCalendar selectedDate={mockDate} onDateSelect={vi.fn()} appointmentDates={[]} />);
  ...
  it("deve marcar a data selecionada", () => {
    const { container } = render(
      <MiniCalendar selectedDate={mockDate} onDateSelect={vi.fn()} appointmentDates={[]} />,
    );
