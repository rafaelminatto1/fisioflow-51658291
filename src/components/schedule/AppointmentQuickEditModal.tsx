...
import { differenceInMinutes, differenceInYears, format, parseISO } from "date-fns";
...
import type { Appointment, AppointmentBase, AppointmentStatus } from "@/types/appointment";
...
export const AppointmentQuickEditModal: React.FC<AppointmentQuickEditModalProps> = ({
  ...
  const { data: patientMonthCancellations = 0, isLoading: isLoadingPatientMonthCancellations } =
    useQuery<number>({
      queryKey: [
  ...
  const dateFormatted = useMemo(
    () =>
      formData.appointment_date
        ? format(parseResponseDate(formData.appointment_date), "EEEE, d 'de' MMMM 'de' yyyy", {
            locale: ptBR,
          })
        : "",
    [formData.appointment_date],
  ...
  const requiresMinHours =
    effectiveCancellationRules.min_hours_before ?? DEFAULT_CANCELLATION_RULES.min_hours_before;
  const monthlyLimit =
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-2">
                  ...
                  {isEditing ? (
                    ...
                    <p className="text-sm font-medium py-2 capitalize">{dateFormatted}</p>
                  )}
                </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong>{appointment.patientName}</strong> para <strong>{dateFormatted}</strong> às{" "}
            ...
            >
              ...
              <div className="grid gap-1.5 text-muted-foreground">
                <p>
                  Antecedência mínima:{" "}
                  <strong className="text-foreground">{requiresMinHours}h</strong>
                </p>
                <p>
                  Tempo restante:{" "}
                  <strong
                    className={cn(
                      requiresPolicyOverride && isLateCancellation && "text-destructive",
                    )}
                  >
                ...
                <p>
                  ...
                  <strong
                    className={cn(
                      requiresPolicyOverride && exceedsMonthlyLimit && "text-destructive",
                    )}
                  >
                    {monthlyLimit > 0
                ...
                {lateFeeApplies && (
                  <p className="text-destructive">
                    ...
                    <strong>
                      R$ {(effectiveCancellationRules.late_cancellation_fee || 0).toFixed(2)}
                    </strong>
                  </p>
                )}
              ...
              {requiresPolicyOverride && (
                <p className="mt-2 text-xs font-medium text-destructive">
                  Cancelamento fora da política. Confirme somente se deseja manter o cancelamento
                  manual.
                </p>
              )}
            </div>
