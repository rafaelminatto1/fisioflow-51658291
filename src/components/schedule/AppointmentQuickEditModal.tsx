...
export const AppointmentQuickEditModal: React.FC<
...
> = ({ appointment, open, onOpenChange, onDeleted: _onDeleted }) => {
	...
	const {
		data: patientMonthCancellations = 0,
		isLoading: isLoadingPatientMonthCancellations,
	} = useQuery<number>({
		queryKey: [
			"patient-cancellations-month",
		...
		queryFn: async () => {
			...
			return (response.data ?? []).filter((data) => {
				...
				return (
					appointmentDate >= monthStartStr && appointmentDate <= monthEndStr
				);
			}).length;
		},
		staleTime: 30000,
				...
				.then(({ data }) => {
					if (data) {
						setPatientDetails({
							...
							birthDate:
								(data as { birth_date?: string | null }).birth_date ||
								undefined,
						});
					}
				})
		...
		if (patientDetails?.birthDate) {
			try {
				return differenceInYears(
					new Date(),
					parseISO(patientDetails.birthDate),
				);
			} catch {
				return null;
			}
	...
	const dateFormatted = useMemo(
		() =>
			formData.appointment_date
				? format(
						parseResponseDate(formData.appointment_date),
	...
	const requiresMinHours =
		effectiveCancellationRules.min_hours_before ??
		DEFAULT_CANCELLATION_RULES.min_hours_before;
	const monthlyLimit =
		effectiveCancellationRules.max_cancellations_month ??
		DEFAULT_CANCELLATION_RULES.max_cancellations_month;
	const isLateCancellation =
		hoursUntilAppointment !== null && hoursUntilAppointment < requiresMinHours;
	const exceedsMonthlyLimit =
		monthlyLimit > 0 && patientMonthCancellations >= monthlyLimit;
	const lateFeeApplies =
		isLateCancellation &&
	...
	const cancellationReason = useMemo(() => {
		...
		if (isLateCancellation && hoursUntilAppointment !== null) {
			reasons.push(
				`fora da antecedência (${Math.max(0, hoursUntilAppointment).toFixed(1)}h < ${requiresMinHours}h)`,
			);
		}
		if (exceedsMonthlyLimit && monthlyLimit > 0) {
			reasons.push(
				`limite mensal atingido (${patientMonthCancellations}/${monthlyLimit})`,
			);
		}
		if (lateFeeApplies) {
			reasons.push(
				`taxa sugerida: R$ ${(effectiveCancellationRules.late_cancellation_fee || 0).toFixed(2)}`,
			);
		}
	...
	const handleWhatsAppConfirm = useCallback(() => {
		...
		const dateShort = formData.appointment_date
			? format(parseResponseDate(formData.appointment_date), "dd 'de' MMMM", {
					locale: ptBR,
				})
			: "";
								...
								<div className="space-y-2">
									...
									{isEditing ? (
										<Input
											...
											onChange={(e) =>
												updateFormField("appointment_date", e.target.value)
											}
											className="h-9"
								...
								<div className="space-y-2">
									...
									{isEditing ? (
										<Input
											...
											onChange={(e) =>
												updateFormField("appointment_time", e.target.value)
											}
											className="h-9"
								...
								<div className="space-y-2">
									...
									{isEditing ? (
										<Select
											...
											onValueChange={(value) =>
												updateFormField("duration", parseInt(value))
											}
										>
											<SelectTrigger
												className="h-9"
								...
								<div className="space-y-2">
									...
									{isEditing ? (
										<Select
											...
											onValueChange={(value) =>
												updateFormField("therapist_id", value)
											}
										>
											<SelectTrigger
												className="h-9"
							...
							<div className="space-y-2">
								...
								{isEditing ? (
									<Select
										...
										onValueChange={(value) =>
											updateFormField("status", value as AppointmentStatus)
										}
									>
										<SelectTrigger
											className="h-9"
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							...
							<strong>{dateFormatted}</strong> às{" "}
						...
						>
							<p className="font-medium mb-2">
								Regras aplicadas neste cancelamento
							</p>
								...
								<p>
									...
									>
										{hoursUntilAppointment === null
											? "N/A"
											: `${Math.max(0, hoursUntilAppointment).toFixed(1)}h`}
									</strong>
								</p>
								<p>
									...
									>
										{monthlyLimit > 0
											? `${patientMonthCancellations}/${monthlyLimit}`
											: `${patientMonthCancellations} (ilimitado)`}
									</strong>
								</p>
								{lateFeeApplies && (
									<p className="text-destructive">
										Taxa de cancelamento tardio sugerida:{" "}
										<strong>
											R${" "}
											{(
												effectiveCancellationRules.late_cancellation_fee || 0
											).toFixed(2)}
										</strong>
									</p>
								)}
							...
							{requiresPolicyOverride && (
								<p className="mt-2 text-xs font-medium text-destructive">
									Cancelamento fora da política. Confirme somente se deseja
									manter o cancelamento manual.
								</p>
							)}
						</div>
					</AlertDialogHeader>
