    ...
    const heatMapData = useMemo(() => {
      ...
      for (const day of days) {
        ...
        const daySlotMap = new Map<
          string,
          { count: number; capacity: number; appointment?: Appointment }
        >();
        ...
        for (const apt of dayAppointments) {
          ...
          for (const slot of timeSlots) {
            ...
            if (slotStart >= aptStart && slotStart < aptEnd) {
              const existing = daySlotMap.get(slot) || {
                count: 0,
                capacity: 4,
              };
        ...
        <div className="overflow-x-auto">
          <div className="min-w-max">
            ...
            <div className="flex border-b border-slate-200 dark:border-slate-800">
              ...
              {Array.from(daysMap.entries()).map(([dayKey]) => {
                ...
                return (
                  <div
                    key={dayKey}
                    className="w-20 shrink-0 text-center py-2 px-1 text-xs font-semibold text-slate-700 dark:text-slate-300 border-r border-slate-200 dark:border-slate-800 first:border-l"
                  >
                    <div className="text-[11px] leading-tight">
                      {format(date, "EEE", { locale: ptBR })}
                    </div>
                    <div className="text-[13px] font-bold">{format(date, "d")}</div>
                  </div>
                );
              })}
            </div>
            ...
            {Array.from(daysMap.entries()).map(([dayKey, slots]) => (
              <div
                key={dayKey}
                className="flex border-b border-slate-100 dark:border-slate-900 last:border-b-0"
              >
                {/* Dia da semana */}
                  ...
                  {slots.map((slot, index) => {
                    ...
                    return (
                      <button
                        ...
                        title={
                          canClick
                            ? `${slot.time} - ${slot.currentCount}/${slot.capacity} ocupados`
                        ...
                        aria-label={
                          canClick
                            ...
                            : slot.availability === "full"
                              ? `Horário ${slot.time} cheio`
                              : slot.availability === "blocked"
                                ? `Horário ${slot.time} bloqueado`
                                : ""
                        }
                      >
                        <span
                          className={cn("text-sm font-medium", getSlotTextColor(slot.availability))}
                        >
                          {slotIcon ? (
