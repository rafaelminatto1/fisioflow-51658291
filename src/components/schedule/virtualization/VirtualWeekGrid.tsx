...
interface ItemData {
  ...
  layoutByAppointmentId: Record<string, { columnIndex: number; columnCount: number }>;
  onTimeSlotClick?: (date: Date, time: string) => void;
  onEditAppointment?: (appointment: Appointment) => void;
  onDeleteAppointment?: (appointment: Appointment) => void;
  onAppointmentReschedule?: (
...
function buildDayOverlapLayout(dayAppointments: Appointment[]) {
  const layoutByAppointmentId: Record<string, { columnIndex: number; columnCount: number }> = {};
    ...
    return (
      <div
        style={{
          ...
          zIndex: appointmentsByTimeSlot[time]?.length ? timeSlots.length - index : 0,
        }}
      ...
      >
        ...
        <div
          className={cn(
            ...
            isHour ? "text-slate-500 dark:text-slate-400 -translate-y-2" : "invisible",
          )}
        >
          {isHour ? time : ""}
        </div>
        ...
        {appointmentsByTimeSlot[time]?.map((apt) => {
          ...
          const height = Math.max(slotHeight * slotCount - verticalInset * 2, slotHeight - 6);
          ...
          const isDraggingThis =
            props.dragState?.isDragging && props.dragState.appointment?.id === apt.id;
          ...
          return (
            <div
              ...
              onDragStart={(e) => (props.handleDragStart || (() => {}))(e, apt)}
      ...
      >
        ...
        {showCurrentTimeIndicator &&
          ...
          (() => {
            ...
            if (
              minutesFromStart < 0 ||
              minutesFromStart > (BUSINESS_HOURS.END - BUSINESS_HOURS.START) * 60
            ) {
              return null;
            }
            ...
            return (
              <div
                ...
                style={{
                  top: `${
                    (minutesFromStart * slotHeight) / BUSINESS_HOURS.DEFAULT_SLOT_DURATION
                  }px`,
                }}
              >
