	...
	return (
		<div className="flex items-center border rounded-lg bg-background overflow-hidden">
			<Button
				...
				disabled={value <= 1} aria-label="Diminuir"
			>
				<Minus className="h-3 w-3" />
			</Button>
			<Input
				id={id} type="number" min={1} max={20} value={value}
				onChange={(e) => {
					const v = parseInt(e.target.value, 10);
					if (!Number.isNaN(v)) onChange(Math.min(20, Math.max(1, v)));
				}}
				className={cn(inputW, "text-center border-0 rounded-none text-sm font-semibold [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none")}
			/>
			<Button
				type="button" size="icon" variant="ghost"
				className={cn(h, "rounded-none shrink-0")}
				onClick={() => onChange(Math.min(20, value + 1))}
				disabled={value >= 20} aria-label="Aumentar"
			>
	...
	return (
		<div className="rounded-xl border bg-muted/30 p-4 space-y-4 animate-in slide-in-from-top-2 duration-200">
				...
				<div className="grid grid-cols-4 gap-x-3 gap-y-2">
					{DAYS_OF_WEEK.map((day) => (
						<div key={day.value} className="flex items-center gap-1.5">
							...
							/>
							<Label htmlFor={`${prefix}-${day.value}`} className="text-xs font-normal cursor-pointer">
								{day.label}
							</Label>
						</div>
					))}
				...
				<div className="flex items-center gap-2 flex-wrap">
					...
					<div className="flex gap-1">
						{CAPACITY_PRESETS.map((p) => (
							<Button
								key={p.value} size="sm" variant={form.max_patients === p.value ? "default" : "outline"}
			...
			<div className="flex gap-2 pt-1">
				...
				<Button
					size="sm" onClick={onSave} disabled={isSaving || form.selectedDays.length === 0}
					className="flex-1"
				>
					{isSaving
						? <><Loader2 className="h-3.5 w-3.5 mr-1.5 animate-spin" />Salvando...</>
						: <><CheckCircle2 className="h-3.5 w-3.5 mr-1.5" />Salvar</>
					}
				</Button>
			</div>
		</div>
		...
		<div className="space-y-3">
			...
			<div className="space-y-1.5">
				{capacityGroups.map((group, index) => {
					...
					const dotColor =
						group.max_patients <= 2 ? "bg-emerald-500"
						: group.max_patients <= 4 ? "bg-amber-500"
						: "bg-rose-500";
						...
						<div key={groupKey}>
							...
							>
								...
								<div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
									<Button
										size="icon" variant="ghost" className="h-7 w-7 hover:text-primary"
										onClick={() => handleDuplicate(group)} title="Duplicar"
									>
										<Copy className="h-3.5 w-3.5" />
									</Button>
									<Button
										size="icon" variant="ghost"
										className={cn("h-7 w-7", isThisEditing ? "text-primary" : "hover:text-amber-600")}
									...
									<Button
										size="icon" variant="ghost" className="h-7 w-7 hover:text-red-500"
										onClick={() => handleDeleteGroup(group)} title="Remover"
									>
			...
			{unconfiguredDays.length > 0 && (
				<div className="flex items-center gap-2 flex-wrap px-1">
					...
					{unconfiguredDays.map((day) => (
						<Badge key={day.value} variant="outline" className="text-xs font-normal text-muted-foreground">
							{day.label}
						</Badge>
					))}
				</div>
			...
			{isAdding ? (
				<CapacityForm
					...
					onSave={handleAdd}
					onCancel={() => { setIsAdding(false); setNewCapacity(EMPTY_FORM); }}
					isSaving={isCreating}
				/>
			) : (
				<Button
