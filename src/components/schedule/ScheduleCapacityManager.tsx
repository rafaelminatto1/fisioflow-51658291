...
import { Trash2, Plus, Minus, Loader2, Users, CheckCircle2, Info, Calendar, Copy, Pencil } from 'lucide-react';
...
type FormData = { selectedDays: string[]; start_time: string; end_time: string; max_patients: number };

const EMPTY_FORM: FormData = { selectedDays: [], start_time: '07:00', end_time: '13:00', max_patients: 3 };

export function ScheduleCapacityManager() {
  const { capacities, capacityGroups, isLoading, createMultipleCapacities, updateCapacityGroup, replaceCapacityGroup, deleteCapacityGroup, organizationId, isCreating, isReplacing, checkConflicts, authError } = useScheduleCapacity();
  ...
  const handleEditSave = async () => {
    ...
    if (start[0] * 60 + start[1] >= end[0] * 60 + end[1]) {
      toast({ title: 'Erro', description: 'Horário de início deve ser anterior ao fim', variant: 'destructive' }); return;
    }
                ...
                <div key={groupKey} className="space-y-2">
                  <div
                    className={cn(
                      ...
                      !isThisEditing && 'hover:scale-[1.01] active:scale-[0.99]'
                    )}
                  >
                    ...
                    <div className="flex items-center gap-3 flex-1 flex-wrap">
                      ...
                      <div className="flex items-center gap-1 border rounded-md bg-background">
                        <Button
                          ...
                          disabled={group.max_patients <= 1} aria-label="Diminuir"
                        >
                          <Minus className="h-4 w-4" />
                        </Button>
                        <Input
                          id={`capacity-group-${groupKey}`}
                          type="number" min={1} max={20} value={displayValue}
                          onChange={(e) => {
                            const raw = e.target.value;
                            if (raw === '') { setEditingCapacityValue((prev) => ({ ...prev, [groupKey]: 1 })); return; }
                            const v = parseInt(raw, 10);
                            if (!Number.isNaN(v)) setEditingCapacityValue((prev) => ({ ...prev, [groupKey]: Math.min(20, Math.max(1, v)) }));
                          }}
                          onBlur={(e) => {
                            const v = parseInt(e.target.value, 10);
                            const safe = Number.isNaN(v) || v < 1 ? 1 : v > 20 ? 20 : v;
                            setEditingCapacityValue((prev) => { const next = { ...prev }; delete next[groupKey]; return next; });
                        ...
                        <Button
                          ...
                          disabled={group.max_patients >= 20} aria-label="Aumentar"
                        >
                  ...
                  {isThisEditing && (
                    <div className="ml-4 p-4 rounded-xl border bg-gradient-to-r from-amber-50 to-yellow-50 dark:from-amber-950/20 dark:to-yellow-950/20 space-y-4 animate-in slide-in-from-top-2 duration-200">
                      <div className="flex items-center justify-between">
                        <h3 className="font-semibold text-base">Editar Configuração {index + 1}</h3>
                        <Button variant="ghost" size="sm" onClick={() => setEditingGroup(null)}>Cancelar</Button>
                      </div>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          {TIME_PRESETS.map((preset) => (
                            <Button
                              ...
                              variant={editData.start_time === preset.start && editData.end_time === preset.end ? "default" : "outline"}
                          ...
                          {DAYS_OF_WEEK.map((day) => (
                            <div key={day.value} className="flex items-center space-x-2">
                              <Checkbox
                                ...
                                checked={editData.selectedDays.includes(day.value)}
                                onCheckedChange={(checked) => {
                                  setEditData((d) => ({
                                    ...d,
                                    selectedDays: checked
                                      ? [...d.selectedDays, day.value]
                                      : d.selectedDays.filter((x) => x !== day.value),
                                  }));
                                }}
                              />
                              <Label htmlFor={`edit-day-${groupKey}-${day.value}`} className="text-sm font-normal cursor-pointer">
                                {day.label}
                              </Label>
                            </div>
                          ))}
                      ...
                      {/* Capacidade */}
                      <div className="space-y-2">
                        <Label className="text-sm font-medium">Pacientes por Horário</Label>
                        <div className="flex items-center gap-1 border rounded-md bg-background w-fit">
                          <Button type="button" size="icon" variant="ghost"
                            ...
                            onClick={() => setEditData((d) => ({ ...d, max_patients: Math.max(1, d.max_patients - 1) }))}
                            disabled={editData.max_patients <= 1}
                          >
                            <Minus className="h-4 w-4" />
                          </Button>
                          <Input type="number" min={1} max={20} value={editData.max_patients}
                            onChange={(e) => {
                              const v = e.target.value === '' ? 1 : parseInt(e.target.value, 10);
                              if (!Number.isNaN(v)) setEditData((d) => ({ ...d, max_patients: Math.min(20, Math.max(1, v)) }));
                            }}
                          ...
                          <Button type="button" size="icon" variant="ghost"
                            ...
                            onClick={() => setEditData((d) => ({ ...d, max_patients: Math.min(20, d.max_patients + 1) }))}
                      ...
                      <Button onClick={handleEditSave} disabled={isReplacing} className="w-full h-10">
                        {isReplacing ? (
                          <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Salvando...</>
                        ) : (
                          <><CheckCircle2 className="h-4 w-4 mr-2" />Salvar Alterações</>
                        )}
                      </Button>
                    </div>
                  )}
