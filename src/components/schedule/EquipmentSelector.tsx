      ...
      {selectedEquipments.length > 0 && (
        <div className="space-y-3 mt-3">
          {selectedEquipments.map((selected) => {
            ...
            return (
              <div
                key={selected.equipmentId}
                className="rounded-[22px] border border-border/70 bg-gradient-to-b from-background to-muted/25 p-4 shadow-[0_18px_32px_-28px_rgba(15,23,42,0.35)]"
              >
                <div className="mb-3 flex items-center gap-2">
                  <Badge
                    variant="secondary"
                    className="rounded-full border border-primary/10 bg-primary/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-primary"
                  >
                  ...
                  {equipment.parameters?.map((param) => (
                    <div key={param.id} className="space-y-1">
                      ...
                      {param.type === "select" ? (
                        <Select
                          value={String(
                            selected.parameters?.[param.id] || param.defaultValue || "",
                          )}
                          onValueChange={(value) =>
                            updateParameter(selected.equipmentId, param.id, value)
                          }
                          disabled={disabled}
                        >
