...
const getSupportedFormat = (formats: string[]): string => {
	...
	if (formats.includes("avif")) {
		img.src =
			"data:image/avif;base64,";
...
export const OptimizedImageLoader = memo(
	({
		...
		const getOptimizedSrc = useCallback(
			(originalSrc: string): string => {
				try {
					...
					if (
						url.hostname.includes(legacyStorageHost) ||
		...
		const blurPlaceholder =
			placeholder || generateBlurPlaceholder(width || 100, height || 100);
