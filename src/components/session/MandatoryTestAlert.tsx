...
export const MandatoryTestAlert = memo(
	({ tests, onResolve }: MandatoryTestAlertProps) => {
		...
		return (
			<Alert
				...
				className={`mb-6 ${isCritical ? "border-red-500 bg-red-50 dark:bg-red-900/10" : "border-amber-500 bg-amber-50 dark:bg-amber-900/10"}`}
			>
				<div className="flex items-start gap-4">
					...
					<div className="flex-1">
						<AlertTitle
							className={`text-base font-semibold ${isCritical ? "text-red-700 dark:text-red-400" : "text-amber-700 dark:text-amber-400"}`}
						>
							{isCritical
								? "Testes Obrigatórios Pendentes (Bloqueia Salvamento)"
								...
								{pendingTests.map((test) => (
									...
									>
										<span>
											...
											{test.critical && (
												<span className="ml-2 text-xs bg-red-100 text-red-800 px-1.5 py-0.5 rounded border border-red-200">
													OBRIGATÓRIO
												</span>
											)}
										</span>
