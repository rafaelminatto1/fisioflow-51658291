...
export const KinoveaStudio: React.FC<KinoveaStudioProps> = ({
  ...
  // Tool + display state
  const [activeTool, setActiveTool] = useState<"none" | "goniometer" | "trajectory" | "jump">(
    "none",
  );
  ...
  // Measurement data
  const [goniometerPoints, setGoniometerPoints] = useState<Point[]>([
    { x: 400, y: 300 },
    { x: 300, y: 200 },
    { x: 300, y: 400 },
  ]);
  ...
  const addTrajectory = useCallback(
    (keypointIdx: number | null) => {
      const color = TRAJ_COLORS[trackedTrajs.length % TRAJ_COLORS.length];
      const label =
        keypointIdx != null ? KP_NAMES[keypointIdx] : `Manual ${trackedTrajs.length + 1}`;
  ...
  const handleCanvasClick = useCallback(
    (e: any) => {
      ...
      setTrackedTrajs((prev) => {
        ...
        arr[arr.length - 1] = {
          ...last,
          points: [...last.points, { x: pos.x, y: pos.y, frame: currentFrame }],
        };
        return arr;
      });
    },
          ...
          <Card className="lg:col-span-3 relative overflow-hidden bg-black border-2 border-primary/20 rounded-3xl shadow-2xl h-[600px]">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-center text-white/80 px-6">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-white/60">
                      Envie um video gravado para usar goniometro, trajetoria, salto e eventos da
                      marcha.
                    </p>
                  </div>
                  <Button
                ...
                <Layer>
                        ...
                        <Group opacity={0.85}>
                          {conn.map(([a, b], i) => {
                            const pa = poseKeypoints[a],
                              pb = poseKeypoints[b];
                            if (!pa || !pb || (pa.score ?? 0) < 0.4 || (pb.score ?? 0) < 0.4)
                              return null;
                            return (
                              <Line
                                key={i}
                  ...
                  {activeTool === "goniometer" && (
                    <Group>
                      <Line
                        points={[
                          goniometerPoints[1].x,
                          goniometerPoints[1].y,
                          goniometerPoints[0].x,
                          goniometerPoints[0].y,
                          goniometerPoints[2].x,
                          goniometerPoints[2].y,
                        ]}
                        stroke="#ff00ff"
                        strokeWidth={3}
                      />
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-black/80 backdrop-blur-2xl p-2 rounded-2xl border border-white/10 z-30">
              ...
              <Button
                ...
                size="sm"
                onClick={() => setActiveTool(activeTool === "jump" ? "none" : "jump")}
                className="rounded-xl gap-2 text-xs font-black"
              >
                ...
                <div className="space-y-2">
                  <div>
                    ...
                    <Input
                      ...
                      value={patientMass ?? ""}
                      onChange={(e) =>
                        setPatientMass(e.target.value ? Number(e.target.value) : null)
                      }
                    />
                  </div>
                  <div>
            ...
            <Card className="border-none shadow-sm bg-blue-500/5">
              <CardContent className="p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 gap-1.5">
                  ...
                  >
                    {jumpEvents.takeoff != null
                      ? `✅ DECOLAGEM · Frame ${jumpEvents.takeoff}`
                      : "1. MARCAR DECOLAGEM"}
                  </Button>
                  <Button
                    size="sm"
                  ...
                  >
                    {jumpEvents.landing != null
                      ? `✅ ATERRISSAGEM · Frame ${jumpEvents.landing}`
                      : "2. MARCAR ATERRISSAGEM"}
                  </Button>
                  <Button
                    size="sm"
                    variant="ghost"
                  ...
                  <div className="pt-2 border-t border-blue-500/10 space-y-2">
                    <div>
                      <p className="text-3xl font-black text-blue-600">
                        {jumpMetrics.height}
                        <span className="text-sm ml-1 font-bold">cm</span>
                      </p>
                    ...
                    {jumpMetrics.peakPower != null &&
                      (() => {
                        ...
                        return (
                          <div
                            className={`p-2 rounded-lg ${z.color === "text-green-500" ? "bg-green-500/10" : z.color === "text-blue-400" ? "bg-blue-400/10" : "bg-muted/10"}`}
                          >
                            <p className={`text-lg font-black ${z.color}`}>
                              {pwr.toFixed(0)}
                              <span className="text-[10px] ml-1 font-bold">W</span>
                            </p>
                ...
                <div className="grid grid-cols-2 gap-1">
                  <Button
                    ...
                    onClick={() =>
                      setGaitEvents((p) => [
                        ...p,
                        { type: "contact", frame: currentFrame, side: "R" },
                      ])
                    }
                  >
                    CONTATO D
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setGaitEvents((p) => [
                        ...p,
                        { type: "contact", frame: currentFrame, side: "L" },
                      ])
                    }
                  >
                    CONTATO E
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setGaitEvents((p) => [
                        ...p,
                        { type: "toe-off", frame: currentFrame, side: "R" },
                      ])
                    }
                  >
                    IMPULSO D
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setGaitEvents((p) => [
                        ...p,
                        { type: "toe-off", frame: currentFrame, side: "L" },
                      ])
                    }
                  >
                    IMPULSO E
                      ...
                      <div className="space-y-2 pt-2 border-t border-green-500/10">
                        ...
                        <div className="flex justify-between items-end">
                          ...
                          <div className="text-right">
                            <span className={`text-xl font-black ${cad.color}`}>
                              {gaitMetrics.cadence}
                            </span>
                        ...
                        <div className={`p-2 rounded-lg ${osc.bg}`}>
                          <div className="flex justify-between items-end">
                            <div>
                              ...
                              <span className={`text-[9px] font-medium ${osc.color}`}>
                                {osc.label}
                              </span>
                            </div>
                            <div className="text-right">
                              ...
                              return (
                                <button
                                  ...
                                  onClick={() =>
                                    tracked
                                      ? setTrackedTrajs((p) =>
                                          p.filter((t) => t.keypointIdx !== kpIdx),
                                        )
                                      : addTrajectory(kpIdx)
                                  }
                                >
                      ...
                      {trackedTrajs.map((t, i) => (
                        <div key={i} className="flex items-center justify-between">
                          ...
                          <button
                            ...
                            onClick={() => setTrackedTrajs((p) => p.filter((_, j) => j !== i))}
                          >
                            ✕
                          </button>
                        </div>
