...
interface JumpAnalysisStudioProps {
  onDataUpdate?: (data: any) => void;
}

export const JumpAnalysisStudio: React.FC<JumpAnalysisStudioProps> = ({ onDataUpdate }) => {
  ...
  const jumpMetrics = useMemo(
    () => calcJumpMetrics(jumpEvents.takeoff, jumpEvents.landing, fps, patientMass),
    [jumpEvents, fps, patientMass],
  );

  React.useEffect(() => {
    if (jumpMetrics) {
      onDataUpdate?.({ metrics: jumpMetrics, events: jumpEvents });
    }
  }, [jumpMetrics, jumpEvents, onDataUpdate]);
              ...
              <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-3 bg-slate-950/90 px-6 text-center">
                ...
                <div>
                  <p className="text-sm font-black uppercase tracking-widest text-white">
                    Envie um vídeo de salto
                  </p>
                  <p className="mt-2 max-w-md text-sm text-white/70">
                    Use um vídeo gravado em câmera lenta sempre que possível. O fluxo web agora é
                    focado em upload e análise frame a frame.
                  </p>
                </div>
              </div>
            )}
