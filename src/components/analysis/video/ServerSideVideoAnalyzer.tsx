...
export const ServerSideVideoAnalyzer: React.FC<
	ServerSideVideoAnalyzerProps
> = ({ exerciseName = "Exercício Livre", onAnalysisComplete }) => {
	...
	return (
		<div className="space-y-6">
				...
				<div className="flex flex-col items-center justify-center text-center space-y-4">
					...
					<div className="space-y-2">
						...
						<p className="text-sm text-muted-foreground max-w-sm mx-auto">
							Envie um vídeo do exercício para análise automática de técnica,
							contagem e feedback via Gemini 1.5 Pro.
						</p>
					</div>
			...
			{result && (
				<MotionCard
					variant="glass"
					className="p-0 overflow-hidden animate-in fade-in slide-in-from-bottom-4"
				>
						...
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-sm text-muted-foreground">
									Baseado na visão computacional do Gemini
								</p>
							</div>
									...
									<div className="p-3 rounded-lg bg-background border border-border">
										...
										<div className="flex items-center gap-2 mt-1">
											...
											<span className="font-medium">
												{result.isValidExercise ? "Válido" : "Inválido"}
											</span>
										</div>
									</div>
								</div>
							...
							{result.errors.length > 0 ? (
								<ul className="space-y-2">
									{result.errors.map((error, idx) => (
										<li
											key={idx}
											className="flex items-start gap-2 text-sm p-2 rounded-md bg-red-500/5 border border-red-500/10 text-red-700 dark:text-red-400"
										>
