...
export const GoniometerPanel: React.FC<DynamicGoniometerProps> = ({
  ...
  const cycleJoint = (idx: number) => {
    ...
    const next =
      current === null
        ? lowerBodyJoints[0]
        : lowerBodyJoints[(lowerBodyJoints.indexOf(current) + 1) % lowerBodyJoints.length];
  ...
  return (
    ...
    >
      <Card className="border-none shadow-2xl bg-slate-900/40 border border-white/5 overflow-hidden">
        <CardContent className="p-4 space-y-4">
          ...
          <div className="flex gap-2">
            ...
            >
              {isRecording ? (
                <>
                  <Square className="w-3 h-3 mr-2 mb-0.5 fill-current" /> Parar
                </>
              ) : (
                <>
                  <Play className="w-3 h-3 mr-2 mb-0.5 fill-current" /> Gravar Curva
                </>
              )}
            </Button>
            <Button
              size="icon"
              variant="outline"
              className="h-9 w-9 border-white/5 hover:bg-white/5"
            ...
            {angleHistory.length > 0 && (
              ...
              >
                <div
                  ...
                  className="h-40 w-full mt-2 bg-slate-950/50 rounded-2xl border border-white/5 p-4 relative group/chart min-w-0"
                >
                  <div className="absolute top-2 left-3 flex items-center gap-2 pointer-events-none">
                    <Activity className="h-3 w-3 text-orange-500" />
                    <span className="text-[8px] font-black text-white/40 uppercase tracking-widest">
                      Série Temporal (Frames)
                    </span>
                  </div>
                  {canRenderChart ? (
                    <LineChart
