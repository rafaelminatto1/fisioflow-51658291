            ...
            <Card className="border-none shadow-2xl bg-emerald-950/20 backdrop-blur-xl border border-emerald-500/10 mt-4 overflow-hidden">
                <CardContent className="p-4 space-y-4">
                    <div className="flex items-center justify-between">
                        ...
                        <Badge variant="outline" className="text-[8px] uppercase font-black text-emerald-400 border-emerald-500/20 bg-emerald-500/5 px-1.5 py-0">
                    ...
                    <div className="space-y-3">
                        <div className="flex justify-between items-center">
                            <p className="text-[9px] font-bold text-emerald-400/80 uppercase tracking-widest">{currentTest.desc}</p>
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                            <div className="space-y-1.5">
                                <label className="text-[8px] font-black uppercase tracking-widest text-muted-foreground/60 px-1">Esquerda ({currentTest.unit})</label>
                                <input 
                                    type="number" 
                            ...
                            <div className="space-y-1.5">
                                <label className="text-[8px] font-black uppercase tracking-widest text-muted-foreground/60 px-1">Direita ({currentTest.unit})</label>
                                <input 
                                    type="number" 
                                    className="flex h-10 w-full rounded-xl border border-white/5 bg-white/5 px-3 py-2 text-xs font-bold text-white transition-all focus:bg-white/10 focus:border-emerald-500/30 focus-visible:outline-none"
                                    value={rightVal}
                        ...
                        {asymmetry !== null && (
                            ...
                            >
                                ...
                                <p className="text-[9px] font-black uppercase tracking-tighter">
                                    {Number(asymmetry) > 15 ? 'ALERTA: CRÍTICO - INTERVIR IMEDIATAMENTE' : 'CONFORME: MARGEM DE SEGURANÇA OK'}
                                </p>
                            </motion.div>
                        )}
                    </AnimatePresence>
