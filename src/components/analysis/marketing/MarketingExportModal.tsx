...
const MarketingExportModal: React.FC<MarketingExportModalProps> = ({
  ...
  const [captionType, setCaptionType] = useState<"technical" | "motivational" | "educational">(
    "technical",
  );
      ...
      checkMarketingConsent(patientId).then((allowed) => {
        ...
        if (!allowed) {
          toast({
            ...
            description:
              "Este paciente não possui autorização vigente para uso de imagem em marketing.",
          });
        }
      });
  ...
  const handleVideoGenerated = async (blob: Blob) => {
    try {
      ...
      toast({
        ...
        description: "Não foi possível registrar o export.",
      });
    }
  };
      ...
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              O paciente <strong>{patientName}</strong> não possui Termo de Consentimento para Uso
              de Imagem (Marketing) ativo.
            </DialogDescription>
          </DialogHeader>
          <div className="bg-red-50 p-4 rounded text-sm text-red-800">
            Para habilitar esta função, é necessário coletar a assinatura de um novo termo
            específico para divulgação.
          </div>
          <DialogFooter>
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Comparativo Antes/Depois com overlay e legenda automática.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                <Label htmlFor="anon-mode" className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground">
                    Aplica blur na região da face (automático)
                  </span>
                </Label>
            ...
            <div className="space-y-4">
              ...
              <Tabs
                value={captionType}
                onValueChange={(v) =>
                  setCaptionType(v as "technical" | "motivational" | "educational")
                }
              >
                <TabsList className="w-full">
                  ...
                  <TabsTrigger value="motivational" className="flex-1">
                    Motivacional
                  </TabsTrigger>
                  <TabsTrigger value="educational" className="flex-1">
                    Educativo
                  </TabsTrigger>
                </TabsList>
              </Tabs>
