...
export function BiomechanicsEvidencePanel({
	mode,
}: {
	mode: BiomechanicsEvidenceMode;
}) {
	...
	const resolvedArticles = useMemo(
		() =>
			protocol.articles.map((article) => {
				const wikiPage =
					wikiPages.find((page) =>
						matchesEvidenceArticle(page.title, page.tags ?? [], article),
					) ?? null;
				const knowledgeArticle =
					knowledgeArticles.find(
						(item) =>
							normalizeEvidenceText(item.title) ===
								normalizeEvidenceText(article.title) ||
	...
	const createWikiEntry = useMutation({
		mutationFn: async (article: BiomechanicsEvidenceArticle) => {
			...
			const existing =
				resolvedArticles.find((item) => item.article.id === article.id) ?? null;
		...
		onError: (error) => {
			toast.error(
				error instanceof Error
					? error.message
					: "Não foi possível adicionar o artigo à wiki.",
			);
		},
	});
					...
					>
						...
						<div className="mt-4 flex flex-wrap gap-2">
							...
							{wikiPage ? (
								<Button
									size="sm"
									onClick={() => navigate(`/wiki/${wikiPage.slug}`)}
								>
