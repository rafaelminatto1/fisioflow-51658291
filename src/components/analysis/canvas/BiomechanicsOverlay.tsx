...
export const BiomechanicsOverlay: React.FC<BiomechanicsOverlayProps> = ({
	...
	}, [landmarks, width, height, showGrid, showSkeleton, showAngles, showSilhouette, silhouetteType, showPlumbLine, highlightPoints, opacity]);
...
function drawAngleOverlay(ctx: CanvasRenderingContext2D, lms: UnifiedLandmark[], w: number, h: number, type: string) {
	ctx.save();
	ctx.fillStyle = "white";
	...
	const drawAngle = (p1: number, p2: number, p3: number, label: string) => {
		const a = lms[p1], b = lms[p2], c = lms[p3];
