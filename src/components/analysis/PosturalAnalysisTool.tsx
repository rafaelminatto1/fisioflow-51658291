...
export const PosturalAnalysisTool: React.FC<PosturalAnalysisToolProps> = ({
  ...
  const [capturedImages, setCapturedImages] = useState<{url: string, type: string}[]>([]);
    ...
    if (video && video.readyState === 4) {
      ...
      if (isAiActive) {
        ...
        const mockLandmarks = {
          ...
          shoulders: [ { x: w * 0.4, y: h * 0.35 }, { x: w * 0.6, y: h * 0.35 } ],
          hips: [ { x: w * 0.42, y: h * 0.6 }, { x: w * 0.58, y: h * 0.6 } ],
          knees: [ { x: w * 0.43, y: h * 0.8 }, { x: w * 0.57, y: h * 0.8 } ],
          ankles: [ { x: w * 0.43, y: h * 0.95 }, { x: w * 0.57, y: h * 0.95 } ]
        };
        ...
        // Draw Joints
        [mockLandmarks.head, ...mockLandmarks.shoulders, ...mockLandmarks.hips, ...mockLandmarks.knees, ...mockLandmarks.ankles].forEach(p => {
          ctx.beginPath();
        ...
        if (showAngles) {
          ...
          ctx.fillText("Alinhamento: 98%", mockLandmarks.head.x + 20, mockLandmarks.head.y);
          ctx.fillText("Nível: 0.5°", mockLandmarks.shoulders[1].x + 10, mockLandmarks.shoulders[1].y);
        }
      }
    }
          ...
          <Card className="lg:col-span-3 relative overflow-hidden bg-black border-2 border-primary/20 rounded-2xl shadow-2xl group">
            {cameraError ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-white p-6 text-center">
                ...
                <Button variant="outline" className="mt-6 border-white/20" onClick={() => window.location.reload()}>
                  Tentar Novamente
                </Button>
              </div>
            ) : (
              <>
                <Webcam
                  ...
                  onUserMediaError={() => setCameraError("Não foi possível acessar a webcam. Verifique as permissões.")}
                ...
                <div className="absolute top-4 left-4 flex flex-col gap-2">
                  ...
                  {patientName && (
                    <Badge variant="outline" className="bg-black/40 backdrop-blur-md text-white border-white/20 font-medium">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button 
                        ...
                        className="rounded-xl h-12 w-12"
                      >
                        <RefreshCcw className={cn("h-5 w-5", isAiActive && "animate-spin-slow")} />
                      </Button>
                    </TooltipTrigger>
            ...
            <Card className="border-none shadow-sm bg-card/50">
              <CardContent className="p-6 space-y-6">
                <div className="space-y-4">
                  <h4 className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Configurações AI</h4>
                  
                  <div className="flex items-center justify-between">
                    <Label htmlFor="ai-toggle" className="text-sm font-medium">Tracking de Pontos</Label>
                    <Switch id="ai-toggle" checked={isAiActive} onCheckedChange={setIsAiActive} />
                  </div>

                  <div className="flex items-center justify-between">
                    <Label htmlFor="angle-toggle" className="text-sm font-medium">Cálculo de Ângulos</Label>
                ...
                <div className="space-y-4">
                  <h4 className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Vistas Sugeridas</h4>
                  <div className="grid grid-cols-2 gap-2">
                    <Button variant="outline" size="sm" className="text-[10px] h-8 uppercase" onClick={() => captureSnapshot("Anterior")}>Anterior</Button>
                    <Button variant="outline" size="sm" className="text-[10px] h-8 uppercase" onClick={() => captureSnapshot("Lateral D")}>Lateral D</Button>
                    <Button variant="outline" size="sm" className="text-[10px] h-8 uppercase" onClick={() => captureSnapshot("Lateral E")}>Lateral E</Button>
                    <Button variant="outline" size="sm" className="text-[10px] h-8 uppercase" onClick={() => captureSnapshot("Posterior")}>Posterior</Button>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <ScrollArea className="flex-1">
                <div className="grid grid-cols-1 gap-3 pr-4">
                  {capturedImages.map((img, idx) => (
                    <div key={idx} className="relative group rounded-xl overflow-hidden border bg-background shadow-sm animate-in zoom-in-95">
                      <img src={img.url} alt={img.type} className="w-full h-24 object-cover" />
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                        <Button size="icon" variant="secondary" className="h-8 w-8"><Maximize2 className="h-4 w-4" /></Button>
                        <Button size="icon" variant="destructive" className="h-8 w-8" onClick={() => setCapturedImages(prev => prev.filter((_, i) => i !== idx))}><Trash2 className="h-4 w-4" /></Button>
                      </div>
                      <div className="absolute bottom-1 left-1">
                        <Badge className="bg-primary text-[10px] px-1.5 py-0 h-4">{img.type}</Badge>
                      </div>
                    </div>
                  ))}
                  {capturedImages.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-12 border-2 border-dashed rounded-xl text-muted-foreground/40">
                      ...
                      <p className="text-[10px] uppercase font-bold">Nenhuma captura</p>
                    </div>
                  )}
                </div>
              </ScrollArea>
