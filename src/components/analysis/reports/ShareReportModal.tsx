  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      await downloadReactPdfDocument({
        ...
        loadDocument: () => import("./ClinicalReportPDF").then((module) => module.default),
        props: {
          report,
          patientName,
          professionalName,
        ...
        <Tabs defaultValue="pdf" className="w-full">
          <TabsList className="grid w-full grid-cols-3">
            <TabsTrigger value="pdf" className="flex gap-2">
              <FileText className="w-4 h-4" /> PDF Clínico
            </TabsTrigger>
            <TabsTrigger value="doctor" className="flex gap-2">
              <Stethoscope className="w-4 h-4" /> Carta ao Médico
            </TabsTrigger>
          ...
          <TabsContent value="pdf" className="space-y-4 py-4">
            <div className="bg-slate-100 p-4 rounded text-sm text-slate-600">
              <p className="mb-2">
                <strong>Conteúdo:</strong> Relatório completo com dados técnicos, resumo ao
                paciente, gráficos e exercícios.
              </p>
              <p>
                <strong>Ideal para:</strong> Registro em prontuário, impressão ou envio por e-mail.
              </p>
            </div>
          ...
          <TabsContent value="whatsapp" className="space-y-4 py-4">
            <div className="bg-green-50 p-3 rounded border border-green-200 text-green-800 text-sm mb-2">
              Atenção: A FisioFlow não envia mensagens automaticamente. Copie o texto abaixo e cole
              no seu WhatsApp.
            </div>
            <div className="space-y-2">
