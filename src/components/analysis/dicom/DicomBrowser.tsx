...
const getTagValue = (
	obj: Record<string, { Value?: unknown[] }>,
	tag: string,
): string => {
			...
			<div className="flex flex-col h-full">
				<div className="p-2 border-b flex items-center gap-2 bg-slate-100">
					...
					<span className="font-semibold">
						{getTagValue(selectedStudy, "00100010")}
					</span>
					<span className="text-muted-foreground text-sm">
						{getTagValue(selectedStudy, "00081030")}
					</span>
				</div>
			...
			<div className="flex justify-between items-center">
				...
				<div className="flex gap-2">
					<Button
						variant="outline"
						size="sm"
						onClick={view === "series" ? () => loadStudies() : loadStudies}
						disabled={loading}
					>
							...
							{studies.map((study, i) => (
								...
								>
									...
									<TableCell className="text-right">
										<FolderOpen className="w-4 h-4 ml-auto text-blue-500" />
									</TableCell>
								</TableRow>
							))}
						</TableBody>
							...
							>
								<CardContent className="p-4 flex flex-col items-center text-center gap-2">
									...
									<div>
										...
										<p className="text-xs mt-1 bg-slate-100 px-2 py-0.5 rounded-full inline-block">
											{instances} imgs
										</p>
									</div>
								</CardContent>
