...
const LiveBiofeedbackSession: React.FC<LiveBiofeedbackSessionProps> = ({
	onClose,
}) => {
	const { videoRef, canvasRef, startCamera, stopCamera, metrics, error } =
		useLivePoseAnalysis();
	...
	const handleRecordToggle = () => {
		if (isRecording) {
			...
			toast({
				...
				description: "Capturando 10 segundos...",
			});
	...
	const setupStopRecording = () => {
		...
		toast({
			title: "Clip Salvo",
			description: "Vídeo salvo na galeria do paciente.",
		});
			...
			<div className="w-full max-w-6xl h-full flex flex-col">
				...
				<div className="flex-1 grid grid-cols-1 lg:grid-cols-4 gap-6 min-h-0">
					...
					<div className="lg:col-span-1 space-y-4 overflow-y-auto">
						{/* Confidence Check */}
						<Card
							className={`border-0 ${metrics && metrics.confidence > 60 ? "bg-green-900/20 border-green-500/30" : "bg-red-900/20 border-red-500/30"} border`}
						>
							<CardContent className="p-4 flex items-center gap-3">
								...
								<div>
									...
									<p className="text-xs text-gray-500">
										{metrics && metrics.confidence > 60
											? "Qualidade ideal para análise."
											: "Ajuste a iluminação ou posição."}
									</p>
								</div>
							</CardContent>
						</Card>
						...
						{metrics && metrics.confidence > 60 && (
							<div className="space-y-3">
								...
								<div className="text-[10px] text-slate-500 text-right px-1">
									Meta: Manter alinhado com quadril
								</div>
								...
								/>
								<div className="text-[10px] text-slate-500 text-right px-1">
									Meta: Peito alto, tronco estável
								</div>
