...
import {
	DynamicAnalysis,
	GaitMetrics,
	OverheadSquatMetrics,
	RombergMetrics,
} from "@/types/analysis/schemas";
...
const DynamicAnalysisViewer: React.FC<DynamicAnalysisViewerProps> = ({
	data,
}) => {
	...
	if (isDynamicCompare(data)) {
		...
		return (
			<div className="space-y-4">
				<Card>
					<CardHeader>
						<CardTitle>Metodologia & Qualidade</CardTitle>
					</CardHeader>
					<CardContent className="text-sm space-y-2">
						...
						<div className="flex justify-between">
							...
							<span
								className={`font-bold ${metrics.quality.analysis_confidence_overall_0_100 > 80 ? "text-green-600" : "text-amber-600"}`}
							>
						...
						<div className="flex justify-between">
							...
							<span>
								{metrics.synchronization.mode} (Ref:{" "}
								{metrics.synchronization.anchor.anchor_type})
							</span>
						</div>
					</CardContent>
				</Card>
	...
	if (legacyData?.trial_type === "GAIT")
		return <GaitDashboard data={legacyData as GaitMetrics} />;
	if (legacyData?.trial_type === "SQUAT_OVERHEAD")
		return <SquatDashboard data={legacyData as OverheadSquatMetrics} />;
	if (legacyData?.trial_type === "ROMBERG")
		return <RombergDashboard data={legacyData as RombergMetrics} />;
...
const GaitDashboard: React.FC<{ data: GaitMetrics }> = ({ data }) => {
	const symmetryData = [
		...
		{
			name: "Passo (Comp.)",
			value: data.summary.symmetry_step_length_pct || 0,
		},
	];
	...
	const valgusData = [
		{
			name: "Esq",
			value: data.summary.knee_valgus_deg_peak_L || 0,
			fill: "#3b82f6",
		},
		{
			name: "Dir",
			value: data.summary.knee_valgus_deg_peak_R || 0,
			fill: "#ef4444",
		},
	];

	const timeSeriesData =
		data.timeseries?.samples.map((s) => ({
			time: s.t_ms / 1000,
	...
	return (
		<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
			<Card>
				<CardHeader>
					<CardTitle className="text-sm">Parâmetros Espaço-Temporais</CardTitle>
				</CardHeader>
				<CardContent className="grid grid-cols-2 gap-4">
					<div>
						<p className="text-xs text-muted-foreground">Cadência</p>
						<p className="text-2xl font-bold">
							{data.summary.cadence_spm?.toFixed(0) || "-"}
					...
					<div>
						...
						<p className="text-2xl font-bold">
							{((data.summary.gait_speed_mm_s || 0) / 1000).toFixed(2) || "-"}
							<span className="text-xs font-normal text-muted-foreground">
								{" "}
								m/s
							</span>
					...
					<div>
						...
						<p className="text-2xl font-bold">
							{data.summary.double_support_pct?.toFixed(1) || "-"}
							<span className="text-xs font-normal text-muted-foreground">
								{" "}
								%
							</span>
			...
			<Card>
				<CardHeader>
					<CardTitle className="text-sm">Índice de Simetria (%)</CardTitle>
				</CardHeader>
			...
			<Card>
				<CardHeader>
					<CardTitle className="text-sm">Valgo Dinâmico (Pico)</CardTitle>
				</CardHeader>
				<CardContent className="h-40">
					<ResponsiveContainer width="100%" height="100%">
			...
			{timeSeriesData.length > 0 && (
				<Card className="md:col-span-2 lg:col-span-3">
					<CardHeader>
						<CardTitle className="text-sm">Cinemática Tóraco-Pélvica</CardTitle>
					</CardHeader>
...
const SquatDashboard: React.FC<{ data: OverheadSquatMetrics }> = ({ data }) => {
	...
	return (
		<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
			<Card>
				<CardHeader>
					<CardTitle className="text-sm">Scores Médios</CardTitle>
				</CardHeader>
				<CardContent className="grid grid-cols-2 gap-4">
					<div>
						<p className="text-xs text-muted-foreground">Profundidade</p>
						<p className="text-2xl font-bold">
							{data.summary.depth_score_mean_0_100?.toFixed(0)}
						</p>
					</div>
					<div>
						<p className="text-xs text-muted-foreground">Fluidez</p>
						<p className="text-2xl font-bold">
							{data.summary.smoothness_mean_0_100?.toFixed(0)}
						</p>
					</div>
				...
				<CardContent className="h-60">
					<ResponsiveContainer width="100%" height="100%">
						<BarChart data={repData}>
							...
							<XAxis
								dataKey="rep"
								label={{ value: "Rep", position: "insideBottom", offset: -5 }}
							/>
							<YAxis unit="°" />
							<Tooltip />
							<Legend />
...
const RombergDashboard: React.FC<{ data: RombergMetrics }> = ({ data }) => {
	const swayData = [
		{
			name: "Olhos Abertos",
			area: data.summary.sway_area_mm2_EO || 0,
			velocity: data.summary.sway_velocity_mm_s_EO || 0,
		},
		{
			name: "Olhos Fechados",
			area: data.summary.sway_area_mm2_EC || 0,
			velocity: data.summary.sway_velocity_mm_s_EC || 0,
		},
	];
	...
	return (
		<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
			<Card>
				<CardHeader>
					<CardTitle className="text-sm">Índice de Romberg</CardTitle>
				</CardHeader>
				<CardContent className="flex flex-col gap-4">
					<div>
						<p className="text-xs text-muted-foreground">Razão (EC / EO)</p>
						<p className="text-4xl font-bold">
							{data.summary.romberg_ratio_EC_over_EO?.toFixed(2) || "-"}
						</p>
						<p className="text-xs text-muted-foreground">
							Valores &gt; 1.0 indicam maior oscilação sem visão.
						</p>
					</div>
				</CardContent>
			</Card>
			...
			<Card className="md:col-span-2">
				<CardHeader>
					<CardTitle className="text-sm">Área de Oscilação (mm²)</CardTitle>
				</CardHeader>
				<CardContent className="h-40">
					<ResponsiveContainer width="100%" height="100%">
					...
					<ResponsiveContainer width="100%" height="100%">
						<ScatterChart>
							...
							<Scatter
								...
								data={samplesEC}
								fill="#ef4444"
								line={{ stroke: "#ef4444", strokeWidth: 1 }}
								shape="cross"
							/>
						</ScatterChart>
					</ResponsiveContainer>
