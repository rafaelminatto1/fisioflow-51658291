...
const DynamicAnalysisViewer = React.lazy(
	() => import("./DynamicAnalysisViewer"),
);
...
import {
	processVideo,
	VideoAnalysisFrame,
} from "@/services/ai/videoPoseService";
import { analyzeDynamicComparison } from "@/utils/dynamicMetricsEngine";
import {
	generateClinicalReport,
	AIAnalysisResult,
} from "@/services/ai/clinicalAnalysisService";
...
const DynamicComparisonPage = () => {
	...
	const [comparisonResult, setComparisonResult] =
		useState<DynamicCompareMetrics | null>(null);
	...
	const runAnalysis = async () => {
		...
		try {
			...
			const resB = await processVideo(videoElB, (p) =>
				setProgress(50 + p * 50),
			);
			setLandmarksB(resB);
	...
	const handleGenerateAI = async () => {
		...
		toast({
			title: "Gerando Laudo...",
			description: "A IA está analisando a biomecânica.",
		});
		...
		try {
			...
			toast({
				title: "Laudo Gerado",
				description: "Análise clínica concluída com sucesso.",
			});
		} catch {
			toast({
				title: "Erro",
				description: "Falha ao conectar com serviço de IA.",
				variant: "destructive",
			});
		} finally {
		...
		<div className="container mx-auto p-6 max-w-7xl h-full flex flex-col">
			...
			{step === "upload" && (
				<Card className="max-w-xl mx-auto mt-10">
					<CardHeader>
						<CardTitle>Carregar Vídeos (Antes vs Depois)</CardTitle>
					</CardHeader>
									...
									<CardContent className="flex items-center justify-between p-6">
										<div>
											<h3 className="font-bold text-blue-900 mb-1">
												Assistente PhysioScience Master AI
											</h3>
											<p className="text-sm text-blue-700">
												Gere um laudo clínico automático comparando a evolução
												biomecânica.
											</p>
										</div>
										<Button
	...
	return (
		<Card>
			...
			<CardContent>
				...
				<div
					className={`mt-2 text-xs font-bold ${improved ? "text-green-600" : "text-amber-600"} flex items-center justify-center bg-slate-100 py-1 rounded`}
				>
					{diff !== undefined
						? (diff! > 0 ? "+" : "") + diff + " " + unit
						: "-"}
