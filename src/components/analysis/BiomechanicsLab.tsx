...
export const BiomechanicsLab: React.FC<BiomechanicsLabProps> = ({ patientId, patientName }) => {
    ...
    <div className="flex flex-col gap-6 p-2 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Análise postural e de movimento para {patientName}
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Button
            variant="outline"
            onClick={() => setActiveTab("new-dynamic")}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          {isLoading ? (
            ...
            <Card className="border-dashed">
              <CardContent className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-muted-foreground max-w-xs mx-auto mt-2">
                  O histórico do laboratório está vazio. Inicie uma nova análise postural ou
                  dinâmica.
                </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {assessments.map((item) => (
                <Card
                  key={item.id}
                  className="overflow-hidden hover:shadow-lg transition-all group border-primary/10"
                >
                  <div className="aspect-[4/3] relative bg-black overflow-hidden">
                    ...
                    <div className="absolute top-3 right-3">
                      <Badge className="bg-primary/90 text-[10px] uppercase">
                        {item.type.replace("_", " ")}
                      </Badge>
                    </div>
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex gap-1">
                        <Badge variant="secondary" className="text-[10px]">
                          {item.analysisData.landmarks?.length || 0} pts
                        </Badge>
                        {item.analysisData.angles && (
                          <Badge variant="secondary" className="text-[10px]">
                            {Object.keys(item.analysisData.angles).length} ângulos
                          </Badge>
                        )}
                      </div>
        ...
        <TabsContent value="new-posture" className="mt-6 min-h-[700px]">
          <PosturalAnalysisTool patientName={patientName} onCapture={handleCapture} />
        </TabsContent>
        ...
        <TabsContent value="new-dynamic" className="mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Carregue um vídeo para análise frame-a-frame assistida por IA.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="border-2 border-dashed rounded-xl p-12 flex flex-col items-center justify-center text-center hover:bg-slate-50 transition-colors cursor-pointer relative">
                ...
                <p className="text-muted-foreground text-sm max-w-xs mt-2">
                  Arraste um vídeo gravado ou clique para escolher um arquivo.
                </p>
              </div>
            </CardContent>
          </Card>
