...
export const TreatmentPlan = () => {
	const [goals, setGoals] = useState<TreatmentGoal[]>([
		...
		{
			id: "2",
			description: "Melhorar amplitude de movimento",
			status: "pending",
		},
	]);
	...
	const [sessions, setSessions] = useState<TreatmentSession[]>([
		{
			id: "1",
			techniques: "Mobilização articular, terapia manual",
			exercises: "Alongamento, fortalecimento core",
			duration: "50 min",
		},
	]);
	...
	const addGoal = () => {
		...
		setGoals([
			...goals,
			{ id: Date.now().toString(), description: newGoal, status: "pending" },
		]);
		setNewGoal("");
	};
								...
								<div className="flex-1">
									...
									<div className="flex gap-2 mt-2">
										<Button
											...
											className={
												goal.status === "pending" ? "bg-yellow-100" : ""
											}
										>
											Pendente
										</Button>
										<Button
											...
											className={
												goal.status === "in-progress" ? "bg-blue-100" : ""
											}
										>
											Em andamento
										</Button>
						...
						<div key={session.id} className="p-4 border rounded-lg space-y-3">
							...
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) =>
										updateSession(session.id, "techniques", e.target.value)
									}
								/>
							</div>
							...
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) =>
										updateSession(session.id, "exercises", e.target.value)
									}
								/>
							</div>
							...
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) =>
										updateSession(session.id, "duration", e.target.value)
									}
								/>
							</div>
						</div>
