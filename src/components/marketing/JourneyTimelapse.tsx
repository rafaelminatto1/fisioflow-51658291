...
import { Upload, Play, Pause, Film, Trash2, Info } from "lucide-react";
...
export function JourneyTimelapse({ _patientName = "Paciente", _onSave }: JourneyTimelapseProps) {
  ...
  const handleImageUpload = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImages((prev) => [...prev, ...newImages].sort((a, b) => a.date.localeCompare(b.date)));
    ...
    const animate = (currentTime: number) => {
      ...
      if (elapsed >= frameDuration) {
        ...
        img.onload = () => {
          ...
          ctx.font = "16px Arial";
          ctx.fillText(`${frameIndex + 1} / ${images.length}`, canvas.width - 80, 30);
        };
  ...
  React.useEffect(() => {
    if (images.length > 0 && canvasRef.current && !isPlaying) {
      ...
      img.onload = () => {
        ...
        ctx.fillText(`${images.length} imagens carregadas`, canvas.width / 2, canvas.height / 2);
        ctx.font = "16px Arial";
        ctx.fillText("Clique em play para visualizar", canvas.width / 2, canvas.height / 2 + 40);
      };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Crie vídeos timelapse automáticos com fotos de evolução</CardDescription>
      </CardHeader>
        ...
        {images.length > 0 && (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <Label className="text-base font-semibold">Fotos da Evolução ({images.length})</Label>
        ...
        {images.length > 0 && (
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              <Button onClick={() => setIsPlaying(!isPlaying)} disabled={images.length === 0}>
                {isPlaying ? (
                  <>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <div className="flex items-center justify-between p-3 border rounded-lg">
                ...
                <Switch id="show-dates" checked={showDates} onCheckedChange={setShowDates} />
              </div>
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-xs mt-1">
                  Esta é uma pré-visualização do timelapse. A exportação completa em formato vídeo
                  será implementada em breve com suporte a todos os formatos populares.
                </p>
              </div>
            </div>
          </div>
