...
export function JourneyTimelapse({
	...
	const handleImageUpload = useCallback(
		async (event: React.ChangeEvent<HTMLInputElement>) => {
			...
			setImages((prev) =>
				[...prev, ...newImages].sort((a, b) => a.date.localeCompare(b.date)),
			);
			...
			<CardContent className="space-y-6">
									...
									<div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity">
										<div className="p-2 h-full flex flex-col justify-between">
											...
											<input
												...
												value={img.date}
												onChange={(e) =>
													updateImageDate(img.id, e.target.value)
												}
									...
									<div className="flex gap-1">
										{FRAME_RATES.map((rate) => (
											<Badge
												key={rate.value}
												variant={
													fps === parseInt(rate.value) ? "default" : "outline"
												}
						...
						<div className="flex items-start gap-2">
							...
							<div>
								...
								<p className="text-xs mt-1">
									Esta é uma pré-visualização do timelapse. A exportação
									completa em formato vídeo será implementada em breve com
									suporte a todos os formatos populares.
								</p>
							</div>
						</div>
					</div>
				...
				{images.length > 0 && (
					<div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800 dark:bg-amber-950 dark:border-amber-900 dark:text-amber-200">
						...
						<p>
							Certifique-se de que o paciente assinou o Termo de Consentimento
							para Uso de Imagem antes de compartilhar qualquer conteúdo com sua
							evolução.
						</p>
					</div>
				)}
