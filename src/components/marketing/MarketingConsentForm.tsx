...
const CONSENT_OPTIONS: ConsentOption[] = [
	{
		...
		icon: Instagram,
		examples: [
			"Posts de antes/depois",
			"Stories de evolução",
			"Depoimentos em vídeo",
		],
		color:
			"text-pink-600 bg-pink-50 border-pink-200 dark:bg-pink-950 dark:border-pink-800",
	},
	{
...
export function MarketingConsentForm({
	...
	const [socialMedia, setSocialMedia] = useState(
		consentDetails?.social_media ?? false,
	);
	const [educationalMaterial, setEducationalMaterial] = useState(
		consentDetails?.educational_material ?? false,
	);
	...
	const previouslyHadConsent =
		consentDetails?.is_active &&
		(consentDetails.social_media ||
			consentDetails.educational_material ||
			consentDetails.website);
			...
			<CardContent className="space-y-6">
				...
				<Alert className="border-blue-200 bg-blue-50 dark:border-blue-900 dark:bg-blue-950">
					...
					<AlertDescription className="text-blue-800 dark:text-blue-200 text-sm">
						O paciente tem o direito de revogar este consentimento a qualquer
						momento. A revogação não afeta o uso autorizado anteriormente, mas
						impedirá novas utilizações de sua imagem e dados.
					</AlertDescription>
				</Alert>
				...
				{previouslyHadConsent && (
					<Alert className="border-emerald-200 bg-emerald-50 dark:border-emerald-900 dark:bg-emerald-950">
						...
						<AlertDescription className="text-emerald-800 dark:text-emerald-200 text-sm">
							...
							{consentDetails?.signed_at &&
								new Date(consentDetails.signed_at).toLocaleDateString("pt-BR")}
				...
				<div className="space-y-4">
					<Label className="text-base font-semibold">
						Selecione os tipos de uso autorizados:
					</Label>
				...
				{!socialMedia && (
					<Alert className="border-amber-200 bg-amber-50 dark:border-amber-900 dark:bg-amber-950">
						...
						<AlertDescription className="text-amber-800 dark:text-amber-200 text-sm">
							Sem consentimento para redes sociais, o sistema não permitirá
							exportar conteúdo com imagem identificável do paciente, mesmo com
							anonimização.
						</AlertDescription>
					</Alert>
				)}
			...
			<Dialog open={showRevokeDialog} onOpenChange={setShowRevokeDialog}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Esta ação não pode ser desfeita. O paciente {patientName} terá
							todos os consentimentos de marketing revogados. Isso impedirá
							novas utilizações de sua imagem e dados em materiais de marketing.
						</DialogDescription>
					</DialogHeader>
					<DialogFooter>
						<Button
