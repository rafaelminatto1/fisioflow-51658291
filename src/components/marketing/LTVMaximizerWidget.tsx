...
export function LTVMaximizerWidget() {
  const { data: opportunitiesResponse, isLoading } = useQuery({
    ...
    queryFn: () =>
      request<{ data: LTVOpportunity[] }>("/api/marketing/ltv-maximizer/opportunities"),
  ...
  if (isLoading) {
    return (
      <div className="h-48 flex items-center justify-center text-xs text-muted-foreground animate-pulse">
        Identificando oportunidades de faturamento...
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {opportunities.slice(0, 4).map((opp) => (
          <Card
            key={opp.patient_id}
            className="border-none shadow-premium bg-white dark:bg-slate-900/50 hover:scale-[1.02] transition-transform"
          >
