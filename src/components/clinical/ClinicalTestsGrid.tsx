...
export function ClinicalTestsGrid({
	...
	const getBadgeColor = (category: string) => {
		switch (category) {
			...
			default:
				return "bg-blue-50 text-blue-700 border-blue-100"; // Ortopedia defaults here
		}
	};
		...
		return (
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				{[1, 2, 3, 4, 5, 6].map((i) => (
					<div
						key={i}
						className="bg-white rounded-2xl shadow-sm border border-slate-100 p-0 flex flex-col h-[320px] overflow-hidden"
					>
		...
		return (
			<div className="flex flex-col items-center justify-center py-24 px-4 text-center">
				...
				<p className="text-slate-500 max-w-sm mx-auto mb-8 leading-relaxed">
					Não encontramos resultados para sua busca. Tente termos mais genéricos
					ou limpe os filtros.
				</p>
				<Button
					variant="outline"
				...
				return (
					...
					>
						<div className="h-40 bg-slate-50 relative overflow-hidden shrink-0 border-b border-slate-50">
							...
							<div
								className={`absolute top-3 right-3 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border backdrop-blur-sm ${badgeClass}`}
							>
								{test.category || "Geral"}
							</div>
						...
						<div className="p-5 flex-1 flex flex-col">
							...
							<div className="flex gap-2 mt-auto flex-wrap pt-3 border-t border-slate-50">
								{test.tags?.slice(0, 2).map((tag: string) => (
									<span
										key={tag}
										className="text-[10px] bg-slate-50 text-slate-500 px-2.5 py-1 rounded-md border border-slate-100 uppercase font-bold tracking-wide"
									>
										#{tag}
									</span>
								))}
