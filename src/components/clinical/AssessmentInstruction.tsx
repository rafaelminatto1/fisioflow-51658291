...
export const AssessmentInstruction: React.FC<InstructionProps> = ({
  ...
  const dictionaryEntry = physioDictionary.find(
    (entry) => entry.id === testId || entry.id === `tst_${testId}`,
  );
        ...
        <CardHeader className="bg-gradient-to-r from-primary/10 via-background to-transparent border-b border-primary/5 py-8">
          <div className="flex justify-between items-center px-2">
            <div className="space-y-1">
              <Badge
                variant="secondary"
                className="bg-primary/10 text-primary font-black text-[10px] uppercase tracking-widest mb-2 border-0"
              >
                Teste Especial
              </Badge>
