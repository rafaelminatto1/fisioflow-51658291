...
function getNdiInterpretation(score: number): { label: string; color: string } {
  if (score <= 8) return { label: 'Sem incapacidade', color: 'bg-green-100 text-green-800' };
  if (score <= 28) return { label: 'Incapacidade leve', color: 'bg-green-100 text-green-700' };
  if (score <= 48) return { label: 'Incapacidade moderada', color: 'bg-yellow-100 text-yellow-800' };
  if (score <= 64) return { label: 'Incapacidade intensa', color: 'bg-orange-100 text-orange-800' };
...
export function NdiForm({ onSave, onCancel }: NdiFormProps) {
  ...
  const ndiScore = answeredCount === totalSections ? (totalScore / 50) * 100 : null;
  const interpretation = ndiScore !== null ? getNdiInterpretation(ndiScore) : null;
    ...
    <Card className="w-full">
      <CardHeader className="pb-3">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-lg">Índice de Incapacidade do Pescoço (NDI)</CardTitle>
        </div>
        <p className="text-sm text-muted-foreground">
          10 seções sobre limitações por dor cervical. Score = (soma / 50) × 100%.
        </p>
      </CardHeader>
      <CardContent className="space-y-5">
        <div className="rounded-md border bg-blue-50/50 p-3 text-sm text-blue-800">
          ...
          <p>
            Este questionário foi elaborado para dar informações sobre o quanto sua dor no pescoço
            afetou sua capacidade de realizar atividades do dia a dia. Selecione em cada seção
            apenas a alternativa que melhor descreve sua situação hoje.
          </p>
        </div>
            ...
            <>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  <span className="text-2xl font-bold tabular-nums">{ndiScore.toFixed(1)}</span>
