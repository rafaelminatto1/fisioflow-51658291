				...
				<div className="flex items-start justify-between gap-3">
					<div>
						<CardTitle className="text-base">
							{config.label} — Evolução
						</CardTitle>
						<p className="text-xs text-muted-foreground mt-0.5">
							{chartData.length} avaliação{chartData.length !== 1 ? "ões" : ""}{" "}
							registrada
						...
						>
							...
							{config.mcid !== null && lastScore !== null && (
								<ReferenceLine
									y={
										lastScore +
										(config.invertedTrend ? -config.mcid : config.mcid)
									}
				...
				{delta !== null && (
					<div className="mt-3 flex items-center justify-between text-sm border-t pt-3">
						<span className="text-muted-foreground">
							Variação total ({chartData[0]?.dateLabel} →{" "}
							{chartData[chartData.length - 1]?.dateLabel}):
						</span>
						<span
