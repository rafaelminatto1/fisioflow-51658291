...
export function PROMSelector({
	...
	const [selectedScale, setSelectedScale] = useState<ScaleKey | null>(
		defaultScale ?? null,
	);
	...
	const handleSave = async (
		...
		try {
			...
			toast.success(
				`${SCALES.find((s) => s.key === selectedScale)?.name} salvo com sucesso!`,
			);
			onSaved?.();
			onOpenChange(false);
				...
				{!selectedScale ? (
					<div className="space-y-3">
						<p className="text-sm text-muted-foreground">
							Selecione o instrumento de avaliação padronizado que deseja
							aplicar:
						</p>
