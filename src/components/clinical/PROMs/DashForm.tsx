...
function getDashInterpretation(score: number): {
  label: string;
  color: string;
} {
...
export function DashForm({ onSave, onCancel }: DashFormProps) {
  ...
  const dashScore =
    answeredCount === totalItems ? ((rawScore - totalItems) * 100) / (4 * totalItems) : null;
  ...
  const groups = [
    {
      label: "Atividades físicas (itens 1–16)",
      items: DASH_ITEMS.slice(0, 16),
    },
    {
      label: "Atividades de lazer e trabalho (itens 17–22)",
      items: DASH_ITEMS.slice(16, 22),
    },
    {
      label: "Sintomas e impacto (itens 23–30)",
      items: DASH_ITEMS.slice(22, 30),
    },
  ];
    ...
    <Card className="w-full">
      <CardHeader className="pb-3">
        ...
        <p className="text-sm text-muted-foreground">
          30 itens sobre dificuldades funcionais do membro superior. Score = (soma − 30) × 100 /
          120.
        </p>
      </CardHeader>
      <CardContent className="space-y-5">
        <div className="rounded-md border bg-blue-50/50 p-3 text-sm text-blue-800">
          ...
          <p>
            Para cada atividade, selecione o número que melhor descreve sua capacidade na última
            semana, independentemente de qual mão ou braço você usa.
          </p>
        </div>
