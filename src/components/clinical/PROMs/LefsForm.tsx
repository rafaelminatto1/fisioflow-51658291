...
export function LefsForm({ onSave, onCancel }: LefsFormProps) {
	...
	const interpretation =
		answeredCount === totalItems ? getLefsInterpretation(totalScore) : null;
		...
		<Card className="w-full">
			<CardHeader className="pb-3">
				<div className="flex items-center gap-2">
					...
					<CardTitle className="text-lg">
						LEFS — Escala Funcional dos Membros Inferiores
					</CardTitle>
				</div>
				<p className="text-sm text-muted-foreground">
					20 itens sobre função dos membros inferiores. Score máximo: 80 pontos.
					MCID: 9 pontos.
				</p>
			</CardHeader>
			<CardContent className="space-y-5">
				<div className="rounded-md border bg-blue-50/50 p-3 text-sm text-blue-800">
					...
					<p>
						Estamos interessados em saber se você tem dificuldade em realizar as
						atividades listadas abaixo por causa do seu problema com o membro
						inferior. Selecione a opção que melhor descreve sua capacidade atual
						para cada atividade.
					</p>
				</div>
				...
				<div className="rounded-md border bg-muted/30 p-4 space-y-2">
					...
					<div className="flex items-center justify-between">
						...
						<div className="flex items-center gap-1">
							<span
								className={`text-2xl font-bold tabular-nums ${getScoreColor(totalScore)}`}
							>
								{totalScore}
							</span>
