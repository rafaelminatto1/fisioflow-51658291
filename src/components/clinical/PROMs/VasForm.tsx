...
function getVasInterpretation(score: number): { label: string; color: string } {
	if (score === 0)
		return { label: "Sem dor", color: "bg-green-100 text-green-800" };
	if (score <= 3)
		return { label: "Dor leve", color: "bg-green-100 text-green-700" };
	if (score <= 6)
		return { label: "Dor moderada", color: "bg-yellow-100 text-yellow-800" };
	if (score <= 9)
		return { label: "Dor intensa", color: "bg-orange-100 text-orange-800" };
		...
		<Card className="w-full">
			<CardHeader className="pb-3">
				...
				<p className="text-sm text-muted-foreground">
					Avaliação da intensidade da dor percebida pelo paciente. MCID: 1,5
					pontos.
				</p>
			</CardHeader>
			<CardContent className="space-y-6">
				<div className="space-y-2">
					<Label className="text-sm font-medium">
						Instrução: Indique a intensidade da sua dor no momento atual, onde 0
						significa nenhuma dor e 10 significa a pior dor imaginável.
					</Label>
				</div>
