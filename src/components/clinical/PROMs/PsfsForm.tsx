...
function getPsfsInterpretation(score: number): { label: string; color: string } {
  if (score >= 8) return { label: 'Função excelente', color: 'bg-green-100 text-green-800' };
  if (score >= 6) return { label: 'Boa função', color: 'bg-green-100 text-green-700' };
  if (score >= 4) return { label: 'Função moderada', color: 'bg-yellow-100 text-yellow-800' };
  if (score >= 2) return { label: 'Função limitada', color: 'bg-orange-100 text-orange-800' };
...
export function PsfsForm({ onSave, onCancel }: PsfsFormProps) {
  ...
  const meanScore =
    validActivities.length > 0
      ? validActivities.reduce((sum, a) => sum + a.score, 0) / validActivities.length
      : 0;
    ...
    <Card className="w-full">
      <CardHeader className="pb-3">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-lg">Escala Funcional Específica do Paciente (EFEP/PSFS)</CardTitle>
        </div>
        <p className="text-sm text-muted-foreground">
          Avaliação de atividades funcionais específicas do paciente. MCID: 2 pontos (melhora clinicamente significativa).
        </p>
      </CardHeader>
      <CardContent className="space-y-5">
        <div className="rounded-md border bg-blue-50/50 p-3 text-sm text-blue-800">
          ...
          <p>
            Identifique até 5 atividades importantes que você tem dificuldade de realizar devido ao seu problema.
            Para cada atividade, indique sua capacidade atual de 0 (incapaz) a 10 (sem dificuldade).
          </p>
        </div>
            ...
            <div key={index} className="rounded-md border p-3 space-y-3 bg-card">
              ...
              <div className="space-y-2 px-1">
                ...
                <div className="flex justify-end">
                  <span className={`text-lg font-bold tabular-nums ${getScoreColor(activity.score)}`}>
                    {activity.score}/10
                  </span>
                </div>
              </div>
        ...
        {validActivities.length > 0 && (
          <div className="rounded-md border bg-muted/30 p-4 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium">Pontuação média (PSFS):</span>
              <div className="flex items-center gap-2">
                <span className={`text-2xl font-bold tabular-nums ${getScoreColor(meanScore)}`}>
                  {meanScore.toFixed(1)}
                </span>
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-muted-foreground">
                Baseado em {validActivities.length} atividade{validActivities.length > 1 ? 's' : ''}
              </span>
        ...
        <div className="rounded-md border bg-muted/30 p-3 text-sm space-y-1">
          ...
          <p className="text-muted-foreground">
            MCID = 2 pontos. Uma melhora de 2 ou mais pontos na média indica mudança clinicamente
