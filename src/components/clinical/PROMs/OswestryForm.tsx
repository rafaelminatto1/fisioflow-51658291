...
function getOswestryInterpretation(score: number): { label: string; color: string } {
  if (score <= 20) return { label: 'Incapacidade mínima', color: 'bg-green-100 text-green-800' };
  if (score <= 40) return { label: 'Incapacidade moderada', color: 'bg-yellow-100 text-yellow-800' };
  if (score <= 60) return { label: 'Incapacidade intensa', color: 'bg-orange-100 text-orange-800' };
  if (score <= 80) return { label: 'Deficiência grave', color: 'bg-red-100 text-red-800' };
...
export function OswestryForm({ onSave, onCancel }: OswestryFormProps) {
  ...
  const interpretation = odScore !== null ? getOswestryInterpretation(odScore) : null;
    ...
    <Card className="w-full">
      <CardHeader className="pb-3">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-lg">Índice de Incapacidade de Oswestry (ODI)</CardTitle>
        </div>
      ...
      <CardContent className="space-y-5">
        <div className="rounded-md border bg-blue-50/50 p-3 text-sm text-blue-800">
          ...
          <p>
            Este questionário foi elaborado para dar informações sobre como a dor nas costas ou perna
            afeta sua capacidade de realizar atividades diárias. Por favor, responda a cada seção
            marcando apenas a opção que melhor se aplica a você hoje.
          </p>
        </div>
          ...
          {OSWESTRY_SECTIONS.map((section, sectionIndex) => (
            <div
              ...
              className={`rounded-md border p-4 space-y-3 transition-colors ${
                responses[sectionIndex] !== undefined ? 'bg-orange-50/30 border-orange-200' : 'bg-card'
              }`}
            >
            ...
            <>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  <span className="text-2xl font-bold tabular-nums">{odScore.toFixed(1)}</span>
        ...
        <div className="rounded-md border bg-muted/30 p-3 text-sm space-y-1">
          ...
          <div className="grid grid-cols-1 gap-0.5 text-muted-foreground text-xs">
            ...
            <span>61–80% = Deficiência (interferência em todos os aspectos da vida)</span>
