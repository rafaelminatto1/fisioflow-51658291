...
export function PROMResultCard({
	result,
	onViewTimeline,
}: PROMResultCardProps) {
	const scale = result.scale_name.toUpperCase();
	const { scoreColor, bgColor, borderColor } = getSeverityColor(
		result.scale_name,
		result.score,
	);
	...
	return (
		<Card
			className={`border ${borderColor} ${bgColor} transition-shadow hover:shadow-md`}
		>
