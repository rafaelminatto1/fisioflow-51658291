...
function StandardFormCard({ type, onCreate, onDuplicate }: StandardFormCardProps) {
  ...
  const getFieldSections = () => {
    const sections = config.campos.reduce(
      (acc, campo) => {
  ...
  return (
    <Card
      className={`transition-all ${exists ? "border-green-500/30 bg-green-50/30 dark:bg-green-950/20" : "hover:shadow-md"}`}
    >
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <div className="text-sm text-muted-foreground">{getFieldSections()}</div>
        </div>

        <div className="flex items-center gap-2 text-sm">
          <Badge variant="secondary">{config.campos.length} campos</Badge>
          <Badge variant="secondary">
            {
              Object.keys(config.campos.reduce((acc, c) => ({ ...acc, [c.secao]: true }), {}))
                .length
            }{" "}
            seções
          </Badge>
        ...
        <div className="flex flex-col gap-2">
          ...
          <Button className="w-full" variant="secondary" onClick={() => onDuplicate(type)}>
    ...
    <div className="space-y-6">
      ...
      {existingStandardForms.length > 0 && (
        <Alert>
          ...
          <AlertDescription>
            Você já tem {existingStandardForms.length} ficha(s) padrão instalada(s). As fichas podem
            ser personalizadas após a criação.
          </AlertDescription>
        </Alert>
      )}
      ...
      <Alert>
        <Info className="h-4 w-4" />
        <AlertDescription>
          As fichas padrão incluem campos comumente utilizados em fisioterapia. Após criar, você
          pode personalizar adicionando, removendo ou editando campos conforme sua necessidade.
        </AlertDescription>
      </Alert>
    </div>
  );
