			...
			<CardContent className="space-y-4">
				...
				<div className="flex items-center gap-2 text-sm">
					...
					<Badge variant="secondary">
						{
							Object.keys(
								config.campos.reduce(
									(acc, c) => ({ ...acc, [c.secao]: true }),
									{},
								),
							).length
						}{" "}
...
export function StandardFormsManager() {
	...
	const handleDuplicateForm = async (type: keyof typeof STANDARD_FORMS) => {
		...
		const fields: EvaluationFormFieldFormData[] = config.campos.map(
			(c: StandardFormField) => ({
		...
		<div className="space-y-6">
			<div className="flex items-center justify-between">
				...
				{existingStandardForms.length > 0 && (
					<Badge variant="secondary">
						{existingStandardForms.length} de{" "}
						{Object.keys(STANDARD_FORMS).length} instaladas
					</Badge>
				)}
			</div>
			...
			{existingStandardForms.length > 0 && (
				<Alert>
					...
					<AlertDescription>
						Você já tem {existingStandardForms.length} ficha(s) padrão
						instalada(s). As fichas podem ser personalizadas após a criação.
					</AlertDescription>
				</Alert>
			)}
			...
			<Alert>
				...
				<AlertDescription>
					...
					Após criar, você pode personalizar adicionando, removendo ou editando
