...
export function ClinicalTestDetailsModal({
  ...
  const relatedTestIds = (currentCluster?.tests || []).filter((id) => id !== test?.id);
  ...
  const handleDownloadPDF = async () => {
    try {
      const { generateClinicalTestPdf } = await import("@/utils/generateClinicalTestPdf");
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {test.regularity_sessions ? (
              <span className="flex items-center gap-1 rounded-full border border-amber-100 bg-amber-50 px-2.5 py-1 text-[10px] font-medium text-amber-700">
                <CalendarCheck className="h-3 w-3" />A cada {test.regularity_sessions} sessões
              </span>
            ) : null}
          </div>
            ...
            <div>
              ...
              <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                <p className="whitespace-pre-line text-sm leading-relaxed text-slate-600">
                  {test.execution || "Nenhuma instrução de execução disponível."}
                </p>
              </div>
            </div>
            ...
            {test.positive_sign ? (
              <div>
                ...
                <div className="rounded-2xl border border-blue-100 border-l-4 border-l-blue-500 bg-blue-50/60 p-4">
                  <p className="text-sm font-medium italic text-blue-900">{test.positive_sign}</p>
                </div>
              </div>
            ) : null}
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {test.lr_positive && (
                    <div className="rounded-2xl bg-teal-50/50 p-3 border border-teal-100/50">
                      ...
                      <p className="text-lg font-black text-teal-700">{test.lr_positive}</p>
                  ...
                  {test.lr_negative && (
                    <div className="rounded-2xl bg-slate-50 p-3 border border-slate-100">
                      ...
                      <p className="text-lg font-black text-slate-700">{test.lr_negative}</p>
