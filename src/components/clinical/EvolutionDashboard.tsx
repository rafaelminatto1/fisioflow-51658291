		...
		return (
			<div className="flex flex-col items-center justify-center min-h-[400px] border-2 border-dashed border-muted rounded-3xl p-8 text-center">
				...
				<p className="text-sm text-muted-foreground max-w-xs">
					Inicie as sessões de reabilitação e registre medições clínicas para gerar insights de progresso.
				</p>
			</div>
		);
	}
				...
				<DashboardCard title="Progresso em Testes Clínicos">
					...
					<div className="mt-4 flex flex-wrap gap-2">
						{Array.from(new Set(data.clinical.metricsHistory.map(m => m.name))).map(name => (
							<span key={name} className="px-2 py-1 bg-primary/10 text-primary rounded-md text-[10px] font-bold uppercase tracking-wider">
								{name}
							</span>
						))}
					</div>
...
const MetricCard: React.FC<MetricCardProps> = ({ title, value, subtext, icon: Icon, trend, color }) => {
	const colors = {
	...
	return (
		<div className={`relative overflow-hidden rounded-3xl border bg-card/40 backdrop-blur-md p-6 transition-all hover:shadow-xl hover:bg-card/60 group ${colors[color]}`}>
			<div className="flex justify-between items-start">
				...
				<div className={`p-3 rounded-2xl bg-white/20 shadow-inner group-hover:scale-110 transition-transform`}>
			...
			<div className="mt-4 flex items-center gap-1">
				...
				<span className="text-[10px] font-bold uppercase opacity-60">Tendência {trend === "up" ? "Positiva" : trend === "down" ? "Negativa" : "Estável"}</span>
			</div>
		</div>
	);
};
