...
export const MovementAnalysis = () => {
  ...
  const processFrame = useCallback((nowMs: number) => {
    ...
    const symmetryRatio = hipWidth > 0 ? Math.min(shoulderWidth, hipWidth) / Math.max(shoulderWidth, hipWidth) : 1;
  ...
  const startAnalysis = async () => {
    try {
      ...
      toast({ title: 'Análise iniciada', description: 'Posicione-se na frente da câmera' });
    } catch (error) {
      logger.error('Erro ao iniciar análise', error, 'MovementAnalysis');
      toast({ title: 'Erro', description: 'Não foi possível acessar a câmera ou carregar o modelo', variant: 'destructive' });
    }
  };
  ...
  const stopAnalysis = () => {
    ...
    toast({ title: 'Análise finalizada', description: 'Relatório salvo com sucesso' });
  };
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {[
                ...
                { label: 'Amplitude de Movimento', value: metrics.rangeOfMotion },
                { label: 'Velocidade de Movimento', value: metrics.speed },
                { label: 'Estabilidade do Centro de Massa', value: metrics.stability },
              ].map(({ label, value }) => (
