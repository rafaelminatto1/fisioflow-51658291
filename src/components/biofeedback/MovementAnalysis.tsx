...
export const MovementAnalysis = () => {
	...
	const processFrame = useCallback((nowMs: number) => {
		...
		const romScore = Math.round(
			100 - Math.min(leftKneeAngle, leftElbowAngle) / 1.8,
		);
		...
		if (symmetryScore < 70)
			newFeedback.push("⚠️ Assimetria detectada — verifique alinhamento");
		if (stabilityScore < 60)
			newFeedback.push("⚠️ Mantenha mais estabilidade no centro de massa");
		if (posture === "ruim")
			newFeedback.push("⚠️ Inclinação lateral excessiva detectada");
		if (posture === "atenção")
			newFeedback.push("ℹ️ Leve desvio lateral — corrija a postura");
		if (
			symmetryScore >= 85 &&
			stabilityScore >= 80 &&
			posture === "excelente"
			...
			<CardContent className="space-y-4">
				...
				{isAnalyzing && (
					<p className="text-center text-xs text-muted-foreground">
						Análise em andamento via MediaPipe PoseLandmarker (lite) — GPU
						delegate
					</p>
				)}
			</CardContent>
