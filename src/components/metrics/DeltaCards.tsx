...
const DeltaCards: React.FC<DeltaCardsProps> = ({
	data,
	mode = "ALL",
				...
				<Card key={metric.key} className="shadow-sm">
					<CardContent className="p-4 flex flex-col gap-2">
						<div className="flex justify-between items-start">
							<span
								className="text-xs font-medium text-muted-foreground line-clamp-2 min-h-[2.5em]"
								title={metric.label}
							>
								{metric.label}
							</span>
							{showConfidence && metric.confidence_0_100 && (
								<span
									className={`text-[10px] ${metric.confidence_0_100 < 70 ? "text-amber-500" : "text-gray-500"}`}
								>
						...
						<div className="flex items-center justify-between mt-1">
							...
							<Badge
								variant="outline"
								className={`text-[10px] px-1 py-0 h-5 gap-1 ${getStatusColor(metric.status)}`}
							>
