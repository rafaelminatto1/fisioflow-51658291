  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      {invitationData && (
        <div className="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/30 p-4 rounded-2xl flex items-start gap-3 text-emerald-800 dark:text-emerald-400 mb-6 animate-in fade-in slide-in-from-top-2 duration-500">
          ...
          <div className="text-xs leading-relaxed">
            ...
            <p>
              Você está se juntando à clínica como{" "}
              <span className="font-bold uppercase tracking-tight">{invitationData.role}</span>.
              Complete seu perfil abaixo.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          {validationErrors.fullName && (
            <span className="text-[11px] text-destructive font-semibold ml-1">
              {validationErrors.fullName}
            </span>
          )}
        </div>
        ...
        {password && (
          <div className="p-3 bg-slate-100/50 dark:bg-slate-900/50 rounded-xl border border-slate-200/50 dark:border-slate-800/50 space-y-1.5">
            {passwordRequirements.map((req, idx) => (
              <div
                key={idx}
                className={`flex items-center gap-2 text-[11px] ${req.met ? "text-emerald-600 dark:text-emerald-400 font-bold" : "text-muted-foreground font-medium"}`}
              >
                {req.met ? (
      ...
      {error && (
        <Alert
          variant="destructive"
          className="border-red-200 bg-red-50 dark:bg-red-950/20 dark:border-red-900/30"
        >
