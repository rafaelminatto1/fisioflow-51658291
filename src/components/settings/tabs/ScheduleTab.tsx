...
export function ScheduleTab(_props: any) {
	const { businessHours, isLoadingHours, upsertBusinessHours, isSavingHours, daysOfWeek } = useScheduleSettings();
			...
			<CardContent className="p-3 sm:p-4 lg:p-6 space-y-6">
                <div className="space-y-3">
                    {daysOfWeek.map(day => {
                        ...
                        return (
                            <div key={day.value} className="flex flex-col xl:flex-row xl:items-center justify-between gap-3 p-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
                                <div className="flex items-center gap-3 min-w-[160px]">
                                    <Switch
                                        checked={isOpen}
                                        onCheckedChange={(v) => updateDay(day.value, "is_open", v)}
                                    />
                                    <Label className="font-medium cursor-pointer" onClick={() => updateDay(day.value, "is_open", !isOpen)}>
                                        {day.label}
                                    </Label>
                                </div>
                                ...
                                {isOpen ? (
                                    ...
                                    <div className="flex-1 text-sm text-muted-foreground italic px-2">
                                        Fechado
                                    </div>
                                )}
                            </div>
