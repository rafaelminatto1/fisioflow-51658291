			...
			await Promise.all(
				collections.map(async (colName) => {
					switch (colName) {
						case "patients":
							backupData[colName] =
								(await patientsApi.list({ limit: 200, minimal: true })).data ?? [];
							break;
						case "appointments":
							backupData[colName] =
								(await appointmentsApi.list({ limit: 1000 })).data ?? [];
							break;
						case "transactions":
							backupData[colName] =
								(await financialApi.transacoes.list({ limit: 1000 })).data ??
								[];
							break;
						case "session_packages":
							backupData[colName] =
								(await financialApi.packageTemplates.list()).data ?? [];
							break;
						case "patient_packages":
							backupData[colName] =
								(await financialApi.patientPackages.list()).data ?? [];
							break;
						default:
							backupData[colName] = [];
							break;
