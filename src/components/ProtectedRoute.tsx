...
export function ProtectedRoute({
  ...
  const { user, profile, loading, initialized, sessionCheckFailed, role, refreshProfile } =
    useAuth();
...
export function TherapistRoute({ children }: { children: React.ReactNode }) {
  return (
    <ProtectedRoute allowedRoles={["fisioterapeuta", "estagiario"]}>{children}</ProtectedRoute>
  );
}

export function PatientRoute({ children }: { children: React.ReactNode }) {
  return <ProtectedRoute allowedRoles={["paciente"]}>{children}</ProtectedRoute>;
}

export function PartnerRoute({ children }: { children: React.ReactNode }) {
  return <ProtectedRoute allowedRoles={["parceiro"]}>{children}</ProtectedRoute>;
}
