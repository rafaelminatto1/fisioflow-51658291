...
export function MergeExercisesModal({
	...
	const handleMerge = async () => {
		...
		setIsLoading(true);
		try {
			const mergeIds = exercises
				.filter((e) => e.id !== selectedKeepId)
				.map((e) => e.id);
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
				<DialogHeader>
					...
					<DialogDescription>
						Escolha qual exercício manter. Os outros serão mesclados e
						removidos.
					</DialogDescription>
				</DialogHeader>
				...
				<div className="space-y-4 py-4">
					...
					>
						...
						<AlertDescription className="text-amber-800 dark:text-amber-200">
							Esta ação é irreversível. Os exercícios selecionados serão
							permanentemente mesclados.
						</AlertDescription>
					</Alert>
					...
					<div className="space-y-3">
						<Label className="text-sm font-medium">
							Selecione o exercício a manter:
						</Label>
						<RadioGroup
