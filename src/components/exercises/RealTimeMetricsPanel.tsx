...
import { ExerciseMetrics } from "@/types/pose";
import { Activity, Clock, TrendingUp, TrendingDown, Award, Zap, Flame, Target } from "lucide-react";
...
const MetricCard: React.FC<{
  ...
  return (
    <div
      className={cn("flex flex-col items-center justify-center p-3 rounded-lg bg-muted/50", color)}
    >
      <div className={cn("p-2 rounded-full mb-2", color)}>{icon}</div>
      <span className="text-2xl font-bold">{value}</span>
      <span className="text-xs text-muted-foreground mt-1">{label}</span>
      {trend === "up" && <TrendingUp className="h-3 w-3 text-emerald-600 mt-1" />}
      {trend === "down" && <TrendingDown className="h-3 w-3 text-red-600 mt-1" />}
    </div>
  );
};
      ...
      <CardContent className="p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{qualityText}</p>
          </div>
        ...
        {targetRepetitions && targetRepetitions > 0 && (
          <div className="mt-6 p-4 rounded-lg bg-muted/30">
            <RepProgressIndicator current={repetitions} target={targetRepetitions} />
          </div>
        )}
