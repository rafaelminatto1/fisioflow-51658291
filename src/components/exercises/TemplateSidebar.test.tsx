...
describe("filterTemplates — testes de propriedade", () => {
  ...
  it("Property 1: filtro por perfil retorna apenas templates do perfil selecionado", () => {
    // Validates: Requirements 1.2, 1.4
    fc.assert(
      fc.property(
  ...
  it("Property 1 (all): quando perfil é 'all', todos os templates são retornados sem filtro de perfil", () => {
    // Validates: Requirements 1.4
    fc.assert(
      fc.property(fc.array(arbitraryTemplate(), { maxLength: 30 }), (templates) => {
        const result = filterTemplates(templates, "all", "");

        // Result must contain exactly the same templates as input (same ids, same order)
        return (
          result.length === templates.length && result.every((r, i) => r.id === templates[i].id)
        );
      }),
      { numRuns: 100 },
    );
  ...
  it("Property 2: busca retorna subconjunto da lista original e cada resultado contém a query em name, conditionName ou templateVariant", () => {
    // Validates: Requirements 1.6
    fc.assert(
      fc.property(
        fc.array(arbitraryTemplate(), { maxLength: 30 }),
        ...
        (templates, query) => {
          ...
          const isSubset = result.every((r) => templates.some((t) => t.id === r.id));
  ...
  it("Property 2 (empty query): query vazia não filtra nenhum template", () => {
    // Validates: Requirements 1.6
    fc.assert(
      fc.property(fc.array(arbitraryTemplate(), { maxLength: 30 }), (templates) => {
        const result = filterTemplates(templates, "all", "");
  ...
  it("Property 2 (whitespace query): query com apenas espaços não filtra nenhum template", () => {
    // Validates: Requirements 1.6
    fc.assert(
      fc.property(
        fc.array(arbitraryTemplate(), { maxLength: 30 }),
