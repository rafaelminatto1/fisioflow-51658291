...
function ComplianceBar({ date, completed }: { date: string; completed: boolean }) {
  const label = new Date(date + 'T00:00:00').toLocaleDateString('pt-BR', { weekday: 'narrow', day: 'numeric' });
  return (
...
function PlanComplianceCard({ planId }: { planId: string }) {
  const { data, isLoading } = useQuery<HEPComplianceData>({
    ...
    queryFn: async () => {
      const res = await request<{ data: HEPComplianceData }>(`/api/exercise-plans/${planId}/compliance`);
      return res.data;
    },
    staleTime: 5 * 60 * 1000,
  });
  ...
  const rateColor = data.rate >= 70 ? 'text-green-600' : data.rate >= 40 ? 'text-yellow-600' : 'text-red-600';
