...
export const CalibrationOverlay: React.FC<CalibrationOverlayProps> = ({
	...
	const isDistanceOk =
		nose &&
		(ankleL || ankleR) &&
		Math.abs(nose.y - (ankleL?.y || ankleR?.y)) > 0.4; // Corpo ocupa boa parte da altura
				...
				<CardContent className="p-6 space-y-6 text-center">
					...
					<div className="relative aspect-video bg-muted rounded-lg overflow-hidden border-2 border-dashed border-primary/50">
						...
						<div className="absolute inset-0 flex items-center justify-center opacity-30 pointer-events-none">
							<svg
								viewBox="0 0 100 200"
								className="h-4/5 text-primary fill-current"
							>
						...
						<div className="absolute top-4 left-4 right-4 flex justify-between text-xs font-bold text-white drop-shadow-md">
							<span className={isCentered ? "text-green-400" : "text-red-400"}>
								{isCentered ? (
									...
									<MoveHorizontal className="inline h-4 w-4 mr-1" />
								)}
								Centralizado
							</span>
							<span
								className={isDistanceOk ? "text-green-400" : "text-red-400"}
							>
								{isDistanceOk ? (
									<Check className="inline h-4 w-4 mr-1" />
								) : (
					...
					<div className="space-y-2">
						...
						<p className="text-sm text-muted-foreground mt-2">
							{!isBodyVisible
								? "Corpo não detectado. Aumente a iluminação."
								: !isCentered
									? "Fique no centro da câmera."
									: !isDistanceOk
										? "Afaste-se para enquadrar corpo inteiro."
					...
					<div className="flex gap-4 pt-4">
						...
						<Button
							...
							disabled={!isReady}
							className={
								isReady ? "bg-green-600 hover:bg-green-700 flex-1" : "flex-1"
							}
						>
