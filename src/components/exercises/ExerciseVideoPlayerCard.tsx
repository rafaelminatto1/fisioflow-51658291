...
export const ExerciseVideoPlayer: React.FC<ExerciseVideoPlayerProps> = ({
  ...
  useEffect(() => {
    ...
    if (autoPlay) {
      video
        ...
        .catch((err) => logger.warn("Video autoplay failed", err, "ExerciseVideoPlayer"));
    }
  }, [autoPlay]);
  ...
  const toggleFullscreen = useCallback(() => {
    ...
    if (!isFullscreen) {
      container
        .requestFullscreen()
        .catch((err) => logger.warn("Fullscreen request failed", err, "ExerciseVideoPlayer"));
    } else {
      document
        .exitFullscreen()
        .catch((err) => logger.warn("Exit fullscreen failed", err, "ExerciseVideoPlayer"));
    }
  }, [isFullscreen]);
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case ",": {
          ...
          e.preventDefault();
          const newSlowerRate =
            SPEED_OPTIONS[Math.max(0, SPEED_OPTIONS.indexOf(playbackRate) - 1)] || playbackRate / 2;
        ...
        case ".": {
          ...
          const newFasterRate =
            SPEED_OPTIONS[
              Math.min(SPEED_OPTIONS.length - 1, SPEED_OPTIONS.indexOf(playbackRate) + 1)
            ] || playbackRate * 2;
  ...
  return (
    ...
    >
      ...
      >
        {/* Title */}
        {title && <h3 className="text-white text-sm font-medium mb-2 truncate">{title}</h3>}
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 flex items-center gap-2">
            ...
            <Slider
              ...
              className="flex-1"
            />
            <span className="text-white text-xs w-10 shrink-0">{formatTime(duration)}</span>
          </div>
  ...
  return (
    ...
    >
      <div className="relative aspect-video bg-muted">
        <CompactVideoPlayer src={video.video_url} thumbnail={video.thumbnail_url} />
