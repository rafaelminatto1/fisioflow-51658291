	...
	useEffect(() => {
		...
		if (autoPlay) {
			video
				...
				.catch((err) =>
					logger.warn("Video autoplay failed", err, "ExerciseVideoPlayer"),
				);
		}
	}, [autoPlay]);
	...
	const toggleFullscreen = useCallback(() => {
		...
		if (!isFullscreen) {
			container
				.requestFullscreen()
				.catch((err) =>
					logger.warn("Fullscreen request failed", err, "ExerciseVideoPlayer"),
				);
		} else {
			document
				.exitFullscreen()
				.catch((err) =>
					logger.warn("Exit fullscreen failed", err, "ExerciseVideoPlayer"),
				);
		}
	}, [isFullscreen]);
		...
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			switch (e.key) {
				...
				case ",": {
					...
					const newSlowerRate =
						SPEED_OPTIONS[
							Math.max(0, SPEED_OPTIONS.indexOf(playbackRate) - 1)
						] || playbackRate / 2;
					...
					const newFasterRate =
						SPEED_OPTIONS[
							Math.min(
								...
								SPEED_OPTIONS.indexOf(playbackRate) + 1,
							)
						] || playbackRate * 2;
	...
	return (
		...
		>
			...
			>
				{/* Title */}
				{title && (
					<h3 className="text-white text-sm font-medium mb-2 truncate">
						{title}
					</h3>
				)}
