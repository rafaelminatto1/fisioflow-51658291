...
const ExerciseCard = React.memo(function ExerciseCard({
	...
	const diffConfig = exercise.difficulty
			...
			<div className="p-4 space-y-3 flex-1 flex flex-col">
				...
				<div className="flex gap-2 flex-wrap">
					{exercise.category && (
						<Badge
							variant="outline"
							className={cn("text-xs font-medium", catColor)}
						>
							{exercise.category}
						</Badge>
					)}
...
export function ExerciseLibrary({
	...
	const { exercises, loading, deleteExercise, mergeExercises, isDeleting } =
		useExercises();
		...
		return searchFiltered.filter((exercise) => {
			...
			if (advancedFilters.bodyParts.length > 0) {
				...
				if (advancedFilters.difficulty.length > 0) {
					if (
						!exercise.difficulty ||
						!advancedFilters.difficulty.includes(exercise.difficulty)
					) {
						return false;
					}
				}
				...
				if (advancedFilters.categories.length > 0) {
					if (
						!exercise.category ||
						!advancedFilters.categories.includes(exercise.category)
					) {
						return false;
					}
				}
					...
					const hasMatchingEquipment = advancedFilters.equipment.some((eq) => {
						...
						if (eq === NO_EQUIPMENT_GROUP_ID) {
							...
							return exerciseEquipment.some((eeq) =>
								homeGroupLabels.some(
									(hgl) => eeq.toLowerCase() === hgl.toLowerCase(),
								),
							);
						}

						return exerciseEquipment.some((eeq) =>
							eeq.toLowerCase().includes(eq.toLowerCase()),
						);
					});
