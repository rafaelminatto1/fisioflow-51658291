...
const ExerciseCard = React.memo(function ExerciseCard({
  ...
  return (
    ...
    >
      {/* Image Section */}
      <div className="relative aspect-video w-full bg-white dark:bg-slate-950 overflow-hidden flex-shrink-0">
        {thumbSrc ? (
          <OptimizedImage
            src={thumbSrc}
        ...
        <div className="flex gap-2 pt-2 mt-auto">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              >
                <Heart
                  className={cn("h-4 w-4 mr-2", isFavorite && "fill-rose-500 text-rose-500")}
                />
...
export function ExerciseLibrary({
  ...
  const filteredExercises = useMemo(() => {
    ...
    const searchFiltered = bilingualFilter(validExercises, debouncedSearchTerm, [
      "name",
      "description",
      "category",
      "body_parts",
    ...
    return searchFiltered.filter((exercise) => {
      ...
      if (advancedFilters.equipment.length > 0) {
        ...
        const hasMatchingEquipment = advancedFilters.equipment.some((eq) => {
          ...
          if (eq === NO_EQUIPMENT_GROUP_ID) {
            ...
            return exerciseEquipment.some((eeq) =>
              homeGroupLabels.some((hgl) => eeq.toLowerCase() === hgl.toLowerCase()),
            );
          }
      ...
      if (advancedFilters.homeOnly) {
        const homeEquipmentLabels = EQUIPMENT.filter(
          (e) => e.homeFrequency === "always" || e.homeFrequency === "common",
        ).map((e) => e.label);

        const exerciseEquipment = exercise.equipment || [];
        const isHomeExercise = exerciseEquipment.every((eq) =>
          homeEquipmentLabels.some((he) => eq.toLowerCase().includes(he.toLowerCase())),
        );
  ...
  useEffect(() => {
    ...
    if (exercises.length === 0) {
      logger.warn(
        "Nenhum exercício disponível para renderizar na biblioteca",
        {
    ...
    if (filteredExercises.length === 0) {
      logger.warn(
        "Filtros ativos ocultaram todos os exercícios da biblioteca",
        {
    ...
    <div className="flex flex-col w-full relative px-4 sm:px-6 pb-6">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground hidden sm:block">
                  {filteredExercises.length} resultado
      ...
      <div className="rounded-xl border bg-card relative min-h-[400px]">
        {filteredExercises.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6 p-2">
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta ação não pode ser desfeita. Isso excluirá permanentemente o exercício da sua
              biblioteca.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
