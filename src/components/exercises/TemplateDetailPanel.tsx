  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center border-2 border-dashed rounded-xl bg-muted/30">
      ...
      <p className="text-xs mt-1 text-muted-foreground">
        Este protocolo base ainda não possui movimentos vinculados.
      </p>
    </div>
  );
}
...
const POS_OP_PHASES = [
  {
    label: "Fase 1 — Proteção",
    weeks: "1–4",
    desc: "Controle de dor e edema, mobilização passiva suave.",
  },
  {
    label: "Fase 2 — Mobilização",
    weeks: "5–8",
    desc: "Ganho de amplitude ativa, fortalecimento inicial sem impacto.",
  },
  {
    label: "Fase 3 — Fortalecimento",
    weeks: "9–16",
    desc: "Fortalecimento progressivo, propriocepção e equilíbrio.",
  },
  {
    label: "Fase 4 — Retorno",
    weeks: "17+",
    desc: "Atividades funcionais complexas e retorno gradual ao esporte/trabalho.",
  },
];
...
function ExerciseTimeline() {
  return (
    <div
      data-testid="exercise-timeline"
      className="space-y-4 bg-primary/5 p-4 rounded-xl border border-primary/10"
    >
          ...
          <div key={i} className="relative flex gap-4 items-start">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2">
                ...
                <Badge
                  variant="outline"
                  className="text-[9px] h-4 px-1.5 font-bold uppercase border-primary/30 text-primary bg-primary/5"
                >
                  Sem. {p.weeks}
                </Badge>
              </div>
        ...
        <AlertDialogContent className="rounded-2xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-sm">
              Esta ação removerá o template permanentemente da biblioteca. Planos de pacientes que
              já utilizam este template não serão alterados.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <div className="relative z-10 space-y-6">
          <div className="flex items-start justify-between gap-6">
            <div className="space-y-3 flex-1">
              <div className="flex flex-wrap items-center gap-2">
                <Badge
                  ...
                  className={cn(
                    "text-[9px] h-5 font-black uppercase tracking-widest",
                    !isSystem
                ...
                {template.conditionName && (
                  <Badge
                    variant="outline"
                    className="text-[9px] h-5 font-black uppercase tracking-widest bg-white/50 border-muted-foreground/10 text-muted-foreground flex items-center gap-1"
                  >
            ...
            {template.evidenceLevel && (
              <div className="flex flex-col items-center justify-center h-20 w-16 rounded-2xl bg-white border border-amber-100 shadow-lg shadow-amber-500/10 shrink-0">
                <span className="text-[8px] font-black uppercase text-amber-500/60 tracking-[0.2em] mb-1">
                  Evidence
                </span>
          ...
          <div className="flex flex-wrap gap-3 pt-2">
            <Button
              onClick={onApply}
              size="lg"
              className="h-12 px-8 gap-3 font-black uppercase tracking-tighter rounded-2xl shadow-xl shadow-primary/25 active:scale-95 transition-all"
            >
            ...
            <div className="flex gap-2">
              {isSystem ? (
                <Button
                  variant="secondary"
                  onClick={onCustomize}
                  className="h-12 px-6 gap-2 font-bold rounded-2xl bg-white border border-muted-foreground/10 hover:bg-muted/50"
                >
                ...
                <>
                  <Button
                    variant="secondary"
                    onClick={onEdit}
                    className="h-12 px-6 gap-2 font-bold rounded-2xl bg-white border border-muted-foreground/10"
                  >
                    <Edit className="h-4 w-4 text-muted-foreground" />
                    Editar
                  </Button>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto pr-2 scrollbar-thin">
              ...
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 gap-4">
                  {template.items?.map((item, idx) => (
                    <div
                      key={item.id || idx}
                      className="group relative flex flex-col sm:flex-row gap-6 p-5 rounded-[2rem] border-2 border-muted hover:border-primary/20 bg-card transition-all hover:shadow-2xl hover:shadow-primary/5"
                    >
                      {/* Thumbnail Container */}
                        ...
                        <div className="flex flex-wrap items-center gap-3 mt-4">
                          ...
                          {item.repetitions && (
                            <div className="flex items-center gap-2.5 px-3 py-1.5 rounded-xl bg-blue-500/[0.03] border border-blue-500/10 transition-colors group-hover:bg-blue-500/5">
                              ...
                              <div className="flex flex-col leading-none">
                                <span className="text-[8px] font-black uppercase text-blue-500/60 tracking-widest mb-0.5">
                                  Repetições
                                </span>
                          ...
                          {item.duration && (
                            <div className="flex items-center gap-2.5 px-3 py-1.5 rounded-xl bg-amber-500/[0.03] border border-amber-500/10 transition-colors group-hover:bg-amber-500/5">
                              ...
                              <div className="flex flex-col leading-none">
                                <span className="text-[8px] font-black uppercase text-amber-500/60 tracking-widest mb-0.5">
                                  Tempo
                                </span>
          ...
          <TabsContent
            ...
            className="mt-0 space-y-6 focus-visible:ring-0 pb-6"
          >
          ...
          <TabsContent value="referencias" className="mt-0 focus-visible:ring-0 pb-6">
            <div className="space-y-4">
              <h4 className="text-[10px] font-black uppercase text-muted-foreground tracking-widest px-1">
                Base Científica
              </h4>
              {template.bibliographicReferences && template.bibliographicReferences.length > 0 ? (
                <div className="space-y-3">
                  {template.bibliographicReferences.map((ref, i) => (
                    <div
                      key={i}
                      className="flex gap-4 p-4 rounded-2xl bg-muted/30 border border-muted-foreground/10 hover:bg-muted/50 transition-colors"
                    >
                      <div className="h-6 w-6 shrink-0 flex items-center justify-center rounded-lg bg-primary/10 text-primary text-[10px] font-black">
                        {i + 1}
                      </div>
                ...
                <EmptyContent
                  label="Nenhuma referência bibliográfica anexada a este template."
                  icon={BookOpen}
                />
              )}
            </div>
