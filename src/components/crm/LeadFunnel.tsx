...
export function LeadFunnel({ leads, estagios }: LeadFunnelProps) {
	...
	const efetivados =
		funnelData.find((f) => f.value === "efetivado")?.count || 0;
	const naoEfetivados =
		funnelData.find((f) => f.value === "nao_efetivado")?.count || 0;
	const emAndamento = total - efetivados - naoEfetivados;
	const taxaConversao =
		total > 0 ? ((efetivados / total) * 100).toFixed(1) : "0";
	const taxaPerda =
		total > 0 ? ((naoEfetivados / total) * 100).toFixed(1) : "0";
				...
				<CardContent>
					<div className="space-y-4">
						{estagiosAtivos.map((estagio, index) => {
							...
							const previousCount =
								index > 0 ? estagiosAtivos[index - 1].count : 0;
							...
							{Array.from(
								...
								const taxaOrigem =
									leadsOrigem.length > 0
										? ((convertidosOrigem / leadsOrigem.length) * 100).toFixed(
												0,
											)
										: "0";
