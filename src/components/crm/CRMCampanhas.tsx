...
import {
  ...
  Play, Trash2, Users, BarChart3, Loader2, Megaphone
} from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Envie mensagens em massa para seus leads</p>
        </div>
        <Button onClick={() => setIsDialogOpen(true)} className="rounded-xl gap-2 shadow-lg">
        ...
        {campanhas.length === 0 ? (
          <Card className="col-span-full p-12 border-dashed border-2">
            <div className="text-center text-muted-foreground">
              ...
              <p className="font-semibold text-slate-600">Nenhuma campanha criada</p>
              <p className="text-sm mt-1">Engaje seus leads com envios em massa via WhatsApp ou Email.</p>
              <Button variant="outline" onClick={() => setIsDialogOpen(true)} className="mt-6 rounded-xl">
          ...
          campanhas.map(campanha => {
            ...
            return (
              <Card key={campanha.id} className="hover:shadow-md transition-all border-slate-200 overflow-hidden">
                ...
                <CardContent className="p-5">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-bold text-slate-800 line-clamp-1">{campanha.nome}</h3>
                        <div className="flex items-center gap-2 mt-1">
                          <Badge className={cn("text-[10px] h-4", statusInfo.color)}>{statusInfo.label}</Badge>
                          <span className="text-[10px] text-slate-400">
                            {format(new Date(campanha.created_at), "dd MMM yyyy", { locale: ptBR })}
                          </span>
                        </div>
                      </div>
                    </div>
                      ...
                      <AlertDialogContent className="rounded-2xl">
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            Esta ação removerá todos os dados de métricas desta campanha e não poderá ser desfeita.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                  ...
                  {campanha.descricao && (
                    <p className="text-xs text-slate-500 mt-3 line-clamp-2">{campanha.descricao}</p>
                  )}

                  <div className="grid grid-cols-2 gap-2 mt-4">
                    <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
                      <p className="text-[10px] text-slate-400 font-bold uppercase">Destinatários</p>
                      <div className="flex items-center gap-1 mt-1">
                        <Users className="h-3 w-3 text-slate-400" />
                        <span className="text-sm font-bold text-slate-700">{campanha.total_destinatarios}</span>
                      </div>
                    </div>
                    <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
                      ...
                      <div className="flex items-center gap-1 mt-1">
                        ...
                        <span className="text-sm font-bold text-slate-700">{campanha.total_enviados || 0}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 mt-4">
                    {campanha.status === 'rascunho' && (
                      <Button size="sm" className="w-full rounded-lg gap-2 bg-slate-900" onClick={() => handleEnviar(campanha)}>
                        <Play className="h-3.5 w-3.5" />
                        Iniciar Disparo
                      </Button>
                    )}
                    {campanha.status === 'enviando' && (
                      <Button size="sm" variant="outline" className="w-full rounded-lg gap-2 border-rose-200 text-rose-600" onClick={() => updateMutation.mutate({ id: campanha.id, status: 'pausada' })}>
                        <Pause className="h-3.5 w-3.5" />
                        Pausar Envios
                      </Button>
                    )}
                    {campanha.status === 'pausada' && (
                      <Button size="sm" variant="outline" className="w-full rounded-lg gap-2" onClick={() => handleEnviar(campanha)}>
                        <Play className="h-3.5 w-3.5" />
                        Retomar
                      </Button>
                    )}
                    {campanha.status === 'concluida' && (
                      <Button size="sm" variant="ghost" className="w-full rounded-lg gap-2 text-emerald-600 bg-emerald-50 cursor-default hover:bg-emerald-50">
      ...
      >
        ...
        <CustomModalBody className="p-0 sm:p-0">
          <div className="px-6 py-4 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label className="font-semibold text-xs text-muted-foreground">Canal de Comunicação</Label>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2 mt-1">
                <p className="text-[10px] text-slate-400 font-bold uppercase">Variáveis: </p>
                <code className="text-[10px] bg-slate-100 px-1 rounded">{"{nome}"}</code>
                <code className="text-[10px] bg-slate-100 px-1 rounded">{"{telefone}"}</code>
                <code className="text-[10px] bg-slate-100 px-1 rounded">{"{email}"}</code>
              </div>
            </div>
                ...
                {ESTAGIOS.map(estagio => (
                  <Badge
                    ...
                    className={cn(
                      ...
                      formData.filtro_estagios.includes(estagio.value) ? "bg-slate-900" : "bg-white"
                    )}
            ...
            <div className="p-4 bg-primary/5 border border-primary/10 rounded-2xl flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Destinatários Estimados</p>
                </div>
              </div>
        ...
        <CustomModalFooter isMobile={isMobile}>
          <Button variant="ghost" type="button" onClick={() => setIsDialogOpen(false)} className="rounded-xl">
            Cancelar
          </Button>
          <Button 
            type="button" 
            onClick={() => handleSubmit()} 
            disabled={createMutation.isPending || !formData.nome || !formData.conteudo}
