...
export function LeadsContent() {
	...
	const leadsPorEstagio = ESTAGIOS.reduce(
		(acc, estagio) => {
			acc[estagio.value] = filteredLeads.filter(
				(l) => l.estagio === estagio.value,
			);
			return acc;
		},
	...
	const handleDragEnd = async (result: DropResult) => {
		...
		if (lead && lead.estagio !== novoEstagio) {
			...
			toast.success(
				`Lead movido para ${ESTAGIOS.find((e) => e.value === novoEstagio)?.label}`,
			);
		}
	};
	...
	const _getEstagioInfo = (estagio: string) =>
			...
			<div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
				<div>
					...
					<p className="text-muted-foreground mt-1">
						Gerencie seus potenciais clientes e acompanhe conversões
					</p>
				</div>
					...
					<div className="flex gap-4 overflow-x-auto pb-4">
						{ESTAGIOS.map((estagio) => (
							<Droppable key={estagio.value} droppableId={estagio.value}>
								{(provided, snapshot) => (
									<Card
										className={`min-w-[280px] flex-shrink-0 transition-colors ${
										...
										<CardHeader className="pb-2">
											<CardTitle className="text-sm flex items-center justify-between">
												<div className="flex items-center gap-2">
													<div
														className={`w-3 h-3 rounded-full ${estagio.color}`}
													/>
													{estagio.label}
												</div>
												...
												>
													{(provided, snapshot) => (
														<div
															...
															className={`p-3 bg-card border rounded-lg cursor-pointer hover:shadow-md transition-all ${
																snapshot.isDragging
