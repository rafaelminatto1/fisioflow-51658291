				...
				<CardHeader className="border-b border-slate-100 dark:border-slate-800 bg-slate-50/50">
					<div className="flex items-center justify-between">
						<div>
							...
							<CardDescription>
								Desempenho dos fluxos ativos no Inngest
							</CardDescription>
						</div>
						...
						{stats.activeAutomations.map((auto) => (
							...
							>
								<div className="flex items-center gap-3">
									<div
										className={`p-2 rounded-xl ${auto.status === "active" ? "bg-emerald-100 text-emerald-600" : "bg-slate-100 text-slate-400"}`}
									>
								...
								<div className="flex items-center gap-6">
									<div className="text-right">
										<div className="text-sm font-medium">
											{auto.success}% sucesso
										</div>
										<Progress value={auto.success} className="w-24 h-1 mt-1" />
									</div>
									<div
										className={`h-2 w-2 rounded-full ${auto.status === "active" ? "bg-emerald-500 animate-pulse" : "bg-slate-300"}`}
									/>
								</div>
							</div>
						))}
