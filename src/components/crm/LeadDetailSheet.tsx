...
import {
  ...
  Plus, User, History, ExternalLink, MessageSquare, ChevronRight, CheckCircle2
} from 'lucide-react';
...
const TIPOS_CONTATO = [
  { value: 'whatsapp', label: 'WhatsApp', icon: <MessageSquare className="h-4 w-4 text-emerald-500" /> },
  { value: 'ligacao', label: 'Ligação', icon: <Phone className="h-4 w-4 text-blue-500" /> },
  { value: 'email', label: 'Email', icon: <Mail className="h-4 w-4 text-purple-500" /> },
  { value: 'presencial', label: 'Presencial', icon: <User className="h-4 w-4 text-orange-500" /> },
  { value: 'instagram', label: 'Instagram', icon: <History className="h-4 w-4 text-pink-500" /> },
];
    ...
    >
      <CustomModalHeader onClose={onClose}>
        <div className="flex flex-col gap-1">
          <Badge className={cn("w-fit rounded-lg px-2 py-0.5 text-[10px] font-bold uppercase", estagioInfo.color, "text-white")}>
            {estagioInfo.label}
          </Badge>
        ...
        <Button variant="outline" size="sm" onClick={() => onEdit(lead)} className="ml-auto mr-4 rounded-xl gap-2">
            ...
            <Card className="border-slate-100 shadow-sm bg-slate-50/50">
              <CardContent className="p-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  {lead.telefone && (
                    <Button variant="ghost" size="sm" onClick={handleWhatsApp} className="h-8 text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50 rounded-lg">
            ...
            <Card className="border-slate-100 shadow-sm bg-slate-50/50">
              <CardContent className="p-4 space-y-2">
                <div className="flex items-center gap-2 text-xs">
                  ...
                  <Badge variant="outline" className="rounded-lg bg-white border-slate-200">{lead.origem || 'Não informada'}</Badge>
                </div>
                <div className="flex items-center gap-2 text-xs">
                ...
                <CardContent className="p-4 space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <Label className="text-[10px] font-bold uppercase text-slate-500">Canal</Label>
                      <Select 
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-[10px] font-bold uppercase text-slate-500">Próximo Contato</Label>
                      <Input
                        type="date"
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-[10px] font-bold uppercase text-slate-500">O que foi conversado?</Label>
                    <Textarea
                  ...
                  <div className="flex justify-end gap-2">
                    <Button variant="ghost" size="sm" onClick={() => setShowAddHistorico(false)} className="rounded-lg h-8">
                      Cancelar
                    </Button>
                    <Button 
                      size="sm" 
                    ...
                    <div key={h.id} className="relative flex gap-4">
                      ...
                      <div className="flex-1 pb-6">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-sm font-bold text-slate-800">{tipoInfo?.label || h.tipo_contato}</span>
                          <span className="text-[10px] font-medium text-slate-400 bg-slate-50 px-2 py-0.5 rounded-full border border-slate-100">
                            {format(new Date(h.created_at), "dd MMM, HH:mm", { locale: ptBR })}
                          </span>
                        </div>
                        {h.descricao && <p className="text-sm text-slate-600 leading-relaxed">{h.descricao}</p>}
                        {h.resultado && (
                          <p className="mt-2 text-xs font-medium text-slate-500 bg-slate-50 p-2 rounded-lg border border-slate-100">
                            <span className="font-bold text-slate-400 uppercase mr-1">Resultado:</span> {h.resultado}
                          </p>
                        )}
                        {h.proximo_contato && (
      ...
      <CustomModalFooter isMobile={isMobile} className="bg-slate-50 border-t-0">
        <Button variant="ghost" onClick={onClose} className="rounded-xl h-11 px-6 font-bold text-slate-500">
          Fechar Detalhes
        </Button>
      </CustomModalFooter>
    </CustomModal>
