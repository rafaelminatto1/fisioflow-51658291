...
export function LeadScoring({ leadId, showSettings = false }: LeadScoringProps) {
  const [activeTab, setActiveTab] = useState<"overview" | "leads" | "rules" | "settings">(
  ...
  const { data: scores = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const leadsById = new Map((leadsResult?.data ?? []).map((lead: any) => [lead.id, lead]));
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      avgScore: Math.round(
        typedScores.reduce((acc, s) => acc + (s.total_score || 0), 0) / scores.length,
      ),
    };
  }, [scores]);
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs
        value={activeTab}
        onValueChange={(v) => setActiveTab(v as "overview" | "leads" | "rules" | "settings")}
      >
                  ...
                  <TableBody>
                    {(
                      ...
                      <TableRow key={score.id}>
                        <TableCell className="font-medium">{score.leads?.name || "Lead"}</TableCell>
                        <TableCell>
                          <div className="flex items-center gap-2">
              ...
              <div className="bg-blue-50 dark:bg-blue-950/30 p-4 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm text-blue-700 dark:text-blue-400">
                    ...
                    <p className="text-xs">
                      O sistema calcula automaticamente a pontuação de cada lead baseado em fatores
                      demográficos, engajamento e comportamentais. Leads com pontuação mais alta
