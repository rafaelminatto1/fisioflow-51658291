...
export function LeadScoring({
	...
	const stats = useMemo(() => {
		...
		return {
			...
			avgScore: Math.round(
				typedScores.reduce((acc, s) => acc + (s.total_score || 0), 0) /
					scores.length,
			),
		};
	}, [scores]);
	...
	return (
		<div className="space-y-6">
			...
			<Tabs
				value={activeTab}
				onValueChange={(v) =>
					setActiveTab(v as "overview" | "leads" | "rules" | "settings")
				}
			>
						...
						<CardContent className="space-y-6">
							<div className="flex items-center justify-between">
								<div>
									...
									<p className="text-sm text-muted-foreground">
										Recalcular scores automaticamente quando há mudanças nos
										leads
									</p>
								</div>
							...
							<div className="bg-blue-50 dark:bg-blue-950/30 p-4 rounded-lg">
								<div className="flex items-start gap-3">
									...
									<div className="text-sm text-blue-700 dark:text-blue-400">
										...
										<p className="text-xs">
											O sistema calcula automaticamente a pontuação de cada lead
											baseado em fatores demográficos, engajamento e
											comportamentais. Leads com pontuação mais alta devem ser
											priorizados.
										</p>
									</div>
								</div>
