...
import { Pen, Eraser, Download, Check, FileSignature, Shield, Clock } from "lucide-react";
...
export function DigitalSignature({
  ...
  const handleSign = async () => {
    ...
    const hash = await generateHash(
      `${documentId}-${name}-${timestamp}-${imageData.slice(0, 100)}`,
    );
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {/* Informações legais */}
        <p className="text-xs text-muted-foreground text-center">
          Ao assinar, você concorda com os termos do documento e declara que as informações são
          verdadeiras. Esta assinatura tem validade legal conforme MP 2.200-2/2001.
        </p>
      </CardContent>
    </Card>
  );
