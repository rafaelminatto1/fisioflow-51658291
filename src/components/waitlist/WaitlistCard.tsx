...
import { type WaitlistEntry, formatPreferences, PRIORITY_CONFIG } from "@/hooks/useWaitlist";
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex-1 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <Badge
                variant={
                  priorityConfig.color as "default" | "secondary" | "destructive" | "outline"
                }
              >
              ...
              {entry.refusal_count > 0 && (
                <Badge variant="outline" className="text-muted-foreground">
                  {entry.refusal_count} recusa
                  {entry.refusal_count > 1 ? "s" : ""}
                </Badge>
              )}
            </div>
            ...
            {isOffered && entry.offered_slot && (
              ...
              >
                ...
                <p className="text-sm">
                  {format(new Date(entry.offered_slot), "EEEE, dd 'de' MMMM 'às' HH:mm", {
                    locale: ptBR,
                  })}
                </p>
                {entry.offer_expires_at && (
                  <p
                    className={`text-xs ${isExpiringSoon ? "text-yellow-600" : "text-muted-foreground"}`}
                  >
                    {isExpiringSoon && <AlertTriangle className="w-3 h-3 inline mr-1" />}
                    Expira{" "}
                    {formatDistanceToNow(new Date(entry.offer_expires_at), {
                      locale: ptBR,
                      addSuffix: true,
                    })}
                  </p>
