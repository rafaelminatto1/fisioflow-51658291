    ...
    >
      <CustomModalHeader onClose={() => onOpenChange(false)}>
        <CustomModalTitle className="flex items-center gap-2">
          {isEditing ? <ClipboardList className="h-5 w-5 text-primary" /> : <UserPlus className="h-5 w-5 text-primary" />}
      ...
      <CustomModalBody className="p-0 sm:p-0">
        <div className="px-6 py-4 space-y-6">
          <p className="text-sm text-muted-foreground">
            {isEditing ? 'Atualize os dados do paciente na lista de espera' : 'Adicione um paciente à lista de espera para agendamentos futuros'}
          </p>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {DAYS_OF_WEEK.map(day => (
                <div key={day.value} className="flex items-center space-x-2 bg-slate-50 p-2 rounded-lg border border-slate-100">
                  ...
                  <label htmlFor={`day-${day.value}`} className="text-sm font-medium cursor-pointer">{day.label}</label>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {TIME_SLOTS.map(slot => (
                <div key={slot.value} className="flex items-center space-x-2 bg-slate-50 p-2 rounded-lg border border-slate-100">
                  <Checkbox id={`slot-${slot.value}`} checked={selectedTimeSlots.includes(slot.value)} onCheckedChange={() => handleTimeSlotToggle(slot.value)} />
                  <label htmlFor={`slot-${slot.value}`} className="text-sm font-medium cursor-pointer">{slot.label}</label>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <RadioGroup value={priority} onValueChange={(v: 'normal' | 'high' | 'urgent') => setPriority(v)} className="flex flex-wrap gap-4">
              <div className="flex items-center space-x-2"><RadioGroupItem value="normal" id="normal" /><Label htmlFor="normal" className="cursor-pointer">Normal</Label></div>
              <div className="flex items-center space-x-2"><RadioGroupItem value="high" id="high" /><Label htmlFor="high" className="cursor-pointer text-orange-600">Alta</Label></div>
              <div className="flex items-center space-x-2"><RadioGroupItem value="urgent" id="urgent" /><Label htmlFor="urgent" className="cursor-pointer text-red-600">Urgente</Label></div>
            </RadioGroup>
          </div>
          ...
          {priority !== 'normal' && (
            <div className="animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              <Input id="priority-reason" value={priorityReason} onChange={(e) => setPriorityReason(e.target.value)} placeholder="Ex: Dor aguda, pós-operatório imediato" className="mt-1" />
            </div>
          )}

          <div>
            <Label htmlFor="notes">Observações Internas</Label>
            <Textarea id="notes" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Informações adicionais relevantes..." rows={3} className="mt-1" />
          </div>
        </div>
      </CustomModalBody>

      <CustomModalFooter isMobile={isMobile}>
        <Button variant="ghost" onClick={() => onOpenChange(false)} disabled={isPending} className="rounded-xl">
          Cancelar
        </Button>
        <Button 
