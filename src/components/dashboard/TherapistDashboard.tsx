...
export function TherapistDashboard({ lastUpdate, profile, period = 'hoje' }: TherapistDashboardProps) {
  ...
  const dashboardStats = useMemo(() => {
    ...
    const periodLabel = period === 'semana' ? 'Esta Semana' : period === 'mes' ? 'Mês Atual' : 'Hoje';

    const todayApts = allAppointments
      .filter((apt) => {
        const aptDate = new Date(apt.date);
        const inPeriod =
          period === 'hoje' ? isSameDay(aptDate, today) : isWithinInterval(aptDate, periodRange);
    ...
    const recentAptsCount = allAppointments.filter(
      (apt) => new Date(apt.date) >= thirtyDaysAgo && apt.therapistId === profile.id
    ).length;

    const avgSessionsPerPatient = myPatients > 0 ? parseFloat((recentAptsCount / myPatients).toFixed(1)) : 0;
        ...
        <CardContent className="flex flex-col gap-4 p-5 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:w-[360px]">
            <div className="rounded-2xl border border-border/60 bg-background px-4 py-3 shadow-sm">
              ...
              <p className="mt-2 text-lg font-semibold text-foreground">{stats.completedSessions}</p>
            </div>
            <div className="rounded-2xl border border-border/60 bg-background px-4 py-3 shadow-sm">
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                Satisfação média
              </p>
              <p className="mt-2 text-lg font-semibold text-foreground">{stats.avgSatisfaction.toFixed(1)}</p>
            </div>
          </div>
        </CardContent>
      </Card>
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardTitle className="mt-1 text-lg font-semibold">Atendimentos do período</CardTitle>
                </div>
              </div>
          ...
          <CardContent className="p-5">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-sm font-medium text-foreground">Nenhum agendamento neste período</p>
                <p className="mt-1 text-xs text-muted-foreground">A agenda será atualizada assim que novos atendimentos entrarem.</p>
              </div>
            ) : (
              <div className="space-y-3">
            ...
            <div className="mt-4">
              <Button variant="outline" className="rounded-xl border-border/60 bg-background/80" onClick={() => navigate('/agenda')}>
                Ver agenda completa
              </Button>
            </div>
          </CardContent>
            ...
            <CardHeader className="border-b border-border/60 pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardTitle className="mt-1 text-lg font-semibold">Ações rápidas</CardTitle>
                </div>
              </div>
            </CardHeader>

            <CardContent className="grid gap-3 p-5 sm:grid-cols-2">
              <Button className="h-24 flex-col items-start justify-between rounded-2xl px-4 py-4 text-left" onClick={() => navigate('/pacientes')}>
                <Users className="h-5 w-5" />
                <span className="text-sm font-semibold">Novo paciente</span>
              </Button>
              <Button variant="outline" className="h-24 flex-col items-start justify-between rounded-2xl border-border/60 bg-background" onClick={() => navigate('/agenda')}>
                <Calendar className="h-5 w-5 text-primary" />
                <span className="text-sm font-semibold">Agendar</span>
              </Button>
              <Button variant="outline" className="h-24 flex-col items-start justify-between rounded-2xl border-border/60 bg-background" onClick={() => navigate('/relatorios')}>
                <TrendingUp className="h-5 w-5 text-primary" />
                <span className="text-sm font-semibold">Relatórios</span>
              </Button>
              <Button variant="outline" className="h-24 flex-col items-start justify-between rounded-2xl border-border/60 bg-background" onClick={() => navigate('/comunicacao')}>
            ...
            <CardHeader className="border-b border-border/60 pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardTitle className="mt-1 text-lg font-semibold">Evolução global</CardTitle>
                </div>
              </div>
            </CardHeader>
            <CardContent className="p-5">
              <ChartWidget title="" data={progressData} type="line" loading={isLoading} height={260} />
            </CardContent>
          </Card>
        </div>
      </div>
