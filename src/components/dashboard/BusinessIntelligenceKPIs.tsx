...
export function BusinessIntelligenceKPIs() {
  ...
  const getStatusBg = (ratio: number | null) => {
    ...
    if (ratio >= 3)
      return "bg-emerald-50 border-emerald-100 dark:bg-emerald-950/20 dark:border-emerald-900/50";
    if (ratio >= 1)
      return "bg-amber-50 border-amber-100 dark:bg-amber-950/20 dark:border-amber-900/50";
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent className="max-w-xs text-xs">
                    Lifetime Value: Receita total estimada que um paciente gera durante o período de
                    tratamento.
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent className="max-w-xs text-xs">
                    Relação entre o valor do paciente (LTV) e o custo de aquisição (CAC). Benchmark
                    ideal: {">"} 3:1.
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            ...
            <div>
              ...
              <h4
                className={cn("text-3xl font-black tracking-tighter", getStatusColor(ltvCacRatio))}
              >
              ...
              <div className="mt-4 h-1.5 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                <motion.div
                  initial={{ width: 0 }}
                  animate={{
                    width: ltvCacRatio ? `${Math.min(100, (ltvCacRatio / 6) * 100)}%` : 0,
                  }}
                  className={cn(
                    "h-full",
                    ltvCacRatio && ltvCacRatio >= 3 ? "bg-emerald-500" : "bg-amber-500",
                  )}
                />
              </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent className="max-w-xs text-xs">
                    Tempo médio (em meses) para que a receita do paciente cubra seu custo de
                    aquisição.
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            ...
            <div>
              ...
              <div className="mt-4 h-1.5 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                <motion.div
                  ...
                  className={cn(
                    "h-full",
                    paybackMonths && paybackMonths <= 3 ? "bg-emerald-500" : "bg-primary",
                  )}
                />
              </div>
              <p className="text-[10px] font-bold text-slate-500 mt-2 uppercase tracking-wider">
        ...
        <div className="pt-8 border-t border-slate-100 dark:border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-900 p-2 rounded-2xl border border-slate-100 dark:border-slate-800">
            <div className="relative">
              <span className="absolute left-3 top-3 text-[10px] font-black text-slate-400">
                R$
              </span>
              <Input
                type="text"
