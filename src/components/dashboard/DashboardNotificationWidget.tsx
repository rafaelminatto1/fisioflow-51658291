...
import { Bell, BellOff, CheckCheck, Calendar, DollarSign, Users, Info, AlertTriangle, MessageSquare } from 'lucide-react';
...
function NotificationItem({ n, onMarkRead }: { n: Notification; onMarkRead: (id: string) => void }) {
  ...
  const colorClass = NOTIFICATION_COLORS[n.type] ?? 'bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400';
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className={cn('text-xs font-bold truncate', n.is_read ? 'text-slate-500 dark:text-slate-400' : 'text-slate-900 dark:text-white')}>
          {n.title}
        </p>
        {n.message && (
...
export function DashboardNotificationWidget() {
  ...
  const { notifications, unreadCount, isLoading, markAsRead, markAllAsRead } = useNotifications(10);
      ...
      <CardContent className="p-4">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-[11px] text-slate-400 mt-1">Nenhuma notificação pendente</p>
          </div>
        ) : (
