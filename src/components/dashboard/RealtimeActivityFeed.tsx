...
type AppointmentDocument = AppointmentRow & { created_at?: string };
  ...
  useEffect(() => {
    ...
    const loadRecentActivities = async () => {
      try {
        ...
        const appointments = (appointmentsResponse?.data ?? []) as AppointmentDocument[];

        const activityList: ActivityEvent[] = appointments.map((apt) => {
          const timestamp =
            apt.created_at && !Number.isNaN(new Date(apt.created_at).getTime())
              ? new Date(apt.created_at)
              : new Date(`${apt.date}T${apt.start_time ?? "00:00:00"}`);

          return {
            id: apt.id,
            type: "appointment",
            title: "Novo Agendamento",
            description: `${apt.patient_name || "Paciente"} - ${safeFormatDate(`${apt.date}T${apt.start_time ?? "00:00:00"}`)}`,
    ...
    const intervalId = window.setInterval(loadRecentActivities, 30000);
              ...
              {activities.map((activity, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`mt-0.5 w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-transform duration-300 group-hover:scale-110 ${
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          <p className="font-bold text-sm text-slate-900 dark:text-white leading-none mb-1.5">
                            {activity.title}
                          </p>
