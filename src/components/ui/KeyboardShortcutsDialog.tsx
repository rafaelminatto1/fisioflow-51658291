...
import {
  ...
  DEFAULT_SHORTCUTS,
} from "@/lib/keyboard/shortcuts-manager";
      ...
      <div className="flex items-center gap-1">
        {keys.split("+").map((key, i) => (
          <span key={i} className="flex items-center">
            ...
            {i < keys.split("+").length - 1 && <span className="mx-0.5 text-slate-500">+</span>}
          </span>
        ))}
      </div>
    </div>
...
function CategoryTab({ name, shortcuts }: CategoryTabProps) {
  ...
  return (
    <div className="space-y-2">
      <div
        className={`flex items-center gap-2 pb-2 border-b border-slate-200 dark:border-slate-700 ${colorClass}`}
      >
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden">
        <DialogHeader>
          ...
          <DialogDescription>
            Pressione{" "}
            <kbd className="px-1.5 py-0.5 text-xs font-semibold bg-slate-100 border border-slate-300 rounded dark:bg-slate-700 dark:border-slate-600">
              ?
            </kbd>{" "}
            a qualquer momento para abrir este diálogo
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-slate-700 text-xs text-slate-500">
          <span>
            Dica: Atalhos com{" "}
            <kbd className="px-1 py-0.5 text-xs bg-slate-100 border border-slate-300 rounded dark:bg-slate-700 dark:border-slate-600">
              ⌘
            </kbd>{" "}
            funcionam com{" "}
            <kbd className="px-1 py-0.5 text-xs bg-slate-100 border border-slate-300 rounded dark:bg-slate-700 dark:border-slate-600">
          ...
          <span>{allShortcuts.length} atalhos disponíveis</span>
        </div>
      </DialogContent>
    </Dialog>
  );
...
export function KeyboardShortcutsTrigger({
  ...
  return (
    <>
      ...
      {variant === "badge" && (
        <Badge variant="outline" className={className} onClick={() => setOpen(true)}>
