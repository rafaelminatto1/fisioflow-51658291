...
export const CommandList = forwardRef((props: CommandListProps, ref) => {
	...
	props.items.forEach((item, idx) => {
		...
		else if (item.category === "exercicios")
			exercicios.push({ item, globalIndex: idx });
	...
	if (formatacao.length > 0)
		groups.push({ key: "fmt", label: "Formatação", items: formatacao });
	if (media.length > 0)
		groups.push({ key: "med", label: "Mídia & Inserir", items: media });
	if (clinico.length > 0)
		groups.push({ key: "cli", label: "Blocos Clínicos", items: clinico });
	if (exercicios.length > 0)
		groups.push({
			key: "exs",
			label: "Exercícios Sugeridos",
			items: exercicios,
		});
