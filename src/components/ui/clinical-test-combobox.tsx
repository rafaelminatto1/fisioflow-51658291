		...
		<Popover open={open} onOpenChange={setOpen}>
			<PopoverTrigger asChild>
				<Button
					...
					aria-expanded={open}
					aria-label={
						selectedTest ? `Teste: ${selectedTest.name}` : placeholder
					}
					className={cn(
						"w-full justify-between bg-background h-11 border-teal-100 hover:border-teal-300 hover:bg-teal-50/30 transition-all focus:ring-2 focus:ring-teal-400 focus:ring-offset-2",
						className,
					)}
					disabled={disabled}
				>
							...
							{selectedTest.category && (
								<Badge
									...
									className={cn(
										...
										getCategoryColor(selectedTest.category),
									)}
								>
									{selectedTest.category}
								</Badge>
			...
			<PopoverContent
				className="w-[450px] p-0 shadow-2xl border-teal-100 rounded-xl overflow-hidden"
				align="start"
			>
				<Command shouldFilter={false} className="rounded-none">
					<div
						className="flex items-center border-b px-3 bg-slate-50/50"
						cmdk-input-wrapper=""
					>
														...
														>
															<div className="space-y-1.5">
																...
																<p className="text-[11px] text-slate-600 leading-relaxed font-medium">
																	<span className="text-teal-600 font-bold mr-1">
																		Propósito:
																	</span>
																	{test.purpose}
																</p>
