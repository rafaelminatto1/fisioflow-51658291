...
interface OptimizedImageProps extends Omit<ImgHTMLAttributes<HTMLImageElement>, "src"> {
  ...
  quality?: number;
  /** URLs alternativas para tentar antes do fallback visual */
  fallbackSrcs?: string[];
  onLoad?: () => void;
  onError?: () => void;
...
export function OptimizedImage({
  ...
  const normalizedFallbackSrcs = useMemo(() => fallbackSrcs.filter(Boolean), [fallbackSrcs]);
  ...
  useEffect(() => {
    ...
    logger.warn(
      "Imagem ignorada por src inválido; usando fallback",
      { src, alt },
      "OptimizedImage",
    );
  }, [alt, src, srcInvalid]);
  ...
  const handleError = () => {
    if (!srcInvalid && currentSrc === optimizedSrc && optimizedSrc !== src) {
      logger.warn(
        "Falha ao carregar imagem otimizada; tentando URL original",
        {
          src,
          optimizedSrc,
          alt,
    ...
    const nextFallbackIndex = normalizedFallbackSrcs.findIndex(
      (candidate, index) =>
        index >= fallbackIndex && candidate !== currentSrc && candidate !== optimizedSrc,
    );
    if (nextFallbackIndex !== -1) {
      setFallbackIndex(nextFallbackIndex + 1);
      setIsLoaded(false);
      setCurrentSrc(normalizedFallbackSrcs[nextFallbackIndex]);
      return;
    }

    if (currentSrc !== fallback) {
      logger.warn(
        "Falha ao carregar imagem original; usando fallback",
        {
          src,
          currentSrc,
          fallback,
    ...
    setHasError(true);
    logger.warn(
      "Falha definitiva ao carregar imagem",
      {
        src,
        fallback,
        alt,
