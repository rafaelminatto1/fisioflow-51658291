...
export const RichTextEditor: React.FC<RichTextEditorProps> = ({
			...
			if ((window as any).__REPLICATE_PREVIOUS_SESSION) {
				...
				if (lastSession) {
					editor
						...
						.insertContent(
							lastSession.plan || "Sem conduta registrada na última sessão",
						)
						.run();
	...
	return (
		<div
			...
			style={
				isTyping
					? ({ "--typing-glow": getAccentGlow() } as React.CSSProperties)
					: undefined
			}
		>
				...
				<DialogContent className="max-w-md bg-white">
					<DialogHeader>
						<DialogTitle className="flex items-center gap-2">
							<ClipboardCheck className="w-5 h-5 text-blue-600" /> Selecionar
							Teste Clínico
						</DialogTitle>
					</DialogHeader>
