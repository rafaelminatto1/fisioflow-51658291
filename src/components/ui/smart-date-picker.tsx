...
import { format, getYear, getMonth, setYear, setMonth, parse, isValid } from "date-fns";
...
export function SmartDatePicker({
  ...
  return (
    <Popover>
      ...
      <PopoverContent className="w-auto p-4 rounded-3xl shadow-2xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950" align="start">
        {showYearPicker && (
          <div className="flex gap-2 mb-4">
            ...
            >
              ...
              <SelectContent className="rounded-xl">
                {months.map((m, index) => (
                  <SelectItem key={m} value={index.toString()} className="text-xs">
                    {m}
                  </SelectItem>
                ))}
              </SelectContent>
        ...
        <Calendar
          ...
          classNames={{
            day_selected: "bg-primary text-white hover:bg-primary hover:text-white rounded-xl shadow-lg shadow-primary/30 font-bold",
            day_today: "bg-slate-100 dark:bg-slate-800 text-primary font-black rounded-xl",
