...
export const PerformanceMonitor = ({
	...
	useEffect(() => {
		const timer = setInterval(() => {
			const data = Array.from(metricsRef.current.entries()).map(
				([name, values]) => ({
					name,
					values,
							...
							<div className="px-4 py-3 space-y-3">
								<div className="grid grid-cols-3 gap-3">
									<MetricCard
										...
										markerColor={getStatusColor(
											performanceData.summary.avgLoadTime >
												PERFORMANCE_MARKERS.calendarLoad.threshold
												? "warning"
												: "ok",
										)}
									/>
									<MetricCard
										...
										markerColor={getStatusColor(
											performanceData.summary.avgRenderTime >
												PERFORMANCE_MARKERS.renderTime.threshold
												? "warning"
												: "ok",
										)}
									/>
									<MetricCard
										...
										value={(performanceData.summary.cacheHitRate * 100).toFixed(
											0,
										)}
										...
										markerColor={getStatusColor(
											performanceData.summary.cacheHitRate >=
												PERFORMANCE_MARKERS.cacheHitRate.threshold
												? "ok"
												: "warning",
										)}
									/>
									<MetricCard
										...
										markerColor={getStatusColor(
											performanceData.summary.cacheSize <=
												PERFORMANCE_MARKERS.cacheSize.threshold
												? "ok"
												: "warning",
										)}
									/>
							...
							<button
								onClick={() => {
									...
									toast({
										title: "Copiado!",
										description:
											"Dados de performance copiados para o clipboard.",
									});
								}}
		...
		return (
			<div className="bg-slate-50 dark:bg-slate-900/50 rounded-lg p-4">
				...
				<div className="flex items-baseline gap-1">
					...
					{marker && (
						<div
							className={cn("ml-2", markerColor)}
							title={`Marcador: ${marker}`}
						>
							<span>●</span>
						</div>
					)}
