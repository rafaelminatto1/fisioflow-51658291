...
const ToolbarSeparator = memo(
	({ orientation = "vertical" }: ToolbarSeparatorProps) => (
		<div
			className={cn(
				"bg-border",
	...
	React.forwardRef<HTMLTextAreaElement, SmartTextareaProps>(
		(
			...
			const handleDecreaseIndent = useCallback(() => {
				...
				const affectedText = text.substring(
					lineStart,
			...
			return (
				<div
					...
					onMouseLeave={() =>
						!showToolbarOnFocus &&
						!internalRef.current?.matches(":focus") &&
						setIsFocused(false)
					}
				>
					{/* Toolbar */}
					...
					{isFullScreen && (
						<div
							...
							aria-live="polite"
						>
							Pressione <kbd className="px-1 py-0.5 bg-muted rounded">Esc</kbd>{" "}
							para sair
						</div>
					)}
				</div>
