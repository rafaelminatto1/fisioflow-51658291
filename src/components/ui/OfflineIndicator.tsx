...
export function OfflineIndicator({
  ...
  const formatTimestamp = () => {
    ...
    try {
      return formatDistanceToNow(new Date(cacheTimestamp), {
        addSuffix: true,
        locale: ptBR,
      });
    } catch {
  ...
  if (minimal) {
    return (
      ...
      >
        <div className={styles.icon}>{getIcon(variant, isLoading)}</div>
    ...
    >
      <div className="flex items-center gap-3">
        <div className={cn("p-1.5 rounded-full", styles.bg)}>
          <span className={styles.icon}>{getIcon(variant, isLoading)}</span>
        </div>
        <div>
          <p className={cn("font-medium", styles.text)}>{getTitle(variant, isStale, dataSource)}</p>
          <p className={cn("text-xs opacity-80", styles.text)}>
            {errorMessage ? (
              ...
              <>
                {dataSource && (
                  <span className="font-semibold">{sourceLabels[dataSource] || dataSource}</span>
                )}
                {dataSource && " • "}
