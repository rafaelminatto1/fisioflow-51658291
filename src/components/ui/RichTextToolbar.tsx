...
export const RichTextToolbar: React.FC<RichTextToolbarProps> = ({
	...
	const commandItems = [
		...
		},
		{
			...
			preview:
				'<pre class="notion-code-block"><code>const exemplo = true;</code></pre>',
	...
	const selectedCommand =
		commandItems.find((item) => item.id === selectedCommandId) ||
		commandItems[0];
				...
				<ToolbarGroup>
					...
					<Popover>
						<PopoverTrigger asChild>
							<button
								className="rich-text-toolbar-btn group"
								title="Cor do texto"
							>
								<Palette
									className="h-4 w-4"
					...
					<DropdownMenu>
						...
						<DropdownMenuContent align="start">
							{LINE_HEIGHTS.map((item) => (
								<DropdownMenuItem
									key={item.value}
									onClick={() => setLineHeight(item.value as any)}
								>
									{item.label}
								</DropdownMenuItem>
							))}
					...
					<DropdownMenu>
						...
						<DropdownMenuContent align="start" className="w-56">
							<DropdownMenuItem onClick={() => imageInputRef.current?.click()}>
								<Image className="h-4 w-4 mr-2 text-rose-500" /> Imagem
								(arquivo)
							</DropdownMenuItem>
							...
							>
								<Minus className="h-4 w-4 mr-2 text-slate-300" /> Linha
								Divisória
							</DropdownMenuItem>
							<DropdownMenuSeparator />
				...
				{isInTable && (
					<ToolbarGroup className="bg-emerald-50/50 border-emerald-100 px-1 py-0.5 rounded-md animate-in slide-in-from-left-2 duration-300">
						...
						<DropdownMenu>
							<DropdownMenuTrigger asChild>
								<button
									className="rich-text-toolbar-btn text-rose-600"
									title="Excluir..."
								>
