      ...
      <DialogPortal>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none p-4">
          <DialogPrimitive.Content
            ref={ref}
            className={cn(
              "pointer-events-auto relative z-50 flex flex-col w-full max-w-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 sm:rounded-[16px] will-change-[transform,opacity] max-h-[85vh] overflow-hidden motion-reduce:animate-none motion-reduce:duration-0",
              "max-h-[calc(100dvh-2rem)] md:max-h-[85vh]",
              premium && "card-premium-hover",
              variantClasses[variant],
              "p-0 gap-0", // Padding and gap moved to internal container
              className,
            )}
            {...descriptionProps}
            {...props}
          >
            <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden p-6 grid gap-4">
              {children}
            </div>
            <DialogPrimitive.Close className="absolute right-4 top-4 z-[60] rounded-lg opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground will-change-[opacity] magnetic-button glow-on-hover">
              <X className="h-5 w-5" />
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn("text-lg font-black leading-none tracking-tight font-display", className)}
    {...props}
  />
));
