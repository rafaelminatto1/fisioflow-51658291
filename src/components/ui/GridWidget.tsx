...
export const GridWidget = forwardRef<HTMLDivElement, GridWidgetProps>(
	(
		...
		const handleMouseDown = (e: React.MouseEvent) => {
			if (
				isDraggable &&
				(e.currentTarget as HTMLElement).closest(".drag-handle")
			) {
				setIsGrabbed(true);
			}
			...
			>
				<MotionCard
					...
					className={cn(
						...
						isDraggable &&
							isGrabbed &&
							"shadow-xl ring-4 ring-primary/10 scale-[1.01] border-primary/50 z-[100]", // Higher z-index when grabbed
					)}
				>
					{(title ||
						icon ||
					...
					<CardContent
						className={cn(
							"p-0 flex-1 overflow-hidden flex flex-col",
							contentClassName,
						)}
					>
						{children}
