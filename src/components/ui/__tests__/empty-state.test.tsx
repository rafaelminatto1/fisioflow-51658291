...
describe("EmptyState Component", () => {
	...
	it("deve renderizar descrição quando fornecida", () => {
		render(
			<EmptyState title="Sem eventos" description="Crie seu primeiro evento" />,
		);
		expect(screen.getByText("Crie seu primeiro evento")).toBeInTheDocument();
	});

	it("deve renderizar ícone quando fornecido", () => {
		const { container } = render(<EmptyState title="Vazio" icon={Calendar} />);
	...
	it("deve renderizar botão de ação quando fornecido", () => {
		const onClick = vi.fn();
		render(
			<EmptyState title="Vazio" action={{ label: "Criar novo", onClick }} />,
		);
