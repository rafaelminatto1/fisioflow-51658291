...
interface PatientComboboxProps extends Omit<
...
export function PatientCombobox({
  ...
  const selectedPatient = React.useMemo(
    () =>
      ...
      (selectedPatientSnapshot?.id === value ? selectedPatientSnapshot : null),
    [patients, selectedPatientSnapshot, value],
  );
  ...
  const getPatientMeta = (patient?: Patient | null, description?: string) => {
    ...
    const details = [patient.phone, patient.cpf ? `CPF ${patient.cpf}` : null].filter(
      Boolean,
    ) as string[];
  ...
  const filteredPatients = React.useMemo(() => {
    ...
    const matchCpf = (p: Patient) => {
      ...
      if (normalizedDigits.length >= 2 && p.cpf.replace(/\D/g, "").includes(normalizedDigits))
        return true;
      return false;
    };
    ...
    const matchPhone = (p: Patient) => {
      ...
      if (normalizedPhone.includes(searchTerm) || p.phone.replace(/\D/g, "").includes(searchTerm))
        return true;
      if (normalizedDigits.length >= 2 && p.phone.replace(/\D/g, "").includes(normalizedDigits))
        return true;
      return false;
    };
        ...
        {(selectedPatient || hasFallbackDisplay) && (
          ...
          >
            ...
            >
              <span className="relative">
                {getPatientInitials(
                  selectedPatient ? getPatientName(selectedPatient) : fallbackDisplayName,
                )}
              </span>
            </div>
