...
function TestMessageDialog() {
	...
	const [message, setMessage] = useState(
		"Olá! Esta é uma mensagem de teste do Activity Fisioterapia.",
	);
...
export function WhatsAppDashboard() {
	...
	const { data: metrics, isLoading: loadingMetrics } =
		useWhatsAppMetricsSummary(30);
	const { data: messages, isLoading: loadingMessages } =
		useWhatsAppMessages(50);
	const { data: templates, isLoading: loadingTemplates } =
		useWhatsAppTemplates();
	const { data: dailyStats, isLoading: loadingStats } =
		useWhatsAppDailyStats(7);
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">
						Dashboard de integração e métricas
					</p>
				</div>
			...
			{loadingMetrics ? (
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
					...
					<MetricCard
						...
						icon={XCircle}
						variant={
							metrics?.failed && metrics.failed > 5 ? "danger" : "default"
						}
					/>
				</div>
			)}
				...
				<TabsContent value="templates">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Templates aprovados para envio via WhatsApp Business
							</CardDescription>
						</CardHeader>
						<CardContent>
							{loadingTemplates ? (
									...
									{webhookLogs?.map(
										(log: {
											...
											>
												<div className="flex items-center gap-3">
													<div
														className={`p-2 rounded-full ${log.processed ? "bg-emerald-100" : "bg-amber-100"}`}
													>
														{log.processed ? (
