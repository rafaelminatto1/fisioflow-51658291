...
export function WhatsAppIntegration({
	...
	const fetchHistory = async () => {
		try {
			...
			logger.error(
				"Error fetching WhatsApp history",
				error,
				"WhatsAppIntegration",
			);
		} finally {
	...
	const templates = [
		{
			...
			message:
				"Olá! Este é um lembrete da sua consulta amanhã às {time}. Até lá! 😊",
		},
		{
			id: "exercise_reminder",
			title: "Lembrete de Exercícios",
			message:
				"Não esqueça de fazer seus exercícios hoje! Você está indo muito bem! 💪",
		},
		{
			id: "motivation",
			title: "Motivação",
		...
		},
		{
			...
			message:
				"Como você está se sentindo hoje? Responda para conversarmos! 😊",
		},
	];
				...
				<CardContent className="space-y-4">
					{!patientPhone ? (
						<div className="flex items-center gap-3 p-4 bg-amber-50 dark:bg-amber-900/20 rounded-lg">
							...
							<p className="text-sm">
								Telefone não cadastrado. Adicione um telefone ao paciente.
							</p>
						</div>
					) : (
						<>
								...
								>
									...
									<div className="flex justify-end mt-1 gap-1">
										{msg.status === "read" ? (
											...
											<Badge
												variant="destructive"
												className="text-[9px] py-0 h-4 border-none"
											>
												Falhou
											</Badge>
										) : null}
					...
					<div className="flex items-start gap-3">
						...
						<div className="text-sm space-y-1">
							...
							<p className="text-muted-foreground">
								As mensagens são enviadas através do WhatsApp Web. Certifique-se
								de ter o WhatsApp conectado no navegador. Os lembretes
								automáticos são enviados via sistema quando habilitados.
							</p>
						</div>
					</div>
				</CardContent>
