...
function getSlaLabel(
  ...
  if (diffMin < 10) return { label: `${diffMin}min`, color: "text-red-500 bg-red-50" };
  if (diffMin < 60) return { label: `${diffMin}min`, color: "text-orange-500 bg-orange-50" };
    ...
    >
      <div className="flex items-start gap-3">
        {bulkMode && (
          ...
          >
            {isSelectedBulk && <Check className="h-3 w-3 text-primary-foreground" />}
          </div>
        )}
