...
const BODY_REGIONS = {
	front: [
		// Head (Split)
		{
			code: "cabeca_frente_esquerda",
			name: "Cabeça (Frente Esq.)",
			x: 44,
			y: 6,
			width: 6,
			height: 8,
		...
		{
			code: "cabeca_frente_direita",
			name: "Cabeça (Frente Dir.)",
			x: 56,
			y: 6,
			width: 6,
			height: 8,
		...
		// Neck (Split)
		{
			code: "pescoco_frontal_esquerdo",
			name: "Pescoço (Frente Esq.)",
			x: 46,
			y: 14,
			width: 4,
			height: 4,
		...
		{
			code: "pescoco_frontal_direito",
			name: "Pescoço (Frente Dir.)",
			x: 54,
			y: 14,
			width: 4,
			height: 4,
		...
		},
		{
			...
			width: 10,
			height: 14,
		}, // Fixed typo coxa_direito -> coxa_direita

		{
			code: "joelho_esquerdo",
		...
		},
		{
			...
			width: 7,
			height: 10,
		}, // Fixed typo perna_direito

		{
			code: "tornozelo_esquerdo",
	...
	back: [
		...
		{
			code: "costas_superior_esquerda",
			name: "Costas Superior (Esq.)",
			x: 40,
			y: 25,
			width: 10,
			height: 12,
		...
		{
			code: "costas_superior_direita",
			name: "Costas Superior (Dir.)",
			x: 60,
			y: 25,
			width: 10,
			height: 12,
...
export function BodyMapRealistic({
	...
	const handleMouseDown = (e: React.MouseEvent) => {
		...
		if (e.button === 0 && (e.altKey || e.ctrlKey || zoom > 1)) {
			// Only drag if zoomed or holding modifier
			setIsDragging(true);
	...
	const chartData = evolutionData
		? evolutionData.slice(-7).map((d) => ({
				day: d.date
					? format(new Date(d.date), "EEE", { locale: ptBR }).toUpperCase()[0]
					: "?",
	...
	if (imageError) {
		return (
			<div
				className={cn(
					"w-full h-[500px] flex flex-col items-center justify-center bg-muted/30 rounded-lg border-2 border-dashed border-muted-foreground/25",
					className,
				)}
			>
	...
	return (
		<div
			className={cn(
				"relative w-full h-full min-h-[500px] overflow-hidden bg-background font-sans selection:bg-primary selection:text-primary-foreground group/canvas rounded-lg border shadow-sm",
				className,
			)}
			ref={containerRef}
		...
		>
			<TooltipProvider>
								...
								>
									...
									>
										...
										>
											<Badge
												variant="secondary"
												className="bg-black/80 text-white hover:bg-black/90 text-[10px] whitespace-nowrap"
											>
												{PainMapService.getRegionLabel(
													point.region as RegionCode,
												)}
											</Badge>
										</div>
									</div>
				...
				<div className="absolute top-6 left-6 z-20 pointer-events-none select-none">
					<h1 className="text-2xl text-white mb-1 drop-shadow-md font-bold tracking-tight">
						Explorer 3D
					</h1>
					<p className="text-slate-300 text-sm font-light flex items-center gap-2">
						{zoom > 1 && (
							<span className="bg-blue-500/20 text-blue-200 px-1.5 py-0.5 rounded text-[10px] font-medium border border-blue-500/30">
					...
					<div className="absolute bottom-0 left-0 right-0 z-40 bg-background/95 backdrop-blur-xl border-t border-border p-4 md:hidden animate-in slide-in-from-bottom-full duration-300 rounded-t-2xl shadow-[0_-5px_20px_rgba(0,0,0,0.2)]">
						<div className="flex justify-between items-center mb-4">
							<h3 className="font-semibold text-sm">
								Detalhes: {currentSelectedPoint.region}
							</h3>
							<Button
								variant="ghost"
								size="sm"
					...
					<Card className="bg-background/60 backdrop-blur-md border-white/10 shadow-sm p-3 flex items-start gap-3">
						...
						<div>
							...
							<p className="text-[10px] text-muted-foreground leading-tight">
								Use o zoom para detalhes. Arraste para mover o modelo quando o
								zoom estiver ativo.
							</p>
						</div>
					</Card>
