...
export function PainMapComparison({
	...
	const [selectedMap2, setSelectedMap2] = useState<string>(
		maps[maps.length - 1]?.id || "",
	);
	const [comparisonView, setComparisonView] = useState<"front" | "back">(
		"front",
	);
			...
			<div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
				...
				<div className="flex items-center gap-2">
					...
					<Select
						...
						onValueChange={(v) => setComparisonView(v as "front" | "back")}
					>
							...
							{maps.map((map) => (
								<SelectItem key={map.id} value={map.id}>
									<div className="flex items-center gap-2">
										...
										{format(new Date(map.createdAt), "dd/MM/yyyy", {
											locale: ptBR,
										})}
										{map.sessionNumber && (
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
						...
						<div className="space-y-2">
							...
							<div className="text-sm space-y-1 text-muted-foreground">
								...
								<p>
									Intensidade média antes:{" "}
									{evolution.avgIntensityBefore.toFixed(1)}
								</p>
								<p>
									Intensidade média depois:{" "}
									{evolution.avgIntensityAfter.toFixed(1)}
								</p>
							</div>
						</div>
					</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
				...
				<Card>
					<CardHeader className="pb-2">
						<CardTitle className="text-base">
							{map1 &&
								format(new Date(map1.createdAt), "dd 'de' MMMM 'de' yyyy", {
									locale: ptBR,
								})}
						</CardTitle>
						<CardDescription>{map1Points.length} pontos de dor</CardDescription>
					</CardHeader>
					<CardContent>
						<div className="aspect-[3/4] max-h-[400px] border rounded-lg bg-muted/20">
							<BodyMap view={comparisonView} points={map1Points} readOnly />
						</div>
					</CardContent>
				</Card>
				...
				<Card>
					<CardHeader className="pb-2">
						<CardTitle className="text-base">
							{map2 &&
								format(new Date(map2.createdAt), "dd 'de' MMMM 'de' yyyy", {
									locale: ptBR,
								})}
						</CardTitle>
						<CardDescription>{map2Points.length} pontos de dor</CardDescription>
					</CardHeader>
					<CardContent>
						<div className="aspect-[3/4] max-h-[400px] border rounded-lg bg-muted/20">
							<BodyMap view={comparisonView} points={map2Points} readOnly />
						</div>
					</CardContent>
				</Card>
			</div>
...
function calculateEvolution(
	pointsBefore: PainPoint[],
	pointsAfter: PainPoint[],
) {
	...
	// Todas as regiões
	const allRegions = [
		...new Set([...Object.keys(beforeByRegion), ...Object.keys(afterByRegion)]),
	];
	...
	allRegions.forEach((region) => {
		...
		const regionName =
			pointsBefore.find((p) => p.regionCode === region)?.region ||
	...
	// Médias
	const avgIntensityBefore =
		pointsBefore.length > 0
			? pointsBefore.reduce((sum, p) => sum + p.intensity, 0) /
				pointsBefore.length
			: 0;

	const avgIntensityAfter =
		pointsAfter.length > 0
			? pointsAfter.reduce((sum, p) => sum + p.intensity, 0) /
				pointsAfter.length
			: 0;
