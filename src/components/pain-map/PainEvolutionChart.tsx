...
const CustomTooltip = ({
	...
	active?: boolean;
	payload?: Array<{
		payload: { dateFormatted: string; pointCount: number; intensity: number };
	}>;
}) => {
	if (active && payload && payload.length) {
		...
		return (
			<div className="bg-background border border-border rounded-lg p-3 shadow-lg">
				<p className="text-sm font-medium mb-1">{data.dateFormatted}</p>
...
const CustomDot = (props: {
	...
	return (
		<Dot cx={cx} cy={cy} r={5} fill={color} stroke="#fff" strokeWidth={2} />
	);
};
...
export function PainEvolutionChart({
	...
	const stats = useMemo(() => {
		...
		const trend =
			improvement > 0 ? "improving" : improvement < 0 ? "worsening" : "stable";
              ...
              rgba(34, 197, 94, 0.1) 100%)`,
				...
				<div className="absolute inset-0 flex flex-col justify-between opacity-20 pointer-events-none">
					{[0, 1, 2].map((i) => (
						<div
							key={i}
							className="w-full h-px bg-muted-foreground border-t border-dashed"
						/>
					))}
				</div>
