...
const getIntensityColor = (intensity: number): string =>
  INTENSITY_COLORS[intensity as keyof typeof INTENSITY_COLORS]?.primary || "#94a3b8";
const getIntensityBg = (intensity: number): string =>
  INTENSITY_COLORS[intensity as keyof typeof INTENSITY_COLORS]?.secondary || "#f1f5f9";
const getIntensityLabel = (intensity: number): string =>
  INTENSITY_COLORS[intensity as keyof typeof INTENSITY_COLORS]?.label || "Desconhecido";
...
export function BodyMapAnatomical({
  ...
  const regionStats = useMemo(() => {
    ...
    points.forEach((point) => {
      ...
      current.avgIntensity =
        (current.avgIntensity * (current.count - 1) + point.intensity) / current.count;
  ...
  const globalStats = useMemo(() => {
    ...
    const avgIntensity =
      totalPoints > 0 ? points.reduce((sum, p) => sum + p.intensity, 0) / totalPoints : 0;
        ...
        <div className="absolute top-2 left-2 right-2 z-20 flex flex-wrap gap-2 items-start justify-between">
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <Tooltip>
              ...
              <TooltipContent>{showLabels ? "Esconder labels" : "Mostrar labels"}</TooltipContent>
            </Tooltip>
          </div>
