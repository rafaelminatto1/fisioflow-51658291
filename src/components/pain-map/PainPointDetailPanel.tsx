...
export function PainPointDetailPanel({
  ...
  const chartData = useMemo(() => {
    ...
    return evolutionData.map((item, _index) => ({
      month: new Date(item.date).toLocaleDateString("pt-BR", {
        month: "short",
      }),
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              <div className="flex flex-col">
                ...
                {point.muscleName && (
                  <span className="text-sm text-primary font-medium">💪 {point.muscleName}</span>
                )}
              </div>
            </div>
            <p className="text-xs text-muted-foreground">{point.regionCode} • Registrado</p>
          </div>
      ...
      <CardContent className="space-y-6">
            ...
            <div className="h-16 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  ...
                  <YAxis hide />
                  <Tooltip />
                  <Bar dataKey="intensity" fill={color} radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            <Label className="text-xs font-semibold text-foreground">Intensidade (VAS)</Label>
            <Badge
              variant="outline"
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {PAIN_QUALITIES.map((quality) => (
              <label key={quality.id} className="cursor-pointer">
                <input
                  type="checkbox"
