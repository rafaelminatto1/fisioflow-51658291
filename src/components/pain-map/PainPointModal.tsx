...
export function PainPointModal({
  ...
  const availableMuscles = point?.regionCode ? getMusclesByRegion(point.regionCode, view) : [];
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>{point?.region || region || "Nova região"}</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-5 mt-4">
          ...
          <div className="space-y-3">
            ...
            <EvaScaleBar value={intensity} onChange={setIntensity} showLabels={true} />
            <Slider
              value={[intensity]}
          ...
          {availableMuscles.length > 0 && (
            <div className="space-y-2">
              ...
              {muscleName && (
                <p className="text-xs text-primary">💪 Músculo selecionado: {muscleName}</p>
              )}
            </div>
          )}
