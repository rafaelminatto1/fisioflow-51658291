      ...
      <div className="space-y-3">
        {points.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-center bg-muted/20 rounded-lg border-2 border-dashed">
            ...
            <p className="text-sm font-medium text-foreground mb-1">Nenhum ponto</p>
            <p className="text-xs text-muted-foreground">Clique no mapa para adicionar</p>
          </div>
        ) : (
          points.map((point) => {
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start mb-1">
                    <h4 className="font-medium text-sm text-foreground truncate">{point.region}</h4>
                    <Badge
                      variant="outline"
                  ...
                  {point.notes && (
                    <p className="text-xs text-muted-foreground line-clamp-2 mt-1">{point.notes}</p>
                  )}
                </div>
