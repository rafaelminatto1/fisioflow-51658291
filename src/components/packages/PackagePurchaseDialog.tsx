...
import { Package, Calendar, DollarSign, Check, Loader2, BadgeCheck, ShoppingBag } from 'lucide-react';
  ...
  return (
    ...
    >
          ...
          <div className="p-6 space-y-6">
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-12 gap-3 text-muted-foreground">
                ...
                <p className="text-sm font-medium">Carregando pacotes disponíveis...</p>
              </div>
                          ...
                          <div className="flex-1 space-y-3">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                ...
                                {isBestValue && (
                                  <Badge className="bg-emerald-500 hover:bg-emerald-600 rounded-lg text-[10px] h-5">Melhor custo</Badge>
                                )}
                              </div>
              ...
              <div className="text-center py-12 border-2 border-dashed rounded-3xl border-slate-100">
                ...
                <p className="text-sm font-bold text-slate-400">Nenhum pacote disponível para venda.</p>
                <p className="text-xs text-slate-400 mt-1">Configure seus pacotes no módulo financeiro.</p>
              </div>
            )}
              ...
              <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                <Card className="border-slate-900 border-2 bg-slate-900 text-white shadow-xl rounded-3xl overflow-hidden">
                  <CardContent className="p-5">
                    <h4 className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-4">Resumo da Transação</h4>
                    <div className="space-y-3">
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-slate-400">Pacote Selecionado:</span>
                        <span className="font-bold">{selectedPackage.name}</span>
                      </div>
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-slate-400">Crédito de Sessões:</span>
                        <span className="font-bold">{selectedPackage.sessions_count} sessões</span>
                      </div>
                      <div className="pt-3 border-t border-slate-800 flex justify-between items-center">
                        <span className="text-sm font-bold uppercase">Total a Pagar:</span>
                        <span className="text-2xl font-black text-white">{formatCurrency(selectedPackage.price)}</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
      ...
      <CustomModalFooter isMobile={isMobile} className="bg-slate-50 border-t-0">
        <Button variant="ghost" onClick={() => onOpenChange(false)} className="rounded-xl h-11 px-6 font-bold text-slate-500">
          Cancelar
        </Button>
        <Button 
