...
import { Package, AlertTriangle, CheckCircle2, XCircle, Clock, Ticket } from "lucide-react";
...
export function PatientPackageCard({
  ...
  const usagePercentage = sessions_purchased > 0 ? (sessions_used / sessions_purchased) * 100 : 0;
    ...
    >
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-medium">
              {sessions_used} / {sessions_purchased}
            </span>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div className="flex items-center gap-2">
            <Clock
              className={`w-4 h-4 ${isNearExpiration ? "text-yellow-500" : "text-muted-foreground"}`}
            />
            <div>
              <p className="text-muted-foreground text-xs">Expira em</p>
              <p className={isNearExpiration ? "text-yellow-600 font-medium" : ""}>
                {expires_at
                  ? format(new Date(expires_at), "dd/MM/yyyy", { locale: ptBR })
                  : "Sem expiração"}
              </p>
            </div>
          </div>
        ...
        {isNearExpiration && (
          <div className="flex items-center gap-2 p-2 bg-yellow-500/10 rounded-lg text-yellow-600 text-sm">
            ...
            <span>
              Expira em {daysUntilExpiration} dia
