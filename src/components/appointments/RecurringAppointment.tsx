...
export const RecurringAppointment: React.FC<RecurringAppointmentProps> = ({
	...
	const occurrences = useMemo(() => {
		...
		while (dates.length < 10) {
			...
			const isExcluded = config.excludeDates?.some((d) =>
	...
	const summary = useMemo(() => {
		...
		const intervalText =
			config.interval && config.interval > 1 ? `a cada ${config.interval}` : "";
		...
		<div className="space-y-4">
			...
			<div className="space-y-2">
				...
				<div className="grid grid-cols-5 gap-2">
					{(
						["once", "daily", "weekly", "monthly", "yearly"] as RecurrenceType[]
					).map((type) => (
						<button
							key={type}
							type="button"
...
export const useRecurringAppointments = () => {
	const generateOccurrences = useCallback(
		(config: RecurringAppointmentConfig): Date[] => {
			if (config.recurrenceType === "once") return [config.startDate];
			...
			while (true) {
				if (config.endDate && currentDate > config.endDate) break;
				if (config.maxOccurrences && dates.length >= config.maxOccurrences)
					break;
	...
	return (
		<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
			<div className="w-full max-w-2xl bg-background rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
				...
				<div className="p-6">
					<RecurringAppointment config={config} onChange={setConfig} />
				</div>
