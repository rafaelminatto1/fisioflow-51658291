...
export const BulkOperations: React.FC<BulkOperationsProps> = ({
  ...
  const handleBulkAction = useCallback(
    async (action: BulkAction, data?: any) => {
  ...
  const handleReschedule = useCallback(
    (minutes: number) => {
      ...
      const updatedData = selectedAppointments.map((apt) => {
        ...
        const newStartTime =
          minutes > 0 ? addMinutes(startTime, minutes) : subMinutes(startTime, Math.abs(minutes));
        ...
        <div className="sticky top-0 z-40 p-4 bg-background border-b shadow-lg flex items-center gap-4 animate-in slide-in-from-top">
          ...
          <div className="flex gap-2">
            ...
            <div className="relative group">
              <button className="px-3 py-1.5 bg-muted hover:bg-muted/80 rounded-lg flex items-center gap-2">
