    ...
    <Card className="rounded-3xl border-border/40 shadow-premium-lg flex flex-col h-full bg-white/50 backdrop-blur-sm">
      <CardHeader className="pb-3 border-b border-border/10">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="destructive" className="rounded-full text-[10px] font-black px-2">
            {atRisk.length}
          </Badge>
        </div>
      </CardHeader>
        ...
        {atRisk.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full p-8 text-center space-y-2">
            ...
            <p className="text-[10px] text-muted-foreground">
              Nenhum paciente com risco detectado hoje.
            </p>
          </div>
        ) : (
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs font-black uppercase tracking-tight truncate w-32">
                        {patient.full_name}
                      </p>
                      <p className="text-[9px] text-muted-foreground font-medium">
                        Última:{" "}
                        {patient.last_appointment
                          ? formatDistanceToNow(new Date(patient.last_appointment), {
                              addSuffix: true,
                              locale: ptBR,
                            })
                          : "Sem registro"}
                  ...
                  <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Button
                      ...
                      onClick={() =>
                        navigate(`/communications?patientId=${patient.id}&channel=whatsapp`)
                      }
                    >
