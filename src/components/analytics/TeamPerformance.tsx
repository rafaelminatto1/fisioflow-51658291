...
import { CheckCircle2, Clock, Award, TrendingUp, RefreshCw } from "lucide-react";
      ...
      return therapists
        .map((member) => {
          ...
          const cancelled = apts.filter(
            (a) => a.status === "cancelled" || a.status === "cancelado",
          ).length;
            ...
            <ScrollArea className="h-[380px]">
              <Table>
                <TableHeader className="bg-muted/30">
                  <TableRow>
                    <TableHead className="text-[10px] font-bold uppercase px-6">
                      Profissional
                    </TableHead>
                    ...
                    <TableHead className="text-[10px] font-bold uppercase text-right px-6">
                      Conversão
                    </TableHead>
                  </TableRow>
                </TableHeader>
                  ...
                  {performanceData?.map((row) => (
                    <TableRow key={row.id} className="border-border/40">
                      ...
                      <TableCell className="text-right px-6">
                        <Badge
                          variant="outline"
                          className={`${row.attendanceRate >= 80 ? "bg-emerald-500/5 text-emerald-600 border-emerald-500/20" : "bg-orange-500/5 text-orange-600 border-orange-500/20"} font-bold`}
                        >
                          {row.attendanceRate}%
                        </Badge>
                      </TableCell>
            ...
            {loadingBi ? (
              ...
              <div className="space-y-2">
                {(biData ?? []).map((t, i) => (
                  ...
                  >
                    <span
                      className={`h-7 w-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${i === 0 ? "bg-amber-100 text-amber-700" : i === 1 ? "bg-slate-100 text-slate-600" : "bg-muted text-muted-foreground"}`}
                    >
                      {i + 1}
                    </span>
                    <span className="font-medium text-sm flex-1 truncate">{t.name}</span>
                    <Badge variant="secondary" className="rounded-lg text-xs">
                      {t.sessions_completed} sessões
                    </Badge>
                    {t.no_shows > 0 && (
                      <Badge
                        variant="outline"
                        className="rounded-lg text-xs text-amber-600 border-amber-200"
                      >
                        {t.no_shows} faltas
                      </Badge>
                    )}
