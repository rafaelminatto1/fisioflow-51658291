  ...
  if (isLoading) {
    return (
      <div className="h-96 flex items-center justify-center text-xs text-muted-foreground animate-pulse">
        Identificando possíveis abandonos...
      </div>
    );
  }
            ...
            {data.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-6 py-12 text-center text-slate-400 font-medium">
                  Nenhum paciente em churn identificado. Bom trabalho!
                </td>
              </tr>
            ) : (
              data.map((p) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-xs text-slate-500">
                    {p.last_session_date
                      ? format(new Date(p.last_session_date), "dd/MM/yyyy", { locale: ptBR })
                      : "N/A"}
                  </td>
                  ...
                  <td className="px-6 py-4 text-right flex justify-end gap-2">
                    <Button
                      ...
                      onClick={() =>
                        window.open(
                          `https://wa.me/?text=${encodeURIComponent(`Oi ${p.full_name.split(" ")[0]}! Tudo bem? Sentimos sua falta na Mooca Fisio. Como está sua recuperação?`)}`,
                          "_blank",
                        )
                      }
                    >
