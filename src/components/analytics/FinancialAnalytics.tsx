		...
		<div className="space-y-6">
						...
						{isLoadingRevenue ? (
							...
							<SafeResponsiveContainer className="h-full" minHeight={300}>
								<LineChart data={revenueData}>
									...
									<Tooltip formatter={(v) => formatCurrency(Number(v))} contentStyle={{ borderRadius: "12px", border: "none", boxShadow: "0 4px 12px -2px rgba(0,0,0,0.05)" }} />
									<Line type="monotone" dataKey="receita" stroke="hsl(var(--primary))" strokeWidth={3} dot={{ fill: "hsl(var(--primary))", strokeWidth: 2, r: 4, stroke: "#fff" }} activeDot={{ r: 6, strokeWidth: 0 }} />
								</LineChart>
						...
						{isLoadingMethods ? (
							...
							<SafeResponsiveContainer className="h-full" minHeight={300}>
								<BarChart data={paymentMethods}>
									<CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#E2E8F0" />
									<XAxis dataKey="metodo" axisLine={false} tickLine={false} tick={{ fill: "#64748B", fontSize: 12 }} dy={10} />
									<YAxis axisLine={false} tickLine={false} tick={{ fill: "#64748B", fontSize: 12 }} tickFormatter={(v) => `R$${v}`} />
									<Tooltip formatter={(v) => formatCurrency(Number(v))} contentStyle={{ borderRadius: "12px", border: "none", boxShadow: "0 4px 12px -2px rgba(0,0,0,0.05)" }} />
			...
			<div className="flex items-center justify-between gap-4 pt-2">
				<div>
					...
					<p className="text-xs text-muted-foreground">Tendências de receita e sessões por período</p>
				</div>
						...
						{isLoadingBi ? (
							...
							<SafeResponsiveContainer className="h-[210px]" minHeight={210}>
								<AreaChart data={revenueMonths} margin={{ left: 0, right: 8, top: 4, bottom: 0 }}>
									...
									<Tooltip formatter={(v: number, name: string) => [name === "receita" ? formatCurrency(v) : v, name === "receita" ? "Receita" : "Sessões"]} />
									<Area type="monotone" dataKey="receita" stroke="#3b82f6" fill="url(#biColorReceita)" strokeWidth={2} dot={false} />
								</AreaChart>
							</SafeResponsiveContainer>
						)}
					</CardContent>
