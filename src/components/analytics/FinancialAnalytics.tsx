...
export function FinancialAnalytics() {
	...
	const { data: paymentMethods, isLoading: isLoadingMethods } = useQuery({
		...
		queryFn: async () => {
			...
			const payments = ((response?.data ?? []) as Pagamento[]).filter(
				(payment) => {
			...
			payments.forEach((payment) => {
				...
				paymentMap.set(
					method,
					(paymentMap.get(method) ?? 0) + Number(payment.valor ?? 0),
				);
			});
		...
		<div className="space-y-4">
			<Card className="border-none shadow-sm ring-1 ring-gray-200/50 dark:ring-gray-800/50">
				<CardHeader>
					...
					<CardDescription>
						Evolução da receita no intervalo selecionado
					</CardDescription>
				</CardHeader>
				<CardContent>
			...
			<Card className="border-none shadow-sm ring-1 ring-gray-200/50 dark:ring-gray-800/50">
				<CardHeader>
					...
					<CardDescription>
						Volume financeiro por método no período selecionado
					</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="h-[300px] w-full">
