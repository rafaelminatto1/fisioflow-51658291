...
export default function AdvancedAnalytics() {
  const { data: metrics } = useQuery<AnalyticsMetrics>({
    ...
    queryFn: async () => {
      const [patientsResponse, appointmentsResponse, eventsResponse, financialResponse] =
        await Promise.all([
      ...
      const appointmentsThisMonth =
        appointments.filter((appointment) => new Date(appointment.date) >= thisMonth).length || 0;
  ...
  const { data: appointmentsByMonth } = useQuery<MonthlyAppointments[]>({
    ...
    queryFn: async () => {
      ...
      data.forEach((apt) => {
        ...
        if (["cancelled", "cancelado"].includes(String(apt.status ?? "").toLowerCase()))
        ...
        <TabsContent value="predictive" className="space-y-6">
          <Card className="border-primary/20 bg-primary/5">
            <CardHeader>
              ...
              <CardDescription>
                O Gemini analisou seu banco de dados para prever tendências
              </CardDescription>
            </CardHeader>
            <CardContent>
              <AIInsightsWidget
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Probabilidade de pacientes não retornarem no próximo mês
                </p>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Baseado na taxa de crescimento atual de 15%
                </p>
              </CardContent>
            </Card>
          </div>
          ...
          <Card>
            <CardHeader className="pb-4 sm:pb-6">
              ...
              <CardDescription className="text-xs sm:text-sm">
                Evolução de consultas nos últimos 6 meses
              </CardDescription>
            </CardHeader>
            <CardContent>
