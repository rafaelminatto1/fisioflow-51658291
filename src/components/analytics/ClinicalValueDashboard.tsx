...
export function ClinicalValueDashboard() {
  ...
  const roiData = healthData?.data?.roiData || [
    { protocolName: "Ortopedia", painReductionAvg: 65, functionalImprovementAvg: 70, avgLtv: 2400, patientCount: 45 },
    { protocolName: "Pilates", painReductionAvg: 40, functionalImprovementAvg: 85, avgLtv: 3800, patientCount: 32 },
    { protocolName: "Neurologia", painReductionAvg: 30, functionalImprovementAvg: 50, avgLtv: 5200, patientCount: 12 },
    { protocolName: "Postura", painReductionAvg: 55, functionalImprovementAvg: 60, avgLtv: 1800, patientCount: 20 },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="border-none shadow-premium bg-white dark:bg-slate-900/50">
          <CardHeader>
            <CardTitle className="text-lg font-black tracking-tight">Matriz de Eficiência Clínica</CardTitle>
            <CardDescription>Correlação entre Redução de Dor (%) e Valor Financeiro (LTV)</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <SafeResponsiveContainer>
                <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                  ...
                  <XAxis 
                    ...
                    name="Redução de Dor" 
                    unit="%" 
                    label={{ value: 'Melhora Clínica (%)', position: 'bottom', offset: 0, fontSize: 10 }}
                  />
                  <YAxis 
                    type="number" 
                    dataKey="avgLtv" 
                    name="LTV" 
                    unit="R$" 
                    label={{ value: 'Valor (LTV)', angle: -90, position: 'insideLeft', fontSize: 10 }}
                  />
                  <ZAxis type="number" dataKey="patientCount" range={[100, 1000]} name="Pacientes" />
            ...
            <div className="mt-4 flex justify-center gap-4 text-[10px] font-bold uppercase tracking-widest text-slate-400">
               <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-blue-400" /> Low LTV</span>
               <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-indigo-500" /> Mid LTV</span>
               <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-purple-600" /> High LTV</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-none shadow-premium bg-white dark:bg-slate-900/50">
          <CardHeader>
            <CardTitle className="text-lg font-black tracking-tight">Performance por Especialidade</CardTitle>
    ...
    <Card className="border-none shadow-sm bg-white/50 dark:bg-slate-900/30 backdrop-blur-sm">
      <CardContent className="p-5">
        <div className="flex justify-between items-start">
          ...
          <div className={`p-2 rounded-xl bg-slate-100 dark:bg-slate-800 ${color.replace('text-', 'text-opacity-70')}`}>
            <Icon size={18} />
          </div>
        </div>
      </CardContent>
