...
const listAppointments = async (dateFrom?: string, dateTo?: string, therapistId?: string) => {
	...
	const limit = 1000;
	while (offset < 10000) {
		const response = await appointmentsApi.list({ dateFrom, dateTo, limit, offset, therapistId: therapistId === "all" ? undefined : therapistId });
		const chunk = response?.data ?? [];
		items.push(...chunk);
		if (chunk.length < limit) break;
		offset += limit;
	...
	const { data: statusData, isLoading: loadingStatus } = useQuery({
		...
		queryFn: async () => {
			const [allPatients, therapistAppointments] = await Promise.all([
				listAllPatients(),
				professionalId !== "all" ? listAppointments(undefined, undefined, professionalId) : Promise.resolve(null),
			]);

			let activeCount = 0;
			let inactiveCount = 0;
			if (professionalId === "all") {
				activeCount = allPatients.filter(p => p.status === "ativo" || p.is_active === true).length;
			...
			return [{ name: "Ativos", value: activeCount }, { name: "Inativos", value: inactiveCount }];
		},
	});
	...
	const { data: inactiveData, isLoading: loadingInactive } = useQuery({
		...
		queryFn: async () => {
			...
			return {
				...
				list: inactive.slice(0, 20).map(p => ({ ...p, lastAppointment: lastVisitMap.get(p.id) || null })),
			};
		},
	});
			...
			if (monthsDiff >= 2) {
				chartData = eachMonthOfInterval({ start: from, end: to }).map(month => {
					...
					return {
						...
						count: patients.filter(p => { const c = new Date(p.created_at); return c >= start && c < end; }).length,
					};
				});
			} else {
				const days = eachDayOfInterval({ start: from, end: to });
				chartData = days.filter((_, i) => days.length <= 15 || i % 2 === 0).map(day => {
					const dStart = startOfDay(day);
					const dEnd = startOfDay(subDays(day, -1));
					return {
						label: format(day, "dd/MM", { locale: ptBR }),
						count: patients.filter(p => { const c = new Date(p.created_at); return c >= dStart && c < dEnd; }).length,
					};
				});
			}
		...
		<div className="space-y-8">
			...
			<div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
				<Card className="relative overflow-hidden group hover:shadow-xl transition-all duration-500 border-none bg-background shadow-sm ring-1 ring-border/50">
					<div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 group-hover:scale-110 transition-all duration-500"><Users className="h-16 w-16 text-primary" /></div>
					<CardHeader className="pb-2">
						<div className="flex items-center gap-2.5">
							<div className="p-2 bg-primary/10 rounded-lg"><Users className="h-4 w-4 text-primary" /></div>
							<CardTitle className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Total de Pacientes</CardTitle>
						</div>
					</CardHeader>
					<CardContent className="pt-2">
						<div className="text-3xl font-black tracking-tighter">{totalPatients || 0}</div>
						<div className="flex items-center gap-1.5 mt-2">
							<Badge variant="secondary" className="bg-primary/5 text-primary text-[10px] font-bold border-none">Base total</Badge>
				...
				<Card className="relative overflow-hidden group hover:shadow-xl transition-all duration-500 border-none bg-background shadow-sm ring-1 ring-border/50">
					<div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 group-hover:scale-110 transition-all duration-500"><TrendingUp className="h-16 w-16 text-emerald-500" /></div>
					<CardHeader className="pb-2">
						<div className="flex items-center gap-2.5">
							<div className="p-2 bg-emerald-500/10 rounded-lg"><TrendingUp className="h-4 w-4 text-emerald-500" /></div>
							<CardTitle className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Pacientes Ativos</CardTitle>
						</div>
					</CardHeader>
					<CardContent className="pt-2">
						<div className="text-3xl font-black tracking-tighter text-emerald-600">{loadingActive ? "..." : activePatientsCount}</div>
						<div className="flex items-center gap-1.5 mt-2">
							<Badge variant="secondary" className="bg-emerald-500/5 text-emerald-600 text-[10px] font-bold border-none">No período</Badge>
				...
				<Card className="relative overflow-hidden group hover:shadow-xl transition-all duration-500 border-none bg-background shadow-sm ring-1 ring-border/50">
					<div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 group-hover:scale-110 transition-all duration-500"><UserMinus className="h-16 w-16 text-orange-500" /></div>
					<CardHeader className="pb-2">
						<div className="flex items-center gap-2.5">
							<div className="p-2 bg-orange-500/10 rounded-lg"><UserMinus className="h-4 w-4 text-orange-500" /></div>
							<CardTitle className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Pacientes Inativos</CardTitle>
						</div>
					</CardHeader>
					<CardContent className="pt-2">
						<div className="text-3xl font-black tracking-tighter text-orange-600">{loadingInactive ? "..." : inactiveData?.total || 0}</div>
						<div className="flex items-center gap-1.5 mt-2">
							<Badge variant="secondary" className="bg-orange-500/5 text-orange-600 text-[10px] font-bold border-none">Risco</Badge>
							<span className="text-[10px] text-muted-foreground font-medium">Sem consulta +30d</span>
						</div>
					</CardContent>
				...
				<Card className="relative overflow-hidden group hover:shadow-xl transition-all duration-500 border-none bg-background shadow-sm ring-1 ring-border/50">
					<div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 group-hover:scale-110 transition-all duration-500"><CreditCard className="h-16 w-16 text-blue-500" /></div>
					<CardHeader className="pb-2">
						<div className="flex items-center gap-2.5">
							<div className="p-2 bg-blue-500/10 rounded-lg"><CreditCard className="h-4 w-4 text-blue-500" /></div>
							<CardTitle className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Sessões em Aberto</CardTitle>
						</div>
					</CardHeader>
					<CardContent className="pt-2">
						<div className="text-3xl font-black tracking-tighter text-blue-600">{loadingSessions ? "..." : patientsWithSessions?.length || 0}</div>
						<div className="flex items-center gap-1.5 mt-2">
							<Badge variant="secondary" className="bg-blue-500/5 text-blue-600 text-[10px] font-bold border-none">Créditos</Badge>
							<span className="text-[10px] text-muted-foreground font-medium">Pacotes com saldo</span>
						</div>
					</CardContent>
			...
			<div className="grid gap-4 md:grid-cols-2">
				<Card className="border-none shadow-sm ring-1 ring-gray-200/50 dark:ring-gray-800/50">
					<CardHeader>
						...
						<CardDescription>{professionalId === "all" ? "Status de todos os pacientes" : "Status dos pacientes do profissional"}</CardDescription>
					</CardHeader>
					<CardContent>
							...
							{loadingStatus ? (
								...
								<SafeResponsiveContainer className="h-full" minHeight={260}>
									<PieChart>
										...
										<Tooltip contentStyle={{ borderRadius: "12px", border: "none", boxShadow: "0 4px 12px -2px rgba(0,0,0,0.05)" }} />
									</PieChart>
							...
							{loadingAge ? (
								...
								<SafeResponsiveContainer className="h-full" minHeight={300}>
									<BarChart data={ageData}>
										...
										<XAxis dataKey="faixa" axisLine={false} tickLine={false} tick={{ fill: "#64748B", fontSize: 12 }} dy={10} />
			...
			<div className="grid gap-6 lg:grid-cols-5">
					...
					<CardHeader className="border-b border-border/40 pb-4 bg-muted/20">
						<div className="flex items-center justify-between">
							<div className="space-y-1">
								...
								<CardDescription className="text-xs font-medium">Crescimento da base no período</CardDescription>
							</div>
							<div className="text-right">
								<div className="text-2xl font-black text-primary leading-tight">{newPatientsData?.periodTotal || 0}</div>
								<div className="text-[10px] font-bold text-muted-foreground uppercase">Total Novos</div>
							</div>
						</div>
					</CardHeader>
						...
						<div className="h-[260px] w-full">
							<SafeResponsiveContainer className="h-full" minHeight={260}>
								<LineChart data={newPatientsData?.chartData || []}>
									...
									<XAxis dataKey="label" axisLine={false} tickLine={false} tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 10, fontWeight: 600 }} dy={15} />
									<YAxis axisLine={false} tickLine={false} tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 10, fontWeight: 600 }} />
									<Tooltip contentStyle={{ borderRadius: "16px", border: "1px solid hsl(var(--border))", boxShadow: "0 10px 15px -3px rgba(0,0,0,0.1)", fontSize: "12px", fontWeight: "bold" }} />
									<Line type="monotone" dataKey="count" stroke="hsl(var(--primary))" strokeWidth={4} dot={{ fill: "hsl(var(--primary))", strokeWidth: 2, r: 4, stroke: "#fff" }} activeDot={{ r: 6, strokeWidth: 0 }} name="Novos Pacientes" />
								</LineChart>
							</SafeResponsiveContainer>
						</div>
					</CardContent>
				...
				<Card className="lg:col-span-2 border-none shadow-sm ring-1 ring-border/50 bg-background flex flex-col">
					<CardHeader className="border-b border-border/40 pb-4 bg-muted/20">
						...
						<CardDescription className="text-xs font-medium">Pacientes com saldo de sessões pré-pagas</CardDescription>
					</CardHeader>
					<CardContent className="p-0 flex-1">
						<ScrollArea className="h-[340px]">
							{loadingSessions ? (
								<div className="flex items-center justify-center h-full text-muted-foreground text-sm font-medium">Carregando...</div>
								...
								<div className="flex flex-col items-center justify-center h-[340px] text-center px-6">
									...
									<p className="text-muted-foreground text-sm font-medium">Nenhum paciente com saldo</p>
								</div>
							) : (
								<Table>
									...
									<TableBody>
										{patientsWithSessions?.map((item) => (
											<TableRow key={item.id} className="border-border/40">
												<TableCell className="font-bold text-sm py-3 px-4">{item.patientName}</TableCell>
			...
			<Card className="border-none shadow-sm ring-1 ring-border/50 bg-background">
				<CardHeader className="border-b border-border/40 pb-4 bg-muted/20">
					<div className="flex items-center justify-between">
						<div className="space-y-1">
							...
							<CardDescription className="text-xs font-medium">Pacientes sem consulta há mais de 30 dias</CardDescription>
						</div>
				...
				<CardContent className="p-0">
					<ScrollArea className="h-[280px]">
						{loadingInactive ? (
							<div className="flex items-center justify-center h-[280px] text-muted-foreground text-sm">Analisando base...</div>
						) : inactiveData?.list?.length === 0 ? (
							<div className="flex items-center justify-center h-[280px] text-muted-foreground text-sm">Excelente! Todos os pacientes estão ativos.</div>
						) : (
							<Table>
								<TableHeader className="bg-muted/30">
									<TableRow className="hover:bg-transparent border-none">
										<TableHead className="text-[10px] font-bold uppercase h-10 px-6">Nome</TableHead>
								...
								<TableBody>
									{inactiveData?.list?.map((patient: any) => (
										<TableRow key={patient.id} className="border-border/40 hover:bg-muted/10 transition-colors">
											<TableCell className="font-bold text-sm py-4 px-6">{PatientHelpers.getName(patient)}</TableCell>
											<TableCell className="text-muted-foreground text-sm font-medium">{patient.phone || "—"}</TableCell>
											<TableCell className="text-sm font-semibold">
												{patient.lastAppointment ? format(new Date(patient.lastAppointment), "dd/MM/yyyy", { locale: ptBR }) : "Sem registro"}
											</TableCell>
											<TableCell className="text-right px-6">
												<Badge variant="outline" className="bg-orange-500/5 text-orange-600 border-orange-500/20 font-bold">Inativo</Badge>
											</TableCell>
										</TableRow>
									))}
								</TableBody>
