...
function CohortHeatmap({
	...
	data: { cohortMonth: string; totalPatients: number; retention: number[] }[];
}) {
					...
					{data.map((cohort) => (
						<tr key={cohort.cohortMonth} className="border-t border-border">
							<td className="p-2 font-medium">
								{format(new Date(cohort.cohortMonth + "-01"), "MMM/yy", {
									locale: ptBR,
								})}
							</td>
							...
							{[...Array(12)].map((_, i) => (
								<td key={i} className="p-1">
									{cohort.retention[i] !== undefined ? (
										<div
											className={`rounded p-2 text-center text-xs font-medium ${getColor(cohort.retention[i])}`}
										>
											{cohort.retention[i]}%
										</div>
									) : (
										<div className="rounded p-2 text-center text-xs text-muted-foreground">
											-
										</div>
									)}
								</td>
				...
				<div className="space-y-4">
					<div className="space-y-2">
						...
						<Select
							value={channel}
							onValueChange={(v) =>
								setChannel(v as "whatsapp" | "email" | "sms")
							}
						>
							<SelectTrigger>
								<SelectValue />
						...
						<div className="flex flex-wrap gap-1">
							...
							{selectedPatients.length > 5 && (
								<Badge variant="outline">
									+{selectedPatients.length - 5} mais
								</Badge>
							)}
						</div>
					</div>
...
export function PatientRetention() {
	...
	const filteredPatients = useMemo(() => {
		...
		return patientsAtRisk.filter((patient) => {
			...
			if (
				riskFilter === "medium" &&
				(patient.riskScore < 50 || patient.riskScore >= 70)
			)
				return false;
	...
	const insights = useMemo(() => {
		...
		if (metrics.churnRate > 30) {
			insights.push(
				"Taxa de churn crítica! Mais de 30% dos pacientes estão inativos.",
			);
		}

		if (metrics.atRiskCount > 10) {
			insights.push(
				`${metrics.atRiskCount} pacientes em risco de abandono identificados.`,
			);
		}

		if (metrics.projectedRevenueLoss > 1000) {
			insights.push(
				`Projeção de perda de receita: R$ ${metrics.projectedRevenueLoss.toLocaleString("pt-BR")}`,
			);
		}
		...
		if (churnTrends.length >= 2) {
			...
			if (lastMonth.churnRate > prevMonth.churnRate) {
				insights.push(
					"Churn aumentou em relação ao mês anterior. Ação necessária!",
				);
			}
		}
		...
		<div className="space-y-6">
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">
						Análise de churn, retenção e campanhas de reativação
					</p>
				</div>
				<Button
					variant="outline"
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
				...
				<MetricCard
					...
					icon={UserMinus}
					variant={
						metrics?.churnRate && metrics.churnRate > 20 ? "danger" : "warning"
					}
				...
				<MetricCard
					...
					icon={AlertTriangle}
					variant={
						metrics?.atRiskCount && metrics.atRiskCount > 5
							? "danger"
							: "warning"
					}
				/>
						...
						<CardHeader>
							<div className="flex items-center justify-between">
								<div>
									...
									<CardDescription>
										Score baseado em: dias sem sessão, taxa de cancelamento e
										número de sessões
									</CardDescription>
								</div>
						...
						<CardContent className="space-y-4">
							...
							<div className="flex flex-wrap items-center gap-4">
								...
								<Select
									value={riskFilter}
									onValueChange={(v) =>
										setRiskFilter(v as "all" | "high" | "medium" | "low")
									}
								>
							...
							<div className="flex items-center gap-4">
								...
								<div>
									...
									<p className="text-3xl font-bold text-red-600">
										R${" "}
										{(metrics?.projectedRevenueLoss || 0).toLocaleString(
											"pt-BR",
											{ minimumFractionDigits: 2 },
										)}
									</p>
									<p className="text-xs text-red-700 dark:text-red-300 mt-1">
										Se os pacientes em risco não forem reativados nos próximos 3
										meses
									</p>
								</div>
							</div>
