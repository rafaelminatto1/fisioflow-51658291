...
export const PatientMessages = () => {
  ...
  const filteredPatients = useMemo(() => {
    ...
    return patients.filter((p) => (p.name || "").toLowerCase().includes(search)).slice(0, 20);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-base sm:text-lg font-semibold leading-tight">Comunicações</h1>
              <div className="flex items-center gap-2 text-xs text-muted-foreground flex-wrap mt-0.5">
                <span>
                  <span className="font-medium text-foreground">{stats?.total || 0}</span> total
                </span>
                <span className="text-border">·</span>
                <span>
                  <span className="font-medium text-green-600">{stats?.sent || 0}</span> enviadas
                </span>
                <span className="text-border hidden sm:inline">·</span>
                <span className="hidden sm:inline">
                  <span className="font-medium text-blue-600">{stats?.delivered || 0}</span>{" "}
                  entregues
                </span>
                {(stats?.failed || 0) > 0 && (
                  <>
                    <span className="text-border">·</span>
                    <span className="font-medium text-red-600">{stats?.failed}</span> falhas
                  </>
                )}
              </div>
            </div>
                  ...
                  <ScrollArea className="h-[600px]">
                    <div className="divide-y divide-border/40">
                      {filteredCommunications.map((comm) => (
                        <div key={comm.id} className="p-4 hover:bg-accent/30 transition-all group">
                                ...
                                >
                                  {comm.status === "falha" && (
                                    <DropdownMenuItem
                                      ...
                                      onClick={() => resendCommunication.mutate(comm.id)}
                                    >
            ...
            <Card className="rounded-3xl border-border/40 shadow-premium-sm sticky top-6">
              ...
              <CardContent className="p-5 space-y-5">
                    ...
                    {(["email", "whatsapp", "sms"] as const).map((channel) => (
                      <Button
                        ...
                        className={cn(
                          ...
                          sendChannel === channel ? "shadow-md scale-105" : "hover:bg-accent/50",
                        )}
                        onClick={() => setSendChannel(channel)}
                      >
                        {channel === "email" && <Mail className="w-4 h-4 mb-1.5" />}
                        {channel === "whatsapp" && <MessageSquareIcon className="w-4 h-4 mb-1.5" />}
                        {channel === "sms" && <Phone className="w-4 h-4 mb-1.5" />}
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">
                      {isBulkMode ? `Destinatários (${selectedPatients.length})` : "Destinatário"}
                    </label>
                ...
                <div className="space-y-2">
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          const suggestion = await suggestReply.mutateAsync({
                            ...
                            context: "Acompanhamento clínico e engajamento no tratamento.",
                          });
                          setMessage(suggestion);
                        }}
                        disabled={suggestReply.isPending || selectedPatients.length === 0}
                      >
                            ...
                            <MessageTemplates
                              onSelectTemplate={(template) => {
                                ...
                                if (selectedPatients.length > 0) {
                                  ...
                                  body = body.replace(/{nome}/g, firstPatient.name);
                                }
                                setMessage(body);
                  ...
                  />
                  {selectedPatients.length > 0 && message.includes("{nome}") && (
                    <p className="text-[10px] font-bold text-primary/80 flex items-center gap-1.5 bg-primary/5 p-2 rounded-xl border border-primary/10">
                      <Sparkles className="h-3 w-3" />O marcador {"{nome}"} será personalizado
                      automaticamente.
                    </p>
                  )}
                </div>
                ...
                <Button
                  ...
                  disabled={
                    sendCommunication.isPending || selectedPatients.length === 0 || !message
                  }
                >
                  {sendCommunication.isPending ? (
                    ...
                    <>
                      ...
                      {isBulkMode ? `Enviar para ${selectedPatients.length}` : "Enviar Mensagem"}
                    </>
                  )}
                </Button>
              </CardContent>
        ...
        <AlertDialogContent className="rounded-3xl border-border/40">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="font-medium text-xs uppercase tracking-widest text-muted-foreground/60 leading-relaxed">
              Esta ação removerá o registro histórico. O paciente não será afetado, mas você não
              verá mais esta mensagem aqui.
            </AlertDialogDescription>
          </AlertDialogHeader>
