...
function BlockEditorFields({
  ...
  switch (block.type) {
    ...
    case "database": {
      ...
      return <WikiDatabaseBlockEditor database={database} onChange={updateDatabase} />;
    }
 ...
 */
export function WikiPageViewer({ page, onEdit }: { page: WikiPage; onEdit: () => void }) {
  const { user } = useAuth();
  ...
  return (
    <WikiPageViewerContent
      ...
      onToggleComment={(blockId) =>
        setActiveCommentBlockId((currentValue) => (currentValue === blockId ? null : blockId))
      }
