...
export function WikiBlockRenderer({ block }: { block: WikiBlockLike }) {
  switch (block.type) {
    ...
    case "heading1":
      return <h1 className="text-3xl font-bold tracking-tight">{block.text || "Titulo H1"}</h1>;
    case "heading2":
      return <h2 className="text-2xl font-semibold tracking-tight">{block.text || "Titulo H2"}</h2>;
    case "heading3":
      return <h3 className="text-xl font-semibold tracking-tight">{block.text || "Titulo H3"}</h3>;
    case "callout": {
