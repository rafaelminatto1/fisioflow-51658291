...
interface CreateBoardModalProps {
  ...
  onSubmit: (data: { name: string; description?: string; background_color: string; icon: string }) => void;
  isLoading?: boolean;
}

export function CreateBoardModal({ open, onOpenChange, onSubmit, isLoading }: CreateBoardModalProps) {
  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [selectedColor, setSelectedColor] = useState(BOARD_PRESET_COLORS[0].value);
  const [selectedIcon, setSelectedIcon] = useState(BOARD_ICONS[0]);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) return;
    onSubmit({ name: name.trim(), description: description.trim() || undefined, background_color: selectedColor, icon: selectedIcon });
    setName('');
    setDescription('');
              ...
              {BOARD_ICONS.map(icon => (
                <button
                  ...
                  className={cn(
                    ...
                    selectedIcon === icon ? 'border-primary bg-primary/10' : 'border-transparent hover:border-muted-foreground/30'
                  )}
              ...
              {BOARD_PRESET_COLORS.map(color => (
                <button
                  ...
                  className={cn(
                    ...
                    selectedColor === color.value ? 'border-foreground scale-110 shadow-md' : 'border-transparent'
                  )}
