...
function AutomationForm({
	...
	const [triggerType, setTriggerType] = useState(
		automation?.trigger.type ?? "status_changed",
	);
	const [actionType, setActionType] = useState(
		automation?.actions[0]?.type ?? "change_status",
	);
	...
	const [notifChannel, setNotifChannel] = useState<"inapp" | "whatsapp">(
		(automation?.actions[0] as { channel?: "inapp" | "whatsapp" })?.channel ??
			"inapp",
	);
...
export function AutomationCenter({
	...
	const [editingAutomation, setEditingAutomation] =
		useState<BoardAutomation | null>(null);
	...
	const automations: BoardAutomation[] = (
		data?.data ?? []
	) as BoardAutomation[];
	...
	const createMutation = useMutation({
		mutationFn: (d: Record<string, unknown>) =>
			boardAutomationsApi.create(boardId, d),
		onSuccess: () => {
			invalidate();
			setFormOpen(false);
		},
			...
			<Dialog open={open} onOpenChange={onOpenChange}>
				<DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
					...
					<div className="flex items-start gap-2 rounded-xl bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 p-3 text-sm text-blue-700 dark:text-blue-300">
						...
						<p>
							Automações executam regras automaticamente quando eventos acontecem no
							board. Você pode ativar ou desativar qualquer regra sem perdê-la.
						</p>
					</div>
					...
					<div className="space-y-2">
						<div className="flex items-center justify-between">
							<p className="text-sm font-semibold">
								Regras ativas ({automations.filter((a) => a.is_active).length}/
								{automations.length})
							</p>
							<Button
								size="sm"
						...
						{!isLoading && automations.length === 0 && (
							<div className="rounded-xl border border-dashed border-border/60 p-6 text-center text-sm text-muted-foreground">
								...
								<p className="text-xs mt-1">
									Use os exemplos abaixo ou crie uma nova regra.
								</p>
							</div>
						)}
			...
			<AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							A regra será removida permanentemente. Você pode pausá-la em vez de
							excluir para mantê-la disponível.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
