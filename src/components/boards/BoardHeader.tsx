...
export function BoardHeader({
  board, currentView, onViewChange, onRename, onStar, onArchive,
}: BoardHeaderProps) {
      ...
      <div className="relative z-10 px-6 pt-4 pb-0 flex flex-col gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          {isRenaming ? (
            <form onSubmit={handleRenameSubmit} className="flex-1">
              <Input
                ...
                onKeyDown={e => {
                  if (e.key === 'Escape') { setRenameValue(board.name); setIsRenaming(false); }
                }}
                autoFocus
              />
            </form>
          ...
          <Button
            variant="ghost"
            size="icon"
            className={cn('text-white/80 hover:text-white hover:bg-white/20', board.is_starred && 'text-yellow-300')}
            onClick={onStar}
          >
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon" className="text-white/80 hover:text-white hover:bg-white/20 ml-auto">
        ...
        {/* View tabs */}
        <Tabs value={currentView} onValueChange={(v) => onViewChange(v as BoardView)}>
          <TabsList className="bg-white/20 border-0 h-8">
            <TabsTrigger value="kanban" className="text-white/80 data-[state=active]:bg-white/30 data-[state=active]:text-white h-7 px-3 text-xs gap-1.5">
              <Layout className="h-3.5 w-3.5" />
              Quadro
            </TabsTrigger>
            <TabsTrigger value="list" className="text-white/80 data-[state=active]:bg-white/30 data-[state=active]:text-white h-7 px-3 text-xs gap-1.5">
              <List className="h-3.5 w-3.5" />
              Lista
            </TabsTrigger>
            <TabsTrigger value="calendar" className="text-white/80 data-[state=active]:bg-white/30 data-[state=active]:text-white h-7 px-3 text-xs gap-1.5">
