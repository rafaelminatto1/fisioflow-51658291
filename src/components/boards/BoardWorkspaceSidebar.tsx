...
export function BoardWorkspaceSidebar({
  ...
  const completionRate = tarefas.length ? Math.round((doneTasks.length / tarefas.length) * 100) : 0;
      ...
      <div className="space-y-4 pr-4">
        ...
        <Card className="border-border/60 bg-card/90 shadow-sm">
          ...
          <CardContent className="space-y-4">
            ...
            {board.description && (
              <p className="text-sm leading-6 text-muted-foreground">{board.description}</p>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-3">
            ...
            {columns
              ...
              .map((column) => {
                ...
                const overLimit = typeof column.wip_limit === "number" && count > column.wip_limit;
                return (
                  <div
                    key={column.id}
        ...
        <Card className="border-border/60 bg-card/90 shadow-sm">
          ...
          <CardContent className="space-y-3">
            {upcomingTasks.length === 0 && (
              <p className="text-sm text-muted-foreground">Nenhum prazo próximo configurado.</p>
            )}
            {upcomingTasks.map((task) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <span className="line-clamp-2 text-sm font-medium">{task.titulo}</span>
