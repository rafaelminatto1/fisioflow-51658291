...
import {
  Plus, MoreHorizontal, Trash2, Pencil, AlertCircle, GripVertical,
} from 'lucide-react';
...
import { KanbanCardV2, KanbanCardContent } from '@/components/tarefas/v2/KanbanCardV2';
      ...
      <Draggable draggableId={`col-${column.id}`} index={index}>
        {(colProvided, colSnapshot) => (
          ...
          >
            ...
            >
              {/* Drag handle */}
              <div {...colProvided.dragHandleProps} className="cursor-grab active:cursor-grabbing text-muted-foreground/60 hover:text-foreground">
                ...
                <TooltipTrigger>
                  <Badge
                    ...
                    className={cn(
                      ...
                      isAtWip && !isOverWip && 'bg-yellow-500/20 text-yellow-700'
                    )}
                  >
                    {tarefas.length}{column.wip_limit ? `/${column.wip_limit}` : ''}
                  </Badge>
                </TooltipTrigger>
                <TooltipContent>
                  {isOverWip
              ...
              {isOverWip && <AlertCircle className="h-4 w-4 text-red-500 flex-shrink-0" />}
              ...
              <div className="flex items-center gap-0.5 flex-shrink-0">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-44">
                    ...
                    <DropdownMenuItem onClick={() => { setWipInput(String(column.wip_limit ?? '')); setWipEditOpen(true); }}>
            ...
            {wipEditOpen && (
              <div className="px-3 py-2 bg-muted/50 border-b flex items-center gap-2">
                <span className="text-xs text-muted-foreground flex-1">Limite WIP (vazio = sem limite):</span>
                <Input
                  ...
                  onKeyDown={e => { if (e.key === 'Enter') handleWipSave(); if (e.key === 'Escape') setWipEditOpen(false); }}
                  autoFocus
                />
                <Button size="sm" className="h-7 px-2 text-xs" onClick={handleWipSave}>OK</Button>
              </div>
            )}
                  ...
                  >
                    ...
                    {tarefas.length === 0 && !snapshot.isDraggingOver && (
                      ...
                      >
                        ...
                        <p className="text-xs text-muted-foreground">Adicionar tarefa</p>
                      </div>
                    )}
                    {tarefas.length === 0 && snapshot.isDraggingOver && (
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              As tarefas desta coluna não serão apagadas, mas perderão a associação com a coluna.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => { onDeleteColumn(column.id); setDeleteConfirmOpen(false); }}
            >
              Deletar
            </AlertDialogAction>
          </AlertDialogFooter>
