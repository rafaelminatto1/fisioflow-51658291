...
export function SyncManager(userOptions: SyncManagerOptions = {}) {
	 ...
	 */
	const performCriticalDataCache = useCallback(
		async (retryCount = 0): Promise<boolean> => {
			if (isCachingRef.current) {
				if (options.verboseLogging) {
					logger.debug(
						"[SyncManager] Cache já em progresso, pulando",
						{},
						"SyncManager",
					);
				}
			...
			try {
				if (options.verboseLogging) {
					logger.info(
						"[SyncManager] Iniciando cache de dados críticos",
						{
							retryCount,
							connectionType,
						},
				...
				logger.error(
					"[SyncManager] Erro ao fazer cache de dados críticos",
					{
				...
				if (retryCount < 3) {
					...
					if (options.verboseLogging) {
						logger.info(
							`[SyncManager] Tentando novamente em ${delay}ms`,
	...
	const forceSyncNow = useCallback(async (): Promise<void> => {
		...
		if (timeSinceLastSync < 5000) {
			if (options.verboseLogging) {
				logger.debug(
					"[SyncManager] Sync muito recente, ignorando",
					{
						timeSinceLastSync,
					},
					"SyncManager",
		...
		try {
			if (options.verboseLogging) {
				logger.info(
					"[SyncManager] Forçando sincronização",
					{
						pendingActions: stats.pendingActions,
					},
					"SyncManager",
			...
			if (options.verboseLogging) {
				logger.info(
					"[SyncManager] Sincronização concluída",
					{
						result,
						stats,
					},
			...
			logger.error(
				"[SyncManager] Erro na sincronização forçada",
				{
					error: error instanceof Error ? error.message : String(error),
				},
				"SyncManager",
	...
	useEffect(() => {
		...
		if (isOnline && !initialCacheDoneRef.current) {
			if (options.verboseLogging) {
				logger.info(
					"[SyncManager] Primeira inicialização online",
					{},
					"SyncManager",
				);
			}
			...
			const timer = setTimeout(() => {
				performCriticalDataCache().then((success) => {
					if (success) {
						...
						if (options.verboseLogging) {
							logger.info(
								"[SyncManager] Cache inicial concluído",
								{},
								"SyncManager",
							);
						}
		...
		isOnline,
		options.enableCriticalDataCache,
		options.verboseLogging,
	...
	useEffect(() => {
		...
		const handleOnline = () => {
			if (options.verboseLogging) {
				logger.info(
					"[SyncManager] Transição offline → online detectada",
					{
						connectionType,
					},
					"SyncManager",
	...
	useEffect(() => {
		if (options.verboseLogging && stats.pendingActions > 0) {
			logger.info(
				"[SyncManager] Status de sync atualizado",
				{
					pendingActions: stats.pendingActions,
	...
	useEffect(() => {
		if (import.meta.env.DEV && typeof window !== "undefined") {
			...
			if (options.verboseLogging) {
				logger.info(
					"[SyncManager] API exposta em window.__fisioflow_sync_manager__",
					{
						version: SYNC_MANAGER_VERSION,
					},
					"SyncManager",
	...
	useEffect(() => {
		return () => {
			if (options.verboseLogging) {
				logger.info(
					"[SyncManager] Componente desmontado",
					{
						stats,
						isOnline,
					},
