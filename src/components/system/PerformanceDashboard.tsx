...
} from "@/lib/database/performanceMonitor";
import {
	Dialog,
	DialogContent,
	DialogHeader,
				...
				<div className="space-y-4">
					...
					<Card>
						<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
							...
							<AlertTriangle
								className={`h-4 w-4 ${report?.slowQueries.length ? "text-amber-500" : "text-muted-foreground"}`}
							/>
						</CardHeader>
						<CardContent>
					...
					<Card>
						...
						<CardContent>
							...
							<p className="text-xs text-muted-foreground">&gt; 10MB size</p>
						</CardContent>
					</Card>
				</div>
			</div>
								...
								<TableRow>
									...
									>
										<div className="flex flex-col items-center justify-center gap-2">
											...
											<p className="text-xs">
												Great job! Your database is performing well.
											</p>
										</div>
									</TableCell>
								</TableRow>
								...
								report.slowQueries.map((query) => (
									<TableRow key={query.queryId}>
										...
										<TableCell>
											<Badge
												variant={
													query.meanTime > 500 ? "destructive" : "secondary"
												}
												className="font-mono"
											>
											...
											<TooltipProvider>
												<Tooltip>
													...
													<TooltipContent
														side="bottom"
														align="start"
														className="max-w-[500px] p-4 font-mono text-xs bg-card border-border shadow-xl"
													>
						...
						<TableBody>
							{stats.length === 0 ? (
								<TableRow>
									...
									>
										No statistics available yet. Run "Analyze Tables" to gather
										data.
									</TableCell>
								</TableRow>
							) : (
	...
	return (
		<Dialog open={isOpen} onOpenChange={setIsOpen}>
          ...
          `}
				...
				>
					<Activity
						className={`h-4 w-4 ${avgTime > 500 ? "animate-pulse" : ""}`}
					/>
			...
			<div className="flex-1 overflow-hidden p-6">
				<Tabs defaultValue="overview" className="h-full flex flex-col">
					<TabsList className="w-full justify-start mb-6 bg-muted/50 p-1">
						<TabsTrigger value="overview" className="flex-1 max-w-[200px]">
							Overview
						</TabsTrigger>
						...
						<TabsTrigger value="database" className="flex-1 max-w-[200px]">
							Database Stats
						</TabsTrigger>
					</TabsList>
						...
						>
							<OverviewTab
								averageTime={
									history.length
										? history[history.length - 1].value
										: QueryOptimizer.getAverageQueryTime()
								}
								report={report}
								history={history}
							/>
