...
const CycleCard: React.FC<CycleCardProps> = ({ cycle, index, onEdit }) => {
  ...
  const progressPct =
    cycle.sessionsCount > 0 ? Math.round((cycle.completedSessions / cycle.sessionsCount) * 100) : 0;
...
const EmptyState: React.FC<EmptyStateProps> = ({ onCreateCycle }) => (
  ...
  >
    ...
    <h3 className="text-lg font-bold text-slate-800 mb-2">Organize o Tratamento em Ciclos</h3>
    <p className="text-sm text-slate-500 max-w-sm leading-relaxed mb-6">
      Agrupe sessões em ciclos terapêuticos de 2-4 semanas, acompanhe o progresso e faça transições
      estruturadas entre fases do tratamento. Inspirado nas melhores práticas de gestão ágil.
    </p>
            ...
            >
              ...
              <div className="flex flex-col gap-3">
                {sortedCycles.map((cycle, i) => (
                  <CycleCard key={cycle.id} cycle={cycle} index={i} onEdit={onEditCycle} />
                ))}
              </div>
            </motion.div>
          ) : (
