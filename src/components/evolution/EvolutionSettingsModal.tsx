  ...
  try {
    ...
    if (stored) {
      ...
      return {
        ...DEFAULT_SETTINGS,
        ...parsed,
        slashCommands: DEFAULT_SETTINGS.slashCommands.map(defaultCmd => {
          const storedCmd = parsed.slashCommands?.find((c: any) => c.id === defaultCmd.id);
          return storedCmd ? { ...defaultCmd, visible: storedCmd.visible } : defaultCmd;
        }).sort((a, b) => {
          const idxA = parsed.slashCommands?.findIndex((c: any) => c.id === a.id) ?? -1;
          const idxB = parsed.slashCommands?.findIndex((c: any) => c.id === b.id) ?? -1;
...
export function EvolutionSettingsModal({ open, onOpenChange, currentViewVersion, onViewVersionChange }: EvolutionSettingsModalProps) {
  ...
  const moveCommand = (index: number, direction: -1 | 1) => {
    ...
    if (index + direction < 0 || index + direction >= newCommands.length) return;
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="py-4 space-y-6">
          <div className="space-y-3">
            <Label className="text-base font-semibold">Visualização Padrão</Label>
            <div className="grid grid-cols-2 gap-2">
              <Button 
                variant={settings.defaultViewVersion === 'v4-tiptap' ? 'default' : 'outline'} 
                className="justify-start"
                onClick={() => setSettings({ ...settings, defaultViewVersion: 'v4-tiptap' })}
              >
                Tiptap (Recomendado)
              </Button>
              <Button 
                variant={settings.defaultViewVersion === 'v3-notion' ? 'default' : 'outline'} 
                className="justify-start"
                onClick={() => setSettings({ ...settings, defaultViewVersion: 'v3-notion' })}
              >
                Estilo Notion
              </Button>
              <Button 
          ...
          <div className="space-y-3">
            <Label className="text-base font-semibold">Comandos Rápidos ( / )</Label>
            <p className="text-xs text-muted-foreground">
              Reordene ou oculte os atalhos disponíveis no editor Tiptap.
            </p>
            <div className="space-y-2 border rounded-md p-2 bg-muted/20">
              {settings.slashCommands.map((cmd, idx) => (
                <div key={cmd.id} className={cn("flex items-center justify-between p-2 rounded-md border bg-background", !cmd.visible && "opacity-60")}>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      {cmd.visible ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
                    </Button>
                  </div>
                </div>
              ))}
