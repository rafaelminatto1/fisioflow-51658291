...
export const OptimizedSection = memo(
  ({
    ...
    return <div className={cn("transition-all duration-200", className)}>{content}</div>;
  },
  (prev, next) => {
...
export const StatCard = memo(
  ({ label, value, change, icon, className, trend = "neutral" }: StatCardProps) => {
...
export const StatusBadge = memo(
  ({ status, variant = "default", className }: StatusBadgeProps) => {
    ...
    return <Badge className={cn(variantStyles, className)}>{status}</Badge>;
  },
...
export const SectionSeparator = memo(
  ({ title, action, className }: SectionSeparatorProps) => {
    return (
