...
export const CompactViewProvider: React.FC<{
  ...
  return <CompactViewContext.Provider value={value}>{children}</CompactViewContext.Provider>;
};
...
export const CompactSection: React.FC<CompactSectionProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {isComplete && <span className="w-2 h-2 rounded-full bg-emerald-500 flex-shrink-0" />}
          </div>
          {subtitle && !compact && (
      ...
      >
        <div className={cn("px-4 py-3", compact && "pt-2")}>{children}</div>
      </div>
    </div>
  );
};
