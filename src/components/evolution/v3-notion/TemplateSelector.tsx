  ...
  const filteredTemplates = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter((template) => {
        ...
        return expandedTerms.some((term: string) => searchIn.some((field) => field.includes(term)));
      });
    }
        ...
        >
          ...
          <div className="px-4 pt-4 pb-3 border-b border-border">
            <div className="flex items-center justify-between mb-3">
              <div>
                <h3 className="text-sm font-semibold text-foreground">Templates Clínicos</h3>
          ...
          <div className="flex-1 min-h-0 overflow-y-auto p-2 space-y-1">
            {filteredTemplates.length === 0 ? (
              <div className="text-center py-10 text-muted-foreground">
                ...
                <p className="text-xs mt-1 opacity-60">Tente outro termo de busca ou categoria</p>
              </div>
            ) : (
              filteredTemplates.map((template) => {
                ...
                return (
                  ...
                  >
                    {/* Emoji icon */}
                    <span className="text-xl mt-0.5 select-none flex-shrink-0">{meta.emoji}</span>
