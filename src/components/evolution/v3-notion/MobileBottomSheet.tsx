...
export const MobileSheetProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const openSheet = useCallback(
    (id: string, title: string, description: string | undefined, content: React.ReactNode) => {
...
const BottomSheet: React.FC<BottomSheetProps> = ({
  ...
  return (
    <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <SheetContent
        side="bottom"
        className={cn("h-[80vh] rounded-t-2xl border-t-0", "flex flex-col", className)}
      >
        {/* Drag handle */}
        ...
        <SheetHeader className="px-6 pb-4 border-b border-border/50">
          ...
          {description && (
            <SheetDescription className="text-base mt-1">{description}</SheetDescription>
          )}
        </SheetHeader>
...
export const MobileSheetWrapper: React.FC<MobileSheetWrapperProps> = ({
  ...
  const handleOpenSheet = useCallback(() => {
    openSheet(
      ...
      title,
      description,
      children({ isMobile: true, openSheet: () => {}, closeSheet }),
    );
  ...
  return (
    <div className={className}>
      {children({
        isMobile,
        openSheet: isMobile ? handleOpenSheet : () => {},
        closeSheet,
      })}
    </div>
  );
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium">{exercises?.length || 0} exercícios</span>
                  </div>
                  <span className="text-sm text-muted-foreground">Tap para gerenciar</span>
                </div>
              </button>
            </div>
          ) : (
            ...
            <div className="mobile-sheet-trigger">
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <GripVertical className="h-5 w-5 text-muted-foreground" />
                    <span className="font-medium">{measurements?.length || 0} mensurações</span>
                  </div>
                  <span className="text-sm text-muted-foreground">Tap para gerenciar</span>
                </div>
              </button>
