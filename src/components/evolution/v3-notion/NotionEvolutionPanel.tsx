...
const NotionEvolutionPanel: React.FC<NotionEvolutionPanelProps> = ({
  ...
  const [localSaveStatus, setLocalSaveStatus] = useState<"idle" | "saving" | "saved" | "error">(
    "idle",
  );
  ...
  const rawEvolutionId =
    evolutionId || `sessao-${data.sessionNumber || "draft"}-${data.sessionDate || "agora"}`;
  ...
  const handleTemplateSelect = useCallback(
    (template: SOAPTemplate) => {
      ...
      const assessmentBlock = template.assessment
        ...
        : "";
      const planBlock = template.plan ? `<h2>Plano</h2>${sectionSep}${template.plan}` : "";
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pb-32">
        <div className="sticky top-0 z-[50] bg-white/95 backdrop-blur-md border-b border-[#E9E9E8] shadow-sm transition-all duration-300">
          <RichTextToolbar
            ...
            imageUploadFolder={
              patientId ? `patients/${patientId}/evolutions/${safeEvolutionId}` : undefined
            }
          />
        </div>
              ...
              <div className="lg:hidden space-y-4 mb-10">
                <div className="p-4 rounded-xl border border-[#ECEBEA] bg-[#FBFBFA]">
                  <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                    ...
                    <span>
                      Palavras‑chave:{" "}
                      {summaryStats.keywords.length ? summaryStats.keywords.join(", ") : "—"}
                    </span>
                  </div>
                </div>
