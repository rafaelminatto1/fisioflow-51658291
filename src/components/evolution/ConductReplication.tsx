...
export const ConductReplication: React.FC<ConductReplicationProps> = ({
	...
	const [activeTab, setActiveTab] = useState<
		"suggestions" | "previous" | "library"
	>("suggestions");
	const [selectedCategory, setSelectedCategory] = useState<string>("all");

	const { data: soapRecords = [], isLoading: loadingRecords } = useSoapRecords(
		patientId,
		20,
	);
	const { data: conductLibrary = [], isLoading: loadingLibrary } =
		useConductLibrary();
	const { data: suggestions = [], isLoading: loadingSuggestions } =
		useIntelligentConductSuggestions(patientId);
	...
	const categories = [
		"all",
		...Array.from(new Set(conductLibrary.map((c) => c.category))),
	];
		...
		<Dialog open={isOpen} onOpenChange={setIsOpen}>
			...
			<DialogContent className="max-w-2xl">
				<DialogHeader>
					...
					<DialogDescription>
						Selecione uma conduta anterior deste paciente ou da biblioteca para
						copiar para o plano atual.
					</DialogDescription>
				</DialogHeader>
							...
							<div className="space-y-2">
								{loadingSuggestions ? (
									...
									<div className="text-center py-8 text-muted-foreground">
										...
										<p className="text-xs">
											Registre patologias e evoluções para receber sugestões
											personalizadas
										</p>
									</div>
								) : (
									suggestions.map((suggestion) => (
										<div
											...
											onClick={() =>
												handleSelectLibraryItem(suggestion.conduct_text)
											}
										>
								...
								{loadingLibrary ? (
									...
									filteredLibrary.map((conduct) => (
										<div
											...
											className="p-3 rounded-lg border hover:bg-muted/50 cursor-pointer transition-colors"
											onClick={() =>
												handleSelectLibraryItem(conduct.conduct_text)
											}
										>
											<div className="flex items-center justify-between mb-2">
												<span className="text-sm font-medium">
