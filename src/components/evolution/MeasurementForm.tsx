...
import {
	...
	type ClinicalTest as ClinicalTestComboboxTest,
} from "@/components/ui/clinical-test-combobox";
import { CustomFieldsConfig } from "./CustomFieldsConfig";
import {
	DEFAULT_MEASUREMENT_FIELDS,
	type CustomField,
} from "@/lib/evolution/customFields";
...
import {
	TestLibraryModal,
	type ClinicalTest,
} from "@/components/clinical/TestLibraryModal";
import {
	Collapsible,
	CollapsibleContent,
	CollapsibleTrigger,
...
export const MeasurementForm: React.FC<MeasurementFormProps> = ({
	...
	const presets = [
		{
			label: "Sinais Vitais",
			type: "Sinais Vitais",
			name: "Checkup de Sinais",
		},
		{
			label: "Fisioterapia Ortopédica",
			type: "Amplitude de Movimento",
			name: "Flexão",
		},
		{
			label: "Esportiva (Hop Test)",
	...
	const handleSelectTest = (
		index: number,
		testId: string,
		test?: ClinicalTest | ClinicalTestComboboxTest,
	) => {
		const updated = [...measurements];
		...
		if (test?.fields_definition && test.fields_definition.length > 0) {
			...
			const newCustomFields = customFields.map((f) => {
				const templateField = test.fields_definition?.find(
					(tf) =>
						tf.id === f.id || tf.label.toLowerCase() === f.label.toLowerCase(),
				);
				if (templateField) {
					return {
						...f,
				...
				if (["measurement_name", "value"].includes(f.id))
	...
	const deriveValueFromCustomData = (
		custom_data: Record<string, string>,
	): number => {
	...
	const hasMultiFieldFromTemplate = (m: MeasurementInput) =>
		m.selectedTest?.fields_definition &&
		m.selectedTest.fields_definition.length >= 3 &&
	...
	const isYBalanceTest = (test?: ClinicalTest) =>
		...
		(!!test?.tags &&
			test.tags.some((t) => /y\s*balance|y\s*test|ybt/i.test(String(t))));

	const showYBalanceDiagram = (m: MeasurementInput) =>
		isYBalanceTest(m.selectedTest) ||
		(!!m.measurement_name &&
			/y\s*test|y\s*balance|ybt/i.test(m.measurement_name));

	const hasYBalanceValue = (m: MeasurementInput) =>
		showYBalanceDiagram(m) &&
		Y_BALANCE_KEYS.some((k) => (m.custom_data[k] ?? "").trim() !== "");

	const hasAnyValue = (m: MeasurementInput) =>
		m.value ||
		m.measurement_type === "Personalizado" ||
		m.measurement_type === "Sinais Vitais" ||
		(hasMultiFieldFromTemplate(m) &&
			Object.values(m.custom_data).some((v) => v !== "")) ||
		hasYBalanceValue(m);
	...
	const hasCustomType = measurements.some(
		(m) => m.measurement_type === "Personalizado",
	);
		...
		<Card className="border-teal-100/50 shadow-xl shadow-teal-900/5 overflow-hidden rounded-2xl bg-white">
					...
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<p className="text-xs text-slate-500 font-medium">
								Capture dados clínicos com precisão
							</p>
						</div>
					</div>
				...
				<div className="px-6 pt-4 pb-2">
					<div className="flex items-center justify-between mb-2">
						...
						<span className="text-xs text-slate-500">
							{
								measurements.filter((m) => m.measurement_name && hasAnyValue(m))
									.length
							}{" "}
							/ {measurements.length}
						</span>
			...
			<CardContent className="p-6 space-y-6">
				...
				<Button
					...
					size="sm"
					variant="outline"
					className="w-full sm:hidden border-teal-100 text-teal-700 font-bold"
				>
					<BookOpen className="h-4 w-4 mr-2" />
						...
						<div className="flex flex-wrap gap-2">
							{requiredMeasurements.map((req) => (
								<Button
									...
									size="sm"
									onClick={() =>
										handleQuickFillRequired(req, measurements.length - 1)
									}
									className={cn(
						...
						{measurements.map((measurement, index) => (
							...
							>
								...
								{measurement.selectedTest && (
									<div
										className="mb-4 rounded-xl border border-teal-100 bg-teal-50/30 overflow-hidden"
										role="region"
										aria-label="Como realizar o teste"
									>
										...
										<div className="p-3">
											{measurement.selectedTest.image_url ||
											measurement.selectedTest.media_urls?.[0] ? (
												...
												<p className="text-sm text-slate-500 italic">
													Nenhuma mídia ou instrução cadastrada para este teste.
												</p>
											)}
										</div>
									</div>
								...
								<div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
									...
									<div className="lg:col-span-5 space-y-4">
										<div className="space-y-2">
											...
											<ClinicalTestCombobox
												...
												onValueChange={(testId, test) =>
													handleSelectTest(index, testId, test)
												}
										...
										<div className="space-y-2">
											...
											>
												<SelectTrigger
													className={cn(
														"h-11 bg-white border-slate-200 font-medium",
														inputFocusClasses,
													)}
												>
									...
									<div className="lg:col-span-7 space-y-4">
										<div className="space-y-2">
											<Label
												htmlFor={`measurement-name-${index}`}
											...
											<Input
												...
												className={cn(
													"h-11 bg-white border-slate-200 font-semibold text-slate-700",
													inputFocusClasses,
												)}
											...
											{showYBalanceDiagram(measurement) ? (
												<div className="col-span-2 space-y-4">
													<MeasurementDiagramYBalance
														values={{
															...
															posteromedial:
																measurement.custom_data.posteromedial ?? "",
															posterolateral:
																measurement.custom_data.posterolateral ?? "",
														}}
														unit={measurement.unit || "cm"}
														onChange={(key, value) =>
															handleUpdateCustomData(index, key, value)
														}
														compositeLabel="Composto (média)"
														compositeValue={
															[
																measurement.custom_data.anterior,
																measurement.custom_data.posteromedial,
																measurement.custom_data.posterolateral,
															].every((v) => v !== "" && v != null)
																? deriveValueFromCustomData(
																		measurement.custom_data,
																	)
																: null
														}
													...
													<div className="space-y-2 pt-2 border-t border-slate-100">
														...
														<Textarea
															...
															className={cn(
																"min-h-[80px] resize-y bg-white border-slate-200 text-sm",
																inputFocusClasses,
															)}
															rows={3}
														/>
												...
												<div className="col-span-2 space-y-4">
													<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
														{measurement.selectedTest!.fields_definition!.map(
															(field) => (
																<div
																	key={field.id}
																	className={cn(
																...
																>
																	...
																	>
																		{field.label}
																		{field.unit && (
																			<span className="text-teal-600 lowercase tracking-normal bg-teal-50 px-1.5 py-0.5 rounded text-[11px]">
																				({field.unit})
																			</span>
																		)}
																		{field.required && (
																	...
																	<Input
																		...
																		value={
																			measurement.custom_data[field.id] || ""
																		}
																		onChange={(e) =>
																			handleUpdateCustomData(
																				index,
																				field.id,
																				e.target.value,
																			)
															...
															return (
																...
																>
																	...
																	{field.type === "select" ? (
																		<Select
																			value={
																				measurement.custom_data[fieldKey] || ""
																			}
																			onValueChange={(val) =>
																				handleUpdateCustomData(
																					index,
																		...
																		>
																			<SelectTrigger
																				className={cn(
																					"h-11 bg-white font-medium border-slate-200",
																					inputFocusClasses,
																				)}
																			>
																		...
																		<Input
																			...
																			placeholder={
																				field.description || "Digite..."
																			}
																			className={cn(
																				"h-11 bg-white border-slate-200 font-semibold",
																				inputFocusClasses,
																			)}
																		/>
																	)}
												...
												<>
													...
													<div className="space-y-2">
														...
														<Input
															...
															className={cn(
																"h-11 bg-white border-slate-200 font-medium text-slate-600",
																inputFocusClasses,
															)}
														/>
													</div>
					...
					<div className="space-y-4">
						...
						>
							<span className="flex items-center gap-2">
								<Settings2
									className={cn(
										...
										showFieldsConfig && "rotate-90 text-teal-600",
									)}
								/>
							...
							{showFieldsConfig ? (
								...
								<ChevronDown className="h-4 w-4 opacity-40 group-hover:translate-y-0.5 transition-transform" />
							)}
						</Button>
				...
				<div className="pt-4 border-t border-slate-100">
					<Button
						...
						disabled={
							...
							measurements.every((m) => !m.measurement_name || !hasAnyValue(m))
						}
