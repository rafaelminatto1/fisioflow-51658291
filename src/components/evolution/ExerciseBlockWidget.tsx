...
const EMPTY_STATE_MESSAGES = {
	noExercises: {
		...
		description:
			"Use a busca e filtros acima para encontrar e adicionar exercícios à sessão",
	},
	noResults: {
			...
			{bodyParts.length > 0 && (
				<div className="space-y-2">
					...
					<div
						...
						role="group"
						aria-label="Filtro por partes do corpo"
					>
						{bodyParts.slice(0, MAX_BODY_PARTS_DISPLAY).map((part) => (
							<Badge
								key={part}
								variant={
									filters.bodyParts?.includes(part) ? "default" : "outline"
								}
								className={cn(
						...
						{bodyParts.length > MAX_BODY_PARTS_DISPLAY && (
							<Badge
								variant="outline"
								className="text-[10px] px-2.5 py-0.5 h-6 text-muted-foreground"
							>
								+{bodyParts.length - MAX_BODY_PARTS_DISPLAY}
							</Badge>
						)}
						...
						{displayedExercises.length === 0 ? (
							<div className="p-6 text-center text-muted-foreground">
								...
								<p className="text-sm font-medium">
									{EMPTY_STATE_MESSAGES.noResults.title}
								</p>
								<p className="text-xs mt-1">
									{EMPTY_STATE_MESSAGES.noResults.description}
								</p>
							</div>
						) : (
							<div className="p-1" role="listbox">
								...
								{exercises.length > 15 && (
									<p className="text-center text-xs text-muted-foreground py-2 border-t">
										+{exercises.length - 15} exercícios. Continue digitando para
										filtrar.
									</p>
								)}
							</div>
...
const ExerciseCard = memo<ExerciseCardProps>(
	({ exercise, index, disabled, onUpdate, onRemove }) => {
		const handleUpdate = useCallback(
			(field: keyof SessionExercise, value: string | number | boolean) => {
		...
		const _statusLabel = exercise.completed
					...
					<div className="grid grid-cols-3 gap-2">
						<div className="space-y-1">
							...
							<Input
								...
								value={exercise.sets}
								onChange={(e) =>
									handleUpdate("sets", parseInt(e.target.value) || 0)
								}
						...
						<div className="space-y-1">
							...
							<Input
								...
								onChange={(e) =>
									handleUpdate("repetitions", parseInt(e.target.value) || 0)
								}
								className="h-7 text-xs px-2 font-bold bg-muted/30 focus:bg-background border-none ring-1 ring-border/50"
								disabled={disabled}
							/>
...
export const ExerciseBlockWidget: React.FC<ExerciseBlockWidgetProps> = memo(
	({
		...
		const { exercises: availableExercises, loading: isLoading } =
			useExercises();
		...
		const categories = useMemo(() => {
			const uniqueCats = new Set(
				availableExercises.map((e) => e.category).filter(Boolean),
			);
		...
		const filteredExercises = useMemo(() => {
			return availableExercises.filter((ex) => {
				...
				if (filters.bodyParts && filters.bodyParts.length > 0) {
					...
					const hasMatchingPart = filters.bodyParts.some((part) =>
						exerciseBodyParts.includes(part),
					);
		...
		const handleFilterChange = useCallback(
			(key: keyof ExerciseFilters, value: string | string[] | undefined) => {
			...
			<TooltipProvider>
				<div className={cn("flex flex-col h-full", className)}>
						...
						<div className="flex flex-wrap items-center gap-2">
							...
							{onSuggest && (
								<Tooltip>
									...
									<TooltipContent>
										<p className="text-sm">
											Sugerir exercícios baseados na evolução
										</p>
									</TooltipContent>
								</Tooltip>
							)}
					...
					<ScrollArea className="flex-1">
						<div
							className="p-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-3"
							role="list"
						>
							{exercises.length === 0 ? (
								<div className="flex flex-col items-center justify-center py-12 border-2 border-dashed rounded-lg text-muted-foreground bg-muted/20">
									...
									<p className="text-sm font-medium">
										{EMPTY_STATE_MESSAGES.noExercises.title}
									</p>
