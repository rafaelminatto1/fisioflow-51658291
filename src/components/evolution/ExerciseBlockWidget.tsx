 ...
 * including image candidates for proper display in the exercise cards.
 */
...
const ExerciseImage = memo(
  ({
    ...
    const optimizedSrc = src
      ...
      : undefined;
    const optimizedFallbackSrcs = fallbackSrcs.map((fallbackSrc) =>
      withImageParams(fallbackSrc, {
        width: 640,
        height: 360,
        dpr: 2,
        format: "auto",
    ...
    const currentSrc = optimizedFallbackSrcs[fallbackIndex - 1] ?? optimizedSrc;
    ...
    return (
      <img
        ...
        onError={() => {
          setIsLoading(false);
          if (fallbackIndex < optimizedFallbackSrcs.length) {
            ...
            {displayedExercises.length === 0 ? (
              ...
              <div className="p-1" role="listbox">
                {displayedExercises.map((exercise) => {
                  const imageSrc = getImageUrlCandidates(exercise)[0];
...
const ExerciseCard = memo<ExerciseCardProps>(
  ({ exercise, index, disabled, onUpdate, onRemove }) => {
    ...
    const imageCandidates = getImageUrlCandidates(exercise);
...
export const ExerciseBlockWidget: React.FC<ExerciseBlockWidgetProps> = memo(
  ({
    ...
    const handleUpdateExercise = useCallback(
      (id: string, field: keyof SessionExercise, value: string | number | boolean) => {
