...
const SHORTCUTS = {
  saving: {
    ...
    items: [
      {
        keys: ["Ctrl", "S"],
        description: "Salvar evolução",
        icon: <Save className="h-3 w-3" />,
      },
      {
        keys: ["⌘", "S"],
        description: "Salvar evolução (Mac)",
        icon: <Save className="h-3 w-3" />,
      },
    ],
  },
  completion: {
    ...
    items: [
      {
        keys: ["Ctrl", "Enter"],
        description: "Concluir sessão",
        icon: <CheckCircle className="h-3 w-3" />,
      },
      {
        keys: ["⌘", "Enter"],
        description: "Concluir sessão (Mac)",
        icon: <CheckCircle className="h-3 w-3" />,
      },
    ],
  },
  navigation: {
    ...
    items: [
      {
        keys: ["1"],
        description: "Ir para Subjetivo",
        icon: (
          <span className="w-4 h-4 text-[10px] font-bold flex items-center justify-center bg-blue-500 text-white rounded">
            S
          </span>
        ),
      },
      {
        keys: ["2"],
        description: "Ir para Objetivo",
        icon: (
          <span className="w-4 h-4 text-[10px] font-bold flex items-center justify-center bg-green-500 text-white rounded">
            O
          </span>
        ),
      },
      ...
      {
        keys: ["4"],
        description: "Ir para Plano",
        icon: (
          <span className="w-4 h-4 text-[10px] font-bold flex items-center justify-center bg-orange-500 text-white rounded">
            P
          </span>
        ),
      },
  ...
  ai: {
    ...
    items: [
      {
        keys: ["Alt", "A"],
        description: "Abrir IA Assistant",
        icon: <Sparkles className="h-3 w-3" />,
      },
      {
        keys: ["Ctrl", "Shift", "S"],
        description: "Salvar + Analisar com IA",
...
export const EvolutionKeyboardShortcuts = memo(
  ({ open, onOpenChange }: EvolutionKeyboardShortcutsProps) => {
    return (
        ...
        <DialogContent className="sm:max-w-lg max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <DialogDescription>Ações rápidas para registro de evolução</DialogDescription>
              </div>
            </div>
          </DialogHeader>
          ...
          <div className="mt-4 pt-4 border-t border-border">
            <p className="text-xs text-muted-foreground">
              Pressione{" "}
              <kbd className="px-1.5 py-0.5 bg-muted border border-border rounded text-[10px] mx-1">
                ?
              </kbd>{" "}
