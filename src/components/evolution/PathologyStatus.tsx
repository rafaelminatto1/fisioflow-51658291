          ...
          {pathologies.map((pathology) => (
            ...
            >
              <div className="flex items-start gap-3 flex-1">
                <div className="mt-0.5">{getStatusIcon(pathology.status)}</div>
                <div className="flex-1 space-y-1">
                  <p className="font-medium text-sm">{pathology.pathology_name}</p>
                  {pathology.diagnosis_date && (
                    <p className="text-xs text-muted-foreground">
                      Diagnóstico:{" "}
                      {format(new Date(pathology.diagnosis_date), "dd/MM/yyyy", { locale: ptBR })}
                      <span className="ml-1 text-primary">
                        ({formatDetailedDuration(pathology.diagnosis_date)})
                      </span>
                    </p>
                  )}
                  {pathology.notes && (
                    <p className="text-xs text-muted-foreground italic mt-1">{pathology.notes}</p>
                  )}
                </div>
              </div>
              <Badge
                variant={getStatusColor(pathology.status) as "default" | "secondary" | "outline"}
                className="text-xs"
              >
