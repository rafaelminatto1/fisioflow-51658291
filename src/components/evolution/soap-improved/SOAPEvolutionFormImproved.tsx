...
const COMMON_PROCEDURES: Array<{ name: string; category: ProcedureCategory }> = [
  ...
  {
    name: "Lib miofascial instrumental (IASTM)",
...
export const SOAPEvolutionFormImproved: React.FC<SOAPEvolutionFormProps> = ({
  ...
  const updateFormData = useCallback(
    <K extends keyof SOAPEvolutionFormData>(key: K, value: SOAPEvolutionFormData[K]) => {
  ...
  const getCompletionPercentage = useMemo(() => {
    const sections = [
      formData.soap.subjective,
      formData.soap.objective,
      formData.soap.assessment,
      formData.soap.plan,
    ];
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {isAnimating && (
              <div
                className={cn("absolute inset-0 rounded-2xl animate-ping", colors.bg)}
                style={{ opacity: 0.3 }}
              />
            )}
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2">
              ...
              <p className={cn("text-base font-semibold", colors.text)}>{config.label}</p>
            </div>
...
}> = ({ soap, onChange, activeSection, setActiveSection, disabled, completionPercentage }) => {
  return (
    <div className="rounded-xl border border-border/50 bg-card overflow-hidden shadow-sm hover:shadow-md transition-all duration-300">
      {/* Header */}
      <div className="relative">
        ...
        <div className="px-4 pb-3">
          ...
          <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${(completedCount / procedures.length) * 100}%`,
              }}
            />
          </div>
        </div>
