...
export const ProceduresChecklistImproved: React.FC<ProceduresChecklistImprovedProps> = ({
  ...
  const [selectedCategoryFilter, setSelectedCategoryFilter] = useState<ProcedureCategory | "all">(
    "all",
  );
              ...
              {showCategories && procedures.length > 0 && (
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="sm" disabled={disabled} className="gap-1.5 h-8">
                        ...
                        <CommandGroup key={categoryLabel} heading={categoryLabel}>
                          {groupedByCategory[categoryLabel]
                            ...
                            .map((proc) => (
                              ...
                              >
                                <span className="mr-2">{CATEGORY_COLORS[proc.category].icon}</span>
                                <span>{proc.name}</span>
                              </CommandItem>
                            ))}
                        </CommandGroup>
...
const EmptyState: React.FC<{
  ...
  const quickSuggestions = ["Liberação miofascial", "Mobilização articular", "TENS", "Crioterapia"];
