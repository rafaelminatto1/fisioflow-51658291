...
const COMMON_PROCEDURES: Array<{ name: string; category: ProcedureCategory }> =
	[
		...
		{
			name: "Liberação miofascial instrumental (IASTM)",
			category: "liberacao_miofascial",
		},
...
export const ProceduresChecklistImproved: React.FC<
	ProceduresChecklistImprovedProps
> = ({
	procedures,
	onChange,
	disabled = false,
							...
							{showCategories && procedures.length > 0 && (
								<DropdownMenu>
									...
									<DropdownMenuContent align="end" className="w-48">
										...
										{Object.entries(PROCEDURE_CATEGORY_LABELS).map(
											([key, label]) => (
												<DropdownMenuItem
													key={key}
													onClick={() =>
														setSelectedCategoryFilter(key as ProcedureCategory)
													}
												>
													<span className="mr-2">
														{CATEGORY_COLORS[key as ProcedureCategory].icon}
													</span>
													{label}
												</DropdownMenuItem>
											),
												...
												>
													{groupedByCategory[categoryLabel]
														.filter((p) => {
															...
															const notAdded = !existingNames.has(
																p.name.toLowerCase(),
															);
				...
				{showProgress && procedures.length > 0 && (
					<div className="px-4 pb-3">
						<div className="flex items-center justify-between text-[10px] text-muted-foreground mb-1.5">
							...
							<span>
								{Math.round((completedCount / procedures.length) * 100)}%
							</span>
						</div>
	...
	return (
		<div className="text-center py-10 text-muted-foreground">
			...
			<p className="text-xs mt-1.5 opacity-70">
				Use o botão "Adicionar" ou digite abaixo
			</p>
			...
			>
				<div className="flex items-center gap-3 py-2.5 px-3">
					...
					{procedure.notes && (
						<Badge
							variant="secondary"
							className="text-[10px] h-6 px-2 gap-1 rounded-full"
						>
							...
							<div className="grid grid-cols-2 gap-3">
								<div className="space-y-1">
									...
									<Input
										...
										onChange={(e) =>
											onUpdateField(procedure.id, "duration", e.target.value)
										}
								...
								<div className="space-y-1">
									...
									<Input
										...
										onChange={(e) =>
											onUpdateField(procedure.id, "parameters", e.target.value)
										}
