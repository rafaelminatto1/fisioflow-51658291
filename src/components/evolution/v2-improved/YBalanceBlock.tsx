...
export const YBalanceBlock: React.FC<YBalanceBlockProps> = ({
	...
	const [hoveredDirection, setHoveredDirection] = useState<YBalanceKey | null>(
		null,
	);
						...
						return (
							<g key={dir.key}>
								...
								<line
									...
									y2={dir.position.y * 2}
									stroke={
										isHovered ? `url(#gradient-${dir.key})` : "currentColor"
									}
						...
						{DIRECTIONS.map((dir) => (
							...
							>
								<stop
									offset="0%"
									className={cn(
										dir.color.replace("text-", "text-").replace("-600", "-400"),
									)}
								/>
								<stop
									offset="100%"
									className={cn(
										dir.color.replace("text-", "text-").replace("-600", "-500"),
									)}
								/>
							</linearGradient>
						))}
			...
			{hasAllValues && (
				<div className="flex items-center justify-center gap-2">
					...
					<span className="text-xs text-muted-foreground">
						Asymetria:{" "}
						{Math.abs(
							(parseFloat(values.posteromedial) || 0) -
								(parseFloat(values.posterolateral) || 0),
						).toFixed(1)}{" "}
						{unit}
					</span>
				</div>
