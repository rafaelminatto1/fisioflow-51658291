...
export const ProtocolMilestonesAlert: React.FC<ProtocolMilestonesAlertProps> = ({
  patientId,
  surgeries,
  pathologies,
  onApplyMilestone,
  ...
  const activeCondition = surgeries[0]?.surgery_name || pathologies[0]?.pathology_name;
  const startDate = surgeries[0]?.surgery_date || pathologies[0]?.diagnosis_date;
  ...
  const currentWeek = Math.max(1, differenceInWeeks(new Date(), parseISO(startDate)) + 1);
  
  // Filtra marcos relevantes para a semana atual ou próximas
  const allMilestones = Array.isArray(protocol.milestones) ? protocol.milestones : [];
  const currentMilestones = allMilestones.filter((m: any) => m.week === currentWeek);
  const nextMilestones = allMilestones.filter((m: any) => m.week > currentWeek).sort((a: any, b: any) => a.week - b.week).slice(0, 1);
  ...
  if (currentMilestones.length === 0 && nextMilestones.length === 0) return null;
        ...
        {nextMilestones.length > 0 && (
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between p-3 bg-white/50 border border-slate-200 rounded-lg group">
              <span className="text-sm text-slate-700">{nextMilestones[0].description}</span>
        ...
        <div className="pt-2 flex justify-between items-center border-t border-emerald-500/10 mt-2">
          {protocol.wiki_page_id && (
            <Button 
              ...
              onClick={() => window.open(`/wiki/article/${protocol.wiki_page_id}`, '_blank')}
            >
