...
export const MedicalReturnFormModal: React.FC<MedicalReturnFormModalProps> = ({
	...
	const createMutation = useMutation({
		mutationFn: (data: MedicalReturnFormData) =>
			MedicalReturnService.addMedicalReturn(data),
		onSuccess: () => {
	...
	const deleteMutation = useMutation({
		mutationFn: (id: string) =>
			MedicalReturnService.deleteMedicalReturn(id, patientId),
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: ["patient-medical-returns", patientId],
			});
	...
	const handleDelete = () => {
		if (
			medicalReturn &&
			confirm("Tem certeza que deseja remover este retorno médico?")
		) {
			...
			<Dialog open={open} onOpenChange={onOpenChange}>
				<DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
					<DialogHeader>
						...
						<DialogDescription>
							Agende ou registre os detalhes do retorno do paciente ao médico
							assistente.
						</DialogDescription>
					</DialogHeader>
