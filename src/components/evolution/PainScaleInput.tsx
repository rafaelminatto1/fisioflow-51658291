...
const PAIN_DESCRIPTIONS: Record<
	number,
...
export const PainScaleInput = React.memo(function PainScaleInput({
	...
	const currentDescription =
		PAIN_DESCRIPTIONS[value.level] || PAIN_DESCRIPTIONS[0];
		...
		<div className="space-y-4">
			...
			<div className="flex items-center justify-between">
				<div className="flex items-center gap-3">
					<div
						className={`w-12 h-12 rounded-full ${currentDescription.color} flex items-center justify-center text-white shadow-lg`}
					>
					...
					<CardContent className="p-4">
						...
						<div className="relative w-full h-8 rounded-full overflow-hidden shadow-inner">
							...
							<div
								...
								style={{
									background:
										"linear-gradient(to right, #22c55e 0%, #84cc16 20%, #eab308 40%, #f97316 60%, #ef4444 80%, #b91c1c 100%)",
								}}
							/>
							...
							{/* Indicador visual */}
							<div
								className="absolute top-0 h-full w-1 bg-white shadow-lg -translate-x-1/2 pointer-events-none transition-all duration-150"
								style={{ left: `${(value / 10) * 100}%` }}
								>
								<div className="absolute -top-1 left-1/2 -translate-x-1/2 w-4 h-4 bg-white rounded-full shadow-md border-2 border-border" />
							</div>
				...
				<div>
					...
					<div className="relative">
						<Input
							...
							onChange={(e) =>
								onChange?.({ ...value, character: e.target.value })
							}
							disabled={readonly}
						/>
