...
export function PainMapManager({
	...
	const autoSave = useCallback(async () => {
		...
		const bodyMapPoints: Omit<
			import("@/components/pain-map/BodyMap").PainPoint,
			"id"
		>[] = painPoints.map((p) => ({
			regionCode: p.region,
			region: p.region,
	...
	const convertToBodyMapPoint = useCallback(
		(point: PainMapPoint): PainPoint => {
			return {
	...
	const handlePointRemove = useCallback(
		(pointId: string) => {
			setPainPoints((prev) =>
				prev.filter((p, _index) => `point-${p.x}-${p.y}` !== pointId),
			);
					...
					<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
						<div>
							...
							<p className="text-2xl font-bold">
								{stats.averagePainLevel.toFixed(1)}/10
							</p>
						</div>
						<div>
				...
				<TabsContent value="current" className="space-y-4 mt-6">
					<div className="flex justify-end mb-2">
						<div className="flex items-center space-x-2">
							<Label htmlFor="3d-mode" className="text-sm font-medium">
								Modo 3D Realista
							</Label>
							<Switch
								id="3d-mode"
