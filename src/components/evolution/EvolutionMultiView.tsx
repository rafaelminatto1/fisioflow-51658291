...
const CalendarView: React.FC<CalendarViewProps> = ({ evolutions }) => {
  ...
  const sessionDates = useMemo(
    () =>
      Array.from(evolutionsByDay.keys()).map((k) => parseISO(k)).filter(isValid),
    [evolutionsByDay]
  );
...
const GrafoView: React.FC<GrafoViewProps> = ({ patientId, sessions = [], goals = [], pathologies = [] }) => {
        ...
        <TabsContent value="grafo" className="mt-4">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              transition={{ duration: 0.2 }}
            >
              <GrafoView patientId={patientId} sessions={previousEvolutions} goals={goals} pathologies={pathologies} />
            </motion.div>
          </AnimatePresence>
        </TabsContent>
      </Tabs>
