...
export const HomeCareWidget: React.FC<HomeCareWidgetProps> = ({
  ...
  const { prescriptions, isLoading, addPrescription, updatePrescription, removePrescription } =
    usePrescribedExercises(patientId);
  ...
  const filteredExercises = useMemo(
    () =>
      availableExercises.filter((ex) => ex.name.toLowerCase().includes(searchTerm.toLowerCase())),
  ...
  const handleWhatsAppShare = useCallback(() => {
    ...
    const exerciseList = prescriptions
      .map((p, i) => {
          ...
          <div className="p-3 border-b flex items-center justify-between gap-2 shrink-0 bg-muted/20">
            <div className="flex-1 relative">
              ...
              <Select
                ...
                disabled={disabled || addPrescription.isPending}
              >
                      ...
                      filteredExercises.map((exercise) => {
                        ...
                        return (
                          <SelectItem
                            key={exercise.id}
                            value={exercise.id}
                            className={cn("text-xs", isAdded && "opacity-50")}
                            disabled={isAdded}
                          >
                            <div className="flex items-center gap-2">
                              ...
                              {isAdded && (
                                <span className="text-[10px] text-green-600">(Prescrito)</span>
                              )}
                            </div>
                          </SelectItem>
                        );
                  ...
                  >
                    ...
                    <div className="relative aspect-video w-full bg-muted overflow-hidden shrink-0">
                      {p.exercise?.image_url ? (
                        <img
                          src={withImageParams(p.exercise.image_url, {
                            width: 640,
                            height: 360,
                            dpr: 2,
                            format: "auto",
