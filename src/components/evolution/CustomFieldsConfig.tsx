...
import { type CustomField } from "@/lib/evolution/customFields";
...
export const CustomFieldsConfig: React.FC<CustomFieldsConfigProps> = ({
  ...
  const handleUpdateUnit = (fieldId: string, unit: string) => {
    const updated = fields.map((f) => (f.id === fieldId ? { ...f, unit } : f));
    onChange(updated);
  };
    ...
    return (
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Badge
            variant="outline"
            className="text-[10px] font-bold bg-teal-50 text-teal-700 border-teal-100 italic"
          >
        ...
        <div className="flex flex-wrap gap-2">
          {fields.map((field) => (
            ...
            >
              ...
              {field.label}
              {field.required && field.enabled && <AlertCircle className="h-3 w-3 text-red-200" />}
            </motion.button>
          ))}
        </div>
      </div>
    ...
    <Card className="border-0 shadow-lg bg-slate-50/50 backdrop-blur-sm rounded-2xl overflow-hidden border border-slate-100/50">
      <CardHeader className="py-4 px-6 bg-white border-b border-slate-100">
        <CardTitle className="text-sm font-bold flex items-center justify-between text-slate-800 uppercase tracking-wider">
          ...
          <Badge
            variant="secondary"
            className="bg-slate-100 text-slate-600 font-bold border-0 h-6 px-3"
          >
      ...
      <CardContent className="py-5 px-6">
        <div className="space-y-6">
          <section className="space-y-3">
            ...
            <div className="space-y-2.5">{basicFields.map(renderFieldRow)}</div>
          </section>
          ...
          <section className="space-y-3">
            ...
            <div className="space-y-2.5">{advancedFields.map(renderFieldRow)}</div>
          </section>
        </div>
      </CardContent>
    </Card>
