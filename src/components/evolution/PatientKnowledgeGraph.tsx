...
const PatientNode: React.FC<NodeProps<PatientNodeData>> = ({
	data,
	selected,
}) => {
	const handleClick = () => {
...
const SessionNode: React.FC<NodeProps<SessionNodeData>> = ({
	data,
	selected,
}) => {
	const handleClick = () => {
...
const PathologyNode: React.FC<NodeProps<PathologyNodeData>> = ({
	data,
	selected,
}) => {
	const handleClick = () => {
...
const ExerciseNode: React.FC<NodeProps<ExerciseNodeData>> = ({
	data,
	selected,
}) => {
	const handleClick = () => {
			...
			<div className="px-4 py-3 space-y-2">
				...
				{Object.entries(detail.details).map(([key, val]) =>
					val !== undefined && val !== "" ? (
						...
						>
							<span className="font-medium text-gray-500 min-w-[72px]">
								{key}:
							</span>
	...
	<div className="flex flex-col items-center justify-center h-full gap-4 text-center px-6">
		...
		<div>
			...
			<p className="text-xs text-gray-500 mt-1 max-w-xs">
				Adicione sessões, metas, patologias ou exercícios para visualizar o
...
export const PatientKnowledgeGraph: React.FC<PatientKnowledgeGraphProps> = ({
	...
	const [selectedDetail, setSelectedDetail] = useState<NodeDetailData | null>(
		null,
	);
