  ...
  const handleClick = () => {
    data.onNodeClick({
      ...
      details: {
        ...
        'Nível de dor': data.painLevel !== undefined ? `${data.painLevel}/10${getPainEmoji(data.painLevel)}` : 'N/A',
  ...
  const handleClick = () => {
    data.onNodeClick({
      ...
      details: {
        ...
        Prioridade: data.priority === 'alta' ? 'Alta' : data.priority === 'media' ? 'Média' : 'Baixa',
      },
    });
  };
  ...
  <div className="flex flex-col items-center justify-center h-full gap-4 text-center px-6">
    ...
    <div>
      ...
      <p className="text-xs text-gray-500 mt-1 max-w-xs">
        Adicione sessões, metas, patologias ou exercícios para visualizar o grafo de conhecimento do paciente.
      </p>
    </div>
    <div className="flex flex-wrap justify-center gap-2 mt-1">
      <Badge variant="outline" className="text-blue-600 border-blue-200 bg-blue-50 text-xs">Sessões</Badge>
      <Badge variant="outline" className="text-emerald-600 border-emerald-200 bg-emerald-50 text-xs">Metas</Badge>
      <Badge variant="outline" className="text-orange-600 border-orange-200 bg-orange-50 text-xs">Patologias</Badge>
      <Badge variant="outline" className="text-purple-600 border-purple-200 bg-purple-50 text-xs">Exercícios</Badge>
    </div>
  </div>
);
...
export const PatientKnowledgeGraph: React.FC<PatientKnowledgeGraphProps> = ({
  ...
  const handleNodeClick = useCallback((data: NodeDetailData) => {
    setSelectedDetail(prev => (prev?.title === data.title && prev?.type === data.type ? null : data));
  }, []);
  ...
  const { initialNodes, initialEdges } = useMemo(() => {
    ...
    const sessionPositions = radialLayout(CENTER_X - 28, CENTER_Y - 28, sessions.length, 170, -Math.PI / 2);
    sessions.forEach((s, i) => {
      const nodeId = `session-${s.id}`;
      nodes.push({
        id: nodeId,
        type: 'session',
        position: { x: sessionPositions[i].x - 28, y: sessionPositions[i].y - 28 },
        data: {
    ...
    const goalPositions = radialLayout(CENTER_X - 28, CENTER_Y - 28, goals.length, 290, Math.PI / 4);
    goals.forEach((g, i) => {
    ...
    const pathologyPositions = radialLayout(CENTER_X - 26, CENTER_Y - 26, pathologies.length, 240, (3 * Math.PI) / 4);
    pathologies.forEach((p, i) => {
      const nodeId = `pathology-${p.id}`;
      nodes.push({
        id: nodeId,
        type: 'pathology',
        position: { x: pathologyPositions[i].x - 26, y: pathologyPositions[i].y - 26 },
        data: {
    ...
    const exercisePositions = radialLayout(CENTER_X - 24, CENTER_Y - 24, exercises.length, 340, (5 * Math.PI) / 6);
    exercises.forEach((ex, i) => {
      ...
      nodes.push({
        id: nodeId,
        type: 'exercise',
        position: { x: exercisePositions[i].x - 24, y: exercisePositions[i].y - 24 },
        data: {
          label: exerciseName,
          onNodeClick: handleNodeClick,
                    ...
                    {pathologies.length > 0 && (
                      <div className="bg-orange-50 border border-orange-100 rounded-lg px-2.5 py-1.5 flex items-center gap-1.5 shadow-sm">
                        ...
                        <span className="text-[10px] font-semibold text-orange-700">
                          {pathologies.length} {pathologies.length === 1 ? 'Patologia' : 'Patologias'}
                        </span>
                      </div>
                    )}
                    {exercises.length > 0 && (
