            ...
            {sequence.map((entry, index) => {
              ...
              const notes =
                entry.type === "procedure"
                  ? entry.item.notes
                  : entry.item.notes || entry.item.observations;

              return (
                <div
                  key={entry.key}
                  className={cn(
                    "group flex items-center gap-2 rounded-xl border border-transparent px-2.5 py-2.5 transition-colors",
                ...
                >
                  ...
                  <button
                    ...
                    aria-label={
                      entry.item.completed ? "Marcar como pendente" : "Marcar como concluído"
                    }
                  >
                    {entry.item.completed ? (
                      <CheckCircle2
                        className={cn(
                          "h-5 w-5",
                          isProcedure ? "text-emerald-600" : "text-blue-600",
                        )}
                      />
                    ) : (
