					...
					{measurements.length === 0 ? (
						<div className="text-center py-10 text-muted-foreground">
							...
							<p className="text-xs mt-1.5 opacity-70">
								Use o botão "Adicionar" para registrar
							</p>
						</div>
					) : (
						measurements.map((measurement, index) => (
							<MeasurementCard
								...
								onUpdate={(field, value) =>
									handleUpdateMeasurement(measurement.id, field, value)
								}
								onUpdateCustomData={(key, value) =>
									handleUpdateCustomData(measurement.id, key, value)
								}
				...
				{measurements.length > 0 && (
					<div className="px-4 pb-3">
						<div className="flex items-center justify-between text-[10px] text-muted-foreground mb-1.5">
							<span>Progresso</span>
							<span>
								{Math.round((completedCount / measurements.length) * 100)}%
							</span>
						</div>
...
const MeasurementCard: React.FC<{
	...
	({
		...
		const isYBalance =
			measurement.measurement_name?.toLowerCase().includes("y-balance") ||
					...
					<div className="flex-1 min-w-0">
						...
						{hasValues && (
							<p className="text-xs text-muted-foreground mt-0.5 truncate">
								...
								{!measurement.value &&
									Object.entries(measurement.custom_data || {})
										.filter(([_, v]) => v !== "")
										.map(([k, v]) => `${k}: ${v}`)
										.join(", ")}
							</p>
										...
										return (
											...
											>
												...
												<Input
													...
													onChange={(e) =>
														onUpdateCustomData(field.id, e.target.value)
													}
