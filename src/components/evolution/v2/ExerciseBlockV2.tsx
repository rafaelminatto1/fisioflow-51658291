...
export const ExerciseBlockV2: React.FC<ExerciseBlockV2Props> = ({
	...
	const handleSmartSuggest = useCallback(async () => {
		try {
			...
			toast.success(
				`Adicionados ${newExercises.length} exercícios do template: ${template.name}`,
				{
	...
	const handleUpdateFeedback = useCallback(
		(id: string, field: keyof ExerciseFeedback, value: boolean | string) => {
			onChange(
				exercises.map((e) => {
					if (e.id !== id) return e;
					const feedback = e.patientFeedback || {
						pain: false,
						fatigue: false,
						...
						<div className="flex flex-col">
							...
							<div className="flex items-center gap-2">
								...
								{issueCount > 0 && (
									<Badge
										variant="destructive"
										className="text-[10px] h-4 px-1.5 gap-1 rounded-full"
									>
				...
				{exercises.length === 0 ? (
					<div className="text-center py-10 text-muted-foreground">
						...
						<p className="text-xs mt-1.5 opacity-70">
							Use "Biblioteca" ou digite abaixo (Enter para adicionar, Tab para
							autocomplete)
						</p>
					</div>
				) : (
					...
					<div className="px-4 pb-3 animate-in slide-in-from-top-2 duration-200">
						...
						<div className="flex flex-wrap items-center gap-2 mb-2">
							...
							<FeedbackToggle
								...
								onToggle={(v) =>
									onUpdateFeedback(exercise.id, "difficultyPerforming", v)
								}
						...
						<Input
							...
							onChange={(e) =>
								onUpdateObservations(exercise.id, e.target.value)
							}
