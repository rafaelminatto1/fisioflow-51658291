...
const HistoryItem: React.FC<{ evolution: any; index: number; total: number }> = ({ evolution, index, total }) => {
  ...
  return (
    <div className="flex flex-col gap-2 p-3 rounded-xl border border-border/40 bg-muted/20 hover:bg-muted/40 transition-all group">
      ...
      <p className="text-xs text-muted-foreground line-clamp-2 leading-relaxed italic">
        {evolution.evolution_text || evolution.objective || evolution.subjective || 'Sem texto de evolução.'}
      </p>

      <div className="flex justify-end">
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="ghost" size="sm" className="h-7 px-2 text-[10px] font-bold uppercase tracking-wider gap-1 hover:text-primary hover:bg-primary/5">
              Detalhes <ExternalLink className="h-3 w-3" />
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-[400px] p-0 overflow-hidden shadow-2xl border-primary/20" align="end">
            <div className="p-4 bg-gradient-to-br from-primary/10 via-background to-background border-b border-border/50">
              <div className="flex items-center justify-between mb-2">
                ...
                <Badge variant="outline" className="text-[10px]">{format(date, "dd/MM/yyyy", { locale: ptBR })}</Badge>
              </div>
              {evolution.pain_level !== undefined && (
            ...
            <div className="p-4 max-h-[400px] overflow-y-auto space-y-4 text-sm custom-scrollbar">
              {evolution.subjective && (
                <div className="space-y-1">
                  <p className="text-[10px] font-black uppercase text-blue-600 tracking-widest">Subjetivo</p>
                  <p className="text-muted-foreground leading-relaxed">{evolution.subjective}</p>
                </div>
              )}
              {evolution.objective && (
                <div className="space-y-1">
                  <p className="text-[10px] font-black uppercase text-green-600 tracking-widest">Objetivo / Evolução</p>
                  <p className="text-muted-foreground leading-relaxed">{evolution.evolution_text || evolution.objective}</p>
                </div>
              )}
              {evolution.assessment && (
                <div className="space-y-1">
                  <p className="text-[10px] font-black uppercase text-purple-600 tracking-widest">Avaliação</p>
                  <p className="text-muted-foreground leading-relaxed">{evolution.assessment}</p>
                </div>
              )}
              {evolution.plan && (
                <div className="space-y-1">
                  <p className="text-[10px] font-black uppercase text-orange-600 tracking-widest">Plano / Conduta</p>
              ...
              {!evolution.subjective && !evolution.objective && !evolution.assessment && !evolution.plan && (
                <p className="text-center py-4 text-muted-foreground italic">Nenhum detalhe textual registrado.</p>
              )}
            </div>
          </PopoverContent>
        </Popover>
          ...
          <CardContent className="p-4">
            {previousEvolutions.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-6 text-center text-muted-foreground">
                ...
                <p className="text-xs italic">Nenhuma evolução anterior encontrada para este paciente.</p>
              </div>
            ) : (
