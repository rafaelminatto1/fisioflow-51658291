...
const HistoryItem: React.FC<{
	...
	const date = new Date(
		evolution.record_date || evolution.created_at || evolution.date,
	);
		...
		<div className="flex flex-col gap-2 p-3 rounded-xl border border-border/40 bg-muted/20 hover:bg-muted/40 transition-all group">
			<div className="flex items-center justify-between gap-2">
				<div className="flex items-center gap-2">
					<Badge
						variant="outline"
						className="h-5 px-1.5 text-[10px] font-bold bg-background"
					>
						Sessão #{sessionNum}
					</Badge>
										...
										<Button
											...
											onClick={() =>
												setHistoryLimit((prev) =>
													Math.min(prev + 3, previousEvolutions.length),
												)
											}
