...
export const NotionEvolutionEditor: React.FC<NotionEvolutionEditorProps> = ({ 
  ...
  useEffect(() => {
    const handleAiEvent = () => {
      ...
      else toast.info("Assistente de IA ativado. Como posso ajudar com esta evolução?");
    };
  ...
  const uploadToCloudflareR2 = useCallback(async (file: File) => {
    setIsUploading(true);
    ...
    try {
      ...
      const response = await fetch(`${CLOUDFLARE_WORKER_URL}?key=${encodeURIComponent(fileName)}`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) throw new Error('Falha no upload para o Cloudflare R2');
  ...
  useEffect(() => {
    const handleUploadEvent = (e: any) => {
      if (e.detail?.file) uploadToCloudflareR2(e.detail.file);
    };
    ...
    <div className="flex flex-col w-full max-w-5xl mx-auto bg-white dark:bg-slate-950 rounded-2xl shadow-premium-lg border border-slate-200 dark:border-slate-800 overflow-hidden transition-all duration-300">
            ...
            <div className="absolute inset-0 z-10 bg-white/60 dark:bg-slate-950/60 backdrop-blur-[2px] flex items-center justify-center animate-in fade-in duration-500">
                <div className="flex flex-col items-center gap-4 bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-2xl border border-slate-100 dark:border-slate-800">
                    ...
                    <div className="text-center">
                        <p className="text-sm font-black uppercase tracking-widest text-slate-800 dark:text-slate-100">Fazendo Upload para a Edge</p>
                        <p className="text-[10px] text-slate-400 font-bold">Cloudflare R2 • Processamento em Borda</p>
                    </div>
                </div>
            </div>
        )}
      ...
      <div className="px-8 py-4 bg-slate-50/30 dark:bg-slate-900/30 border-t border-slate-100 dark:border-slate-800 flex flex-wrap justify-between items-center gap-4">
        <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex items-center gap-1.5 group">
                <kbd className="px-2 py-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md text-[10px] font-black text-slate-500 shadow-sm group-hover:border-emerald-400 transition-colors"> CMD+K </kbd>
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider">Busca Global</span>
            </div>
        </div>
