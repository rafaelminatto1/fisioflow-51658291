				...
				<DialogHeader className="p-8 pb-0">
					<div className="flex items-center gap-3 mb-2">
						...
						<div className="text-left">
							<DialogTitle className="text-2xl font-black tracking-tight uppercase italic">AI Scribe</DialogTitle>
				...
				<div className="p-8 pt-6 space-y-8">
					...
					<div className="flex flex-col items-center justify-center py-10 rounded-[2rem] bg-slate-50/50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 relative overflow-hidden">
						...
						<div className={cn(
							...
							isRecording ? "bg-red-500 shadow-xl shadow-red-500/30 scale-110" : "bg-slate-200 dark:bg-slate-800"
						)}>
							{isRecording ? (
						...
						<div className="mt-6 text-center z-10">
							...
							{voiceState === "transcribing" && (
								<div className="flex flex-col items-center gap-2">
									...
									<p className="text-sm font-bold text-blue-600 uppercase tracking-widest">Processando Inteligência...</p>
								</div>
							)}
							{voiceState === "done" && (
								<div className="flex flex-col items-center gap-1">
									<CheckCircle2 className="w-8 h-8 text-emerald-500" />
									<p className="text-sm font-bold text-emerald-600 uppercase tracking-widest">Evolução Gerada!</p>
								</div>
							)}
							{voiceState === "error" && (
								<div className="flex flex-col items-center gap-1 px-4">
									<AlertCircle className="w-8 h-8 text-red-500" />
									<p className="text-xs font-bold text-red-600 uppercase tracking-widest">{error || "Erro na captura"}</p>
								</div>
							)}
						</div>
					</div>
					...
					{soapFields && (
						<div className="max-h-48 overflow-y-auto p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 animate-in fade-in slide-in-from-bottom-4 duration-500">
							<div className="flex items-center gap-2 mb-3 border-b border-slate-100 dark:border-slate-800 pb-2">
								...
								<span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Preview SOAP</span>
							</div>
