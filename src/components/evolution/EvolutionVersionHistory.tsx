...
import {
	useEvolutionVersionHistory,
	type EvolutionVersion,
} from "@/hooks/evolution/useEvolutionVersionHistory";
...
function getContentPreview(content: EvolutionVersion["content"]): string {
	...
	if (content.evolution_notes)
		parts.push(content.evolution_notes.slice(0, 120));
...
function VersionCard({ version, index, onRestoreClick }: VersionCardProps) {
	...
	const config =
		CHANGE_TYPE_CONFIG[version.changeType] ?? CHANGE_TYPE_CONFIG.auto;
			...
			<div className="flex items-start justify-between gap-2">
				...
				<div className="flex items-center gap-1.5 shrink-0">
					{isLatest && (
						<Badge
							variant="outline"
							className="text-[10px] px-1.5 py-0 bg-primary/10 text-primary border-primary/30 font-bold"
						>
							Atual
						</Badge>
					)}
...
export function EvolutionVersionHistoryTrigger({
	soapRecordId,
	onRestore,
}: EvolutionVersionHistoryProps) {
	const [open, setOpen] = useState(false);
	const [confirmVersion, setConfirmVersion] = useState<EvolutionVersion | null>(
		null,
	);
	const { versions, isLoading, saveVersion, isSaving } =
		useEvolutionVersionHistory(soapRecordId);
	...
	const handleConfirmRestore = async () => {
		...
		await saveVersion({
			content: confirmVersion.content,
			changeType: "restore",
		});
		...
		toast({
			...
			description:
				"O conteúdo foi restaurado com sucesso. A versão anterior foi salva como backup.",
		});
	};
					...
					<Button
						...
						title={
							soapRecordId
								...
								: "Salve a evolução para ver o histórico"
						}
					>
						...
						{versions.length > 0 && (
							<Badge
								variant="secondary"
								className="h-4 min-w-4 px-1 text-[10px] font-black"
							>
								{versions.length}
							</Badge>
						)}
								...
								<div className="flex flex-col items-center justify-center py-12 text-center gap-3">
									...
									<div>
										...
										<p className="text-xs text-muted-foreground mt-1">
											As versões são salvas automaticamente ao gravar a
											evolução.
										</p>
									</div>
								</div>
							...
							{!isLoading && versions.length > 0 && (
								<>
									<div className="flex items-center gap-1.5 text-xs text-muted-foreground mb-1">
										...
										Restaurar substitui o conteúdo atual. Um backup será criado
										automaticamente.
									</div>
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							...
							<strong>
								{confirmVersion
									? formatSavedAt(confirmVersion.savedAt).absolute
									: ""}
							</strong>
							. Um backup da versão atual será criado automaticamente antes da
							restauração.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
