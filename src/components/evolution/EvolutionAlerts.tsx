	...
	({
		...
		if (overdueGoals.length > 0) {
			criticalAlerts.push(
				<Alert
					key="overdue"
					variant="destructive"
					className="animate-pulse border-red-600 border-l-4 py-4"
					role="alert"
				>
					...
					<AlertDescription className="text-sm leading-relaxed mt-1">
						{overdueGoals.length} meta(s) vencida(s). Reavalie o plano de
						tratamento e ajuste as datas na aba Tratamento.{" "}
						<button
							type="button"
		...
		if (painScale.level >= 7) {
			criticalAlerts.push(
				<Alert
					key="pain-high"
					variant="destructive"
					className="animate-pulse border-l-4 py-4"
					role="alert"
				>
					...
					<AlertDescription className="text-sm leading-relaxed mt-1">
						...
						{painTrend === "worsening" &&
							" Tendência de PIORA nas últimas sessões."}
						{painTrend === "improving" &&
							" Tendência de MELHORA nas últimas sessões."}
		...
		if (daysSinceLastEvolution !== null && daysSinceLastEvolution > 21) {
			criticalAlerts.push(
				<Alert
					key="long-period"
					className="border-red-500/50 bg-red-50 dark:bg-red-950/20 border-l-4 border-l-red-500 py-4"
					role="alert"
				>
					<FileText className="h-5 w-5 text-red-600" aria-hidden />
					<AlertTitle className="text-base font-bold text-red-800 dark:text-red-200">
						Longo Período Sem Evolução
					</AlertTitle>
		...
		if (
			...
			warningAlerts.push(
				<Alert
					key="pain-trend"
					className="border-rose-500/50 bg-rose-50 dark:bg-rose-950/20 border-l-4 border-l-rose-500 py-4"
					role="alert"
				>
					<HeartPulse className="h-5 w-5 text-rose-600" />
					<AlertTitle className="text-base font-bold text-rose-800 dark:text-red-200">
						Tendência de Aumento da Dor
					</AlertTitle>
					<AlertDescription className="text-sm leading-relaxed text-rose-700 dark:text-red-300 mt-1">
						Dor atual: {painScale.level}/10. A tendência nas últimas sessões é
						de PIORA. Avalie a necessidade de ajustar o tratamento.
					</AlertDescription>
				</Alert>,
			);
		}
		if (upcomingGoals.length > 0) {
			warningAlerts.push(
				<Alert
					key="upcoming"
					className="border-amber-500/50 bg-amber-50 dark:bg-amber-950/20 border-l-4 border-l-amber-500 py-4"
					role="alert"
				>
					<Clock className="h-5 w-5 text-amber-600" aria-hidden />
					<AlertTitle className="text-base font-bold text-amber-800 dark:text-red-200">
		...
		if (
			...
			infoAlerts.push(
				<Alert
					key="session-60"
					className="border-blue-500/50 bg-blue-50 dark:bg-blue-950/20 border-l-4 border-l-blue-500 py-4"
					role="alert"
				>
					<Timer className="h-5 w-5 text-blue-600" />
					<AlertTitle className="text-base font-bold text-blue-800 dark:text-blue-200">
						Sessão em Andamento
					</AlertTitle>
					<AlertDescription className="text-sm leading-relaxed text-blue-700 dark:text-blue-300 mt-1">
						Tempo de sessão: {Math.floor(sessionDurationMinutes / 60)}h{" "}
						{sessionDurationMinutes % 60}min.
					</AlertDescription>
				</Alert>,
			);
		}
		if (sessionDurationMinutes > 90 && !sessionLongAlertShown) {
			infoAlerts.push(
				<Alert
					key="session-90"
					className="border-purple-500/50 bg-purple-50 dark:bg-purple-950/20 animate-pulse border-l-4 border-l-purple-500 py-4"
					role="alert"
				>
					<Timer className="h-5 w-5 text-purple-600" />
					<AlertTitle className="text-base font-bold text-purple-800 dark:text-purple-200">
						Sessão Prolongada
					</AlertTitle>
					<AlertDescription className="text-sm leading-relaxed text-purple-700 dark:text-purple-300 mt-1">
						Esta sessão já dura {Math.floor(sessionDurationMinutes / 60)}h{" "}
						{sessionDurationMinutes % 60}min. Considere fazer uma pausa ou
		...
		if (activePathologies.length >= 3) {
			infoAlerts.push(
				<Alert
					key="complexity"
					className="border-orange-500/50 bg-orange-50 dark:bg-orange-950/20 border-l-4 border-l-orange-500 py-4"
					role="alert"
				>
					<TrendingUp className="h-5 w-5 text-orange-600" />
					<AlertTitle className="text-base font-bold text-orange-800 dark:text-orange-200">
						Complexidade Clínica Elevada
					</AlertTitle>
					<AlertDescription className="text-sm leading-relaxed text-orange-700 dark:text-orange-300 mt-1">
						Paciente com {activePathologies.length} patologia(s) ativa(s).
						Requer atenção especial e planejamento cuidadoso do tratamento.{" "}
						<button
							type="button"
							onClick={() => onTabChange("tratamento")}
		...
		if (
			...
			successAlerts.push(
				<Alert
					key="progress"
					className="border-emerald-500/50 bg-emerald-50 dark:bg-emerald-950/20 border-l-4 border-l-emerald-500 py-4"
					role="alert"
				>
					<HeartPulse className="h-5 w-5 text-emerald-600" />
					<AlertTitle className="text-base font-bold text-emerald-800 dark:text-emerald-200">
						Progresso Positivo!
					</AlertTitle>
					<AlertDescription className="text-sm leading-relaxed text-emerald-700 dark:text-emerald-300 mt-1">
						Excelente! O paciente mostra tendência de melhora na última sessão.
						Continue com o tratamento atual.
					</AlertDescription>
				</Alert>,
			);
		}
