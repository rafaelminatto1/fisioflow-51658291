...
const LazyPainMapManager = lazy(() =>
	import("@/components/evolution/PainMapManager").then((m) => ({
		default: m.PainMapManager,
	})),
);
const LazyMeasurementForm = lazy(() =>
	import("@/components/evolution/MeasurementForm").then((m) => ({
		default: m.MeasurementForm,
	})),
);
const LazyMeasurementCharts = lazy(() =>
	import("@/components/evolution/MeasurementCharts").then((m) => ({
		default: m.MeasurementCharts,
	})),
);
			...
			{pendingRequiredMeasurements.length > 0 && (
				<Card className="border-destructive/30 shadow-sm">
					...
					<CardContent className="space-y-2 pt-3 px-3 pb-3">
						{requiredMeasurements
							.filter((req) => {
								const completedToday = todayMeasurements.some(
									(m) => m.measurement_name === req.measurement_name,
								);
							...
							.map((req) => (
								<Alert
									key={req.id}
									variant={
										req.alert_level === "high" ? "destructive" : "default"
									}
									className="py-2"
								>
									<AlertTriangle className="h-3.5 w-3.5" />
