...
export const ClinicalInsightCard: React.FC<ClinicalInsightCardProps> = ({
    ...
    const getStyles = () => {
        switch (type) {
            case 'alert': return { bg: 'bg-red-500/10', border: 'border-red-500/50', icon: <ShieldAlert className="text-red-500" />, label: 'Alerta Crítico' };
            case 'precaution': return { bg: 'bg-amber-500/10', border: 'border-amber-500/50', icon: <AlertTriangle className="text-amber-500" />, label: 'Precaução' };
            case 'exercise': return { bg: 'bg-blue-500/10', border: 'border-blue-500/50', icon: <Lightbulb className="text-blue-500" />, label: 'Sugestão' };
            default: return { bg: 'bg-emerald-500/10', border: 'border-emerald-500/50', icon: <Info className="text-emerald-500" />, label: 'Protocolo' };
        }
    };
    ...
    return (
        ...
        >
            ...
            <div className={`absolute bottom-0 left-0 h-1 bg-current opacity-20 ${styles.border.replace('border-', 'bg-')}`} style={{ width: priority === 'high' ? '100%' : priority === 'medium' ? '60%' : '30%' }} />
        </motion.div>
    );
};
