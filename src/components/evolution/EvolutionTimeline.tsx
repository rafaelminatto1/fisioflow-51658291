...
const SessionDetailsModal: React.FC<{
	...
	const sessionDate = isValid(sessionDateObj)
	...
	const sessionMeasurements = measurements.filter((m) => {
		...
		return (
			isValid(mDateObj) && mDateObj.toISOString().split("T")[0] === sessionDate
		);
	});
	const sessionAttachments = attachments.filter(
		(a) => a.soap_record_id === session.id,
	);
				...
				<div className="bg-gradient-to-r from-blue-500/10 to-purple-500/10 border-b px-6 py-4">
					<DialogHeader className="space-y-2">
						...
						<div className="flex items-center gap-4 text-sm">
							...
							<Badge
								variant={
									session.status === "finalized" ? "default" : "secondary"
								}
								className="text-xs"
							>
							...
							{session.appointment_id && (
								<div className="flex justify-between items-center">
									...
									<span className="font-mono">
										{session.appointment_id.slice(0, 8)}...
									</span>
								</div>
							)}
...
export const EvolutionTimeline: React.FC<EvolutionTimelineProps> = ({
	...
	const [selectedSession, setSelectedSession] = useState<SoapRecord | null>(
		null,
	);
	...
	const { data: attachments = [] } = useSessionAttachments(
		undefined,
		patientId,
	);
	...
	const timelineEvents = useMemo(() => {
		...
		surgeries.forEach((surgery) => {
			events.push({
				...
				description: surgery.affected_side
			...
			<Card>
							...
							{showGamification && !isLoadingGamification && (
								<div className="flex items-center gap-2 ml-2">
									...
									{unlockedAchievements.length > 0 && (
										<Badge
											variant="outline"
											className="gap-0.5 px-2 py-0.5 text-xs"
										>
						...
						{showFilters && (
							<div className="flex items-center gap-2 flex-wrap">
								...
								<Select
									value={filterType}
									onValueChange={(v) =>
										setFilterType(v as TimelineEventType | "all")
									}
								>
								...
								<Tabs
									value={viewMode}
									onValueChange={(v) => setViewMode(v as "timeline" | "list")}
								>
				...
				<CardContent>
					{filteredEvents.length === 0 ? (
						...
						<ScrollArea
							className={compactView ? "h-[500px] pr-4" : "h-[600px] pr-4"}
						>
							<div className="relative">
								{/* Linha central */}
