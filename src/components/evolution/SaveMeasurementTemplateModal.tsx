...
export const SaveMeasurementTemplateModal: React.FC<
	SaveMeasurementTemplateModalProps
> = ({ open, onOpenChange, fields, onSaved }) => {
	const { user } = useAuth();
	...
	const handleSave = async () => {
		...
		try {
			await clinicalTestsApi.create({
				...
				created_by: user?.id,
				organization_id: (user as { organization_id?: string } | null)
					?.organization_id,
				is_custom: true,
			});
					...
					<div className="flex items-center gap-3 mb-1">
						...
						<div>
							<DialogTitle className="text-xl font-bold text-slate-800 flex items-center gap-2">
								...
								<Badge
									variant="secondary"
									className="bg-teal-50 text-teal-700 border-teal-100 text-[10px] font-bold h-5 px-2"
								>
									PREMIUM
								</Badge>
							</DialogTitle>
				...
				<div className="p-6 space-y-6 max-h-[60vh] overflow-y-auto custom-scrollbar">
					<div className="grid grid-cols-1 md:grid-cols-2 gap-5">
						...
						<div className="space-y-2">
							...
							<Select
								...
								onValueChange={(val) =>
									setFormData({ ...formData, target_joint: val })
								}
							>
								<SelectTrigger className="h-11 bg-white border-slate-200 font-medium">
									<SelectValue placeholder="Selecione..." />
								...
								<SelectContent className="rounded-xl border-slate-100 shadow-xl">
									{joints.map((j) => (
										<SelectItem key={j} value={j} className="font-medium">
											{j}
										</SelectItem>
									))}
								</SelectContent>
					...
					<section className="space-y-3 p-5 bg-teal-50/30 rounded-2xl border border-teal-100/50">
						<div className="flex items-center justify-between border-b border-teal-100/30 pb-2 mb-2">
							...
							<Badge
								variant="outline"
								className="text-[9px] bg-white border-teal-200 text-teal-600 font-bold px-2 py-0"
							>
								PRÉ-VISUALIZAÇÃO
							</Badge>
						</div>
						<div className="flex flex-wrap gap-2">
							...
							{enabledFields.length === 0 && (
								<p className="text-xs text-gray-500 italic font-medium w-full text-center py-4">
