...
const frontPaths: Partial<
	Record<BodyRegion, { path: string; centerX: number; centerY: number }>
> = {
	// Head Split
...
export function PainMapCanvas({
	...
	const handleBodyMapPointRemove = (pointId: string) => {
		...
		if (point) {
			onPainPointsChange(
				painPoints.filter((p) => p.x !== point.x || p.y !== point.y),
			);
		}
	};
	...
	const selectedPoint = selectedRegion
		? painPoints.find((p) => p.region === selectedRegion)
		: null;
				...
				<div className="flex items-center justify-between mb-4">
					<div>
						...
						<p className="text-xs text-muted-foreground">
							Clique nas regiões para registrar a dor
						</p>
					</div>
					{!readOnly && (
			...
			<Card className="p-6">
				...
				{selectedRegion ? (
					<div className="space-y-5 animate-fade-in">
						...
						<div className="space-y-2">
							<Label htmlFor="painType" className="text-sm">
								Tipo de Dor
							</Label>
							<Select
								...
								<Badge
									...
									style={{
										backgroundColor: getIntensityColor(
											Math.round(
												painPoints.reduce((a, p) => a + p.intensity, 0) /
													painPoints.length,
											),
										),
										color: "white",
							...
							<div className="flex justify-between items-center p-2 bg-muted/30 rounded-lg">
								...
								<Badge variant="outline" className="text-xs">
									{PainMapService.getRegionLabel(
										[...painPoints].sort((a, b) => b.intensity - a.intensity)[0]
											.region,
									)}
								</Badge>
							</div>
