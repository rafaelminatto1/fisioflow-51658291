...
const GoalPanel: React.FC<GoalPanelProps> = ({ compareData, promSnapshot }) => {
	...
	const [selectedProfileId, setSelectedProfileId] =
		useState<string>("acl_rts_readiness");

	const availableProfiles = Object.values(goalProfiles).filter(
		(_) =>
				...
				<div className="flex justify-between items-start">
					<div>
						<CardTitle className="text-lg flex items-center gap-2">
							...
							{evaluation?.overallStatus === "ON_TRACK" && (
								<Badge className="bg-green-500">No Caminho</Badge>
							)}
							{evaluation?.overallStatus === "NEEDS_FOCUS" && (
				...
				<div className="space-y-4">
					...
					{evaluation?.overallStatus === "NA" && (
						<div className="bg-amber-50 p-2 text-amber-800 text-xs rounded mb-2">
							Atenção: A confiança da análise é muito baixa para validar este
							perfil de metas.
						</div>
					)}
					...
					<div className="grid gap-2">
						{evaluation?.targets.map((res) => (
							<div
								key={res.key}
								className="flex items-center justify-between p-2 rounded bg-slate-50 border border-slate-100 dark:bg-slate-900 dark:border-slate-800"
							>
								<div className="flex flex-col">
									<span className="text-sm font-medium">{res.label}</span>
									<div className="flex items-center gap-2 text-xs text-muted-foreground">
										<span>
											Atual: <b>{res.currentValue}</b>
										</span>
										<span>•</span>
