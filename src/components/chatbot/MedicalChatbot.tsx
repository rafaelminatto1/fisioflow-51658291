...
interface ChatBubbleProps {
  message: {
    ...
    timestamp?: string;
    metadata?: { confidence?: number; intent?: string; suggestions?: string[] };
  };
  isUser: boolean;
  showTime?: boolean;
}
      ...
      <div className={`flex items-start space-x-2 max-w-xs lg:max-w-md`}>
        ...
        <div className={`px-4 py-2 rounded-lg ${bubbleClass} shadow-sm`}>
          <div className="flex items-center space-x-1 mb-1">
            <span className="text-xs opacity-75">{getMessageTypeIcon(message.type || "text")}</span>
  ...
  return (
    <button
      ...
      className={`p-2 rounded-full transition-colors ${
        isRecording ? "bg-red-500 text-white animate-pulse" : "text-gray-500 hover:text-gray-600"
      }`}
...
const MedicalChatbot: React.FC<MedicalChatbotProps> = ({
  ...
  const containerClass = embedded ? "w-full h-full" : "fixed bottom-4 right-4 w-96 h-[600px] z-50";
                ...
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 mb-4">
                  <div className="flex items-center space-x-2">
                    ...
                    <p className="text-sm text-orange-800">
                      Solicitação de atendimento humano enviada. Um especialista entrará em contato
                      em breve.
                    </p>
                  </div>
                </div>
              ...
              <div className="px-4 pb-2">
                <div className="flex flex-wrap gap-2">
                  {quickReplies.map((reply) => (
                    <QuickReplyButton key={reply.id} reply={reply} onClick={handleQuickReply} />
                  ))}
                </div>
              </div>
            )}
