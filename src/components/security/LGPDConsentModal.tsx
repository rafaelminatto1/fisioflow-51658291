...
> = {
	dados_pessoais: {
		...
		description:
			"Autorizo o tratamento dos meus dados pessoais (nome, email, telefone, CPF) para fins de gestão clínica e atendimento fisioterapêutico.",
	},
	dados_sensiveis: {
		title: "Tratamento de Dados Sensíveis de Saúde",
		description:
			"Autorizo o tratamento dos meus dados sensíveis de saúde (prontuários médicos, histórico de tratamentos, exames) exclusivamente para fins terapêuticos.",
	},
	comunicacao_marketing: {
		title: "Comunicação e Marketing",
		description:
			"Autorizo o envio de comunicações sobre novidades, promoções e conteúdos informativos relacionados à fisioterapia e saúde.",
	},
	compartilhamento_terceiros: {
		title: "Compartilhamento com Terceiros",
		description:
			"Autorizo o compartilhamento dos meus dados com parceiros para fins específicos (ex: laboratórios, convênios médicos) quando necessário para o tratamento.",
	},
};
			...
			<CustomModalBody className="p-0 sm:p-0">
				<div className="px-6 py-4 space-y-6">
					<p className="text-sm text-muted-foreground">
						Gerencie suas autorizações de uso de dados conforme a Lei Geral de
					...
					<Alert className="bg-primary/5 border-primary/10">
						...
						<AlertDescription className="text-xs text-slate-600">
							Você pode revogar qualquer consentimento a qualquer momento. Isso
							pode impactar algumas funcionalidades do sistema que dependem
							desses dados.
						</AlertDescription>
					</Alert>
					...
					<div className="space-y-3">
						{(Object.keys(consentDescriptions) as ConsentType[]).map((type) => {
							...
							const isRequired =
								type === "dados_pessoais" || type === "dados_sensiveis";
								...
								>
									...
									<div className="flex-1 space-y-1">
										...
										>
											{title}
											{isRequired && (
												<span className="text-[10px] bg-rose-50 text-rose-600 px-1.5 py-0.5 rounded uppercase font-bold">
													Obrigatório
												</span>
											)}
										</Label>
					...
					<div className="flex items-center gap-2 text-[10px] text-slate-400 bg-slate-50 p-2 rounded-lg border border-dashed">
						...
						<span>
							Ao conceder consentimento, você declara estar ciente da nossa
