...
export const CalendarSync: React.FC = () => {
		...
		const icons: Record<CalendarProvider, React.ReactNode> = {
			google: (
				<svg className="w-5 h-5" viewBox="0 0 24 24">
					<path
						fill="#4285F4"
						d="M22.56 12.25c0-.78-.07-1.53-.2-2.25L12 1a1.5 1.5 0 00-1.5 1.5v3.5c-3.05 0-5.5 2.45-5.5 5.5 0 .3.05.07.61.2.89l10.36 10a1.5 1.5 0 001.41-1.41c.13-.13.2-.45.2-.7V11.5c0-3.05-2.45-5.5-5.5-5.5z"
					/>
				</svg>
			),
			icloud: (
			...
			outlook: (
				<svg className="w-5 h-5" viewBox="0 0 24 24">
					<path
						fill="#0078D4"
						d="M21.5 6.5h-19A2.5 2.5 0 010 9v9a2.5 2.5 0 002.5 2.5h19a2.5 2.5 0 002.5-2.5v-9a2.5 2.5 0 00-2.5-2.5zM12 11a2 2 0 110 4 2 2 0 010-4z"
					/>
				</svg>
			),
	...
	const handleDisconnect = useCallback(
		async (provider: CalendarProvider) => {
			if (!window.confirm(`Deseja desconectar ${getProviderLabel(provider)}?`))
				return;
	...
	const handleSync = useCallback(
		async (provider: CalendarProvider) => {
			...
			try {
				...
				setConfigs((prev) =>
					prev.map((c) =>
						c.provider === provider ? { ...c, lastSync: new Date() } : c,
					),
				);
	...
	const handleUpdateConfig = useCallback(
		(provider: CalendarProvider, updates: Partial<CalendarSyncConfig>) => {
			setConfigs((prev) =>
	...
	const handleResolveConflict = useCallback(
		(conflict: SyncConflict, keep: "local" | "remote") => {
			setConflicts((prev) =>
				prev.filter((c) => c.localId !== conflict.localId),
			);
			toast({
					...
					<div className="flex items-start gap-3">
						...
						<div className="flex-1">
							<h3 className="font-semibold text-amber-900 dark:text-amber-100 mb-2">
								...
								encontrado{conflicts.length !== 1 ? "s" : ""}
							</h3>
							<p className="text-sm text-amber-800 dark:text-amber-200 mb-3">
								Há eventos que foram modificados tanto localmente quanto
								remotamente. Selecione qual versão manter.
							</p>
							<div className="space-y-2">
								{conflicts.map((conflict) => (
									<div
										key={conflict.localId}
										className="p-3 bg-white dark:bg-slate-900 rounded border"
									>
										<div className="flex gap-4">
											...
											>
												...
												<div className="text-sm text-muted-foreground">
													...
													{conflict.localEvent.start.toLocaleString()}
												</div>
											</button>
											<button
												onClick={() =>
											...
											>
												...
												<div className="text-sm text-muted-foreground">
													{conflict.remoteEvent.title} -{" "}
													{conflict.remoteEvent.start.toLocaleString()}
												</div>
											</button>
										</div>
									</div>
								...
								<div>
									...
									{config.lastSync && (
										<div className="flex items-center gap-1 text-xs text-muted-foreground">
											...
											<span>
												Última sync: {config.lastSync.toLocaleString()}
											</span>
										</div>
									)}
								</div>
									...
									<>
										...
										>
											<RefreshCw
												className={cn(
													"w-4 h-4",
													syncing.has(config.provider) && "animate-spin",
												)}
											/>
										</button>
										<button
						...
						{config.connected && (
							<div className="mt-4 pt-4 border-t space-y-3">
								...
								<div className="flex items-center justify-between">
									...
									<button
										onClick={() =>
											handleUpdateConfig(config.provider, {
