...
export const CalendarSync: React.FC = () => {
  ...
  const handleConnect = useCallback(
    async (provider: CalendarProvider) => {
      ...
      try {
        ...
        setConfigs((prev) =>
          prev.map((c) =>
            c.provider === provider ? { ...c, connected: true, lastSync: new Date() } : c,
          ),
        );
  ...
  const handleDisconnect = useCallback(
    async (provider: CalendarProvider) => {
      ...
      try {
        ...
        setConfigs((prev) =>
          prev.map((c) =>
            c.provider === provider ? { ...c, connected: false, lastSync: undefined } : c,
          ),
        );
  ...
  const handleSync = useCallback(
    async (provider: CalendarProvider) => {
      ...
      try {
        ...
        setConfigs((prev) =>
          prev.map((c) => (c.provider === provider ? { ...c, lastSync: new Date() } : c)),
        );
  ...
  const handleUpdateConfig = useCallback(
    (provider: CalendarProvider, updates: Partial<CalendarSyncConfig>) => {
      setConfigs((prev) => prev.map((c) => (c.provider === provider ? { ...c, ...updates } : c)));
    },
    [],
  );
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-amber-900 dark:text-amber-100 mb-2">
                {conflicts.length} conflito{conflicts.length !== 1 ? "s" : ""} encontrado
                {conflicts.length !== 1 ? "s" : ""}
              </h3>
              <p className="text-sm text-amber-800 dark:text-amber-200 mb-3">
                Há eventos que foram modificados tanto localmente quanto remotamente. Selecione qual
                versão manter.
              </p>
              ...
              <div className="flex items-center gap-2">
                {config.connected ? (
                  <>
                    ...
                    >
                      <RefreshCw
                        className={cn("w-4 h-4", syncing.has(config.provider) && "animate-spin")}
                      />
                    </button>
                    <button
