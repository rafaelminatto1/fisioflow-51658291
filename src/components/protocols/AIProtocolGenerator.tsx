...
export const AIProtocolGenerator: React.FC<AIProtocolGeneratorProps> = ({ open, onOpenChange }) => {
      ...
      <DialogContent className="sm:max-w-[700px] max-h-[85vh] flex flex-col p-0 overflow-hidden rounded-[2rem] border-none shadow-2xl">
        <DialogHeader className="p-8 bg-gradient-to-br from-brand-blue to-indigo-600 text-white">
          ...
          <DialogDescription className="text-blue-100 font-medium">
            Gere um plano de tratamento de elite baseado em evidências para qualquer diagnóstico em
            segundos.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-6 py-10">
              ...
              <div className="p-4 rounded-2xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 flex gap-3">
                ...
                <p className="text-xs text-slate-500 leading-relaxed">
                  A IA analisará a condição informada e criará fases de tratamento baseadas em
                  protocolos de reabilitação modernos (warm-up, fortalecimento, propriocepção e
                  critérios de alta).
                </p>
              </div>
            </div>
              ...
              <div className="space-y-6 pb-4">
                ...
                <div className="space-y-4">
                  ...
                  {generatedProtocol.phases.map((phase: any, idx: number) => (
                    <div
                      key={idx}
                      className="relative pl-8 pb-4 border-l-2 border-brand-blue/20 last:border-l-0"
                    >
                      ...
                      <div className="p-5 rounded-2xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                        <div className="flex justify-between items-center mb-2">
                          <h5 className="font-black text-brand-blue uppercase text-xs tracking-wider">
                            {phase.name}
                          </h5>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {phase.exercises.map((ex: string, eIdx: number) => (
                            <span
                              key={eIdx}
                              className="px-3 py-1 rounded-full bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-[10px] font-bold text-slate-500"
                            >
                              {ex}
                            </span>
                          ))}
        ...
        <DialogFooter className="p-6 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800">
          {generatedProtocol ? (
            <div className="flex w-full gap-3">
              ...
              <Button
                onClick={handleSave}
                className="flex-1 rounded-xl h-12 bg-emerald-600 hover:bg-emerald-700 font-bold gap-2"
              >
