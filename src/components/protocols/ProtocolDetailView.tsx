...
export function ProtocolDetailView({ protocol, onBack, onEdit, onDelete }: ProtocolDetailViewProps) {
  ...
  const restrictions = (protocol.restrictions as Restriction[] | undefined) ?? [];
  const phases = ((protocol.phases && protocol.phases.length > 0
    ? protocol.phases
    : details?.phases || []) as ProtocolPhase[]);
  ...
  const categoryInfo = PROTOCOL_CATEGORIES.find((item) => item.id === category) || PROTOCOL_CATEGORIES[0];
  ...
  const summaryCards = [
    { label: 'Semanas', value: protocol.weeks_total, icon: Calendar, tone: 'text-primary' },
    { label: 'Fases', value: phases.length, icon: Dumbbell, tone: 'text-indigo-600' },
    { label: 'Marcos', value: milestones.length, icon: Target, tone: 'text-emerald-600' },
    { label: 'Alertas', value: restrictions.length, icon: Shield, tone: 'text-amber-600' },
  ];
    ...
    <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex min-w-0 items-start gap-4">
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <h1 className="text-2xl font-bold tracking-tight text-foreground">{protocol.name}</h1>
                  <Badge
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.55fr)_340px]">
              ...
              <div className="space-y-5">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <h2 className="mt-1 text-xl font-semibold tracking-tight">Leitura rápida do protocolo</h2>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    {overviewObjectives.map((objective, index) => (
                      ...
                      >
                        ...
                        <span className="text-sm font-medium leading-5">{objective}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="rounded-[28px] border border-border/60 bg-background/80 p-5 shadow-sm">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <h3 className="mt-1 text-lg font-semibold">Marcos principais</h3>
                  </div>
                ...
                <div className="mt-5 flex items-center gap-3">
                  ...
                  <span className="text-xs font-medium text-muted-foreground">Alta</span>
                </div>
                ...
                {milestones.length > 0 ? (
                  ...
                  <div className="mt-5 rounded-2xl border border-dashed border-border/70 bg-background/60 px-4 py-5 text-sm text-muted-foreground">
                    Nenhum marco definido. Use as fases abaixo para acompanhar a progressão clínica.
                  </div>
                )}
              </div>
            </div>
            ...
            <Card className="border-border/60 bg-background/70 p-4 shadow-sm sm:p-5">
              <div className="flex flex-col gap-2 border-b border-border/60 pb-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  ...
                  <h2 className="mt-1 text-xl font-semibold tracking-tight">Fases do tratamento</h2>
                  <p className="mt-1 text-sm text-muted-foreground">
                    Cada fase agora fica compacta por padrão e expande só quando você precisa entrar em detalhe.
                  </p>
                </div>
                        ...
                        <CollapsibleTrigger asChild>
                          ...
                          >
                            <div className="flex min-w-0 items-start gap-4">
                              ...
                              <div className="min-w-0">
                                <div className="flex flex-wrap items-center gap-2">
                                  <h3 className="text-base font-semibold text-foreground">{phaseName}</h3>
                                  <Badge
                                    variant="outline"
                              ...
                              <div className="space-y-2.5">
                                {goals.length > 0 ? (
                                  goals.map((goal, goalIndex) => (
                                    <div key={`${goal}-${goalIndex}`} className="flex items-start gap-2.5 text-sm leading-5">
                                      <span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-primary" />
                                      <span>{goal}</span>
                                    </div>
                                  ))
                                ) : (
                                  <p className="text-sm text-muted-foreground">Sem objetivos específicos nesta fase.</p>
                                )}
                              </div>
                            </div>
            ...
            <Card className="border-border/60 bg-background/70 p-5 shadow-sm">
              <div className="flex flex-col gap-2 border-b border-border/60 pb-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  ...
                  <h2 className="mt-1 text-xl font-semibold tracking-tight">Referências científicas</h2>
                </div>
                <Badge variant="outline" className="w-fit border-border/70 bg-background/80 text-muted-foreground">
              ...
              <div className="mt-4 grid gap-4 lg:grid-cols-2">
                {protocol.references.map((reference, index) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <Badge className="bg-primary text-[10px] font-bold">{reference.year}</Badge>
                    ...
                    {reference.url ? (
                      ...
                      <p className="mt-3 text-base font-semibold leading-6 text-foreground">{reference.title}</p>
                    )}

                    <p className="mt-3 line-clamp-2 text-sm text-muted-foreground">{reference.authors}</p>
                  </div>
                ))}
              </div>
            </Card>
        ...
        <div className="space-y-6 xl:sticky xl:top-24 xl:self-start">
          <Card className="border-border/60 bg-background/70 p-5 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-primary/75">Resumo rápido</p>
              ...
              {summaryCards.map((item) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className={cn('text-lg font-bold', item.tone)}>{item.value}</span>
                  </div>
          ...
          <Card className="border-border/60 bg-background/70 p-5 shadow-sm">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-600/80">Linha do tempo</p>
                <h2 className="mt-1 text-lg font-semibold">Marcos do protocolo</h2>
              </div>
              <Badge variant="outline" className="border-emerald-500/15 bg-emerald-500/5 text-emerald-700 dark:text-emerald-300">
                {milestones.length}
              </Badge>
            </div>
          ...
          <Card className="border-border/60 bg-background/70 p-5 shadow-sm">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.22em] text-cyan-600/80">Avaliação</p>
                <h2 className="mt-1 text-lg font-semibold">Testes clínicos</h2>
              </div>
              <Badge variant="outline" className="border-cyan-500/15 bg-cyan-500/5 text-cyan-700 dark:text-cyan-300">
                {linkedTests.length}
              </Badge>
            </div>
                ...
                linkedTests.map((test) => (
                  ...
                  >
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      <Badge variant="outline" className="border-cyan-500/15 bg-background/80 text-cyan-700 dark:text-cyan-300">
                        {test.target_joint}
                      </Badge>
          ...
          <Card className="border-border/60 bg-background/70 p-5 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-amber-600/80">Segurança clínica</p>
                ...
                restrictions.slice(0, 4).map((restriction, index) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline" className="border-amber-500/15 bg-background/80 text-amber-700 dark:text-amber-300">
                        W{restriction.weekStart}
                        {restriction.weekEnd != null ? ` - W${restriction.weekEnd}` : '+'}
                      </Badge>
                    </div>
                    <p className="mt-2 text-sm leading-5 text-foreground">{restriction.description}</p>
                  </div>
                ))
              ) : (
                <div className="rounded-2xl border border-dashed border-border/70 bg-background/60 px-4 py-5 text-sm text-muted-foreground">
              ...
              <div className="mt-5 rounded-2xl border border-red-500/10 bg-red-500/[0.04] p-4">
                ...
                <div className="space-y-2.5">
                  {contraindications.slice(0, 3).map((item, index) => (
                    <div key={`${item}-${index}`} className="flex items-start gap-2.5 text-sm leading-5 text-red-900 dark:text-red-200">
