...
export function ProtocolDetailView({
  ...
  const phases = (
    protocol.phases && protocol.phases.length > 0 ? protocol.phases : details?.phases || []
  ) as ProtocolPhase[];
  ...
  const togglePhase = (phaseName: string) => {
    setExpandedPhases((prev) =>
      prev.includes(phaseName) ? prev.filter((phase) => phase !== phaseName) : [...prev, phaseName],
    );
  };

  const category = getProtocolCategory(protocol.condition_name);
  const categoryInfo =
    PROTOCOL_CATEGORIES.find((item) => item.id === category) || PROTOCOL_CATEGORIES[0];
  ...
  const handleExportPDF = async () => {
    try {
      const { generateProtocolPdf } = await import("@/utils/generateProtocolPdf");
                  ...
                  <div className="mt-2.5 flex justify-between">
                    {phases.slice(0, 5).map((phase, i) => (
                      <div
                        ...
                        style={{ width: `${100 / Math.min(phases.length, 5)}%` }}
                      >
              ...
              <div className="flex flex-col gap-2 border-b border-border/60 pb-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    Cada fase agora fica compacta por padrão e expande só quando você precisa entrar
                    em detalhe.
                  </p>
                </div>
                <Badge
                              ...
                              <div className="min-w-0">
                                ...
                                <div className="mt-2 flex flex-wrap gap-2">
                                  ...
                                  {exercises.length > 0 && (
                                    <Badge
                                      variant="outline"
                                      className="border-indigo-500/20 bg-indigo-500/5 text-indigo-700 dark:text-indigo-300"
                                    >
          ...
          <Card className="border-border/60 bg-background/70 p-5 shadow-sm">
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex flex-wrap gap-1.5 justify-end">
                <Badge
                  variant="outline"
                  className="border-primary/15 bg-primary/5 text-primary text-[10px]"
                >
                  {categoryInfo.name}
                </Badge>
                {protocol.wiki_page_id && (
            ...
            <div className="mt-4 grid grid-cols-4 gap-2">
              {summaryCards.map((item) => (
                <div
                  key={item.label}
                  className="flex flex-col items-center rounded-2xl border border-border/60 bg-background py-3 shadow-sm"
                >
            ...
            {milestones.length > 0 && (
              <>
                <div className="mt-5 flex items-center justify-between gap-2">
                  ...
                  <Badge
                    variant="outline"
                    className="border-emerald-500/15 bg-emerald-500/5 text-emerald-700 dark:text-emerald-300 text-[10px]"
                  >
                    {milestones.length}
                  </Badge>
                </div>
