...
export function ContratadosTab({ eventoId, evento }: ContratadosTabProps) {
	...
	const { data: eventoContratados = [], isLoading } =
		useEventoContratados(eventoId);
	...
	const totals = useMemo(() => {
		const total = eventoContratados.reduce(
			(sum, c) => sum + Number(c.valor_acordado || 0),
			0,
		);
	...
	const handleCreate = assignmentForm.handleSubmit(async (assignmentData) => {
		...
		const novoContratado = await createContratado.mutateAsync(
			contratadoForm.getValues(),
		);
	...
	const handleDeleteContratado = async (contratadoId: string) => {
		if (
			confirm(
				"Deseja excluir este contratado? Isso pode afetar outros eventos.",
			)
		) {
	...
	const handleTogglePagamento = async (
		...
		await updateEventoContratado.mutateAsync({
			...
			data: {
				status_pagamento:
					assignment.status_pagamento === "PAGO" ? "PENDENTE" : "PAGO",
				horario_inicio: assignment.horario_inicio,
				horario_fim: assignment.horario_fim,
			},
			eventoId,
				...
				<div>
					...
					<div className="flex gap-4 mt-2 text-sm">
						<span className="text-green-600">
							Pago: R$ {totals.pagos.toFixed(2)}
						</span>
						<span className="text-yellow-600">
							Pendente: R$ {totals.pendente.toFixed(2)}
						</span>
					</div>
				</div>
				{canWrite("eventos") && (
					<Dialog open={open} onOpenChange={setOpen}>
						...
						<DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
							<DialogHeader>
								...
								<DialogDescription>
									Vincule um contratado existente ou cadastre um novo para este
									evento.
								</DialogDescription>
							</DialogHeader>
							<Tabs
								value={mode}
								onValueChange={(value) =>
									setMode(value as "existente" | "novo")
								}
							>
								...
								<TabsContent value="existente" className="space-y-4 mt-4">
									<div className="space-y-2">
										...
										<ContratadoCombobox
											...
											onValueChange={(value) =>
												assignmentForm.setValue("contratado_id", value)
											}
										/>
								...
								<TabsContent value="novo" className="space-y-4 mt-4">
									<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
										...
										<div className="space-y-2">
											...
											<Input
												id="novo-contato"
												{...contratadoForm.register("contato")}
											/>
										...
										<div className="space-y-2">
											...
											<Input
												id="novo-cpf"
												{...contratadoForm.register("cpf_cnpj")}
											/>
										...
										<div className="space-y-2">
											...
											<Input
												...
												{...contratadoForm.register("especialidade")}
											/>
										...
										<div className="space-y-2 sm:col-span-2">
											...
											<Textarea
												id="novo-observacoes"
												rows={3}
												{...contratadoForm.register("observacoes")}
											/>
							...
							<div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
								...
								<div className="space-y-2">
									...
									<Input
										...
										type="number"
										step="0.01"
										{...assignmentForm.register("valor_acordado", {
											valueAsNumber: true,
										})}
									/>
								...
								<div className="space-y-2">
									...
									<Input
										id="horario_inicio"
										type="time"
										{...assignmentForm.register("horario_inicio")}
									/>
								...
								<div className="space-y-2">
									...
									<Input
										id="horario_fim"
										type="time"
										{...assignmentForm.register("horario_fim")}
									/>
							...
							<div className="flex justify-end gap-2 mt-6">
								...
								<Button
									...
									disabled={
										createEventoContratado.isPending ||
										createContratado.isPending
									}
								>
									{createEventoContratado.isPending ||
									createContratado.isPending
			...
			<CardContent>
				{(!evento?.hora_inicio || !evento?.hora_fim) && (
					<div className="mb-4 rounded-md border border-yellow-200 bg-yellow-50 px-4 py-3 text-sm text-yellow-800">
						Para evitar conflitos de horário, defina o horário do evento (início
						e fim).
					</div>
				)}
				{isLoading ? (
							...
							{eventoContratados.map((item) => {
								...
								return (
									<TableRow key={item.id}>
										<TableCell>
											{contratado?.nome || "Contratado removido"}
										</TableCell>
										...
										<TableCell>
											R$ {Number(item.valor_acordado || 0).toFixed(2)}
										</TableCell>
										<TableCell>
											<Badge
												variant={
												...
												onClick={() =>
													canWrite("eventos") &&
													handleTogglePagamento(item.id, item.contratado_id)
												}
											>
												{item.status_pagamento === "PAGO" ? (
				...
				<Dialog open={editOpen} onOpenChange={setEditOpen}>
					<DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
						<DialogHeader>
							...
							<DialogDescription>
								Atualize os dados do contratado e as informações da alocação no
								evento.
							</DialogDescription>
						</DialogHeader>
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
							...
							<div className="space-y-2 sm:col-span-2">
								...
								<Textarea
									rows={3}
									{...editContratadoForm.register("observacoes")}
								/>
							</div>
						</div>
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
							...
							<div className="space-y-2">
								...
								<Input
									type="number"
									step="0.01"
									{...editAssignmentForm.register("valor_acordado", {
										valueAsNumber: true,
									})}
								/>
							</div>
							<div className="space-y-2">
								...
								<Input
									type="time"
									{...editAssignmentForm.register("horario_inicio")}
								/>
							</div>
							<div className="space-y-2">
								...
								<Input
									type="time"
									{...editAssignmentForm.register("horario_fim")}
								/>
							</div>
						</div>
						...
						<div className="flex justify-between gap-2 mt-6">
							...
							<div className="flex gap-2 ml-auto">
								...
								<Button
									onClick={handleEdit}
									disabled={
										updateEventoContratado.isPending ||
										updateContratado.isPending
									}
								>
									{updateEventoContratado.isPending ||
									updateContratado.isPending
