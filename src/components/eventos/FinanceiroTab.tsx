...
export function FinanceiroTab({ eventoId, evento }: FinanceiroTabProps) {
	...
	const [selectedPagamento, setSelectedPagamento] = useState<Record<
		string,
		unknown
	> | null>(null);
	...
	const custoPrestadores =
		prestadores?.reduce((sum, p) => sum + Number(p.valor_acordado), 0) || 0;
	const custoInsumos =
	...
	const custoOutros =
		pagamentos?.reduce((sum, p) => sum + Number(p.valor), 0) || 0;
	...
	const percentualPago =
		custoPrestadores > 0 ? (prestadoresPagos / custoPrestadores) * 100 : 0;

	const checklistOk =
		checklistItems?.filter((i) => i.status === "OK").length || 0;
	const checklistTotal = checklistItems?.length || 0;
	const percentualChecklist =
		checklistTotal > 0 ? (checklistOk / checklistTotal) * 100 : 0;
			...
			<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
				...
				<Card className="hover-scale">
					...
					<CardContent>
						...
						<p className="text-xs text-muted-foreground mt-1">
							{prestadores?.filter((p) => p.status_pagamento === "PENDENTE")
								.length || 0}{" "}
							prestador(es)
						</p>
					</CardContent>
						...
						<div className="flex-1">
							...
							<div className="flex gap-3 mt-2">
								<span className="text-xs text-green-600">
									✓{" "}
									{prestadores?.filter((p) => p.status_pagamento === "PAGO")
										.length || 0}{" "}
									Pagos
								</span>
						...
						<div className="pl-4 space-y-2">
							{["levar", "alugar", "comprar"].map((tipo) => {
								const custoTipo = checklistItems
									...
									.reduce(
										(sum, item) =>
											sum + Number(item.custo_unitario) * item.quantidade,
										0,
									);
								const qtdTipo = checklistItems.filter(
									(item) => item.tipo === tipo,
								).length;
				...
				<CardContent className="space-y-2 text-sm">
					...
					<div className="flex justify-between">
						<span className="text-muted-foreground">
							Valor padrão prestador:
						</span>
						<span className="font-medium">
							R$ {Number(evento.valor_padrao_prestador || 0).toFixed(2)}
						</span>
					</div>
							...
							<TableBody>
								{pagamentos.map((pagamento) => (
									<TableRow key={pagamento.id}>
										<TableCell>
											{format(new Date(pagamento.pago_em), "dd/MM/yyyy")}
										</TableCell>
										<TableCell>
											<Badge variant="outline">
												{pagamento.tipo.charAt(0).toUpperCase() +
													pagamento.tipo.slice(1)}
											</Badge>
										</TableCell>
