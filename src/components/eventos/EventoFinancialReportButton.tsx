...
export function EventoFinancialReportButton({ eventoId }: EventoFinancialReportButtonProps) {
  const [open, setOpen] = useState(false);
  const { data: report, isLoading } = useEventoFinancialReport(eventoId, open);
  const detalhePagamentos = Array.isArray(
    (report as { detalhePagamentos?: unknown[] } | undefined)?.detalhePagamentos,
  )
    ? (
        report as {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>{report?.eventoNome || "Carregando..."}</DialogDescription>
        </DialogHeader>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{formatCurrency(report.custosInsumos ?? 0)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Outros Custos</span>
                  <span className="font-medium">{formatCurrency(report.outrosCustos ?? 0)}</span>
                </div>
                {(report.pagamentosPendentes ?? 0) > 0 && (
                  <div className="flex justify-between pt-2 border-t">
                    <span className="text-orange-600 font-medium">Pagamentos Pendentes</span>
                          ...
                          <p className="text-xs text-muted-foreground">
                            ...
                            {pag.pagoEm && (
                              <>
                                {" "}
                                •{" "}
                                {format(new Date(pag.pagoEm), "dd/MM/yyyy", {
                                  locale: ptBR,
                                })}
                              </>
                            )}
