...
export function RewardsShop({ patientId }: RewardsShopProps) {
	...
	const { shopItems, userInventory, totalPoints, buyItem, isLoading } =
		...
		<div className="space-y-6 animate-fade-in">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
				{shopItems.map((item) => {
					...
					const ownedQuantity =
						userInventory.find((i) => i.item_id === item.id)?.quantity || 0;
					...
					<div className="p-8 text-center space-y-6">
						...
						<div className="bg-blue-50 p-4 rounded-2xl border border-blue-100 flex items-start gap-3 text-left">
							...
							<p className="text-[10px] text-blue-700 font-bold leading-relaxed uppercase">
								O item será adicionado ao seu inventário imediatamente após a
								confirmação. Alguns itens podem exigir ação do profissional.
							</p>
						</div>
					</div>
				</CustomModalBody>
