...
export function GamificationBadges({
  ...
  const RankIcon =
    reputation.rank === "legendary"
      ? Crown
      : reputation.rank === "master"
        ? Star
        : reputation.rank === "experienced"
          ? Award
          : reputation.rank === "beginner"
            ? Sparkles
            : Trophy;
  ...
  if (variant === "compact") {
    return (
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <Badge
          variant="secondary"
          className={cn("gap-1.5 px-3 py-1.5 text-sm font-semibold", levelTitle.color)}
        >
  ...
  return (
    <div className="space-y-4">
      {/* Main Profile Card */}
      <Card className={cn("relative overflow-hidden", getRankGradient(reputation.rank))}>
        <div className={cn("absolute left-0 top-0 bottom-0 w-1", reputation.rankColor)} />
          ...
          <div className="flex items-start justify-between">
            <div className="space-y-1">
              <CardTitle className="text-xl font-bold flex items-center gap-2">
                ...
                <Badge
                  variant="outline"
                  className={cn("ml-2", getRankBorderColor(reputation.rank))}
                >
                  {getRankLabel(reputation.rank, locale)}
                </Badge>
              </CardTitle>
              <p className="text-sm text-muted-foreground">{levelTitle.description}</p>
            </div>

            {/* Sound Toggle */}
            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={handleToggleSound}>
              {soundEnabled ? (
          ...
          <div className="space-y-2">
            ...
            {nextTitle && (
              <p className="text-xs text-muted-foreground">
                Próximo título:{" "}
                <strong>
                  {nextTitle.icon} {nextTitle.title}
                </strong>{" "}
                em {nextTitle.level - currentLevel} níveis
              </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="text-center p-3 rounded-lg bg-gradient-to-br from-orange-500/10 to-red-500/10 border border-orange-500/20">
              ...
              {longestStreak > streak && (
                <p className="text-[10px] text-muted-foreground mt-0.5">recorde: {longestStreak}</p>
              )}
            </div>
            ...
            <div className="text-center p-3 rounded-lg bg-gradient-to-br from-yellow-500/10 to-amber-500/10 border border-yellow-500/20">
              ...
              <p className="text-[10px] text-muted-foreground mt-0.5">{totalAchievements} totais</p>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {reputation.achievementTitles.map((title) => (
                  <Badge
                    key={title.code}
                    variant="outline"
                    className={cn("gap-1.5 px-2.5 py-1", title.color, "border-current/30")}
                  >
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {unlockedAchievements.map((ua) => (
                          <Badge key={ua.id} variant="secondary" className="gap-1.5 px-2.5 py-1">
