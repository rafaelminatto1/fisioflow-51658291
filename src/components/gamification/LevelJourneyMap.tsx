  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between px-1">
        <h3 className="font-semibold text-lg flex items-center gap-2">Mapa da Jornada</h3>
        <span className="text-xs text-muted-foreground">
          Próxima recompensa: Nível {Math.ceil((currentLevel + 0.1) / 5) * 5}
        </span>
      </div>
          ...
          {levels.map((level) => {
            ...
            return (
              <div key={level} className="relative flex flex-col items-center gap-3 group">
                ...
                {level < levels.length && (
                  <div
                    className={`absolute top-6 left-1/2 w-[calc(100%+1rem)] h-1 -z-10
