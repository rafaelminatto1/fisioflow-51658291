...
export function PatientGamification({ patientId }: PatientGamificationProps) {
	...
	const hasFreeze = userInventory?.some(
		(i) => i.item?.code === "streak_freeze" && i.quantity > 0,
	);
					...
					<div className="flex-1 space-y-1">
						...
						{unlocked && (
							<p className="text-[10px] text-muted-foreground pt-1 flex items-center">
								{(() => {
									const CheckCheckIcon = getGamificationIcon(
										"CheckCheck",
										Star,
									);
									return (
										<CheckCheckIcon className="h-3 w-3 mr-1 text-primary" />
									);
								})()}
								Desbloqueado{" "}
		...
		<div className="space-y-6 animate-fade-in">
						...
						<div className="text-center p-4 bg-background/50 backdrop-blur-sm rounded-xl border border-border/50 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden">
							...
							<div className="bg-orange-100 dark:bg-orange-900/20 w-10 h-10 rounded-full flex items-center justify-center mx-auto mb-2">
								<Flame
									className={cn(
										"h-5 w-5",
										hasFreeze ? "text-blue-500" : "text-orange-500",
									)}
								/>
							</div>
			...
			<Tabs defaultValue="overview" className="w-full space-y-6">
				<TabsList className="w-full justify-start overflow-x-auto h-auto p-1 bg-muted/50">
					...
					<TabsTrigger
						value="shop"
						className="rounded-full px-4 text-xs sm:text-sm flex gap-2"
					>
						Loja
				...
				>
					<div className="space-y-4">
						...
						<Tabs defaultValue="all" className="w-full">
							...
							{["streak", "milestone", "timing", "recovery", "level"].map(
								(tab) => {
