  ...
  useEffect(() => {
    ...
    const loadGamificationData = async () => {
      ...
      try {
        ...
        const levelCalculation = profile
          ? await GamificationTriggerService.calculateLevelAsync(profile.total_points || 0)
          : null;
        const dailyQuests = questsRes.data?.quests_data ?? [];
        const unlockedAchievements = (achievementsRes.data?.unlocked ?? [])
          .slice(0, 10)
          .map((log) => {
            const achievement = achievementsRes.data?.all?.find(
              (item) =>
                item.id === log.achievement_id ||
                item.code === log.achievement_title ||
                item.title === log.achievement_title,
            );
            return {
              id: log.id,
      ...
      <CardContent>
        <Tabs defaultValue="overview" className="w-full">
          ...
          <TabsContent value="achievements" className="mt-4">
            <RecentAchievementsWidget achievements={data.unlockedAchievements} maxItems={10} />
          </TabsContent>
        </Tabs>
      </CardContent>
    </Card>
