...
export function EnhancedLeaderboard({ currentPatientId, locale = "pt" }: EnhancedLeaderboardProps) {
  ...
  const { data: leaderboard = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await gamificationApi.getLeaderboard({
        period: period === "week" ? "weekly" : period === "month" ? "monthly" : "all",
        limit: 100,
      });
  ...
  const getRankIcon = (rank: number) =>
    rank === 1 ? (
      ...
      <span className="text-muted-foreground font-bold w-6 text-center">{rank}</span>
    );
  ...
  const getPeriodIcon = (per: LeaderboardPeriod) =>
    per === "all" ? <Trophy className="h-4 w-4" /> : <Calendar className="h-4 w-4" />;
    ...
    <Card className="border-none shadow-none bg-transparent animate-fade-in">
      <CardHeader className="px-0 pt-0">
        ...
        <div className="mt-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            {(["xp", "level", "streak", "achievements"] as LeaderboardCategory[]).map((item) => (
              <Button
                key={item}
      ...
      <CardContent className="px-0 pb-0">
        {filteredLeaderboard.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground">{t.noRanking}</div>
        ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {isCurrentUser && isTop3 && <Badge variant="secondary">Top 3</Badge>}
                    </div>
                    <p className="text-sm text-muted-foreground">{t.categories[category]}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-lg font-bold">{getDisplayValue(entry, category)}</p>
