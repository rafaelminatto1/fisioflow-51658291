...
import { Sparkles, Clock, Calendar, User, Check, ChevronRight, Lightbulb } from "lucide-react";
...
class NLPParser {
  ...
  private extractDuration(text: string): {
    ...
    for (const [term, value] of this.commonTerms.entries()) {
      if (
        typeof value === "number" &&
        (term === "hora" || term === "horas" || term.includes("minuto"))
      ) {
  ...
  private extractDate(text: string): { date: Date | null; remainder: string } {
    ...
    // Dia da semana
    const dayMatch = remainder.match(
      /(?:agendar|marcar|na|para|de)\s+(segunda|terça|quarta|quinta|sexta|sábado|domingo)/gi,
    );
    if (dayMatch) {
  ...
  private extractService(text: string): {
    ...
    for (const [term, value] of this.commonTerms.entries()) {
      if (
        typeof value === "string" &&
        (term === "avaliação" || term === "sessão" || term === "retorno")
      ) {
  ...
  private extractPatient(text: string): {
    patient: string | null;
    remainder: string;
  } {
  ...
  private extractNotes(text: string): {
    notes: string | null;
    remainder: string;
  } {
    ...
    <div className="space-y-4">
            ...
            <div className="flex-1">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                {parsed.patientName && (
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      <span className="text-muted-foreground">Paciente:</span> {parsed.patientName}
                    </span>
                  </div>
                )}
                {parsed.date && (
                ...
                {parsed.time && (
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      <span className="text-muted-foreground">Horário:</span> {parsed.time}
                    </span>
                  </div>
                )}
                {parsed.duration && (
                  <div className="col-span-2 flex items-center gap-2">
                    <Clock className="w-4 h-4 text-muted-foreground" />
                    <span>
                      <span className="text-muted-foreground">Duração:</span> {parsed.duration}{" "}
                      minutos
                    </span>
                  </div>
                )}
              ...
              <div className="mt-3 pt-3 border-t border-primary/20">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">Confiança da detecção:</span>
                  <span
                    className={cn(
                      "font-semibold",
      ...
      {!input && !parsed && (
        <div className="mt-6 space-y-3">
          <h3 className="font-semibold text-sm text-muted-foreground">Tente estes exemplos:</h3>
