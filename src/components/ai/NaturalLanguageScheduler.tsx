...
class NLPParser {
	...
	private extractDate(text: string): { date: Date | null; remainder: string } {
		...
		// Hoje/amanhã
		const todayMatch = remainder.match(
			/(?:agendar|marcar|para)\s+(hoje|amanhã)/gi,
		);
		if (todayMatch) {
		...
		const explicitDateMatch = remainder.match(
...
export const NaturalLanguageScheduler: React.FC<
...
> = ({
	...
	const handleInput = useCallback((value: string) => {
		...
		if (value.length > 2 && parserRef.current) {
			...
			if (result.date) {
				newSuggestions.push(
					`Data: ${format(result.date, "dd 'de' MMMM", { locale: ptBR })}`,
				);
			}
			if (result.time) {
