		...
		try {
			...
			setMessages((prev) => [
				...prev,
				{
					role: "model",
					content:
						"❌ Erro ao conectar com assistente clínico. Tente novamente.",
				},
			]);
		} finally {
			setLoading(false);
		...
		<Card className="h-[600px] flex flex-col shadow-lg border-t-4 border-t-purple-500">
			<CardHeader className="pb-3 border-b bg-purple-50/50">
				...
				<p className="text-xs text-gray-500">
					Pergunte sobre o histórico de {patientName}. Ex: "Resuma a evolução da
					dor".
				</p>
				{analyticsLoading && (
					<p className="text-[11px] text-purple-600">
						Preparando contexto clinico do paciente...
					</p>
				)}
			</CardHeader>
				...
				<ScrollArea className="h-full p-4">
					{messages.length === 0 && (
						<div className="text-center text-gray-500 mt-20 space-y-2">
							...
							<p className="text-sm">
								Analiso todo o prontuário deste paciente em segundos.
							</p>
						</div>
					)}
