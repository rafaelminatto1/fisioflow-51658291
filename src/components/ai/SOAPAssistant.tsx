...
export function SOAPAssistant({
	...
	const [finalResult, setFinalResult] = useState<SOAPGenerationResult | null>(
		null,
	);
	...
	const generateSOAP = useCallback(
		async (text?: string) => {
			...
			try {
				...
				const { generateSOAPNote } = await import(
					"@/services/ai/geminiAiService"
				);
				...
				const errorMessage =
					err instanceof Error ? err.message : "Erro ao gerar nota SOAP";
				setError(errorMessage);
				setOptimisticResult(null);
				toast({
		...
		<div className="space-y-6">
			...
			{finalResult && (
				<>
							...
							<div className="space-y-2">
								...
								{finalResult.soap.objective ? (
									...
									<p className="text-sm text-muted-foreground">
										Nenhum dado objetivo registrado
									</p>
								)}
							</div>
					...
					{/* ICD-10 Codes */}
					{finalResult.suggestedCodes &&
						finalResult.suggestedCodes.length > 0 && (
							<Card>
								<CardHeader>
