...
export function ExerciseAI({
	...
	const patientName = useMemo(
		() => patient?.name ?? "recomendações gerais",
		[patient?.name],
	);
	...
	const [optimisticResult, setOptimisticResult] =
		useState<ExerciseProgramResponse | null>(null);
	const [finalResult, setFinalResult] =
		useState<ExerciseProgramResponse | null>(null);
	const [genkitResult, setGenkitResult] = useState<ExercisePlanResponse | null>(
		null,
	);
	...
	const generateFullPlanWithGenkit = useCallback(async () => {
		...
		try {
			...
			setError(
				"Falha ao gerar plano com o AI Coach. Tente as recomendações rápidas.",
			);
		} finally {
			setLoading(false);
			setProgress(0);
	...
	const generateRecommendations = useCallback(async () => {
		...
		setOptimisticResult({
			exercises: [],
			programRationale:
				"Analisando perfil do paciente e gerando recomendações...",
			expectedOutcomes: [],
			progressionCriteria: [],
			estimatedDuration: 0,
		});
		...
		try {
			...
			const { getExerciseSuggestions } = await import(
			...
			const data: ExerciseProgramResponse =
				result.data as unknown as ExerciseProgramResponse;
			...
			const errorMessage =
				err instanceof Error ? err.message : "Erro ao gerar recomendações";
	...
	const contextSnapshot = useMemo(() => {
		...
		return {
			...
			equipmentText:
				availableEquipment.slice(0, 4).join(", ") || "não informado",
			treatmentPhase,
			sessionCount,
			soapSummary,
		};
...
`
	...
	const clearConversation = useCallback(() => {
		...
		try {
			...
			logger.warn(
				"[ExerciseAI] Failed clearing chat history",
				err,
				"ExerciseAI",
			);
		}
	...
	const exportChatToProntuario = useCallback(() => {
		...
		const body = chatMessages
			.map(
				(m) =>
					`[${m.role === "assistant" ? "ASSISTENTE" : "PROFISSIONAL"}] ${m.text}`,
			)
			.join("\n");
		...
		toast({
			...
			description:
				"Arquivo .txt baixado e conteúdo copiado para anexar no prontuário.",
		});
	}, [
		chatMessages,
		hasPatient,
	...
	useEffect(() => {
		try {
			...
			logger.warn(
				"[ExerciseAI] Failed loading chat history",
				loadErr,
				"ExerciseAI",
			);
		}
	...
	useEffect(() => {
		...
		try {
			...
			logger.warn(
				"[ExerciseAI] Failed saving chat history",
				saveErr,
				"ExerciseAI",
			);
		}
			...
			<Tabs defaultValue="suggestions" className="w-full">
				<div className="flex items-center justify-between mb-4">
					<TabsList className="bg-muted p-1 rounded-lg">
						<TabsTrigger
							value="suggestions"
							className="px-4 py-2 rounded-md data-[state=active]:bg-background shadow-sm"
						>
							Sugestões Rápidas
						</TabsTrigger>
						<TabsTrigger
							value="aicoach"
							className="px-4 py-2 rounded-md data-[state=active]:bg-background shadow-sm flex items-center gap-2"
						>
						...
						<div className="xl:col-span-4 space-y-4">
								...
								<CardContent className="space-y-2 text-sm">
									<div className="flex items-center justify-between">
										...
										<span className="font-medium">
											{hasPatient ? patientName : "Não selecionado"}
										</span>
									</div>
									<div className="flex items-center justify-between">
							...
							<Card className="border-blue-200">
								<CardHeader className="pb-3">
									...
									<CardDescription>
										Fluxo recomendado: gerar, revisar, aplicar.
									</CardDescription>
								</CardHeader>
									...
									<div className="grid grid-cols-2 gap-4">
										<div className="flex items-center gap-2">
											...
											<span className="text-sm">
												Duração estimada:{" "}
												<strong>{result.estimatedDuration} min</strong>
											</span>
										</div>
										<div className="flex items-center gap-2">
													...
													<CardContent className="p-4">
														<div className="flex items-start gap-3">
															<div className="flex-1">
																...
																{exercise.goalsAddressed &&
																	exercise.goalsAddressed.length > 0 && (
																		<div className="flex flex-wrap gap-1">
																			{exercise.goalsAddressed.map(
																				(goal, i) => (
																					<Badge
																						key={i}
																						variant="outline"
						...
						<CardHeader>
							<div className="flex items-center gap-3">
								...
								<div>
									...
									<CardDescription>
										Geração de plano completo e estruturado usando Gemini 1.5
										Flash.
									</CardDescription>
								</div>
							</div>
							...
							<Card className="border-l-4 border-l-amber-500">
								...
								<CardContent className="space-y-4">
									...
									<div className="p-3 border rounded-lg bg-green-50/30">
										<h4 className="text-xs font-bold uppercase text-green-700 mb-1">
											Aquecimento (Warm-up)
										</h4>
									...
									<div className="space-y-3">
										...
										{genkitResult.exercises.map((ex, i) => (
											<div
												key={i}
												className="p-4 border rounded-xl hover:bg-muted/30 transition-colors relative group"
											>
										...
										<Button
											...
											onClick={() => {
												navigator.clipboard.writeText(
													JSON.stringify(genkitResult, null, 2),
												);
												toast({
													title: "Copiado!",
													description: "Dados do plano copiados.",
												});
											}}
										>
					...
					<div className="flex items-start gap-3">
						...
						<div className="text-sm space-y-1">
							<p className="font-medium text-xs">
								Powered by FisioFlow Intelligence (Gemini & Genkit)
							</p>
							<p className="text-muted-foreground text-[11px]">
								As recomendações de IA são ferramentas de suporte e não
								substituem o julgamento clínico do profissional.
							</p>
						</div>
					</div>
				</CardContent>
