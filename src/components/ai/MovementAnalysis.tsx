...
export function MovementAnalysis({
  ...
  const analyzeForm = useCallback(async () => {
    ...
    try {
      ...
      const { url: patientVideoUrl } = await uploadToR2(
        videoFile,
        `movement_analysis/${patientId}`,
      );
      ...
      setAnalysisProgress({
        ...
        progress: 40,
        message:
          language === "pt-BR" ? "Analisando movimento com IA..." : "Analyzing movement with AI...",
      });
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={analyzeForm} disabled={loading} size="lg">
                  {loading ? (
                    <>
        ...
        <Card className="border-destructive">
          <CardContent className="p-6">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h3 className="font-semibold">{isPTBR ? "Erro na análise" : "Analysis Error"}</h3>
                <p className="text-sm text-muted-foreground">{error}</p>
                <Button variant="outline" size="sm" className="mt-2" onClick={analyzeForm}>
      ...
      {analysisResult && (
        <>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="mt-4 p-4 bg-muted/50 rounded">
                <p className="text-sm">
                  <strong>{isPTBR ? "Repetições:" : "Repetitions:"}</strong>{" "}
                  {analysisResult.repetitions}
                  {expectedReps && ` / ${expectedReps} ${isPTBR ? "esperadas" : "expected"}`}
                </p>
                <p className="text-sm mt-1">
                  <strong>{isPTBR ? "Duração:" : "Duration:"}</strong>{" "}
                  {Math.round(analysisResult.patientVideoDuration)}s
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-sm bg-muted/50 p-3 rounded">{analysisResult.summary}</p>
            </CardContent>
          </Card>
                  ...
                  <div className="space-y-3 pr-4">
                    {analysisResult.deviations.map((deviation, i) => (
                      ...
                      >
                        ...
                        <p className="text-xs">
                          <strong>{isPTBR ? "Correção:" : "Correction:"}</strong>{" "}
                  ...
                  {analysisResult.safetyConcerns.map((concern, i) => (
                    ...
                    >
                      <div className="flex items-start gap-2 mb-2">
                        ...
                        <div className="flex-1">
                          <Badge
                            variant={concern.severity === "danger" ? "destructive" : "secondary"}
                            className="mb-1"
                          >
                            {concern.type}
                          </Badge>
                          ...
                          <p className="text-xs mt-1">
                            <strong>{isPTBR ? "Recomendação:" : "Recommendation:"}</strong>{" "}
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-sm bg-muted/50 p-3 rounded">{analysisResult.progression}</p>
            </CardContent>
          </Card>
