...
export const ActivityLabChart: React.FC<ActivityLabChartProps> = ({
	session,
}) => {
	const data = (session.raw_force_data || session.rawForceData || []).map(
		(point: RawForceDataPoint) => ({
	...
	const peakPoint = data.reduce(
		(prev, current) => (prev.force > current.force ? prev : current),
		{ time: 0, force: 0 },
	);
		...
		return (
			<div className="flex items-center justify-center h-64 bg-gray-50 rounded-lg border border-dashed">
				<p className="text-gray-500">
					Nenhum dado de força disponível para este teste.
				</p>
			</div>
		);
	}
	...
	const exportToCSV = () => {
		...
		const csvRows = data.map(
			(point) => `${point.time.toFixed(3)},${point.force.toFixed(3)}`,
		);
		...
		link.setAttribute(
			"download",
			`activity_lab_${session.protocol_name}_${session.id}.csv`,
		);
			...
			<CardHeader className="bg-slate-50/50 border-b border-slate-100 flex flex-row items-center justify-between space-y-0">
				<div>
					...
					<CardDescription className="font-medium text-slate-500">
						{session.body_part || session.bodyPart} | Lado:{" "}
						<span className="text-indigo-600 font-bold">{session.side}</span>
					</CardDescription>
				</div>
				<Button
					variant="outline"
						...
						>
							...
							<XAxis
								...
								label={{
									...
									offset: 0,
									className:
										"text-[10px] font-bold fill-slate-400 uppercase tracking-widest",
								}}
								type="number"
								domain={[0, "auto"]}
							...
							<ReferenceLine
								...
								label={{
									position: "right",
									value: `PICO: ${peakPoint.force.toFixed(1)}kg`,
									fill: "#ef4444",
									fontSize: 10,
									fontWeight: "bold",
								}}
				...
				<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
					<div className="p-4 bg-blue-50/50 rounded-2xl border border-blue-100/50">
						...
						<p className="text-2xl font-black text-blue-700">
							{(session.peak_force || session.peakForce || 0).toFixed(1)}{" "}
					...
					<div className="p-4 bg-emerald-50/50 rounded-2xl border border-emerald-100/50">
						...
						<p className="text-2xl font-black text-emerald-700">
							{(session.avg_force || session.avgForce || 0).toFixed(1)}{" "}
							<span className="text-sm font-bold">kg</span>
						</p>
					</div>
				...
				<div className="mt-6 pt-4 border-t border-slate-100 flex flex-wrap gap-x-6 gap-y-2">
					<div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-tighter">
						...
						<span className="text-slate-600">
							{session.device_model || session.deviceModel}
						</span>
					</div>
					<div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-tighter">
						<span className="w-1.5 h-1.5 rounded-full bg-slate-300" />
						Firmware:{" "}
						<span className="text-slate-600">
							{session.device_firmware || session.deviceFirmware || "N/A"}
						</span>
					</div>
					<div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-tighter">
						<span className="w-1.5 h-1.5 rounded-full bg-slate-300" />
						...
						<span
							className={cn(
								"font-black",
								(session.device_battery || 0) < 20
									? "text-red-500"
									: "text-emerald-500",
							)}
						>
							{session.device_battery || session.deviceBattery || 0}%
						</span>
					</div>
					<div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-tighter">
						<span className="w-1.5 h-1.5 rounded-full bg-slate-300" />
						Amostragem:{" "}
						<span className="text-slate-600">
							{session.sample_rate || session.sampleRate || 80} Hz
						</span>
					</div>
				</div>
			</CardContent>
