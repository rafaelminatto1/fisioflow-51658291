...
export const ActivityLabChart: React.FC<ActivityLabChartProps> = ({ session }) => {
  ...
  const peakPoint = data.reduce((prev, current) => (prev.force > current.force ? prev : current), {
    time: 0,
    force: 0,
  });
      ...
      <CardContent className="pt-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
          ...
          <div className="p-4 bg-amber-50/50 rounded-2xl border border-amber-100/50">
            ...
            <p className="text-2xl font-black text-amber-700">
              {(session.duration || 0).toFixed(1)} <span className="text-sm font-bold">s</span>
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 pt-4 border-t border-slate-100 flex flex-wrap gap-x-6 gap-y-2">
          <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-tighter">
            ...
            Dispositivo:{" "}
            <span className="text-slate-600">{session.device_model || session.deviceModel}</span>
          </div>
          <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-tighter">
            <span className="w-1.5 h-1.5 rounded-full bg-slate-300" />
            Firmware:{" "}
