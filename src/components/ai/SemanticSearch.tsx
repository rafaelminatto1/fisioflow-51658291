          ...
          {results.length === 0 ? (
            ...
            <>
              <p className="text-sm text-muted-foreground">
                {results.length}{" "}
                {results.length === 1 ? "evolução encontrada" : "evoluções encontradas"}
              </p>
...
export function SimilarityBadge({ similarity }: { similarity: number }) {
  ...
  return <Badge className={getColor()}>{(similarity * 100).toFixed(0)}% similar</Badge>;
}

/**
 * Example queries display
 */
export function SemanticSearchExamples({ onSelect }: { onSelect: (query: string) => void }) {
  const examples = [
    {
 ...
 */
export function SemanticSearchEmptyState({ onSearch }: { onSearch: (query: string) => void }) {
  return (
    <Card className="border-dashed">
      <CardContent className="py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground mb-4 max-w-md mx-auto">
          Encontre evoluções similares usando linguagem natural. Descreva sintomas, tratamentos ou
          condições.
        </p>
