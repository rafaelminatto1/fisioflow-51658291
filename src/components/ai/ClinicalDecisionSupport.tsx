...
export interface ClinicalDecisionSupportProps {
	...
	patient: Pick<
		Patient,
		"id" | "name" | "birthDate" | "gender" | "mainCondition"
	> & {
		age: number;
	};
...
export function ClinicalDecisionSupport({
	...
	const [activeTab, setActiveTab] = useState<"pain" | "recovery" | "response">(
		"pain",
	);
	const [optimisticResult, setOptimisticResult] = useState<string | null>(null);
	const [painAnalysis, setPainAnalysis] = useState<PainAnalysisResult | null>(
		null,
	);
	const [recoveryPrediction, setRecoveryPrediction] =
		useState<RecoveryPrediction | null>(null);
	const [treatmentResponse, setTreatmentResponse] =
	...
	const runAllAnalyses = useCallback(async () => {
		...
		try {
			...
			logger.error(
				"[ClinicalDecisionSupport] Error",
				err,
				"ClinicalDecisionSupport",
			);
			const errorMessage =
				err instanceof Error ? err.message : "Erro ao realizar análises";
	 ...
	 */
	const analyzePainEvolution =
		useCallback(async (): Promise<PainAnalysisResult> => {
			const { analyzePainEvolution: analyzePain } = await import(
	...
	const predictRecovery = useCallback(async (): Promise<RecoveryPrediction> => {
		const { predictRecovery: predict } = await import(
			"@/services/ai/geminiAiService"
		);
	 ...
	 */
	const evaluateTreatmentResponse =
		useCallback(async (): Promise<TreatmentResponse> => {
			const { evaluateTreatmentResponse: evaluate } = await import(
				"@/services/ai/geminiAiService"
			);
		...
		<div className="space-y-6">
			...
			{(painAnalysis || recoveryPrediction || treatmentResponse) && (
				<Tabs
					value={activeTab}
					onValueChange={(v) =>
						setActiveTab(v as "pain" | "recovery" | "response")
					}
				>
								...
								<CardContent className="space-y-4">
									<div className="text-center p-4 bg-muted/50 rounded-lg">
										<p className="text-3xl font-bold mb-1">
											{painAnalysis.globalPainChange > 0 ? "+" : ""}
											{painAnalysis.globalPainChange}
										</p>
										...
										<Badge
											variant={
												painAnalysis.overallTrend === "improving"
													? "default"
													: painAnalysis.overallTrend === "worsening"
														? "destructive"
														: "secondary"
											}
											className="mt-2"
								...
								<CardContent className="space-y-4">
									<div className="text-center p-4 bg-muted/50 rounded-lg">
										...
										<div className="flex items-center justify-center gap-2 mt-2">
											<Badge variant="outline">
												Confiança:{" "}
												{Math.round(recoveryPrediction.confidenceScore * 100)}%
											</Badge>
										</div>
									</div>
									...
									<div className="grid grid-cols-3 gap-2 text-center">
										<div className="p-2 bg-muted/30 rounded">
											...
											<p className="font-semibold text-sm">
												{formatDate(
													recoveryPrediction.confidenceInterval.lower,
												)}
											</p>
										</div>
										<div className="p-2 bg-muted/30 rounded">
											...
											<p className="font-semibold text-sm">
												{recoveryPrediction.confidenceInterval.expectedDays}{" "}
												dias
											</p>
										</div>
										<div className="p-2 bg-muted/30 rounded">
											...
											<p className="font-semibold text-sm">
												{formatDate(
													recoveryPrediction.confidenceInterval.upper,
												)}
											</p>
										</div>
									</div>
														...
														<div className="flex items-start justify-between">
															<div className="flex-1">
																...
																<p className="text-xs mt-1">
																	Previsto: {formatDate(milestone.expectedDate)}
																</p>
															</div>
															{milestone.achieved && (
									...
									<div>
										...
										<div className="grid grid-cols-3 gap-4 text-center">
											<div>
												<p className="text-2xl font-bold">
													{recoveryPrediction.similarCases.totalAnalyzed}
												</p>
												<p className="text-xs text-muted-foreground">
													Casos analisados
												</p>
											...
											<div>
												<p className="text-2xl font-bold">
													{recoveryPrediction.similarCases.averageRecoveryTime}
												</p>
												<p className="text-xs text-muted-foreground">
													Média de sessões
												</p>
											...
											<div>
												<p className="text-2xl font-bold">
													{recoveryPrediction.similarCases.successRate}%
												</p>
												<p className="text-xs text-muted-foreground">
													Taxa de sucesso
												</p>
										...
										{recoveryPrediction.similarCases.keyInsights.length > 0 && (
											<div className="mt-3 p-3 bg-muted/50 rounded">
												{recoveryPrediction.similarCases.keyInsights.map(
													(insight, i) => (
						...
						<TabsContent value="response" className="space-y-4">
							<Card>
								...
								<CardContent className="space-y-4">
									<div className="text-center p-4 bg-muted/50 rounded-lg">
										<Badge
											variant={getEffectivenessColor(
												treatmentResponse.effectiveness,
											)}
											className="mb-2"
										>
											{treatmentResponse.effectiveness === "excellent" &&
												"Excelente"}
											{treatmentResponse.effectiveness === "good" && "Boa"}
											{treatmentResponse.effectiveness === "moderate" &&
												"Moderada"}
											{treatmentResponse.effectiveness === "minimal" &&
												"Mínima"}
											{treatmentResponse.effectiveness === "none" &&
												"Sem melhora"}
											{treatmentResponse.effectiveness === "worsening" &&
												"Piora"}
										</Badge>
										<p className="text-3xl font-bold mt-2">
											{treatmentResponse.painReductionPercentage > 0 ? "+" : ""}
											{treatmentResponse.painReductionPercentage}%
										</p>
										<p className="text-sm text-muted-foreground">
											Redução da dor
										</p>
