...
export const ActivityLabComparisonChart: React.FC<
	ActivityLabComparisonChartProps
> = ({ session1, session2 }) => {
	// Normalize data for comparison
	const data1 = (session1.raw_force_data || session1.rawForceData || []).map(
		(point) => ({
	...
	const data2 = (session2.raw_force_data || session2.rawForceData || []).map(
		(point) => ({
			time: point.timestamp / 1000,
			force2: point.value,
		}),
				...
				<div className="grid grid-cols-2 gap-4 mt-8">
					<div className="p-4 rounded-2xl border border-blue-100 bg-blue-50/30">
						<p className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-2">
							{session1.side} -{" "}
							{format(new Date(session1.created_at), "dd/MM/yyyy")}
						</p>
					...
					<div className="p-4 rounded-2xl border border-red-100 bg-red-50/30">
						<p className="text-[10px] font-black text-red-400 uppercase tracking-widest mb-2">
							{session2.side} -{" "}
							{format(new Date(session2.created_at), "dd/MM/yyyy")}
						</p>
						<div className="flex justify-between items-end">
							<div>
