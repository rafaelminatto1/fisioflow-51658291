...
import { AlertTriangle, Bell, Clock, Calendar, TrendingDown, X, Sparkles } from "lucide-react";
import { useQuery } from "@tanstack/react-query";
import { cn } from "@/lib/utils";
import { generateDashboardInsights, ProactiveSuggestion } from "@/lib/ai/proactiveAssistant";
...
export function ProactiveSuggestions({ limit = 5, className }: ProactiveSuggestionsProps) {
  ...
  const visibleSuggestions = useMemo(() => {
    if (!insights) return [];
    return insights.suggestions.filter((s) => !dismissedIds.has(s.id)).slice(0, limit);
  ...
  const handleDismiss = (suggestionId: string) => {
    setDismissedIds((prev) => {
      ...
      localStorage.setItem("fisioflow-dismissed-suggestions", JSON.stringify(Array.from(newSet)));
      return newSet;
    });
  ...
  return (
    <Card className={className}>
      <CardHeader>
        ...
        <CardDescription>
          {visibleSuggestions.length} sugestão
          {visibleSuggestions.length !== 1 ? "ões" : ""} para melhorar sua clínica
        </CardDescription>
      </CardHeader>
                ...
                <div className="flex items-start gap-3 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <Badge
                        variant="outline"
                        className={cn("text-xs shrink-0", config.border, config.color)}
                      >
                        {config.label}
                      </Badge>
                    </div>
