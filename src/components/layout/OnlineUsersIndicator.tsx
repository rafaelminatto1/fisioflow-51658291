    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          <Badge variant="secondary" className="font-semibold min-w-[24px] justify-center">
            {onlineCount}
          </Badge>

          <span className="text-sm text-muted-foreground hidden md:inline">online</span>
        </button>
      </PopoverTrigger>
      ...
      <PopoverContent className="w-80 p-0" align="end" sideOffset={8}>
        ...
        <div className="px-4 py-3 border-b bg-gradient-to-r from-primary/10 to-primary/5">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-sm">Usuários Online</h3>
          ...
          {onlineUsers.length === 0 ? (
            ...
            <div className="divide-y">
              {onlineUsers.map((user) => (
                <div key={user.userId} className="px-4 py-3 hover:bg-muted/30 transition-colors">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {(() => {
                        ...
                        const diffMinutes = Math.floor(
                          (now.getTime() - joinedTime.getTime()) / 60000,
                        );
        ...
        <div className="px-4 py-2 border-t bg-muted/30 text-center">
          <p className="text-xs text-muted-foreground">Baseado em atividade recente dos usuários</p>
        </div>
      </PopoverContent>
    </Popover>
  );
