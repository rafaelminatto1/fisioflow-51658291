...
import { Search, Sparkles, MessageSquare, Activity, DollarSign, X, Bot, CornerDownLeft } from "lucide-react";
...
export function AICommandBar() {
  ...
  const { messages, input, handleInputChange, handleSubmit, setMessages: _setMessages } = useChat({
          ...
          >
            ...
            <div className="flex items-center px-4 py-4 border-b border-slate-100 dark:border-slate-800/60 bg-slate-50/50 dark:bg-slate-900">
              ...
              <div className="flex items-center gap-2 ml-4 shrink-0">
                ...
                <button onClick={() => setOpen(false)} className="p-1 rounded-md hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-600 transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
            </div>
              ...
              {messages.length > 0 ? (
                ...
                <Command className="bg-transparent" label="Global Command Menu" shouldFilter={true}>
                  <Command.List className="p-2 pb-4">
                    ...
                    <Command.Group heading="Insights Rápidos (IA)" className="text-xs text-slate-500 dark:text-slate-400 px-2 py-1">
                      <Command.Item 
                    ...
                    <Command.Group heading="Navegação do Sistema" className="text-xs text-slate-500 dark:text-slate-400 px-2 py-1">
                      <Command.Item 
