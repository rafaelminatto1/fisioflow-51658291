...
export function ResponsiveGridLayout({
  ...
  return (
    <div className={cn(gridCols, className)} style={{ gap }}>
      {children}
    </div>
  );
}
...
export function CardGrid({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <ResponsiveGridLayout cols={{ xs: 1, sm: 2, md: 2, lg: 3 }} className={className}>
      {children}
    </ResponsiveGridLayout>
  );
}
 ...
 */
export function ResponsiveCard({ children, className, span }: ResponsiveCardProps) {
  return (
    <div
      className={cn(
