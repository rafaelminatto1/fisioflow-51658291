...
export const AIReportGeneratorModal: React.FC<AIReportGeneratorModalProps> = ({
  ...
  const { isGenerating, generatedReport, setGeneratedReport, generateReport, error } =
  ...
  const handleExportPDF = async () => {
    ...
    try {
      setIsExporting(true);
      const { exportClinicalReportToPDF } = await import("@/lib/export/clinicalReportPdfExport");
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] p-0 overflow-hidden flex flex-col bg-white">
        <DialogHeader className="px-6 pt-6 pb-4 border-b bg-slate-50">
          ...
          <DialogDescription className="text-slate-500">
            A inteligência artificial analisou as evoluções do paciente e gerou este relatório. Faça
            ajustes manuais se necessário antes de exportar.
          </DialogDescription>
        </DialogHeader>
