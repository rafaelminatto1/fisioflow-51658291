...
export function PatientAnalytics({ totalPatients, classificationStats }: PatientAnalyticsProps) {
  ...
  const activePercentage =
    totalPatients > 0 ? ((classificationStats.active / totalPatients) * 100).toFixed(1) : "0";
  const riskPercentage =
    totalPatients > 0
      ? (
          ((classificationStats.noShowRisk + classificationStats.hasUnpaid) / totalPatients) *
          100
        ).toFixed(1)
      : "0";
  const newPatientsPercentage =
    totalPatients > 0 ? ((classificationStats.newPatients / totalPatients) * 100).toFixed(1) : "0";
...
function AnalyticsMiniCard({
  ...
  const colors = {
    emerald:
      "from-emerald-500/20 text-emerald-600 bg-emerald-500/10 border-emerald-500/20 shadow-emerald-500/5",
    orange:
      "from-orange-500/20 text-orange-600 bg-orange-500/10 border-orange-500/20 shadow-orange-500/5",
