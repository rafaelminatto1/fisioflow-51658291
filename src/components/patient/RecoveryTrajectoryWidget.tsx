...
export const RecoveryTrajectoryWidget: React.FC<RecoveryTrajectoryWidgetProps> = ({
  patientId,
}) => {
  const { data: twinResponse, isLoading } = useQuery({
    queryKey: ["patient-digital-twin", patientId],
    queryFn: () =>
      request<{ data: DigitalTwinData }>(`/api/clinic-metrics/patients/${patientId}/digital-twin`),
    enabled: !!patientId,
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-sm font-black uppercase tracking-widest">
              Trajetória Preditiva (Gêmeo Digital)
            </CardTitle>
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent className="max-w-xs bg-slate-900 text-white border-slate-800 p-4 rounded-xl shadow-2xl">
                  <p className="text-xs leading-relaxed">
                    A IA analisa a aderência, evolução da dor e dados de wearables para prever o
                    tempo de recuperação baseado em protocolos de evidência científica.
                  </p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          ...
          <div
            className={cn(
              ...
              data.ai_risk_level === "high"
                ? "bg-red-500"
                : data.ai_risk_level === "medium"
                  ? "bg-amber-500"
                  : "bg-emerald-500",
            )}
          >
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-1.5">
          <div className="flex justify-between items-center text-[10px] font-bold uppercase">
            ...
            <span className={adherence > 80 ? "text-emerald-400" : "text-amber-400"}>
              {adherence.toFixed(1)}%
            </span>
          </div>
