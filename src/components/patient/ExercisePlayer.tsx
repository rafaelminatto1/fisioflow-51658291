...
const ExerciseExecutionScreen = lazy(() =>
	import("@/components/exercises/ExerciseExecutionScreen").then((m) => ({
		default: m.ExerciseExecutionScreen,
	})),
);
...
export function ExercisePlayer({
	...
	const getMappedExerciseType = (name: string): ExerciseType => {
		...
		if (lowerName.includes("flexão") || lowerName.includes("pushup"))
			return ExerciseType.PUSHUP;
		if (lowerName.includes("levantamento lateral"))
			return ExerciseType.LATERAL_RAISE;
		if (lowerName.includes("prancha") || lowerName.includes("plank"))
			return ExerciseType.PLANK;
		if (lowerName.includes("avanço") || lowerName.includes("lunge"))
			return ExerciseType.LUNGE;
		if (
			lowerName.includes("desenvolvimento") ||
	...
	useEffect(() => {
		...
		if (isTimerRunning && timeLeft > 0) {
			interval = setInterval(() => {
				...
				if (currentExercise?.duration_seconds) {
					setProgress(
						((currentExercise.duration_seconds - timeLeft) /
		...
		return (
			<div className="flex flex-col items-center justify-center p-8 text-center h-[400px]">
				...
				<p className="text-muted-foreground mb-6">
					Não foi possível encontrar os dados deste exercício.
				</p>
	...
	const togglePlay = () => {
		if (videoRef.current) {
			if (isPlaying) {
				...
				if (currentExercise.duration_seconds && timeLeft > 0)
	...
	const handleAiSessionComplete = (session: any) => {
		...
		setDifficulty(
			Math.max(1, Math.min(5, Math.ceil((100 - session.totalScore) / 20))),
		);
		setAiMode(false);
			...
			<div className="flex items-center justify-between">
				...
				<div className="flex gap-2">
					<Button
						...
						className={cn(
							aiMode && "bg-gradient-to-r from-purple-600 to-blue-600 border-0",
						)}
					>
						{aiMode ? (
						...
						<Suspense
							fallback={
								<Card className="aspect-video flex flex-col items-center justify-center space-y-4 bg-muted/20">
									...
									<p className="text-muted-foreground">
										Carregando motor de Biofeedback...
									</p>
								</Card>
							}
						>
					...
					<div className="flex items-center justify-between gap-4 p-4 bg-muted/30 rounded-xl">
						...
						<div className="text-center">
							<p className="font-semibold text-lg">
								{currentExercise.sets} séries x {currentExercise.reps} reps
							</p>
						...
						<div>
							...
							<p className="text-sm text-muted-foreground leading-relaxed">
								{currentExercise.description ||
									"Siga as orientações do vídeo e mantenha a postura correta durante a execução."}
							</p>
						</div>
