...
export function ExercisePlayer({
  ...
  const getMappedExerciseType = (name: string): ExerciseType => {
    ...
    if (lowerName.includes("desenvolvimento") || lowerName.includes("shoulder press"))
  ...
  useEffect(() => {
    ...
    if (isTimerRunning && timeLeft > 0) {
      interval = setInterval(() => {
        ...
        if (currentExercise?.duration_seconds) {
          setProgress(
            ((currentExercise.duration_seconds - timeLeft) / currentExercise.duration_seconds) *
              100,
          );
        }
      }, 1000);
                ...
                <div className="w-full h-full flex items-center justify-center flex-col bg-muted/20">
                  ...
                  {currentExercise.duration_seconds && (
                    <div className="mt-8 text-center">
                      ...
                      <Button size="lg" className="rounded-full h-16 w-16" onClick={togglePlay}>
                        {isPlaying ? (
