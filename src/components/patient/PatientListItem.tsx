...
export const PatientListItem = ({ patient, onClick }: PatientListItemProps) => {
  ...
  const badgeEntries = [
    ...
    patient.financialStatus
      ? {
          label:
            PATIENT_FINANCIAL_STATUS_LABELS[patient.financialStatus] ?? patient.financialStatus,
    ...
    label: string;
    type: 'clinical' | 'financial' | 'operational' | 'neutral';
  }>;

  return (
    <Card
      className="group h-full rounded-2xl border-border bg-card p-5 shadow-none transition duration-200 hover:-translate-y-px hover:border-primary/30 hover:shadow-md dark:bg-slate-950/75"
          ...
          <div className="min-w-0 flex-1 space-y-3">
            ...
            <div className="grid gap-3 text-[12px] text-slate-600 dark:text-slate-300 md:grid-cols-2">
              <div className="flex items-center gap-2">
                ...
                <span>{patient.sessionsCompleted ?? 0} sessões concluídas</span>
              </div>
              <div className="flex items-center gap-2">
