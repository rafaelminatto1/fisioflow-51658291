      ...
      <div className="p-3">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {detail && (
              <p className={cn("text-sm mt-1", isExpanded ? "block" : "line-clamp-2")}>{detail}</p>
            )}
            ...
            {insight.metric_value !== undefined && (
              <div className="flex items-center gap-4 mt-2 text-xs">
                <span className="text-muted-foreground">
                  Atual: <strong>{insight.metric_value}</strong>
                </span>
                {insight.comparison_value !== undefined && (
                  <>
                    <span className="text-muted-foreground">vs.</span>
                    <span className="text-muted-foreground">
                      Comparativo: <strong>{insight.comparison_value}</strong>
                    </span>
                    <span
                      className={cn(
                        "font-medium",
    ...
    <div className="flex flex-wrap gap-2">
      {summaryItems.slice(0, 4).map(({ type, count, config }) => {
        ...
        return (
          <div
            key={type}
            className={cn("flex items-center gap-1 px-2 py-1 rounded-md text-xs", config.bgColor)}
          >
...
export const PatientInsightsPanel = memo(function PatientInsightsPanel({
  ...
  const acknowledgeAll = useCallback(async () => {
    ...
    for (const insight of insights) {
      await acknowledgeInsight.mutateAsync({
      ...
      insights
        ?.filter((insight) => {
          if (filter === "actionable") {
            return (
              insight.insight_type === "recommendation" || insight.insight_type === "risk_detected"
            );
          }
          if (filter === "alerts") {
            return (
              insight.insight_type === "risk_detected" || insight.insight_type === "trend_alert"
            );
          }
          return true;
        })
        ...
        {insights && insights.length > 0 && (
          <div className="flex gap-2" role="group" aria-label="Filtros de insights">
            ...
            >
              ...
              {
                insights.filter(
                  (i) => i.insight_type === "risk_detected" || i.insight_type === "trend_alert",
                ).length
              }
              )
            </Button>
