...
const InsightItem = memo(function InsightItem({
	...
	const config =
		INSIGHT_CONFIGS[insight.insight_type] || INSIGHT_CONFIGS.progress_summary;
...
export const PatientInsightsPanel = memo(function PatientInsightsPanel({
	...
	const {
		...
		isLoading,
		refetch,
	} = usePatientInsights(patientId, false);
			...
			insights
				?.filter((insight) => {
					...
					if (filter === "alerts") {
						return (
							insight.insight_type === "risk_detected" ||
							insight.insight_type === "trend_alert"
						);
					}
					return true;
				})
