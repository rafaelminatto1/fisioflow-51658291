      ...
      <div className="flex flex-col items-center">
        <div
          className={cn(
            ...
            isActive
              ? config.bgColor + " " + config.color + " border-current scale-110"
        ...
        {!isLast && (
          <div
            className={cn("w-0.5 flex-1 my-2", isActive ? "bg-current" : "bg-muted")}
            style={{ minHeight: "40px" }}
          />
        )}
      </div>
      <div className={cn("flex-1 pb-8 transition-all", isActive ? "opacity-100" : "opacity-60")}>
  ...
  return (
    <div className="grid grid-cols-3 gap-2">
      {stats.map((stat, index) => {
        ...
        return (
          <div key={index} className={cn("p-3 rounded-lg border text-center", stat.bgColor)}>
