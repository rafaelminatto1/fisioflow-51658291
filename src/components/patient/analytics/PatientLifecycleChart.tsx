	...
	const stats = [
		{
			label: "Estágio Atual",
			value:
				STAGE_CONFIGS[summary.current_stage]?.label || summary.current_stage,
		...
		<div className="grid grid-cols-3 gap-2">
			{stats.map((stat, index) => {
				...
				return (
					<div
						key={index}
						className={cn("p-3 rounded-lg border text-center", stat.bgColor)}
					>
...
function JourneyProgress({ summary }: JourneyProgressProps) {
	...
	const progressPercentage =
		((currentStageIndex + 1) / journeyStages.length) * 100;
...
export function PatientLifecycleChart({
	summary,
	isLoading,
}: PatientLifecycleChartProps) {
	if (isLoading) {
		return (
			<Card>
				<CardHeader>
	...
	if (
		...
		summary.stage_history.length === 0
	) {
		return (
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Histórico completo da jornada na clínica
					</CardDescription>
				</CardHeader>
