...
describe("AIAssistantPanel", () => {
	...
	it("should render the panel with patient context", () => {
		...
		expect(
			screen.getByText("Análise inteligente do paciente"),
		).toBeInTheDocument();
	});
	...
	it("should switch between chat and insights tabs", () => {
		...
		expect(
			screen.getByText("Análise Clínica Gerada por IA"),
		).toBeInTheDocument();
	});
	...
	it("should display assistant messages", () => {
		...
		expect(
			screen.getByText("Qual é o progresso do paciente?"),
		).toBeInTheDocument();
	});
	...
	it("should show loading indicator when chat is loading", () => {
		...
		const loadingDots = screen
			...
			.find((el) => el.classList.contains("animate-bounce"));
