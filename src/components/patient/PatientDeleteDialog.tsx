...
export const PatientDeleteDialog: React.FC<PatientDeleteDialogProps> = ({
  ...
  const isAdmin = user?.email === process.env.E2E_EMAIL || "" || profile?.role === "admin";
        ...
        <AlertDialogDescription className="py-4">
          {hardDelete ? (
            <div className="bg-destructive/10 p-3 rounded-lg border border-destructive/20 text-destructive mb-4">
              ...
              <strong>"{patientName}"</strong>, incluindo histórico clínico, evoluções e exames.
              Esta ação não é recomendada por razões legais.
            </div>
          ) : (
            <>
              ...
              <br />
              <br />
              O paciente será marcado como <strong>Arquivado</strong> e não aparecerá nas listagens
              ativas. Todos os dados, histórico clínico e evoluções serão{" "}
              <strong className="text-foreground">preservados</strong> conforme exigido pela
              legislação vigente (CFisio / LGPD).
            </>
          )}
        </AlertDialogDescription>
