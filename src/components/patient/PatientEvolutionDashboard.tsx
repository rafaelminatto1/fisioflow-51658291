...
export const PatientEvolutionDashboard = ({
  ...
  const improvementPercentage =
    initialPainLevel > 0 ? ((painReduction / initialPainLevel) * 100).toFixed(1) : 0;

  const stats = [
    {
      label: "Sessões Realizadas",
      value:
        prescribedSessions && prescribedSessions > 0
      ...
      progressPercent:
        prescribedSessions && prescribedSessions > 0
          ? Math.min(100, (totalSessions / prescribedSessions) * 100)
          : undefined,
    },
    {
      label: "Dor Atual",
    ...
    <div className="space-y-6">
                        ...
                        <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full transition-all duration-500 ${
                              stat.progressPercent >= 100 ? "bg-green-500" : "bg-blue-500"
                            }`}
      ...
      <Tabs defaultValue="pain" className="w-full">
        ...
        <TabsContent value="biomechanics" className="mt-4">
          <BiomechanicsEvolutionTab patientId="" patientName={patientName} />
        </TabsContent>
      </Tabs>
    </div>
  );
