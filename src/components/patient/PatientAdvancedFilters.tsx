...
const classificationOptions: PatientClassificationFilter[] = Object.values(
	PATIENT_CLASSIFICATIONS,
);
...
export function PatientAdvancedFilters({
	...
	const toggleClassification = (classification: PatientClassification) => {
		const newValue =
			filters.classification === classification ? "all" : classification;
		...
		<div className="space-y-4 py-2">
				...
				<div className="flex flex-wrap gap-1.5">
					{classificationOptions.map((option) => (
						<Button
							key={option.value}
							variant={
								filters.classification === option.value ? "default" : "outline"
							}
							size="sm"
							onClick={() =>
								toggleClassification(option.value as PatientClassification)
							}
				...
				<div className="grid grid-cols-1 gap-2">
					<div className="flex items-center gap-2">
						<Switch
							...
							onCheckedChange={(checked) =>
								updateFilter("hasUnpaid", checked || undefined)
							}
						/>
					...
					<div className="flex items-center gap-2">
						<Switch
							...
							onCheckedChange={(checked) =>
								updateFilter("hasNoShow", checked || undefined)
							}
						/>
					...
					<div className="flex items-center gap-2">
						<Switch
							...
							onCheckedChange={(checked) =>
								updateFilter("hasUpcoming", checked || undefined)
							}
						/>
			...
			<div className="space-y-3">
				...
				<Slider
					...
					onValueChange={([value]) =>
						updateFilter("daysInactive", value > 0 ? value : undefined)
					}
					min={0}
					max={90}
					step={7}
