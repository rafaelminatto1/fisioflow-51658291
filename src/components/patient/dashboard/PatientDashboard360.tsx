            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <Card className="border-blue-100 shadow-sm bg-white overflow-hidden group">
                    <CardContent className="p-4 flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">Saldo de Sessões</p>
                            <div className="flex items-baseline gap-1">
                                <span className={cn(
                                    "text-2xl font-bold",
                                    (patient.balance ?? patient.sessions_available ?? 0) <= 1 ? 'text-rose-600' : 'text-slate-900'
                                )}>
                                    {patient.balance ?? patient.sessions_available ?? 0}
                                </span>
                                <span className="text-xs text-slate-500 font-medium">sessões</span>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card className="border-blue-100 shadow-sm bg-white overflow-hidden group">
                    <CardContent className="p-4 flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">Próximo Agendamento</p>
                            <p className="text-sm font-bold text-slate-900 truncate">
                                {nextAppointment 
                                    ? format(parseResponseDate(nextAppointment.date || nextAppointment.appointment_date), "dd/MM 'às' HH:mm", { locale: ptBR })
                                    : 'Nenhum agendado'}
                            </p>
                        </div>
                    </CardContent>
                ...
                <Card className="border-blue-100 shadow-sm bg-white overflow-hidden group">
                    <CardContent className="p-4 flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">Condição Principal</p>
                            <p className="text-sm font-bold text-slate-900 truncate max-w-[150px]" title={patient.mainCondition}>
                ...
                <Card className="border-blue-100 shadow-sm bg-white overflow-hidden group">
                    <CardContent className="p-4 flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">Nível Gamificação</p>
                    ...
                    <CardContent className="p-6">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="font-bold text-slate-900">Resumo Inteligente (IA)</h3>
                                    <p className="text-xs text-slate-500 font-medium">Análise automática de tendências e histórico clínico</p>
                                </div>
                            </div>
                            <Button
                                size="sm"
                        ...
                        {insightResult ? (
                            ...
                            <div className="text-center py-8 px-4 bg-white/50 rounded-xl border border-dashed border-blue-200">
                                ...
                                <p className="text-sm text-slate-500 max-w-md mx-auto font-medium leading-relaxed">
                                    Ainda não há uma análise gerada. Clique no botão acima para que a IA processe o histórico do paciente.
                                </p>
                            </div>
                        )}
                    </CardContent>
                    ...
                    <Card className="bg-white border-blue-100 shadow-sm rounded-xl overflow-hidden">
                        <CardContent className="p-6">
                            ...
                            <Link to="/gamification" className="mt-4 block">
                                <Button variant="outline" size="sm" className="w-full text-blue-700 border-blue-200 hover:bg-blue-50 hover:text-blue-800 font-medium">
                    ...
                    <Card className="bg-white border-blue-100 shadow-sm rounded-xl overflow-hidden">
                        <CardContent className="p-6">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <Badge variant="secondary" className="bg-orange-50 text-orange-700 font-bold">
                    ...
                    <Card className="bg-white border-blue-100 shadow-sm rounded-xl overflow-hidden">
                        <CardContent className="p-6">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <Badge variant="secondary" className="bg-amber-50 text-amber-700 font-bold">
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`w-full aspect-square rounded-xl flex items-center justify-center transition-all ${
                                                isUnlocked ? 'bg-amber-100 text-amber-500 shadow-sm' : 'bg-slate-50 text-slate-300'
                                            }`}
                                        >
                                            <Star className="w-5 h-5" fill={isUnlocked ? "currentColor" : "none"} />
                                        </div>
                                    );
                                })}
                            </div>
                        ...
                        {resolvedActiveGoals.length > 0 ? (
                            <div className="space-y-3">
                                {resolvedActiveGoals.map((goal) => (
                                    <div key={goal.id} className="flex items-start justify-between gap-3 p-3 rounded-xl border border-blue-50 bg-blue-50/10 group hover:bg-blue-50/20 transition-colors">
                                        <div className="min-w-0">
                                            <p className="font-semibold text-sm text-slate-700">{goal.goal_title}</p>
                                            {goal.goal_description && (
                                                <p className="text-xs text-slate-500 mt-1 line-clamp-2">{goal.goal_description}</p>
                                            )}
                                            {goal.target_date && (
                        ...
                        {resolvedActivePathologies.length > 0 ? (
                            <div className="space-y-3">
                                {resolvedActivePathologies.map((pathology) => (
                                    <div key={pathology.id} className="flex items-start justify-between gap-3 p-3 rounded-xl border border-blue-50 bg-blue-50/10 group hover:bg-blue-50/20 transition-colors">
                                        <div className="min-w-0">
                                            <p className="font-semibold text-sm text-slate-700">{pathology.pathology_name}</p>
                                            <div className="flex items-center gap-1.5 mt-2 text-[10px] font-bold uppercase tracking-wider text-slate-400">
