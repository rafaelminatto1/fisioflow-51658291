...
export const PatientDashboard360 = ({
  ...
  // Gamification hook
  const { profile, unlockedAchievements, allAchievements, xpPerLevel } = useGamification(
    patientId || "",
  );

  // Memoized data processing
  const { nextAppointment, calculatedAge, patientName } = useMemo(() => {
          ...
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-slate-900 truncate">
                {nextAppointment
                  ? format(
                      parseResponseDate(nextAppointment.date || nextAppointment.appointment_date),
                      "dd/MM 'às' HH:mm",
                      { locale: ptBR },
                    )
                  : "Nenhum agendado"}
            ...
            {insightResult ? (
              ...
              <div className="text-center py-8 px-4 bg-white/50 rounded-xl border border-dashed border-blue-200">
                ...
                <p className="text-sm text-slate-500 max-w-md mx-auto font-medium leading-relaxed">
                  Ainda não há uma análise gerada. Clique no botão acima para que a IA processe o
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="bg-white border-blue-100 shadow-sm rounded-xl overflow-hidden">
            <CardContent className="p-6">
              ...
              <Progress
                value={
                  xpPerLevel > 0 ? (((profile.current_xp || 0) % xpPerLevel) / xpPerLevel) * 100 : 0
                }
          ...
          <CardContent className="p-4 space-y-3">
            {resolvedActiveGoals.length > 0 ? (
              ...
              <div className="text-center py-6 px-4 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                ...
                <p className="text-xs text-slate-500 font-medium">Nenhum objetivo ativo.</p>
                <Button
                  variant="link"
                  size="sm"
          ...
          <CardContent className="p-4 space-y-3">
            {resolvedActivePathologies.length > 0 ? (
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-2 text-[10px] font-bold uppercase tracking-wider text-slate-400">
                        ...
                        {pathology.diagnosis_date
                          ? format(new Date(pathology.diagnosis_date), "dd/MM/yyyy")
                          : "N/A"}
                      </div>
                    </div>
                    <Button
              ...
              <div className="text-center py-6 px-4 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                ...
                <p className="text-xs text-slate-500 font-medium">Nenhuma patologia registrada.</p>
                <Button
                  variant="link"
                  size="sm"
                  className="text-blue-600 font-bold text-[10px] uppercase mt-1 p-0 h-auto"
                  ...
                  <div className="mt-2 sm:mt-0 text-right">
                    ...
                    {surgery.surgeryDate && (
                      <p className="text-xs text-muted-foreground">
                        há{" "}
                        {Math.floor(
                          (new Date().getTime() - new Date(surgery.surgeryDate).getTime()) /
                            (1000 * 60 * 60 * 24 * 30),
                        )}{" "}
                        meses
                      </p>
