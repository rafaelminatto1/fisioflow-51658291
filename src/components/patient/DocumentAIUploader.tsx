  ...
  const onDrop = useCallback(
    (acceptedFiles: File[], rejectedFiles: FileRejection[]) => {
      ...
      rejectedFiles.forEach(
        ({ file, errors }: { file: File; errors: { code: string; message: string }[] }) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          ...
          toast.error(
            language === "pt-BR" ? `Erro ao analisar ${file.name}` : `Error analyzing ${file.name}`,
          );
        }
      }
      ...
      toast.error(
        language === "pt-BR" ? "Erro ao analisar documentos" : "Error analyzing documents",
      );
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium">
                  {language === "pt-BR" ? "Arquivos selecionados" : "Selected files"} (
                  {files.length})
                </p>
                <Button variant="ghost" size="sm" onClick={clearFiles} disabled={analyzing}>
                  {labels.clear}
                </Button>
              </div>
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          {results.some((r) => r.alerts.some((a) => a.level === "critical")) && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                {results
                  .flatMap((r) => r.alerts.filter((a) => a.level === "critical"))
                  .map((alert, i) => (
          ...
          {results.map((result, _resultIndex) => (
            <Card key={result.documentId} className="overflow-hidden">
              <CardHeader
                ...
                onClick={() =>
                  setExpandedResult(expandedResult === result.documentId ? null : result.documentId)
                }
              >
                <div className="flex items-center justify-between">
                    ...
                    <TabsContent value={labels.contraindications} className="space-y-3">
                      {result.contraindications.length === 0 ? (
                        ...
                        result.contraindications.map((contra, i) => (
                          <Alert
                            key={i}
                            variant={contra.type === "absolute" ? "destructive" : "default"}
                          >
