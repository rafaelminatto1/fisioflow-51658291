...
export function DocumentAIUploader({
	...
	const labels = {
		...
		contraindications:
			language === "pt-BR" ? "Contraindicações" : "Contraindications",
		...
		types: {
			medical_report:
				language === "pt-BR" ? "Relatório Médico" : "Medical Report",
			...
			discharge_summary:
				language === "pt-BR" ? "Sumário de Alta" : "Discharge Summary",
			surgery_report:
				language === "pt-BR" ? "Relatório Cirúrgico" : "Surgery Report",
			imaging: language === "pt-BR" ? "Imagem" : "Imaging",
			lab_results:
				language === "pt-BR" ? "Resultados de Laboratório" : "Lab Results",
	...
	const handleAnalyze = async () => {
		...
		try {
			const { analyzeMedicalDocument } = await import(
				"@/lib/ai/document-analysis"
			);
					...
					const result = await analyzeMedicalDocument(file, {
						...
						onProgress: (progress, stage) => {
							logger.debug(
								...
								{ fileName: file.name, stage, progress },
								"DocumentAIUploader",
							);
						},
					});
						...
						>
							{analyzing ? (
								<>
									...
									{labels.analyzing} ({analysisProgress.current}/
				...
				<div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
					...
					{results.some((r) =>
						...
						<Alert variant="destructive">
							...
							<AlertTitle>
								{language === "pt-BR"
									? "Atenção: Alertas Críticos"
									: "Warning: Critical Alerts"}
							</AlertTitle>
							<AlertDescription>
								{results
									.flatMap((r) =>
										r.alerts.filter((a) => a.level === "critical"),
									)
									.map((alert, i) => (
								...
								<div className="flex items-center justify-between">
									<div className="flex items-center gap-3">
										...
										<div>
											...
											<CardDescription>
												{labels.types[result.documentType]}
											</CardDescription>
										</div>
									</div>
									{expandedResult === result.documentId ? (
								...
								<CardContent className="pt-6">
									<Tabs defaultValue={labels.diagnoses} className="w-full">
										<TabsList className="grid w-full grid-cols-5">
											...
											<TabsTrigger value={labels.summary}>
												{labels.summary}
											</TabsTrigger>
										</TabsList>
										...
										{/* Contraindicações */}
										<TabsContent
											value={labels.contraindications}
											className="space-y-3"
										>
														...
														<AlertDescription>
															<div className="space-y-1">
																<div className="flex items-center gap-2">
																	<Badge
																		className={getSeverityColor(contra.type)}
																	>
																		{contra.type}
																	</Badge>
