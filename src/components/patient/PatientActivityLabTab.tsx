...
export const PatientActivityLabTab: React.FC<PatientActivityLabTabProps> = ({
	patientId,
}) => {
			...
			<Card className="border-dashed border-2 bg-slate-50/50">
				<CardContent className="flex flex-col items-center justify-center py-16 text-center">
					...
					<p className="text-slate-500 max-w-xs mx-auto mb-6">
						Este paciente ainda não possui testes de força registrados no
						Activity Lab.
					</p>
					<Button
						variant="outline"
						onClick={() => window.open("/ai/activity-lab", "_blank")}
					>
					...
					<Card className="h-full flex items-center justify-center border-dashed border-2 bg-slate-50/30">
						<div className="text-center p-12">
							...
							<p className="text-slate-500 max-w-xs mx-auto">
								Escolha um teste no histórico ao lado para analisar a curva de
								força e métricas biomecânicas.
							</p>
						</div>
					</Card>
				)}
