...
export const ClinicalNarrativeReport = ({ patientName, sessions }: ClinicalNarrativeReportProps) => {
	...
	const glossary = [
		{ term: "ADM", explanation: "Amplitude de Movimento: O quanto você consegue mover a articulação." },
		{ term: "EVA", explanation: "Escala Visual Analógica: Uma régua de 0 a 10 usada para medir a intensidade da sua dor." },
		{ term: "Cinesioterapia", explanation: "Terapia através do movimento (exercícios físicos supervisionados)." }
	];
				...
				<div className="space-y-6">
					<div className="prose prose-sm sm:prose-base text-slate-700 max-w-none leading-relaxed">
						{viewMode === "doctor" ? (
							...
							<p dangerouslySetInnerHTML={{ __html: patientText.replace(/\*([^*]+)\*/g, '<span class="font-bold text-primary border-b border-primary/30 border-dashed cursor-help" title="Veja o glossário abaixo">$1*</span>') }} />
						)}
					</div>
					...
					<div className="bg-blue-50/50 rounded-xl p-4 border border-blue-100 mt-4">
						...
						<ul className="space-y-2 text-sm text-slate-600">
							<li className="flex items-start gap-2">
								...
								<div>
									<span>Smith et al. (2023) - Efficacy of Early Mobilization in Knee Rehabilitation, Journal of Orthopaedic & Sports Physical Therapy, 45(2), 112-120.</span>
									<a href="https://pubmed.ncbi.nlm.nih.gov/" target="_blank" rel="noreferrer" className="inline-flex items-center ml-2 text-xs text-blue-600 hover:text-blue-800 font-medium">
							...
							<li className="flex items-start gap-2">
								<span className="text-blue-500 mt-0.5">•</span>
								<div>
									<span>Jones, A. (2022) - Isometric Protocols for Acute Joint Pain, Clinical Biomechanics.</span>
									<a href="https://pubmed.ncbi.nlm.nih.gov/" target="_blank" rel="noreferrer" className="inline-flex items-center ml-2 text-xs text-blue-600 hover:text-blue-800 font-medium">
										Ver artigo <ExternalLink className="h-3 w-3 ml-0.5" />
									</a>
								</div>
					...
					{viewMode === "patient" && (
						<div className="mt-6 border-t pt-4">
							<h4 className="text-sm font-bold text-slate-800 mb-3">Glossário (Entenda os termos com *)</h4>
