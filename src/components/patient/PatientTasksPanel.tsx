...
import { AlertTriangle, CheckCircle2, CircleDot, Plus, X } from "lucide-react";
...
export function PatientTasksPanel({ patientId }: PatientTasksPanelProps) {
  ...
  const overdueTasks = openTasks.filter(
    (t) => t.data_vencimento && isBefore(new Date(t.data_vencimento), today),
  );
      ...
      {openTasks.length > 0 && (
        <Card className="border-border/60 shadow-sm">
          ...
          <CardContent className="space-y-2">
            {openTasks.map((task) => {
              const overdue =
                task.data_vencimento && isBefore(new Date(task.data_vencimento), today);
              return (
                <div
                  key={task.id}
                  className={cn(
