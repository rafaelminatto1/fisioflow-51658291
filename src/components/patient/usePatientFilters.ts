...
import type { Patient } from "@/hooks/patients/usePatientCrud/index";
...
export const usePatientFilters = ({
	...
	const uniqueConditions = useMemo(() => {
		const conditions = [
			...new Set(patients.map((p) => p.main_condition).filter(Boolean)),
		];
	...
	const filteredPatients = useMemo(() => {
		return patients.filter((patient) => {
			...
			return (
				matchesSearch && matchesStatus && matchesCondition && matchesAdvanced
			);
		});
