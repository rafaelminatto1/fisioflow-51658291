...
export function PatientFinancialTab({ patientId, appointments }: PatientFinancialTabProps) {
  const {
        ...
        <Card className="bg-white border-emerald-100 shadow-sm rounded-xl overflow-hidden">
          <CardContent className="p-5 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-slate-900">R$ {totalPaid.toFixed(2)}</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white border-amber-100 shadow-sm rounded-xl overflow-hidden">
          <CardContent className="p-5 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-slate-900">R$ {totalPending.toFixed(2)}</p>
            </div>
          </CardContent>
        </Card>
              ...
              {patientTransactions.map((tx) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-semibold text-sm text-slate-800">{tx.descricao}</p>
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {tx.forma_pagamento
                          ? paymentMethodLabels[tx.forma_pagamento] || tx.forma_pagamento
                          : "Pendente"}
                        {tx.categoria && ` • ${categoryLabels[tx.categoria] || tx.categoria}`}
                      </p>
                      {tx.created_at && (
                    ...
                    <div className="text-right">
                      ...
                      <div className="flex items-center gap-1 mt-1">
                        {tx.status !== "cancelado" && (
                          <Select
                            value={tx.status}
                            onValueChange={(value) => handleStatusChange(tx, value)}
                          >
