...
import { Download, File as FileIcon, Files, Trash, FileSearch } from "lucide-react";
import {
  useDeleteDocument,
  useDownloadDocument,
  useUploadDocument,
...
export function PatientDocumentsTab({ patientId, documents, isLoading }: PatientDocumentsTabProps) {
  ...
  const handleScannerUpload = async () => {
    ...
    try {
      await uploadDocument.mutateAsync({
        patient_id: patientId,
        file: selectedFile,
        category: (scannedData.category as PatientDocument["category"]) || "laudo",
          ...
          {scannedData && (
            <div className="space-y-4">
              <div className="p-4 bg-blue-50 rounded-lg">
                <p className="text-sm font-medium text-blue-800 mb-2">Resumo IA</p>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => setSelectedCategory(v as PatientDocument["category"])}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="max-h-40 overflow-y-auto p-3 bg-muted rounded-lg">
                <p className="text-xs text-muted-foreground font-medium mb-2">Texto Extraído:</p>
              ...
              <CardContent className="p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4 w-full sm:w-auto overflow-hidden">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {doc.description && (
                      <p className="text-xs text-slate-500 mt-1 truncate" title={doc.description}>
                        {doc.description}
                      </p>
                    )}
                  </div>
