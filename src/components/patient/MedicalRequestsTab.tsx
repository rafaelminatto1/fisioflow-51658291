...
export const MedicalRequestsTab: React.FC<MedicalRequestsTabProps> = ({
	patientId,
}) => {
	const { requests, isLoading, addRequest, deleteRequest } =
		useMedicalRequests(patientId);
			...
			{requests.length === 0 ? (
				<Card className="bg-muted/30 border-dashed">
					<CardContent className="flex flex-col items-center justify-center py-12 text-center text-muted-foreground">
						...
						<p className="text-sm">
							Adicione fotos e detalhes dos pedidos médicos do paciente.
						</p>
					</CardContent>
				</Card>
			) : (
						...
						>
							<CardHeader className="pb-2">
								<div className="flex justify-between items-start">
									<div>
										<CardTitle className="text-base font-semibold truncate pr-6 text-primary">
											{req.doctor_name || "Médico não informado"}
										</CardTitle>
										<CardDescription className="flex items-center gap-1 mt-1 text-xs">
											<Calendar className="w-3 h-3" />
											{req.request_date &&
												format(new Date(req.request_date), "dd/MM/yyyy", {
													locale: ptBR,
												})}
										</CardDescription>
									</div>
