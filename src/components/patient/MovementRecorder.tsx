...
export function MovementRecorder({
	...
	const labels = {
		title:
			language === "pt-BR" ? "Gravação de Movimento" : "Movement Recording",
		description:
			language === "pt-BR"
		...
		startRecording:
			language === "pt-BR" ? "Iniciar Gravação" : "Start Recording",
		...
		cameraUnavailable:
			language === "pt-BR" ? "Câmera indisponível" : "Camera unavailable",
		grantPermission:
			language === "pt-BR"
		...
		analysisComplete:
			language === "pt-BR" ? "Análise Concluída!" : "Analysis Complete!",
		...
		expectedReps:
			language === "pt-BR" ? "Repetições esperadas" : "Expected reps",
		demoAvailable:
			language === "pt-BR" ? "Vídeo demo disponível" : "Demo video available",
	};
	...
	const handleAnalyze = async () => {
		...
		try {
			...
			const { analyzeExerciseForm } = await import(
				"@/lib/ai/movement-analysis"
			);

			const file = new File([recordedBlob], `exercise-${exerciseId}.webm`, {
				type: "video/webm",
			});
					...
					<Card>
						...
						<CardContent>
							<div className="grid grid-cols-2 md:grid-cols-3 gap-4">
								...
								<div className="space-y-1">
									<p className="text-sm text-muted-foreground">
										{language === "pt-BR" ? "Postura" : "Posture"}
									</p>
									<p className="text-2xl font-bold">
										{analysisResult.formQuality.posture}
									</p>
								</div>
								<div className="space-y-1">
									<p className="text-sm text-muted-foreground">
										{language === "pt-BR" ? "Amplitude" : "ROM"}
									</p>
									<p className="text-2xl font-bold">
										{analysisResult.formQuality.rangeOfMotion}
									</p>
								</div>
								<div className="space-y-1">
									<p className="text-sm text-muted-foreground">
										{language === "pt-BR" ? "Controle" : "Control"}
									</p>
									<p className="text-2xl font-bold">
										{analysisResult.formQuality.control}
									</p>
								</div>
								<div className="space-y-1">
									<p className="text-sm text-muted-foreground">
										{language === "pt-BR" ? "Tempo" : "Tempo"}
									</p>
									<p className="text-2xl font-bold">
										{analysisResult.formQuality.tempo}
									</p>
								</div>
								<div className="space-y-1">
									<p className="text-sm text-muted-foreground">
										{language === "pt-BR" ? "Respiração" : "Breathing"}
									</p>
									<p className="text-2xl font-bold">
										{analysisResult.formQuality.breathing}
									</p>
								</div>
							</div>
						</CardContent>
										...
										>
											...
											<div className="flex-1">
												...
												<p className="text-xs text-muted-foreground mt-1">
													{formatTime(deviation.timestamp)} -{" "}
													{deviation.bodyPart}
												</p>
											</div>
										</div>
