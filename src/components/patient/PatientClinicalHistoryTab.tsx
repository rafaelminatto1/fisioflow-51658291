				...
				{isLoadingEvaluations ? (
					...
					<div className="rounded-2xl border border-dashed bg-muted/20 p-6 text-center">
						...
						<p className="mt-1 text-xs text-muted-foreground">
							Use a biblioteca de templates ou inicie uma nova avaliação pelo
					...
					<div className="grid grid-cols-1 gap-3">
						{evaluations.map((evaluation) => {
							const status =
								statusConfig[evaluation.status] ?? statusConfig.in_progress;
								...
								>
									<div className="min-w-0 space-y-2">
										<div className="flex flex-wrap items-center gap-2">
											<Badge
												variant="outline"
												className={`${status.className} gap-1.5 rounded-lg`}
											>
									...
									<Button
										variant={
											evaluation.status === "completed" ? "outline" : "default"
										}
										size="sm"
