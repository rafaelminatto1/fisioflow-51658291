...
export const AssessmentComparison: React.FC<ComparisonProps> = ({
	records,
	onClose,
}) => {
			...
			<ScrollArea className="w-full whitespace-nowrap pb-4">
				<div className="flex w-max space-x-4">
					...
					<div className="w-[200px] shrink-0 space-y-4 pt-14">
						<div className="h-8 font-semibold text-sm flex items-center">
							Data
						</div>
						<div className="h-8 font-semibold text-sm flex items-center">
							Terapeuta
					...
					{sortedRecords.map((record, index) => {
						...
						return (
							<div
								key={record.id}
								className={`w-[350px] shrink-0 border rounded-lg p-4 space-y-4 ${isLatest ? "bg-primary/5 border-primary/20" : "bg-card"}`}
							>
								...
								<ScrollArea className="h-20 w-full rounded border p-2 bg-muted/30">
									<p className="text-sm whitespace-pre-wrap">
										{content.diagnosis || "Não informado"}
									</p>
								</ScrollArea>
								<Separator />
								<div className="pt-2"></div>
