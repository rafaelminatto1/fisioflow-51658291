...
export const SimilarPatientsWidget: React.FC<SimilarPatientsWidgetProps> = ({ patientId }) => {
  ...
  const { data: similarPatients = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await request<{ data: SimilarPatient[] }>(
        `/api/ai-clinical-search/patients/${patientId}/similar?limit=3`,
      );
      return res.data;
    },
    enabled: !!patientId,
  ...
  if (isLoading) {
    return (
      <div className="h-40 flex items-center justify-center text-xs text-muted-foreground animate-pulse">
        Buscando casos clínicos similares...
      </div>
    );
  }
            ...
            <Tooltip>
              ...
              <TooltipContent className="max-w-xs bg-slate-900 text-white border-slate-800 p-4 rounded-xl shadow-2xl">
                <p className="text-xs leading-relaxed">
                  Utilizamos busca vetorial de alta performance (Neon pgvector) para encontrar
                  correlações entre diagnósticos e evoluções de diferentes pacientes.
                </p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
      ...
      <CardContent className="space-y-3">
        {similarPatients.map((item) => (
          ...
          >
            ...
            <p className="text-[10px] text-slate-500 line-clamp-2 italic mb-2">"{item.summary}"</p>
