...
export function TarefaAnalytics({ tarefas }: TarefaAnalyticsProps) {
	...
	const stats = useMemo(() => {
		...
		tarefas.forEach((t) => {
			...
			if (t.responsavel_id) {
				porResponsavel[t.responsavel_id] =
					(porResponsavel[t.responsavel_id] || 0) + 1;
			}
	...
	const prioridadeData = Object.entries(stats.porPrioridade).map(
		([prioridade, count]) => ({
	...
	const evolucaoData = useMemo(() => {
		...
		return dias.map((dia) => {
			...
			const criadasNesseDia = tarefas.filter((t) =>
				t.created_at?.startsWith(diaStr),
			).length;
			return {
		...
		<div className="space-y-6">
			...
			<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
				...
				<Card>
					...
					<CardContent>
						<div className="text-2xl font-bold text-green-600">
							{stats.porStatus.CONCLUIDO}
						</div>
						<p className="text-xs text-muted-foreground mt-1">
							{stats.taxaConclusao.toFixed(1)}% do total
						</p>
					</CardContent>
				</Card>
				...
				<Card>
					...
					<CardContent>
						<div className="text-2xl font-bold text-yellow-600">
							{stats.porStatus.EM_PROGRESSO}
						</div>
						<p className="text-xs text-muted-foreground mt-1">
							{stats.porStatus.REVISAO} em revisão
						</p>
					</CardContent>
				</Card>
				...
				<Card>
					...
					<CardContent>
						<div
							className={`text-2xl font-bold ${stats.atrasadas > 0 ? "text-red-600" : "text-muted-foreground"}`}
						>
							{stats.atrasadas}
						</div>
						...
						<div className="flex items-start gap-2">
							...
							<div>
								<p className="font-medium text-green-700 dark:text-green-400">
									Ótima taxa de conclusão!
								</p>
								<p className="text-green-600/70 dark:text-green-400/70">
									Sua equipe está concluindo {stats.taxaConclusao.toFixed(0)}%
									das tarefas.
								</p>
							</div>
						</div>
						...
						<div className="flex items-start gap-2">
							...
							<div>
								<p className="font-medium text-red-700 dark:text-red-400">
									{stats.atrasadas} tarefas atrasadas
								</p>
