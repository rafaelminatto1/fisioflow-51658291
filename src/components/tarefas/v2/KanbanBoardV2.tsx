...
const DEFAULT_STATUSES: TarefaStatus[] = [
  "BACKLOG",
  "A_FAZER",
  "EM_PROGRESSO",
  "REVISAO",
...
export function KanbanBoardV2({
  ...
  const groupedTarefas = useMemo(() => {
    ...
    const filtered = tarefas.filter((t) => {
      ...
      if (filters.search) {
        ...
        const checklistText =
          t.checklists?.flatMap((c) => c.items?.map((i) => i.text?.toLowerCase() ?? "") ?? []) ??
          [];
        const matchesSearch =
      ...
      if (
        ...
        !filters.prioridade.includes(t.prioridade)
      ) {
        return false;
      }
    ...
    // Group by status
    const getOrderIndex = (value: unknown) =>
  ...
  const handleDragEnd = useCallback(
    async (result: DropResult) => {
      ...
      const destTasks =
        sourceStatus === destStatus ? sourceTasks : [...(groupedTarefas[destStatus] || [])];
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto flex-wrap">
                ...
                {(Object.keys(TIPO_LABELS) as TarefaTipo[]).map((t) => (
                  <DropdownMenuCheckboxItem
                    ...
                    onCheckedChange={(checked) => {
                      setFilters((f) => ({
                        ...f,
                        tipo: checked ? [...(f.tipo || []), t] : f.tipo?.filter((x) => x !== t),
                      }));
                    }}
                  >
                    {TIPO_LABELS[t]}
            ...
            <Button
              ...
              className={cn(
                ...
                filters.pendingAcknowledgment &&
                  "bg-orange-100 text-orange-700 border-orange-200 hover:bg-orange-200",
              )}
              onClick={() =>
                setFilters((f) => ({
                  ...f,
                  pendingAcknowledgment: !f.pendingAcknowledgment,
                }))
              }
            >
              <ShieldAlert
                className={cn(
                  "h-4 w-4",
                  filters.pendingAcknowledgment ? "text-orange-600" : "text-slate-400",
                )}
              />
            ...
            >
              <CheckSquare
                className={cn(
                  "h-4 w-4",
                  filters.has_pending_checklist ? "text-blue-600" : "text-slate-400",
                )}
              />
      ...
      <DragDropContext onDragEnd={handleDragEnd}>
        <div className="flex gap-6 overflow-x-auto pb-4">
          {statuses.map((status) => (
            <Suspense
              key={status}
              fallback={<LoadingSkeleton type="card" className="w-[320px] h-[500px]" />}
            >
              <LazyKanbanColumnV2
                status={status}
