	...
	const groupedTarefas = useMemo(() => {
		...
		const filtered = tarefas.filter((t) => {
			...
			if (filters.is_overdue && t.data_vencimento) {
				const isOverdue =
					new Date(t.data_vencimento) < new Date() && t.status !== "CONCLUIDO";
		...
		return statuses.reduce(
			(acc, status) => {
				acc[status] = filtered
					...
					.sort(
						(a, b) =>
							getOrderIndex(a.order_index) - getOrderIndex(b.order_index),
					);
				return acc;
			},
				...
				<div className="flex gap-6 overflow-x-auto pb-4">
					{statuses.map((status) => (
						<Suspense
							key={status}
							fallback={
								<LoadingSkeleton type="card" className="w-[320px] h-[500px]" />
							}
						>
							<LazyKanbanColumnV2
								status={status}
