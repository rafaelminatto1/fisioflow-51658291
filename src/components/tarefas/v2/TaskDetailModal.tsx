...
const tarefaDetailSchema = z.object({
  ...
  status: z.enum(["BACKLOG", "A_FAZER", "EM_PROGRESSO", "REVISAO", "CONCLUIDO", "ARQUIVADO"]),
  prioridade: z.enum(["BAIXA", "MEDIA", "ALTA", "URGENTE"]),
  tipo: z.enum(["TAREFA", "BUG", "FEATURE", "MELHORIA", "DOCUMENTACAO", "REUNIAO"]),
...
export function TaskDetailModal({ open, onOpenChange, tarefa, teamMembers }: TaskDetailModalProps) {
  ...
  const useTemplateMutation = useMutation({
    mutationFn: (templateId: string) => boardChecklistTemplatesApi.use(templateId),
  });
  ...
  const onSubmitRef = useRef<(data: TarefaDetailFormData) => Promise<void>>(async () => {});
                    ...
                    {(tarefa.linked_entity_type || tarefa.linked_entity_id) && (
                      <div>
                        ...
                        <div className="mt-1.5 flex items-center gap-2 rounded-xl border border-border/60 bg-slate-50/60 px-3 py-2">
                          ...
                          {!["patient", "appointment", "session", "goal", "exercise_plan"].includes(
                            tarefa.linked_entity_type ?? "",
                          ) && <ClipboardList className="h-4 w-4 text-slate-400 shrink-0" />}
                          <span className="text-sm font-medium capitalize">
                            {tarefa.linked_entity_type?.replace(/_/g, " ") ?? "Entidade"}
                          </span>
                                    ...
                                    <div className="flex-1 min-w-0">
                                      ...
                                      {tpl.items.length > 0 && (
                                        <p className="text-xs text-muted-foreground">
                                          ...
                                          {tpl.usage_count > 0 && ` · usado ${tpl.usage_count}x`}
                                        </p>
                                      )}
                                    </div>
                                  </button>
                          ...
                          {checklistFields.length > 0 && (
                            <Popover
                              ...
                              onOpenChange={(o) => {
                                ...
                                if (o) setSaveTemplateName(checklistFields[0]?.title ?? "");
                              }}
                            >
                              <PopoverTrigger asChild>
                                <Button
                              ...
                              <PopoverContent className="w-64 p-3" align="start">
                                <p className="text-xs font-medium mb-2">Nome do template</p>
                                <Input
                                  ...
                                  onKeyDown={(e) => {
                                    if (e.key === "Enter" && saveTemplateName.trim()) {
                                      const items = checklistFields.flatMap((cl) =>
                                        cl.items.map((item) => ({
                                          text: item.text,
                                        })),
                                      );
                                ...
                                <div className="flex gap-2">
                                  ...
                                  <Button
                                    ...
                                    className="flex-1 h-7 text-xs"
                                    disabled={
                                      !saveTemplateName.trim() || saveTemplateMutation.isPending
                                    }
                                    onClick={() => {
                                      const items = checklistFields.flatMap((cl) =>
                                        cl.items.map((item) => ({
                              ...
                              >
                                <Checkbox
                                  ...
                                  onCheckedChange={(checked) => {
                                    updateChecklistItem(checklistIndex, itemIndex, {
                                      completed: !!checked,
                                    });
                                    handleAutoSave();
                                  }}
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex-1 min-w-0 space-y-1">
                                <Input
                                  ...
                                  onChange={(e) =>
                                    form.setValue(`attachments.${index}.name`, e.target.value)
                                  }
                                ...
                                <Input
                                  ...
                                  onChange={(e) =>
                                    form.setValue(`attachments.${index}.url`, e.target.value)
                                  }
                        ...
                        >
                          <CardContent className="p-4 space-y-4">
                            <div className="flex items-center justify-between">
                              <Select
                                onValueChange={(v) => {
                                  form.setValue(`references.${index}.type`, v as any);
                                  handleAutoSave();
                                }}
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                form.setValue(`references.${index}.title`, e.target.value)
                              }
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              <Input
                                ...
                                onChange={(e) =>
                                  form.setValue(`references.${index}.author`, e.target.value)
                                }
                                placeholder="Autor..."
                              ...
                              <Input
                                ...
                                onChange={(e) =>
                                  form.setValue(`references.${index}.year`, e.target.value)
                                }
                                placeholder="Ano..."
                                className="rounded-xl h-9 text-xs border-slate-200"
                                onBlur={handleAutoSave}
                      ...
                      {tarefa?.acknowledgments && tarefa.acknowledgments.length > 0 ? (
                                ...
                                <div className="flex flex-col gap-1 mt-1">
                                  ...
                                  {ack.acknowledged_at && (
                                    <span className="text-[10px] text-slate-500 flex items-center gap-1 font-semibold">
                                      <CheckCircle2 className="h-3 w-3 text-green-500" /> Aceito em{" "}
                        ...
                        <div className="flex flex-col items-center justify-center py-12 px-6 text-center bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
                          ...
                          <p className="text-[11px] text-slate-400 mt-1 max-w-[200px]">
                            Ative o "Requer Aceite" para rastrear quem leu e confirmou esta tarefa.
                          </p>
                        </div>
                      )}
                    </div>
