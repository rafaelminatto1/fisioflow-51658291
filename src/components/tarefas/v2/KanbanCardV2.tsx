...
const KanbanCardContent = memo(function KanbanCardContent({
  ...
  const initials = useMemo(() => {
    if (tarefa.assignees && tarefa.assignees.length > 0) {
      return tarefa.assignees
        .slice(0, 3)
        .map((assignee) => assignee.full_name?.slice(0, 2).toUpperCase());
  ...
  const checklistProgress = useMemo(() => {
    ...
    const completedItems = tarefa.checklists.reduce(
  ...
  const commentCount = useMemo(
    () => tarefa.comment_count || tarefa.comments?.length || 0,
    [tarefa.comment_count, tarefa.comments],
  );
    ...
    <div className={cn(isGhost && "pointer-events-none")}>
      ...
      {tarefa.cover_image && (
        <div
          ...
          style={{ backgroundImage: `url(${tarefa.cover_image})` }}
        />
      )}

      {/* Card Color Indicator */}
      {tarefa.color && <div className={cn("h-1.5 w-full", tarefa.color)} />}
      ...
      <div className="p-3">
        ...
        {tarefa.descricao && (
          <p className="text-xs text-muted-foreground line-clamp-2 mb-2">{tarefa.descricao}</p>
        )}
        ...
        {((tarefa.label_ids && tarefa.label_ids.length > 0) ||
          ...
          <div className="flex flex-wrap gap-1 mb-2">
            ...
            {/* Overflow counter */}
            {(() => {
              const total = (tarefa.label_ids?.length ?? 0) + (tarefa.tags?.length ?? 0);
            ...
            {tarefa.dependencies &&
              ...
              tarefa.status !== "CONCLUIDO" && (
                <Tooltip>
                  ...
                  <TooltipContent>
                    {tarefa.dependencies.length} dependência
                    {tarefa.dependencies.length > 1 ? "s" : ""} pendente
                    {tarefa.dependencies.length > 1 ? "s" : ""}
                  </TooltipContent>
                </Tooltip>
              )}
...
export const KanbanCardV2 = memo(function KanbanCardV2({
  tarefa,
  index,
  onEdit,
  onDelete,
