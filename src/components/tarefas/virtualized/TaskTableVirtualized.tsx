...
const TaskRow: React.FC<TaskRowProps> = ({ index, style, data }) => {
  ...
  const isOverdue = isDateValid && isPast(dueDate) && task.status !== "CONCLUIDO";
  ...
  const priorityConfig = PRIORIDADE_COLORS[task.prioridade] || {
    bg: "bg-gray-500/20",
    text: "text-gray-500",
  };
  ...
  return (
    ...
    >
      <TableCell onClick={(e) => e.stopPropagation()}>
        <Checkbox checked={isSelected} onCheckedChange={() => toggleTaskSelection(task.id)} />
      </TableCell>
      <TableCell onClick={(e) => e.stopPropagation()}>
