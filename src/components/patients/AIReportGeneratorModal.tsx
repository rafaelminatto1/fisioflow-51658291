...
export const AIReportGeneratorModal: React.FC<AIReportGeneratorModalProps> = ({
  ...
  const { isGenerating, generatedReport, setGeneratedReport, generateReport, error } = useAIReportGenerator();
  ...
  const handleExportPDF = async () => {
    ...
    try {
      setIsExporting(true);
      await exportClinicalReportToPDF(patientName, generatedReport, reportInput);
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] p-0 overflow-hidden flex flex-col bg-white">
        <DialogHeader className="px-6 pt-6 pb-4 border-b bg-slate-50">
          ...
          <DialogDescription className="text-slate-500">
            A inteligência artificial analisou as evoluções do paciente e gerou este relatório. Faça ajustes manuais se necessário antes de exportar.
          </DialogDescription>
        </DialogHeader>
          ...
          {isGenerating ? (
            <div className="flex flex-col items-center justify-center py-20 flex-1">
              ...
              <p className="text-slate-500 text-center max-w-sm mt-2 text-sm leading-relaxed">
                Analisando as queixas, condutas e evolução funcional a partir das sessões documentadas do paciente.
              </p>
            </div>
          ) : error ? (
            <div className="flex flex-col items-center justify-center py-10 flex-1 text-center">
              <div className="bg-red-50 text-red-600 p-4 rounded-lg max-w-lg border border-red-100">
                <p className="font-medium text-red-800">Erro ao gerar o relatório</p>
                <p className="text-sm mt-1">{error}</p>
              </div>
              <Button onClick={() => reportInput && generateReport(reportInput)} className="mt-6" variant="outline">
                Tentar Novamente
              </Button>
            </div>
          ) : generatedReport ? (
            <div className="flex flex-col flex-1 h-full gap-4">
              <div className="flex items-center justify-between mb-1">
                 <span className="text-sm font-semibold text-slate-700">Edição do Relatório</span>
                 <Button variant="ghost" size="sm" onClick={handleRegenerate} className="h-8 gap-2 text-slate-500 hover:text-purple-700">
              ...
              <div className="flex items-center justify-between pt-2">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Button disabled={isExporting} onClick={handleExportPDF} className="gap-2 bg-purple-600 hover:bg-purple-700 text-white shadow shadow-purple-200">
