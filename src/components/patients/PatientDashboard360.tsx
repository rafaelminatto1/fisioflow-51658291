...
export const PatientDashboard360: React.FC<PatientDashboard360Props> = ({
	...
	const activePathologies = pathologies.filter(
									...
									<div key={surgery.id} className="text-sm">
										...
										<p className="text-xs text-muted-foreground">
											...
											{surgery.affected_side &&
												surgery.affected_side !== "nao_aplicavel" && (
													<>
														{" "}
														-{" "}
								...
								{upcomingAppointments.slice(0, 2).map((apt) => (
									<div key={apt.id} className="text-sm">
										<p className="font-medium">
											{format(
												parseResponseDate(apt.appointment_date),
												"dd/MM/yyyy",
											)}
										</p>
										<p className="text-xs text-muted-foreground">
			...
			{(activeGoals.length === 0 || soapRecords.length === 0) && (
				<Alert>
					...
					<AlertDescription>
						...
						{activeGoals.length === 0 &&
							"Configure objetivos para este paciente. "}
						{soapRecords.length === 0 &&
							"Registre a primeira evolução para iniciar o acompanhamento."}
					</AlertDescription>
				</Alert>
			)}
		</div>
