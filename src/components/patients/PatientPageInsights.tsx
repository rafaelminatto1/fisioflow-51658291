...
export function PatientPageInsights({
	totalPatients,
	classificationStats,
}: PatientPageInsightsProps) {
	const [isExpanded, setIsExpanded] = useState(false);
	const { inactive30, inactive60, noShowRisk, hasUnpaid, newPatients, active } =
		classificationStats;

	const activePercentage =
		totalPatients > 0 ? (active / totalPatients) * 100 : 0;
	...
	if (noShowRisk > 0) {
		insights.push({
			...
			type: "warning",
			bgClass:
				"bg-orange-50 text-orange-700 border-orange-100 dark:bg-orange-950/30 dark:text-orange-300 dark:border-orange-900/50",
	...
	if (hasUnpaid > 0) {
		insights.push({
			...
			type: "warning",
			bgClass:
				"bg-yellow-50 text-yellow-700 border-yellow-100 dark:bg-yellow-950/30 dark:text-yellow-300 dark:border-yellow-900/50",
							...
							<div className="flex -space-x-1">
								{insights.slice(0, 3).map((insight, i) => (
									<div
										key={i}
										className={cn(
											"flex items-center justify-center h-6 w-6 rounded-full border-2 border-background ring-1 ring-primary/5",
											insight.bgClass.split(" ")[0],
										)}
									>
