...
import {
	usePatientAnalyticsDashboard,
	useUpdatePatientRiskScore,
} from "@/hooks/usePatientAnalytics";
import {
	LineChart,
	Line,
	AreaChart,
...
function InsightCard({ insight, onAcknowledge }: InsightCardProps) {
	...
	const config =
		insightConfig[insight.insight_type] || insightConfig.progress_summary;
	...
	return (
		<Card
			className={cn(
				"border-l-4",
				insight.insight_type === "risk_detected"
					? "border-l-red-500"
					: "border-l-primary",
			)}
		>
...
export function PatientAnalyticsDashboard({
	...
	const { data, isLoading, isFetching, isError, isEmpty, refetch } =
		usePatientAnalyticsDashboard(patientId);
	...
	const radarData = useMemo(
		() =>
			data?.risk_score
				? [
						{
							factor: "Risco de Abandono",
							value: data.risk_score.dropout_risk_score,
						},
						{
							factor: "Risco de No-Show",
							value: data.risk_score.no_show_risk_score,
						},
						{
							factor: "Risco de Desfecho",
							value: data.risk_score.poor_outcome_risk_score,
						},
						{
							factor: "Adesão",
							value:
								100 -
								(data.risk_score.risk_factors?.attendance_rate
			...
			<Card>
				<CardContent className="p-8 text-center">
					...
					<p className="font-medium">
						Ainda não há dados de analytics para este paciente
					</p>
					<p className="text-sm text-muted-foreground mt-1">
						Os indicadores aparecerão após registrar sessões, objetivos,
						avaliações ou insights.
					</p>
					<Button variant="outline" onClick={() => refetch()} className="mt-4">
						Atualizar dados
					</Button>
		...
		<div className="space-y-6">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
				...
				<MetricCard
					...
					icon={Activity}
					variant={
						progress_summary.total_pain_reduction > 0 ? "success" : "warning"
					}
				...
				<MetricCard
					...
					variant={
						...
						progress_summary.overall_progress_percentage > 50
							? "success"
							: "default"
					}
					trend={
					...
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Painel de progresso do paciente
								</CardDescription>
							</CardHeader>
							<CardContent>
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Jornada do paciente na clínica
								</CardDescription>
							</CardHeader>
							<CardContent>
								<ScrollArea className="h-[250px]">
									<div className="space-y-2 pr-4">
										...
										lifecycle.stage_history.length === 0 ? (
											...
											lifecycle.stage_history.map((stage, index) => (
												<LifecycleStage
													...
													isActive={
														index === lifecycle.stage_history.length - 1
													}
												/>
											))
										)}
							...
							<Card>
								<CardHeader>
									<CardTitle className="flex items-center justify-between">
										...
										<Badge
											variant={
												pain_trend.trend === "improving"
													? "default"
													: pain_trend.trend === "declining"
														? "destructive"
														: "secondary"
											}
										>
									...
									<CardDescription>
										{pain_trend.change > 0 ? "+" : ""}
										{pain_trend.change_percentage}% desde o início
									</CardDescription>
								</CardHeader>
								<CardContent>
							...
							<Card>
								<CardHeader>
									<CardTitle className="flex items-center justify-between">
										...
										<Badge
											variant={
												function_trend.trend === "improving"
													? "default"
													: function_trend.trend === "declining"
														? "destructive"
														: "secondary"
											}
										>
									...
									<CardDescription>
										{function_trend.change > 0 ? "+" : ""}
										{function_trend.change_percentage}% desde o início
									</CardDescription>
								</CardHeader>
								<CardContent>
									<ResponsiveContainer width="100%" height={200}>
				...
				<TabsContent value="predictions" className="space-y-4">
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
						<MetricCard
							...
							icon={Target}
							variant={
								predictions.success_probability > 70
									? "success"
									: predictions.success_probability > 50
										? "warning"
										: "danger"
							}
						/>
						<MetricCard
							...
							icon={AlertTriangle}
							variant={
								predictions.dropout_probability < 20
									? "success"
									: predictions.dropout_probability < 50
										? "warning"
										: "danger"
							}
						/>
								...
								<div className="flex items-center gap-4">
									...
									<div className="flex-1">
										...
										<p className="text-xs text-purple-700 dark:text-purple-300 mt-1">
											...
											{Math.round(
												predictions.predicted_recovery_confidence * 100,
											)}
											%
										</p>
									</div>
					...
					{risk_score ? (
						<>
								...
								<Card>
									...
									<CardContent>
										<div className="flex items-center justify-center gap-8">
											<RiskGauge
												score={risk_score.overall_risk_score}
												label="Risco Geral"
											/>
											<div className="space-y-3">
												...
												<Button
													variant="outline"
													size="sm"
													onClick={handleRefreshRiskScore}
													className="w-full gap-2"
												>
													<RefreshCw
														className={cn(
															"h-3 w-3",
															updateRiskScore.isPending && "animate-spin",
														)}
													/>
													Atualizar
												</Button>
							...
							<Card>
								...
								<CardContent>
									<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
										{Object.entries(risk_score.risk_factors || {}).map(
											([key, value]) => (
												<div
													key={key}
							...
							{risk_score.recommended_actions &&
								risk_score.recommended_actions.length > 0 && (
									<Card>
										<CardHeader>
					...
					{goals.length > 0 ? (
						...
						<Card>
							<CardContent className="p-8 text-center">
								...
								<p className="text-muted-foreground">
									Nenhum objetivo definido para este paciente
								</p>
							</CardContent>
						</Card>
					)}
