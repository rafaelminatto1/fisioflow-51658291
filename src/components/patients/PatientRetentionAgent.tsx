...
export const PatientRetentionAgent: React.FC<PatientRetentionAgentProps> = ({ patientId, patientName }) => {
  ...
  const fetchAgentStatus = async () => {
    try {
      ...
      const res = await fetch(`https://fisioflow-api.rafalegollas.workers.dev/agents/patient-agent/${patientId}`);
      if (res.ok) {
  ...
  const handleManualUpdate = async () => {
    ...
    try {
      ...
      const res = await fetch(`https://fisioflow-api.rafalegollas.workers.dev/agents/patient-agent/${patientId}/updateClinicalStatus`, {
        method: 'POST',
  ...
  const handleDismiss = async () => {
    try {
      await fetch(`https://fisioflow-api.rafalegollas.workers.dev/agents/patient-agent/${patientId}/dismissAction`, { method: 'POST' });
  ...
  const riskColor = (state?.riskScore || 0) > 70 ? "text-red-500" : (state?.riskScore || 0) > 40 ? "text-orange-500" : "text-emerald-500";
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <CardDescription className="text-[10px] uppercase font-bold tracking-widest text-indigo-600/60">Cloudflare Autonomous Agent</CardDescription>
            </div>
          </div>
          <Badge variant="outline" className={cn("text-[10px] font-black uppercase tracking-tighter px-2 py-0", riskColor)}>
      ...
      <CardContent className="pt-4 space-y-4">
        ...
        <div className="space-y-1.5">
          <div className="flex justify-between items-end">
            <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Risco de Abandono</span>
            <span className={cn("text-lg font-black tracking-tighter", riskColor)}>{state?.riskScore || 0}%</span>
          </div>
        ...
        {state?.draftMessage ? (
          <div className="p-3 rounded-xl bg-white/80 dark:bg-black/40 border border-indigo-500/20 animate-in zoom-in-95 duration-300">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[11px] font-bold text-indigo-600 uppercase">Sugestão de Recuperação</span>
            </div>
            <p className="text-xs text-muted-foreground leading-relaxed italic mb-3">
              "{state.draftMessage}"
            </p>
            <div className="flex gap-2">
              <Button size="sm" className="flex-1 h-8 text-[10px] font-black uppercase tracking-widest bg-emerald-600 hover:bg-emerald-700">
                <Send className="h-3 w-3 mr-1.5" /> Enviar WhatsApp
              </Button>
              <Button size="sm" variant="ghost" onClick={handleDismiss} className="h-8 text-[10px] font-black uppercase tracking-widest text-muted-foreground">
          ...
          <div className="p-3 rounded-xl border border-dashed border-border/60 text-center py-6">
            ...
            <p className="text-[11px] font-bold text-muted-foreground uppercase">Paciente Engajado</p>
            <p className="text-[10px] text-muted-foreground/60">O Agente não detectou riscos imediatos.</p>
          </div>
        )}
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          >
            <RefreshCcw className={cn("h-3.5 w-3.5", isUpdating && "animate-spin")} />
          </Button>
        </div>
      </CardContent>
    </Card>
