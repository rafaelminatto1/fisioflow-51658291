...
export function TouchButton({
	...
	const variantStyles = {
		primary:
			"bg-gradient-primary text-primary-foreground hover:shadow-medical active:scale-95",
		secondary:
		...
		(e: React.TouchEvent) => {
			...
			if (progress > 0) {
				const index = Math.min(
					Math.floor(progress / (1 / actions.length)),
					actions.length - 1,
				);
				...
				{actions.map((action, index) => (
					<div
						...
						style={{
							...
							width:
								activeActionIndex === index ? `${swipeProgress * 100}%` : "0px",
