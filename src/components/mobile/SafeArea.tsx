...
export function SafeAreaView({
  ...
  return <SafeArea className={cn("min-h-screen bg-gray-50", className)}>{children}</SafeArea>;
}
  ...
  return (
    <header className={cn("bg-white border-b border-gray-200", className)}>
      <SafeArea top bottom={false} left={false} right={false}>
        <div className="flex items-center justify-between h-14 px-4">{children}</div>
      </SafeArea>
    </header>
  );
}
