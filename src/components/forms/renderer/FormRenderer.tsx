...
export const FormRenderer: React.FC<FormRendererProps> = ({
	...
	const handleGenerateSuggestions = async (fieldId: string) => {
		...
		try {
			...
			toast({
				...
				description:
					"O campo foi preenchido com sugestões baseadas na avaliação.",
			});
		} catch {
			toast({
				title: "Erro na IA",
	...
	const renderField = (field: EvaluationFormField) => {
		...
		const isAISuggestionField =
			field.label.includes("Sugestões de Conduta") ||
			field.label.includes("IA/Protocolos");
			...
			case "opcao_unica": // Radio
				return (
					<RadioGroup
						value={value || ""}
						onValueChange={(val) =>
							!readOnly && handleInputChange(field.id, val)
						}
						disabled={readOnly}
					>
								...
								<Checkbox
									...
									onCheckedChange={(checked) => {
										...
										if (checked) {
											...
											handleInputChange(
												field.id,
												currentSelection.filter((v: string) => v !== opt),
											);
										}
									}}
								/>
			...
			case "escala":
				return (
					<RadioGroup
						...
						onValueChange={(val) =>
							!readOnly && handleInputChange(field.id, parseInt(val))
						}
						disabled={readOnly}
						...
						{[1, 2, 3, 4, 5].map((num) => (
							<div key={num} className="flex flex-col items-center gap-1">
								<RadioGroupItem
									...
									className="h-6 w-6"
								/>
								<Label htmlFor={`${field.id}-${num}`} className="text-xs">
									{num}
								</Label>
							</div>
						))}
						...
						{!readOnly && (
							<PopoverContent className="w-auto p-0">
								<Calendar
									...
									onSelect={(date) =>
										handleInputChange(field.id, date?.toISOString())
									}
									initialFocus
								/>
							</PopoverContent>
			...
			default:
				return (
					<p className="text-sm text-muted-foreground">
						Tipo de campo não suportado: {field.tipo_campo}
					</p>
				);
		}
	};
