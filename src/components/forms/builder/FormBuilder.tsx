...
export const FormBuilder: React.FC<FormBuilderProps> = ({
	...
	const [nome, setNome] = useState(
		initialData?.nome || "Nova Ficha de Avaliação",
	);
	...
	const handleSave = async () => {
		...
		if (fields.length === 0) {
			...
			toast({
				title: "Erro",
				description: "Adicione pelo menos um campo à ficha.",
				variant: "destructive",
			});
			return;
		}
		...
		try {
			...
			const existingFieldIds = new Set(
				(initialData?.fields ?? []).map((field) => field.id),
			);
	...
	const icons: Record<ClinicalFieldType, React.ReactNode> = {
		...
		lista: <List size={16} />,
		escala: (
			<div className="text-[10px] font-bold border rounded px-1">1-5</div>
		),
					...
					<div className="space-y-4 pb-10">
									...
									>
										<CardContent className="p-4 flex items-start gap-3">
											...
											<div className="flex-1 space-y-2">
												...
												{(field.tipo_campo === "opcao_unica" ||
													field.tipo_campo === "selecao" ||
													field.tipo_campo === "lista") &&
													field.opcoes && (
														<div className="flex flex-wrap gap-2 mt-2">
															{field.opcoes.map((opt, i) => (
																<div
																	key={i}
																	className="text-xs bg-secondary px-2 py-1 rounded-md text-secondary-foreground border"
																>
																	{opt}
																</div>
															))}
						...
						{fields.length === 0 && (
							<div className="text-center py-16 border-2 border-dashed rounded-lg text-muted-foreground bg-muted/5">
								...
								<p className="text-sm">
									Selecione um tipo de campo no painel à direita para começar.
								</p>
							</div>
						)}
					</div>
				...
				{selectedField ? (
					<Card className="flex-1 flex flex-col overflow-hidden border-l-4 border-l-primary">
						...
						<CardContent className="space-y-6 pt-6 overflow-y-auto flex-1">
							...
							<div className="flex items-center justify-between py-2 border px-3 rounded-lg bg-muted/20">
								<Label className="cursor-pointer" htmlFor="required-switch">
									Campo Obrigatório
								</Label>
								<Switch
									id="required-switch"
									...
									onCheckedChange={(c) =>
										updateField(selectedField.id, { obrigatorio: c })
									}
								/>
							</div>

							{(selectedField.tipo_campo === "opcao_unica" ||
								selectedField.tipo_campo === "selecao" ||
								selectedField.tipo_campo === "lista") && (
								<div className="space-y-2">
									<Label>Opções</Label>
									<Textarea
					...
					<Card className="flex-1 flex flex-col overflow-hidden">
						...
						<CardContent className="pt-4 grid gap-2 overflow-y-auto flex-1 content-start">
							<Button
								variant="outline"
								className="justify-start gap-3 h-auto py-4 hover:border-primary hover:bg-primary/5 transition-all text-left group"
								onClick={() => addField("texto_curto")}
							>
								<div className="bg-primary/10 p-2 rounded group-hover:bg-primary/20">
							...
							<Button
								variant="outline"
								className="justify-start gap-3 h-auto py-4 hover:border-primary hover:bg-primary/5 transition-all text-left group"
								onClick={() => addField("escala")}
							>
								<div className="bg-primary/10 p-2 rounded group-hover:bg-primary/20">
									<div className="text-[10px] font-bold border border-primary text-primary rounded px-1">
										1-5
									</div>
								</div>
								<div>
