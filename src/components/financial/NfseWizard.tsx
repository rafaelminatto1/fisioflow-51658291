...
import { CheckCircle2, ShieldCheck, UploadCloud, Zap, ChevronRight, ChevronLeft, Loader2, AlertCircle } from "lucide-react";
...
export function NfseWizard() {
  ...
  const testConnection = useMutation({
    ...
    onError: () => setTestResult({ ok: false, message: "Não foi possível contactar a Prefeitura de SP." }),
  });
  ...
  const activateProduction = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["nfse-config"] });
      toast.success("NFS-e ativada!", { description: "Sua clínica já pode emitir notas fiscais em produção." });
    },
          ...
          return (
            <div key={step.id} className="relative z-10 flex flex-col items-center gap-2">
              <div className={`h-12 w-12 rounded-full flex items-center justify-center transition-all duration-300 border-4 ${
                isActive ? "bg-emerald-500 border-emerald-100 dark:border-emerald-900 text-white shadow-xl" :
                isCompleted ? "bg-emerald-500 border-white dark:border-slate-900 text-white" :
                "bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700 text-slate-400"
              }`}>
                <Icon className="h-5 w-5" />
              </div>
              <div className="absolute top-14 text-center w-32 -ml-10">
                <p className={`text-[10px] font-black uppercase tracking-widest ${isActive ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400"}`}>{step.title}</p>
              </div>
            </div>
          );
        })}
              ...
              <Card className="p-8 rounded-[2rem] border-none shadow-xl shadow-slate-100/50 dark:shadow-none bg-white dark:bg-slate-900/50">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {[
                    ...
                    { label: "Inscrição Municipal", key: "inscricao_municipal", placeholder: "00000000" },
          ...
          {currentStep === 3 && (
            <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <Card className="p-8 rounded-[2rem] border-none shadow-xl shadow-slate-100/50 dark:shadow-none bg-amber-50 dark:bg-amber-950/20 text-center">
                ...
                <h3 className="text-xl font-black text-amber-900 dark:text-amber-50 mb-2">Ambiente de Homologação</h3>
                <p className="text-amber-700/70 dark:text-amber-200/60 max-w-md mx-auto mb-8 font-medium">
                  Vamos testar a comunicação com a PMSP para garantir que a configuração está correta.
                </p>

                {testResult && (
                  <div className={`flex items-center gap-3 justify-center mb-6 p-4 rounded-2xl text-sm font-medium ${
                    testResult.ok
            ...
            <motion.div key="step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <Card className="p-12 rounded-[3rem] border-none shadow-xl shadow-emerald-100/50 dark:shadow-none bg-emerald-50 dark:bg-emerald-950/20 text-center relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col items-center">
                  ...
                  <h3 className="text-3xl font-black text-slate-900 dark:text-white mb-4 font-display">Tudo Pronto!</h3>
                  <p className="text-slate-600 dark:text-slate-300 max-w-sm mb-10 font-medium">
                    Configure e teste concluídos. Clique abaixo para ativar a emissão de notas fiscais em produção.
                  </p>
                  <Button
                  ...
                  >
                    {activateProduction.isPending ? <Loader2 className="h-5 w-5 animate-spin mr-2" /> : null}
                    {activateProduction.isSuccess ? "✓ Produção Ativada" : activateProduction.isPending ? "Ativando..." : "Ativar Produção"}
                  </Button>
                </div>
              </Card>
            </motion.div>
