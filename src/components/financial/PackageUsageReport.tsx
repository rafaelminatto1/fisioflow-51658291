...
export function PackageUsageReport() {
	...
	const activePackages = packages.filter(
		(p) => !p.is_expired && (p.sessions_remaining || 0) > 0,
	);
	const expiredPackages = packages.filter((p) => p.is_expired);
	const depletedPackages = packages.filter(
		(p) => !p.is_expired && (p.sessions_remaining || 0) <= 0,
	);

	const totalRevenue = packages.reduce((sum, p) => sum + p.price_paid, 0);
	const totalSessionsSold = packages.reduce(
		(sum, p) => sum + p.sessions_purchased,
		0,
	);
	const totalSessionsUsed = packages.reduce(
		(sum, p) => sum + p.sessions_used,
		0,
	);
		...
		<div className="space-y-6">
			...
			<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
				...
				<Card>
					...
					<CardContent>
						<div className="text-2xl font-bold">
							{expiredPackages.length + depletedPackages.length}
						</div>
						<p className="text-xs text-muted-foreground">
							{expiredPackages.length} expirados, {depletedPackages.length}{" "}
							esgotados
						</p>
					</CardContent>
				</Card>
							...
							{packages.slice(0, 10).map((pkg) => (
								<TableRow key={pkg.id}>
									<TableCell className="font-medium">
										{(pkg as { patient_name?: string }).patient_name ||
											"Paciente"}
									</TableCell>
									<TableCell>{pkg.package?.name}</TableCell>
									<TableCell>
										{new Date(pkg.purchased_at).toLocaleDateString("pt-BR")}
									</TableCell>
									<TableCell>
