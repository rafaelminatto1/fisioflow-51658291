          ...
          >
            {preview ? (
              <div className="relative w-full h-40">
                <img src={preview} alt="Preview" className="w-full h-full object-contain opacity-50 blur-[1px]" />
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/20 backdrop-blur-sm">
                   {isAnalyzing ? (
                     <>
                       <Loader2 className="h-8 w-8 animate-spin text-primary mb-2" />
                       <p className="text-[10px] font-black uppercase tracking-widest text-primary animate-pulse">IA Analisando Comprovante...</p>
                     </>
                   ) : (
                     <>
                       <Check className="h-8 w-8 text-emerald-500 mb-2" />
                       <p className="text-[10px] font-black uppercase tracking-widest text-emerald-600">Comprovante Processado</p>
                     </>
                   )}
                </div>
              </div>
              ...
              <>
                ...
                <div className="text-center space-y-1">
                  <p className="text-sm font-black tracking-tightest">Anexar Comprovante (PIX/Foto)</p>
      ...
      {!preview && !isAnalyzing && (
        <div className="p-3 rounded-xl bg-blue-50 border border-blue-100 flex gap-3 items-start animate-in fade-in duration-700">
           ...
           <p className="text-[10px] font-bold text-blue-700 leading-tight">
             Dica: Tire um print do PIX ou uma foto do recibo físico. Nossa IA identificará o valor e o paciente automaticamente.
           </p>
        </div>
      )}
    </div>
