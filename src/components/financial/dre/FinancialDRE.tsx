...
export const FinancialDRE: React.FC = () => {
  ...
  const dreData = useMemo<DREData>(() => {
    ...
    transactions.forEach((t: Transacao) => {
      ...
      if (t.tipo === "receita") {
        if (
          dreCategoriaStr === "receitas_financeiras" ||
          desc.includes("juros") ||
        ...
        if (
          dreCategoriaStr === "despesas_financeiras" ||
          ...
          dreCategoriaStr === "deducoes" ||
          desc.includes("imposto") ||
          desc.includes("taxa") ||
          desc.includes("simples") ||
          desc.includes("iss")
        ) {
          deducoes += valor;
        } else if (
          dreCategoriaStr === "custos_operacionais" ||
  ...
  const renderDRELine = (
    label: string,
    value: number,
    type: "total" | "subtotal" | "detail" = "detail",
    isNegative = false,
  ) => {
    ...
    return (
      <TableRow className={rowClass}>
        <TableCell className={`pl-${type === "detail" ? "8" : "4"} ${textClass}`}>
          {label}
        </TableCell>
        <TableCell className={`text-right ${textClass}`}>{formatCurrency(displayValue)}</TableCell>
      </TableRow>
    );
  };
    ...
    <div className="space-y-6 animate-in fade-in-50 slide-in-from-bottom-2 duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        >
          ...
          <CardContent>
            <div
              className={`text-2xl font-bold flex items-center gap-2 ${dreData.lucroLiquido >= 0 ? "text-emerald-600" : "text-red-600"}`}
            >
      ...
      <Card className="border-none shadow-premium-sm overflow-hidden rounded-2xl">
        <Table>
          ...
          <TableBody>
            ...
            {renderDRELine(
              "(-) Custos dos Serviços Prestados (CPV/CSP)",
              dreData.custosOperacionais,
              "detail",
              true,
            )}
            ...
            {renderDRELine(
              "4. Lucro Operacional (EBITDA / LAJIDA)",
              dreData.lucroOperacional,
              "total",
            )}
