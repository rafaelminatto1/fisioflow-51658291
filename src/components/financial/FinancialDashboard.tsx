			...
			<div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
				...
				<div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
					<Tabs
						value={period}
						onValueChange={(val) =>
							setPeriod(val as "daily" | "weekly" | "monthly" | "all")
						}
				...
				<Card>
					...
					<CardContent>
						...
						<div className="flex items-center text-xs text-muted-foreground mt-1">
							...
							<span
								className={
									data.monthlyGrowth >= 0 ? "text-emerald-500" : "text-red-500"
								}
							>
									...
									stats.totalRevenue > 0
										? transactions.reduce(
												(acc, t) => {
													...
													else if (desc.includes("fisioterapia"))
														category = "Fisioterapia";
													else if (desc.includes("pilates"))
														category = "Pilates";
													else if (desc.includes("consulta"))
														category = "Consultas";

													acc[category] =
														(acc[category] || 0) + Number(t.valor);
													return acc;
												},
