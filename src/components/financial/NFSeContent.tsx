...
function EmitModal({
	...
	const handleSubmit = () => {
		if (!form.valor_servico || !form.tomador_nome || !form.discriminacao)
			return;
		generate.mutate(
			{
			...
			<DialogContent className="max-w-md">
				...
				<DialogFooter>
					...
					<Button
						...
						disabled={
							generate.isPending || !form.valor_servico || !form.tomador_nome
						}
					>
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
					...
					<div className="space-y-3 py-2">
						...
						<div className="grid grid-cols-2 gap-3">
							<div className="space-y-1">
								...
								<Input
									...
									onChange={(e) =>
										update("inscricao_municipal", e.target.value)
									}
								/>
							</div>
							<div className="space-y-1">
						...
						<div className="grid grid-cols-2 gap-3">
							<div className="space-y-1">
								...
								<Input
									...
									onChange={(e) =>
										update("codigo_servico_padrao", e.target.value)
									}
								/>
							</div>
							<div className="space-y-1">
								...
								<Input
									...
									onChange={(e) =>
										update("aliquota_padrao", Number(e.target.value) / 100)
									}
								/>
							</div>
						</div>
						...
						<div className="flex items-center gap-3">
							<Switch
								...
								onCheckedChange={(v) =>
									update("ambiente", v ? "producao" : "homologacao")
								}
							/>
							<Label>
								Ambiente de{" "}
				...
				<DialogFooter>
					...
					<Button
						onClick={() =>
							saveConfig.mutate(form, { onSuccess: () => onOpenChange(false) })
						}
											...
											<div className="flex gap-1">
												...
												{(rec.status === "rascunho" ||
													...
													<Button
														...
														onClick={() => {
															if (confirm("Cancelar esta NFS-e?"))
																cancel.mutate(rec.id);
														}}
													>
														<X className="h-3 w-3" />
													</Button>
