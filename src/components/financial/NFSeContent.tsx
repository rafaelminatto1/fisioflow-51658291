...
import {
  FileText, Plus, Send, X, Settings, ExternalLink, AlertCircle, CheckCircle2,
  Clock, XCircle, FileCheck
} from 'lucide-react';
import { formatCurrency } from '@/lib/utils';
import {
  useNFSeRecords, useNFSeConfig, useSaveNFSeConfig,
  useGenerateNFSe, useSendNFSe, useCancelNFSe,
  type NFSeRecord, type NFSeConfig,
} from '@/hooks/useNFSe';

function statusBadge(status: NFSeRecord['status']) {
  const map: Record<NFSeRecord['status'], { label: string; variant: 'default' | 'secondary' | 'destructive' | 'outline' }> = {
...
function statusIcon(status: NFSeRecord['status']) {
  switch (status) {
    case 'autorizado': return <CheckCircle2 className="h-4 w-4 text-green-500" />;
    case 'enviado': return <Clock className="h-4 w-4 text-blue-500" />;
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-3 py-2">
          <div className="space-y-1">
            ...
            <Input value={form.tomador_nome} onChange={(e) => update('tomador_nome', e.target.value)} placeholder="Nome completo" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <Label>CPF/CNPJ</Label>
              <Input value={form.tomador_cpf_cnpj} onChange={(e) => update('tomador_cpf_cnpj', e.target.value)} placeholder="000.000.000-00" />
            </div>
            <div className="space-y-1">
              <Label>E-mail</Label>
              <Input value={form.tomador_email} onChange={(e) => update('tomador_email', e.target.value)} type="email" placeholder="email@..." />
            </div>
          </div>
          <div className="space-y-1">
...
function ConfigModal({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
        ...
        {isLoading ? (
          <div className="space-y-2 py-4">{[1,2,3].map(i => <Skeleton key={i} className="h-8 w-full" />)}</div>
        ) : (
          <div className="space-y-3 py-2">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <Label>Razão Social *</Label>
                <Input value={(merged.razao_social as string) ?? ''} onChange={(e) => update('razao_social', e.target.value)} />
              </div>
              <div className="space-y-1">
                <Label>CNPJ *</Label>
                <Input value={(merged.cnpj as string) ?? ''} onChange={(e) => update('cnpj', e.target.value)} placeholder="00.000.000/0001-00" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <Label>Inscrição Municipal *</Label>
                <Input value={(merged.inscricao_municipal as string) ?? ''} onChange={(e) => update('inscricao_municipal', e.target.value)} />
              </div>
              <div className="space-y-1">
                <Label>Código IBGE do Município</Label>
                <Input value={(merged.codigo_municipio as string) ?? '3550308'} onChange={(e) => update('codigo_municipio', e.target.value)} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <Label>Código de Serviço</Label>
                <Input value={(merged.codigo_servico_padrao as string) ?? '14.01'} onChange={(e) => update('codigo_servico_padrao', e.target.value)} />
              </div>
              <div className="space-y-1">
            ...
            <div className="space-y-1">
              ...
              <Input value={(merged.discriminacao_padrao as string) ?? ''} onChange={(e) => update('discriminacao_padrao', e.target.value)} />
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              />
              <Label>
                Ambiente de {(merged.ambiente as string) === 'producao' ? 'Produção' : 'Homologação'}
                {(merged.ambiente as string) !== 'producao' && (
                  <Badge variant="secondary" className="ml-2 text-[10px]">Testes</Badge>
                )}
              </Label>
            </div>
          </div>
...
export function NFSeContent({ patientId, appointmentId }: NFSeContentProps = {}) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline" className="text-xs">ABRASF</Badge>
        </div>
                  ...
                  <TableRow key={rec.id}>
                    <TableCell>
                      <div className="flex items-center gap-1">
                        ...
                        <div>
                          <p className="text-sm font-mono">RPS {rec.numero_rps}/{rec.serie_rps}</p>
                          {rec.numero_nfse && (
                    ...
                    <TableCell className="text-right font-medium">{formatCurrency(Number(rec.valor_servico))}</TableCell>
