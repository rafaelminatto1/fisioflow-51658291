              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  <p className="max-w-2xl text-sm text-slate-600 dark:text-slate-300">
                    Período {data.period.label.toLowerCase()} com visão unificada de caixa,
                    cobranças, agenda, CRM e marketing.
                  </p>
                </div>
              </div>
            ...
            <div className="grid gap-3 md:grid-cols-3">
              ...
              <div className="rounded-3xl border border-white/70 bg-white/70 p-4 dark:border-slate-800/70 dark:bg-slate-950/40">
                ...
                <p className="mt-3 text-xs text-slate-500 dark:text-slate-400">
                  Bruto de {formatCurrency(data.projection.rawExpectedRevenue)} com ajuste de
                  no-show.
                </p>
              </div>
              ...
              <div className="rounded-3xl border border-white/70 bg-white/70 p-4 dark:border-slate-800/70 dark:bg-slate-950/40">
                ...
                <p className="mt-3 text-xs text-slate-500 dark:text-slate-400">
                  No-show recente em {(data.projection.noShowRate * 100).toFixed(1)}% no horizonte
                  projetado.
                </p>
              </div>
            </div>
        ...
        <div className="grid gap-4">
          <Card className="rounded-[28px] border-white/70 bg-white/90 shadow-[0_24px_70px_-52px_rgba(15,23,42,0.45)] dark:border-slate-800/80 dark:bg-slate-950/70">
            <CardHeader className="flex flex-row items-start justify-between gap-3 pb-2">
              ...
              <Button asChild variant="ghost" className="rounded-2xl text-xs font-bold">
            ...
            <CardContent className="space-y-3">
              {data.alerts.length === 0 ? (
                <div className="rounded-2xl border border-emerald-200/80 bg-emerald-50/90 p-4 text-sm text-emerald-900 dark:border-emerald-900/50 dark:bg-emerald-950/20 dark:text-emerald-100">
                  Nenhum alerta crítico no momento. O hub segue estável para o período selecionado.
                </div>
              ) : (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="space-y-2">
                        <Badge
                          className={cn(
                            "rounded-full border-0 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.16em]",
                            toneBadgeClasses(alert.tone),
                          )}
                        >
                          {alert.tone === "critical"
                            ? "Crítico"
      ...
      <div className="grid gap-4 xl:grid-cols-[1.05fr_1.05fr_0.9fr]">
        <Card className="rounded-[28px] border-white/70 bg-white/90 shadow-[0_24px_70px_-52px_rgba(15,23,42,0.45)] dark:border-slate-800/80 dark:bg-slate-950/70">
          ...
          <CardContent className="space-y-3">
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                        Última sessão: {patient.lastAppointment || "sem registro"} •{" "}
            ...
            <Button asChild variant="ghost" className="rounded-2xl px-0 text-xs font-bold">
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="rounded-2xl bg-emerald-500/8 p-4">
                ...
                <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                  recalls ativos • {data.integrations.marketing.referralRedemptions} referrals no
                  período
                </p>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-slate-100 bg-slate-50/80 p-4 dark:border-slate-800 dark:bg-slate-900/60">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {data.integrations.crm.overdueTasks} atrasadas •{" "}
                    {data.integrations.crm.openTasks} abertas
                  </p>
                </div>
            ...
            <div className="rounded-2xl border border-slate-100 bg-slate-50/80 p-4 dark:border-slate-800 dark:bg-slate-900/60">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {data.integrations.schedule.scheduledNext7Days} sessões em 7 dias • no-show
                    recente {(data.integrations.schedule.noShowRate90d * 100).toFixed(1)}%
                  </p>
                </div>
                <Button asChild variant="outline" className="rounded-2xl font-bold">
            ...
            <CardContent className="space-y-3">
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="rounded-2xl bg-slate-50/80 p-4 dark:bg-slate-900/60">
                  ...
                  <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                    pendentes • {data.documents.authorizedNfse} autorizadas •{" "}
                    {data.documents.failedNfse} com erro
                  </p>
                </div>
              </div>
