...
export function ReimbursementInsights() {
  const { data: insightsRes, isLoading } = useQuery({
    ...
    queryFn: () => request<{ data: { patterns: ReimbursementPattern[], aiInsights: AIInsights } }>("/api/financial-analytics/reimbursement-patterns"),
    staleTime: 1000 * 60 * 60, // 1h
  });

  if (isLoading) {
    return <div className="h-96 flex items-center justify-center text-xs text-muted-foreground animate-pulse">Analisando padrões de reembolso em SP...</div>;
  }
    ...
    <div className="space-y-6">
      ...
      <Card className="border-none shadow-premium bg-gradient-to-br from-brand-blue to-indigo-700 text-white overflow-hidden">
        <CardHeader className="pb-2">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-sm font-black uppercase tracking-widest">Consultoria IA: Faturamento Mooca</CardTitle>
          </div>
        </CardHeader>
        ...
        {data.patterns.slice(0, 3).map((p, i) => (
          <Card key={i} className="border-none shadow-sm bg-white dark:bg-slate-900/50">
            <CardContent className="p-4">
              <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest mb-1">{p.insurance_name}</p>
              <h4 className="font-bold text-sm text-slate-900 dark:text-white mb-2">{p.procedure || "Sessão Fisioterapia"}</h4>
              <div className="flex items-baseline gap-1">
                <span className="text-lg font-black text-indigo-600">{formatCurrency(p.avg_reimbursement)}</span>
      ...
      <Card className="border-none shadow-premium bg-white dark:bg-slate-900/50">
        <CardHeader>
          ...
          <CardDescription>Otimize sua agenda priorizando os convênios com melhor margem</CardDescription>
        </CardHeader>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {data.patterns.map((p, i) => (
                <tr key={i} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="px-6 py-4 text-center font-medium text-slate-500">{p.volume} guias</td>
                  <td className="px-6 py-4 text-right font-black text-emerald-600">{formatCurrency(p.avg_reimbursement)}</td>
                </tr>
              ))}
            </tbody>
          </table>
