...
function PayConfirmModal({ row, period, onConfirm, onCancel, isPending }: PayConfirmModalProps) {
  return (
    <Dialog open={Boolean(row)} onOpenChange={(v) => { if (!v) onCancel(); }}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
        ...
        <div className="py-2 space-y-2 text-sm">
          ...
          <div className="bg-muted rounded p-3 space-y-1">
            <div className="flex justify-between"><span>Terapeuta:</span><span className="font-mono text-xs truncate max-w-[160px]">{row?.therapist_id}</span></div>
            <div className="flex justify-between"><span>Sessões:</span><span>{row?.total_sessions}</span></div>
            <div className="flex justify-between"><span>Receita:</span><span>{formatCurrency(Number(row?.total_revenue ?? 0))}</span></div>
            <div className="flex justify-between font-medium"><span>Comissão ({row?.commission_rate}%):</span><span>{formatCurrency(Number(row?.commission_amount ?? 0))}</span></div>
          </div>
        </div>
        <DialogFooter>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1">
            <Button variant="ghost" size="icon" onClick={() => setMonth((m) => navigateMonth(m, -1))}>
              ...
              <TableBody>
                {rows.map((row) => (
                  <TableRow key={row.therapist_id}>
                    <TableCell className="font-mono text-xs max-w-[120px] truncate">{row.therapist_id}</TableCell>
                    ...
                    <TableCell className="text-right">{Number(row.commission_rate).toFixed(0)}%</TableCell>
                    <TableCell className="text-right font-medium">{formatCurrency(Number(row.commission_amount))}</TableCell>
                    <TableCell>
