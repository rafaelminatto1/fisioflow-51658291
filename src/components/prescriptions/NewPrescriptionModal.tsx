...
import { Dumbbell, GripVertical, Plus, Trash2, BadgeCheck } from "lucide-react";
  ...
  useEffect(() => {
    ...
    if (initialProtocolId) {
      ...
      if (protocol && protocol.phases[initialPhaseIndex]) {
        ...
        const protocolExercises = phase.exercises
          .map((exId) => {
                ...
                <div className="bg-primary/5 border border-primary/20 rounded-xl p-3 flex items-start gap-3">
                  ...
                  <div className="space-y-1">
                    <p className="text-[11px] font-bold text-primary uppercase">
                      Carga Automática de Protocolo
                    </p>
                    <p className="text-xs text-slate-600">
                      Os exercícios abaixo foram sugeridos com base no protocolo de{" "}
                      <strong>
                        {protocolDictionary.find((p) => p.id === initialProtocolId)?.pt}
                      </strong>
                      .
                    </p>
                  </div>
              ...
              {selectedExercises.length > 0 && (
                <div className="space-y-3">
                  ...
                  {selectedExercises.map((exercise, index) => (
                    <div key={exercise.id} className="border rounded-lg p-4 space-y-3 bg-muted/30">
