			...
			<AlertDialogContent className="max-w-md border-rose-200">
				<AlertDialogHeader>
					<div className="flex items-center gap-2 text-rose-600 mb-2">
						...
						<AlertDialogTitle className="text-xl font-bold">Alerta de Contraindicação</AlertDialogTitle>
					</div>
					<AlertDialogDescription className="space-y-4">
						<p className="text-sm text-foreground">
							O exercício <span className="font-bold">"{exercise.name}"</span> possui contraindicações clínicas para as seguintes condições detectadas no prontuário:
						</p>
						
						<div className="flex flex-wrap gap-2 py-2">
							{conflicts.map((conflict, i) => (
								<Badge key={i} variant="destructive" className="bg-rose-100 text-rose-700 hover:bg-rose-100 border-rose-200">
									{conflict}
								</Badge>
							))}
						</div>
						...
						{exercise.scientific_references && exercise.scientific_references.length > 0 && (
							<div className="space-y-2">
								...
								{exercise.scientific_references.map((ref, i) => (
									<div key={i} className="text-[11px] text-muted-foreground border-l-2 border-primary/20 pl-2">
						...
						<p className="text-xs text-muted-foreground italic border-t pt-4">
							Deseja prosseguir com a prescrição mesmo assim? O sistema registrará esta decisão baseada no seu julgamento clínico.
						</p>
					</AlertDialogDescription>
				</AlertDialogHeader>
