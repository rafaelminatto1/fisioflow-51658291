...
// Utilitários para notificações específicas do sistema
export const systemNotifications = {
  // Autenticação
  loginSuccess: (userName?: string) => {
    const { showSuccess } = useGlobalNotifications();
    return showSuccess(
  ...
  logoutSuccess: () => {
    const { showSuccess } = useGlobalNotifications();
    return showSuccess('Logout realizado com sucesso', 'Até logo!');
  },
  
  sessionExpired: () => {
    const { showWarning } = useGlobalNotifications();
    return showWarning(
      'Sessão expirada',
  ...
  createSuccess: (entity: string) => {
    const { showSuccess } = useGlobalNotifications();
    return showSuccess(`${entity} criado com sucesso`);
  },
  
  updateSuccess: (entity: string) => {
    const { showSuccess } = useGlobalNotifications();
    return showSuccess(`${entity} atualizado com sucesso`);
  },
  
  deleteSuccess: (entity: string) => {
    const { showSuccess } = useGlobalNotifications();
    return showSuccess(`${entity} excluído com sucesso`);
  },
  
  // Erros comuns
  networkError: () => {
    const { showError } = useGlobalNotifications();
    return showError(
      'Erro de conexão',
  ...
  validationError: (message: string) => {
    const { showError } = useGlobalNotifications();
    return showError('Erro de validação', message);
  },
  ...
  uploadSuccess: (fileName: string) => {
    const { showSuccess } = useGlobalNotifications();
    return showSuccess('Upload concluído', `Arquivo "${fileName}" enviado com sucesso`);
  },
  ...
  syncSuccess: () => {
    const { showSuccess } = useGlobalNotifications();
    return showSuccess('Sincronização concluída', 'Dados atualizados com sucesso');
  },
  ...
  backupSuccess: () => {
    const { showSuccess } = useGlobalNotifications();
    return showSuccess('Backup criado', 'Seus dados foram salvos com segurança');
  },
  
  backupRestored: () => {
    const { showSuccess } = useGlobalNotifications();
    return showSuccess('Backup restaurado', 'Dados restaurados com sucesso');
  }
};
