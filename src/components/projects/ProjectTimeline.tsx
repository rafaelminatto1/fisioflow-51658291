...
const TaskRow = memo(
				...
				<div className="flex relative flex-1">
					...
					{days.map((day) => (
						<div
							...
							className="w-[40px] shrink-0 border-r h-full"
						/>
					))}
						...
						{task.responsavel?.avatar_url && (
							<Avatar className="h-5 w-5 mr-1 border-white border">
								...
								<AvatarFallback className="text-[10px]">
									{task.responsavel.full_name?.charAt(0)}
								</AvatarFallback>
							</Avatar>
						)}
	...
	(prevProps, nextProps) => {
		...
		return (
			...
			prevProps.task.responsavel?.avatar_url ===
				nextProps.task.responsavel?.avatar_url &&
...
function ProjectTimelineContent({
	...
	const onMouseDown = useCallback(
		(e: React.MouseEvent, task: Tarefa) => {
			// Only left click
			...
			const onMouseMove = (moveEvent: MouseEvent) => {
				setDragState((prev) =>
					prev ? { ...prev, currentX: moveEvent.clientX } : null,
				);
			};
					...
					if (daysDiff !== 0) {
						...
						if (task) {
							const originalStart = task.start_date
								? new Date(task.start_date)
								: new Date(task.created_at);
							const originalEnd = task.data_vencimento
								? new Date(task.data_vencimento)
								: addDays(originalStart, 1);
	...
	const taskCoordinates = useMemo(() => {
		...
		tasks.forEach((task, index) => {
			let taskStart = task.start_date
				? new Date(task.start_date)
				: new Date(task.created_at);
			let taskEnd = task.data_vencimento
				? new Date(task.data_vencimento)
				: addDays(taskStart, 1);
				...
				<ScrollArea className="flex-1 w-full whitespace-nowrap rounded-md border">
					<div className="flex flex-col min-w-[800px] relative select-none">
						...
						<div className="relative">
							...
							<svg
								className="absolute top-0 left-64 w-full h-full pointer-events-none z-10"
