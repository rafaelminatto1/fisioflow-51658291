    ...
    >
      ...
      >
        ...
        {task.progress > 0 && task.progress < 100 && (
          <div
            className={cn("absolute left-0 top-0 bottom-0 rounded-l-md", statusColor)}
      ...
      <div className="sr-only" data-tooltip-content>
        <div className="space-y-1">
          ...
          <div className="text-xs text-muted-foreground">
            {format(task.start_date, "dd/MMM", { locale: ptBR })} -{" "}
            {format(task.end_date, "dd/MMM", { locale: ptBR })}
          </div>
          {task.assignee_name && <div className="text-xs">Responsável: {task.assignee_name}</div>}
