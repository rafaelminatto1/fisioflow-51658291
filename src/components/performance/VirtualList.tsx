...
export function VirtualList<T>({
	...
	const { startIndex, endIndex, _visibleCount } = React.useMemo(() => {
		...
		const endIndex = Math.min(
			items.length - 1,
			Math.ceil((scrollState.scrollTop + height) / effectiveItemHeight) +
				overscan,
		);
		...
		>
			...
			{visibleItems.length > 0
				? visibleItems.map((item, index) => {
						...
						const isVisible =
							actualIndex >= startIndex - overscan &&
							actualIndex <= endIndex + overscan;
			...
			<div
				style={{
					height: Math.max(
						0,
						totalHeight - offsetY - visibleItems.length * itemHeight,
					),
				}}
				aria-hidden="true"
			/>
...
export function VariableVirtualList<T>({
	...
	const totalHeight =
		positions.length > 0
			? positions[positions.length - 1].offsetTop +
						...
						<div
							style={{
								height: Math.max(
									0,
									totalHeight -
										(positions[endIndex]?.offsetTop || 0) -
										(positions[endIndex]?.height || 0),
								),
							}}
						/>
					)}
