...
export const LinkManagement = ({
	...
	const getFullLink = (token: string) =>
		`${window.location.origin}/pre-cadastro/${token}`;
	...
	const toggleFieldRequired = (fieldId: string) => {
		...
		if (isRequired) {
			setNewToken({
				...newToken,
				campos_obrigatorios: newToken.campos_obrigatorios.filter(
					(f) => f !== fieldId,
				),
			...
			setNewToken({
				...newToken,
				campos_opcionais: newToken.campos_opcionais.filter(
					(f) => f !== fieldId,
				),
			});
		} else {
			...
			<CardHeader>
				<div className="flex justify-between items-center">
					<div>
						...
						<CardDescription>
							Links para compartilhar com potenciais pacientes
						</CardDescription>
					</div>
							...
							<div className="space-y-6 py-4">
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
									<div className="space-y-2">
										...
										<Input
											value={newToken.nome}
											onChange={(e) =>
												setNewToken({ ...newToken, nome: e.target.value })
											}
									...
									<div className="grid grid-cols-2 gap-2">
										<div className="space-y-2">
											...
											<Input
												...
												onChange={(e) =>
													setNewToken({
														...newToken,
														validade_dias: parseInt(e.target.value) || 30,
													})
												}
											/>
										</div>
										...
										<div className="space-y-2">
											...
											<Input
												...
												onChange={(e) =>
													setNewToken({ ...newToken, max_usos: e.target.value })
												}
												placeholder="∞"
											/>
										</div>
								...
								<div className="space-y-2">
									...
									<Textarea
										...
										onChange={(e) =>
											setNewToken({ ...newToken, descricao: e.target.value })
										}
				...
				{isLoading ? (
					...
					<div className="space-y-4">
						{tokens?.map((t) => (
							<div
								key={t.id}
								className="flex flex-col md:flex-row md:items-center justify-between p-4 border rounded-lg gap-4 bg-card hover:bg-accent/5 transition-colors"
							>
								<div className="flex-1">
									<div className="flex items-center gap-2 flex-wrap">
										<h4 className="font-medium text-lg text-primary">
											{t.nome || "Link de Pré-cadastro"}
										</h4>
									...
									<p className="text-sm text-muted-foreground mt-1">
										{t.usos_atuais} usos{" "}
										{t.max_usos ? `de ${t.max_usos}` : "(ilimitado)"}
										{t.expires_at && (
											<>
												{" • "}
												Expira em{" "}
												{format(new Date(t.expires_at), "dd/MM/yyyy", {
													locale: ptBR,
												})}
											</>
										)}
									</p>
									{((t.campos_obrigatorios as string[])?.length > 0 ||
										(t.campos_opcionais as string[])?.length > 0) && (
										<div className="flex flex-wrap gap-1 mt-3">
											{((t.campos_obrigatorios as string[]) || []).map(
												(campo: string) => (
													<Badge
														key={campo}
														variant="outline"
													...
													>
														{AVAILABLE_FIELDS.find((f) => f.id === campo)
															?.label || campo}
														*
													</Badge>
												),
											)}
											{((t.campos_opcionais as string[]) || []).map(
												(campo: string) => (
													<Badge
														key={campo}
														variant="secondary"
								...
								<div className="flex items-center gap-2">
									...
									<Dialog
										open={qrCodeOpen === t.token}
										onOpenChange={(open) =>
											setQrCodeOpen(open ? t.token : null)
										}
									>
										<DialogTrigger asChild>
										...
										<DialogContent className="sm:max-w-sm flex flex-col items-center justify-center py-6">
											...
											<Button
												...
												variant="secondary"
												onClick={() =>
													window.open(getFullLink(t.token), "_blank")
												}
											>
												Abrir Link
											</Button>
