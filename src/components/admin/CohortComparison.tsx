...
import { type AppointmentRow, appointmentsApi, type PatientRow, patientsApi } from "@/api/v2";
 ...
 */
function calculateWeightedAverage(cohorts: CohortData[], key: keyof CohortData): number {
  if (cohorts.length === 0) return 0;

  const totalWeight = cohorts.reduce((sum, c) => sum + c.patients, 0);
  if (totalWeight === 0) return 0;

  const weightedSum = cohorts.reduce((sum, c) => sum + (c[key] as number) * c.patients, 0);
    ...
    <Card>
      <CardContent className="p-4">
        <div className="flex items-center gap-3">
          <div className={cn("p-3 rounded-lg", classes.bg)}>{icon}</div>
          <div>
...
export function CohortComparison({
  ...
  const {
    ...
    queryFn: async (): Promise<PatientWithCohortData[]> => {
      const completedStatuses = new Set(["atendido", "confirmado", "completed", "realizado"]);

      const startDate =
        timeRange !== "all" ? subMonths(new Date(), TIME_RANGE_CONFIGS[timeRange].months) : null;
      ...
      const patientsWithAnalytics = await Promise.all(
        data.map(async (p) => {
          ...
          const pathologiesResponse = await patientsApi
            .pathologies(p.id)
            .catch(() => ({ data: [] }));
  ...
  const cohorts = useMemo((): CohortData[] => {
    ...
    const cohortData: CohortData[] = Array.from(groups.entries()).map(([name, group]) => {
  ...
  const toggleCohortSelection = useCallback((cohortName: string) => {
    setSelectedCohorts((prev) =>
      prev.includes(cohortName) ? prev.filter((c) => c !== cohortName) : [...prev, cohortName],
    );
  }, []);
    ...
    return (
      <Card className="border-destructive/50">
        <CardContent className="p-12 text-center">
          <p className="text-destructive font-medium mb-2">Erro ao carregar dados dos cohortes</p>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <RadarChart data={radarData}>
                  ...
                  />
                  {(selectedCohorts.length > 0
                    ? filteredCohorts.filter((c) => selectedCohorts.includes(c.name)).slice(0, 5)
                    : filteredCohorts.slice(0, 3)
                  ).map((cohort, index) => (
                    <Radar
                      key={cohort.name}
                ...
                >
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <span className="text-sm font-medium">{formatCohortName(cohort.name)}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">{cohort.patients}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">{cohort.avgSessions}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">{cohort.successRate}%</td>
                  <td className="px-6 py-4 whitespace-nowrap">
