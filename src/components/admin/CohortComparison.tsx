...
export function CohortComparison({
	...
	const {
		...
		queryFn: async (): Promise<PatientWithCohortData[]> => {
			...
			const startDate =
				timeRange !== "all"
					? subMonths(new Date(), TIME_RANGE_CONFIGS[timeRange].months)
					: null;
			...
			const patientsWithAnalytics = await Promise.all(
				data.map(async (p) => {
					...
					const totalSessions = patientAppointments.reduce(
						(count, appointment) => {
					...
					const pathology = String(
						pathologiesResponse.data?.[0]?.name || "Não especificado",
					);
	...
	const cohorts = useMemo((): CohortData[] => {
		...
		patients.forEach((patient) => {
			...
			switch (cohortBy) {
				...
				case "age_group":
					...
					break;
				case "session_frequency":
					cohortKey = getSessionFrequency(
						patient.total_sessions,
						patient.created_at,
					);
					break;
				case "outcome":
					cohortKey = getOutcomeCategory(
						patient.overall_progress,
						patient.dropout_risk,
					);
					break;
				case "treatment_type":
				default:
		...
		const cohortData: CohortData[] = Array.from(groups.entries()).map(
			([name, group]) => {
				const totalSessions = group.reduce(
					(sum, p) => sum + p.total_sessions,
					0,
				);
				const totalProgress = group.reduce(
					(sum, p) => sum + p.overall_progress,
					0,
				);
				const totalDropoutRisk = group.reduce(
					(sum, p) => sum + p.dropout_risk,
					0,
				);
	...
	const radarData = useMemo(() => {
		...
		const maxSessions = Math.max(
			...filteredCohorts.map((c) => c.avgSessions),
			1,
		);
	...
	const summaryData = useMemo(() => {
		const totalPatients = filteredCohorts.reduce(
			(sum, c) => sum + c.patients,
			0,
		);
		const avgProgress = calculateWeightedAverage(
			filteredCohorts,
			"avgProgress",
		);
		const avgSessions = calculateWeightedAverage(
			filteredCohorts,
			"avgSessions",
		);
						...
						<div className="space-y-2">
							...
							<div className="relative">
								...
								>
									...
									<option value="session_frequency">
						...
						<tbody className="divide-y divide-border">
							{filteredCohorts.map((cohort, index) => (
								...
								>
									...
									<td className="px-6 py-4 whitespace-nowrap">
										<Badge
											className={getDropoutRateBadgeClasses(cohort.dropoutRate)}
										>
											{cohort.dropoutRate}%
										</Badge>
									</td>
