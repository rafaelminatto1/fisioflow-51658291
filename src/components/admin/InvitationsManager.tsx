...
export function InvitationsManager() {
	...
	const invitationToDelete = useMemo(
		() =>
			invitations.find((invitation) => invitation.id === deleteInvitationId) ||
			null,
	...
	return (
		<div className="space-y-6">
			...
			<Card>
				<CardHeader>
					<CardTitle>
						Convites Enviados ({filteredInvitations.length})
					</CardTitle>
				</CardHeader>
				<CardContent>
					...
					<form onSubmit={handleCreateInvitation} className="space-y-4">
						...
						<div className="space-y-2">
							...
							<Select
								...
								onValueChange={(value) =>
									setCreateForm((prev) => ({ ...prev, role: value as AppRole }))
								}
							>
			...
			<Dialog
				open={editDialogOpen}
				onOpenChange={(open) =>
					!open ? closeEditDialog() : setEditDialogOpen(true)
				}
			>
				<DialogContent>
					<DialogHeader>
			...
			<AlertDialog
				...
				onOpenChange={() => setDeleteInvitationId(null)}
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>
							{invitationToDelete &&
							getInvitationStatus(invitationToDelete) === "pending"
