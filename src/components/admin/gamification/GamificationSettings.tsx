...
type GamificationSetting = Omit<GamificationSettingRow, "value"> & {
...
export default function GamificationSettings() {
	...
	const [changedSettings, setChangedSettings] = useState<
		Record<string, GamificationSettingValue>
	>({});

	const { data: settings = [], isLoading } = useQuery({
		queryKey: ["gamification-settings"],
		queryFn: async () =>
			(await gamificationApi.getSettings()).data as GamificationSetting[],
	});

	const updateSettings = useMutation({
		mutationFn: async (
			updatedItems: Record<string, GamificationSettingValue>,
		) => gamificationApi.updateSettings(updatedItems),
		onSuccess: () => {
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Ajuste os parâmetros do sistema de gamificação
				</CardDescription>
			</CardHeader>
			<CardContent className="space-y-6">
				...
				<div className="flex justify-end">
					<Button
						onClick={handleSave}
						disabled={
							!Object.keys(changedSettings).length || updateSettings.isPending
						}
					>
