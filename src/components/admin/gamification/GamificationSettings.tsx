...
export default function GamificationSettings() {
  ...
  const handleSave = () => {
    if (Object.keys(changedSettings).length) updateSettings.mutate(changedSettings);
  };
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        <div className="space-y-4">
          {settings.map((setting) => (
            <div key={setting.key} className="grid gap-2">
              ...
              <Input
                id={setting.key}
                defaultValue={String(changedSettings[setting.key] ?? setting.value ?? "")}
        ...
        <div className="flex justify-end">
          <Button
            onClick={handleSave}
            disabled={!Object.keys(changedSettings).length || updateSettings.isPending}
          >
