...
export const LevelSystemConfig: React.FC = () => {
	const { levelSettings, levelSettingsLoading, updateLevelSettings } =
		useGamificationAdmin();

	const [progressionType, setProgressionType] =
		useState<ProgressionType>("linear");
	...
	const levelCurve = calculateLevelCurve(
		progressionType,
		baseXp,
		multiplier,
		20,
					...
					<div className="space-y-3">
						{levelCurve.slice(0, 10).map((level) => {
							...
							const progressPercentage =
								level.level <= 1
									? 100
									: ((level.xpRequired - (previousLevel?.xpRequired || 0)) /
											level.xpRequired) *
										100;
