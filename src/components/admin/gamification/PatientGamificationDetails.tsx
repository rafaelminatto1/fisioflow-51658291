...
import { gamificationApi, patientsApi, type PatientChallengeRow } from "@/api/v2";
...
export const PatientGamificationDetails: React.FC<PatientGamificationDetailsProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "history" | "achievements" | "challenges"
  >("overview");
  ...
  const {
    ...
    queryFn: async (): Promise<GamificationProfile | null> =>
      (await gamificationApi.getProfile(patientId)).data ?? null,
  ...
  const { data: achievements } = useQuery<AchievementLogEntry[]>({
    ...
    queryFn: async () => {
      ...
      return unlocked.map((entry) => ({
        ...entry,
        achievements: all.find((achievement) => achievement.id === entry.achievement_id) as
          | Achievement
          | undefined,
      }));
    },
  ...
  const handleResetStreak = async () => {
    ...
    await onResetStreak({ patientId, patientName: patient?.full_name });
    refetchProfile();
  };
  if (!open) return null;
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-lg sm:max-w-xl overflow-hidden">
        <SheetHeader className="px-6">
          ...
          <SheetDescription>{patient?.full_name || "Carregando..."}</SheetDescription>
        </SheetHeader>
        {profileLoading ? (
          ...
          <>
            <Tabs
              value={activeTab}
              onValueChange={(v) =>
                setActiveTab(v as "overview" | "history" | "achievements" | "challenges")
              }
              className="px-6"
            >
                ...
                <ScrollArea className="px-1">
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>Adicione ou remova XP manualmente</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div className="flex gap-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setXpAmount(Number(e.target.value) * (xpAmount < 0 ? -1 : 1))
                          }
                        ...
                        <Button
                          ...
                          className={
                            xpAmount > 0 ? "bg-green-600 text-white hover:bg-green-700" : ""
                          }
                        >
                          ...
                          <TableRow key={tx.id}>
                            ...
                            <TableCell>
                              <Badge variant="outline" className="capitalize">
                                {tx.reason === "manual_adjustment"
                                  ? "Ajuste Manual"
                                  : tx.reason === "session_completed"
                                    ? "Sessão"
                                    : tx.reason === "daily_quest"
                                      ? "Missão"
                                      : tx.reason === "achievement_unlocked"
                                        ? "Conquista"
                                        : tx.reason}
                              </Badge>
                            </TableCell>
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="font-semibold text-sm text-yellow-900">
                                  {entry.achievements?.title || "Conquista"}
                                </p>
                                ...
                                <div className="flex items-center gap-2 mt-2">
                                  ...
                                  <span className="text-xs text-yellow-600">
                                    {format(new Date(entry.unlocked_at), "dd/MM/yyyy", {
                                      locale: ptBR,
                                    })}
                                  </span>
                                </div>
                ...
                <ScrollArea className="h-[calc(100vh-200px)]">
                  {!challenges?.length ? (
                    <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
                      ...
                      <h3 className="text-lg font-semibold mb-2">Sem desafios</h3>
                      <p className="text-sm text-muted-foreground">
                        Nenhum progresso registrado em desafios
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {challenges.map((challenge) => (
                        <Card
                          key={`${challenge.challenge_id}-${challenge.completed_at || "pending"}`}
                        >
                          <CardContent className="p-4 flex items-center justify-between">
                            <div>
                              <p className="font-medium">Desafio {challenge.challenge_id}</p>
                              <p className="text-sm text-muted-foreground">
