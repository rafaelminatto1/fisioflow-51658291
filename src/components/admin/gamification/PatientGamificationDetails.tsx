...
interface PatientGamificationDetailsProps {
	...
	onResetStreak?: (params: {
		patientId: string;
		patientName?: string;
	}) => Promise<void>;
}

export const PatientGamificationDetails: React.FC<
	PatientGamificationDetailsProps
> = ({ patientId, open, onOpenChange, onAdjustXp, onResetStreak }) => {
	...
	const { data: transactions } = useQuery({
		...
		queryFn: async () =>
			(await gamificationApi.getTransactions(patientId)).data ?? [],
	...
	const { data: achievements } = useQuery<AchievementLogEntry[]>({
		...
		queryFn: async () => {
			...
			return unlocked.map((entry) => ({
				...entry,
				achievements: all.find(
					(achievement) => achievement.id === entry.achievement_id,
	...
	const { data: challenges } = useQuery<PatientChallengeRow[]>({
		...
		queryFn: async () =>
			(await gamificationApi.getPatientChallenges(patientId)).data ?? [],
				...
				{profileLoading ? (
					...
					<>
						<Tabs
							...
							onValueChange={(v) =>
								setActiveTab(
									v as "overview" | "history" | "achievements" | "challenges",
								)
							}
							className="px-6"
						>
								...
								<ScrollArea className="px-1">
									...
									<Card>
										<CardHeader>
											...
											<CardDescription>
												Adicione ou remova XP manualmente
											</CardDescription>
										</CardHeader>
										<CardContent className="space-y-4">
											<div className="flex gap-2">
												...
												<Input
													...
													onChange={(e) =>
														setXpAmount(
															Number(e.target.value) * (xpAmount < 0 ? -1 : 1),
														)
													}
											...
											{challenges.map((challenge) => (
												...
												>
													<CardContent className="p-4 flex items-center justify-between">
														...
														<Badge
															variant={
																challenge.completed ? "default" : "outline"
															}
														>
															{challenge.completed
																? "Concluído"
