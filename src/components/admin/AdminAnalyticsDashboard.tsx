...
import { appointmentsApi, patientsApi, type AppointmentRow, type PatientRow } from "@/api/v2";
    ...
    queryFn: async (): Promise<{
      ...
      appointments.forEach((appointment) => {
        ...
        if (
          patientId &&
          date &&
          ["completed", "concluido", "realizado", "atendido"].includes(status)
        ) {
      ...
      const avgProgress =
        patientsWithAnalytics.reduce((sum, p) => sum + p.overall_progress, 0) /
        (patientsWithAnalytics.length || 1);
      const avgSessions =
        patientsWithAnalytics.reduce((sum, p) => sum + p.total_sessions, 0) /
        (patientsWithAnalytics.length || 1);
      ...
      const retentionRate =
        activePatients > 0 ? (activeInLast30Days.size / activePatients) * 100 : 0;
...
export function AdminAnalyticsDashboard({ className }: AdminAnalyticsDashboardProps) {
  ...
  const statusDistribution = useMemo(() => {
    ...
    const inactiveCount =
      data.metrics.totalPatients - data.metrics.activePatients - data.metrics.atRiskPatients;
    return [
    ...
    return (
      <Card className="border-red-200 bg-red-50 dark:bg-red-950/20">
        <CardContent className="p-6 text-center">
          ...
          <p className="text-sm text-red-600 dark:text-red-400 mb-4">{(error as Error).message}</p>
    ...
    <div className={cn("space-y-6", className)}>
        ...
        <div className="flex gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={handleExportAll}>Exportar Excel (Todos)</DropdownMenuItem>
              <DropdownMenuItem
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard
          ...
          color="blue"
          trend={
            data.metrics.newPatientsThisMonth > 0 ? data.metrics.newPatientsThisMonth : undefined
          }
        />
        <MetricCard
                        ...
                        <td className="p-3">
                          ...
                          {patient.last_session && (
                            <div className="text-xs text-muted-foreground">
                              Últ:{" "}
                              {format(new Date(patient.last_session), "dd/MMM", { locale: ptBR })}
                            </div>
                          )}
                        </td>
                        <td className="p-3">
                            ...
                            <div className="w-16 h-2 bg-muted rounded-full overflow-hidden">
                              <div
                                className={cn(
                                  ...
                                  patient.overall_progress >= 75
                                    ? "bg-green-500"
                                    : patient.overall_progress >= 50
                                      ? "bg-yellow-500"
                                      : patient.overall_progress >= 25
                                        ? "bg-orange-500"
                                        : "bg-red-500",
                                )}
                                style={{
                                  width: `${patient.overall_progress}%`,
                                }}
                              />
                            </div>
                        ...
                        <td className="p-3 text-right">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem
                                onSelect={() =>
                                  setExpandedRow(expandedRow === patient.id ? null : patient.id)
                                }
                              >
