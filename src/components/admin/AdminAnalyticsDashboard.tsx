...
function useAdminAnalytics() {
	return useQuery({
		...
		queryFn: async (): Promise<{
			...
			const patientsWithAnalytics: PatientWithAnalytics[] = (
				patients || []
			).map((p) => {
			...
			const activePatients = patientsWithAnalytics.filter(
				(p) => p.status === "active",
			).length;
			const atRiskPatients = patientsWithAnalytics.filter(
				(p) => p.status === "at_risk",
			).length;
			const avgProgress =
				...
				appointments
					?.filter((a) => {
						...
						return (
							...
							["completed", "concluido", "realizado", "atendido"].includes(
								status,
							)
						);
					})
			...
			const retentionRate =
				activePatients > 0
					? (activeInLast30Days.size / activePatients) * 100
					: 0;
...
function SortableHeader({
	...
	return (
		...
		>
			...
			{isActive && (
				<ChevronDown
					className={cn("h-3 w-3", order === "asc" && "rotate-180")}
				/>
			)}
		</button>
	);
...
export function AdminAnalyticsDashboard({
	...
	const statusDistribution = useMemo(() => {
		...
		const inactiveCount =
			...
			data.metrics.activePatients -
			data.metrics.atRiskPatients;
		return [
		...
		<div className={cn("space-y-6", className)}>
			...
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
				<div>
					...
					<p className="text-muted-foreground">
						Visão geral de todos os pacientes e analytics
					</p>
				</div>
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
				<MetricCard
					...
					color="blue"
					trend={
						data.metrics.newPatientsThisMonth > 0
				...
				<CardHeader>
					<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
						<div>
							...
							<CardDescription>
								Mostrando {filteredPatients.length} de {data.patients.length}{" "}
								pacientes
							</CardDescription>
						</div>
						<div className="flex gap-2">
