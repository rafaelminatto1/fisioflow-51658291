...
export function DoctorReferralReportGenerator({
	...
	const { profile: gamificationProfile, progressPercentage } =
	...
	const generateAISummary = async () => {
		...
		try {
			...
			setAiSummary(
				"Paciente em tratamento fisioterapêutico regular apresentando evolução gradual conforme plano de cuidados estabelecido.",
			);
		} finally {
	...
	const reportData: DoctorReferralPdfData = {
		...
		analysis: {
			summary:
				aiSummary ||
				'Clique em "Analisar com IA" para gerar um parecer detalhado.',
			evolution: records
				.slice(0, 3)
				.map(
					(r) =>
			...
			<CardHeader>
				<div className="flex items-center gap-2">
					...
					<div>
						...
						<CardDescription>
							Gerar PDF profissional com métricas de IA e adesão
						</CardDescription>
					</div>
				</div>
			</CardHeader>
				...
				<div className="grid grid-cols-2 gap-3">
					<div className="p-3 bg-white dark:bg-slate-900 rounded-xl border border-indigo-100 dark:border-indigo-800 flex items-center gap-3">
						...
						<div className="text-xs">
							...
							<p
								className={
									aiSummary ? "text-green-600 font-medium" : "text-slate-400"
								}
							>
