...
export function ReportGeneratorDialog({
  ...
  const handleGenerate = async () => {
    ...
    try {
      const { ReportGeneratorService } = await import("@/lib/services/ReportGeneratorService");
      const therapistName = profile?.full_name || "Fisioterapeuta";
      const reportDate = new Date().toLocaleDateString("pt-BR");

      // Get initial and current assessment data
      const initialPain =
        painEvolution.length > 0
          ? (
              painEvolution[painEvolution.length - 1] as {
                globalPainLevel?: number;
                averageIntensity?: number;
              }
            ).globalPainLevel ||
            painEvolution[painEvolution.length - 1].averageIntensity ||
            0
          : 0;
      const currentPain =
        painEvolution.length > 0
          ? (
            ...
            painEvolution[0].averageIntensity ||
            0
          : 0;
      ...
      if (reportType === "medical") {
        ...
        ReportGeneratorService.generateMedicalReport({
          ...
          challenges: ["Manter adesão aos exercícios domiciliares"],
        });
        ...
        ReportGeneratorService.generatePatientReport({
          ...
          painReduction,
          motivationalMessage:
            "Parabéns pelo seu progresso! Continue assim e você alcançará seus objetivos.",
        });
      } else {
        // Relatório Interno
        ReportGeneratorService.generateInternalReport({
          ...
          challenges: ["Baixa adesão aos exercícios domiciliares em dias chuvosos"],
          riskFactors:
            pathologies.filter((p) => p.status === "em_tratamento").length > 2
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="space-y-6 py-4">
                  ...
                  <div className="flex-1">
                    <label htmlFor="medical" className="cursor-pointer">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Relatório técnico completo para envio ao médico solicitante. Inclui
                        histórico cirúrgico, patologias, medicações, evolução clínica detalhada com
                        testes e medições.
                      </p>
                    </label>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <label htmlFor="patient" className="cursor-pointer">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Relatório simplificado e motivacional para o paciente. Destaca conquistas,
                        progresso em linguagem acessível, objetivos atuais e próximos passos.
                      </p>
                    </label>
                    <Badge variant="secondary" className="mt-2">
                      Simplificado
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <label htmlFor="internal" className="cursor-pointer">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Relatório para equipe interna de fisioterapeutas. Inclui métricas de adesão,
                        análise de risco, recomendações técnicas e observações clínicas.
                      </p>
                    </label>
                    <Badge variant="secondary" className="mt-2">
                      Interno
          ...
          <div>
            <Label htmlFor="customNotes" className="mb-2 block">
