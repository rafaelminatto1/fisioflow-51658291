...
const mapPainMap = (map: Record<string, unknown>): PainMapRecord => ({
	...
	global_pain_level: Number(
		map.pain_level ?? map.global_pain_level ?? 0,
	) as PainIntensity,
		...
		queryFn: async (): Promise<PainMapHistoryData> => {
			...
			const regionRanking = Array.from(regionStats.entries())
				.map(([region, stats]) => ({
					...
					avgIntensity:
						Math.round((stats.totalIntensity / stats.count) * 10) / 10,
			...
			if (painMaps.length >= 2) {
				...
				const firstAvg =
					(firstSession.pain_points as PainMapPoint[]).length > 0
						? (firstSession.pain_points as PainMapPoint[]).reduce(
								(s, p) => s + p.intensity,
								0,
				...
				const lastAvg =
					(lastSession.pain_points as PainMapPoint[]).length > 0
						? (lastSession.pain_points as PainMapPoint[]).reduce(
								(s, p) => s + p.intensity,
								0,
			...
			const avgPainLevel =
				painMaps.length > 0
					? painMaps.reduce((sum, pm) => sum + pm.global_pain_level, 0) /
						painMaps.length
					: 0;

			const firstPain =
				painMaps.length > 0
					? painMaps[painMaps.length - 1].global_pain_level
					: 0;
			...
			const mostAffectedRegion =
				regionRanking.length > 0 ? regionRanking[0].label : "N/A";
			...
			if (painMaps.length >= 3 && lastPain > 0 && painReduction > 0) {
				...
				if (reductionPerWeek > 0) {
					estimatedWeeksToRecovery = Math.ceil(
						(100 - painReduction) / reductionPerWeek,
					);
				}
			}
...
export function comparePainMaps(
	...
	(session1.pain_points as PainMapPoint[]).forEach((p) =>
		allRegions.add(p.region),
	);
	(session2.pain_points as PainMapPoint[]).forEach((p) =>
		allRegions.add(p.region),
	);
	...
	(session1.pain_points as PainMapPoint[]).forEach((p) =>
		session1Map.set(p.region, p.intensity),
	);
	(session2.pain_points as PainMapPoint[]).forEach((p) =>
		session2Map.set(p.region, p.intensity),
	);
	...
	const avg1 =
		(session1.pain_points as PainMapPoint[]).length > 0
			? (session1.pain_points as PainMapPoint[]).reduce(
					(s, p) => s + p.intensity,
					0,
	...
	const avg2 =
		(session2.pain_points as PainMapPoint[]).length > 0
			? (session2.pain_points as PainMapPoint[]).reduce(
					(s, p) => s + p.intensity,
					0,
