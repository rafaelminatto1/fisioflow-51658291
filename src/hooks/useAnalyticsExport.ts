...
async function generatePDF(
	...
	const fileName =
		options.fileName || `analytics_${sanitizedName}_${timestamp}.pdf`;
			...
			...(data.progressSummary.avgSessionDuration
				? [
						[
							...
							`${data.progressSummary.avgSessionDuration} min`,
						],
					]
				: []),
		],
...
function getRiskInterpretation(
	value: number,
	type: "dropout" | "success",
): string {
	if (type === "dropout") {
...
function convertToCSV(
	...
	rows.push([
		"Total de Sessões",
		data.progressSummary.totalSessions.toString(),
	]);
	rows.push([
		"Redução Total da Dor",
		`${data.progressSummary.totalPainReduction}%`,
		...
		mutationFn: async ({
			...
			const exportData: AnalyticsExportData = {
				...
				progressSummary: {
					...
					totalPainReduction:
						analyticsData.progress_summary.total_pain_reduction,
						...
						? [
								{
									...
									changePercentage:
										analyticsData.function_trend.change_percentage,
								},
							]
						: []),
				],
				predictions: {
					dropoutRisk: Math.round(
						analyticsData.predictions.dropout_probability,
					),
					successProbability: Math.round(
						analyticsData.predictions.success_probability,
					),
					predictedRecoveryDate: analyticsData.predictions
						.predicted_recovery_date
						? format(
								new Date(analyticsData.predictions.predicted_recovery_date),
								"dd/MM/yyyy",
								{ locale: ptBR },
							)
						: undefined,
			...
			for (let i = 0; i < total; i++) {
				...
				const progressSummary =
					progressResult.status === "fulfilled"
						...
						: null;
				const riskScore =
					riskResult.status === "fulfilled" ? riskResult.value.data : null;
				const goals =
					goalsResult.status === "fulfilled" ? goalsResult.value.data : [];
				const sessionCount =
					stats?.totalSessions ?? progressSummary?.total_sessions ?? 0;
				const goalsAchieved =
