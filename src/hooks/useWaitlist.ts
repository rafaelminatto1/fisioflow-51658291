...
export function useWaitlist(filters?: { status?: string; priority?: string }) {
	const queryResult = useQuery({
		...
		queryFn: async () => {
			...
			return list.sort((a, b) => {
				const pa =
					PRIORITY_CONFIG[a.priority as keyof typeof PRIORITY_CONFIG]?.order ??
					2;
				const pb =
	...
	return {
		...queryResult,
		isFromCache:
			queryResult.isStale && !queryResult.isLoading && !!queryResult.data,
	...
	return useQuery({
		...
		queryFn: async () => {
			...
			data.forEach((item) => {
				...
				if (item.priority === "urgent" && item.status === "waiting")
					counts.urgent++;
...
export function useOfferSlot() {
	...
	return useMutation({
		...
		onError: (error: unknown) => {
			toast.error(
				error instanceof Error ? error.message : "Erro ao oferecer vaga",
			);
		},
	});
}
...
export function useRejectOffer() {
	...
	return useMutation({
		mutationFn: async (waitlistId: string) => {
			...
			const refusalCount =
				((entry?.refusal_count as number | undefined) ?? 0) + 1;
			const maxRefusals = 3;
...
export function isSlotCompatible(
	...
	return (
		(entry.preferred_days ?? []).includes(dayOfWeek) &&
