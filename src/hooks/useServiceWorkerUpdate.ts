...
export function useServiceWorkerUpdate() {
		...
		const registerSW = async () => {
			try {
				...
				wb.addEventListener("waiting", (_event) => {
					logger.info(
						"[SW] Nova versão disponível (waiting)",
						undefined,
						"useServiceWorkerUpdate",
					);
					...
					toast.info("Nova versão disponível!", {
						...
						action: {
							...
							onClick: () => {
								...
								if (wbRef.current) {
									...
									wbRef.current.addEventListener(
										"controlling",
										() => {
											logger.info(
												"[SW] Novo controller ativado, recarregando",
												undefined,
												"useServiceWorkerUpdate",
											);
						...
						cancel: {
							...
							onClick: () => {
								logger.info(
									"[SW] Usuário optou por atualizar depois",
									undefined,
									"useServiceWorkerUpdate",
								);
				...
				wb.addEventListener("activated", (event) => {
					...
					if (event.isUpdate) {
						logger.info(
							"[SW] Atualização aplicada com sucesso",
							undefined,
							"useServiceWorkerUpdate",
						);
					} else {
						logger.info(
							"[SW] Service Worker instalado pela primeira vez",
							undefined,
							"useServiceWorkerUpdate",
						);
				...
				wb.addEventListener("controlling", () => {
					logger.info(
						"[SW] Service Worker agora está controlando a página",
						undefined,
						"useServiceWorkerUpdate",
					);
				});
					...
					() => {
						if (wbRef.current) {
							...
							wbRef.current.update().catch((_err) => {
								logger.debug(
									"[SW] Nenhuma atualização disponível",
									undefined,
									"useServiceWorkerUpdate",
								);
							});
				...
				logger.error(
					"[SW] Falha ao registrar Service Worker",
					error,
					"useServiceWorkerUpdate",
				);
				if (mounted) {
	...
	const forceUpdate = () => {
		logger.info(
			"[SW] Forçando atualização manual",
			undefined,
			"useServiceWorkerUpdate",
		);
		setState((prev) => ({ ...prev, isUpdating: true }));
