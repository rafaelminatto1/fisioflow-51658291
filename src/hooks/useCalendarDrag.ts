...
export const useCalendarDrag = ({
	...
	const [pendingOverCapacity, setPendingOverCapacity] =
		useState<PendingOverCapacity | null>(null);
	...
	const handleDragStart = useCallback(
		(e: React.DragEvent, appointment: Appointment) => {
			...
			logger.debug(
				"[useCalendarDrag] handleDragStart",
				{
			...
			if (!onAppointmentReschedule) {
				logger.warn(
					"[useCalendarDrag] onAppointmentReschedule não está definido, drag não será iniciado",
					{},
					"useCalendarDrag",
				);
				return;
	...
	const handleDragOver = useCallback(
		(e: React.DragEvent, date: Date, time: string) => {
			...
			const otherApps = getAppointmentsForSlot
				? getAppointmentsForSlot(date, time).filter(
						(apt) => apt.id !== dragState.appointment?.id,
					)
				: [];

			lastDropTargetKeyRef.current = dropKey;
			const nextDropState: DropState = {
				dropTarget: { date, time },
				targetAppointments: otherApps,
			};
	...
	const handleDrop = useCallback(
		(e: React.DragEvent, targetDate: Date, time: string) => {
			e.preventDefault();
			e.stopPropagation();
			...
			if (!appointmentToMove) {
				try {
					...
					if (data) {
						...
						if (
							...
							appointmentToMove = {
								...
								patientId: "",
								status:
									"agendado" as import("@/types/appointment").AppointmentStatus,
					...
					logger.warn(
						"[useCalendarDrag] Failed to parse dataTransfer",
						err,
						"useCalendarDrag",
					);
				}
			...
			if (getAppointmentsForSlot && getMinCapacityForInterval) {
				const slotAppointments = getAppointmentsForSlot(newDate, time).filter(
					(apt) => apt.id !== appointmentToMove.id,
				);
				const activeAppointmentsCount = slotAppointments.filter((apt) =>
					isCapacityCountedStatus(apt.status),
				).length;
	...
	const executeReschedule = useCallback(
		async (payload: PendingReschedule) => {
			...
			try {
				...
				if (conflictMessage) {
					...
					toast.error(
						"Erro ao reagendar: não foi possível salvar o agendamento.",
					);
				}
