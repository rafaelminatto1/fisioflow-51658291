...
>(
  ...
  return useMutation({
    ...
    onMutate: async (variables) => {
      ...
      await Promise.all(updateQueries.map((queryKey) => queryClient.cancelQueries({ queryKey })));
 ...
 */
export function useCreateMutation<TData, TVariables = void>(options: {
  mutationFn: (variables: TVariables) => Promise<TData>;
  invalidateQueries?: string[][];
  successMessage?: string;
  errorMessage?: string;
 ...
 */
export function useUpdateMutation<TData, TVariables = void>(options: {
  mutationFn: (variables: TVariables) => Promise<TData>;
  invalidateQueries?: string[][];
  successMessage?: string;
  errorMessage?: string;
      ...
      options.updateQueries?.forEach((queryKey) => {
        ...
        if (Array.isArray(previousData) && options.findItem && options.getOptimisticUpdate) {
          const optimisticData = previousData.map((item) => {
            ...
            return item === targetItem ? options.getOptimisticUpdate!(variables)(item) : item;
          });
          queryClient.setQueryData(queryKey, optimisticData);
        }
      });
 ...
 */
export function useDeleteMutation<TData, TVariables = void>(options: {
  mutationFn: (variables: TVariables) => Promise<TData>;
  invalidateQueries?: string[][];
  successMessage?: string;
  errorMessage?: string;
