	...
	return useMutation({
		...
		onSuccess: (_data, variables) => {
			queryClient.invalidateQueries({
				queryKey: ["pain-maps", "session", variables.sessionId],
			});
...
export function useComparePainMaps(
	patientId: string | undefined,
	mapIds: [string, string] | null,
) {
	return useQuery({
		...
		queryFn: async () => {
			...
			const maps = [r1?.data ?? r1, r2?.data ?? r2].filter(
				Boolean,
			) as PainMap[];
		...
		queryFn: async () => {
			...
			return maps.map((map) => {
				const rawPoints = Array.isArray(map.pain_points)
					? (map.pain_points as RawPainPoint[])
					: [];
				const totalIntensity = rawPoints.reduce(
					(sum, p) => sum + (p.intensity || 0),
					0,
				);
				const avgIntensity =
					rawPoints.length > 0
...
export function usePainStatistics(patientId: string | undefined) {
	return useQuery({
		...
		queryFn: async () => {
			...
			allPoints.forEach((p) => {
				const t = p.pain_type || p.painType || "unknown";
			...
			const firstMap = maps[0],
				lastMap = maps[maps.length - 1];
			const fp = Array.isArray(firstMap.pain_points)
				? (firstMap.pain_points as RawPainPoint[])
				: [];
			const lp = Array.isArray(lastMap.pain_points)
				? (lastMap.pain_points as RawPainPoint[])
				: [];
			const fa =
				fp.length > 0
					? fp.reduce((s, p) => s + (p.intensity || 0), 0) / fp.length
					: firstMap.global_pain_level || 0;
			const la =
				lp.length > 0
					? lp.reduce((s, p) => s + (p.intensity || 0), 0) / lp.length
					: lastMap.global_pain_level || 0;
			const trend = fa > la ? "improving" : fa < la ? "worsening" : "stable";
			const totalIntensity = allPoints.reduce(
				(sum, p) => sum + (p.intensity || 0),
				0,
			);
			const averagePainLevel =
				allPoints.length > 0
					...
					: maps.reduce((sum, m) => sum + (m.global_pain_level || 0), 0) /
						maps.length;
...
export function usePainMaps(options: {
	...
	const createPainMap = useCallback(
		async (view: "front" | "back", points: Omit<PainPoint, "id">[]) => {
			if (!sessionId) {
