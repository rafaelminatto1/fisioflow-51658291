...
import { sessionsApi, sessionAttachmentsApi, sessionTemplatesApi } from '@/lib/api/workers-client';
...
export const useInfiniteSoapRecords = (patientId: string, limitValue = 20) => {
  return useInfiniteQuery({
    ...
    initialPageParam: 0,
    enabled: !!patientId,
    getNextPageParam: (_lastPage, _allPages, lastPageParam) => (lastPageParam as number) + 1,
    staleTime: 1000 * 60 * 5,
  });
};
...
export const useCreateSoapRecord = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({ title: 'Evolução salva', description: 'A evolução do paciente foi registrada com sucesso.' });
    },
    onError: (error, _variables, context) => {
      if (context?.previousLists) queryClient.setQueryData(soapKeys.all, context.previousLists);
      toast({
  ...
  return useMutation({
    ...
    onMutate: async ({ recordId, data }) => {
      ...
      queryClient.setQueryData(
        ...
        (old: SoapRecord | undefined) =>
          ({ ...old, ...data, updated_at: new Date().toISOString() }) as SoapRecord,
      );
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: soapKeys.list(data.patient_id) });
      queryClient.setQueryData(soapKeys.detail(data.id), data);
      toast({ title: 'Evolução atualizada', description: 'A evolução foi atualizada com sucesso.' });
    },
    onError: (error, _variables, context) => {
...
export const useSignSoapRecord = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.setQueryData(soapKeys.detail(data.id), data);
      toast({ title: 'Evolução finalizada', description: 'A evolução foi finalizada e assinada com sucesso.' });
    },
...
export const useDeleteSoapRecord = () => {
  ...
  return useMutation({
    ...
    onSuccess: (result) => {
      ...
      toast({ title: 'Evolução excluída', description: 'A evolução foi excluída com sucesso.' });
    },
    onError: (error: unknown) => {
      toast({
...
export const useSessionAttachments = (soapRecordId?: string, _patientId?: string) => {
  return useQuery({
...
export const useUploadSessionAttachment = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!soapRecordId) throw new SoapOperationError('soapRecordId é obrigatório', 'VALIDATION');
    ...
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({
        queryKey: soapKeys.attachments(variables.soapRecordId, variables.patientId),
      });
    },
    onError: (error: unknown) => {
      toast({
...
export const useDeleteSessionAttachment = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (fileUrl) {
        try { await deleteFromR2(fileUrl); } catch { /* ignore */ }
      }
...
export const useCreateSessionTemplate = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: 'Template criado', description: 'O template de sessão foi criado com sucesso.' });
    },
    onError: (error: unknown) => {
      toast({
