...
export function useAppointmentPositioning(
   ...
   * Calcula a altura para uma duração
   */
  const getDurationHeight = useCallback(
  ...
  const hasOverlap = useCallback(
    (appointment1: Appointment, appointment2: Appointment): boolean => {
  ...
  const getAppointmentStyle = useCallback(
    (appointment: Appointment, weekDays: Date[], timeSlots: string[]): CSSProperties | null => {
      const aptDate = parseAppointmentDate(appointment.date);
