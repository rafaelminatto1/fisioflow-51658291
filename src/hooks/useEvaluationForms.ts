...
export function useDuplicateEvaluationForm() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      ...
      const duplicatedId = (duplicated?.data?.id ?? duplicated?.data?.data?.id) as
        | string
        | undefined;
...
export const patientEvaluationKeys = {
  ...
  byPatient: (patientId?: string) => ["patient-evaluations", "patient", patientId] as const,
  detail: (responseId?: string) => ["patient-evaluations", "detail", responseId] as const,
};
  ...
  return useMutation({
    mutationFn: async ({
      ...
      Pick<
        ...
        "appointment_id" | "responses" | "status" | "scheduled_for" | "started_at" | "completed_at"
      >
    > & { id: string }) => {
