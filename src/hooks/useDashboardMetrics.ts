  ...
  return useQuery({
    ...
    queryFn: async (): Promise<DashboardMetrics> => {
      ...
      const primaryDateFrom =
        period === "semana" ? weekStart : period === "mes" ? startCurrentMonth : today;
      ...
      const [
        ...
        financialApi.pagamentos.list({ limit: 1000, offset: 0 }).catch((error) => {
      ...
      return {
        ...
        tendenciaSemanal,
        ticketMedio:
          totalSessions30d > 0 ? Math.round((receitaMensal / totalSessions30d) * 100) / 100 : 0,
        agendamentosSemana,
        cancelamentosSemana,
        periodLabel: period === "semana" ? "Esta Semana" : period === "mes" ? "Mês Atual" : "Hoje",
      };
    },
  });
};
