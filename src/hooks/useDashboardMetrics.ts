	...
	return useQuery({
		...
		queryFn: async (): Promise<DashboardMetrics> => {
			...
			const primaryDateFrom =
				period === "semana"
					...
					: period === "mes"
						? startCurrentMonth
						: today;
			const primaryDateTo = period === "semana" ? weekEnd : today;
			...
			const [
				...
				patientsApi.list({ limit: 1000, offset: 0 }),
				...
				profileApi.listTherapists().catch((error) => {
					logger.warn(
						"Falha ao carregar fisioterapeutas para dashboard",
						error,
						"useDashboardMetrics",
					);
			...
			const agendamentosRestantes = Math.max(
				0,
			...
			const contasMesAtual = contas.filter((r) =>
				isDateInRange(
			...
			const contasMesAnterior = contas.filter((r) =>
				isDateInRange(
					r.pago_em ?? r.data_vencimento,
					startLastMonthDate,
					endLastMonthDate,
			...
			const pagamentosMesAtual = pagamentos.filter((r) =>
				isDateInRange(r.pago_em, startCurrentMonthDate, now),
			);
			const pagamentosMesAnterior = pagamentos.filter((r) =>
				isDateInRange(r.pago_em, startLastMonthDate, endLastMonthDate),
			);
			...
			const receitaMensal =
				receitaMensalContas > 0 ? receitaMensalContas : receitaMensalPagamentos;
			const receitaMesAnterior =
				receitaMesAnteriorContas > 0
					? receitaMesAnteriorContas
					: receitaMesAnteriorPagamentos;

			const therapistById = new Map(therapists.map((t) => [t.id, t.name]));

			const fisioStats = new Map<string, TherapistPerformance>();
			const monthCompletedAppointments = appointmentsMonth.filter((a) =>
				isCompletedStatus(a.status),
			);
			...
			const appointmentsById = new Map(
				monthCompletedAppointments.map((a) => [a.id, a]),
			);
			...
			const receitaPorFisioterapeuta = Array.from(fisioStats.values())
				.map((item) => ({
					...
					taxaOcupacao:
						Math.round(
							(item.atendimentos / Math.max(1, agendamentosHoje)) * 1000,
						) / 10,
				}))
				.sort(
					(a, b) => b.receita - a.receita || b.atendimentos - a.atendimentos,
				)
				.slice(0, 5);
			...
			const tendenciaSemanal: WeeklyTrend[] = weekDays.map((dia, i) => {
				...
				const dayApts = appointmentsWeek.filter(
					(a) => safeDay(a.date) === dayStr,
				);
			...
			const cancelamentosSemana = appointmentsWeek.filter((a) =>
				isCancelledStatus(a.status),
			).length;
			const totalSessions30d = appointments30d.filter((a) =>
				isCompletedStatus(a.status),
			).length;
			...
			return {
				...
				taxaNoShow:
					totalAppointments30d > 0
						? Math.round((noShowCount / totalAppointments30d) * 1000) / 10
						: 0,
				taxaOcupacao:
					Math.round((agendamentosHoje / capacidadeBase) * 1000) / 10,
				receitaMensal,
				receitaMesAnterior,
				crescimentoMensal:
				...
				mediaSessoesPorPaciente:
					pacientesAtivos > 0
						? Math.round((totalSessions30d / pacientesAtivos) * 10) / 10
						: 0,
				...
				ticketMedio:
					totalSessions30d > 0
						? Math.round((receitaMensal / totalSessions30d) * 100) / 100
						: 0,
				agendamentosSemana,
				cancelamentosSemana,
				periodLabel:
