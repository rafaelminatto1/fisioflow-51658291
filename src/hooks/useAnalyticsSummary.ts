	...
	const {
		...
		queryFn: async () => {
			...
			try {
				...
				logger.error(
					"Erro ao carregar analytics summary",
					err,
					"useAnalyticsSummary",
				);
				return defaultSummary;
...
function calculateStats(
	...
	return {
		...
		appointmentGrowth: lastCount
			? Math.round(((currentCount - lastCount) / lastCount) * 100)
			: 0,
		activePatients: activePatientsSet.size,
		patientGrowth: lastPatientsSet.size
			? Math.round(
					((activePatientsSet.size - lastPatientsSet.size) /
						lastPatientsSet.size) *
						100,
				)
			: 0,
		monthlyRevenue: monthlyRev,
		revenueGrowth: lastRev
			? Math.round(((monthlyRev - lastRev) / lastRev) * 100)
			: 0,
