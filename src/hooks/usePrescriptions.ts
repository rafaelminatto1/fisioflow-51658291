...
export const usePrescriptions = (patientId?: string) => {
  ...
  const { data: prescriptions = [], isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const res = await clinicalApi.prescriptions.list(patientId ? { patientId } : undefined);
  ...
  const createMutation = useMutation({
    ...
    onError: (error: Error) => toast.error('Erro ao criar prescrição: ' + error.message),
  });

  const updateMutation = useMutation({
    mutationFn: async ({ id, ...updates }: Partial<Prescription> & { id: string }) => {
      const res = await clinicalApi.prescriptions.update(id, updates as Partial<ExercisePrescription>);
      return (res?.data ?? res) as Prescription;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['prescriptions'] });
...
export const usePublicPrescription = (qrCode: string) => {
  ...
  const { data: prescription, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      clinicalPublicApi.prescriptions.updateByQr(qrCode, {
  ...
  const markExerciseComplete = useMutation({
    mutationFn: async ({ exerciseId }: { prescriptionId: string; exerciseId: string }) => {
      const current = prescription?.completed_exercises ?? [];
      const isCompleted = current.includes(exerciseId);
      const newCompleted = isCompleted ? current.filter((id) => id !== exerciseId) : [...current, exerciseId];

      await clinicalPublicApi.prescriptions.updateByQr(qrCode, { completed_exercises: newCompleted });
      return newCompleted;
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['public-prescription', qrCode] }),
  });
