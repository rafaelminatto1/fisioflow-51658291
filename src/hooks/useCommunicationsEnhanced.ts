...
import { PatientCommunicationService } from "../lib/skills/fase4-conteudo/patient-communication";
...
export function useCommunicationsEnhanced(
	options?: UseCommunicationsEnhancedOptions,
) {
	...
	const sendExerciseReminder = useCallback(
		async (data: {
			...
			try {
				...
				const error =
					err instanceof Error ? err : new Error("Erro ao enviar lembrete");
				setError(error);
				if (options?.onError) {
	...
	const sendSatisfactionSurvey = useCallback(
		async (survey: {
			...
			try {
				...
				const error =
					err instanceof Error ? err : new Error("Erro ao enviar pesquisa");
				setError(error);
				if (options?.onError) {
					options.onError(error);
				}
	...
	const sendBirthdayMessage = useCallback(
		async (patient: { name: string; phone: string; clinicName: string }) => {
			setIsSending(true);
			setError(null);
			...
			try {
				...
				const error =
					err instanceof Error
						? err
						: new Error("Erro enviar mensagem de aniversário");
				setError(error);
				if (options?.onError) {
					options.onError(error);
				}
	...
	const sendCampaign = useCallback(
		async (campaign: {
			...
			try {
				...
				const error =
					err instanceof Error ? err : new Error("Erro ao enviar campanha");
				setError(error);
				if (options?.onError) {
					options.onError(error);
				}
	...
	const sendAppointmentConfirmation = useCallback(
		async (appointment: {
			...
			try {
				...
				const error =
					err instanceof Error ? err : new Error("Erro ao enviar confirmação");
				setError(error);
				if (options?.onError) {
					options.onError(error);
				}
