...
export function useMedicalReturnsUpcoming(daysAhead: number = DEFAULT_DAYS_AHEAD) {
  ...
  const upcoming = useMemo(() => {
    ...
    for (const p of patients) {
      ...
      const name =
        (p as Record<string, unknown>).full_name ?? (p as Record<string, unknown>).name ?? "";
      items.push({
        ...
        referring_doctor_name: (p as Record<string, unknown>).referring_doctor_name as
          | string
          | undefined,
        referring_doctor_phone: (p as Record<string, unknown>).referring_doctor_phone as
          | string
          | undefined,
        medical_report_done: (p as Record<string, unknown>).medical_report_done as
          | boolean
          | undefined,
        medical_report_sent: (p as Record<string, unknown>).medical_report_sent as
          | boolean
          | undefined,
      });
    }
