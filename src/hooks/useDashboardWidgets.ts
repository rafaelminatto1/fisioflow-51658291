...
export function useDashboardWidgets() {
      ...
      if (saved) {
        try {
          ...
          if (
            ...
            parsed.every((w) => w.id && w.type && w.title && typeof w.position === "number")
          ) {
            setWidgets(parsed);
          } else {
            logger.warn(
              "Invalid widget structure, using defaults",
              undefined,
              "useDashboardWidgets",
            );
          }
          ...
          logger.error(
            "Failed to parse widgets, using defaults",
            parseError,
            "useDashboardWidgets",
          );
        }
  ...
  const toggleWidget = (widgetId: string) => {
    const newWidgets = widgets.map((w) => (w.id === widgetId ? { ...w, visible: !w.visible } : w));
