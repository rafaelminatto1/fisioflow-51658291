...
export function parseUpdatesToAppointment(updates: Partial<AppointmentFormData>): Partial<AppointmentBase> {
  ...
  if (updates.appointment_date || updates.date) {
    ...
    result.date = typeof dateStr === 'string' ? parseResponseDate(dateStr) : dateStr as Date;
  }
