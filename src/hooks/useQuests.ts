...
const toQuestDefinition = (row: Partial<QuestDefinitionRow>): QuestDefinition => ({
  ...
  repeat_interval: (row.repeat_interval as QuestDefinition['repeat_interval']) || 'daily',
});
...
export const useQuests = (patientId?: string): UseQuestsResult => {
  ...
  const availableQuests = questRows.filter((row) => row.is_active).map(toQuestDefinition);

  const { data: patientQuests = [], isLoading: loadingPatientQuests, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const daily: PatientQuest[] = (dailyRes.data?.quests_data ?? []).map((quest) => ({
        id: quest.id,
        patient_id: patientId,
        quest_id: quest.id,
        status: quest.completed ? 'completed' : 'in_progress',
        progress: { current: quest.completed ? 1 : 0, target: 1 },
        started_at: dailyRes.data?.date ? `${dailyRes.data.date}T00:00:00.000Z` : null,
        completed_at: quest.completed ? new Date().toISOString() : null,
        expires_at: dailyRes.data?.date ? `${dailyRes.data.date}T23:59:59.999Z` : null,
        quest_definition: {
          id: quest.id,
          code: quest.id,
          title: quest.title,
      ...
      const weekly: PatientQuest[] = (challengesRes.data ?? []).map((challenge) => ({
        id: `challenge-${challenge.id}`,
        patient_id: patientId,
        quest_id: challenge.id,
        status: challenge.patient_progress?.completed ? 'completed' : 'in_progress',
        progress: { current: challenge.patient_progress?.progress || 0, target: challenge.target?.count || 1 },
        started_at: challenge.start_date ? `${challenge.start_date}T00:00:00.000Z` : null,
        completed_at: challenge.patient_progress?.completed_at || null,
        expires_at: challenge.end_date ? `${challenge.end_date}T23:59:59.999Z` : null,
        quest_definition: {
          id: challenge.id,
          code: challenge.id,
  ...
  const dailyQuests = patientQuests.filter((q) => q.quest_definition?.category === 'daily');
  const weeklyQuests = patientQuests.filter((q) => q.quest_definition?.category === 'weekly');

  const startQuestMutation = useMutation({
    mutationFn: async (_questId: string) => undefined,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['patient-quests', patientId] });
      toast({ title: 'Quest iniciada!', description: 'A missão foi marcada como ativa.' });
    },
    onError: (error: Error) => toast({ title: 'Erro ao iniciar quest', description: error.message, variant: 'destructive' }),
  });
  ...
  const claimRewardMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: 'Recompensa recebida!', description: 'A missão foi concluída e o XP foi aplicado.' });
    },
    onError: (error: Error) => toast({ title: 'Erro ao reclamar recompensa', description: error.message, variant: 'destructive' }),
  });

  const refreshQuests = async () => {
    await queryClient.invalidateQueries({ queryKey: ['patient-quests', patientId] });
    toast({ title: 'Quests atualizadas', description: 'Dados de missões recarregados.' });
  };
