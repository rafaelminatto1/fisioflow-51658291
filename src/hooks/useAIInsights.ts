...
function buildClinicalInsightsPrompt(options: AIInsightOptions): string {
  ...
  const { progress_summary, pain_trend, function_trend, risk_score, predictions, goals } =
    analyticsData;
...
export function useAIInsights(options: AIInsightOptions) {
  ...
  const prompt = useMemo(
    ...
    [options, options.patientId, options.patientName, options.analyticsData, options.language],
  );
