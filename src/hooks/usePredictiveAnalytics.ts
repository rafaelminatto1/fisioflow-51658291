...
const mapPredictionToRecoveryPrediction = (
  prediction: PatientPrediction &
    Partial<
      Record<"milestones" | "risk_factors" | "treatment_recommendations" | "similar_cases", unknown>
    > &
    Record<string, unknown>,
): RecoveryPrediction => {
  const features = prediction.features ?? {};
  const confidenceRaw =
    prediction.confidence_interval ?? (prediction as any).confidenceInterval ?? {};
  const interval = {
    ...
    expectedDays:
      Number(confidenceRaw?.expectedDays ?? confidenceRaw?.expected_days ?? 0) ||
      Number(prediction.predicted_value ?? 0),
    upperDays:
  ...
  return {
    ...
    treatmentRecommendations: {
      optimalFrequency: rawTreatment?.optimal_frequency ?? rawTreatment?.optimalFrequency ?? "",
      sessionsPerWeek: Number(
        rawTreatment?.sessions_per_week ?? rawTreatment?.sessionsPerWeek ?? 0,
      ),
      estimatedTotalSessions: Number(
        rawTreatment?.estimated_total_sessions ?? rawTreatment?.estimatedTotalSessions ?? 0,
      ),
      intensity: (rawTreatment?.intensity ??
        (prediction as Record<string, any>).intensity ??
        "moderate") as "low" | "moderate" | "high",
      focusAreas: ensureArray<string>(rawTreatment?.focus_areas ?? rawTreatment?.focusAreas),
    },
    similarCases: {
      totalAnalyzed: Number(rawSimilarCases?.total_analyzed ?? rawSimilarCases?.totalAnalyzed ?? 0),
      matchingCriteria:
        rawSimilarCases?.matching_criteria ?? rawSimilarCases?.matchingCriteria ?? [],
      averageRecoveryTime: Number(
        rawSimilarCases?.average_recovery_time ?? rawSimilarCases?.averageRecoveryTime ?? 0,
      ),
      successRate: Number(rawSimilarCases?.success_rate ?? rawSimilarCases?.successRate ?? 0),
      keyInsights: rawSimilarCases?.key_insights ?? rawSimilarCases?.keyInsights ?? [],
    },
    modelVersion: prediction.model_version ?? "unknown",
    dataSource: (prediction as any).data_source ?? (prediction as any).dataSource ?? "historical",
    requiresValidation:
...
export function useMilestonesProgress(patientId: string) {
  ...
  const data = {
    ...
    progressPercentage:
      milestones.length > 0 ? Math.round((achievedCount / milestones.length) * 100) : 0,
  };
