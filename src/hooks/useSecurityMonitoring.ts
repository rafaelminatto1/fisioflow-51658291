...
function isLoginAction(action: string | undefined): boolean {
  ...
  return normalized.includes('login') || normalized.includes('signin') || normalized.includes('auth');
}

function isSuccessfulLogin(action: string | undefined, changes: unknown): boolean {
  const normalized = (action ?? '').toLowerCase();
...
export function useSecurityMonitoring() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      recentAttempts
        .filter((attempt) => !attempt.success && new Date(attempt.created_at).getTime() >= oneHourAgo)
        .forEach((attempt) => {
          ...
          current.last_attempt =
            new Date(attempt.created_at) > new Date(current.last_attempt)
              ? attempt.created_at
              : current.last_attempt;
          if (attempt.ip_address && !current.ip_addresses.includes(attempt.ip_address)) {
            current.ip_addresses.push(attempt.ip_address);
          }
          grouped.set(attempt.email, current);
        });

      const suspiciousActivity = Array.from(grouped.values()).filter((item) => item.failed_attempts >= 3);
