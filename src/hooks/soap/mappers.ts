...
export const toEvolutionRecord = (record: Record<string, unknown>): EvolutionRecord => ({
  ...
  session_number:
    record.session_number != null ? Number(record.session_number) : undefined,
  ...
  duration_minutes:
    record.duration_minutes != null ? Number(record.duration_minutes) : undefined,
  ...
  record_date: String(
    record.record_date ?? new Date().toISOString().slice(0, 10),
  ),
