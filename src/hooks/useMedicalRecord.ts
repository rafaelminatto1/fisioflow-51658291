...
export const medicalRecordKeys = {
	...
	anamnesis: (patientId: string) =>
		[...medicalRecordKeys.all, "anamnesis", patientId] as const,
	examinations: (patientId: string) =>
		[...medicalRecordKeys.all, "examinations", patientId] as const,
	treatmentPlans: (patientId: string) =>
		[...medicalRecordKeys.all, "treatment-plans", patientId] as const,
	attachments: (patientId: string) =>
		[...medicalRecordKeys.all, "attachments", patientId] as const,
	history: (patientId: string) =>
		[...medicalRecordKeys.all, "history", patientId] as const,
	summary: (patientId: string) =>
		[...medicalRecordKeys.all, "summary", patientId] as const,
} as const;
...
export function useSaveAnamnesis() {
	...
	return useMutation({
		mutationFn: async ({
			...
			if (!user)
				throw new MedicalRecordError("Usuário não autenticado", "UNAUTHORIZED");
		...
		onSuccess: (data, variables) => {
			queryClient.invalidateQueries({
				queryKey: medicalRecordKeys.anamnesis(variables.patientId),
			});
			toast({
		...
		onError: (error) => {
			toast({
				...
				description:
					error instanceof MedicalRecordError
						? error.message
						: "Erro desconhecido",
				variant: "destructive",
			});
...
export function useDeleteAnamnesis() {
	...
	return useMutation({
		...
		onSuccess: (data) => {
			queryClient.invalidateQueries({
				queryKey: medicalRecordKeys.anamnesis(data.patientId),
			});
			toast({
...
export function useSavePhysicalExamination() {
	...
	return useMutation({
		mutationFn: async ({
			...
			return medicalRecordService.savePhysicalExamination(
				patientId,
				data,
				profileId,
			);
		...
		onSuccess: (data, variables) => {
			queryClient.invalidateQueries({
				queryKey: medicalRecordKeys.examinations(variables.patientId),
			});
			toast({
...
export function useSaveTreatmentPlan() {
	...
	return useMutation({
		...
		onSuccess: (data, variables) => {
			queryClient.invalidateQueries({
				queryKey: medicalRecordKeys.treatmentPlans(variables.patientId),
			});
			toast({
...
export function useDeleteAttachment() {
	...
	return useMutation({
		...
		onSuccess: (data) => {
			queryClient.invalidateQueries({
				queryKey: medicalRecordKeys.attachments(data.patientId),
			});
			toast({
...
export function useConsultationHistory(
	...
	return useQuery({
		...
		queryFn: () =>
			medicalRecordService.getConsultationHistory(
				patientId,
				startDate,
				endDate,
			),
...
export function useMedicalRecordSummary(
	...
	return useQuery({
		...
		queryFn: () =>
			medicalRecordService.generateMedicalRecordSummary(patientId, patientData),
...
export function useExportMedicalRecord() {
	...
	const exportAsHTML = async (
		...
		try {
			const summary = await medicalRecordService.generateMedicalRecordSummary(
				patientId,
				patientData,
			);
