...
export function useSmartDashboardData(viewMode: ViewMode = "today") {
  ...
  const { isLoading: isLoadingPagamentos } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        logger.error("Error loading pagamentos", { error }, "useSmartDashboard");
        return [];
      }
    },
    enabled: !!organizationId,
  ...
  const metrics = useMemo<DashboardMetrics>(() => {
    ...
    const metrics: DashboardMetrics = {
      ...
      patientsAtRisk: atRiskPatients.length || adData?.patientsAtRisk || 0,
      ...
      evolutionChart: (() => {
        const data = selfAssessments
          .slice(0, 10)
          .reverse()
          .map((s: PatientSelfAssessment, idx: number) => ({
            day: idx + 1,
