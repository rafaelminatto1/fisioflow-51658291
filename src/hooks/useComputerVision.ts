...
export const exerciseTemplates: Record<string, ExerciseTemplate> = {
	squat: {
		...
		difficulty: "beginner",
		phases: [
			{ keyPositions: ["Costas retas", "Joelhos alinhados", "Descer até 90°"] },
		],
	},
	pushup: {
		id: "pushup",
...
export const useComputerVision = () => {
	...
	const [currentSession, setCurrentSession] = useState<ExerciseSession | null>(
		null,
	);
	...
	const { load: loadMediaPipe, isLoaded: mediaPipeLoaded } =
		useMediaPipeVision();
	...
	const processFrame = useCallback(() => {
		...
		if (result.landmarks && result.landmarks.length > 0) {
			...
			if (settings.showSkeleton) {
				...
				const feedbackColor = getBiomechanicalFeedbackColor(
					landmarks,
					currentExercise || "",
				);
				drawSkeleton(
			...
			if (currentExercise === "squat") {
				...
				if (hip && knee && ankle) {
					...
					if (settings.showAngles)
						drawAngle(ctx, knee, angle, canvas.width, canvas.height);
				}
				...
				if (shoulder && elbow && wrist) {
					...
					if (settings.showAngles)
						drawAngle(ctx, elbow, angle, canvas.width, canvas.height);
				}
				...
				if (hip && shoulder && elbow) {
					...
					if (settings.showAngles)
						drawAngle(ctx, shoulder, angle, canvas.width, canvas.height);
				}
			}
			...
			if (state.reps !== currentSession?.totalRepetitions) {
				setCurrentSession((prev) =>
					prev ? { ...prev, totalRepetitions: state.reps } : null,
				);
			}
	...
	const getBiomechanicalFeedbackColor = (
		landmarks: any,
		exercise: string,
	): string => {
