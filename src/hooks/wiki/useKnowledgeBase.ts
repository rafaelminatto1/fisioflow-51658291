...
export function useKnowledgeBase(currentOrganizationId?: string | null, currentUserId?: string | null) {
  ...
  return {
    ...
    knowledgeStats: {
      ...
      verified: (knowledgeArticles.length > 0 ? knowledgeArticles : knowledgeBase).filter(item => item.status === 'verified').length,
    },
    kbFilters: {
      query: kbQuery,
      group: kbGroup,
