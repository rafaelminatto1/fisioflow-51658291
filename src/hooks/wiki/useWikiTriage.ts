...
export function useWikiTriage(
	...
	const [triageTemplateFilter, setTriageTemplateFilter] =
		useState<string>("all");
	...
	const triageOwnerOptions = useMemo(
		() =>
			Array.from(
				new Set(triagePages.map((page) => page.created_by).filter(Boolean)),
			).sort(),
		[triagePages],
	);
	...
	const triageMetrics = useMemo(() => {
		...
		const avgInProgress = calculateAverageTimeInColumnDays(
			triagePages,
			"in-progress",
		);
	...
	const wipAlerts = useMemo(
		() => ({
			...
			"in-progress":
				triageBuckets["in-progress"].length > TRIAGE_WIP_LIMITS["in-progress"],
	...
	const buildTriageEventPayloads = (
		...
		return updates
			.map((update) => {
				...
				const afterStatus = getTriageStatus({ tags: update.tags } as Pick<
					WikiPage,
					"tags"
				>);
	...
	const handleTriageDragEnd = async (result: DropResult) => {
		...
		try {
			...
			await wikiService.recordTriageEvents(
				currentOrganizationId,
				currentUserId,
				events,
			);
	...
	const handleQuickStatusChange = async (
		...
		try {
			...
			const events = buildTriageEventPayloads(
				updates,
				"quick-action",
				"quick-status-change",
			);
