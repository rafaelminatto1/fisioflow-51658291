...
import { clinicalApi, type ConductLibraryRecord } from "@/api/v2";
...
export const useConductLibrary = (category?: string) =>
	useQuery({
		...
		queryFn: async () => {
			const res = await clinicalApi.conductLibrary.list(
...
export const useCreateConduct = () => {
	...
	return useMutation({
		...
		onError: (error: unknown) => {
			toast.error(
				error instanceof Error ? error.message : "Erro ao salvar conduta",
			);
		},
	});
};
...
export const useUpdateConduct = () => {
	...
	return useMutation({
		mutationFn: async ({
			id,
			...data
		}: Partial<CreateConductData> & { id: string }) => {
		...
		onError: (error: unknown) => {
			toast.error(
				error instanceof Error ? error.message : "Erro ao atualizar conduta",
			);
		},
	});
};
...
export const useDeleteConduct = () => {
	...
	return useMutation({
		...
		onError: (error: unknown) => {
			toast.error(
				error instanceof Error ? error.message : "Erro ao remover conduta",
			);
		},
	});
};
