...
import { clinicalApi, type EvolutionTemplate } from '@/lib/api/workers-client';
...
export function useCreateEvolutionTemplate() {
  ...
  return useMutation({
    ...
    onMutate: async (newTemplate) => {
      ...
      const optimistic = { ...newTemplate, id: `temp-${Date.now()}`, created_at: new Date().toISOString(), updated_at: new Date().toISOString() } as EvolutionTemplate;
      queryClient.setQueryData<EvolutionTemplate[]>(['evolution-templates'], (old) => [...(old ?? []), optimistic]);
...
export function useUpdateEvolutionTemplate() {
  ...
  return useMutation({
    ...
    onMutate: async ({ id, ...template }) => {
      ...
      const previousTemplates = queryClient.getQueryData<EvolutionTemplate[]>(['evolution-templates']);
      queryClient.setQueryData<EvolutionTemplate[]>(['evolution-templates'], (old) =>
        old?.map((t) => (t.id === id ? { ...t, ...template, updated_at: new Date().toISOString() } : t)) ?? [],
      );
      return { previousTemplates };
    },
    onError: (_err, _vars, context) => {
...
export function useDeleteEvolutionTemplate() {
  ...
  return useMutation({
    ...
    onMutate: async (id) => {
      await queryClient.cancelQueries({ queryKey: ['evolution-templates'] });
      const previousTemplates = queryClient.getQueryData<EvolutionTemplate[]>(['evolution-templates']);
      queryClient.setQueryData<EvolutionTemplate[]>(['evolution-templates'], (old) => (old ?? []).filter((t) => t.id !== id));
      return { previousTemplates };
    },
