  ...
  return useMutation({
    mutationFn: async (code: string): Promise<{ success: boolean; email?: string }> => {
      try {
        ...
        return { success: true, email: result.data?.external_email ?? undefined };
      } catch (error) {
...
export function useSyncToGoogle() {
  ...
  return useMutation({
    mutationFn: async (appointment: Appointment): Promise<SyncResult> => {
      ...
      const result = await integrationsApi.google.calendar.syncAppointment(appointment as unknown as Record<string, unknown>);
      return {
