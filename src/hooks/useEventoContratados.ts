...
type EventoSnapshot = Pick<
	Evento,
	"id" | "nome" | "data_inicio" | "data_fim" | "hora_inicio" | "hora_fim"
>;
...
async function checkContratadoConflict(params: {
	...
	const assignmentsRes = await eventoContratadosApi.list({
		contratadoId: params.contratadoId,
	});
	const assignments = (assignmentsRes?.data ?? []) as EventoContratado[];

	for (const assignment of assignments) {
		if (!assignment.evento_id || assignment.evento_id === params.eventoId)
			continue;

		try {
			const otherEventoRes = await eventosApi.get(assignment.evento_id);
			const otherEvento = (otherEventoRes?.data ??
				null) as EventoSnapshot | null;
			if (!otherEvento) continue;

			const otherRange = buildRange(
				otherEvento,
				assignment.horario_inicio,
				assignment.horario_fim,
			);
			...
			if (rangesOverlap(targetRange, otherRange)) {
				...
				throw new Error(
					`Conflito de horário: contratado já alocado em ${nome}.`,
				);
			}
		} catch (error) {
			if (
...
export function useCreateEventoContratado() {
	...
	return useMutation({
		mutationFn: async (data: EventoContratadoCreate) => {
			...
			const res = await eventoContratadosApi.create(
				data as Partial<EventoContratado>,
			);
			return (res?.data ?? res) as EventoContratado;
		},
		onSuccess: (created) => {
			queryClient.invalidateQueries({
				queryKey: ["evento-contratados", created.evento_id],
			});
			toast.success("Contratado vinculado ao evento com sucesso.");
		},
		onError: (error: Error) =>
			toast.error(`Erro ao vincular contratado: ${error.message}`),
	});
}
...
export function useUpdateEventoContratado() {
	...
	return useMutation({
		mutationFn: async (params: {
			...
			const res = await eventoContratadosApi.update(
				id,
				data as Partial<EventoContratado>,
			);
		...
		onSuccess: (updated) => {
			queryClient.invalidateQueries({
				queryKey: ["evento-contratados", updated.evento_id],
			});
			toast.success("Vínculo de contratado atualizado com sucesso.");
		},
		onError: (error: Error) =>
			toast.error(`Erro ao atualizar contratado: ${error.message}`),
	});
}
...
export function useDeleteEventoContratado() {
	...
	return useMutation({
		...
		onError: (error: Error) =>
			toast.error(`Erro ao remover contratado: ${error.message}`),
	});
}
