...
export function useSettingsState() {
  ...
  useEffect(() => {
    if (!user?.uid) return;
    mfaService.getMFASettings(user.uid).then(s => setMfaEnabled(s.enabled)).catch(() => {});
    
    const savedHours = localStorage.getItem('workingHours');
    if (savedHours) {
      try { setWorkingHours(JSON.parse(savedHours)); } catch (e) { logger.warn('Failed to parse working hours', e); }
    }
  }, [user?.uid]);
  ...
  useEffect(() => {
    ...
    accessibilitySettings.highContrast ? cl.add('high-contrast') : cl.remove('high-contrast');
    accessibilitySettings.reducedMotion ? cl.add('reduced-motion') : cl.remove('reduced-motion');
  ...
  return {
    ...
    mfa: {
      enabled: mfaEnabled, loading: mfaLoading, showModal: showMfaModal, setShowModal: setShowMfaModal,
      qrCode: mfaQrCode, secret: mfaSecret, verifyCode: mfaVerifyCode, setVerifyCode: setMfaVerifyCode,
      verifying: mfaVerifying, handleEnable: handleEnable2FA, verify: verifyMfa
    },
    notifications, setNotifications,
    workingHours, setWorkingHours, saveWorkingHours,
    accessibility: { settings: accessibilitySettings, setSettings: setAccessibilitySettings },
    password: {
      form: passwordForm, setForm: setPasswordForm,
      show: showPassword, setShow: setShowPassword,
      changing: isChangingPassword, change: changePassword,
