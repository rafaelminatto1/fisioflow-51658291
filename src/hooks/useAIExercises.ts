...
export const aiExercisesQueryKeys = {
	...
	suggestions: (patientId: string) =>
		["ai", "exercises", "suggestions", patientId] as const,
	lastRequest: (patientId: string) =>
		["ai", "exercises", "lastRequest", patientId] as const,
} as const;
		...
		mutationFn: async ({
			...
			try {
				...
				const response = exerciseLibrary
					? await assistant.suggestExercisesFromLibrary(
							patientContext,
							exerciseLibrary,
						)
				...
				if (!response.success) {
					throw new Error(
						response.error || "Failed to generate exercise suggestions",
					);
				}
...
export function useBatchAIExercises(
	...
	generateAll: (
		contexts: Record<string, PatientProfileContext>,
	) => Promise<void>;
} {
	const queryClient = useQueryClient();
