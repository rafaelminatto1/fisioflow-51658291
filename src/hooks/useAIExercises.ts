...
export interface UseAIExercisesReturn {
  ...
  generateSuggestions: (
    variables: ExerciseSuggestionVariables,
  ...
  const generateMutation = useMutation({
    ...
    onSuccess: (response) => {
      if (response.success && response.data) {
        ...
        queryClient.setQueryData(aiExercisesQueryKeys.suggestions(patientId), response.data);

        // Cache the timestamp
        queryClient.setQueryData(aiExercisesQueryKeys.lastRequest(patientId), Date.now());
      }
    },
    onError: (err: Error) => {
      setLastError(err);
...
export function useAIExercisesStale(
  ...
  const isStale = lastRequestTime ? Date.now() - lastRequestTime > staleTimeMs : true;
...
export function useBatchAIExercises(
  ...
  const suggestionsMap = Object.fromEntries(
    patientIds.map((id) => [
      id,
      queryClient.getQueryData<ExerciseProgramRecommendation>(aiExercisesQueryKeys.suggestions(id)),
    ]),
  );
 ...
 */
export function invalidateAllAIExercises(queryClient: ReturnType<typeof useQueryClient>): void {
  queryClient.invalidateQueries({
