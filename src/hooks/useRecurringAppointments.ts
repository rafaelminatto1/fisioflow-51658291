...
export function generateOccurrencesPreview(
  ...
  while (index < maxIterations) {
    ...
    if (endType === 'occurrences' && maxOccurrences && index >= maxOccurrences) break;

    if (shouldIncludeDate(currentDate, recurrence)) {
      occurrences.push({ date: currentDate, time, index, seriesId: formData.id || '' });
    }
...
export function useCreateRecurringSeries() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success(`Série recorrente criada — ${data.occurrences.length} agendamentos.`);
    },
    onError: (error: Error) => {
...
export function useCancelOccurrence() {
  ...
  return useMutation({
    mutationFn: async ({ occurrenceId }: { occurrenceId: string; reason?: string }) => {
...
export function useModifyOccurrence() {
  ...
  return useMutation({
    mutationFn: async ({ occurrenceId, modifications }: {
      occurrenceId: string;
      modifications: { duration?: number; notes?: string; time?: string; room_id?: string };
    }) => {
