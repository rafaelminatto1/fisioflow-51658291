...
const mockMedicalKnowledge: MedicalKnowledge = {
	...
	treatments: {
		fisioterapia_lombar: {
			...
			precautions: [
				"Não forçar movimentos dolorosos",
				"Comunicar qualquer piora",
			],
...
export const useMedicalChatbot = () => {
	...
	const startChatSession = useCallback(
		(userId: string, patientContext?: Partial<ChatContext>) => {
			...
			setQuickReplies([
				...
				},
				{
					id: "exercises",
					text: "Ver exercícios",
					payload: "show_exercises",
					icon: "🏃‍♂️",
				},
				{
					id: "symptoms",
					text: "Relatar sintomas",
					payload: "report_symptoms",
					icon: "🩺",
				},
	...
	const processUserMessage = useCallback(
		async (content: string, type: "text" | "quick_reply" = "text") => {
	...
	const generateBotResponse = async (
		...
		if (
			...
			input.includes("horário")
		) {
			return {
				message:
					"Vou ajudá-lo a agendar uma consulta. Qual tipo de atendimento você precisa?",
				confidence: 0.9,
				...
				quickReplies: [
					...
					},
					{
						id: "fisio_ortopedic",
						text: "Ortopédica",
						payload: "schedule_orthopedic",
						icon: "🦴",
					},
					{
						id: "fisio_neuro",
						text: "Neurológica",
						payload: "schedule_neuro",
						icon: "🧠",
					},
					{
						id: "evaluation",
						text: "Avaliação",
						payload: "schedule_evaluation",
						icon: "📋",
					},
				],
			};
		...
		if (
			input.includes("exercício") ||
			input.includes("alongamento") ||
			input.includes("treino")
		) {
			return {
				message:
					"Posso sugerir exercícios personalizados. Qual região você gostaria de trabalhar?",
				confidence: 0.85,
				...
				quickReplies: [
					{
						id: "back_exercises",
						text: "Coluna/Lombar",
						payload: "exercises_back",
						icon: "🔄",
					},
					{
						id: "knee_exercises",
						text: "Joelho",
						payload: "exercises_knee",
						icon: "🦵",
					},
					{
						id: "shoulder_exercises",
						text: "Ombro",
						payload: "exercises_shoulder",
						icon: "💪",
					},
					{
						id: "general_exercises",
						text: "Geral",
						payload: "exercises_general",
						icon: "🏃‍♂️",
					},
				],
			};
		...
		if (
			input.includes("dor") ||
			input.includes("sintoma") ||
			input.includes("sinto")
		) {
			let symptomResponse =
				"Me conte mais sobre seus sintomas. Onde você sente dor?";
			let confidence = 0.8;
		...
		if (
			input.includes("medicamento") ||
			input.includes("remédio") ||
			input.includes("medicação")
		) {
			return {
				message:
		...
		return {
			message:
				predefinedResponses.unknown[
					Math.floor(Math.random() * predefinedResponses.unknown.length)
				],
			confidence: 0.3,
			intent: "unknown",
	...
	const requestHumanHandoff = useCallback(() => {
		...
		const handoffMessage: ChatMessage = {
			...
			content:
				"Entendi que você precisa de ajuda especializada. Vou conectá-lo com um de nossos fisioterapeutas. Aguarde um momento.",
			sender: "bot",
			timestamp: new Date(),
			type: "text",
			metadata: {
		...
		(rating: number) => {
			if (currentSession) {
				setCurrentSession((prev) =>
					prev ? { ...prev, satisfaction: rating } : null,
				);
