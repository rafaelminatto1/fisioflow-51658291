...
export function useCreateParticipante() {
  ...
  return useMutation({
    ...
    onSuccess: (created) => {
      queryClient.invalidateQueries({ queryKey: ['participantes', created.evento_id] });
...
export function useUpdateParticipante() {
  ...
  return useMutation({
    ...
    onSuccess: (updated) => {
      queryClient.invalidateQueries({ queryKey: ['participantes', updated.evento_id] });
      queryClient.invalidateQueries({ queryKey: ['eventos-stats'] });
...
export function useExportParticipantes() {
  return useMutation({
    ...
    onSuccess: () => toast.success('CSV de participantes exportado com sucesso.'),
    onError: (error: Error) => toast.error(`Erro ao exportar participantes: ${error.message}`),
  });
}
