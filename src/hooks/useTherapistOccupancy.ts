...
export const useTherapistOccupancy = (
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<OccupancyMetrics> => {
      ...
      const therapists: TherapistProfile[] = (therapistsRes?.data ?? []).map((therapist) => ({
        id: therapist.id,
      ...
      const appointments = rawAppointments.flatMap((appointment) => {
        const therapistId =
          typeof appointment.therapist_id === "string" ? appointment.therapist_id : "";
      ...
      const therapistMetrics: TherapistOccupancyData[] = therapists.map((therapist) => {
        ...
        const totalMinutes = therapistAppointments.reduce((acc, apt) => {
          ...
          const normalized =
            Number.isFinite(duration ?? 0) && (duration ?? 0) > 0 ? (duration ?? 0) : 50;
