...
const getProfileDailyHours = (
	availableHours: AvailableHoursLike,
	dayOfWeek: number,
): number => {
	...
	for (const key of dayKeys) {
		...
		const parsed = parseHourRange(
			dayConfig.start ?? dayConfig.start_time,
			dayConfig.end ?? dayConfig.end_time,
		);
...
const getCapacityHoursForPeriod = (
	start: Date,
	end: Date,
	availableHours?: AvailableHoursLike,
): number => {
...
const getAppointmentHour = (appointment: {
	start_time?: string;
}): number | null => {
...
export const useTherapistOccupancy = (
	options: UseTherapistOccupancyOptions = { period: "today" },
) => {
	...
	return useQuery({
		...
		queryFn: async (): Promise<OccupancyMetrics> => {
			...
			const therapists: TherapistProfile[] = (therapistsRes?.data ?? []).map(
				(therapist) => ({
					id: therapist.id,
			...
			if (therapists.length === 0) {
				logger.info(
					"[useTherapistOccupancy] No active therapists found for organization",
					{ organizationId },
					"useTherapistOccupancy",
				);
				return {
					ocupacaoMedia: 0,
					totalConsultasHoje: 0,
			...
			therapists.forEach((therapist) => {
				...
				if (therapist.user_id)
					therapistLookup.set(therapist.user_id, therapist);
			});
			...
			const therapistMetrics: TherapistOccupancyData[] = therapists.map(
				(therapist) => {
					const therapistAppointments = (appointments || []).filter(
						(a) => a.therapist_id === therapist.id,
					);
					const todayTherapistAppointments = therapistAppointments.filter(
					...
					const totalMinutes = therapistAppointments.reduce((acc, apt) => {
						const duration = getAppointmentDurationMinutes(
							apt.start_time,
							(apt as AppointmentRow).end_time,
						);
						const normalized =
					...
					const capacidadeTotal = getCapacityHoursForPeriod(
						start,
						end,
					...
					const taxaOcupacao =
						capacidadeTotal > 0
							? Math.min(
									...
									Math.max(
										0,
										Math.round((horasTrabalhadas / capacidadeTotal) * 100),
									),
								)
							: 0;
			...
			for (let hour = 7; hour <= 21; hour++) {
				...
				const hourData: { hour: string; [key: string]: number | string } = {
					hour: hourStr,
				};
			...
			// Calcular totais
			const totalConsultasHoje = therapistMetrics.reduce(
				(acc, t) => acc + t.consultasHoje,
				0,
			);
			const totalHorasTrabalhadas = therapistMetrics.reduce(
				(acc, t) => acc + t.horasTrabalhadas,
				0,
			);
			const ocupacaoMedia =
				therapistMetrics.length > 0
					? Math.round(
							therapistMetrics.reduce((acc, t) => acc + t.taxaOcupacao, 0) /
			...
			return {
				...
				therapists: therapistMetrics.sort(
					(a, b) => b.taxaOcupacao - a.taxaOcupacao,
				),
				hourlyData,
				suggestions,
			};
