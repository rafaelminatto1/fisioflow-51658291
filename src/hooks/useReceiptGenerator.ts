...
import { saveReceiptPDF, type ReceiptData } from "../lib/skills/fase2-documentos/receipt-generator";
...
export function useReceiptGenerator(options?: UseReceiptGeneratorOptions) {
  ...
  const generateReceiptFromTransaction = useCallback(
    async (
      ...
      try {
        const amount =
          typeof transaction.valor === "string" ? parseFloat(transaction.valor) : transaction.valor;
