	...
	const generateReceipt = useCallback(
		async (
			...
			try {
				...
				const error =
					err instanceof Error ? err : new Error("Erro ao gerar recibo");
				setError(error);
				if (options?.onError) {
		...
		async (
			...
			try {
				...
				const receiptData: ReceiptData = {
					...
					description:
						transaction.descricao || "Pagamento de sessão de fisioterapia",
					paymentMethod:
						(transaction.payment_method as ReceiptData["paymentMethod"]) ||
						"dinheiro",
