...
export function useScheduleHandlers(
	...
	const [quickEditAppointment, setQuickEditAppointment] =
		useState<Appointment | null>(null);
	...
	const [waitlistQuickAdd, setWaitlistQuickAdd] = useState<{
		date: Date;
		time: string;
	} | null>(null);
	const [scheduleFromWaitlist, setScheduleFromWaitlist] = useState<{
		patientId: string;
		patientName: string;
	} | null>(null);
	...
	const [showCancelAllTodayDialog, setShowCancelAllTodayDialog] =
		useState(false);
	...
	const handleScheduleFromWaitlistFn = useCallback(
	...
	const handleCancelAllToday = useCallback(async () => {
		...
		try {
			const { cancelled, errors } =
				await AppointmentService.cancelAllAppointmentsForDate(
					organizationId,
					dateStr,
				);
			setShowCancelAllTodayDialog(false);
			...
			logger.error(
				"Erro ao cancelar agendamentos do dia",
				err,
				"ScheduleHandlers",
			);
			toast({
