...
export const roundToNextSlot = (date: Date): string => {
  const roundedDate = roundDateToNextInterval(date, BUSINESS_HOURS.defaultRound);
    ...
    (date: Date, time: string) => {
      ...
      if (time) {
        ...
        const roundedTime = roundDateToNearestInterval(
          new Date(date.getFullYear(), date.getMonth(), date.getDate(), hours || 0, minutes || 0),
          BUSINESS_HOURS.defaultRound,
        );
        setModalDefaultTime(formatTime(roundedTime.getHours(), roundedTime.getMinutes()));
      } else {
