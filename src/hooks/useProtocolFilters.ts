...
export function useProtocolFilters(protocols: ExerciseProtocol[]) {
  ...
  const filteredProtocols = useMemo(() => {
    ...
    return protocols.filter((p) => {
      ...
      if (activeTemplate) {
        switch (activeTemplate) {
          ...
          case "Tratamento Conservador":
            if (p.protocol_type !== "conservador" && p.protocol_type !== "patologia") return false;
            break;
      ...
      if (search) {
        const matchesSearch =
          includesTerm(p.name, search) || includesTerm(p.condition_name, search);
      ...
      if (muscleFilter !== "all") {
        const muscleMatch =
          includesTerm(p.name, muscleFilter) ||
