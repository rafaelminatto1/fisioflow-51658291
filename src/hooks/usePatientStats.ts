...
function determineFirstEvaluationDate(
  ...
  return firstSoap?.created_at || firstAppointment?.date || lastAppointment?.date;
}
...
export const fetchAllAppointments = async (patientId: string): Promise<Appointment[]> => {
  const pageSize = 1000;
...
export const fetchFinalizedSessions = async (patientId: string): Promise<SOAPRecord[]> => {
  const pageSize = 200;
