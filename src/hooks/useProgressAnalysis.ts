...
export const useProgressAnalysis = (
	...
	const analysis = useMemo(() => {
		...
		const mobilityImprovement =
			((currentMobility - initialMobility) / initialMobility) * 100;

		// Calcular taxa de melhora média por sessão
		let totalImprovement = 0;
		for (let i = 1; i < sortedSessions.length; i++) {
			const improvement =
				sortedSessions[i - 1].painLevel - sortedSessions[i].painLevel;
			totalImprovement += improvement;
		}
		const averageImprovementPerSession =
		...
		if (recentSessions.length >= 2) {
			const recentAvg =
				recentSessions.slice(-2).reduce((sum, s) => sum + s.painLevel, 0) / 2;
			const previousAvg =
				recentSessions.slice(0, 2).reduce((sum, s) => sum + s.painLevel, 0) / 2;
		...
		if (painReduction > 50 && mobilityImprovement > 30) {
			alerts.push({
				...
				recommendation:
					"Continue o tratamento atual. Considere iniciar exercícios mais avançados.",
			});
		}
		...
		if (isStagnant && sortedSessions.length >= 5) {
			alerts.push({
				...
				message:
					"O paciente não apresentou melhora significativa nas últimas 3 sessões.",
				recommendation:
					"Reavalie o plano de tratamento. Considere ajustar condutas ou encaminhar para especialista.",
			});
		}
		...
		if (trend === "declining") {
			alerts.push({
				...
				message:
					"Os níveis de dor do paciente estão aumentando nas sessões recentes.",
				recommendation:
					"Investigue possíveis causas: execução incorreta de exercícios, sobrecarga, nova lesão. Considere pausar atividades de alto impacto.",
			});
		}
		...
		if (daysSinceStart <= 30 && painReduction > 40) {
			alerts.push({
				...
				recommendation:
					"Paciente responde bem ao tratamento. Mantenha motivação alta e considere aumentar gradualmente a intensidade.",
			});
		}
		...
		if (daysSinceStart > 90 && painReduction < 30) {
			alerts.push({
				...
				recommendation:
					"Considere reavaliação completa, exames complementares ou interconsulta com médico especialista.",
			});
		}
		...
		if (sortedSessions.length === 10) {
			alerts.push({
				...
				message:
					"10ª sessão concluída. Momento ideal para reavaliação completa.",
				recommendation:
					"Agende reavaliação com testes padronizados (Oswestry, Lysholm) e atualize metas de tratamento.",
			});
		}
