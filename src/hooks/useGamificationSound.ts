...
export function useGamificationSound() {
	...
	const soundCacheRef = useRef<Map<GamificationSoundType, HTMLAudioElement>>(
		new Map(),
	);
	...
	const playSound = useCallback(
		(type: GamificationSoundType, volume?: number) => {
			try {
	...
	const playSoundSequence = useCallback(
		async (sounds: GamificationSoundType[], delay = 300) => {
