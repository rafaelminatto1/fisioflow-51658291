...
export const useOpticalFlow = (videoRef: React.RefObject<HTMLVideoElement>) => {
  ...
  const addPointToTrack = useCallback((pos: Point) => {
    setTrackedPoints((prev) => [
      ...prev,
      { id: Math.random().toString(36).substr(2, 9), pos, active: true },
    ]);
  }, []);
          ...
          for (let dx = -searchSize; dx <= searchSize; dx++) {
            ...
            for (let py = -1; py <= 1; py++) {
              for (let px = -1; px <= 1; px++) {
                ...
                // Luma difference
                const curL =
                  currentImageData[pIdx] * 0.299 +
                  currentImageData[pIdx + 1] * 0.587 +
                  currentImageData[pIdx + 2] * 0.114;
                const prevL =
                  prevImageData.current![prevIdx] * 0.299 +
                  prevImageData.current![prevIdx + 1] * 0.587 +
                  prevImageData.current![prevIdx + 2] * 0.114;
