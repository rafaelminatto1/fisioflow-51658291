...
export function usePoseDetection(options: UsePoseDetectionOptions = {}) {
	...
	const initLandmarker = useCallback(async () => {
		...
		try {
			...
			logger.info("PoseLandmarker inicializado com sucesso", { mode: mergedOptions.runningMode }, "usePoseDetection");
		} catch (err) {
	...
	}, [load, isInitializing, mergedOptions.modelAssetPath, mergedOptions.runningMode, mergedOptions.minPoseDetectionConfidence, mergedOptions.minPosePresenceConfidence, mergedOptions.minTrackingConfidence, mergedOptions.outputSegmentationMasks]);
	 ...
	 */
	const detect = useCallback((image: HTMLImageElement | HTMLCanvasElement | HTMLVideoElement) => {
