  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AttendanceMetrics> => {
      ...
      const appointmentsRaw = await listAppointmentsPaginated({
        ...
        dateTo: endDateStr,
        therapistId:
          filters.therapistId && filters.therapistId !== "all" ? filters.therapistId : undefined,
      });
      ...
      const therapistMap = new Map<string, string>(
        ((therapistsRes?.data ?? []) as Array<{ id: string; name: string }>).map((t) => [
          String(t.id),
          String(t.name),
        ]),
      );
      ...
      appointments.forEach((apt) => {
        const hour = Number(String(apt.appointment_time ?? "0:00").split(":")[0]);
      ...
      const appointmentDetails: AppointmentDetail[] = appointments
        .map((apt) => ({
          ...
          therapistName: therapistMap.get(apt.therapist_id || "") || "Não atribuído",
...
export const useTherapists = () => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return ((res?.data ?? []) as Array<{ id: string; name: string }>).map((t) => ({
        id: t.id,
        full_name: t.name,
      }));
    },
