...
export const useStandardizedTests = (patientId: string, options?: UseStandardizedTestsOptions) => {
...
export const useCreateStandardizedTest = () => {
  ...
  return useMutation({
    mutationFn: async (data: CreateStandardizedTestInput): Promise<StandardizedTestResult> => {
      const res = await request<{ data: StandardizedTestResult }>("/api/standardized-tests", {
        method: "POST",
...
export const useDeleteStandardizedTest = () => {
  ...
  return useMutation({
    mutationFn: async ({ id }: { id: string; patientId: string }): Promise<void> => {
    ...
    mutationFn: async (testData: {
      ...
      const res = await request<{ data: StandardizedTestResult }>("/api/standardized-tests", {
        ...
        body: JSON.stringify({
          ...
          scale_name: (testData.test_type ?? testData.test_name ?? "CUSTOM").toUpperCase(),
