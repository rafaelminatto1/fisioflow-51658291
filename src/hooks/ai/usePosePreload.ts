...
export function usePosePreload() {
	...
	const preload = useCallback(async () => {
		...
		try {
			logger.info(
				"[PosePreload] Iniciando pré-carregamento global...",
				null,
				"PosePreload",
			);
			...
			setIsReady(true);
			logger.info(
				"[PosePreload] MediaPipe pré-carregado com sucesso",
				null,
				"PosePreload",
			);
		} catch (err) {
			const error =
				err instanceof Error ? err : new Error("Falha no pré-carregamento");
			setError(error);
