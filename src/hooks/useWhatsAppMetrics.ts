...
const mapMessage = (message: WhatsAppMessage): WhatsAppMetric => {
  ...
  return {
    ...
    template_key:
      ...
      null,
    reply_content:
      (metadata.response_content as string | undefined) ?? message.response_content ?? null,
    error_message: (metadata.error_message as string | undefined) ?? null,
    retry_count: Number(metadata.retry_count ?? 0) || 0,
    patients: typeof metadata.patient_name === "string" ? { name: metadata.patient_name } : null,
  };
};

const summarizeMetrics = (messages: WhatsAppMetric[]): MetricsSummary => {
  const outbound = messages.filter(
    (message) => !message.metadata?.direction || message.metadata?.direction === "outbound",
  );
  ...
  const responseTimes = outbound
    .map((message) =>
      toMinutes(message.sent_at ?? message.created_at, message.response_received_at),
    )
