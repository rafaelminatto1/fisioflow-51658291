 ...
 */
export function usePrefetchPatientData(patientId: string | undefined, enabled: boolean = true) {
  const queryClient = useQueryClient();
  ...
  useEffect(() => {
    ...
    const prefetchData = async () => {
      ...
      queryClient.prefetchQuery({
        ...
        queryFn: async () => {
          const { data } = await fetch(`/api/patients/${patientId}/evolutions`).then((r) =>
            r.json(),
          );
          return data;
        },
      ...
      queryClient.prefetchQuery({
        ...
        queryFn: async () => {
          const { data } = await fetch(`/api/patients/${patientId}/pain-maps`).then((r) =>
            r.json(),
          );
          return data;
        },
