 ...
 */
export function useTrackedMemo<T>(factory: () => T, deps: unknown[]): [T, boolean] {
  const prevDepsRef = useRef<unknown[]>();
  const valueRef = useRef<T>();

  const depsChanged =
    !prevDepsRef.current || deps.some((dep, i) => !Object.is(dep, prevDepsRef.current![i]));
...
export function useMemoWithTTL<T>(
  ...
  const depsChanged =
    !lastDepsRef.current || deps.some((dep, i) => !Object.is(dep, lastDepsRef.current![i]));
 ...
 * ], [[a, b], [c, d], [e, f]]);
 */
export function useMultiMemo<T>(factories: (() => T)[], depsArrays: unknown[][]): T[] {
 ...
 * );
 */
export function useConditionalMemo<T>(factory: () => T, shouldRender: boolean): T | null {
