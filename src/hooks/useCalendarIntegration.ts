...
function mapIntegration(
	row: GoogleIntegrationRecord | null,
	...
	return {
		...
		default_calendar_id:
			typeof settings.default_calendar_id === "string"
...
export function useCalendarIntegration() {
	...
	const { data: integration, isLoading: isLoadingIntegration } = useQuery({
		...
		queryFn: async () =>
			mapIntegration((await integrationsApi.google.calendar.get()).data),
		enabled: !!userId,
	});
	...
	const { data: syncLogs, isLoading: isLoadingLogs } = useQuery({
		...
		queryFn: async () =>
			((await integrationsApi.google.calendar.logs()).data ?? []).map(
				mapSyncLog,
			),
		enabled: !!userId,
	});
	...
	const connectGoogle = useMutation({
		mutationFn: async () => {
			...
			try {
				const authUrl = await integrationsApi.google.authUrl(
			...
			const result = await integrationsApi.google.connect({
	...
	const updateSettings = useMutation({
		...
		onError: (error) => {
			logger.error(
				"Erro ao atualizar configurações do calendário",
				error,
				"useCalendarIntegration",
			);
