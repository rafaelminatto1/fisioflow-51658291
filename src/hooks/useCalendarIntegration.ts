...
import { integrationsApi, type GoogleIntegrationRecord, type GoogleSyncLogRecord } from "@/api/v2";
...
function mapIntegration(row: GoogleIntegrationRecord | null): CalendarIntegration | null {
  ...
  return {
    ...
    default_calendar_id:
      typeof settings.default_calendar_id === "string" ? settings.default_calendar_id : null,
...
export function useCalendarIntegration() {
  ...
  const { data: syncLogs, isLoading: isLoadingLogs } = useQuery({
    ...
    queryFn: async () =>
      ((await integrationsApi.google.calendar.logs()).data ?? []).map(mapSyncLog),
    enabled: !!userId,
  });
