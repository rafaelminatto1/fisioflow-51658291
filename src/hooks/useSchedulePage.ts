...
const mapAppointmentRowToCalendarAppointment = (row: ScheduleAppointmentRow): Appointment => ({
  id: row.id,
  ...
  status: normalizeStatus(String(row.status || "agendado")) as Appointment["status"],
  notes: row.notes || "",
...
const matchesScheduleFilters = (appointment: Appointment, filters?: ScheduleFilters): boolean => {
  if (!filters) return true;

  if (filters.status.length > 0 && !filters.status.includes(String(appointment.status))) {
    return false;
  }

  if (filters.types.length > 0 && !filters.types.includes(String(appointment.type))) {
    return false;
  }

  if (
    filters.therapists.length > 0 &&
    (!appointment.therapistId || !filters.therapists.includes(String(appointment.therapistId)))
  ) {
    return false;
  }
...
export function useSchedulePageData(date: string, view: ViewType, filters?: ScheduleFilters) {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        ...
        return (res?.data ?? [])
          .map((row) => mapAppointmentRowToCalendarAppointment(row as ScheduleAppointmentRow))
          .filter((appointment) => matchesScheduleFilters(appointment, filters));
      } catch (error) {
        logger.error("Error loading appointments", { error }, "useSchedulePage");
        throw error;
      }
    },
  ...
  const createMutation = useMutation({
    mutationFn: async (data: { appointment: Record<string, unknown>; organizationId: string }) => {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: {
      ...
      return await AppointmentService.updateAppointment(data.id, data.appointment);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["schedule-appointments"],
  ...
  const deleteMutation = useMutation({
    mutationFn: async (data: { id: string; organizationId: string }) => {
      return await AppointmentService.deleteAppointment(data.id, data.organizationId);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["schedule-appointments"],
