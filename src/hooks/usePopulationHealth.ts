...
import {
	...
	PopulationAnalysisOptions,
} from "@/lib/ai/population-health";

export const POPULATION_HEALTH_KEYS = {
	all: ["population-health"] as const,
	analysis: (clinicId: string, period: string) =>
		[...POPULATION_HEALTH_KEYS.all, "analysis", clinicId, period] as const,
	conditions: (clinicId: string) =>
		[...POPULATION_HEALTH_KEYS.all, "conditions", clinicId] as const,
	retention: (clinicId: string) =>
		[...POPULATION_HEALTH_KEYS.all, "retention", clinicId] as const,
	benchmarks: (clinicId: string) =>
		[...POPULATION_HEALTH_KEYS.all, "benchmarks", clinicId] as const,
};
 ...
 */
export function usePopulationHealthAnalysis(
	options: UsePopulationHealthOptions = {},
) {
	const {
		clinicId = "default",
		period = "90d",
...
export function useBenchmarks(options: UsePopulationHealthOptions = {}) {
	...
	const analysis = usePopulationHealthAnalysis({
		clinicId,
		period,
		includeBenchmarks,
		enabled,
	...
	return useMutation({
		...
		onSuccess: (data, variables) => {
			queryClient.invalidateQueries({
				queryKey: POPULATION_HEALTH_KEYS.analysis(
					variables.clinicId || "default",
					"90d",
				),
			});
 ...
 */
export function usePopulationHealthChartData(
	options: UsePopulationHealthOptions = {},
) {
	...
	const conditionsDistribution = () => {
		...
		return analysis.data.chartData.conditionsDistribution.map((item) => ({
			...
			value: item.value,
			percentage: (
				(item.value / analysis.data!.populationOverview.totalPatients) *
				100
			).toFixed(1),
		}));
	};
