...
import {
	...
	logRepetitionCompleted,
} from "@/services/ai/poseAnalytics";
...
export function useExerciseExecution({
	...
	const { speak, announceCount, announceCorrection, playSound } =
		useAudioFeedback({ enabled: enableAudio });
	...
	const {
		...
		exerciseType,
		enabled:
			state === SessionState.EXERCISING || state === SessionState.CALIBRATING,
		onAnalysisResult: (result) => {
			...
			if (
				...
				const criticalIssue = result.postureIssues.find(
	...
	const startSession = useCallback(() => {
		logger.info(
			"[ExerciseExecution] Iniciando sessão",
	...
	const completeSession = useCallback(async () => {
		...
		logger.info(
			"[ExerciseExecution] Sessão completada",
			{
				reps: repCount,
		...
		try {
			...
			logger.error(
				"Falha ao salvar sessão automaticamente",
				err,
				"ExerciseExecution",
			);
		}
		...
		logExerciseCompleted(
			exerciseId,
			repCount,
			finalSession.metrics.formScore,
