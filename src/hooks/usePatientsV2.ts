	...
	return useQuery({
		...
		queryFn: async () => {
			if (!isOnline()) {
				...
				fisioLogger.debug(
					"Offline: V2 cache not implemented yet",
					undefined,
					"usePatientsV2",
				);
				return [];
			...
			fisioLogger.debug(
				"useActivePatientsV2: fetching from API V2",
				undefined,
				"usePatientsV2",
			);
				...
				return {
					...
					pagination: {
						...
						totalPages: Math.ceil(
							(response.total || patients.length) / pageSize,
						),
						hasNext:
							nextPage <
							Math.ceil((response.total || patients.length) / pageSize),
						hasPrev: nextPage > 1,
					},
				};
			},
