...
export const useAppointmentData = (
	...
	const {
		...
		queryFn: async () => {
			...
			fisioLogger.debug(
				"Fetching appointment from Workers API",
				{ appointmentId },
				"useAppointmentData",
			);
	...
	const {
		...
		queryFn: async () => {
			if (!patientId) {
				fisioLogger.debug(
					"No patientId provided, skipping patient query",
					undefined,
					"useAppointmentData",
				);
			...
			if (
				options?.initialPatientData &&
				options.initialPatientData.id === patientId
			) {
			...
			try {
				...
				fisioLogger.warn(
					`[useAppointmentData] Direct fetch failed for ${patientId}`,
					{ error: err },
					"useAppointmentData",
				);
				...
				const patientName =
					...
					options?.initialPatientData?.full_name;
				if (patientName) {
					try {
						fisioLogger.debug(
							`[useAppointmentData] Attempting fallback by name: ${patientName}`,
							undefined,
							"useAppointmentData",
						);
						...
						const found = searchResult.data.find(
							(p: any) =>
								p.id === patientId ||
								p.objectID === patientId ||
								p.uuid === patientId,
						);
						if (found) {
							fisioLogger.info(
								`[useAppointmentData] Found patient via name search fallback: ${patientName}`,
								{ patientId },
								"useAppointmentData",
							);
						...
						fisioLogger.warn(
							`[useAppointmentData] Name search fallback failed`,
							{ error: searchErr },
							"useAppointmentData",
						);
					}
				...
				try {
					fisioLogger.debug(
						"[useAppointmentData] Attempting fallback by listing recent patients",
						undefined,
						"useAppointmentData",
					);
					...
					if (found) {
						fisioLogger.info(
							"[useAppointmentData] Found patient via list fallback",
							{ patientId },
							"useAppointmentData",
						);
						return mapPatientDBToApp(found);
					...
					fisioLogger.error(
						"[useAppointmentData] All patient fallbacks failed",
						{ error: fallbackErr, patientId },
						"useAppointmentData",
					);
				}
		...
		enabled: !!patientId,
		initialData: (appointment?.patient || options?.initialPatientData) as
			| Patient
			| undefined,
		initialDataUpdatedAt:
			appointment || options?.initialPatientData ? Date.now() : undefined,
	...
	return {
		...
		isLoading:
			appointmentLoading || (!!patientId && patientLoading && !patient),
