...
export function useNPSMetrics() {
  return useQuery({
    ...
    queryFn: async () => ({ total: 0, promotores: 0, neutros: 0, detratores: 0, nps: 0 }),
    staleTime: Infinity,
  });
}
...
export function useCRMAnalytics() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return { conversionBySource, coldLeads, totalLeads: leads.length, temperatureDistribution: {}, stageAnalysis: [] };
    },
  });
}
...
export interface LeadImportData {
  nome?: string; name?: string; Nome?: string;
  telefone?: string; phone?: string; Telefone?: string;
  email?: string; Email?: string;
  origem?: string; Origem?: string;
  interesse?: string; Interesse?: string;
  observacoes?: string; 'Observações'?: string;
}
  ...
  return useMutation({
    mutationFn: async (leads: Array<Partial<LeadImportData>>) => {
      const formatted = leads.map(lead => ({
        ...
        observacoes: lead.observacoes || (lead as Record<string, unknown>)['Observações'] as string || undefined,
        estagio: 'aguardando',
