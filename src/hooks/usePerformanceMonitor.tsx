...
export function usePerformanceMonitor(
  ...
  const ComponentWrapper = useCallback(({ children }: { children: React.ReactNode }) => {
    return <>{children}</>;
  }, []);
...
export function useAsyncPerformance() {
  const measureAsync = useCallback(async <T,>(name: string, fn: () => Promise<T>): Promise<T> => {
    ...
    try {
      ...
      if (duration > 100) {
        logger.warn(
          `[Async Performance] ${name} levou ${duration.toFixed(2)}ms`,
          undefined,
          "usePerformanceMonitor",
        );
      }
      ...
      const duration = endTime - startTime;
      logger.error(
        `[Async Performance] ${name} falhou após ${duration.toFixed(2)}ms`,
        error,
        "usePerformanceMonitor",
      );
      throw error;
...
export class PerformanceTimer {
  ...
  constructor(name: string) {
    ...
    logger.debug(
      `[Timer] ${this.name}: ${duration.toFixed(2)}ms`,
      undefined,
      "usePerformanceMonitor",
    );
    return duration;
  ...
  const measureInteraction = useCallback(async (name: string, fn: () => void | Promise<void>) => {
    ...
    try {
      ...
      if (entries.length > 0) {
        ...
        if (duration > 100) {
          logger.warn(
            `[Interaction] ${name} levou ${duration.toFixed(2)}ms (acima de 100ms)`,
            undefined,
            "usePerformanceMonitor",
          );
        }
