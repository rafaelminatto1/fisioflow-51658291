...
import { financialApi, type PatientPackageRow, type SessionPackageTemplateRow } from "@/api/v2";
...
const mapPatientPackage = (row: PatientPackageRow): PatientPackage => {
  ...
  return {
    ...
    package: {
      ...
      validity_days:
        row.expires_at && row.purchased_at
          ? Math.max(
              1,
              Math.round(
  ...
  return useMutation({
    ...
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({
        queryKey: ["patient-packages", variables.patient_id],
      });
...
export function useUsePackageSession() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const res = await financialApi.patientPackages.consume(patientPackageId, {
        appointmentId,
      });
      return mapPatientPackage(res.data as PatientPackageRow);
    },
