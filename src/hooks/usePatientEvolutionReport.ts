	...
	return useQuery({
		...
		queryFn: async (): Promise<PatientEvolutionData> => {
			...
			const soapRecords = sessionRecords.map((record) => ({
				...
				created_by:
					(record as any).therapist_id || (record as any).created_by || "",
			}));
			...
			const measurementEvolution = Array.from(evolutionMap.entries()).map(
				([_key, history]) => {
					...
					return {
						...
						initial: {
							value: first.value,
							unit: first.unit || "",
							date: first.measured_at,
						},
						current: {
							value: last.value,
							unit: last.unit || "",
							date: last.measured_at,
						},
						improvement:
						...
						vitalSigns:
							last.measurement_type === "Sinais Vitais"
								? (last.custom_data as Record<string, unknown>)
								: undefined,
					};
				},
			);
			...
			for (let i = 1; i < sessions.length; i++) {
				...
				const percentImprovement =
					sessions[i - 1].painLevel > 0
						? (improvement / sessions[i - 1].painLevel) * 100
						: 0;
				totalImprovement += percentImprovement;
			}
			const averageImprovement =
				sessions.length > 1 ? totalImprovement / (sessions.length - 1) : 0;
