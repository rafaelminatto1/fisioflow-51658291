...
export function useBiometricAuth(): BiometricAuthState {
	...
	const [deviceType, _setDeviceType] = useState<"faceId" | "touchId" | "none">(
		"none",
	);
	...
	const authenticate = useCallback(async (): Promise<boolean> => {
		...
		try {
			...
			if (error.code === "USER_CANCELATION") {
				...
				logger.info(
					"App foi colocado em background",
					undefined,
					"useBiometricAuth",
				);
			}
...
export async function deleteBiometricCredentials(): Promise<void> {
	try {
		...
		logger.error(
			"Erro ao remover credenciais biométricas",
			error,
			"useBiometricAuth",
		);
	}
...
export async function getBiometricCredentials(): Promise<{
	...
	try {
		...
		logger.error(
			"Erro ao obter credenciais biométricas",
			error,
			"useBiometricAuth",
		);
		return null;
