...
export function usePDFGenerator(options?: UsePDFGeneratorOptions) {
	...
	const generateAtestado = useCallback(
		async (
			patient: PatientData,
			...
			try {
				...
				const error =
					err instanceof Error ? err : new Error("Erro ao gerar atestado");
				setError(error);
				if (options?.onError) {
	...
	const generateDeclaracao = useCallback(
		async (
			patient: PatientData,
			professional: ProfessionalData,
			clinic: ClinicData,
			...
			try {
				...
				const error =
					err instanceof Error ? err : new Error("Erro ao gerar declaração");
				setError(error);
				if (options?.onError) {
					options.onError(error);
				}
	...
	const generateReceituario = useCallback(
		async (
			patient: PatientData,
			professional: ProfessionalData,
			clinic: ClinicData,
			...
			try {
				...
				const error =
					err instanceof Error ? err : new Error("Erro ao gerar receituário");
				setError(error);
				if (options?.onError) {
					options.onError(error);
				}
	...
	const generateEvolucao = useCallback(
		async (
			patient: PatientData,
			professional: ProfessionalData,
			clinic: ClinicData,
			...
			try {
				...
				const error =
					err instanceof Error ? err : new Error("Erro ao gerar evolução");
				setError(error);
				if (options?.onError) {
					options.onError(error);
				}
	...
	const generatePlanoTratamento = useCallback(
		async (
			patient: PatientData,
			professional: ProfessionalData,
			clinic: ClinicData,
			...
			try {
				...
				const error =
					err instanceof Error
						? err
						: new Error("Erro ao gerar plano de tratamento");
				setError(error);
				if (options?.onError) {
					options.onError(error);
				}
