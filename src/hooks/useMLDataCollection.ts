...
export function useBatchCollectMLData() {
	...
	return useMutation({
		mutationFn: async (options?: { limit?: number }) => {
			...
			const patientsResponse = await analyticsApi.mlTrainingData.patients({
				limit: limitNum,
			});
...
export function useGeneratePatientPredictions() {
	...
	return useMutation({
		mutationFn: async (patientId: string) => {
			...
			const functionalImprovement = Number(
				trainingData.functional_improvement_percentage ?? 0,
			);
			const sessionFrequency = Number(
				trainingData.session_frequency_weekly ?? 0,
			);
			...
			const recoveryTimeline =
				trainingData.total_sessions > 0
					? Math.max(
							1,
							Math.round(12 / (attendanceRate * sessionFrequency || 1)),
						)
					: 12;
			...
			const predictionsPayload = [
				{
					...
					predicted_class:
						dropoutRisk > 50 ? "high" : dropoutRisk > 20 ? "medium" : "low",
