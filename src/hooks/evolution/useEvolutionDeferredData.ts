...
export function useEvolutionDeferredData({
  ...
  const measurementsQuery = useEvolutionMeasurements(patientId, {
    limit: shouldLoadMeasurements ? 50 : 10,
    enabled: shouldLoadMeasurements,
  });

  // Cirurgias e retornos apenas na aba histórico
  const surgeriesQuery = usePatientSurgeries(activeTab === "historico" ? patientId : "");

  const medicalReturnsQuery = usePatientMedicalReturns(activeTab === "historico" ? patientId : "");
  ...
  const shouldLoadRequired =
    loadRequiredMeasurements && (activeTab === "avaliacao" || activeTab === "evolucao");
