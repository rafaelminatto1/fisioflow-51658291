...
export const evolutionKeys = {
	...
	pathologies: (id: string) =>
		[...evolutionKeys.patient(id), "pathologies"] as const,
	...
	surgeries: (id: string) =>
		[...evolutionKeys.patient(id), "surgeries"] as const,
	medicalReturns: (id: string) =>
		[...evolutionKeys.patient(id), "medical-returns"] as const,
...
export function useEvolutionDataOptimized(options: EvolutionDataOptions) {
	...
	const shouldLoadMeasurements =
		shouldLoadData("measurements") && loadMeasurements;

	// OTIMIZAÇÃO: Limite dinâmico baseado na aba
	const measurementsLimit = activeTab === "avaliacao" ? 50 : 10;
	const measurementsQuery = useEvolutionMeasurements(patientId, {
		limit: shouldLoadMeasurements ? measurementsLimit : 0,
		enabled: shouldLoadMeasurements,
	});
	...
	const shouldLoadRequired =
		shouldLoadData("required") && loadRequiredMeasurements;
		...
		(
			...
			if (!dataType || dataType === "all") {
				...
				queryClient.invalidateQueries({
					queryKey: evolutionKeys.patient(patientId),
				});
			} else {
				...
				switch (dataType) {
					...
					case "soap":
						queryClient.invalidateQueries({
							queryKey: evolutionKeys.soapRecords(patientId),
						});
						queryClient.invalidateQueries({
							queryKey: evolutionKeys.drafts(patientId),
						});
						break;
					case "surgeries":
						queryClient.invalidateQueries({
	...
	return {
		...
		isLoading:
			...
			pathologiesQuery.isLoading ||
			soapRecordsQuery.isLoading,
		isLoadingCritical:
			goalsQuery.isLoading ||
			pathologiesQuery.isLoading ||
			soapRecordsQuery.isLoading,
...
export function useEvolutionData() {
	...
	return {
		...
		getData: (patientId: string) => ({
			...
			pathologies: queryClient.getQueryData(
				evolutionKeys.pathologies(patientId),
			),
			measurements: queryClient.getQueryData(
				evolutionKeys.measurements(patientId),
			),
		}),
		...
		prefetch: (
			patientId: string,
			dataType: "goals" | "pathologies" | "measurements",
		) => {
			switch (dataType) {
				case "goals":
					prefetchPatientGoals(
...
export function useEvolutionPrefetch() {
	...
	const prefetchMultiplePatients = useCallback(
		(patientIds: string[]) => {
