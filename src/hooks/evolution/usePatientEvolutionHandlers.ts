...
export function usePatientEvolutionHandlers({
	...
	const handleSave = async () => {
		...
		const isV2orV3 =
			evolutionVersion === "v2-texto" || evolutionVersion === "v3-notion";
			...
			toast({
				...
				description:
					error instanceof Error
						? error.message
						: "Ocorreu um erro ao salvar a evolução.",
				variant: "destructive",
			});
		}
	};
			...
			completeAppointment(appointmentId, {
				onSuccess: async () => {
					...
					toast({
						...
						description:
							"O atendimento foi marcado como concluído com sucesso.",
					});
	...
	const handleExportPDF = async () => {
		if (!patient || !user?.professional || !user?.clinic) {
			toast({
				...
				description:
					"Não foi possível obter os dados do paciente, profissional ou clínica.",
				variant: "destructive",
			});
			return;
		}
