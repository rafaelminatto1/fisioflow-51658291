...
export function usePatientEvolutionState() {
			...
			try {
				return (
					(localStorage.getItem(
						"fisioflow-evolution-version",
					) as EvolutionVersion) || "v1-soap"
				);
			} catch {
				return "v1-soap";
			}
	...
	// ========== DATA STATE ==========
	const [currentSoapRecordId, setCurrentSoapRecordId] = useState<
		string | undefined
	>();
	...
	const { lastSession, isLoadingLastSession, suggestExerciseChanges } =
		useSessionExercises(patientId || "");
