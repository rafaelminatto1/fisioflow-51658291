...
export function useTimeTracker(
			...
			const partialEntry: Omit<TimeEntry, "id" | "created_at" | "updated_at"> =
				{
					...
					start_time:
						activeTimer.start_time instanceof Date
							? activeTimer.start_time.toISOString()
							: String(activeTimer.start_time),
	...
	const stopTimer = useCallback(async (): Promise<TimeEntry | null> => {
		...
		try {
			...
			const timeEntry = await finalizeTimer(
				organizationId,
				userId,
				activeTimer,
			);
	...
	const updateActiveTimer = useCallback(
		async (updates: Partial<ActiveTimer>) => {
			if (!activeTimer) return;
	...
	const loadEntries = useCallback(
		async (filters?: TimeEntryFilters) => {
			if (!organizationId) {
				setEntries([]);
				return;
			}
			...
			try {
				...
				const message =
					err instanceof Error ? err.message : "Erro ao carregar entradas";
				setError(message);
			} finally {
				setIsLoading(false);
			}
	...
	const createEntry = useCallback(
		async (
			...
			try {
				...
				const message =
					err instanceof Error ? err.message : "Erro ao criar entrada";
				setError(message);
				throw err;
			}
		},
	...
	const deleteEntry = useCallback(
		async (id: string) => {
			...
			try {
				...
				const message =
					err instanceof Error ? err.message : "Erro ao deletar entrada";
				setError(message);
				throw err;
			}
		},
 ...
 */
export function useQuickTimer(options: {
	organizationId: string;
	userId: string;
}) {
	const {
