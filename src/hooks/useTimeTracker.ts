...
import type { ActiveTimer, TimeEntry, TimeEntryFilters, TimeStats } from "@/types/timetracking";
...
export function useTimeTracker(options: UseTimeTrackerOptions): UseTimeTrackerReturn {
  ...
  useEffect(() => {
    ...
    unsubscribeRef.current = listenToTodayTimeEntries(organizationId, userId, (todayEntries) => {
  ...
  const createEntry = useCallback(
    async (data: Omit<TimeEntry, "id" | "created_at" | "updated_at">): Promise<TimeEntry> => {
      if (!organizationId) {
