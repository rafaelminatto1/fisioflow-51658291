...
const mockInsights: PredictiveInsight[] = [
	{
		...
		description:
			"Com base no crescimento atual, a receita pode atingir R$ 95.000 no próximo mês",
		confidence: 87,
		impact: "high",
		actionable: true,
		suggestedActions: [
	...
	},
	{
		...
		description:
			"Taxa de retenção de pacientes aumentou 15% nos últimos 3 meses",
		confidence: 92,
		impact: "high",
		actionable: false,
		suggestedActions: [],
	...
	},
	{
		...
		description:
			"Horários entre 14h-16h têm menor ocupação. Considere promoções para este período",
		confidence: 78,
		impact: "medium",
		actionable: true,
		suggestedActions: [
...
export const useAdvancedAnalytics = () => {
	...
	const [realtimeMetrics, setRealtimeMetrics] =
		useState<RealtimeMetric[]>(mockRealtimeMetrics);
				...
				prev.map((metric) => ({
					...
					status:
						metric.value > (metric.threshold?.critical || 100)
							? "critical"
							: metric.value > (metric.threshold?.warning || 80)
								? "warning"
								: "normal",
				})),
			);
	...
	const refreshMetrics = async (): Promise<void> => {
		...
		try {
			...
			logger.error(
				"Erro ao atualizar métricas de analytics",
				undefined,
