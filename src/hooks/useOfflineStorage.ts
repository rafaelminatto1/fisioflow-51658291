...
export function useOfflineStorage(options: OfflineStorageOptions = {}) {
	...
	const loadCacheStats = useCallback(async (): Promise<void> => {
		try {
			...
			logger.error(
				"[OfflineStorage] Error loading cache stats",
				error,
				"useOfflineStorage",
			);
		}
	...
	> => {
		try {
			...
			return actions.filter((a) =>
				canRetryAction(a.retryCount, maxRetryAttempts),
			);
		} catch (error) {
			logger.error(
				"[OfflineStorage] Error getting pending actions",
				error,
				"useOfflineStorage",
			);
			return [];
	...
	const cachePatientAnalytics = useCallback(
		async (patientId: string, data: unknown): Promise<void> => {
			try {
				...
				logger.error(
					"Error caching patient analytics",
					error,
					"useOfflineStorage",
				);
				throw error;
	...
	const getCachedPatientAnalytics = useCallback(
		async (patientId: string): Promise<unknown | null> => {
			try {
				...
				logger.error(
					"Error getting cached analytics",
					error,
					"useOfflineStorage",
				);
				return null;
	...
	async function executeAction(
		...
		switch (actionType) {
			...
			default:
				logger.warn(
					"[OfflineStorage] Unknown action type",
					{ actionType },
					"useOfflineStorage",
				);
		}
	...
	const cacheSessionData = useCallback(
		async (
			...
			try {
				...
				logger.error(
					"[OfflineStorage] Error caching session data",
					error,
					"useOfflineStorage",
				);
				throw error;
	...
	const getCachedSessionData = useCallback(
		async (sessionId: string): Promise<unknown | null> => {
			try {
				...
				logger.error(
					"[OfflineStorage] Error getting cached session data",
					error,
					"useOfflineStorage",
				);
				return null;
	...
	const removeSessionData = useCallback(
		async (sessionId: string): Promise<void> => {
			try {
				...
				logger.error(
					"[OfflineStorage] Error removing session data",
					error,
					"useOfflineStorage",
				);
			}
	...
	const clearExpiredSessions = useCallback(async (): Promise<number> => {
		try {
			...
			logger.error(
				"[OfflineStorage] Error clearing expired sessions",
				error,
				"useOfflineStorage",
			);
			return 0;
	...
	const clearAllCache = useCallback(async (): Promise<void> => {
		try {
			...
			logger.error(
				"[OfflineStorage] Error clearing cache",
				error,
				"useOfflineStorage",
			);
	...
	const getStorageSize = useCallback(async (): Promise<StorageQuota | null> => {
		try {
			...
			logger.error(
				"[OfflineStorage] Error getting storage size",
				error,
				"useOfflineStorage",
			);
			return null;
	...
	const exportAllData = useCallback(async (): Promise<{
		...
		try {
			...
			logger.error(
				"[OfflineStorage] Error exporting data",
				error,
				"useOfflineStorage",
			);
			throw error;
	...
	const clearFailedActions = useCallback(async (): Promise<number> => {
		try {
			...
			for await (const cursor of tx.store) {
				if (
					cursor.value.retryCount >= maxRetryAttempts &&
					!cursor.value.synced
				) {
					await cursor.delete();
					clearedCount++;
			...
			logger.error(
				"[OfflineStorage] Error clearing failed actions",
				error,
				"useOfflineStorage",
			);
			return 0;
