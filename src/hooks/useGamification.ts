...
const calculateLevel = (totalXp: number) => {
	...
	return {
		...
		progress:
			xpForNextLevel > 0
				? ((totalXp - accumulated) / xpForNextLevel) * 100
				: 100,
	};
};
...
export interface UseGamificationResult {
	...
	completeQuest: ReturnType<
		typeof useMutation<void, Error, { questId: string }>
	>;
	...
	const { data: shopItemsRaw = [] } = useQuery({
		...
		queryFn: async () => {
			try {
				...
				logger.warn(
					"[useGamification] Falha ao carregar loja",
					{ err },
					"useGamification",
				);
				return [];
	...
	const { data: inventoryRaw = [] } = useQuery({
		...
		queryFn: async () => {
			try {
				...
				logger.warn(
					"[useGamification] Falha ao carregar inventário",
					{ err },
					"useGamification",
				);
				return [];
	...
	const { data: profileRaw, isLoading: isLoadingProfile } = useQuery({
		...
		queryFn: async () => {
			try {
				...
				logger.warn(
					"[useGamification] Falha ao carregar perfil",
					{ err },
					"useGamification",
				);
				return null;
	...
	const { data: questsRecord } = useQuery({
		...
		queryFn: async () => {
			try {
				...
				logger.warn(
					"[useGamification] Falha ao carregar missões",
					{ err },
					"useGamification",
				);
				return null;
	...
	const { data: achievementsData } = useQuery({
		...
		queryFn: async () => {
			try {
				...
				logger.warn(
					"[useGamification] Falha ao carregar conquistas",
					{ err },
					"useGamification",
				);
				return { all: [], unlocked: [] };
	...
	const { data: recentTransactions = [] } = useQuery({
		...
		queryFn: async () => {
			try {
				...
				logger.warn(
					"[useGamification] Falha ao carregar transações",
					{ err },
					"useGamification",
				);
				return [];
