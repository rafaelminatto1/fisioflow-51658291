...
async function fetchAppointments(
  ...
  logger.info('Carregando agendamentos da API principal', {}, 'useAppointmentsData');
  ...
  try {
    ...
    if (!organizationId) {
      logger.warn('Abortando fetch: organization_id não encontrado', {}, 'useAppointmentsData');
    ...
    const data = await retryWithBackoff(() =>
      withTimeout(
        AppointmentService.fetchAppointments(organizationId, { dateFrom, dateTo, limit: 500 }),
        15000
      )
    );
    ...
    logger.error('Erro crítico no fetchAppointments', error, 'useAppointmentsData');
    timer();
    if (isNetworkError(error)) {
      logger.warn('Erro de rede, usando fallback cache', {}, 'useAppointmentsData');
    }
...
export function useAppointmentsData(options: UseAppointmentsDataOptions = {}) {
  ...
  let finalData = [] as (typeof appointmentsQuery.data extends AppointmentsQueryResult ? AppointmentsQueryResult['data'] : never);
  ...
  if (result?.data && result.data.length > 0) {
    ...
    dataSource = 'previous';
    logger.debug('Usando dados anteriores do React Query como fallback', { count: finalData.length }, 'useAppointmentsData');
