...
export async function getFromCacheWithMetadata(
	...
	try {
		const cacheResult =
			await appointmentsCacheService.getFromCache(organizationId);
		if (cacheResult.data.length > 0) {
			const ageMinutes = appointmentsCacheService.getCacheAgeMinutes();
			logger.info(
				"Usando dados do IndexedDB (Fallback Camada 1)",
				{
		...
		logger.error(
			"Falha ao ler IndexedDB, tentando localStorage",
			indexedDbError,
			"useAppointmentsCache",
		);
	}
	...
	if (emergencyResult.data.length > 0) {
		logger.warn(
			"Usando backup de emergência do localStorage (Fallback Camada 2)",
			{
	...
	// CAMADA 3: sem dados
	logger.error(
		"NENHUM CACHE DISPONÍVEL - retornando array vazio",
		{},
		"useAppointmentsCache",
	);
	return {
