...
export function saveEmergencyBackup(
  ...
  try {
    ...
    localStorage.setItem(
      EMERGENCY_CACHE_KEY,
      JSON.stringify({ data: minimal, timestamp: new Date().toISOString(), organizationId, count: minimal.length })
    );
    logger.debug(`Backup de emergência salvo: ${minimal.length} agendamentos`, {}, 'appointmentHelpers');
  } catch (err) {
...
export function loadEmergencyBackup(organizationId?: string): EmergencyBackupResult {
  ...
  try {
    ...
    if (backupAge > EMERGENCY_CACHE_MAX_AGE) {
      logger.warn('Backup de emergência expirado', { ageHours: backupAge / 3600000 }, 'appointmentHelpers');
      return empty;
    }
    ...
    return { data: appointments, isFromCache: true, cacheTimestamp: backup.timestamp, source: 'localstorage' };
  } catch (err) {
