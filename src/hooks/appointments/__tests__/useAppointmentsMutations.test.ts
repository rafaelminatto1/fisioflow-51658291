...
vi.mock("@/services/appointmentService", () => ({
	AppointmentService: {
		...
		updateAppointment: vi
			.fn()
			.mockResolvedValue({ ...mockCreatedAppointment, status: "confirmed" }),
...
describe("useCreateAppointment", () => {
	...
	it("aplica optimistic update antes da resposta do servidor", async () => {
		const { useCreateAppointment } = await import(
	...
	it("faz rollback ao erro", async () => {
		...
		const afterError = queryClient.getQueryData<any>([
			"appointments_v2",
			"list",
			"org-001",
		]);
