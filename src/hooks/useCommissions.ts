...
export function useCommissionSummary(month: string) {
  return useQuery<{ data: CommissionSummaryRow[]; period: { start: string; end: string } }>({
...
export function useSetCommissionRate() {
  ...
  return useMutation({
    mutationFn: (input: { therapist_id: string; commission_rate: number; notes?: string }) =>
      request('/api/commissions/config', { method: 'POST', body: JSON.stringify(input) }),
    onSuccess: () => {
...
export function useMarkCommissionPaid() {
  ...
  return useMutation({
    mutationFn: (input: {
      ...
      commission_amount: number;
      notes?: string;
    }) => request('/api/commissions/payout', { method: 'POST', body: JSON.stringify(input) }),
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['commissions-summary'] });
      queryClient.invalidateQueries({ queryKey: ['commissions-therapist', variables.therapist_id] });
