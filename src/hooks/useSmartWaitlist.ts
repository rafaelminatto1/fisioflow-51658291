...
export function useSmartWaitlist(options: UseSmartWaitlistOptions = {}) {
	...
	const { data: waitlist = [], isLoading: isLoadingWaitlist } = useWaitlist({
		status: "waiting",
	});
	...
	const recommendationsQuery = useQuery({
		...
		queryFn: async (): Promise<WaitlistRecommendation[]> => {
			...
			try {
				...
				((appointmentsResponse?.data ?? []) as AppointmentRow[]).forEach(
					(appointment) => {
						if (appointment.date && appointment.start_time) {
							if (!blockedTimes[appointment.date])
								blockedTimes[appointment.date] = [];
				...
				logger.error(
					"Erro ao buscar bloqueios para waitlist",
					error,
					"useSmartWaitlist",
				);
			}
			...
			return generateWaitlistRecommendations(
				waitlist,
				availableSlots,
				candidatesPerSlot,
			);
...
export function useAutoOfferSlots() {
	...
	return useMutation({
		mutationFn: async ({
			...
			for (
				let i = 0;
				i < Math.min(recommendation.candidates.length, maxCandidates);
				i++
			) {
		...
		onSuccess: (results) => {
			...
			if (successCount > 0) {
				logger.info(
					`[useAutoOfferSlots] ${successCount} ofertas enviadas com sucesso`,
					undefined,
					"useSmartWaitlist",
				);
			}
			if (failCount > 0) {
				logger.warn(
					`[useAutoOfferSlots] ${failCount} ofertas falharam`,
					undefined,
					"useSmartWaitlist",
				);
			}
...
export function useSmartWaitlistManager(
	options: SmartWaitlistManagerOptions = {},
) {
	const { daysAhead = 14, candidatesPerSlot = 3 } = options;
	...
	const highPriorityCount = waitlist.filter(
		(e) => e.priority === "high",
	).length;
	...
	// Helpers
	const getRecommendationForDate = useCallback(
		(date: Date) => {
	...
	const getCandidatesForDate = useCallback(
		(date: Date): SlotCandidate[] => {
