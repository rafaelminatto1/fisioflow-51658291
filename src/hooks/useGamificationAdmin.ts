...
const buildEngagementData = (transactions: Array<{ patient_id: string; amount: number; reason: string; created_at: string }>): EngagementData[] => {
  const grouped = transactions.reduce<Record<string, EngagementData>>((acc, t) => {
    ...
    if (t.reason === 'achievement_unlocked') acc[date].achievementsUnlocked += 1;
    return acc;
  }, {});
  ...
  transactions.forEach((t) => {
    const date = t.created_at.split('T')[0];
    if (!uniquePatientsPerDate.has(date)) uniquePatientsPerDate.set(date, new Set());
...
export const useGamificationAdmin = (days = 30): UseGamificationAdminResult => {
  ...
  const { data: atRiskPatients, isLoading: atRiskPatientsLoading } = useQuery({
    ...
    queryFn: async (): Promise<AtRiskPatient[]> => {
      const res = await gamificationApi.getAtRiskPatients({ days: 7, limit: 50 });
      return res.data ?? [];
    },
    staleTime: 1000 * 60 * 5,
    gcTime: 1000 * 60 * 10,
  ...
  const { data: levelSettings, isLoading: levelSettingsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const find = (key: string) => settings.find((row) => row.key === key)?.value;

      return {
        progressionType: (find('level_progression_type') ?? find('progression_type') ?? 'linear') as ProgressionType,
        baseXp: parseSettingNumber(find('level_base_xp') ?? find('base_xp'), 1000),
        multiplier: parseSettingNumber(find('level_multiplier') ?? find('multiplier'), 1.5),
...
function useAdjustXp() {
  ...
  return useMutation({
    ...
    onSuccess: (result) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['gamification-patient-detail', result.patientId] });
      queryClient.invalidateQueries({ queryKey: ['gamification-transactions', result.patientId] });
      toast({
...
function useUpdateLevelSettings() {
  ...
  return useMutation({
    mutationFn: async (settings: { progressionType: ProgressionType; baseXp: number; multiplier: number }) => {
