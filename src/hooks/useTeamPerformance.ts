...
import { format, startOfMonth, endOfMonth, subMonths, startOfYear, endOfYear, startOfDay, endOfDay, differenceInCalendarDays } from 'date-fns';
import { ptBR } from 'date-fns/locale';
import { innovationsApi, organizationMembersApi, type StaffPerformanceRow } from '@/lib/api/workers-client';
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<TeamPerformanceMetrics> => {
      ...
      const therapistMetrics: TherapistPerformance[] = members.map((member) => {
        ...
        const sessions = rows.reduce((sum, row) => sum + Number(row.completed_appointments ?? 0), 0);
        const npsSource = rows.filter((row) => row.patient_satisfaction_avg != null);
        const nps = npsSource.length > 0
          ? Math.round(
              npsSource.reduce((sum, row) => sum + Number(row.patient_satisfaction_avg ?? 0), 0) / npsSource.length,
            )
          : 0;
        const newPatients = rows.reduce((sum, row) => sum + Number(row.new_patients ?? 0), 0);
        const returningPatients = rows.reduce((sum, row) => sum + Number(row.returning_patients ?? 0), 0);
        const retentionRate = newPatients + returningPatients > 0
      ...
      const totalSessions = therapistMetrics.reduce((sum, therapist) => sum + therapist.sessions, 0);
      const averageTicket = totalSessions > 0 ? Math.round(totalRevenue / totalSessions) : 0;
      const averageNps = therapistMetrics.length > 0
        ? Math.round(therapistMetrics.reduce((sum, therapist) => sum + therapist.nps, 0) / therapistMetrics.length)
        : 0;

      const totalNewPatients = currentRows.reduce((sum, row) => sum + Number(row.new_patients ?? 0), 0);
      const totalReturningPatients = currentRows.reduce((sum, row) => sum + Number(row.returning_patients ?? 0), 0);
      const overallRetentionRate = totalNewPatients + totalReturningPatients > 0
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        const monthData: MonthlyRevenue = { month: format(monthDate, 'MMM', { locale: ptBR }) };
