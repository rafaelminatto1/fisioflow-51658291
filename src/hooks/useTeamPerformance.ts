...
const aggregateMetrics = (rows: StaffPerformanceRow[]) => ({
	revenue: rows.reduce(
		(sum, row) => sum + Number(row.revenue_generated ?? 0),
		0,
	),
	sessions: rows.reduce(
...
export const useTeamPerformance = (
	filters: PerformanceFilters = { period: "month" },
) => {
	return useQuery({
		queryKey: [
			"team-performance",
		...
		queryFn: async (): Promise<TeamPerformanceMetrics> => {
			...
			const [membersRes, metricsRes, previousMetricsRes, sixMonthMetricsRes] =
				await Promise.all([
			...
			const members = (membersRes.data ?? []).filter(
				(member) =>
					member.active && ["admin", "fisioterapeuta"].includes(member.role),
			);
			const currentRows = (metricsRes.data ?? []) as StaffPerformanceRow[];
			const previousRows = (previousMetricsRes.data ??
				[]) as StaffPerformanceRow[];
			const historyRows = (sixMonthMetricsRes.data ??
				[]) as StaffPerformanceRow[];

			const therapistMetrics: TherapistPerformance[] = members.map((member) => {
				const rows = currentRows.filter(
					(row) => row.therapist_id === member.user_id,
				);
				...
				const npsSource = rows.filter(
					(row) => row.patient_satisfaction_avg != null,
				);
				const nps =
					npsSource.length > 0
						? Math.round(
								npsSource.reduce(
									(sum, row) => sum + Number(row.patient_satisfaction_avg ?? 0),
									0,
								) / npsSource.length,
							)
						: 0;
			...
			const totalRevenue = therapistMetrics.reduce(
				(sum, therapist) => sum + therapist.revenue,
				0,
			);
				...
				therapistMetrics.length > 0
					? Math.round(
							therapistMetrics.reduce(
								(sum, therapist) => sum + therapist.nps,
								0,
				...
				totalNewPatients + totalReturningPatients > 0
					? Math.round(
							(totalReturningPatients /
								(totalNewPatients + totalReturningPatients)) *
								100,
						)
					: 0;
			...
			for (let i = 5; i >= 0; i--) {
				...
				const monthRows = historyRows.filter((row) =>
					String(row.metric_date).startsWith(monthKey),
				);
			...
			const maxRevenue = Math.max(
				...therapistMetrics.map((therapist) => therapist.revenue),
				1,
			);
			const maxSessions = Math.max(
				...therapistMetrics.map((therapist) => therapist.sessions),
				1,
			);
			...
			therapistMetrics.slice(0, 4).forEach((therapist) => {
				radarData[0][therapist.name] = Math.round(
					(therapist.revenue / maxRevenue) * 100,
				);
				radarData[1][therapist.name] = Math.round(
					(therapist.sessions / maxSessions) * 100,
				);
			...
			return {
				...
				previousPeriodComparison: {
					revenueChange: pct(
						currentAggregate.revenue,
						previousAggregate.revenue,
					),
					sessionsChange: pct(
						currentAggregate.sessions,
						previousAggregate.sessions,
					),
					retentionChange: pct(
						currentAggregate.retention,
						previousAggregate.retention,
					),
				},
			};
		},
