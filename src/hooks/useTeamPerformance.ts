...
export const exportTeamPerformanceToExcel = async (data: TeamPerformanceMetrics) => {
  const XLSX = await import('xlsx');
  
  // Summary sheet
  const summaryData = [
    ['Resumo Executivo - Performance da Equipe'],
    [''],
    ['Métrica', 'Valor'],
    ['Receita Total', `R$ ${data.totalRevenue.toLocaleString('pt-BR')}`],
    ['Ticket Médio', `R$ ${data.averageTicket.toLocaleString('pt-BR')}`],
    ['Taxa de Retenção', `${data.retentionRate}%`],
    ['NPS Médio', data.averageNps.toString()],
    [''],
    ['Top 3 Fisioterapeutas'],
    ...data.therapists.slice(0, 3).map((t, i) => [`${i + 1}º - ${t.name}`, `R$ ${t.revenue.toLocaleString('pt-BR')}`])
  ];
  ...
  const revenueData = [
    ...
    [''],
    ['Posição', 'Nome', 'Receita', 'Sessões', 'Score'],
    ...data.therapists.map(t => [t.rank, t.name, t.revenue, t.sessions, t.score])
  ];
  
  // Retention data
  const retentionData = [
    ['Taxa de Retenção por Fisioterapeuta'],
    [''],
    ['Nome', 'Taxa de Retenção (%)', 'Pacientes Atendidos'],
    ...data.therapists.map(t => [t.name, t.retentionRate, t.sessions])
  ];
  ...
  const sessionsData = [
    ['Sessões Realizadas'],
    [''],
    ['Nome', 'Sessões Concluídas', 'Receita Gerada'],
    ...data.therapists.map(t => [t.name, t.sessions, t.revenue])
  ];
  ...
  const rawData = [
    ['Dados Brutos'],
    [''],
    ['ID', 'Nome', 'Receita', 'Sessões', 'Retenção', 'NPS', 'Score', 'Ranking'],
    ...data.therapists.map(t => [t.id, t.name, t.revenue, t.sessions, t.retentionRate, t.nps, t.score, t.rank])
  ];
  ...
  const ws3 = XLSX.utils.aoa_to_sheet(retentionData);
  const ws4 = XLSX.utils.aoa_to_sheet(sessionsData);
  const ws5 = XLSX.utils.aoa_to_sheet(rawData);
  
  XLSX.utils.book_append_sheet(wb, ws1, 'Resumo Executivo');
  XLSX.utils.book_append_sheet(wb, ws2, 'Receita por Fisioterapeuta');
  XLSX.utils.book_append_sheet(wb, ws3, 'Taxa de Retenção');
  XLSX.utils.book_append_sheet(wb, ws4, 'Sessões Realizadas');
  XLSX.utils.book_append_sheet(wb, ws5, 'Dados Brutos');
  
  XLSX.writeFile(wb, `performance_equipe_${format(new Date(), 'yyyy-MM-dd')}.xlsx`);
};
