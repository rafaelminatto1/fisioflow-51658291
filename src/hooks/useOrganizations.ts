...
import { organizationMembersApi, organizationsApi, profileApi } from '@/lib/api/workers-client';
...
const normalizeOrganization = (orgId: string | null | undefined, name?: string): Organization | null => {
...
const extractOrganizationId = (payload: Record<string, unknown> | null | undefined): string | null => {
...
export const useOrganizations = () => {
  ...
  const deriveCandidateOrganizationId = async (): Promise<string | null> => {
    ...
    const memberships = await organizationMembersApi.list({ userId: user.uid, limit: 1 });
  ...
  const createOrganization = useMutation({
    mutationFn: async (payload: { name: string; slug: string; settings?: Record<string, unknown>; active?: boolean }) => {
  ...
  const updateOrganization = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ORGANIZATION_KEYS.list });
      queryClient.invalidateQueries({ queryKey: ORGANIZATION_KEYS.current(user?.uid, profile?.organization_id, authOrganizationId) });
