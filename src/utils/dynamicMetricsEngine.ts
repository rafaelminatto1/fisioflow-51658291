...
export const analyzeDynamicComparison = (
	...
	metric_deltas.push(
		createDelta(
			METRIC_KEYS.GAIT.KNEE_VALGUS_L,
			"Valgo Din. (Esq)",
			"deg",
			metricsA.valgusL,
			metricsB.valgusL,
	...
	metric_deltas.push(
		createDelta(
			METRIC_KEYS.GAIT.KNEE_VALGUS_R,
			"Valgo Din. (Dir)",
			"deg",
			metricsA.valgusR,
			metricsB.valgusR,
	...
	const summary: Summary = {
		improvements: metric_deltas
			.filter((m) => m.status === "IMPROVED")
			.map((m) => m.label),
		still_to_improve: metric_deltas
			.filter((m) => m.status === "WORSE" || m.status === "UNCHANGED")
			.map((m) => m.label),
		key_findings: [
			"Comparação automática realizada baseada em picos cinemáticos.",
		],
...
function generateMarkdownTable(deltas: MetricDelta[]): string {
	let md =
		"| Métrica | Inicial | Reavaliação | Delta | Status |\n|---|---|---|---|---|\n";
	deltas.forEach((d) => {
