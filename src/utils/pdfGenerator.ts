...
export const generatePrescriptionPDF = (prescription: Prescription) => {
	...
	if (prescription.valid_until) {
		yPos += 6;
		doc.text(
			`Válido até: ${format(new Date(prescription.valid_until), "dd/MM/yyyy")}`,
			margin,
			yPos,
		);
	}
	...
	doc.save(
		`prescricao-${prescription.patient?.name?.replace(/\s+/g, "-").toLowerCase()}-${format(new Date(), "yyyyMMdd")}.pdf`,
	);
};
