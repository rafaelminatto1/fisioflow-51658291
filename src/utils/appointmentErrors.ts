...
function formatConflictMessage(details: AppointmentConflictDetails): string {
	...
	const patient =
		conflict.patient_name || conflict.patient_id || "este paciente";
	const therapist =
		conflict.therapist_name || conflict.therapist_id || "este terapeuta";
	const time = conflict.start_time
...
export function getAppointmentConflictUserMessage(
	error: unknown,
): string | null {
	...
	return isAppointmentConflictError(error)
