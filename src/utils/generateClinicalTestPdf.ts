...
export const generateClinicalTestPdf = (test: ClinicalTest) => {
	...
	doc.text(
		"Base de Dados de Testes Clínicos Baseados em Evidências",
		margin,
		yPos,
	);
	...
	autoTable(doc, {
		startY: yPos,
		body: [
			["Categoria", test.category || "N/A"],
			["Articulação Atvo", test.target_joint || "N/A"],
			[
				"Regularidade",
	...
	const purposeLines = doc.splitTextToSize(
		test.purpose || "Sem objetivo descrito.",
		pageWidth - margin * 2,
	);
	...
	const executionLines = doc.splitTextToSize(
		test.execution || "Sem instruções de execução.",
		pageWidth - margin * 2,
	);
	...
	if (test.positive_sign) {
		...
		const signLines = doc.splitTextToSize(
			test.positive_sign,
			pageWidth - margin * 2,
		);
	...
	if (test.reference || test.sensitivity_specificity) {
		...
		if (test.sensitivity_specificity) {
			doc.setFontSize(10);
			doc.text(
				`Sensibilidade/Especificidade: ${test.sensitivity_specificity}`,
				margin,
				yPos,
			);
			yPos += 7;
	...
	for (let i = 1; i <= pageCount; i++) {
		...
		doc.text(
			`FisioFlow - Gerenciamento Inteligente para Fisioterapia | Página ${i} de ${pageCount}`,
			pageWidth / 2,
