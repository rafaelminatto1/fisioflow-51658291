...
export const generateBiomechanicsReport = async (data: ReportData) => {
  ...
  if (data.metrics) {
    ...
    const tableData = Object.entries(data.metrics)
      ...
      .map(([key, value]) => [key.replace(/([A-Z])/g, " $1").toUpperCase(), String(value)]);
  ...
  if (data.asymmetry) {
    ...
    doc.text(
      isHighRisk
        ? "ALERTA: Risco de lesão elevado identificado."
        : "STATUS: Simetria dentro dos padrões clínicos.",
      20,
      currentY + 17,
    );
  ...
  addFooter(doc);
  doc.save(
    `Fisioflow_Biomecanica_${data.patientName.replace(/\s+/g, "_")}_${format(data.date, "yyyyMMdd")}.pdf`,
  );
};
    ...
    for (const { a, b } of pairs) {
      ...
      autoTable(doc, {
        ...
        head: [
          [
            "Métrica",
            `Sessão A (${format(a.date, "dd/MM")})`,
            `Sessão B (${format(b.date, "dd/MM")})`,
            "Δ Delta",
          ],
        ],
        body: rows,
