...
export const handleChunkError = (error: unknown, source: string): boolean => {
  ...
  if (reloadCount < 3) {
    ...
    logger.warn(
      `[Vite] Erro de carregamento de chunk detectado via ${source}. Recarregando (tentativa ${reloadCount + 1}/3)...`,
      {
        error,
        source,
  ...
  logger.error(
    `[Vite] Limite de recargas por erro de chunk atingido (${source}).`,
    {
      error,
      source,
    },
