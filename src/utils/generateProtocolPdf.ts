...
export const generateProtocolPdf = async (
	...
	// Milestones Table
	const milestones = Array.isArray(protocol.milestones)
		? protocol.milestones
		: [];
	if (milestones.length > 0) {
		...
		autoTable(doc, {
			...
			body: milestones.map(
				(m) => [`Semana ${m.week}`, m.description] as [string, string],
			),
			theme: "grid",
	...
	// Restrictions Table
	const restrictions = Array.isArray(protocol.restrictions)
	...
	// Footer
	const pageCount =
		(doc as { getNumberOfPages?: () => number }).getNumberOfPages?.() || 1;
