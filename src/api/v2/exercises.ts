...
export const exercisePlansApi = {
  list: (params: { patientId: string }) =>
    request<{ data: any[] }>(`/api/clinical/exercise-plans?patientId=${encodeURIComponent(params.patientId)}`),
  create: (data: { patient_id: string; template_id?: string; name: string; notes?: string; items: any[] }) =>
    request<{ data: any }>('/api/clinical/exercise-plans', {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  delete: (id: string) =>
    request<{ ok: boolean }>(`/api/clinical/exercise-plans/${id}`, { method: 'DELETE' }),
};
