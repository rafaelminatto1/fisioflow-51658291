...
export const exercisesApi = {
	...
	analyzeImage: async (imageUrl: string) => {
		...
		const apiBase = (
			import.meta.env.VITE_EXERCISE_ANALYSIS_API_URL ?? ""
		).replace(/\/$/, "");
		if (!apiBase) {
...
export const exerciseTemplatesApi = {
	...
	get: (id: string) =>
		request<{ data: ExerciseTemplateRecord }>(
...
export const protocolsApi = {
	list: (params?: {
		...
		return request<PaginatedResponse<Protocol>>(
			`/api/protocols${qs ? `?${qs}` : ""}`,
		);
	},

	get: (id: string) =>
		request<{ data: Protocol & { protocolExercises: unknown[] } }>(
			`/api/protocols/${id}`,
		),
...
export const templatesApi = {
	list: (params?: {
		...
		return request<PaginatedResponse<ExerciseTemplate>>(
			`/api/templates${qs ? `?${qs}` : ""}`,
		);
	},
