...
export const medicalRequestsApi = {
  ...
  delete: (id: string) =>
    request<{ ok: boolean; deleted_files?: string[] }>(`/api/medical-requests/${id}`, {
      method: "DELETE",
    }),

  deleteFile: (requestId: string, fileId: string) =>
    request<{ ok: boolean }>(`/api/medical-requests/${requestId}/files/${fileId}`, {
      method: "DELETE",
    }),
};
...
export const sessionsApi = {
  list: (params: {
    ...
    return request<{ data: SessionRecord[]; total: number }>(`/api/sessions${qs ? `?${qs}` : ""}`);
  },
...
export const biomechanicsApi = {
  ...
  listByPatient: (params: {
    ...
    return request<{
      data: (SessionRecord & { biomechanics_data?: BiomechanicsData })[];
      total: number;
    }>(`/api/sessions${qs ? `?${qs}` : ""}`);
  },
};
...
export const clinicalApi = {
  ...
  evolutionTemplates: {
    list: (p?: { ativo?: boolean }) =>
      clin(`/evolution-templates${p?.ativo != null ? `?ativo=${p.ativo}` : ""}`),
    ...
    delete: (id: string) => clin(`/evolution-templates/${id}`, { method: "DELETE" }),
  },
  conductLibrary: {
  ...
  prescribedExercises: {
    ...
    delete: (id: string) => clin(`/prescribed-exercises/${id}`, { method: "DELETE" }),
  },
  patientObjectives: {
    ...
    delete: (id: string) => clin(`/patient-objectives/${id}`, { method: "DELETE" }),
  },
  patientObjectiveAssignments: {
    ...
    delete: (id: string) => clin(`/patient-objective-assignments/${id}`, { method: "DELETE" }),
  },
  standardizedTests: {
    list: (patientId: string) =>
