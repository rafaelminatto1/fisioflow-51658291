...
export const knowledgeApi = {
  ...
  upsertAnnotation: (
    ...
    request<{ data: KnowledgeAnnotationRow }>(
      `/api/knowledge/annotations/${encodeURIComponent(articleId)}`,
      {
        method: "PUT",
        body: JSON.stringify(data),
      },
  ...
  updateCuration: (
    ...
    request<{ data: KnowledgeCurationRow }>(
      `/api/knowledge/curation/${encodeURIComponent(articleId)}`,
      {
        method: "PUT",
        body: JSON.stringify(data),
      },
  ...
  addAuditEntry: (data: Omit<KnowledgeAuditRow, "id" | "organization_id" | "created_at">) =>
  ...
  getProfiles: (ids: string[]) => {
    ...
    return request<{ data: Record<string, KnowledgeProfileSummary> }>(
      `/api/knowledge/profiles${qs.toString() ? `?${qs}` : ""}`,
    );
  },
  listNotes: (articleId: string) =>
    request<{ data: Array<Record<string, unknown>> }>(
      `/api/knowledge/articles/${encodeURIComponent(articleId)}/notes`,
    ),
  ...
  askArticle: (articleId: string, query: string) =>
    request<{ data: { answer: string; contextUsed?: string } }>(
      `/api/knowledge/articles/${encodeURIComponent(articleId)}/ask`,
      {
        method: "POST",
  ...
  processArticle: (articleId: string, textContent?: string) =>
    request<{ data: { success: boolean } }>(
      `/api/knowledge/articles/${encodeURIComponent(articleId)}/process`,
      {
        method: "POST",
...
export const wikiApi = {
  ...
  get: (slug: string) => request<{ data: WikiPageFull }>(`/api/wiki/${slug}`),

  children: (slug: string) => request<{ data: WikiPage[] }>(`/api/wiki/${slug}/children`),

  versions: (slug: string) => request<{ data: unknown[] }>(`/api/wiki/${slug}/versions`),

  create: (
    data: Omit<
      WikiPageFull,
      "id" | "slug" | "updatedAt" | "createdAt" | "viewCount" | "version"
    > & { comment?: string },
  ) =>
  ...
  listCategories: () => request<{ data: unknown[] }>("/api/wiki/categories"),
  ...
  deleteCategory: (id: string) =>
    request<{ ok: boolean }>(`/api/wiki/categories/${id}`, {
      method: "DELETE",
    }),

  listComments: (pageId: string) => request<{ data: unknown[] }>(`/api/wiki/${pageId}/comments`),
  ...
  listDictionary: (params?: { q?: string; category?: string }) => {
    ...
    return request<{ data: unknown[] }>(`/api/wiki/dictionary${qs ? `?${qs}` : ""}`);
  },
  ...
  updateDictionaryTerm: (id: string, data: Record<string, unknown>) =>
    request<{ data: unknown }>(`/api/wiki/dictionary/${encodeURIComponent(id)}`, {
      method: "PUT",
      body: JSON.stringify(data),
    }),
