...
export const knowledgeApi = {
	listArticles: () =>
		request<{ data: KnowledgeArticleRow[] }>("/api/knowledge/articles"),
	...
	semanticSearch: (params: { query: string; limit?: number }) =>
		request<{ data: KnowledgeSemanticResultRow[] }>(
	...
	listAnnotations: () =>
		request<{ data: KnowledgeAnnotationRow[] }>("/api/knowledge/annotations"),
	upsertAnnotation: (
		articleId: string,
		data: {
	...
	listCuration: () =>
		request<{ data: KnowledgeCurationRow[] }>("/api/knowledge/curation"),
	updateCuration: (
		articleId: string,
...
export const wikiApi = {
	list: (params?: {
		...
		return request<PaginatedResponse<WikiPage>>(
			`/api/wiki${qs ? `?${qs}` : ""}`,
		);
	},
	...
	listOrg: (params?: { q?: string; category?: string }) => {
		...
		return request<{ data: WikiPage[] }>(
			`/api/wiki/org/list${qs ? `?${qs}` : ""}`,
		);
	},

	getById: (id: string) =>
		request<{ data: WikiPageFull }>(
			`/api/wiki/by-id/${encodeURIComponent(id)}`,
		),
