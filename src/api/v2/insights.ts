...
export const analyticsApi = {
	...
	intelligentReports: {
		...
		generate: (payload: {
			...
			request<{ data: IntelligentReportResponse }>(
				"/api/insights/intelligent-reports",
				{
					method: "POST",
	...
	patientEvolution: (patientId: string) =>
		request<{ data: PatientEvolutionPoint[] }>(
			`/api/insights/patient-evolution/${patientId}`,
		),
	patientProgress: (patientId: string) =>
		request<{ data: PatientProgressSummary }>(
			`/api/insights/patient-progress/${patientId}`,
		),
	patientLifecycleEvents: {
		...
		create: (data: {
			...
			request<{ data: PatientLifecycleEvent }>(
				"/api/insights/patient-lifecycle-events",
				{
					method: "POST",
	...
	patientOutcomeMeasures: {
		list: (
			patientId: string,
			params?: { measureType?: string; limit?: number },
		) => {
			const qs = new URLSearchParams(
		...
		create: (data: {
			...
			request<{ data: PatientOutcomeMeasure }>(
				"/api/insights/patient-outcome-measures",
				{
					method: "POST",
					body: JSON.stringify(data),
				},
	...
	patientSessionMetrics: {
		...
		create: (data: Partial<PatientSessionMetrics & { patient_id: string }>) =>
			request<{ data: PatientSessionMetrics }>(
				"/api/insights/patient-session-metrics",
				{
					method: "POST",
					body: JSON.stringify(data),
				},
	...
	patientPredictions: {
		list: (
			patientId: string,
			params?: { predictionType?: string; limit?: number },
		) => {
			const qs = new URLSearchParams(
				Object.entries(params ?? {})
					.filter(([, v]) => v != null)
		...
		upsert: (payload: {
			...
			request<{ data: PatientPrediction[] }>(
				"/api/insights/patient-predictions/upsert",
				{
					method: "POST",
					body: JSON.stringify(payload),
				},
	...
	patientRisk: (patientId: string) =>
		request<{ data: PatientRiskScore | null }>(
			`/api/insights/patient-risk/${patientId}`,
		),
	patientInsights: {
		...
		acknowledge: (insightId: string) =>
			request<{ data: PatientInsight }>(
				`/api/insights/patient-insights/${insightId}/acknowledge`,
				{
					method: "PATCH",
				},
			),
	...
	patientGoals: {
		list: (patientId: string) =>
			request<{ data: PatientGoalTracking[] }>(
				`/api/insights/patient-goals/${patientId}`,
			),
		...
		complete: (id: string) =>
			request<{ data: PatientGoalTracking }>(
				`/api/insights/patient-goals/${id}/complete`,
				{
					method: "PATCH",
				},
			),
	...
	clinicalBenchmarks: {
		list: (category?: string) => {
			...
			return request<{ data: ClinicalBenchmark[] }>(
				`/api/insights/clinical-benchmarks${qs}`,
			);
		},
	},
	mlTrainingData: {
		collect: (patientId: string) =>
			request<{ data: MLTrainingData }>(
				`/api/insights/ml-training-data/patient/${patientId}`,
			),
		...
		stats: () =>
			request<{ data: MlTrainingStats }>(
				"/api/insights/ml-training-data/stats",
			),
...
export const innovationsApi = {
	inventory: {
		list: (params?: { activeOnly?: boolean }) => {
			...
			return innovations(
				`/inventory${query.toString() ? `?${query.toString()}` : ""}`,
			);
		},
	...
	inventoryMovements: {
		list: (params?: { inventoryId?: string; limit?: number }) => {
			...
			return innovations(
				`/inventory-movements${query.toString() ? `?${query.toString()}` : ""}`,
			);
		},
	...
	staffPerformance: {
		list: (params?: {
			...
			return innovations(
				`/staff-performance${query.toString() ? `?${query.toString()}` : ""}`,
			);
		},
	},
	...
	revenueForecasts: {
		list: (params?: { limit?: number }) => {
			...
			return innovations(
				`/revenue-forecasts${query.toString() ? `?${query.toString()}` : ""}`,
			);
		},
	},
