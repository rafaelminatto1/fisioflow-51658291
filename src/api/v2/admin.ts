...
export const sessionAttachmentsApi = {
  ...
  delete: (sessionId: string, attachmentId: string) =>
    request<{ ok: boolean }>(`/api/sessions/${sessionId}/attachments/${attachmentId}`, {
      method: "DELETE",
    }),
};
...
export const goalProfilesApi = {
  ...
  get: (id: string) => request<{ data: GoalProfileRow }>(`/api/goal-profiles/${id}`),
...
export const adminDoctorsApi = {
  ...
  search: (params: { searchTerm: string; limit?: number }) => adminDoctorsApi.list(params),
