...
export const patientsApi = {
  ...
  delete: (id: string, mode?: "hard") =>
    request<{ success: boolean }>(`/api/patients/${id}${mode ? `?mode=${mode}` : ""}`, {
      method: "DELETE",
    }),
  stats: (id: string) => request<{ data: PatientStats }>(`/api/patients/${id}/stats`),
  lastUpdated: () =>
    request<{ data: { last_updated_at: string | null } }>("/api/patients/last-updated"),
  ...
  updatePhysicalExamination: (
    ...
    examId: string,
    data: Partial<
      Omit<PatientPhysicalExamination, "id" | "patient_id" | "created_at" | "updated_at">
    >,
  ) =>
    request<{ data: PatientPhysicalExamination }>(
