...
export const appointmentsApi = {
	list: (params?: {
		...
		return request<{ data: AppointmentRow[] }>(
			`/api/appointments${qs ? `?${qs}` : ""}`,
		);
	},
	get: (id: string) =>
		request<{ data: AppointmentRow & { patient?: PatientRow | null } }>(
			`/api/appointments/${id}`,
		),
	checkConflict: (data: {
		therapistId: string;
	...
	update: (
		id: string,
		data: Partial<AppointmentRow> & Record<string, unknown>,
	) =>
	...
	lastUpdated: () =>
		request<{ data: AppointmentsLastUpdated }>(
...
export const schedulingApi = {
	...
	getIntervalCapacity: (params: {
		date: string;
		startTime: string;
		endTime: string;
	}) => {
		const qs = new URLSearchParams(params).toString();
		return request<{ capacity: number }>(
			`/api/scheduling/interval-capacity?${qs}`,
		);
	},
};
