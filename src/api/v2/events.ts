...
export const eventosApi = {
  list: (p?: { status?: string; categoria?: string; limit?: number; offset?: number }) =>
...
export const checklistApi = {
  list: (eventoId: string) =>
    request<{ data: ChecklistItem[] }>(`/api/checklist?eventoId=${encodeURIComponent(eventoId)}`),
...
export const eventoContratadosApi = {
  list: (p?: { eventoId?: string; contratadoId?: string }) =>
    request<{ data: EventoContratado[] }>(withQuery("/api/activity-contractors", p)),
