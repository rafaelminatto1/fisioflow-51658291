...
export const integrationsApi = {
	...
	google: {
		...
		status: () =>
			request<{ data: GoogleIntegrationRecord | null }>(
		...
		calendar: {
			...
			logs: () =>
				request<{ data: GoogleSyncLogRecord[] }>(
					"/api/integrations/google/calendar/logs",
				),
...
export const securityApi = {
	lgpd: {
		...
		update: (
			consentType: string,
			data: { granted: boolean; version?: string },
		) =>
	...
	mfa: {
		...
		enable: (method: string) =>
			request<{ data: MFASettingsRecord; backupCodes: string[] }>(
		...
		verifyEnrollment: (factorId: string, code: string) =>
			request<{ data: { verified: boolean } }>(
				"/api/security/mfa/enroll/verify",
				{
					method: "POST",
...
export const organizationMembersApi = {
	...
	update: (
		id: string,
		data: Partial<Pick<OrganizationMember, "role" | "active">>,
	) =>
...
export const invitationsApi = {
	...
	update: (
		id: string,
		data: { email?: string; role?: string; expires_at?: string },
	) =>
	...
	delete: (id: string) =>
		request<{ ok: boolean }>(`/api/invitations/${id}`, { method: "DELETE" }),
	validate: (token: string) =>
		requestPublic<{ data: { email: string; role: string } }>(
...
export const healthApi = {
	check: () =>
		requestPublic<{ status: string; timestamp: string }>("/api/health"),
};
