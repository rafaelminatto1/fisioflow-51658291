...
export const financialApi = {
  transacoes: { 
    list: (p?: {tipo?:string;status?:string;dateFrom?:string;dateTo?:string;limit?:number;offset?:number}) => fin(`/transacoes?${new URLSearchParams(Object.fromEntries(Object.entries(p??{}).filter(([,v])=>v!=null).map(([k,v])=>[k,String(v)])))}`), 
    create: (d: Partial<Transacao>) => fin('/transacoes',{method:'POST',body:JSON.stringify(d)}), 
    update: (id:string,d:Partial<Transacao>)=>fin(`/transacoes/${id}`,{method:'PUT',body:JSON.stringify(d)}), 
    delete: (id:string)=>fin(`/transacoes/${id}`,{method:'DELETE'}) 
  },
  contas: { 
    list: (p?: {tipo?:string;status?:string;dateFrom?:string;dateTo?:string;limit?:number;offset?:number}) => fin(`/contas?${new URLSearchParams(Object.fromEntries(Object.entries(p??{}).filter(([,v])=>v!=null).map(([k,v])=>[k,String(v)])))}`), 
    create: (d: Partial<ContaFinanceira>) => fin('/contas',{method:'POST',body:JSON.stringify(d)}), 
    update: (id:string,d:Partial<ContaFinanceira>)=>fin(`/contas/${id}`,{method:'PUT',body:JSON.stringify(d)}), 
    delete: (id:string)=>fin(`/contas/${id}`,{method:'DELETE'}) 
  },
  centrosCusto: { 
    list: () => fin('/centros-custo'), 
    create: (d:Partial<CentroCusto>)=>fin('/centros-custo',{method:'POST',body:JSON.stringify(d)}), 
    update:(id:string,d:Partial<CentroCusto>)=>fin(`/centros-custo/${id}`,{method:'PUT',body:JSON.stringify(d)}), 
    delete:(id:string)=>fin(`/centros-custo/${id}`,{method:'DELETE'}) 
  },
  convenios: { 
    list: () => fin('/convenios'), 
    create: (d:Partial<Convenio>)=>fin('/convenios',{method:'POST',body:JSON.stringify(d)}), 
    update:(id:string,d:Partial<Convenio>)=>fin(`/convenios/${id}`,{method:'PUT',body:JSON.stringify(d)}), 
