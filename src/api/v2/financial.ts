...
export type FinancialCommandCenterPeriod = "daily" | "weekly" | "monthly" | "all";
...
export const financialApi = {
  ...
  transacoes: {
    ...
    create: (d: Partial<Transacao>) =>
      fin("/transactions", { method: "POST", body: JSON.stringify(d) }),
    update: (id: string, d: Partial<Transacao>) =>
      fin(`/transactions/${id}`, { method: "PUT", body: JSON.stringify(d) }),
    delete: (id: string) => fin(`/transactions/${id}`, { method: "DELETE" }),
  },
  contas: {
    list: (p?: {
      tipo?: string;
    ...
    create: (d: Partial<ContaFinanceira>) =>
      fin("/accounts", { method: "POST", body: JSON.stringify(d) }),
    update: (id: string, d: Partial<ContaFinanceira>) =>
      fin(`/accounts/${id}`, { method: "PUT", body: JSON.stringify(d) }),
    delete: (id: string) => fin(`/accounts/${id}`, { method: "DELETE" }),
  },
  centrosCusto: {
    list: () => fin("/cost-centers"),
    create: (d: Partial<CentroCusto>) =>
      fin("/cost-centers", { method: "POST", body: JSON.stringify(d) }),
    update: (id: string, d: Partial<CentroCusto>) =>
      fin(`/cost-centers/${id}`, { method: "PUT", body: JSON.stringify(d) }),
    delete: (id: string) => fin(`/cost-centers/${id}`, { method: "DELETE" }),
  },
  convenios: {
    list: () => fin("/convenios"),
  ...
  pagamentos: {
    list: (
      ...
      return fin(`/payments${query.toString() ? `?${query.toString()}` : ""}`);
    },
    create: (d: Partial<Pagamento>) =>
      fin("/payments", { method: "POST", body: JSON.stringify(d) }),
    update: (id: string, d: Partial<Pagamento>) =>
      fin(`/payments/${id}`, { method: "PUT", body: JSON.stringify(d) }),
    delete: (id: string) => fin(`/payments/${id}`, { method: "DELETE" }),
  },
  empresasParceiras: {
  ...
  vouchers: {
    ...
    checkout: (id: string) => fin(`/vouchers/${id}/checkout`, { method: "POST" }),
