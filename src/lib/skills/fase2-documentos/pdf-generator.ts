...
abstract class BasePDFGenerator {
	...
	protected addText(
		text: string,
		maxWidth: number = this.pageWidth - 40,
	): void {
	...
	protected addSignature(professional: ProfessionalData, city: string): void {
		...
		this.doc.setFontSize(10);
		this.doc.setFont(...CONFIG.fonts.bold);
		this.doc.text(professional.name, 85, signatureYFinal + 5, {
			align: "center",
		});
		this.doc.setFont(...CONFIG.fonts.normal);
		this.doc.text(
			`Fisioterapeuta ${professional.crf}-${professional.uf}`,
			85,
			signatureYFinal + 10,
			{ align: "center" },
		);
		...
		this.doc.text(`${city}, ${today}`, 85, signatureYFinal + 15, {
			align: "center",
		});
...
export class AtestadoGenerator extends BasePDFGenerator {
	generate(
		...
		this.doc.setFont(...CONFIG.fonts.bold);
		this.doc.setTextColor(...CONFIG.colors.primary);
		this.doc.text("ATESTADO", this.pageWidth / 2, this.yPosition, {
			align: "center",
		});
		this.yPosition += 15;
		...
		if (data.cid) {
			this.yPosition += 5;
			this.doc.setFont(...CONFIG.fonts.bold);
			this.doc.text(`CID: ${data.cid}`, this.pageWidth / 2, this.yPosition, {
				align: "center",
			});
			this.yPosition += 10;
		}
...
export class ReceituarioGenerator extends BasePDFGenerator {
	generate(
		...
		for (const prescription of data.prescriptions) {
			const tipo =
				prescription.type === "exercicio"
					? "Exercício"
					: prescription.type === "medicamento"
						? "Medicamento"
						: "Orientação";
		...
		return this.generate(
			`receituario-${patient.name.replace(/\s+/g, "-")}.pdf`,
		);
	}
}
	...
	generate(
		...
		for (const evaluation of data.evaluations) {
			this.addSectionTitle(
				`Evolução - ${format(evaluation.date, "dd/MM/yyyy")}`,
			);
...
export class PlanoTratamentoGenerator extends BasePDFGenerator {
	generate(
		...
		this.yPosition =
			(this.doc as { lastAutoTable: { finalY: number } }).lastAutoTable.finalY +
			15;

		// Duração estimada
		this.doc.setFont(...CONFIG.fonts.bold);
		this.doc.text(
			`Duração estimada: ${data.estimatedDuration}`,
			25,
			this.yPosition,
		);
		...
		return this.generate(
			`plano-tratamento-${patient.name.replace(/\s+/g, "-")}.pdf`,
		);
	}
}
...
export type { PatientData, ProfessionalData, ClinicData };
