...
export class ReceiptGenerator {
	...
	private numberToExtensive(value: number): string {
		...
		if (reais > 0) {
			if (reais >= 1000) {
				...
				result +=
					(milhares === 1 ? "mil" : this.numberToExtensive(milhares) + " mil") +
					" ";
			}
	...
	private addHeader(data: ReceiptData) {
		...
		if (data.clinicCNPJ || data.clinicPhone) {
			...
			const info = [
				data.clinicCNPJ ? `CNPJ: ${data.clinicCNPJ}` : null,
				data.clinicPhone ? `Tel: ${data.clinicPhone}` : null,
			]
				.filter(Boolean)
				.join(" | ");
		...
		if (data.clinicAddress) {
			...
			this.doc.text(data.clinicAddress, this.pageWidth / 2, 30, {
				align: "center",
			});
		}
		...
		this.doc.text(`Nº ${data.receiptNumber}`, this.pageWidth / 2, 52, {
			align: "center",
		});
	...
	private addBody(data: ReceiptData) {
		...
		// Valor por extenso
		const amountExtensive =
			data.amountExtensive || this.numberToExtensive(data.amount);
		...
		this.doc.text(
			`Forma de pagamento: ${paymentLabels[data.paymentMethod] || data.paymentMethod}`,
			25,
			this.yPosition,
		);
		this.yPosition += 8;
		...
		if (data.professionalName) {
			this.doc.setFontSize(10);
			this.doc.setFont("helvetica", "bold");
			this.doc.text(data.professionalName, 85, this.yPosition + 20, {
				align: "center",
			});
			...
			if (data.professionalCRF) {
				this.doc.setFont("helvetica", "normal");
				this.doc.setFontSize(9);
				this.doc.text(data.professionalCRF, 85, this.yPosition + 25, {
					align: "center",
				});
			}
		} else {
			this.doc.setFontSize(10);
			this.doc.setFont("helvetica", "bold");
			this.doc.text(data.clinicName, 85, this.yPosition + 20, {
				align: "center",
			});
		}
	...
	private addFooter() {
		...
		const today = format(new Date(), "dd 'de' MMMM 'de' yyyy' às 'HH:mm", {
			locale: ptBR,
		});
		this.doc.text(`Emitido em: ${today}`, this.pageWidth - 20, footerY, {
			align: "right",
		});
	...
	public save(data: ReceiptData, filename?: string): void {
		...
		link.download =
			filename ||
			`recibo-${data.receiptNumber}-${format(data.date, "yyyy-MM-dd")}.pdf`;
