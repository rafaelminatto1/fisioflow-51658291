...
export async function exportPatientsToExcel(
	...
	worksheet.getCell(`C${statsRow + 1}`).value = patients.filter(
		(p) => p.status === "active",
	).length;
	...
	worksheet.getCell(`C${statsRow + 2}`).value = patients.reduce(
...
export async function exportFinancialReport(
	...
	const totalExpenses =
		data.expenses?.reduce((sum, e) => sum + e.value, 0) || 0;
	...
	[revenueSheet, summarySheet].forEach((sheet) => {
		...
		sheet.mergeCells(
			`A${headerRow}:${String.fromCharCode(65 + sheet.columnCount - 1)}${headerRow}`,
		);
 ...
 */
export async function importPatientsFromExcel(buffer: Buffer): Promise<
	Array<{
		name: string;
		cpf?: string;
		birthDate?: Date;
...
> {
	...
	worksheet.eachRow((row, rowNumber) => {
		...
		const patient = {
			...
			birthDate:
				row.getCell(3).value instanceof Date ? row.getCell(3).value : undefined,
 ...
 */
export function uploadExcelFile(file: File): Promise<ArrayBuffer> {
