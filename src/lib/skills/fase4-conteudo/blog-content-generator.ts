...
#Fisioterapia #SaúdeOcupacional #QualidadeDeVida`;
  ...
  private generateHashtags(tags: string[]): string[] {
    return tags.map((tag) => `#${tag.replace(/\s+/g, "").replace(/-/g, "")}`).slice(0, 10);
  }
    ...
    const allTopics = [
      {
        ...
        category: "exercicios",
        keywords: ["dor lombar", "coluna", "exercícios", "alongamento", "fortalecimento"],
      },
      {
        title: "Como Prevenir Lesões no Trabalho: Guia Completo",
        category: "prevencao",
        keywords: ["LER", "ergonomia", "trabalho", "saúde ocupacional", "prevenção"],
      },
      {
        title: "Fisioterapia Pós-Operatória: O Que Esperar",
        category: "reabilitacao",
        keywords: ["pós-operatório", "cirurgia", "recuperação", "fisioterapia", "reabilitação"],
      },
      {
        title: "A Ciência do Alongamento: Quando e Como Fazer",
        category: "exercicios",
        keywords: ["alongamento", "flexibilidade", "exercícios", "mobilidade", "ciência"],
      },
      {
      ...
      },
      {
        ...
        category: "reabilitacao",
        keywords: ["joelho", "reabilitação", "ligamento", "menisco", "fisioterapia"],
      },
      {
        title: "10 Dicas para Melhorar Sua Postura Hoje",
        category: "bem-estar",
        keywords: ["postura", "coluna", "saúde", "ergonomia", "qualidade de vida"],
      },
      {
        title: "Fisioterapia para Atletas: Prevenção e Performance",
        category: "exercicios",
        keywords: ["atletas", "esporte", "performance", "prevenção", "fisioterapia esportiva"],
      },
      {
        title: "Entendendo a Dor Crônica: Novas Abordagens",
        category: "saude",
        keywords: ["dor crônica", "tratamento", "fisioterapia", "qualidade de vida", "saúde"],
      },
      {
        title: "Exercícios em Casa: Guia Durante a Quarentena",
        category: "exercicios",
        keywords: ["casa", "quarentena", "exercícios", "treino em casa", "saúde"],
      },
    ];
...
export class ContentGeneratorFactory {
  ...
  static async generateTopicFromTrend(trend: string): Promise<ArticleTopic> {
    ...
    if (keywords.some((k) => ["exercício", "treino", "musculo", "alongamento"].includes(k))) {
      category = "exercicios";
    } else if (keywords.some((k) => ["prevenir", "evitar", "lesão"].includes(k))) {
      category = "prevencao";
    } else if (keywords.some((k) => ["recuperar", "reabilitar", "pós"].includes(k))) {
      category = "reabilitacao";
