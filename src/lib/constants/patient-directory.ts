...
export type PatientCareProfile = (typeof PATIENT_CARE_PROFILE_OPTIONS)[number]["value"];

export type PatientTherapyFocus = (typeof PATIENT_THERAPY_FOCUS_OPTIONS)[number]["value"];

export type PatientPayerModel = (typeof PATIENT_PAYER_MODEL_OPTIONS)[number]["value"];

export type PatientFinancialStatus = (typeof PATIENT_FINANCIAL_STATUS_OPTIONS)[number]["value"];
...
export const PATIENT_PATHOLOGY_STATUS_LABELS = Object.fromEntries(
  PATIENT_DIRECTORY_PATHOLOGY_STATUSES.map((option) => [option.value, option.label]),
