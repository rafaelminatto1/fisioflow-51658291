 ...
 */
export function buildSelectQuery<T extends readonly string[]>(
	columns: T,
): string {
	return columns.join(",");
}
 ...
 */
export const VALID_APPOINTMENT_COLUMNS = new Set<AppointmentColumn>(
	APPOINTMENT_COLUMNS_STANDARD,
);
...
export function validateAppointmentSelectQuery(query: string): void {
	...
	for (const column of columns) {
		...
		if (
			INVALID_APPOINTMENT_COLUMNS.has(baseColumn as InvalidAppointmentColumn)
		) {
			throw new Error(
