...
import type { TimeEntry, TimeEntryFilters, ActiveTimer } from '@/types/timetracking';
...
export async function getTimeEntries(
  ...
  const result = await timeEntriesApi.list({
    ...
    startDate: filters.start_date ? (filters.start_date instanceof Date ? filters.start_date.toISOString() : String(filters.start_date)) : undefined,
    endDate: filters.end_date ? (filters.end_date instanceof Date ? filters.end_date.toISOString() : String(filters.end_date)) : undefined,
...
export async function saveActiveTimerDraft(userId: string, timer: ActiveTimer): Promise<void> {
  await timeEntriesApi.saveTimerDraft(userId, timer);
}

export async function getActiveTimerDraft(userId: string): Promise<ActiveTimer | null> {
...
export async function finalizeTimer(
  ...
  const entry = await createTimeEntry(organizationId, {
    ...
    total_value: timer.hourly_rate ? (durationSeconds / 3600) * timer.hourly_rate : undefined,
...
export async function getDailyBreakdown(
  ...
  entries.forEach((entry) => {
    ...
    if (!byDate.has(dateKey)) byDate.set(dateKey, { total_seconds: 0, billable_seconds: 0, entries: 0 });
