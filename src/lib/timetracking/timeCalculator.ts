 ...
 */
export function calculateTimeStats(entries: TimeEntry[], timeZone: string = "UTC"): TimeStats {
  const now = new Date();
  ...
  const { week_start: _lastWeekStart } = getWeekBounds(now.getFullYear(), getWeekNumber(now) - 1);
  ...
  const averageDaily = last30DaysEntries.length > 0 ? sumDurations(last30DaysEntries) / 30 : 0;
...
export function generateProductivityReport(
  ...
  return {
    ...
    hourly_average:
      totalSeconds > 0
        ? (filteredEntries.reduce((sum, e) => sum + (e.total_value || 0), 0) / totalSeconds) * 3600
        : 0,
  };
}
