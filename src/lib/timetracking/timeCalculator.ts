...
export function groupByDay(
	...
	return Array.from(grouped.entries())
		.map(([date, dayEntries]) => {
			const { billableSeconds, _nonBillableSeconds } =
				splitBillableTime(dayEntries);
			return {
				date,
...
export function groupByWeek(
	...
	return Array.from(grouped.entries()).map(([key, weekEntries]) => {
		const { billableSeconds, nonBillableSeconds } =
			splitBillableTime(weekEntries);
		...
		return {
			...
			total_value: weekEntries.reduce(
				(sum, e) => sum + (e.total_value || 0),
				0,
			),
...
export function groupByProject(entries: TimeEntry[]): ProjectTimeSummary[] {
	const grouped = new Map<
		string,
		{ entries: TimeEntry[]; totalValue: number }
	>();
	...
	return Array.from(grouped.entries()).map(
		([projectId, { entries, totalValue }]) => ({
			project_id: projectId,
			project_title: entries[0]?.project_id
				? `Project ${projectId}`
				: "Sem projeto",
...
export function calculateTimeStats(
	...
	const averageDaily =
		last30DaysEntries.length > 0 ? sumDurations(last30DaysEntries) / 30 : 0;
	...
	return {
		...
		this_week: {
			...
			billable_seconds: sumDurations(
				thisWeekEntries.filter((e) => e.is_billable),
			),
		...
		this_month: {
			...
			billable_seconds: sumDurations(
				thisMonthEntries.filter((e) => e.is_billable),
			),
...
export function generateProductivityReport(
	...
	const billableSeconds = sumDurations(
		filteredEntries.filter((e) => e.is_billable),
	);
	...
	return {
		...
		billable_seconds: billableSeconds,
		utilization_rate:
			totalSeconds > 0 ? (billableSeconds / totalSeconds) * 100 : 0,
