...
function normalizeGoalStatus(value?: string | null): PatientGoal["status"] {
  const normalized = String(value ?? "")
    .trim()
    .toLowerCase();
  if (["concluido", "concluído", "completed", "done"].includes(normalized)) return "concluido";
...
export function normalizeGoalRow(row: GoalApiRow): PatientGoal {
  ...
  const goalTitle = metadataString(metadata.goal_title) ?? row.description ?? "Meta";
...
export function normalizeGoalRows(rows: GoalApiRow[] | null | undefined): PatientGoal[] {
