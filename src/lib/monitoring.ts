...
export function initGoogleAnalytics(measurementId: string): void {
	...
	if (!measurementId || typeof measurementId !== "string") {
		if (import.meta.env.DEV) {
			logger.debug(
				"GA4 skipped: VITE_GA_MEASUREMENT_ID not configured",
				{},
				"Monitoring",
			);
		}
	...
	try {
		...
		script.onerror = () => {
			logger.warn(
				"GA4 script failed to load (possible CSP block)",
				{ measurementId },
				"Monitoring",
			);
		};
...
export const trackMetric = (metric: string, data: number | MetricData) => {
	const value =
		typeof data === "number" ? Math.round(data) : Math.round(data.value);
	...
	if (import.meta.env.VITE_ANALYTICS_ENDPOINT) {
		...
		if (isProduction && isLocalhost) {
			logger.warn(
				"Endpoint de analytics ignorado em produção por ser localhost",
				{ endpoint },
				"Monitoring",
			);
			return;
