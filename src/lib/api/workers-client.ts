...
export const crmApi = {
  leads: { list: (p?:{estagio?:string})=>crm(`/leads?${new URLSearchParams(Object.fromEntries(Object.entries(p??{}).filter(([,v])=>v!=null).map(([k,v])=>[k,String(v)])))}`), get:(id:string)=>crm(`/leads/${id}`), create:(d:Partial<Lead>)=>crm('/leads',{method:'POST',body:JSON.stringify(d)}), update:(id:string,d:Partial<Lead>)=>crm(`/leads/${id}`,{method:'PUT',body:JSON.stringify(d)}), delete:(id:string)=>crm(`/leads/${id}`), historico:(id:string)=>crm(`/leads/${id}/historico`), addHistorico:(id:string,d:Partial<LeadHistorico>)=>crm(`/leads/${id}/historico`,{method:'POST',body:JSON.stringify(d)}) },
...
export const dicomApi = {
  studies: (params?: Record<string, string | number | boolean | undefined>) =>
    request<{ data: Record<string, unknown>[] }>(withQuery('/api/dicom/studies', params)),
...
export const documentSignaturesApi = {
  ...
  verify: (documentId: string, hash: string) =>
    request<{ data: { valid: boolean; signature: Record<string, unknown> | null } }>(
...
export const exercisePlansApi = {
  ...
  update: (id: string, data: Record<string, unknown>) =>
    request<{ data: Record<string, unknown> }>(`/api/exercise-plans/${encodeURIComponent(id)}`, {
      method: 'PATCH',
      body: JSON.stringify(data),
    }),
  delete: (id: string) =>
...
export const timeEntriesApi = {
  list: (params?: {
    ...
    patientId?: string;
    limit?: number;
  }) => request<{ data: Array<Record<string, any>> }>(withQuery('/api/time-entries', params)),
  create: (data: Record<string, unknown>) =>
  ...
  update: (id: string, data: Record<string, unknown>) =>
    request<{ data: Record<string, any> }>(`/api/time-entries/${encodeURIComponent(id)}`, {
      method: 'PATCH',
      body: JSON.stringify(data),
    }),
  delete: (id: string) =>
  ...
  stats: (params?: { userId?: string; startDate?: string; endDate?: string }) =>
    request<{ data: Record<string, any> }>(withQuery('/api/time-entries/stats', params)),
...
export const treatmentCyclesApi = {
  ...
  delete: (id: string) =>
    request<{ ok: boolean }>(`/api/treatment-cycles/${encodeURIComponent(id)}`, {
      method: 'DELETE',
    }),
};
...
export const tarefasApi = {
  list: (params?: { projectId?: string }) => {
    ...
    return request<{ data: Record<string, unknown>[] }>(`/api/tarefas${qs ? `?${qs}` : ''}`);
  },
  create: (data: Record<string, unknown>) =>
  ...
  update: (id: string, data: Record<string, unknown>) =>
    request<{ data: Record<string, unknown> }>(`/api/tarefas/${encodeURIComponent(id)}`, {
      method: 'PATCH',
      body: JSON.stringify(data),
    }),
  delete: (id: string) =>
...
export const boardsApi = {
  list: (archived = false) =>
    request<{ data: Record<string, unknown>[] }>(`/api/boards?archived=${archived}`),

  get: (id: string) =>
    request<{ data: Record<string, unknown> }>(`/api/boards/${id}`),

  create: (data: { name: string; description?: string; background_color?: string; background_image?: string; icon?: string }) =>
  ...
  listTarefas: (boardId: string) =>
    request<{ data: Record<string, unknown>[] }>(`/api/boards/${boardId}/tarefas`),

  listColumns: (boardId: string) =>
    request<{ data: Record<string, unknown>[] }>(`/api/boards/${boardId}/columns`),

  createColumn: (boardId: string, data: { name: string; color?: string; wip_limit?: number }) =>
    request<{ data: Record<string, unknown> }>(`/api/boards/${boardId}/columns`, {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  ...
  deleteColumn: (colId: string) =>
    request<{ ok: boolean }>(`/api/boards/columns/${colId}`, { method: 'DELETE' }),
...
export const commissionsApi = {
  summary: (month: string) =>
    request<{ data: unknown[]; period: { start: string; end: string } }>(`/api/commissions/summary?month=${month}`),

  config: (therapistId?: string) =>
    request<{ data: unknown[] }>(`/api/commissions/config${therapistId ? `?therapistId=${therapistId}` : ''}`),

  therapistHistory: (therapistId: string) =>
    request<{ data: unknown[] }>(`/api/commissions/therapist/${therapistId}`),

  setRate: (data: { therapist_id: string; commission_rate: number; notes?: string }) =>
    request<{ data: unknown }>('/api/commissions/config', { method: 'POST', body: JSON.stringify(data) }),

  payout: (data: Record<string, unknown>) =>
    request<{ data: unknown }>('/api/commissions/payout', { method: 'POST', body: JSON.stringify(data) }),
};
...
export const nfseApi = {
  ...
  saveConfig: (data: Record<string, unknown>) =>
    request<{ data: unknown }>('/api/nfse/config', { method: 'PUT', body: JSON.stringify(data) }),

  generate: (data: Record<string, unknown>) =>
    request<{ data: unknown }>('/api/nfse/generate', { method: 'POST', body: JSON.stringify(data) }),

  send: (id: string) =>
    request<{ data: unknown }>(`/api/nfse/send/${id}`, { method: 'POST', body: JSON.stringify({}) }),
