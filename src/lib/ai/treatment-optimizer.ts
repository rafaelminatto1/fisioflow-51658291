...
export async function optimizeTreatmentPlan(
	...
	try {
		...
		if (includeGrounding) {
			researchSummary = await performGroundedResearch(
				currentCondition,
				currentTreatmentPlan,
			);
		}
		...
		const optimization: TreatmentOptimization = {
			...
			currentPlanSummary: {
				...
				intensity: currentTreatmentPlan.intensity as
					| "low"
					| "moderate"
					| "high",
			},
			recommendations: object.recommendations.map((rec) => ({
				...
				implementation: {
					...rec.implementation,
					timeline:
						rec.implementation.timeline || "Implementar na próxima sessão",
				},
			})),
		...
		logger.error(
			"[Treatment Optimizer] AI optimization error",
			error,
			"treatment-optimizer",
		);
		throw error;
...
async function performGroundedResearch(
	...
	const topicsResearched = [
		...
		...currentPlan.techniques.map(
			(t) => `${t} ${condition.primaryDiagnosis} eficácia`,
		),
	...
	const keyStudies = [
		{
			...
			finding:
				"Exercise therapy and manual therapy show significant improvements in pain and function",
			relevance: "high",
		},
		{
			...
			finding:
				"Combined therapy approaches show superior outcomes compared to single-modality treatments",
			relevance: "high",
		},
	];
...
function buildProgressionPlan(
	...
	const basePhases = [
		...
		},
		{
			...
			techniques: [
				...optimizedPlan.recommendedTechniques.slice(0, 3),
...
function calculateResourceImplications(
	...
	newTechniques.forEach((tech) => {
		if (tech.suitability === "highly_recommended") {
			if (
				tech.description.toLowerCase().includes("eletro") ||
				tech.description.toLowerCase().includes("tens")
			) {
...
async function storeOptimization(
	...
	try {
		...
		logger.info(
			`[Treatment Optimizer] Optimization generated for patient ${patientId} (Not stored - API pending)`,
			undefined,
			"treatment-optimizer",
		);
	} catch (error) {
		logger.error(
			"[Treatment Optimizer] Error storing optimization",
			error,
			"treatment-optimizer",
		);
	}
