...
export class ExerciseCoach {
	 ...
	 */
	async startSession(coachConfig: ExerciseCoachConfig): Promise<string> {
		try {
			// Generate session ID
	...
	async stopSession(): Promise<void> {
		...
		try {
			...
			this.session.state.duration =
				Date.now() - (this.session.state.startTime || 0);
		...
		try {
			...
			const frame: VideoFrame = {
				...
				frameNumber: Math.floor(
					Date.now() / (1000 / this.session.config.video!.frameRate),
				),
			};
	...
	async startRecording(): Promise<void> {
		...
		try {
			const source = this.session.audioContext.createMediaStreamSource(
				this.session.mediaStream,
			);
			const bufferSize = 4096;
			const processor = this.session.audioContext.createScriptProcessor(
				bufferSize,
				1,
				1,
			);
	...
	private async playAudio(audioData: ArrayBuffer): Promise<void> {
		...
		try {
			...
			const audioBuffer =
				await this.session.audioContext.decodeAudioData(audioData);
	...
	private async announceExerciseStart(exercise: Exercise): Promise<void> {
		const message =
			`Vamos começar o exercício: ${exercise.name}. ` +
	...
	private async announceSetComplete(set: ExerciseSet): Promise<void> {
		const avgScore =
			set.reps.length > 0
				? set.reps.reduce((sum, rep) => sum + (rep.formScore || 0), 0) /
					set.reps.length
				: 0;

		const message =
			`Série ${set.number} concluída! ` +
			(avgScore > 80
				? "Ótima forma!"
				: avgScore > 60
					? "Bom trabalho!"
					: "Vamos melhorar a forma na próxima.");
	...
	private async announceWorkoutComplete(): Promise<void> {
		const totalReps =
			this.session?.completedSets.reduce(
		...
		const message =
			...
			(avgScore > 80
				? "Excelente forma durante todo o exercício!"
				: avgScore > 60
					? "Bom trabalho! Continue praticando para melhorar a forma."
	 ...
	 */
	private async speakModification(
		reason: string,
		modification: string,
	): Promise<void> {
	...
	private async connectToLiveAPI(): Promise<void> {
		if (!this.session) throw new Error("No active session");
		throw new Error(
			"Live AI is not available on the Neon/Cloudflare stack yet.",
		);
	}
	...
	private updateFormMetrics(_formScore: number): void {
		...
		const totalScore = allReps.reduce(
		...
		this.session.metrics.correctReps = allReps.filter(
			(rep) => (rep.formScore || 0) >= 80,
		).length;
	}
	...
	private buildSystemInstruction(coachConfig: ExerciseCoachConfig): string {
		...
		const skillTips = {
			beginner:
				"This patient is a BEGINNER. Provide very detailed instructions, be patient, and praise small improvements.",
			intermediate:
				"This patient is at INTERMEDIATE level. Focus on refining form and progressing difficulty.",
			advanced:
				"This patient is ADVANCED. Focus on perfect form, subtle corrections, and performance optimization.",
		};
	...
	generateSessionSummary(): {
		...
		const totalReps = this.session.completedSets.reduce(
			(sum, set) => sum + set.reps.length,
			0,
		);
...
export const COMMON_EXERCISES: Record<string, Exercise> = {
	squat: {
		...
		name: "Agachamento",
		description:
			"Agache mantendo as costas retas e joelhos alinhados com os pés.",
		targetReps: 10,
		targetSets: 3,
	...
	plank: {
		...
		name: "Prancha",
		description:
			"Apoie antebraços e ponta dos pés, mantendo corpo reto como uma tábua.",
		targetReps: 1,
		targetSets: 3,
		enableVideoAnalysis: true,
		formCheckpoints: [
	...
	wallPushup: {
		...
		description:
			"Em pé frente à parede, flexione os braços empurrando o corpo.",
		targetReps: 10,
		targetSets: 3,
		enableVideoAnalysis: true,
		formCheckpoints: [
