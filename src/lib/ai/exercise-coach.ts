...
export class ExerciseCoach {
  ...
  private async announceWorkoutComplete(): Promise<void> {
    const totalReps =
      this.session?.completedSets.reduce((sum, set) => sum + set.reps.length, 0) || 0;
...
export const COMMON_EXERCISES: Record<string, Exercise> = {
  ...
  plank: {
    ...
    commonMistakes: ["Quadril muito alto ou muito baixo", "Costas curvadas", "Relaxar abdômen"],
  ...
  bridge: {
    ...
    commonMistakes: ["Não elevar o suficiente", "Arquear as costas", "Não contrair glúteos"],
  ...
  wallPushup: {
    ...
    commonMistakes: ["Cotovelos muito abertos", "Arquear as costas", "Movimento muito curto"],
...
export type { Exercise, Repetition, ExerciseSet, ExerciseCoachSession, ExerciseCoachConfig };
