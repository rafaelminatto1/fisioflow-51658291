 ...
 */
export function generatePatientRecommendations(patient: PatientData): PatientRecommendation[] {
...
export function findOptimalSlots(
  ...
  const topSlots = scoredSlots.sort((a, b) => b.score - a.score).slice(0, 3);
 ...
 */
export function generateTreatmentInsights(data: EvolutionData): TreatmentInsight[] {
 ...
 */
export function suggestNextAppointment(history: AppointmentHistory[]): {
  suggestedDate: Date;
  suggestedTime: string;
  confidence: number;
  reason: string;
  ...
  const mostCommonDay = Array.from(dayCounts.entries()).sort((a, b) => b[1] - a[1])[0]?.[0];
