...
export function findOptimalSlots(
	...
	const scoredSlots = available.map((slot) => {
		...
		if (
			request.lastTherapistId &&
			slot.therapistId === request.lastTherapistId
		) {
			score += 40;
		...
		return [
			{
				...
				description:
					"Mais dados de evolução são necessários para gerar insights",
				value: 0,
				trend: "stable",
			},
		];
...
export function suggestNextAppointment(history: AppointmentHistory[]): {
	...
	const mostCommonTime = Array.from(timeCounts.entries()).sort(
		(a, b) => b[1] - a[1],
	)[0]?.[0];
	...
	const mostCommonDay = Array.from(dayCounts.entries()).sort(
		(a, b) => b[1] - a[1],
	)[0]?.[0];
