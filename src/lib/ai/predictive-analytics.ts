...
const recoveryPredictionSchema = z.object({
	predictedRecoveryDate: z
		.string()
		.describe("ISO date string for expected recovery"),
...
async function fetchSimilarCases(
	...
	try {
		...
		logger.error(
			"[Predictive Analytics] Error fetching similar cases",
			error,
...
function calculateSimilarCaseStats(cases: SimilarCase[]) {
	...
	if (averageRecoveryTime > 0) {
		keyInsights.push(
			`Tempo médio de recuperação: ${Math.round(averageRecoveryTime)} sessões`,
		);
	}
...
export async function predictRecoveryTimeline(
	...
	try {
		...
		expectedDate.setDate(
			expectedDate.getDate() + object.confidenceInterval.expectedDays,
		);

		const lowerDate = new Date(now);
		lowerDate.setDate(
			lowerDate.getDate() + object.confidenceInterval.lowerDays,
		);

		const upperDate = new Date(now);
		upperDate.setDate(
			upperDate.getDate() + object.confidenceInterval.upperDays,
		);
			...
			similarCases: {
				...
				matchingCriteria: [
					...
					similarCases.length > 0
						? `Dados de ${similarCases.length} casos similares`
						: "Dados limitados",
				],
			},
		...
		logger.error(
			"[Predictive Analytics] AI prediction error",
			error,
			"predictive-analytics",
		);
...
interface PredictionContext {
	...
	progress: PredictionInput["progressData"] & {
		painLevelHistory: Array<{ date: string; level: number }>;
		functionalScores: Array<{ date: string; score: number }>;
		attendanceRate: number;
	};
...
function generateFallbackPrediction(
	...
	if (conditionLower.includes("aguda") || conditionLower.includes("agudo")) {
		...
		conditionLower.includes("crônica") ||
		conditionLower.includes("crônico")
	) {
		baseDays = 60;
	} else if (
		conditionLower.includes("lombalgia") ||
		conditionLower.includes("dor lombar")
	) {
		baseDays = 21;
	} else if (
		conditionLower.includes("lesão") ||
		conditionLower.includes("entorse")
	) {
		baseDays = 28;
	} else if (
		conditionLower.includes("pós-operatório") ||
		conditionLower.includes("pos-operatorio")
	) {
		baseDays = 45;
	}

	const expectedDays =
		similarStats.averageRecoveryTime > 0
...
async function storePrediction(
	...
	try {
		...
		logger.error(
			"[Predictive Analytics] Error storing prediction",
			error,
			"predictive-analytics",
		);
	}
