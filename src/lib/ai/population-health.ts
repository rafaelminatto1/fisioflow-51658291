...
function calculatePopulationStats(data: PopulationStatsInput): PopulationStatsResult {
  ...
  const avgSessions =
    mlData.length > 0
      ? mlData.reduce((sum: number, r: MLDataRecord) => sum + (r.total_sessions || 0), 0) /
        mlData.length
      : 0;
...
function generateChartData(
  ...
  const retentionFunnel = [
    ...
    },
    {
      stage: "Alta Clínica",
      count: Math.round(
        stats.overview.totalPatients * (stats.retention.overallRetentionRate / 100),
      ),
...
export async function analyzeClinicPopulation(
  ...
  const context = {
    ...
    treatmentsData: Array.from(stats.treatments.entries()).map(([treatment, cases]) => {
      ...
      const avgOutcome =
        cases.reduce(
          (sum: number, c: MLDataRecord) => sum + (c.functional_improvement_percentage || 0),
          0,
        ) / cases.length;
      ...
      return {
        ...
        patientSatisfaction:
          cases.reduce(
            (sum: number, c: MLDataRecord) => sum + (c.patient_satisfaction_score || 70),
            0,
          ) / cases.length,
      };
    }),
...
function calculateRecoveryMetrics(
  ...
  return Array.from(conditions.entries())
    .map(([condition, cases]) => {
      ...
      const variance =
        recoveryTimes.reduce((acc, val) => acc + Math.pow(val - mean, 2), 0) / recoveryTimes.length;
...
function calculateRetentionByCondition(conditions: Map<string, MLDataRecord[]>): Array<{
  condition: string;
  retentionRate: number;
  averageSessions: number;
}> {
  return Array.from(conditions.entries())
    .map(([condition, cases]) => {
      ...
      const sessions = cases
        ...
        .filter((t): t is number => typeof t === "number");
      const avgSessions =
        sessions.length > 0 ? sessions.reduce((sum, t) => sum + t, 0) / sessions.length : 0;
...
function generateBenchmarks(clinicSuccessRate: number): PopulationHealthAnalysis["benchmarks"] {
