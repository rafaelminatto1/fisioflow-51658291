...
async function aggregatePopulationData(
	...
	try {
		...
		return {
			...
			mlData: payload.mlData.map((record) => ({
				...
				functional_improvement_percentage:
					record.functional_improvement_percentage,
		...
		logger.error(
			"[Population Health] Error aggregating data",
			error,
			"population-health",
		);
		return {
...
function calculatePopulationStats(
	...
	const activePatients = patients.filter(
		(p: PatientRecord) => p.status === "active",
	).length;
	...
	const genderDistribution = patients.reduce(
		(acc: GenderDistribution, p: PatientRecord) => {
	...
	const completedCases = mlData.filter(
		(r: MLDataRecord) => r.outcome_category === "success",
	);
	const avgSessions =
		mlData.length > 0
			? mlData.reduce(
					(sum: number, r: MLDataRecord) => sum + (r.total_sessions || 0),
					0,
				) / mlData.length
			: 0;
	...
	const retention = {
		overallRetentionRate:
			mlData.length > 0 ? (completedCases.length / mlData.length) * 100 : 0,
		dropoutRate:
			100 -
			(mlData.length > 0 ? (completedCases.length / mlData.length) * 100 : 0),
...
function generateChartData(
	...
	const totalCases = Array.from(conditions.values()).reduce(
		...
		.map(([condition, cases]) => {
			...
			const avgDays =
				recoveryTimes.length > 0
					? (recoveryTimes.reduce((sum, t) => sum + t, 0) /
	...
	const retentionFunnel = [
		{
			stage: "Novos Pacientes",
			count: stats.overview.totalPatients,
			percentage: 100,
		},
		{
			stage: "Iniciaram Tratamento",
			count: Math.round(stats.overview.totalPatients * 0.85),
			percentage: 85,
		},
		{
			stage: "Completaram 5+ Sessões",
			count: Math.round(stats.overview.totalPatients * 0.65),
			percentage: 65,
		},
		{
			stage: "Alta Clínica",
			count: Math.round(
				stats.overview.totalPatients *
					(stats.retention.overallRetentionRate / 100),
			),
			percentage: Math.round(stats.retention.overallRetentionRate),
		},
	];
...
export async function analyzeClinicPopulation(
	...
	const context = {
		...
		conditionsData: Array.from(stats.conditions.entries()).map(
			([condition, cases]) => {
				const recoveryTimes = cases
					.map((c) => c.sessions_to_discharge)
				...
				return {
					...
					averageSessions:
						recoveryTimes.length > 0
							...
							: 0,
					successRate:
						cases.length > 0 ? (successfulCases / cases.length) * 100 : 0,
				};
			},
		),
		treatmentsData: Array.from(stats.treatments.entries()).map(
			([treatment, cases]) => {
				const successfulCases = cases.filter(
					(c) => c.outcome_category === "success",
				).length;
				const avgOutcome =
					cases.reduce(
						(sum: number, c: MLDataRecord) =>
							sum + (c.functional_improvement_percentage || 0),
						0,
					) / cases.length;
				...
				return {
					...
					patientSatisfaction:
						cases.reduce(
							(sum: number, c: MLDataRecord) =>
								sum + (c.patient_satisfaction_score || 70),
							0,
						) / cases.length,
				};
			},
	...
	try {
		...
		const analysis: PopulationHealthAnalysis = {
			...
			dataQuality: {
				...
				completeRecords: rawData.mlData.filter(
					(d: MLDataRecord) => d.outcome_category,
				).length,
				hasFollowUpData: rawData.mlData.some(
					(d: MLDataRecord) => d.outcome_category,
				),
			},
		};
		...
		logger.error(
			"[Population Health] AI analysis error",
			error,
			"population-health",
		);
		throw error;
...
function calculateRecoveryMetrics(
	...
	return Array.from(conditions.entries())
		.map(([condition, cases]) => {
			...
			const percentile25 =
				recoveryTimes[Math.floor(recoveryTimes.length * 0.25)];
			const percentile75 =
				recoveryTimes[Math.floor(recoveryTimes.length * 0.75)];

			const variance =
				recoveryTimes.reduce((acc, val) => acc + Math.pow(val - mean, 2), 0) /
				recoveryTimes.length;
