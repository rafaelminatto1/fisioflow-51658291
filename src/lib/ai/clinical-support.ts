...
const RedFlagSchema = z.object({
	...
	urgency: z
		.enum(["immediate", "urgent", "monitor", "informational"])
...
const ClinicalAnalysisSchema = z.object({
	...
	recommendedAssessments: z
		.array(RecommendedAssessmentSchema)
		.describe("Recommended assessments"),
	caseSummary: z.string().describe("Overall case summary"),
	keyConsiderations: z
		.array(z.string())
		.describe("Key clinical considerations"),
	...
	searchQueries: z
		.array(z.string())
		.optional()
		.describe("Search queries used for evidence"),
});
...
export class ClinicalDecisionSupport {
	...
	private groundingModel: ReturnType<
	...
	constructor(apiKey?: string, groundingOptions?: Partial<GroundingOptions>) {
		...
		this.groundingEnabled =
			groundingOptions?.enabled ?? CLINICAL_AI_CONFIG.grounding.enabled;
	...
	async analyzeCase(
		...
		try {
			...
			const selectedModel =
				enableGrounding && this.groundingModel
					? this.groundingModel
					: this.model;
			...
			try {
				...
				logger.error(
					"[ClinicalSupport] JSON parse error",
					parseError,
					"clinical-support",
				);
				return {
			...
			logger.error(
				"[ClinicalSupport] Analysis error",
				error,
				"clinical-support",
			);
			return {
	...
	async searchEvidence(query: string): Promise<{
		...
		if (!this.groundingModel) {
			return {
				success: false,
				error:
					"Grounding not enabled. Enable grounding to use evidence search.",
			};
		}
 ...
 */
export function requiresImmediateReferral(
	redFlags: ClinicalRedFlag[],
): boolean {
	return redFlags.some(
		(flag) => flag.urgency === "immediate" || flag.urgency === "urgent",
	);
}
 ...
 */
export function getCriticalRedFlags(
	redFlags: ClinicalRedFlag[],
): ClinicalRedFlag[] {
