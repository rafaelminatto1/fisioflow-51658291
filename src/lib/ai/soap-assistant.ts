...
export interface PatientSOAPContext {
	...
	previousSOAP?: Array<
		Pick<
			SOAPRecord,
			"sessionNumber" | "subjective" | "objective" | "assessment" | "plan"
		>
	>;
...
const SOAPSectionSchema = z.object({
	subjective: z
		.string()
		.describe("Patient reported symptoms and complaints in Portuguese"),
	objective: ObjectiveSchema.optional().describe(
		"Physical examination findings",
	),
	assessment: z
		.string()
		.describe("Clinical assessment and diagnosis in Portuguese"),
...
const SOAPGenerationSchema = z.object({
	...
	keyFindings: z
		.array(z.string())
		.describe("Key clinical findings from this session"),
	recommendations: z
		.array(z.string())
		.describe("Recommended actions for next session"),
	redFlags: z
		.array(z.string())
		.optional()
		.describe("Any red flags requiring immediate attention"),
	suggestedCodes: z
		.array(z.string())
		.optional()
		.describe("Suggested ICD-10 codes"),
});
...
export class SOAPAssistant {
	...
	private transcriptionModel: ReturnType<
...
Retorne APENAS JSON válido.`;
	...
	private buildSOAPPrompt(
		...
		const { patient, previousSOAP, sessionNumber, sessionType, _language } =
			context;
		...
		const historyText =
			previousSOAP && previousSOAP.length > 0
				? `
