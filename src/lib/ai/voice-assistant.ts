...
export class VoiceAssistant {
	...
	async startRecording(): Promise<void> {
		...
		try {
			// Create source node
			this.session.sourceNode =
				this.session.audioContext.createMediaStreamSource(
					this.session.mediaStream,
				);

			// Create processor for audio data
			const bufferSize = 4096;
			this.session.processorNode =
				this.session.audioContext.createScriptProcessor(bufferSize, 1, 1);
	...
	private calculateImportance(
		...
		if (
			text.toLowerCase().includes("muito") ||
			text.toLowerCase().includes("severo")
		) {
	...
	generateSessionSummary(): {
		...
		// Build summary
		const keyPointsByCategory = this.session.keyPoints.reduce(
			(acc, kp) => {
		...
		if (keyPointsByCategory.pain?.length) {
			summaryParts.push(
				`Pain mentioned ${keyPointsByCategory.pain.length} times`,
			);
		}

		if (keyPointsByCategory.symptom?.length) {
			summaryParts.push(
				`Symptoms reported: ${keyPointsByCategory.symptom.length}`,
			);
		}

		if (keyPointsByCategory.progress?.length) {
			summaryParts.push(
				`Progress indicators: ${keyPointsByCategory.progress.length}`,
			);
		}
...
export type { VoiceAssistantConfig, KeyPoint, VoiceAssistantSession };
