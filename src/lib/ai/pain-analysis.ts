...
export async function analyzePainEvolution(
	...
	const migrationPatterns = analyzeMigrationPatterns(
		initialAssessment,
		currentAssessment,
	);
...
function evaluateTreatmentResponse(
	...
	const allRegions = new Set(
		[...initial.painPoints, ...current.painPoints].map((p) => p.region),
	);
...
function generateVisualAnnotations(
	...
	current.painPoints.forEach((point) => {
		...
		const change = initialPoint
			? point.intensity - initialPoint.intensity
			: point.intensity;
...
async function getPainAssessments(
	...
	const records: PainMapRecord[] =
		await PainMapService.getPainMapsByPatientId(patientId);
