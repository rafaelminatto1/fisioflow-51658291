...
export async function analyzePainEvolution(
  ...
  const visualAnnotations = generateVisualAnnotations(initialAssessment, currentAssessment);
...
function analyzeTrend(assessments: PainEvolutionData[]): PainTrendAnalysis {
  ...
  const percentageChange =
    first.globalPainLevel > 0 ? (absoluteChange / first.globalPainLevel) * 100 : 0;
...
function evaluateTreatmentResponse(
  ...
  const percentageReduction =
    initial.globalPainLevel > 0 ? (painReduction / initial.globalPainLevel) * 100 : 0;
...
function getMostIntenseRegion(points: PainMapPoint[]): string {
  ...
  return points.reduce((max, point) => (point.intensity > max.intensity ? point : max)).region;
}
