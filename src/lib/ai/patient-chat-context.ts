...
function buildConditionSummary(
	...
	const progress = formatMetric(
		analyticsData?.progress_summary.overall_progress_percentage,
		"%",
	);
	...
	const functionScore = formatMetric(
		analyticsData?.function_trend?.current_score,
		"/100",
	);
	if (functionScore) summaryParts.push(`Funcao atual: ${functionScore}`);

	const dropoutRisk = formatMetric(
		analyticsData?.risk_score?.dropout_risk_score ??
	...
	const activeGoals =
		analyticsData?.goals.filter((goal) => goal.status === "in_progress")
			.length ?? 0;
...
function buildRecentEvolutions(
	...
	const insightEvents = analyticsData.recent_insights.map((insight) => ({
		...
		notes: truncateText(
			`Insight (${insight.insight_type}): ${insight.insight_text}`,
		),
...
export function buildPatientChatContext(
	...
	const {
		...
		patientName,
		analyticsData,
		maxRecentEvolutions = DEFAULT_MAX_RECENT_EVOLUTIONS,
	} = options;
	...
	const recentEvolutions = buildRecentEvolutions(
		analyticsData,
		maxRecentEvolutions,
	);
