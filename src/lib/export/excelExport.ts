...
export const exportToExcel = async (
	...
	sheets.forEach((sheet) => {
		...
		const dataAoa = dataToAoa(
			sheet.data,
			sheet.columns,
			sheet.includeHeader !== false,
		);
...
export const exportOccupancyReport = async (
	data: TherapistOccupancyExportData,
) => {
	const summaryData = [
		{
	...
	const detailData = data.therapists.map((t) => ({
		...
		status:
			t.status === "otimo" ? "Ótimo" : t.status === "bom" ? "Bom" : "Baixo",
	}));
...
export const exportTeamPerformance = async (
	data: TeamPerformanceExportData,
) => {
	const summaryData = [
		...
		sheets: [
			...
			},
			{
				...
				columns: [
					{ header: "Fisioterapeuta", key: "fisioterapeuta", width: 30 },
					{
						header: "Sessões Concluídas",
						key: "sessoesConcluidas",
						type: "number",
						width: 20,
					},
					{
