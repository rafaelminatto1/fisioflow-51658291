...
export const generateEvolutionPDF = (
	...
	const age = patient.birthDate
		...
		: "N/A";
	doc.text(
		`Idade: ${age} anos | Tel: ${patient.phone || "N/A"} | Email: ${patient.email || "N/A"}`,
		20,
		yPos + 6,
	);
	...
	const sessionValue =
		metrics.prescribedSessions && metrics.prescribedSessions > 0
	...
	drawCard(
		...
		28,
		"Nível de Dor",
		`${metrics.initialPainLevel} → ${metrics.currentPainLevel}`,
		"",
	);
	drawCard(
		140,
	...
	if (metrics.measurementEvolution && metrics.measurementEvolution.length > 0) {
		...
		const measTableData = metrics.measurementEvolution.map(
			(m: {
				name: string;
				initial: { value: number | string; unit: string };
				current: { value: number | string; unit: string };
				improvement: number | string;
			}) => {
	...
	yPos =
		(doc as { lastAutoTable: { finalY: number } }).lastAutoTable.finalY + 20;
	...
	for (let i = 1; i <= pageCount; i++) {
		...
		doc.text(
			`FisioFlow - Gerenciamento de Fisioterapia de Alta Performance | Página ${i} de ${pageCount}`,
			105,
			285,
			{ align: "center" },
		);
