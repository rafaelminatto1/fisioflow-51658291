...
class ExcelJSUtils implements Utils {
	...
	private encodeRange(range: {
		s: { c: number; r: number };
		e: { c: number; r: number };
	}): string {
		return (
			this.encodeCell(range.s.c, range.s.r) +
			":" +
			this.encodeCell(range.e.c, range.e.r)
		);
	}

	private decodeRange(ref: string): {
		s: { c: number; r: number };
		e: { c: number; r: number };
	} {
...
export async function read(
	...
	options?: { type?: "buffer" | "binary" },
): Promise<WorkBook> {
	const ExcelJS = (await import("exceljs")).default;
	const workbook = new ExcelJS.Workbook();
	const buffer =
		options?.type === "binary" && typeof data === "string"
				...
				for (let c = range.s.c; c <= range.e.c; c++) {
					...
					if (cell) {
						...
						if (cell.t === "n") {
							...
							worksheet.getCell(`${excelCol}${excelRow}`).value = String(
								cell.v || "",
							);
						}
					}
...
function encodeRangeValue(range: {
	s: { c: number; r: number };
	e: { c: number; r: number };
}): string {
	return (
		encodeCell(range.s.c, range.s.r) + ":" + encodeCell(range.e.c, range.e.r)
	);
}

function decodeRangeValue(ref: string): {
	s: { c: number; r: number };
	e: { c: number; r: number };
} {
	const parts = ref.split(":");
