...
export function exportPrestadoresPDF(
	...
	const finalY =
		(doc as { lastAutoTable?: { finalY: number } }).lastAutoTable?.finalY || 45;
...
export function exportParticipantesPDF(
	...
	const percentual =
		participantes.length > 0 ? (seguidores / participantes.length) * 100 : 0;
	...
	doc.text(
		`Seguem o Perfil: ${seguidores} (${percentual.toFixed(1)}%)`,
		14,
		finalY + 16,
	);

	doc.save(
		`participantes-${eventoNome.replace(/\s+/g, "-").toLowerCase()}.pdf`,
	);
}
