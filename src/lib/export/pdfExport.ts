...
export function exportPrestadoresPDF(prestadores: PrestadorExport[], eventoNome: string) {
  const doc = new jsPDF();
...
export function exportParticipantesPDF(participantes: ParticipanteExport[], eventoNome: string) {
  const doc = new jsPDF();
  ...
  const percentual = participantes.length > 0 ? (seguidores / participantes.length) * 100 : 0;
