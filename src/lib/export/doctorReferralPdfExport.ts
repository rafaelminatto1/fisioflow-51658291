...
export async function exportDoctorReferralPdf(fileName: string, data: DoctorReferralPdfData) {
  ...
  line(`Adesao: ${data.analysis.adherence}% dos exercicios prescritos concluidos`);
  line(
  ...
  const summaryLines = doc.splitTextToSize(data.analysis.summary, pageWidth - margin * 2);
  ...
  doc.text("Documento gerado eletronicamente por FisioFlow AI Platform", pageWidth / 2, y + 4, {
    align: "center",
  });
