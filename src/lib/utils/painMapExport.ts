...
export function generateTextReport(data: PainMapExportData): string {
	...
	lines.push(
		`Intensidade média: ${data.statistics.averageIntensity.toFixed(1)}/10`,
	);
	lines.push(
		`Pontos de alta intensidade: ${data.statistics.highIntensityCount}`,
	);
	...
	if (data.frontPoints.length > 0) {
		...
		data.frontPoints.forEach((point, index) => {
			lines.push(
				`${index + 1}. ${point.region}${point.muscleName ? ` (${point.muscleName})` : ""}`,
			);
...
export async function generatePDFReport(
	...
	y += 6;
	doc.text(
		`• Músculos específicos: ${data.statistics.muscleSpecificCount}`,
		margin + 5,
		y,
	);
	y += 12;
	...
	if (allPoints.length > 0) {
		...
		allPoints.forEach((point, index) => {
			...
			doc.setFont("helvetica", "normal");
			doc.text(
				`   Intensidade: ${point.intensity}/10 | Tipo: ${point.painType}`,
				margin + 5,
				y,
			);
			y += 5;
