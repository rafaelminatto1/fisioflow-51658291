...
export async function measureAsync<T>(
  ...
  try {
    ...
    logger.info(`Async operation completed: ${metricName}`, { duration, context }, "Performance");
 ...
 */
export function measureSync<T>(fn: () => T, metricName: string, context?: string): T {
  const _startTime = performance.now();
  const startMark = `${metricName}-start`;
  const endMark = `${metricName}-end`;
  ...
  try {
    ...
    logger.info(`Sync operation completed: ${metricName}`, { duration, context }, "Performance");
...
export function getResourceTimingStats(): {
  ...
  resources.forEach((resource: PerformanceResourceTiming) => {
    ...
    if (duration > 1000) {
      // Recursos que levam mais de 1 segundo
      slowResources.push({
        name: resource.name,
        duration,
        size,
