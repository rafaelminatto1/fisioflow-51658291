...
const appendQueryConstraints = <T>(
	query: Query<T>,
	...constraints: unknown[]
): Query<T> => ({
	...query,
...
export function applyFilters<T>(
	...
	if (filters.status && typeof filters.status === "string") {
		...
		if (filters.inStatus.length === 1) {
			result = appendQueryConstraints(
				result,
				where("status", "==", filters.inStatus[0]),
			);
		}
	...
	if (filters.orderBy) {
		result = appendQueryConstraints(
			result,
			orderBy(filters.orderBy, filters.ascending ? "asc" : "desc"),
		);
	}
