 ...
 * ```
 */
export function withTimeout<T>(promise: PromiseLike<T> | T, timeoutMs: number = 8000): Promise<T> {
  return Promise.race([
...
export async function retryWithBackoff<T>(
  ...
  const { maxRetries = 3, initialDelayMs = 1000, shouldRetry = () => true, onRetry } = options;
