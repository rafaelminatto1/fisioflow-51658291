  ...
  return (async (...args: any[]) => {
    try {
      ...
      if (error instanceof Error) {
        ...
        captureMessage(`Erro em ${functionName}: ${String(error)}`, "error", {
          args: JSON.stringify(args),
        });
      }
...
export const fisioflowSentry = {
   ...
   */
  trackValidationError: (field: string, value: any, message: string) => {
   ...
   * Rastreia erro de IA
   */
  trackAIError: (model: string, operation: string, error: Error | unknown) => {
    captureError(error, {
      category: "ai_error",
      model,
      operation,
   ...
   */
  trackDataLayerError: (collection: string, operation: string, error: Error | unknown) => {
    captureError(error, {
   ...
   */
  trackAuthError: (operation: string, error: Error | unknown) => {
    captureError(error, {
      category: "auth_error",
      operation,
    });
