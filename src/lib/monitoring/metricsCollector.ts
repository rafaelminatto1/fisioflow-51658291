...
class MetricsCollector {
		...
		try {
			...
			if (import.meta.env.DEV) {
				logger.debug("📊 Page Load Metrics", {
					...
					firstPaint: this.pageMetrics.firstPaint
						? `${this.pageMetrics.firstPaint.toFixed(0)}ms`
						: "N/A",
					fcp: this.pageMetrics.firstContentfulPaint
						? `${this.pageMetrics.firstContentfulPaint.toFixed(0)}ms`
						: "N/A",
				});
			}
	...
	getSummary(): {
		...
		const slowestComponent = components.reduce<ComponentMetrics | null>(
			(slowest, current) =>
				!slowest || current.lastRenderTime > slowest.lastRenderTime
					? current
					: slowest,
			null,
		);
