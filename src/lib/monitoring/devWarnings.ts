...
export function warnExcessiveRenders(componentName: string): void {
  ...
  if (
    ...
    logger.warn(`🔄 Excessive re-renders: ${componentName} rendered ${warning.renderCount} times`, {
      component: componentName,
      renderCount: warning.renderCount,
      suggestion:
        "Check if props/state are changing unnecessarily. Consider using React.memo, useMemo, or useCallback.",
    });
...
export function warnLargeState(componentName: string, stateSize: number): void {
  ...
  if (stateSize > threshold) {
    logger.warn(`📦 Large state object in ${componentName}: ${(stateSize / 1024).toFixed(2)}KB`, {
      component: componentName,
      size: `${(stateSize / 1024).toFixed(2)}KB`,
      threshold: `${(threshold / 1024).toFixed(2)}KB`,
      suggestion:
        "Consider normalizing state, using context, or moving to server state with TanStack Query",
    });
  }
}
...
export function warnUnoptimizedImage(src: string, size: number): void {
  ...
  if (size > threshold) {
    logger.warn(`🖼️ Large image detected: ${src} (${(size / 1024 / 1024).toFixed(2)}MB)`, {
      src,
...
export function warnPotentialMemoryLeak(
  ...
  logger.warn(`💧 Potential memory leak in ${componentName}: ${leakType} not cleaned up`, {
    component: componentName,
    leakType,
 ...
 */
export function warnBlockingOperation(operationName: string, duration: number): void {
  if (!IS_DEV) return;
...
export function warnLargeChunk(chunkName: string, size: number): void {
  ...
  if (size > threshold) {
    logger.warn(`📦 Large bundle chunk: ${chunkName} (${(size / 1024).toFixed(2)}KB)`, {
      chunk: chunkName,
...
export function warnInefficientQuery(
  ...
  logger.warn(`🔍 Inefficient query: ${queryKey} - ${messages[issue]}`, {
    queryKey,
    issue,
