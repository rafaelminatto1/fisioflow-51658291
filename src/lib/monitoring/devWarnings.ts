	...
	if (size > threshold) {
		logger.warn(
			`🖼️ Large image detected: ${src} (${(size / 1024 / 1024).toFixed(2)}MB)`,
			{
				...
				suggestion:
					"Consider using WebP format, lazy loading, or responsive images",
			},
		);
	}
}
...
export function warnPotentialMemoryLeak(
	...
	logger.warn(
		`💧 Potential memory leak in ${componentName}: ${leakType} not cleaned up`,
		{
			...
			leakType,
			suggestion:
				"Ensure cleanup in useEffect return function or component unmount",
		},
	);
}
	...
	if (duration > threshold) {
		logger.warn(
			`⏸️ Blocking operation: ${operationName} blocked main thread for ${duration.toFixed(2)}ms`,
			{
				...
				threshold: `${threshold}ms`,
				suggestion:
					"Consider using Web Workers, async operations, or breaking into smaller chunks",
			},
		);
	}
}
...
export function warnInefficientQuery(
	...
	const suggestions = {
		...
		"large-response":
			"Consider pagination, filtering, or selecting specific fields",
	};
