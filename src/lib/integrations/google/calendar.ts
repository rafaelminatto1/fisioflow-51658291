...
import { CalendarEvent, IntegrationConfig, SyncResult } from "@/types/integrations";
...
export async function handleCalendarWebhook(
  ...
  // Validar HMAC
  const expectedSignature = CryptoJS.HmacSHA256(JSON.stringify(body), channelToken).toString(
    CryptoJS.enc.Hex,
  );
