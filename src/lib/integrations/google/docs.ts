...
export class DocsService {
   ...
   */
  async replacePlaceholders(documentId: string, replacements: TextReplacement[]): Promise<void> {
   ...
   */
  async replacePlaceholder(documentId: string, placeholder: string, value: string): Promise<void> {
  ...
  async listTemplates(folderId?: string): Promise<DocsTemplate[]> {
    ...
    for (const file of files) {
      if (
        ...
        if (
          file.name.toLowerCase().includes("relatorio") ||
          file.name.toLowerCase().includes("clinical")
        ) {
