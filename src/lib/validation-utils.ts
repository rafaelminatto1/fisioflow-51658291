...
export function validateOrNull<T>(schema: z.ZodType<T>, data: unknown, context?: string): T | null {
  try {
    ...
    if (context) {
      logger.warn(`Validation failed in ${context}`, { errors: result.error.issues }, "Validation");
    }
    return null;
  } catch (error) {
    if (context) {
 ...
 */
export function validateOrThrow<T>(schema: z.ZodType<T>, data: unknown, errorMessage?: string): T {
  const result = schema.safeParse(data);
  if (!result.success) {
    const message =
      errorMessage || `Validation failed: ${result.error.issues.map((e) => e.message).join(", ")}`;
 ...
 */
export function validateArray<T>(schema: z.ZodType<T>, items: unknown[], context?: string): T[] {
  const validItems: T[] = [];
...
export async function validateApiResponse<T>(
  ...
  if (!result.success) {
    const message = context ? `Invalid API response for ${context}` : "Invalid API response";
    logger.error(message, { errors: result.error.issues, rawData }, "Validation");
    throw new Error(`${message}: ${result.error.issues.map((e) => e.message).join(", ")}`);
  }
 ...
 */
export function matchesSchema<T>(schema: z.ZodType<T>, data: unknown): data is T {
  const result = schema.safeParse(data);
