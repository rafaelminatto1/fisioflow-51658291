	...
	schedule: [
		{
			...
			action: () => {
				window.dispatchEvent(
					new CustomEvent("schedule:view-change", { detail: "day" }),
				);
			},
			category: "schedule",
			preventDefault: true,
		},
		{
			...
			action: () => {
				window.dispatchEvent(
					new CustomEvent("schedule:view-change", { detail: "week" }),
				);
			},
			category: "schedule",
			preventDefault: true,
		},
		{
			...
			action: () => {
				window.dispatchEvent(
					new CustomEvent("schedule:view-change", { detail: "month" }),
				);
			},
			category: "schedule",
			preventDefault: true,
		...
		},
		{
			...
			action: () => {
				window.dispatchEvent(
					new CustomEvent("schedule:navigate", { detail: "back" }),
				);
			},
			category: "schedule",
			preventDefault: true,
		},
		{
			...
			action: () => {
				window.dispatchEvent(
					new CustomEvent("schedule:navigate", { detail: "forward" }),
				);
			},
			category: "schedule",
			preventDefault: true,
	...
	private handleKeyDown(event: KeyboardEvent): void {
		...
		for (const shortcut of this.shortcuts.values()) {
			if (shortcut.enabled && this.matchesShortcut(event, shortcut)) {
				this.log(
					`Shortcut triggered: ${this.getShortcutKey(shortcut)} - ${shortcut.description}`,
				);
