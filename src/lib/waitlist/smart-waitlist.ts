	...
	if (entry.preferred_therapist_id && therapistAvailability) {
		...
		if (preferredTherapist) {
			const isTherapistAvailable = preferredTherapist.available_slots.some(
				(availableSlot) =>
					availableSlot.date === slot.dateString &&
			...
			if (isTherapistAvailable) {
				...
				matchReasons.push(
					`Terapeuta preferencial disponível: ${preferredTherapist.therapist_name}`,
				);
			}
		}
	}
...
export function generateWaitlistRecommendations(
	...
	for (const slot of availableSlots) {
		...
		if (candidates.length > 0) {
			...
			if (
				topCandidate.entry.priority === "urgent" ||
				topCandidate.waitingDays > 7
			) {
				urgencyLevel = "high";
			} else if (
				topCandidate.entry.priority === "high" ||
				topCandidate.waitingDays > 3
			) {
				urgencyLevel = "medium";
			}
 ...
 */
export function generateWaitlistAnalytics(
	waitlist: WaitlistEntry[],
): WaitlistAnalytics {
	...
	const estimatedWaitTime = {
		urgent:
			countByPriority.urgent > 0 ? Math.ceil(countByPriority.urgent / 2) : 0,
		high: countByPriority.high > 0 ? Math.ceil(countByPriority.high / 3) : 0,
		normal:
			countByPriority.normal > 0 ? Math.ceil(countByPriority.normal / 5) : 0,
	};
...
export function detectWaitlistAnomalies(waitlist: WaitlistEntry[]): string[] {
	...
	if (longWaiting.length > 0) {
		anomalies.push(
			`${longWaiting.length} pacientes aguardando há mais de 14 dias`,
		);
	}
	...
	const urgentCount = waitingEntries.filter(
		(e) => e.priority === "urgent",
	).length;
	if (urgentCount > 5) {
