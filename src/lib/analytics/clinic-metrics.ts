 ...
 */
import { startOfDay, endOfDay, differenceInDays, subDays, format, parseISO } from "date-fns";
...
export function compareAppointmentMetrics(
  ...
  const changePercent = previousPeriod.total > 0 ? (change / previousPeriod.total) * 100 : 0;
...
export function calculatePatientMetrics(
  ...
  const returningPatients = Array.from(patientAppointmentCounts.values()).filter(
    (c) => c > 1,
  ).length;
...
export function calculateTherapistMetrics(
  ...
  const occupancyRate = completed + missed > 0 ? (completed / (completed + missed)) * 100 : 0;
