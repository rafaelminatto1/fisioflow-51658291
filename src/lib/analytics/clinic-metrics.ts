...
export function compareAppointmentMetrics(
	...
	const changePercent =
		previousPeriod.total > 0 ? (change / previousPeriod.total) * 100 : 0;
...
export function calculateRevenueMetrics(
	...
	const appointmentCount = new Set(
		filtered.filter((p) => p.appointment_id).map((p) => p.appointment_id),
	).size;
	const averagePerAppointment =
		appointmentCount > 0 ? total / appointmentCount : 0;
	...
	const previousPeriodPayments = payments.filter((p) => {
		const payDate = parseISO(p.date);
		return (
			payDate >= startOfDay(previousPeriodStart) &&
			payDate <= endOfDay(previousPeriodEnd)
		);
	});
...
export function calculatePatientMetrics(
	...
	const returningPatients = Array.from(
		patientAppointmentCounts.values(),
	).filter((c) => c > 1).length;
...
export function calculateTherapistMetrics(
	...
	const averageAppointmentsPerDay =
		totalAppointments / days / Math.max(1, active);
	...
	const occupancyRate =
		completed + missed > 0 ? (completed / (completed + missed)) * 100 : 0;
...
export function calculateTimeSlotMetrics(
	...
	const lowDemandHours = sortedHours
		.slice(-3)
		.reverse()
		.map(([h]) => `${String(h).padStart(2, "0")}:00`);
...
export function generateDashboardMetrics(
	...
	return {
		...
		patients: calculatePatientMetrics(
			patients,
			appointments,
			startDate,
			endDate,
		...
		timeSlots: calculateTimeSlotMetrics(
			appointments,
			startDate,
			endDate,
			businessHours,
