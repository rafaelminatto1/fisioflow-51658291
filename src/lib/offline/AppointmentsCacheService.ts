...
class AppointmentsCacheService {
	...
	private getCacheKey(organizationId?: string): string {
		return organizationId
			? `${this.CACHE_KEY_PREFIX}_${organizationId}`
			: this.CACHE_KEY_PREFIX;
	}

	private getTimestampKey(organizationId?: string): string {
		return organizationId
			? `${this.CACHE_TIMESTAMP_KEY_PREFIX}_${organizationId}`
			: this.CACHE_TIMESTAMP_KEY_PREFIX;
	}
	...
	async isCacheValid(organizationId?: string): Promise<boolean> {
		const timestamp = localStorage.getItem(
			this.getTimestampKey(organizationId),
		);
	...
	getCacheAgeMinutes(organizationId?: string): number {
		const key = this.getTimestampKey(
	...
	async getFromCache(
		organizationId?: string,
	): Promise<CacheResult<AppointmentBase>> {
		...
		try {
			...
			const data: AppointmentBase[] = (rawData || []).map(
				(apt: Record<string, unknown>) => ({
					...apt,
					date: apt.date ? new Date(apt.date as string) : new Date(),
					createdAt: apt.createdAt
						? new Date(apt.createdAt as string)
						: new Date(),
					updatedAt: apt.updatedAt
						? new Date(apt.updatedAt as string)
						: new Date(),
				}),
			);
			...
			return {
				...
				metadata: timestamp
					? {
							lastUpdated: new Date(parseInt(timestamp, 10)).toISOString(),
							organizationId,
						}
					: null,
				source: "localstorage",
			...
			logger.error(
				"Error reading appointments cache",
				error,
	...
	async clearCache(organizationId?: string): Promise<void> {
		try {
			...
			logger.error(
				"Error clearing appointments cache",
				error,
				"AppointmentsCacheService",
			);
		}
