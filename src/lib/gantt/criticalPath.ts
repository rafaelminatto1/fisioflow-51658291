 ...
 */
import { GanttTask, GanttDependency, CriticalPath } from "@/types/gantt";
...
export function calculateCriticalPath(
  ...
  const { latestStart, _latestFinish } = calculateLatestTimes(tasks, depMap, earliestFinish);
 ...
 */
export function topologicalSort(tasks: GanttTask[], dependencies: GanttDependency[]): GanttTask[] {
 ...
 */
export function detectCycles(tasks: GanttTask[], dependencies: GanttDependency[]): string[][] {
  const cycles: string[][] = [];
 ...
 */
export function adjustToWorkingDay(date: Date, workingDays: number[] = [1, 2, 3, 4, 5]): Date {
...
function calculateEarliestTimes(
  ...
  const projectStart = new Date(Math.min(...tasks.map((t) => t.start_date.getTime())));
...
export function getBlockedByTasks(
  ...
  const predecessors = depMap.predecessors[taskId] || [];
  return predecessors.map((id) => taskMap.get(id)).filter((t): t is GanttTask => t !== undefined);
}
