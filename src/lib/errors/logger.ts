...
function shouldLogLongTasks(): boolean {
	...
	return (
		getMinLogLevel() === "debug" ||
		window.localStorage?.getItem("DEBUG_LONG_TASKS") === "1"
	);
}
...
class Logger {
	...
	private isDevelopment = (
		import.meta as ImportMeta & { env: { DEV?: boolean; PROD?: boolean } }
	).env.DEV;
	...
	private addLog(entry: LogEntry) {
		...
		const forceLongTask =
			entry.message === "Long Task Detected" &&
			entry.level === "debug" &&
			shouldLogLongTasks();
		if (
			this.isDevelopment &&
			(this.shouldOutput(entry.level) || forceLongTask)
		) {
