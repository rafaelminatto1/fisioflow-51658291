...
function getMinLogLevel(): LogLevel {
  const env = (
    import.meta as ImportMeta & {
      env: { VITE_LOG_LEVEL?: string; DEV?: boolean };
    }
  ).env;
...
class Logger {
  ...
  private isDevelopment = (import.meta as ImportMeta & { env: { DEV?: boolean; PROD?: boolean } })
    .env.DEV;
  ...
  private addLog(entry: LogEntry) {
    ...
    const forceLongTask =
      entry.message === "Long Task Detected" && entry.level === "debug" && shouldLogLongTasks();
  ...
  error(message: string, error?: unknown, component?: string) {
    ...
    if (
      (import.meta as ImportMeta & { env: { DEV?: boolean; PROD?: boolean } }).env.PROD &&
      typeof window !== "undefined" &&
      (
        window as Window & {
          Sentry?: {
            captureException: (error: Error, config: Record<string, unknown>) => void;
            captureMessage: (message: string, config: Record<string, unknown>) => void;
          };
        }
      ).Sentry
    ) {
...
export const log = {
  debug: (message: string, data?: unknown, component?: string) =>
    fisioLogger.debug(message, data, component),
  info: (message: string, data?: unknown, component?: string) =>
    fisioLogger.info(message, data, component),
  warn: (message: string, data?: unknown, component?: string) =>
    fisioLogger.warn(message, data, component),
  error: (message: string, error?: unknown, component?: string) =>
    fisioLogger.error(message, error, component),
  performance: (message: string, data?: unknown, component?: string) =>
    fisioLogger.performance(message, data, component),
};
