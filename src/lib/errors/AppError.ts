...
export class AppError extends Error {
   ...
   */
  static rateLimit(
    message: string = "Muitas tentativas. Tente novamente em alguns instantes.",
   ...
   */
  static timeout(
    message: string = "Operação expirou",
    code: string = "TIMEOUT",
    context?: Record<string, unknown>,
  ) {
  ...
  static from(error: unknown, context?: string | Record<string, unknown>): AppError {
    ...
    const newError = new AppError(message, "UNKNOWN_ERROR", 500, true, errorContext, error);
...
export class DatabaseError extends AppError {
  ...
  static fromNetworkError(error: Error): DatabaseError {
    ...
    if (message.includes("timeout") || message.includes("timed out")) {
      return new DatabaseError("Operação expirou", "TIMEOUT", 504, {
    ...
    if (
      ...
      return new DatabaseError("Erro de conexão com o servidor", "NETWORK_ERROR", 503, {
        originalMessage: error.message,
      });
    }

    return new DatabaseError("Erro de comunicação com o servidor", "COMMUNICATION_ERROR", 503, {
      originalMessage: error.message,
    });
  }
}
