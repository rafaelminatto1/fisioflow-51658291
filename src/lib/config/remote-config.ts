...
export async function getRemoteBoolean(
	key: string,
	defaultValue = false,
): Promise<boolean> {
...
export async function getRemoteNumber(
	key: string,
	defaultValue = 0,
): Promise<number> {
...
export async function getAllRemoteConfig(): Promise<
...
export const featureFlags = {
	...
	isSemanticSearchEnabled: async () =>
		getRemoteBoolean("semantic_search_enabled", false),
	isRAGEabled: async () => getRemoteBoolean("rag_enabled", false),
	isTelemedicineEnabled: async () =>
		getRemoteBoolean("telemedicine_enabled", true),
	isGamificationEnabled: async () =>
		getRemoteBoolean("gamification_enabled", true),
	isMaintenanceMode: async () => getRemoteBoolean("maintenance_mode", false),
	getMaxDailyAppointments: async () =>
		getRemoteNumber("max_daily_appointments", 50),
	getBrandPrimaryColor: async () =>
		getRemoteString("brand_primary_color", "#3b82f6"),
	isDebugLoggingEnabled: async () =>
		getRemoteBoolean("debug_logging_enabled", import.meta.env.DEV),
