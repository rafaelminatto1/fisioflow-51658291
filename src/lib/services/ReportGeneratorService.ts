...
export class ReportGeneratorService {
      ...
      body: [
        [
          ...
          this.calculateProgress(
            data.initialAssessment.painLevel,
            data.currentAssessment.painLevel,
            true,
          ),
        ],
        [
          ...
          this.calculateProgress(
            data.initialAssessment.mobilityScore,
            data.currentAssessment.mobilityScore,
          ),
        ],
        [
          "Funcionalidade (%)",
          ...
          this.calculateProgress(
            data.initialAssessment.functionalScore,
            data.currentAssessment.functionalScore,
          ),
        ],
      ],
      theme: "striped",
  ...
  private static addFooter(doc: jsPDF, therapistName: string): void {
    ...
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text("Activity Fisioterapia | www.activityfisio.com.br", 105, pageHeight - 18, {
      align: "center",
    });
    doc.text(`Responsável: ${therapistName}`, 105, pageHeight - 13, {
      align: "center",
    });
    doc.text(`Gerado em: ${new Date().toLocaleString("pt-BR")}`, 105, pageHeight - 8, {
      align: "center",
    });
  }
  ...
  static generateMedicalReport(data: MedicalReportData): void {
    ...
    if (data.surgeries && data.surgeries.length > 0) {
      ...
      autoTable(doc, {
        ...
        theme: "striped",
        headStyles: {
          fillColor: [...this.HEADER_COLOR] as [number, number, number],
        },
      });
    ...
    if (data.testResults && data.testResults.length > 0) {
      autoTable(doc, {
        ...
        theme: "striped",
        headStyles: {
          fillColor: [...this.SECONDARY_COLOR] as [number, number, number],
        },
      });
      yPos = doc.lastAutoTable?.finalY ? doc.lastAutoTable.finalY + 10 : yPos + 40;
    }
    ...
    if (data.painEvolution && data.painEvolution.length > 0) {
      ...
      autoTable(doc, {
        ...
        body: data.painEvolution.map((p) => [p.date, `${p.level}/10`, p.regions.toString()]),
        theme: "grid",
        headStyles: {
          fillColor: [...this.SECONDARY_COLOR] as [number, number, number],
        },
  ...
  static generatePatientReport(data: PatientFriendlyReportData): void {
    ...
    if (data.painReduction > 0) {
      ...
      doc.text(`-${data.painReduction.toFixed(0)}%`, 105, yPos, {
        align: "center",
      });
      doc.setTextColor(0, 0, 0);
      yPos += 15;
    ...
    doc.setTextColor(0, 0, 0);
    doc.setFont("helvetica", "normal");
    doc.text("Activity Fisioterapia", 105, pageHeight - 18, {
      align: "center",
    });
    doc.text(`Fisioterapeuta: ${data.therapistName}`, 105, pageHeight - 13, {
      align: "center",
    });
    doc.text(`Gerado em: ${new Date().toLocaleString("pt-BR")}`, 105, pageHeight - 8, {
      align: "center",
