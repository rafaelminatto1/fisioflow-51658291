...
export const wikiService = {
	...
	async getTemplateUsageStats(
		...
		return pages.reduce<Record<string, number>>((acc, page) => {
			const templateId =
				(page as { template_id?: string }).template_id || "manual";
	...
	async recordTriageEvents(
		...
		_events: Array<
			Omit<
				WikiTriageEvent,
				"id" | "organization_id" | "changed_by" | "created_at"
			>
		>,
	): Promise<void> {
	...
	async savePage(
		...
		if (existingPage?.id) {
			const result = await wikiApi.update(
				slug,
				data as Parameters<typeof wikiApi.update>[1],
			);
			return (result.data as { id: string }).id ?? existingPage.id;
		}

		const result = await wikiApi.create(
			data as Parameters<typeof wikiApi.create>[0],
		);
