...
export class MandatoryTestAlertService {
	...
	static async checkMandatoryTests(
		...
		try {
			...
			const mandatoryTests = await TestEvolutionService.getMandatoryTests(
				patientId,
				sessionNumber,
			);
			...
			logger.error(
				"Erro ao verificar testes obrigatórios",
				error,
	...
	static async registerException(
		...
		logger.warn(
			"Mandatory test exception registered",
			exceptionRecord,
			"MandatoryTestAlertService",
		);
	 ...
	 */
	static getTestConfigsByPathology(
