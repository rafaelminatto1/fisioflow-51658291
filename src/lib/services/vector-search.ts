...
const indexedEmbeddings = new Map<string, { embedding: number[]; evolution: Partial<Evolution> }>();

function toEvolution(session: Awaited<ReturnType<typeof sessionsApi.list>>['data']['data'][number]): Evolution {
  return {
    id: session.id,
...
export async function findSimilarEvolutions(queryText: string, options: VectorSearchOptions = {}): Promise<VectorSearchResult[]> {
  return withPerformanceTrace('vector_search_similar_evolutions', async () => {
    const { limit = 10, minSimilarity = 0.7, patientId } = options;
    if (!patientId) {
      logger.warn('Vector search global ainda não está disponível sem patientId', undefined, 'vector-search');
      return [];
    }

    try {
      const queryEmbedding = await generateEmbedding(queryText);
      const sessions = (await sessionsApi.list({ patientId, limit: 100 })).data ?? [];
      const results: VectorSearchResult[] = [];

      for (const session of sessions) {
        const evolution = toEvolution(session);
        const text = [session.subjective, session.objective, session.assessment, session.plan].filter(Boolean).join('\n');
        if (!text) continue;
      ...
      logger.error('Erro ao buscar evoluções similares', error, 'vector-search');
      return [];
    }
  });
}

export async function indexEvolution(evolutionId: string, evolution: Partial<Evolution>): Promise<void> {
...
export async function indexBatchEvolutions(evolutionIds: string[]): Promise<void> {
  logger.info(`Indexação em lote compatível solicitada para ${evolutionIds.length} evoluções`, undefined, 'vector-search');
}

export async function findEvolutionsWithoutEmbedding(_limit = 100): Promise<string[]> {
  return [];
}

export async function reindexPatientEvolutions(patientId: string): Promise<void> {
  const sessions = (await sessionsApi.list({ patientId, limit: 100 })).data ?? [];
  await Promise.all(sessions.map((session) => indexEvolution(session.id, toEvolution(session))));
}
