...
const serializePathology = (row: PatientPathology): Pathology => ({
  ...
  severity:
    (row as PatientPathology & { severity?: Pathology["severity"] | null }).severity ?? undefined,
  affected_region:
    (row as PatientPathology & { affected_region?: string | null }).affected_region ?? undefined,
  status: mapStatus(row.status),
  notes: row.notes ?? undefined,
  created_at: row.created_at,
  updated_at:
    (row as PatientPathology & { updated_at?: string | null }).updated_at ?? row.created_at,
});
