...
export class GamificationNotificationService {
  ...
  static questCompleted(title: string, xpReward: number) {
    return toast({
      ...
      description: (
        <div className="space-y-1">
          <p>{title}</p>
          <p className="text-xs font-semibold text-green-600 dark:text-green-400">+{xpReward} XP</p>
        </div>
      ),
      variant: "default",
    });
      ...
      description: (
        <div className="space-y-2">
          <p className="font-semibold text-lg">
            Você alcançou o nível <span className="text-yellow-600">{newLevel}</span>!
          </p>
  ...
  static rewardPurchased(itemName: string, cost: number) {
    return toast({
      ...
      description: (
        <div className="space-y-1">
          <p>
            Você comprou: <span className="font-semibold">{itemName}</span>
          </p>
          <p className="text-xs text-muted-foreground">-{cost} pontos</p>
        </div>
      ),
      variant: "default",
    });
  ...
  static leaderboardPositionChanged(newPosition: number, movedUp: boolean) {
    return toast({
      ...
      description: (
        <div className="space-y-1">
          <p className="font-semibold">{movedUp ? "📈 Você subiu" : "📉 Você desceu"} no ranking</p>
          <p className="text-sm text-muted-foreground">Posição atual: #{newPosition}</p>
        </div>
      ),
      variant: "default",
    });
  ...
  static bonusXP(amount: number, reason: string) {
    return toast({
      ...
      description: (
        <div className="space-y-1">
          <p>{reason}</p>
          <p className="text-xs font-semibold text-yellow-600 dark:text-yellow-400">+{amount} XP</p>
        </div>
      ),
      variant: "default",
    });
