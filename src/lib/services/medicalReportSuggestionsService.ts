...
export class MedicalReportSuggestionsService {
	...
	private static createInsightFromStats(
		...
		if (
			testName.toLowerCase().includes("dor") ||
			testName.toLowerCase().includes("eva")
		) {
			...
			testName.toLowerCase().includes("amplitude") ||
			testName.toLowerCase().includes("rom")
		) {
			...
			testName.toLowerCase().includes("força") ||
			testName.toLowerCase().includes("strength")
		) {
	...
	static formatInsightsForReport(insights: MedicalInsight[]): string {
		...
		if (byType.range_improvement) {
			...
			byType.range_improvement.forEach(
				(i) => (report += `- ${i.insight_text}\n`),
			);
			report += "\n";
		}
		...
		if (byType.functional_milestone) {
			...
			byType.functional_milestone.forEach(
				(i) => (report += `- ${i.insight_text}\n`),
			);
			report += "\n";
		}
