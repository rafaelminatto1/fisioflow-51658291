...
export class GamificationTriggerService {
	...
	static async calculateLevelAsync(
		totalXp: number,
	...
	static async awardSessionCompletion(
		...
		const exerciseBonus =
			Math.min(sessionData.exercisesCount, 20) *
			XP_REWARDS.SESSION_WITH_EXERCISES;
		let bonusXP = 0;
		let bonusReason = "";
	...
	static async awardGoalAchievement(
		...
		if (progress >= 100) {
			...
			if (progressMilestones > previouslyAwardedMilestones) {
				xp =
					(progressMilestones - previouslyAwardedMilestones) *
					XP_REWARDS.GOAL_PROGRESS;
	...
	static async getNextLevelInfo(
		patientId: string,
	): Promise<LevelCalculationResult | null> {
		try {
