...
export class GamificationTriggerService {
  ...
  private static async getSettings(): Promise<GamificationSettings> {
    ...
    try {
      ...
      for (const row of res.data ?? []) {
        if (row.key === 'base_xp' || row.key === 'level_base_xp') settings.base_xp = Number(row.value) || 1000;
        if (row.key === 'multiplier' || row.key === 'level_multiplier') settings.multiplier = Number(row.value) || 1.2;
      }
  ...
  private static async awardXp(
    ...
    try {
      const result = await gamificationApi.awardXp({ patientId, amount, reason, description });
      return {
        success: true,
        xpAwarded: amount,
      ...
      return { success: false, xpAwarded: 0, message: error instanceof Error ? error.message : 'Unknown error' };
    }
  }

  static async awardSessionCompletion(patientId: string, sessionData: { sessionNumber: number; exercisesCount: number; completionTime?: Date; }): Promise<{ success: boolean; xpAwarded: number; newLevel?: number; message?: string }> {
    if (!patientId || !sessionData.sessionNumber) return { success: false, xpAwarded: 0, message: 'Invalid input parameters' };
    const baseXP = XP_REWARDS.SESSION_COMPLETION;
    const exerciseBonus = Math.min(sessionData.exercisesCount, 20) * XP_REWARDS.SESSION_WITH_EXERCISES;
    let bonusXP = 0;
    let bonusReason = '';
    if (sessionData.completionTime) {
      const hour = sessionData.completionTime.getHours();
      if (hour < 10) { bonusXP += XP_REWARDS.EARLY_BIRD; bonusReason = ' (Early Bird)'; }
      else if (hour >= 18) { bonusXP += XP_REWARDS.NIGHT_OWL; bonusReason = ' (Night Owl)'; }
    }
    const finalXP = baseXP + exerciseBonus + bonusXP;
    return this.awardXp(patientId, finalXP, 'session_completed', `Sessão ${sessionData.sessionNumber} concluída${sessionData.exercisesCount > 0 ? ` com ${sessionData.exercisesCount} exercício(s)` : ''}${bonusReason}`);
  }

  static async awardExerciseCompletion(patientId: string, exerciseData: { exerciseId: string; exerciseName: string; repetitionCount?: number; }): Promise<{ success: boolean; xpAwarded: number; message?: string }> {
    if (!patientId || !exerciseData.exerciseName) return { success: false, xpAwarded: 0, message: 'Invalid input parameters' };
    return this.awardXp(patientId, XP_REWARDS.EXERCISE_COMPLETED, 'exercise_completed', `Exercício: ${exerciseData.exerciseName}`);
  }

  static async awardGoalAchievement(patientId: string, goalData: { goalId: string; goalTitle: string; progressPercentage: number; }): Promise<{ success: boolean; xpAwarded: number; message?: string }> {
    if (!patientId || !goalData.goalTitle) return { success: false, xpAwarded: 0, message: 'Invalid input parameters' };
  ...
  static async awardAppointmentAttendance(patientId: string, appointmentData: { appointmentId: string; wasOnTime: boolean; }): Promise<{ success: boolean; xpAwarded: number; message?: string }> {
    if (!patientId || !appointmentData.appointmentId) return { success: false, xpAwarded: 0, message: 'Invalid input parameters' };
    const xp = XP_REWARDS.APPOINTMENT_ATTENDED + (appointmentData.wasOnTime ? XP_REWARDS.APPOINTMENT_ON_TIME : 0);
    return this.awardXp(patientId, xp, 'appointment_attended', appointmentData.wasOnTime ? 'Compareceu ao agendamento pontualmente' : 'Compareceu ao agendamento');
  }

  static async checkAndAwardStreakBonus(patientId: string, currentStreak: number): Promise<{ success: boolean; xpAwarded: number; bonusType?: string; message?: string }> {
    if (!patientId || currentStreak <= 0) return { success: false, xpAwarded: 0, message: 'Invalid input parameters' };
    let xp = 0;
    let bonusType: string | undefined;
    if (currentStreak === 3) { xp = XP_REWARDS.STREAK_3_DAYS; bonusType = '3 dias consecutivos'; }
    else if (currentStreak === 7) { xp = XP_REWARDS.STREAK_7_DAYS; bonusType = '7 dias consecutivos'; }
    else if (currentStreak === 30) { xp = XP_REWARDS.STREAK_30_DAYS; bonusType = '30 dias consecutivos'; }
    else if (currentStreak > 30 && currentStreak % 30 === 0) { xp = XP_REWARDS.STREAK_30_DAYS; bonusType = `${currentStreak} dias consecutivos!`; }
    if (!xp || !bonusType) return { success: true, xpAwarded: 0 };
    const result = await this.awardXp(patientId, xp, 'streak_bonus', `Bônus de sequência: ${bonusType}!`);
