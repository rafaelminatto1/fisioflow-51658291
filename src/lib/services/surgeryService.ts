...
export class SurgeryService {
  static async getSurgeriesByPatientId(patientId: string): Promise<Surgery[]> {
    ...
    return (res.data ?? []).map((row) => ({ ...serializeSurgery(row), patient_id: patientId }));
  }
  ...
  static async updateSurgery(surgeryId: string, data: Partial<SurgeryFormData>): Promise<Surgery> {
    ...
    if (!patientId) throw new Error('patient_id é obrigatório para atualizar cirurgia');

    const res = await patientsApi.updateSurgery(patientId, surgeryId, data);
    return { ...serializeSurgery(res.data), patient_id: patientId };
  }

  static async deleteSurgery(surgeryId: string, patientId: string): Promise<void> {
