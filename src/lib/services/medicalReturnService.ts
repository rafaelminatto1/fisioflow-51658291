...
export class MedicalReturnService {
	static async getMedicalReturnsByPatientId(
		patientId: string,
	...
	static async addMedicalReturn(
		data: MedicalReturnFormData,
	): Promise<MedicalReturn> {
	...
	static async updateMedicalReturn(
		...
		if (!patientId)
			throw new Error("patient_id é obrigatório para atualizar retorno médico");

		const res = await patientsApi.updateMedicalReturn(
			patientId,
			returnId,
			data,
		);
	...
	static async deleteMedicalReturn(
		returnId: string,
		patientId: string,
	): Promise<void> {
