...
export const knowledgeBaseService = {
	...
	async createArticle(
		...
		const res = await knowledgeApi.createArticle(
	...
	async updateArticle(
		...
		const res = await knowledgeApi.updateArticle(
			articleId,
			data as Record<string, unknown>,
		);
		return res.data as unknown as KnowledgeArticle;
	...
	async addAuditEntry(
		entry: Omit<KnowledgeAuditEntry, "id" | "created_at">,
	): Promise<void> {
	...
	async indexKnowledgeArticles(_params: {
		organizationId: string;
	...
	async syncArticles(params: {
		...
		await knowledgeApi.syncArticles(
			params.articles as unknown as Parameters<
				typeof knowledgeApi.syncArticles
			>[0],
		);
	},
};
