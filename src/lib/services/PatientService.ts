...
import { patientsApi, clinicalApi as workersClinicalApi, exerciseSessionsApi } from "@/api/v2";
...
export class PatientService {
  ...
  static async getPatientByProfileId(profileId: string): Promise<Patient | null> {
    ...
    return {
      ...
      status:
        data.status === "active" || data.status === "Em Tratamento" ? "Em Tratamento" : "Inicial",
      progress: data.progress || 0,
