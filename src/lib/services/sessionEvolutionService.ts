...
export class SessionEvolutionService {
	static async calculateSessionNumber(
		patientId: string,
		recordDate: string,
	): Promise<number> {
		const res = await sessionsApi.list({ patientId, limit: 500 });
		return (
			(res.data ?? []).filter((session) => session.record_date < recordDate)
				.length + 1
		);
	}

	static async getSessionEvolution(
		sessionId: string,
	): Promise<SessionEvolution | null> {
		const sessionRes = await sessionsApi.get(sessionId).catch(() => null);
		if (!sessionRes?.data) return null;

		const session = sessionRes.data;
		const measurementsRes = await evolutionApi.measurements.list(
			session.patient_id,
			{ limit: 500 },
		);
		const testResults = (measurementsRes.data ?? [])
			.filter((row) => {
				...
				return (
					customData?.soap_record_id === sessionId ||
					customData?.session_id === sessionId
				);
			})
			.map((row) =>
				mapTestResult(sessionId, row as unknown as Record<string, unknown>),
			);
	...
	static async saveSessionEvolution(
	...
	static async getEvolutionsByPatientId(
		patientId: string,
	): Promise<SessionEvolution[]> {
		const res = await sessionsApi.list({ patientId, limit: 10 });
		const rows = res.data ?? [];
		const evolutions = await Promise.all(
			rows.map((row) => this.getSessionEvolution(row.id)),
		);
