...
export class SessionEvolutionService {
  ...
  static async getSessionEvolution(sessionId: string): Promise<SessionEvolution | null> {
    ...
    const measurementsRes = await evolutionApi.measurements.list(session.patient_id, { limit: 500 });
    ...
    const sessionNumber = await this.calculateSessionNumber(session.patient_id, session.record_date);
