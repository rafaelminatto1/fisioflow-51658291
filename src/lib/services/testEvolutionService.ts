...
const normalizeAlertLevel = (
	value?: string | null,
): AssessmentTestConfig["alert_level"] => {
...
export class TestEvolutionService {
	static async getTestEvolutionData(
		...
		return rows.map((item, index) => ({
			...
			variation:
				index > 0
					? Number(item.value ?? 0) - Number(rows[index - 1].value ?? 0)
					: 0,
		}));
	}
	...
	static async getTestHistory(
		...
		rows.forEach((item) => {
			...
			const previousValue =
				previous.length > 0 ? previous[previous.length - 1].value : null;
			...
			const current: TestEvolutionData = {
				...
				variation:
					previousValue !== null ? Number(item.value ?? 0) - previousValue : 0,
			};
	...
	static async getMandatoryTests(
		...
		const activePathologies = (pathologiesRes.data ?? [])
			.filter(
				(row) => row.status === "active" || row.status === "em_tratamento",
			)
			.map((row) => row.name)
			.filter(Boolean);
		...
		const testsRes =
			await evolutionApi.requiredMeasurements.list(activePathologies);
