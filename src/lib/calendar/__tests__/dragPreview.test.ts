...
describe('dragPreview', () => {
  it('detecta quando o target é o mesmo slot do agendamento', () => {
    ...
    expect(isSameAppointmentSlot(appointment, {
  ...
  it('reutiliza a lista original quando não existe preview válido', () => {
    ...
    expect(buildDragPreviewAppointments(appointments, appointments[0], {
      date: new Date(2026, 2, 12, 12, 0, 0),
      time: '11:00',
    })).toBe(appointments);

    expect(buildDragPreviewAppointments(appointments, null, null)).toBe(appointments);
  });
});
