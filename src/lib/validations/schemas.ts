...
export function sanitizeLongText(text: string): string {
  return text.trim().split("\u0000").join("").replace(/\s+/g, " ").slice(0, 50000); // 50KB max
}
 ...
 * Schema para datas
 */
export const dateSchema = z.string().refine((date) => !isNaN(Date.parse(date)), "Data inválida");
 ...
 */
export const UserRoleEnum = z.enum(["admin", "fisioterapeuta", "estagiario", "paciente"], {
  errorMap: () => ({
    message: "Cargo inválido. Use: admin, fisioterapeuta, estagiário ou paciente",
  }),
});
 ...
 * Status de pagamento
 */
export const PaymentStatusEnum = z.enum(["pending", "paid", "cancelled", "refunded"], {
  errorMap: () => ({ message: "Status inválido" }),
});
 ...
 */
export const ExerciseDifficultyEnum = z.enum(["facil", "medio", "dificil", "avancado"]);
 ...
 * Status de patologia
 */
export const PathologyStatusEnum = z.enum(["em_tratamento", "tratada", "cronica"], {
  errorMap: () => ({ message: "Status inválido" }),
});
