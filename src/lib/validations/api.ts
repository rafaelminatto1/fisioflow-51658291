...
export const commonSchemas = {
  ...
  phone: z
    .string()
    .regex(/^(\+?\d{1,3}[- ]?)?\(?\d{2,3}\)?[- ]?\d{4,5}[- ]?\d{4}$/, "Telefone inválido"),
   ...
   */
  paginatedApiResponse: <T extends z.ZodType>(dataSchema: T) =>
    z.object({
 ...
 */
export async function parseApiResponse<T>(schema: z.ZodType<T>, response: Response): Promise<T> {
...
export function validateWithLogging<T>(
  ...
  if (!result.success) {
    logger.error(`[Validation Error - ${context}]`, result.error.issues, "api-validations");
    return null;
  }
