...
export const patientSchema = z.object({
	...
	session_price: z
		.number()
		.min(0, "Preço da sessão deve ser positivo")
		.max(9999.99, "Preço da sessão muito alto"),
	package_sessions: z
		.number()
		.int()
		.min(0, "Sessões do pacote devem ser não negativas")
		.max(999, "Sessões do pacote muito altas"),
	remaining_sessions: z
		.number()
		.int()
		.min(0, "Sessões restantes devem ser não negativas")
		.max(999, "Sessões restantes muito altas"),
	important_notes: z
		.string()
		.max(2000, "Observações importantes não podem ter mais de 2000 caracteres"),
...
export const paymentSchema = z
	.object({
		...
		sessions_count: z
			.number()
			.int()
			.min(1, "Quantidade de sessões deve ser pelo menos 1")
			.max(999, "Quantidade de sessões muito alta")
			.optional(),
		...
		notes: z
			...
			.max(
				500,
				"Observações do pagamento não podem ter mais de 500 caracteres",
			),
		created_at: z.string().datetime(),
	})
	.refine(
...
export const createPaymentSchema = z
	.object({
		...
		notes: z
			.string()
			.max(500, "Observações do pagamento não podem ter mais de 500 caracteres")
			.optional()
			.default(""),
	})
	.refine(
...
// Filter schemas
export const agendaFiltersSchema = z
	.object({
...
export const appointmentFormSchema = z
	...
	.refine(
		...
		{
			message:
				"Data final da recorrência deve ser posterior à data do agendamento",
