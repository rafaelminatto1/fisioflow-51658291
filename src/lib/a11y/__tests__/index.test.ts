...
describe("ariaAttributes", () => {
  describe("expanded", () => {
    ...
    it("should return aria-expanded false", () => {
      expect(ariaAttributes.expanded(false)).toEqual({
        "aria-expanded": false,
      });
    });
  });
  ...
  describe("checked", () => {
    ...
    it("should accept mixed value", () => {
      expect(ariaAttributes.checked("mixed")).toEqual({
        "aria-checked": "mixed",
      });
    });
  });
...
describe("useKeyboardListNavigation", () => {
  ...
  it("should call onSelect on activation key", () => {
    ...
    const { result } = renderHook(() => useKeyboardListNavigation(items, onSelect));
