...
export function createLazyComponent<T = Record<string, unknown>>(
  ...
  const { importFn, fallback, displayName } = options;
...
export const usePreloadOnHover = (preloadFn: () => void, options: { delay?: number } = {}) => {
...
export const useRoutePreload = (config: PreloadConfig) => {
  ...
  useEffect(() => {
    ...
    const currentRoute = config.routes.find((r) => currentPath.startsWith(r.path));
...
export const PreloadLink = ({ to, children, preload, className, onClick }: PreloadLinkProps) => {
  const navigate = useNavigate();
