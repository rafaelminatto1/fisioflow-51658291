...
export const PROM_SNAPSHOTS_SEED: PromSnapshot[] = [
	...
	},
	{
		...
		notes:
			"Após 5 semanas. Relata mais confiança e menos receio em tarefas específicas.",
		tags: ["acl", "reassess"],
	},
	...
	},
	{
		...
		notes:
			"Refere melhora importante na tolerância à carga; retorno progressivo.",
...
export function getLatestPromByPatient(seed: PromSnapshot[]) {
	...
	for (const s of seed) {
		...
		if (
			!prev ||
			new Date(s.captured_at).getTime() > new Date(prev.captured_at).getTime()
		) {
