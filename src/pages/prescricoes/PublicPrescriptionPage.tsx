...
export default function PublicPrescriptionPage() {
  ...
  const { prescription, loading, error, markExerciseComplete, isMarking } = usePublicPrescription(
    qrCode || "",
  );
  ...
  const isExpired =
    prescription.valid_until && isAfter(new Date(), parseISO(prescription.valid_until));
  ...
  const progressPercent =
    exercises.length > 0 ? (completedExercises.length / exercises.length) * 100 : 0;
      ...
      <div className="max-w-2xl mx-auto px-4 -mt-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground">
                  ...
                  {prescription.valid_until
                    ? format(parseISO(prescription.valid_until), "dd/MM/yyyy", {
                        locale: ptBR,
                      })
                    : "N/A"}
                </span>
        ...
        <div className="space-y-3">
          ...
          {exercises.map((exercise, index: number) => {
            ...
            return (
              <Card
                key={exercise.id}
                className={cn("transition-all", isCompleted && "bg-green-50 border-green-200")}
              >
        ...
        {prescription.therapist?.full_name && (
          <div className="mt-6 text-center text-sm text-muted-foreground">
            ...
            <p className="font-medium text-foreground">{prescription.therapist.full_name}</p>
          </div>
        )}

        {/* Footer */}
        <div className="mt-8 text-center">
          <p className="text-xs text-muted-foreground">Desenvolvido por FisioFlow</p>
        </div>
      </div>
    </div>
  );
