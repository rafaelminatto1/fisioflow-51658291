...
const SHOP_ITEMS: ShopItem[] = [
	{
		...
		description:
			"Proteja sua sequência por um dia caso você esqueça de treinar",
		price: 100,
...
export default function GamificationShopPage() {
	...
	const { profile, xpPerLevel, patientInventory } = useGamification(
		user?.id || "",
	);

	const [selectedCategory, setSelectedCategory] = useState<
		"all" | "streak_freeze" | "avatar" | "title" | "boost" | "special"
	>("all");
				...
				<div className="max-w-6xl mx-auto p-4 md:p-6 space-y-6">
					...
					>
						{filteredItems.map((item) => {
							...
							const outOfStock =
								item.limited && item.stock !== undefined && item.stock <= 0;
                  ...
                  `}
                      ...
                      `}
											...
											<div className="flex-1 min-w-0">
												<h3
													className={`font-bold mb-1 ${owned ? "text-green-900" : "text-gray-900"}`}
												>
													{item.name}
												</h3>
					...
					<Card className="bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-200">
						<div className="p-6">
							...
							<ul className="text-sm text-muted-foreground space-y-1">
								<li>
									• Use seus pontos acumulados para adquirir itens exclusivos
								</li>
								<li>• Itens limitados podem esgotar - garanta o seu!</li>
								<li>
									• Títulos e avatares ficam permanentemente no seu perfil
								</li>
