...
export default function GamificationLeaderboardPage() {
	const { user } = useAuth();
	const { profile, xpPerLevel } = useGamification(user?.id || "");
	...
	const currentUserRank =
		leaderboard.findIndex((e) => e.patient_id === user?.id) + 1;
		...
		<PageLayout>
			<PageContainer>
				<PageHeader
					...
					subtitle="Veja como você se destaca entre os demais pacientes"
				/>
									...
									<div className="flex items-center justify-between">
										...
										<div className="text-right">
											...
											<p className="text-blue-100 text-sm">
												Nível {currentUserEntry.current_level}
											</p>
										</div>
									</div>
								</div>
						...
						<Card className="p-4">
							<div className="flex flex-col md:flex-row gap-4">
								...
								<Tabs
									value={period}
									onValueChange={(v) =>
										setPeriod(v as "weekly" | "monthly" | "all")
									}
									className="w-auto"
								>
									<TabsList>
                        ...
                        `}
													...
													<div className="flex-1 min-w-0">
														<div className="flex items-center gap-2">
															<p
																className={`font-medium truncate ${isCurrentUser && "text-primary font-bold"}`}
															>
																{entry.patient_name}
															</p>
															{isCurrentUser && (
														...
														<p className="text-xs text-muted-foreground">
															...
															{entry.current_streak > 0 &&
																` • ${entry.current_streak} dias seguidos`}
														</p>
													</div>
													...
													<div className="text-right shrink-0">
														<span
															className={`text-lg font-bold ${isCurrentUser ? "text-primary" : "text-slate-700"}`}
														>
															{entry.total_xp}
														</span>
							...
							<Card className="p-6 bg-gradient-to-br from-yellow-50 to-amber-50 border-yellow-200">
								...
								<div className="flex items-end justify-center gap-4">
									...
									<div className="flex flex-col items-center">
										...
										<p className="font-medium text-sm truncate max-w-[80px]">
											{leaderboard[2]?.patient_name}
										</p>
										<Badge
											variant="secondary"
											className="mt-1 bg-amber-200 text-amber-700"
										>
											3º lugar
										</Badge>
										<p className="text-xs text-muted-foreground mt-1">
