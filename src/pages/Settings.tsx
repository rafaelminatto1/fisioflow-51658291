    ...
    <MainLayout>
      <div className="space-y-4 sm:space-y-6 animate-fade-in">
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground">Gerencie sua conta e as preferências do sistema</p>
        </div>
        ...
        <Tabs value={state.activeTab} onValueChange={state.handleTabChange} className="w-full">
          <TabsList className={`grid w-full ${state.isAdmin ? 'grid-cols-6' : 'grid-cols-5'} h-9 sm:h-10 gap-0.5`}>
            ...
            <TabsTrigger value="notifications" className="flex items-center gap-1 sm:gap-2 text-[10px] sm:text-xs">
              <Bell className="w-3.5 h-3.5 sm:w-4 sm:h-4" /><span className="hidden xs:inline">Notif</span>
            </TabsTrigger>
            ...
            <TabsTrigger value="a11y" className="flex items-center gap-1 sm:gap-2 text-[10px] sm:text-xs">
              <Contrast className="w-3.5 h-3.5 sm:w-4 sm:h-4" /><span className="hidden xs:inline">Acessib.</span>
            </TabsTrigger>
          </TabsList>

          <TabsContent value="profile" className="pt-4"><ProfileTab /></TabsContent>
          <TabsContent value="notifications" className="pt-4"><NotificationsTab /></TabsContent>
          <TabsContent value="security" className="pt-4">
            <SecurityTab 
              isAdmin={state.isAdmin} user={state.user} mfa={state.mfa} 
          ...
          <TabsContent value="a11y" className="pt-4">
            <AccessibilityTab settings={state.accessibility.settings} setSettings={state.accessibility.setSettings} />
          </TabsContent>
        </Tabs>
      </div>

      <Suspense fallback={null}>
        <InviteUserModal open={state.inviteModalOpen} onOpenChange={state.setInviteModalOpen} />
      </Suspense>
    </MainLayout>
  );
}
