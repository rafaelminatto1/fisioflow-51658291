...
const PreCadastroAdmin = () => {
  const { data: tokens = [], isLoading: tokensLoading } = usePrecadastroTokens();
  const { data: precadastros = [], isLoading: precadastrosLoading } = usePrecadastros();
  ...
  const handleCreateLink = async (newToken: NewTokenData) => {
    ...
    await createToken.mutateAsync({
      ...
      expires_at: new Date(Date.now() + validadeDias * 24 * 60 * 60 * 1000).toISOString(),
