		...
		<MainLayout>
			<div className="container max-w-7xl mx-auto py-6 md:py-8 space-y-8 animate-fade-in pb-20 md:pb-0">
				...
				<div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-premium-sm border border-slate-100 dark:border-slate-800/50">
					<div>
						...
						<p className="text-sm font-medium text-slate-500 dark:text-slate-400 mt-2">
							Visão consolidada de receitas, despesas e performance
						</p>
					</div>
				...
				<section className="animate-in fade-in slide-in-from-bottom-4 duration-700">
					<Suspense
						fallback={
							<Card className="h-48 animate-pulse bg-muted/30 border-border/50 rounded-2xl" />
						}
					>
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
					...
					<Card className="rounded-2xl border-none shadow-premium-sm hover-lift bg-white dark:bg-slate-900 overflow-hidden group">
						...
						<CardContent className="px-6 pb-6">
							<p className="text-3xl font-black tracking-tighter text-slate-900 dark:text-white">
								R$ {safeStats.averageTicket.toLocaleString("pt-BR")}
							</p>
							<p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-2">
								Média p/ sessão
							</p>
						...
						<CardContent className="px-6 pb-6">
							<p className="text-3xl font-black tracking-tighter text-slate-900 dark:text-white">
								{safeStats.totalCount
									? Math.round(
											(safeStats.paidCount / safeStats.totalCount) * 100,
										)
									: 0}
								%
							</p>
							<p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-2">
								{safeStats.paidCount} de {safeStats.totalCount} realizados
							</p>
						</CardContent>
					</Card>
				</div>
				...
				<Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
					...
					>
						<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
							<h3 className="text-xl font-black tracking-tighter text-slate-900 dark:text-white uppercase tracking-[0.1em]">
								Transações Recentes
							</h3>
							<Select
								value={period}
											...
											{transactions.map((t: Transaction) => (
												...
												>
													...
													<td className="px-6 py-5">
														<p className="font-bold text-sm text-slate-900 dark:text-white truncate max-w-[200px] leading-tight">
													...
													<td className="px-6 py-5 text-right space-x-2">
														<div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
															...
															<Button
																...
																className="h-8 w-8 rounded-lg text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800"
															>
			...
			>
				<AlertDialogContent className="rounded-2xl">
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Esta ação removerá o registro permanentemente do fluxo de caixa.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
