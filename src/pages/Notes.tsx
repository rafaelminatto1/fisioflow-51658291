...
import {
  NoteEditor,
  NotesHome,
  type NoteEditorViewModel,
  type NoteKind,
  type NotesHomeViewModel,
} from '../../apps/web/src/features/notes';
...
import {
  ...
  useNoteRevisions,
  useNoteTasks,
  useRevokeNotePermission,
  useToggleNoteFavorite,
  useUpdateNote,
  useUpdateNoteCommentStatus,
  useUploadNoteAttachment,
  useWorkerNote,
  useWorkerNotes,
} from '@/hooks/useWorkerNotes';
...
import {
  NotesRichTextEditor,
  type NotesCollaborationStatus,
} from '@/components/notes/NotesRichTextEditor';
...
const STARTER_TEMPLATES: NoteTemplate[] = [
  {
    ...
    type: 'meeting',
    classification: 'team',
    contentText: 'Objetivo da reunião\n\nParticipantes\n\nPauta\n\nDecisões\n\nPróximos passos',
  },
  {
    id: 'team',
    title: 'Nota de equipe',
    description: 'Alinhamentos operacionais do time.',
    type: 'team',
    classification: 'team',
  ...
  },
  {
    id: 'protocol',
    title: 'Protocolo',
    description: 'Base reutilizável de conhecimento interno.',
  ...
  },
  {
    ...
    description: 'Preparação segura antes da sessão.',
  ...
  },
  {
    ...
    description: 'Pendências e acompanhamento após a sessão.',
    type: 'patient_context',
    contentText: 'Resumo do contexto\n\nPendências\n\nOrientações a reforçar\n\nPróximo passo',
    requiresPatient: true,
  },
];
...
function toHomeModel(
  ...
  activeFilter: NotesHomeViewModel['activeFilter'] = 'all',
  currentUserId?: string,
  allNotes: NoteRecord[] = notes,
  ...
  return {
    ...
    spaces: [
      {
        ...
        count: allNotes.filter((note) => note.classification === 'private').length,
        accent: 'primary',
      },
      {
        id: 'team',
        ...
        count: allNotes.filter((note) => note.classification === 'team').length,
        accent: 'accent',
      },
      {
        id: 'clinical',
        ...
        count: allNotes.filter((note) => note.patientId).length,
      },
    ],
    activeSpaceId,
    notes: notes.map((note) => ({
      ...
      updatedLabel: new Intl.DateTimeFormat('pt-BR', { dateStyle: 'medium' }).format(
...
function toEditorModel(
  note: NoteRecord,
  context: {
    taskData: ReturnType<typeof useNoteTasks>['data'];
    commentCount: number;
    backlinkData: ReturnType<typeof useNoteBacklinks>['data'];
  }
): NoteEditorViewModel {
  ...
  return {
    ...
    breadcrumbs: [
      note.patientId ? 'Contexto clínico' : 'Meu espaço',
      note.type === 'meeting' ? 'Reuniões' : 'Notas',
    ],
    blocks: paragraphs,
    patient: note.patientId
      ? {
          ...
          details: 'Acesso restrito à equipe autorizada',
        }
      : undefined,
    collaborators: [],
    linkedTasks: (context.taskData ?? []).map((task) => ({
      id: task.id,
      title: task.titulo,
      status:
    ...
    backlinks: (context.backlinkData ?? []).map((backlink) => ({
      id: backlink.id,
      title: backlink.title,
      kind: 'note',
      description: backlink.relationTypes.join(', '),
    })),
...
export default function Notes() {
  ...
  const [homeFilter, setHomeFilter] =
    useState<NonNullable<NotesHomeViewModel['activeFilter']>>('all');
  ...
  const mentionMatches = useQuery({
    queryKey: ['notes', 'mentionables', mentionType, patientSearch],
    queryFn: ({ signal }) => notesApi.mentionables(patientSearch, [mentionType], { signal }),
    enabled: mentionOpen && patientSearch.trim().length >= 2,
  });
  const mentionItems = useCallback(
    async (query: string, signal: AbortSignal): Promise<MentionSuggestionItem[]> => {
      if (query.trim().length < 1) return [];
      const result = await notesApi.mentionables(
        query.trim(),
        MENTIONABLE_TYPES.map((item) => item.value),
        { signal }
      );
  ...
  const onInlineMentionSelected = useCallback(
    (item: MentionSuggestionItem) => {
      if (!detail.data) return;
      createMention.mutate({
        noteId: detail.data.id,
        mentionType: item.type as NoteMentionRecord['mentionType'],
        targetId: item.id,
  ...
  const scopedHomeNotes = useMemo(() => {
    if (activeSpaceId === 'mine')
      return homeNotes.filter((note) => note.classification === 'private');
    if (activeSpaceId === 'team') return homeNotes.filter((note) => note.classification === 'team');
    if (activeSpaceId === 'clinical') return homeNotes.filter((note) => Boolean(note.patientId));
    return homeNotes;
  }, [activeSpaceId, homeNotes]);
  const filteredHomeNotes = useMemo(() => {
    const favoriteIds = new Set(favorites.data ?? []);
    if (homeFilter === 'favorites')
      return scopedHomeNotes.filter((note) => favoriteIds.has(note.id));
    if (homeFilter === 'patient') return scopedHomeNotes.filter((note) => Boolean(note.patientId));
    if (homeFilter === 'templates')
      return scopedHomeNotes.filter((note) => note.type === 'template');
    if (homeFilter === 'archived')
      return scopedHomeNotes.filter((note) => note.status === 'archived');
    if (homeFilter === 'shared')
      return scopedHomeNotes.filter((note) =>
        Boolean(note.ownerId && user?.uid && note.ownerId !== user.uid)
      );
    if (homeFilter === 'recent') {
      const cutoff = Date.now() - 1000 * 60 * 60 * 24 * 30;
      return scopedHomeNotes.filter((note) => new Date(note.updatedAt).getTime() >= cutoff);
    }
    return scopedHomeNotes;
  }, [favorites.data, homeFilter, scopedHomeNotes, user?.uid]);
  const homeModel = useMemo(
    ...
    [activeSpaceId, favorites.data, filteredHomeNotes, homeFilter, list.data, user?.uid]
  );
  const returnTo =
    typeof (location.state as { returnTo?: unknown } | null)?.returnTo === 'string' &&
    (location.state as { returnTo: string }).returnTo.startsWith('/') &&
    !(location.state as { returnTo: string }).returnTo.startsWith('//') &&
    !(location.state as { returnTo: string }).returnTo.startsWith('/notes')
  ...
  const handleReturnToSystem = useCallback(() => {
    ...
    // Navega pelo histórico somente em uma sessão do SPA; entradas diretas começam no índice zero.
  ...
  const handleChangeFilter = useCallback(
    (filter: NonNullable<NotesHomeViewModel['activeFilter']>) => {
      setHomeFilter(filter);
      if (filter === 'all') setActiveSpaceId(undefined);
    },
    []
  );
  const createNote = (template?: NoteTemplate) => {
    ...
    create.mutate(
      patientId
        ? {
            title: selected?.title ?? 'Nota de contexto',
            type: 'patient_context',
            patientId,
        ...
        : {
            title: selected?.title ?? 'Sem título',
            type: selected?.type,
          ...
          },
      {
        onSuccess: ({ data }) => {
          ...
          navigate(`/notes/${data.id}`, { state: location.state });
        },
      }
    );
  };
    ...
    grantPermission.mutate(
      ...
      },
      {
        onSuccess: () => {
          setShareOpen(false);
          setMemberId('');
          setShareSubjectType('user');
        },
      }
    );
  };
  const memberName = (userId: string | null) =>
    users.find((user) => user.id === userId)?.full_name ?? 'Membro da equipe';
  const permissionSubjectName = (item: NonNullable<typeof notePermissions.data>[number]) => {
    ...
    if (item.subjectType === 'role')
      return (
        SHAREABLE_ROLES.find((role) => role.value === item.subjectId)?.label ??
        `Cargo: ${item.subjectId}`
      );
    return memberName(item.subjectId);
  };
  const mentionEntity = (entity: {
    id: string;
    type: NoteMentionRecord['mentionType'];
    label: string;
  }) => {
    if (!detail.data) return;
    createMention.mutate(
  ...
  const submitComment = () => {
    ...
    createComment.mutate(
      { noteId: detail.data.id, body: commentBody.trim() },
  ...
  const publishToPortal = async () => {
    ...
    try {
      await notesApi.publishToPatientPortal(detail.data.id, {
        contentText: portalText.trim(),
  ...
  const restoreRevision = async (revision: NoteRevisionRecord) => {
    if (
      !detail.data ||
      !window.confirm(
        `Restaurar a versão ${revision.versionNumber}? As sessões colaborativas serão sincronizadas antes da próxima edição.`
      )
    )
      return;
    try {
      ...
      <>
        <div className="flex min-h-[calc(100vh-4rem)] min-w-0 flex-col overflow-hidden bg-background lg:flex-row">
          <aside className="order-2 h-[38vh] min-h-0 w-full shrink-0 border-t border-border bg-card lg:order-1 lg:h-[calc(100vh-4rem)] lg:w-[280px] lg:border-t-0 lg:border-r xl:w-[320px]">
            <NotesHome
              ...
              onSelectNote={(id) => navigate(`/notes/${id}`, { state: location.state })}
              onSelectSpace={setActiveSpaceId}
              onToggleFavorite={(id) =>
                toggleFavorite.mutate({ noteId: id, favorite: !favorites.data?.includes(id) })
              }
            />
          </aside>
          <main className="order-1 min-h-0 min-w-0 flex-1 lg:order-2">
            <NoteEditor
              viewModel={{
                ...toEditorModel(detail.data, {
                  taskData: noteTasks.data,
                  commentCount: noteComments.data?.length ?? 0,
                  backlinkData: noteBacklinks.data,
                }),
                title,
                savedLabel:
                ...
                editorContent: (
                  <NotesRichTextEditor
                    contentHtml={
                      ...
                      (detail.data.contentText ? `<p>${detail.data.contentText}</p>` : '')
                    }
                    ...
                    offlinePersistence={
                      showFeature('notes_offline') &&
                      !detail.data.patientId &&
                      detail.data.sensitivity === 'internal'
                    }
                    offlinePersistenceKey={
                      showFeature('notes_offline') && user?.uid && detail.data
                        ? `notes:${user.uid}:${user.organizationId ?? 'unknown'}:${detail.data.id}`
                        : undefined
                    }
              ...
              onInsert={(command, file) =>
                command === 'mention'
                  ? setMentionOpen(true)
                  : command === 'attachment' && file && detail.data
                    ? uploadAttachment.mutate(
                        { noteId: detail.data.id, file },
                        { onSuccess: () => setAttachmentsOpen(true) }
                      )
                    : undefined
              }
            />
        ...
        <div className="fixed right-4 bottom-4 left-4 z-20 flex flex-wrap justify-center gap-2 lg:left-[360px]">
          ...
          <Button variant="outline" size="sm" onClick={() => setAttachmentsOpen(true)}>
            ...
            Anexos{attachments.data?.length ? ` (${attachments.data.length})` : ''}
          </Button>
          <Button variant="outline" size="sm" onClick={() => setCommentsOpen(true)}>
            <MessageSquarePlus className="mr-2 size-4" />
            Comentar
          </Button>
          <Button variant="outline" size="sm" onClick={() => void exportNote()}>
          ...
          {!detail.data.patientId &&
          detail.data.classification !== 'clinical' &&
          detail.data.sensitivity === 'internal' ? (
            <Button
              variant="outline"
              size="sm"
        ...
        <Dialog open={attachmentsOpen} onOpenChange={setAttachmentsOpen}>
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle>Anexos da nota</DialogTitle>
              <DialogDescription>
                Arquivos protegidos, disponíveis apenas para pessoas com acesso à nota.
              </DialogDescription>
            </DialogHeader>
            <Label className="block cursor-pointer rounded-md border border-dashed border-primary/40 p-3 text-center text-sm font-medium text-primary hover:bg-primary/5">
              Adicionar arquivo
              <input
                className="sr-only"
                type="file"
                accept="application/pdf,image/jpeg,image/png,image/webp,text/plain,application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                onChange={(event) => {
                  const file = event.target.files?.[0];
                  if (file) uploadAttachment.mutate({ noteId: detail.data!.id, file });
                  event.currentTarget.value = '';
                }}
              />
              <span className="mt-1 block text-xs font-normal text-muted-foreground">
                PDF, imagem, TXT ou DOCX · até 25 MB
              </span>
            </Label>
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {(attachment.sizeBytes / 1024).toLocaleString('pt-BR', {
        ...
        <Dialog open={shareOpen} onOpenChange={setShareOpen}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Defina quem pode acessar e o nível de permissão. Notas clínicas nunca podem ser
                abertas para toda a equipe.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-2">
              <div className="space-y-2">
                ...
                <Select
                  value={shareSubjectType}
                  onValueChange={(value) => {
                    setShareSubjectType(value as ShareSubjectType);
                    setMemberId('');
                  }}
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    <SelectItem value="user">Uma pessoa</SelectItem>
                    <SelectItem value="role">Um cargo da equipe</SelectItem>
                    {detail.data.classification !== 'clinical' ? (
              ...
              {shareSubjectType === 'role' ? (
                <div className="space-y-2">
                  <Label>Cargo da equipe</Label>
                  <Select value={memberId} onValueChange={setMemberId}>
                    <SelectTrigger>
              ...
              {shareSubjectType === 'organization' ? (
                <div className="flex gap-3 rounded-md border border-amber-500/30 bg-amber-500/5 p-3 text-sm text-muted-foreground">
                  <ShieldCheck className="mt-0.5 size-4 shrink-0 text-amber-600" />
                  <p>
                    Todos os membros ativos da clínica poderão acessar esta nota com a permissão
                    escolhida.
                  </p>
                </div>
              ) : null}
            ...
            <DialogFooter>
              ...
              <Button
                disabled={
                  (shareSubjectType !== 'organization' && !memberId) || grantPermission.isPending
                }
                onClick={shareNote}
              >
                Compartilhar
        ...
        <Dialog open={mentionOpen} onOpenChange={setMentionOpen}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>Inserir menção</DialogTitle>
              <DialogDescription>
                A busca respeita as permissões da organização e cria uma relação auditável com a
                nota.
              </DialogDescription>
            </DialogHeader>
            <Select
              value={mentionType}
              onValueChange={(value) => {
                setMentionType(value as NoteMentionRecord['mentionType']);
                setPatientSearch('');
              }}
            >
              <SelectTrigger>
            ...
            {patientSearch.trim().length >= 2 ? (
              <div className="max-h-56 space-y-1 overflow-y-auto">
                {(mentionMatches.data?.data ?? []).map((entity) => (
                  <button
                    type="button"
                    key={`${entity.type}-${entity.id}`}
                    onClick={() => mentionEntity(entity)}
                    className="w-full rounded-md px-3 py-2 text-left text-sm hover:bg-muted"
                  >
                    <span className="block font-medium">{entity.label}</span>
                    {entity.subtitle ? (
                      <span className="block text-xs text-muted-foreground">{entity.subtitle}</span>
                    ) : null}
                  </button>
                ))}
                {!mentionMatches.isLoading && !(mentionMatches.data?.data ?? []).length ? (
        ...
        <Dialog open={commentsOpen} onOpenChange={setCommentsOpen}>
          <DialogContent className="max-w-lg">
            <DialogHeader>
              ...
              <DialogDescription>Discussões vinculadas a esta nota.</DialogDescription>
            </DialogHeader>
            ...
            <p className="text-xs text-muted-foreground">Use Ctrl/Cmd + Enter para enviar.</p>
            <div className="max-h-80 space-y-3 overflow-y-auto">
              {noteComments.data?.length ? (
                noteComments.data.map((comment) => (
                  <article
                    key={comment.id}
                    className={`rounded-md border p-3 ${comment.status === 'resolved' ? 'border-border/70 bg-muted/30' : 'border-border'}`}
                  >
                    <div className="flex items-start gap-3">
                      <p
                        className={`min-w-0 flex-1 text-sm ${comment.status === 'resolved' ? 'text-muted-foreground line-through' : ''}`}
                      >
                        {comment.body}
                      </p>
                      <Button
        ...
        <Dialog open={historyOpen} onOpenChange={setHistoryOpen}>
          <DialogContent className="max-w-lg">
            <DialogHeader>
              ...
              <DialogDescription>
                Snapshots imutáveis desta nota. Restaurar cria uma nova versão e sincroniza a
                colaboração.
              </DialogDescription>
            </DialogHeader>
            <div className="max-h-80 space-y-3 overflow-y-auto">
              {noteRevisions.data?.length ? (
                noteRevisions.data.map((revision) => (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-semibold">Versão {revision.versionNumber}</p>
                      <p className="mt-1 text-xs text-muted-foreground">
                        {revision.reason || 'Snapshot automático'} ·{' '}
                        {new Intl.DateTimeFormat('pt-BR', {
                          dateStyle: 'short',
                          timeStyle: 'short',
        ...
        <Dialog open={portalOpen} onOpenChange={setPortalOpen}>
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle>Publicar no portal do paciente</DialogTitle>
              <DialogDescription>
                Escolha exatamente o texto que o paciente verá. A publicação expira em 7 dias e pode
                ser revogada.
              </DialogDescription>
            </DialogHeader>
            <textarea
              ...
              maxLength={20000}
              rows={9}
              placeholder="Escreva a orientação ou resumo destinado ao paciente…"
            ...
            <DialogFooter>
              ...
              <Button
                disabled={!portalText.trim() || portalLoading}
                onClick={() => void publishToPortal()}
              >
                {portalLoading ? 'Publicando…' : 'Publicar por 7 dias'}
              </Button>
            </DialogFooter>
            <div className="border-t border-border pt-4">
              <p className="mb-2 text-sm font-semibold">Publicações desta nota</p>
              {portalPublications.data?.length ? (
                <div className="space-y-2">
                  {portalPublications.data.map((publication) => (
                    <div
                      key={publication.id}
                      className="flex items-center gap-3 rounded-md border border-border p-2 text-xs"
                    >
                      <span className="min-w-0 flex-1 truncate">
                        {publication.title} · expira{' '}
                        {new Intl.DateTimeFormat('pt-BR', { dateStyle: 'short' }).format(
                          new Date(publication.expiresAt)
                        )}
                      </span>
                      {!publication.revokedAt && new Date(publication.expiresAt) > new Date() ? (
                        <Button
                          variant="ghost"
                          size="sm"
                          className="text-destructive"
                          onClick={() => void revokePortalPublication(publication.id)}
                        >
                          Revogar
                        </Button>
                      ) : (
                ...
                <p className="text-xs text-muted-foreground">Nenhuma publicação registrada.</p>
              )}
            </div>
          </DialogContent>
        </Dialog>
        ...
        >
          <DialogContent className="max-w-lg">
            <DialogHeader>
              ...
              <DialogDescription>
                Gerado por IA a partir desta nota interna. Revise antes de reutilizar.
              </DialogDescription>
            </DialogHeader>
            <p className="whitespace-pre-wrap text-sm leading-7">{aiSummary}</p>
          </DialogContent>
        </Dialog>
      </>
    );
      ...
      <Dialog open={templateOpen} onOpenChange={setTemplateOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Modelos estruturados para começar rápido, sem substituir a evolução clínica oficial.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3 sm:grid-cols-2">
            {STARTER_TEMPLATES.map((template) => {
              const unavailable = Boolean(template.requiresPatient && !patientId);
              return (
                <button
                  type="button"
                  key={template.id}
                  ...
                  className="rounded-lg border border-border p-4 text-left transition hover:border-primary hover:bg-primary/5 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  <p className="font-semibold">{template.title}</p>
                  <p className="mt-1 text-sm text-muted-foreground">{template.description}</p>
                  {unavailable ? (
                    <p className="mt-3 text-xs text-amber-700">
                      Abra as notas pela ficha do paciente para usar este modelo.
                    </p>
                  ) : null}
                </button>
              );
