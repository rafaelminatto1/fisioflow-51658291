			...
			<Page size="A4" style={styles.page}>
				...
				<View style={styles.header}>
					...
					<View>
						...
						<Text style={styles.metaInfo}>
							{format(new Date(data.data_emissao), "dd 'de' MMMM 'de' yyyy", {
								locale: ptBR,
							})}
						</Text>
					</View>
				...
				<View style={{ flexDirection: "row", marginBottom: 25, gap: 15 }}>
					...
					>
						...
						<Text
							style={{ fontWeight: "bold", fontSize: 11, color: "#0f172a" }}
						>
							{data.paciente.nome}
						</Text>
				...
				<View style={styles.section}>
					<View style={styles.sectionHeader}>
						<Text style={styles.sectionTitle}>
							Análise Narrativa e Conduta Clínica
						</Text>
					</View>
					<View style={styles.narrativeBox}>
						<Text style={{ fontSize: 10, lineHeight: 1.6 }}>
							{data.narrativa_medica}
						</Text>
						...
						{data.evolucoes.slice(0, 5).map((ev, idx) => (
							<View key={idx} style={styles.timelineItem}>
								...
								<View style={styles.timelineContent}>
									<Text style={styles.soapTitle}>
										Sessão #{data.evolucoes.length - idx}
									</Text>
									<Text style={styles.soapText}>
										{formatClinicalText(ev.objetivo)}
									</Text>
									<View style={{ flexDirection: "row", gap: 10, marginTop: 4 }}>
										<Text
											style={{
				...
				<View style={styles.section}>
					<View style={styles.sectionHeader}>
						<Text style={styles.sectionTitle}>
							Métricas de Evolução Funcional
						</Text>
					</View>
				...
				<View style={styles.referenceBox}>
					<Text style={styles.referenceTitle}>
						Referências e Protocolos de Evidência
					</Text>
					{data.referencias.map((ref, idx) => (
						<View key={idx} style={{ marginBottom: 8 }}>
							<Text style={styles.referenceItem}>
								• {ref.autor} ({new Date().getFullYear()}). {ref.titulo}.{" "}
								{ref.periodico}.
							</Text>
