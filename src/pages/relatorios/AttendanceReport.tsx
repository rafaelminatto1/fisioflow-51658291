...
const getStatusBadge = (status: string) => {
  ...
  switch (s) {
    ...
    case "faltou_com_aviso":
      return (
        <Badge className="bg-teal-500/20 text-teal-700 border-teal-500/30">Faltou (Aviso)</Badge>
      );
    case "cancelado":
      return (
        <Badge className="bg-slate-500/20 text-slate-700 border-slate-500/30">Cancelado</Badge>
      );
    case "agendado":
    ...
    case "remarcar":
      return (
        <Badge className="bg-yellow-500/20 text-yellow-700 border-yellow-500/30">Remarcar</Badge>
      );
    default:
...
export default function AttendanceReport() {
  ...
  const handleExport = async () => {
    ...
    setIsExporting(true);
    try {
      const { exportAttendanceReport } = await import("@/lib/export/excelExport");
