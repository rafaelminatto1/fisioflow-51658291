...
export default function RelatorioMedicoPage() {
  ...
  const criarRelatorioPaciente = async (pacienteId: string, options?: { openEditor?: boolean }) => {
    ...
    const evolucoes = (sessionsRes.data ?? []).map((session, index) => ({
      ...
      descricao:
        session.assessment || session.plan || session.objective || session.subjective || "",
    }));

    const doctorName = (paciente.referring_doctor_name ?? paciente.referringDoctorName) as
      | string
      | undefined;
    const doctorPhone = (paciente.referring_doctor_phone ?? paciente.referringDoctorPhone) as
      | string
      | undefined;
    ...
    const relatorio: RelatorioMedicoData = {
      ...
      profissional_destino:
        doctorName || doctorPhone ? { nome: doctorName, telefone: doctorPhone } : {},
      clinica: {
        nome: org?.name || "",
        cnpj: org?.cnpj || "",
      ...
      urgencia: "baixa",
      patientId: pacienteId,
      relatorio_feito: (paciente.medical_report_done ?? paciente.medicalReportDone) as
        | boolean
        | undefined,
      relatorio_enviado: (paciente.medical_report_sent ?? paciente.medicalReportSent) as
        | boolean
        | undefined,
    };
