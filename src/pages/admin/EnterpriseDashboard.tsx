...
export default function EnterpriseDashboard() {
	...
	const { data: regionalData, isLoading } = useQuery({
		...
		queryFn: () =>
			request<{ data: RegionalMetric[] }>("/api/enterprise/regional-summary"),
	...
	if (isLoading) {
		return (
			<div className="h-screen flex items-center justify-center">
				Consolidando dados das filiais...
			</div>
		);
	}
	...
	const runRegionalAudit = async () => {
		setIsAuditing(true);
		try {
			const res = await request<{ data: { summary: string } }>(
				"/api/enterprise/regional-audit",
			);
	...
	return (
		<PageLayout>
			<PageContainer>
				<PageHeader title="Regional São Paulo" />
				<div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
							...
							<CardHeader className="pb-2">
								<div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400">
									...
									<CardTitle className="text-sm font-black uppercase tracking-widest">
										Relatório Estratégico Regional
									</CardTitle>
								</div>
							</CardHeader>
							<CardContent>
					...
					<div className="grid gap-6 md:grid-cols-3">
						<Card className="border-none shadow-premium bg-white dark:bg-slate-900/50">
							<CardContent className="p-6">
								<p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2">
									Faturamento Regional (Mês)
								</p>
								<h3 className="text-2xl font-black text-slate-900 dark:text-white">
									{formatCurrency(
										clinics.reduce((acc, c) => acc + Number(c.revenue || 0), 0),
									)}
								</h3>
							</CardContent>
						</Card>
						...
						<Card className="border-none shadow-premium bg-white dark:bg-slate-900/50">
							<CardContent className="p-6">
								...
								<h3 className="text-2xl font-black text-slate-900 dark:text-white">
									{clinics.reduce(
										(acc, c) => acc + Number(c.appointments_today || 0),
										0,
									)}{" "}
									sessões
								</h3>
					...
					<div className="grid gap-6">
						<h2 className="text-xl font-black text-slate-900 dark:text-white flex items-center gap-2">
							<Building2 className="h-5 w-5 text-brand-blue" /> Desempenho por
							Unidade
						</h2>
						...
						<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
							{clinics.map((clinic) => (
								<Card
									...
									className="border-none shadow-premium hover:shadow-xl transition-all group overflow-hidden"
								>
									...
									<CardContent className="space-y-4">
										<div className="flex justify-between items-center">
											...
											<span className="font-black text-emerald-600">
												{formatCurrency(clinic.revenue || 0)}
											</span>
										</div>
										<div className="flex justify-between items-center">
											<span className="text-xs text-slate-500 font-medium">
											...
											<span className="font-bold text-slate-700 dark:text-slate-300">
												{clinic.appointments_today} slots
											</span>
										</div>
