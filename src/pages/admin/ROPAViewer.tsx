...
export default function ROPAViewer() {
	...
	const handleTriggerArchive = async () => {
		...
		try {
			...
			if (r.status === "success") {
				...
				toast.error(
					`Pipeline falhou: ${r.errorMessage ?? "erro desconhecido"}`,
				);
			}
		} catch (e) {
	...
	const filtered = useMemo(() => {
		...
		return rows.filter((r) => {
			if (areaFilter !== "all" && r.area_responsavel !== areaFilter)
				return false;
			if (!ql) return true;
			...
			<div className="max-w-7xl mx-auto space-y-6">
				<header className="flex items-start justify-between gap-4">
					<div>
						...
						<p className="text-sm text-slate-500 mt-2">
							LGPD art. 37 · Activity Fisioterapia · {rows.length} atividades
							mapeadas
						</p>
					</div>
					<div className="flex flex-col sm:flex-row gap-2">
						<Button asChild variant="outline">
							<a
								href="/ropa-fisioflow.csv"
								download="ropa-activity-fisioterapia.csv"
							>
				...
				<Card>
					<CardHeader className="pb-3">
						...
						<CardDescription>
							Inventário de todas as atividades da clínica que processam dados
							pessoais. Exigido pela LGPD art. 37 e modelo aderente à Nota
							Técnica ANPD 33/2022. Importe o CSV no Google Sheets para edição
							colaborativa. Revise a cada 6-12 meses.
						</CardDescription>
					</CardHeader>
				</Card>
