...
const TARGET_MODES: { value: TargetMode; label: string; description: string }[] = [
    { value: 'CUT_OFF', label: 'Ponto de Corte', description: 'Valor fixo a ser atingido (ex: Score > 80).' },
    { value: 'IMPROVEMENT_ABS', label: 'Melhora Absoluta', description: 'Ganho bruto em relação à primeira avaliação.' },
    { value: 'IMPROVEMENT_PCT', label: 'Melhora Percentual', description: 'Ganho em % (ex: melhorar 20% a força).' },
    { value: 'RANGE', label: 'Faixa de Valores', description: 'Manter métrica dentro de um intervalo (mín/máx).' },
    { value: 'CUSTOM', label: 'Personalizado', description: 'Critério manual definido pelo fisioterapeuta.' },
];
...
export default function GoalProfileEditorPage() {
    ...
    const updateProfileMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            toast({ title: "Configurações Salvas", description: "O template foi atualizado com sucesso.", variant: "default" });
        },
        onError: (error: any) => {
        ...
        return (
            <MainLayout>
                <div className="flex h-[80vh] items-center justify-center flex-col gap-6 bg-[#f8faff]">
                    ...
                    <p className="text-slate-500 font-medium animate-pulse">Carregando evidências clínicas...</p>
                </div>
            </MainLayout>
        );
    }
                ...
                <div className="container p-6 flex justify-center bg-[#f8faff] min-h-screen">
                    <Card className="max-w-md w-full border-red-100 shadow-xl shadow-red-900/5 rounded-2xl overflow-hidden">
                        ...
                        <CardHeader className="text-center">
                            ...
                            <CardDescription>Não foi possível localizar este template de metas.</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            ...
                            <Button onClick={() => navigate('/admin/goals')} variant="outline" className="w-full h-11 rounded-xl">
        ...
        <MainLayout>
            <div className="container mx-auto p-6 space-y-8 max-w-6xl bg-[#f8faff] min-h-screen pb-24">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <div className="flex items-center gap-3">
                                <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">{formData.name || 'Novo Perfil'}</h1>
                                {profile?.status && (
                                    <Badge className={profile.status === 'PUBLISHED' ? 'bg-emerald-100 text-emerald-800 border-emerald-200' : 'bg-amber-100 text-amber-800 border-amber-200'}>
                                        {profile.status === 'PUBLISHED' ? 'Publicado' : 'Rascunho'}
                                    </Badge>
                                )}
                            </div>
                            <div className="flex items-center gap-2 mt-1">
                                <span className="text-xs font-mono text-slate-400 uppercase tracking-widest">{id}</span>
                    ...
                    <div className="flex items-center gap-3">
                        <Button variant="ghost" className="text-slate-600 hidden sm:flex" onClick={() => navigate('/admin/goals')}>
                            Descartar
                        </Button>
                        <Button 
                            onClick={handleSave} 
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                            ...
                            <p className="text-xs text-slate-500 leading-relaxed">
                                Para metas de retorno ao esporte (RTS), combine sempre métricas de <strong>percepção subjetiva</strong> (PROMs) com <strong>biomecânica</strong>.
                            </p>
                    ...
                    <div className="lg:col-span-3 space-y-6">
                        {activeTab === 'targets' && (
                            <div className="space-y-6">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <h2 className="text-xl font-bold text-slate-900">Configuração de Alvos Clínicos</h2>
                                        <p className="text-sm text-slate-500">Defina quais métricas o sistema deve monitorar para este template.</p>
                                    </div>
                                    <Button variant="outline" className="rounded-xl border-blue-200 text-blue-600 hover:bg-blue-50 bg-white" onClick={addTarget}>
                                ...
                                {formData.targets?.length === 0 ? (
                                    <Card className="border-dashed border-2 border-slate-200 bg-slate-50/50 rounded-2xl py-16 text-center">
                                        <div className="flex flex-col items-center gap-4">
                                            ...
                                            <div>
                                                <p className="font-bold text-slate-800">Nenhuma meta configurada</p>
                                                <p className="text-sm text-slate-500 max-w-xs mx-auto">Adicione métricas para que o sistema possa analisar automaticamente o progresso do paciente.</p>
                                            </div>
                                            <Button variant="default" className="bg-blue-600 rounded-xl" onClick={addTarget}>Começar Agora</Button>
                                        </div>
                                    </Card>
                                ) : (
                                    <div className="space-y-4">
                                        {formData.targets?.map((target, index) => (
                                            <Card key={index} className="border-slate-100 shadow-sm hover:shadow-md hover:border-blue-100 transition-all rounded-2xl overflow-hidden group">
                                                <div className="flex">
                                                    {/* Sort Handle Area */}
                                                    <div className="w-12 bg-slate-50/50 flex flex-col items-center justify-center border-r border-slate-50 gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <Button variant="ghost" size="icon" className="h-6 w-6 text-slate-400" disabled={index === 0} onClick={() => moveTarget(index, 'up')}>
                                                            <ArrowUp className="h-4 w-4" />
                                                        </Button>
                                                        <Button variant="ghost" size="icon" className="h-6 w-6 text-slate-400" disabled={index === (formData.targets?.length || 0) - 1} onClick={() => moveTarget(index, 'down')}>
                                                    ...
                                                    <div className="flex-1 p-5 space-y-4">
                                                        <div className="flex justify-between items-start">
                                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1 mr-4">
                                                                <div className="space-y-1.5">
                                                                    <Label className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Métrica Monitorada</Label>
                                                                    <Select value={target.key} onValueChange={(val) => updateTarget(index, 'key', val)}>
                                                                            ...
                                                                            {METRIC_OPTIONS.map(opt => (
                                                                                <SelectItem key={opt.value} value={opt.value}>
                                                                                    <div className="flex flex-col">
                                                                                        ...
                                                                                        <span className="text-[10px] text-slate-400">{opt.group} — {opt.value}</span>
                                                                                    </div>
                                                                                </SelectItem>
                                                                            ))}
                                                                        </SelectContent>
                                                                ...
                                                                <div className="space-y-1.5">
                                                                    <Label className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Modo de Avaliação</Label>
                                                                    <Select value={target.mode} onValueChange={(val) => updateTarget(index, 'mode', val as TargetMode)}>
                                                                        <SelectTrigger className="rounded-lg h-10 border-slate-200 focus:ring-blue-100">
                                                                            <SelectValue />
                                                                        </SelectTrigger>
                                                                        <SelectContent>
                                                        ...
                                                        <div className="bg-blue-50/50 border border-blue-100 p-4 rounded-xl grid grid-cols-2 md:grid-cols-4 gap-4 items-end">
                                                            {(target.mode === 'CUT_OFF' || target.mode === 'RANGE') && (
                                                                <div className="space-y-1.5">
                                                                    <Label className="text-xs font-bold text-blue-700">Valor Mínimo</Label>
                                                                    <Input
                                                                        type="number"
                                                                        value={target.min ?? ''}
                                                            ...
                                                            {(target.mode === 'RANGE') && (
                                                                <div className="space-y-1.5">
                                                                    <Label className="text-xs font-bold text-blue-700">Valor Máximo</Label>
                                                                    <Input
                                                                        type="number"
                                                                        value={target.max ?? ''}
                                                            ...
                                                            {(target.mode === 'IMPROVEMENT_ABS') && (
                                                                <div className="space-y-1.5">
                                                                    <Label className="text-xs font-bold text-blue-700">Delta Mínimo</Label>
                                                                    <Input
                                                                        type="number"
                                                            ...
                                                            {(target.mode === 'IMPROVEMENT_PCT') && (
                                                                <div className="space-y-1.5">
                                                                    <Label className="text-xs font-bold text-blue-700">Melhora Mínima (%)</Label>
                                                                    <div className="relative">
                                                                        <Input
                                                                            type="number"
                                                                        ...
                                                                        />
                                                                        <span className="absolute right-2 top-2 text-xs font-bold text-blue-300">%</span>
                                                                    </div>
                                                                </div>
                                                            )}

                                                            <div className="col-span-2 space-y-1.5">
                                                                <Label className="text-xs font-bold text-slate-400 uppercase tracking-tighter">Observação Clínica para o Sistema</Label>
                                                                <Input
                                                                    value={target.notes || ''}
                        ...
                        {activeTab === 'metadata' && (
                            <Card className="border-slate-100 shadow-sm rounded-2xl overflow-hidden bg-white">
                                <CardHeader className="bg-slate-50/50 border-b border-slate-100 p-6">
                                    <CardTitle className="text-lg">Informações Descritivas</CardTitle>
                                    <CardDescription>Estes detalhes ajudam a identificar e selecionar o template no prontuário.</CardDescription>
                                </CardHeader>
                                <CardContent className="p-8 space-y-6">
                                    ...
                                    <div className="space-y-2">
                                        <Label className="font-bold text-slate-700">Descrição e Objetivo Clínico</Label>
                                        <Textarea
                        ...
                        {activeTab === 'json' && (
                            <Card className="border-slate-100 shadow-sm rounded-2xl overflow-hidden bg-slate-900">
                                <CardHeader className="bg-slate-800 border-b border-slate-700 p-6">
                                    <CardTitle className="text-lg text-slate-100">Estrutura Bruta de Dados (JSON)</CardTitle>
                                    <CardDescription className="text-slate-400">Modo de leitura técnica do template para auditoria.</CardDescription>
                                </CardHeader>
                                <CardContent className="p-0">
                                    <div className="p-6">
