...
const ServerSideVideoAnalyzer = lazy(() =>
	import("@/components/analysis/video/ServerSideVideoAnalyzer").then(
		(module) => ({
...
export default function BiomechanicsAnalysisPage() {
	const [activeView, setActiveTab] = useState<
		"hub" | "instructions" | "library"
	>("hub");
	...
	const categories = [
		{
			...
			description:
				"Cadência, tempo de contato, corrida em esteira e comparação frame a frame.",
		...
		},
		{
			...
			description:
				"Altura de salto, tempo de voo, potência e comparação entre tentativas.",
		...
		},
		{
			...
			description:
				"Avaliação fotogramétrica e ângulo de rotação de tronco (ATR).",
		...
		},
		{
			...
			description:
				"Agachamento, gesto esportivo e testes funcionais com checkpoints e overlays.",
			...
			<div className="space-y-4">
				<div className="flex items-center justify-between">
					...
					<Button
						...
						size="sm"
						className="gap-2"
						onClick={() => navigate("/clinical/biomechanics/gait")}
					>
					...
					<div className="space-y-6 animate-in fade-in duration-500">
						<div className="flex items-center justify-between">
							<h2 className="text-lg font-bold flex items-center gap-2 uppercase tracking-tighter">
								<History className="h-5 w-5 text-primary" /> Biblioteca de
								Análises
							</h2>
							<Button
								variant="default"
						...
						<p className="text-[10px] text-muted-foreground text-center">
							Vídeos são salvos no R2 (media.moocafisio.com.br) e analisados
							pelo Gemini 1.5 Pro
						</p>
					</div>
				);
