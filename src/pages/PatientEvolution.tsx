...
const PatientEvolution = () => {
  ...
  useEffect(() => {
    ...
    if (!saved) {
      ...
      const currentIsEmpty = !(
        ...
        (state.evolutionV2Data.evolutionText || state.evolutionV2Data.observations || '').trim()
          .length > 0
      );
        ...
        try {
          ...
          if (altRaw) {
            ...
            if (
              altEnv &&
              altEnv.version === 1 &&
              Date.now() - altEnv.savedAt <= 1000 * 60 * 60 * 24 * 14
            ) {
      ...
      try {
        ...
        if (record?.id) {
          ...
          queryClient.setQueryData(
            ['evolution-records', 'drafts', state.patientId, 'byAppointment', state.appointmentId],
            record
          );
          draft.clearDraft();
        }
        ...
        if (err?.status === 409 && err?.payload?.error === 'conflict') {
          ...
          setConflict({
            message: err.payload.message ?? 'Esta evolução foi editada em outro dispositivo.',
            current: serverCurrent,
          });
  ...
  const pendingRequiredMeasurements =
    state.requiredMeasurements?.filter(
      (rm: any) => !state.measurements?.some((m: any) => m.measurement_name === rm.measurement_name)
    ) || [];
        ...
        <div className="flex flex-col h-full w-full bg-background overflow-hidden animate-fade-in">
              ...
              <TabsContent value="assistente">
                <Suspense fallback={<LoadingSkeleton />}>
                  <LazyAssistenteTab
                    ...
                    patientPhone={PatientHelpers.getPhone(state.patient as any)}
                    currentObservation={stripHtml(state.evolutionData.observacao || '')}
                    onApplyToSoap={(field, content) => {
                      const sectionTitle =
                        field === 'plan' ? 'Plano sugerido pela IA' : 'Análise da IA';
          ...
          <AIScribeModal
            ...
            onApply={(soap) => {
              ...
              state.setEvolutionData((prev) => ({
                ...prev,
                observacao: (prev.observacao || '') + html,
              }));
            }}
          />
          ...
          {state.patientId ? (
            <>
              <MedicalReturnFormModal
                ...
                patientId={state.patientId}
                patientName={PatientHelpers.getName(state.patient as any)}
                patientGender={(state.patient as any)?.gender ?? null}
