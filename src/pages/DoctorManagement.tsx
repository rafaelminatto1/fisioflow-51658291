        ...
        <div className="space-y-6">
            ...
            {isLoading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {[1, 2, 3].map((i) => (
                        <Card key={i} className="h-[180px]"><CardContent className="p-6 space-y-4"><Skeleton className="h-4 w-2/3" /><Skeleton className="h-4 w-1/2" /><Skeleton className="h-10 w-full" /></CardContent></Card>
                    ))}
                </div>
                            ...
                            <CardHeader className="p-4 pb-2">
                                <div className="flex items-start justify-between gap-2">
                                    <div className="min-w-0">
                                        ...
                                        <CardDescription className="text-xs truncate">{doctor.specialty || 'Sem especialidade'}</CardDescription>
                                    </div>
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end">
                                            <DropdownMenuItem onClick={() => handleEdit(doctor)}><Edit className="h-4 w-4 mr-2" />Editar</DropdownMenuItem>
                                            <DropdownMenuItem onClick={() => handleDelete(doctor)} className="text-destructive"><Trash2 className="h-4 w-4 mr-2" />Remover</DropdownMenuItem>
                                        </DropdownMenuContent>
                                    </DropdownMenu>
                                </div>
                            </CardHeader>
                            <CardContent className="p-4 pt-0 space-y-2">
                                {doctor.crm && <Badge variant="secondary" className="text-[10px]">CRM {doctor.crm}</Badge>}
                                {doctor.phone && <div className="flex items-center gap-2 text-xs text-muted-foreground"><Phone className="h-3 w-3" />{doctor.phone}</div>}
                                {doctor.clinic_name && <div className="flex items-center gap-2 text-xs text-muted-foreground"><MapPin className="h-3 w-3" />{doctor.clinic_name}</div>}
                            </CardContent>
                        </Card>
                    ))}
                </div>
