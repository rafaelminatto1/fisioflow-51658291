...
export default function OrganizationSettings() {
  const { currentOrganization, isCurrentOrgLoading, updateOrganization, isUpdating } =
    useOrganizations();
  const {
    members,
    ...
    <PageLayout>
      <PageContainer>
        <PageHeader
          ...
          subtitle={<>{currentOrganization?.name ?? "Gerenciar configurações da clínica"}</>}
        />
          ...
          <DialogContent>
            ...
            <DialogFooter>
              ...
              <Button
                ...
                disabled={isAdding || (!inviteUserId.trim() && !inviteEmail.trim())}
                className="gap-2"
              >
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                <strong>{removeTarget?.user?.name ?? removeTarget?.user_id}</strong> da equipe? Esta
