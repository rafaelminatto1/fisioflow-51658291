...
export function DocumentSigning() {
	...
	useEffect(() => {
		...
		fetch(
			`${API_BASE}/api/document-signatures/sign/${encodeURIComponent(token)}`,
		)
				...
				<CardContent className="pt-8 pb-8 flex flex-col items-center text-center gap-6">
						...
						<>
							...
							<label className="flex items-start gap-3 text-left cursor-pointer">
								...
								<span className="text-sm text-gray-600">
									Li e concordo com o conteúdo do documento acima. Entendo que
									esta assinatura eletrônica tem validade legal conforme a MP
									2.200-2/2001 e a Lei 14.063/2020.
								</span>
							</label>
					...
					{state === "signed" && (
						<div className="flex flex-col items-center gap-3">
							...
							<p className="text-xs text-muted-foreground text-center mt-2">
								Guarde esta confirmação. O fisioterapeuta responsável já foi
								notificado.
							</p>
						</div>
					)}
					...
					{state === "error" && (
						<div className="flex flex-col items-center gap-3">
							...
							<p className="text-xs text-muted-foreground">
								Por favor, contate o fisioterapeuta.
							</p>
						</div>
					)}
				</CardContent>
