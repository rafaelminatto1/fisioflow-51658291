...
type Status = "loading" | "ready" | "already" | "expired" | "submitting" | "submitted" | "error";
  ...
  return (
    <button
      ...
      className={`h-12 w-12 rounded-lg text-white font-bold tabular-nums transition-all ${
        selected ? `${tone} ring-2 ring-offset-2 ring-slate-900 scale-110` : `${tone} opacity-70 hover:opacity-100`
      }`}
    >
      {value}
    </button>
...
export default function NpsPublic() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrorMsg((err as Error).message || "Não foi possível registrar sua resposta.");
      setStatus("error");
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-emerald-50 p-4">
      <div className="w-full max-w-xl rounded-3xl bg-white border border-slate-200 shadow-xl p-8 space-y-6">
        <div className="flex items-center gap-2 text-emerald-600">
          ...
          <span className="text-sm font-medium uppercase tracking-wider">Pesquisa NPS</span>
        </div>
          ...
          <>
            <div className="space-y-2">
              ...
              <p className="text-slate-600">
                Em uma escala de <strong>0 a 10</strong>, o quanto você indicaria a nossa
                clínica para um amigo ou familiar?
              </p>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-700">
                Quer deixar um comentário? <span className="text-muted-foreground">(opcional)</span>
              </label>
              <Textarea
                value={comentario}
        ...
        {status === "already" && (
          <div className="text-center space-y-3 py-4">
            ...
            <h2 className="text-lg font-bold">Você já respondeu esta pesquisa</h2>
        ...
        {status === "expired" && (
          <div className="text-center space-y-3 py-4">
            ...
            <p className="text-slate-600">
              O prazo para responder esta pesquisa terminou. Obrigado pelo interesse.
            </p>
          </div>
        )}
