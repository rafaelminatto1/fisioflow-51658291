...
const typeIcons: Record<Notification["type"], React.ReactNode> = {
  ...
  waitlist: <Users className="h-5 w-5 text-cyan-500" />,
};
...
const typeColors: Record<Notification["type"], string> = {
  ...
  waitlist: "bg-cyan-500/10 border-cyan-500/20",
};
    ...
    >
      <CardContent className="p-4">
        <div className="flex items-start gap-4">
          <div className="mt-1 shrink-0">{typeIcons[notification.type]}</div>
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className={cn("font-semibold", !notification.is_read && "text-foreground")}>
                  {notification.title}
                </p>
...
const EmptyState: React.FC<{ message?: string }> = ({ message = "Nenhuma notificação" }) => (
