  ...
  return (
    <MainLayout>
      <div className="container mx-auto p-4 sm:p-6 space-y-4 sm:space-y-6 animate-fade-in">
        {evento && <EditEventoModal open={editOpen} onOpenChange={setEditOpen} evento={evento} />}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <EventoFinancialReportButton eventoId={id!} />
            <SaveAsTemplateButton eventoId={id!} eventoNome={evento.nome} />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-sm">{evento.participantes_previstos || 0} pessoas</p>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="flex items-center gap-2">
                <p className="text-sm font-bold">
                  {evento.colaboradores_confirmados || 0} / {evento.minimo_colaboradores || 0}
                </p>
                {evento.minimo_colaboradores > 0 && (
                  <Badge
                    className={cn(
                      "text-[10px] font-bold",
                      (evento.colaboradores_confirmados || 0) / evento.minimo_colaboradores >= 1
                        ? "bg-green-500"
                        : (evento.colaboradores_confirmados || 0) / evento.minimo_colaboradores >=
                            0.5
                          ? "bg-yellow-500"
                          : "bg-red-500",
                    )}
                  >
                    {Math.round(
                      ((evento.colaboradores_confirmados || 0) / evento.minimo_colaboradores) * 100,
                    )}
                    %
                  </Badge>
                )}
        ...
        <Tabs defaultValue="colaboradores" className="w-full">
          <TabsList className="grid w-full grid-cols-2 sm:grid-cols-4 h-auto gap-1">
            ...
            <TabsTrigger value="financeiro" className="text-sm">
              Financeiro
            </TabsTrigger>
          </TabsList>
