...
export default function Protocols() {
	...
	async function searchFB() {
		...
		try {
			...
			const res = await fetch(
				`${getWorkersApiUrl()}/api/fisiobrain/search?${params}`,
			);
	...
	const handleLoadSeedProtocols = () => {
		...
		SEED_PROTOCOLS_DATA.forEach((seed) => {
			createProtocol(
				seed as Omit<ExerciseProtocol, "id" | "created_at" | "updated_at">,
			);
		});
		toast({
											...
											return (
												...
												>
													<div className="flex items-start gap-1 flex-wrap">
														<Badge
															className={cn("text-[10px] px-1 py-0", badge.cls)}
														>
															{badge.label}
														</Badge>
														{hasArticle && (
					...
					<AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
						<AlertDialogContent>
							<AlertDialogHeader>
								...
								<AlertDialogDescription>
									Tem certeza que deseja excluir este protocolo? Esta ação não
									pode ser desfeita.
								</AlertDialogDescription>
							</AlertDialogHeader>
							<AlertDialogFooter>
		...
		<PageLayout>
			<PageContainer>
				<PageHeader
					...
					subtitle="Gerencie e aplique protocolos de reabilitação baseados em evidência"
					actions={
						<>
							...
							{quickTemplateCounts.map((template) => {
								...
								return (
									<Card
										...
										onClick={() =>
											setActiveTemplate(isActive ? null : template.name)
										}
									>
										<div
								...
								<div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
									{PROTOCOL_CATEGORIES.map((cat) => (
										<Button
											...
											onClick={() =>
												setCategoryFilter(
													cat.id === categoryFilter ? "all" : cat.id,
												)
											}
										>
								...
								<div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
									...
									{MUSCULATURE_FILTERS.map((muscle) => (
										<Button
											key={muscle.id}
											variant={
												muscleFilter === muscle.id ? "default" : "outline"
											}
											size="sm"
											className={cn(
											...
											onClick={() =>
												setMuscleFilter(
													muscle.id === muscleFilter ? "all" : muscle.id,
												)
											}
										>
											{muscle.label}
							...
							{error ? (
								...
								>
									{[1, 2, 3, 4, 5, 6].map((i) => (
										<Card
											key={i}
											className="p-4 h-[200px] flex flex-col justify-between"
										>
								...
								<div className="text-center py-16 bg-muted/20 rounded-xl border border-dashed animate-fade-in">
									...
									<div className="flex flex-wrap gap-2 justify-center">
										...
										{protocols.length === 0 && (
											...
											>
												{isSeeding || isCreating
													? "Carregando..."
													: "Carregar protocolos de exemplo"}
											</Button>
										)}
									</div>
								</div>
