          ...
          <div className="flex items-center justify-between text-[11px]">
            <span className="text-muted-foreground">
              ...
              {cls.waitlist_count > 0 && (
                <span className="text-amber-600 ml-1">+{cls.waitlist_count} fila</span>
              )}
            </span>
            <span className={cn("font-bold", isFull ? "text-red-500" : "text-emerald-500")}>
...
function ClassFormModal({
  ...
  const [maxCapacity, setMaxCapacity] = useState(String(initial?.max_capacity ?? 10));
  const [duration, setDuration] = useState(String(initial?.duration_minutes ?? 60));
  const [color, setColor] = useState(initial?.color ?? "#6366f1");
  const [isActive, setIsActive] = useState(initial?.is_active ?? true);
  const [schedules, setSchedules] = useState<{ weekday: number; start_time: string }[]>(
...
function CheckinModal({
  ...
  const addCheckin = useMutation({
    mutationFn: (patientId: string) => groupsApi.checkins.add(session.id, patientId),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["group-checkins", session.id] }),
  });
  const removeCheckin = useMutation({
    mutationFn: (patientId: string) => groupsApi.checkins.remove(session.id, patientId),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["group-checkins", session.id] }),
  });
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Check-in — {new Date(session.date + "T12:00:00").toLocaleDateString("pt-BR")}
          </DialogTitle>
        </DialogHeader>
        <div className="py-1">
...
function ClassDetailPanel({ cls, onClose }: { cls: GroupClass; onClose: () => void }) {
  ...
  const unenroll = useMutation({
    mutationFn: (patientId: string) => groupsApi.enrollments.unenroll(cls.id, patientId),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["group-enrollments", cls.id] }),
  });
  ...
  return (
    <div
      className={cn(
        ...
        past ? "border-border/30 bg-muted/10 opacity-70" : "border-border/50 bg-card",
      )}
    >
      <div>
...
export default function GroupsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (body: Parameters<typeof groupsApi.create>[0]) => groupsApi.create(body),
    onSuccess: () => {
    ...
    <MainLayout>
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-black font-display">Gestão de Turmas</h1>
            <p className="text-sm text-muted-foreground mt-0.5">
              Pilates, Hidroterapia, Grupos de Fisioterapia e mais
            </p>
          </div>
