...
import {
	useAtestadoTemplates,
	useCreateAtestadoTemplate,
	useUpdateAtestadoTemplate,
	useDeleteAtestadoTemplate,
	AtestadoTemplate,
...
export default function AtestadosPage() {
	...
	const [selectedTemplateForPDF, setSelectedTemplateForPDF] =
		useState<AtestadoTemplate | null>(null);
	...
	const handleGeneratePDF = async () => {
		...
		if (!user?.professional?.name || !user?.clinic?.name) {
			toast({
				title: "Dados do profissional ou clínica não configurados.",
				variant: "destructive",
			});
			return;
		}
		...
		try {
			...
			if (blob) {
				downloadPDF(
					blob,
					`atestado-${patient.name?.replace(/\s+/g, "-")}-${Date.now()}.pdf`,
				);
				...
				<div className="flex items-center justify-between">
					<div>
						...
						<p className="text-muted-foreground">
							Gerencie templates de atestados com variáveis dinâmicas
						</p>
					</div>
				...
				{isLoading ? (
					...
					<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
						{templates.map((template) => (
							<Card
								key={template.id}
								className={!template.ativo ? "opacity-60" : ""}
							>
				...
				<Dialog open={isPDFDialogOpen} onOpenChange={setIsPDFDialogOpen}>
					<DialogContent className="max-w-md">
						<DialogHeader>
							...
							<DialogDescription>
								Preencha os dados para gerar o PDF do atestado "
								{selectedTemplateForPDF?.nome}"
							</DialogDescription>
						</DialogHeader>
						<div className="space-y-4 py-4">
							<div className="space-y-2">
								...
								<Select
									value={pdfForm.patientId}
									onValueChange={(value) =>
										setPdfForm((p) => ({ ...p, patientId: value }))
									}
								>
							...
							<div className="space-y-2">
								...
								<Input
									...
									value={pdfForm.city}
									onChange={(e) =>
										setPdfForm((p) => ({ ...p, city: e.target.value }))
									}
							...
							<div className="space-y-2">
								...
								<Textarea
									...
									onChange={(e) =>
										setPdfForm((p) => ({ ...p, reason: e.target.value }))
									}
							...
							<div className="space-y-2">
								...
								<Input
									...
									value={pdfForm.cid}
									onChange={(e) =>
										setPdfForm((p) => ({ ...p, cid: e.target.value }))
									}
						...
						<DialogFooter>
							...
							<Button
								...
								disabled={isGenerating || !pdfForm.patientId || !pdfForm.reason}
							>
								{isGenerating ? (
									<>
