...
interface BuilderQuestion
	extends Omit<AssessmentQuestion, "id" | "section_id"> {
	id: string; // Temporary ID for UI
}
...
const QUESTION_TYPES: {
...
}[] = [
	{
		...
		description: "Apenas uma linha de texto",
	},
	{
		type: "long_text",
		label: "Resposta Longa",
		icon: <AlignLeft className="w-4 h-4" />,
		description: "Permite escrever quanto for necessário",
	},
	{
		type: "scale",
		label: "Escala",
		icon: <List className="w-4 h-4" />,
		description: "Uma escala numrica ou visual",
	},
	{
		type: "single_choice",
		label: "Opo nica",
		icon: <CheckSquare className="w-4 h-4" />,
		description: "Apenas uma opo pode ser selecionada",
	},
	{
		type: "multiple_choice",
		label: "Seleo Mltipla",
		icon: <CheckSquare className="w-4 h-4" />,
		description: "Vrias opes podem ser selecionadas",
	},
	{
		type: "body_map",
		label: "Mapa Corporal",
		icon: <User className="w-4 h-4" />,
		description: "Identificao de dor em imagem",
	},
];
		...
		<PageLayout>
			<PageContainer>
				<PageHeader
					...
					subtitle="Arraste os campos para construir seu formulário."
					actions={
						<>
							<Button
								variant="outline"
								onClick={() =>
									setActiveTab(activeTab === "build" ? "preview" : "build")
								}
							>
									...
									{sections.length === 0 && (
										<div className="flex flex-col items-center justify-center p-12 border-2 border-dashed rounded-lg bg-muted/20 text-muted-foreground">
											...
											<p className="text-lg font-medium">
												Comece adicionando um grupo ou clicando em um campo
											</p>
											<p className="text-sm mb-4">
												Grupos ajudam a organizar suas perguntas.
											</p>
											<Button
												onClick={addSection}
												variant="outline"
											...
											>
														...
														{(provided) => (
															...
															>
																...
																>
																	{(provided) => (
																		<div
																			ref={provided.innerRef}
																			{...provided.droppableProps}
																			className="space-y-4 pl-4 border-l-2 border-muted min-h-[50px]"
																		>
																					...
																					>
																						{(provided) => (
																							...
																							>
																								<div
																									{...provided.dragHandleProps}
																									className="mt-2.5 cursor-move text-muted-foreground opacity-50 group-hover:opacity-100"
																								>
																...
																<div className="pl-4 pt-2">
																	<p
																		className="text-xs text-muted-foreground italic text-center py-2 border-2 border-dashed border-transparent hover:border-muted rounded transition-colors cursor-pointer"
																		onClick={() => {
																			addQuestionToSection(section.id, "text");
																		}}
																	>
																		Clique para adicionar campo de texto ou
																		selecione na lateral
																	</p>
																</div>
															</Card>
														)}
												...
												{sections.length > 0 && (
													<Button
														variant="outline"
														onClick={addSection}
														className="w-full py-8 border-dashed gap-2 text-muted-foreground hover:text-primary hover:border-primary/50"
													>
														<Plus className="w-4 h-4" />
														Adicionar Novo Grupo
													</Button>
							...
							<div className="space-y-8">
								{sections.map((section) => (
									<div key={section.id} className="space-y-4">
										{section.title && (
											<h3 className="text-lg font-semibold border-l-4 border-primary pl-3 bg-accent/20 py-1">
												{section.title}
											</h3>
										)}
