  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Fornecedores e Parceiros</h2>
          <p className="text-muted-foreground">Gerencie sua rede de fornecedores e prestadores de serviço</p>
        </div>
                    ...
                    <TableCell>
                      <div>
                        ...
                        <div className="flex gap-2 mt-1">
                          <Badge variant="outline" className="text-[10px]">{fornecedor.tipo_pessoa === 'pj' ? 'PJ' : 'PF'}</Badge>
                          {fornecedor.cpf_cnpj && <span className="text-[10px] text-muted-foreground">{fornecedor.cpf_cnpj}</span>}
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="text-xs space-y-0.5">
                        {fornecedor.telefone && <div className="flex items-center gap-1"><Phone className="h-3 w-3" />{fornecedor.telefone}</div>}
                        {fornecedor.email && <div className="flex items-center gap-1 text-muted-foreground"><Mail className="h-3 w-3" />{fornecedor.email}</div>}
                      </div>
                    </TableCell>
                    <TableCell>
                      {fornecedor.categoria && <Badge variant="secondary" className="text-[10px]">{fornecedor.categoria}</Badge>}
                    </TableCell>
                    <TableCell>
                      <Badge variant={fornecedor.ativo ? "default" : "secondary"} className="text-[10px]">
                        {fornecedor.ativo ? 'Ativo' : 'Inativo'}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-1">
                        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => openEditModal(fornecedor)}><Edit className="h-4 w-4" /></Button>
                        <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => handleDelete(fornecedor.id)}><Trash2 className="h-4 w-4" /></Button>
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select value={watch('tipo_pessoa')} onValueChange={(v) => setValue('tipo_pessoa', v as 'pj' | 'pf')}>
