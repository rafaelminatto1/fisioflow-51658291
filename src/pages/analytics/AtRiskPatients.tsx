...
export default function AtRiskPatientsPage() {
	...
	const minRiskScore =
		daysFilter >= 90 ? 80 : daysFilter >= 60 ? 60 : daysFilter >= 30 ? 30 : 10;
	const { data: retentionAtRisk = [], isLoading } =
		usePatientsAtRisk(minRiskScore);

	const atRiskPatients = useMemo<AtRiskPatient[]>(() => {
		return retentionAtRisk
			.map((patient) => {
				let riskLevel: AtRiskPatient["risk_level"] = "low";
				if (patient.daysSinceLastSession >= 90 || patient.riskScore >= 80)
					riskLevel = "critical";
				else if (patient.daysSinceLastSession >= 60 || patient.riskScore >= 60)
					riskLevel = "high";
				else if (patient.daysSinceLastSession >= 30 || patient.riskScore >= 30)
					riskLevel = "medium";
				...
				return {
					...
					last_appointment_date:
						patient.lastAppointmentDate || new Date().toISOString(),
					last_activity_date:
						patient.lastAppointmentDate || new Date().toISOString(),
	...
	const handleExport = () => {
		const csv = [
			...
			...filteredPatients.map((p) => [
				...
				p.email || "",
				p.phone || "",
				format(new Date(p.last_appointment_date), "dd/MM/yyyy", {
					locale: ptBR,
				}),
				p.days_inactive,
		...
		<PageLayout>
			<PageContainer>
				<PageHeader
					...
					subtitle="Identifique e reative pacientes que não retornam há muito tempo"
					actions={
						<>
							<Button
				...
				<div className="container max-w-7xl mx-auto py-6 md:py-10 space-y-8 px-4 sm:px-6">
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
						...
						<Card
							className={cn("rounded-xl border", riskConfig.critical.border)}
						>
							<CardHeader className="pb-2">
								<CardTitle
									className={cn(
										"text-sm font-medium flex items-center gap-2",
										riskConfig.critical.color,
									)}
								>
						...
						<Card className={cn("rounded-xl border", riskConfig.high.border)}>
							...
							<CardContent>
								<p className={cn("text-3xl font-bold", riskConfig.high.color)}>
									{stats.high}
								</p>
						...
						<Card className={cn("rounded-xl border", riskConfig.medium.border)}>
							...
							<CardContent>
								<p
									className={cn("text-3xl font-bold", riskConfig.medium.color)}
								>
									{stats.medium}
								</p>
						...
						<Card className={cn("rounded-xl border", riskConfig.low.border)}>
							...
							<CardContent>
								<p className={cn("text-3xl font-bold", riskConfig.low.color)}>
									{stats.low}
								</p>
					...
					<Card className="rounded-xl">
						<CardContent className="p-4">
							<div className="flex flex-col md:flex-row gap-4">
								...
								<Select
									value={riskFilter}
									onValueChange={(v: unknown) => setRiskFilter(v)}
								>
							...
							{filteredPatients.map((patient) => {
								...
								return (
									<Card
										key={patient.id}
										className={cn(
											"rounded-xl border hover:shadow-md transition-all",
											config.border,
										)}
									>
										<CardContent className="p-4">
													...
													<div className="flex-1 min-w-0">
														...
														<div className="flex items-center gap-4 mt-1 text-sm text-muted-foreground">
															<span className="flex items-center gap-1">
																...
																{format(
																	new Date(patient.last_appointment_date),
																	"dd/MM/yyyy",
																	{
																		locale: ptBR,
																	},
