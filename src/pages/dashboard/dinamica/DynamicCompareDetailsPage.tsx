...
import MetricsTrendChart, {
	TrendSeriesPoint,
} from "@/components/metrics/MetricsTrendChart";
...
const fetchCompareData = async (
	...
	return {
		...
		trial_B: {
			label: "Post-Op (6 weeks)",
			captured_at: "2025-02-15T10:00:00Z",
			source_asset_ids: ["b1"],
		},
		synchronization: {
			mode: "AUTO",
			anchor: {
				anchor_type: "Heel Strike",
		...
		fetchCompareData(id)
			.then((raw) => {
				try {
					...
					logger.error(
						"Error loading dynamic compare data",
						e,
	...
	const trendSeries: TrendSeriesPoint[] = [
		{
			...
			compare_id: "c1",
			test_type: "GAIT",
			values: { "gait.cadence": 98, "gait.velocity": 0.8 },
		},
		{
			date: "2025-01-15",
			compare_id: "c2",
			test_type: "GAIT",
			values: { "gait.cadence": 102, "gait.velocity": 0.9 },
		},
		{
			date: "2025-02-15",
			compare_id: "c3",
			test_type: "GAIT",
			values: { "gait.cadence": 108, "gait.velocity": 1.1 },
		},
	];

	// Extract available keys from current data for the dropdown
	const availableKeys = data.metric_deltas.map((d) => ({
		key: d.key,
		label: d.label,
		unit: d.unit || "",
	}));
			...
			<PageContainer>
					...
					subtitle={
						<>
							...
							<Badge
								variant="outline"
								className={
									data.quality.analysis_confidence_overall_0_100 > 80
				...
				<div className="container mx-auto p-6 space-y-6 max-w-7xl">
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
						...
						<Card>
							<CardHeader>
								<CardTitle className="text-sm">Sessão A (Base)</CardTitle>
							</CardHeader>
							<CardContent>
								<p className="font-medium">{data.trial_A.label}</p>
								<p className="text-xs text-muted-foreground">
									{new Date(data.trial_A.captured_at).toLocaleString()}
								</p>
							</CardContent>
						</Card>
						<Card>
							<CardHeader>
								<CardTitle className="text-sm">Sessão B (Atual)</CardTitle>
							</CardHeader>
							<CardContent>
								<p className="font-medium">{data.trial_B.label}</p>
								<p className="text-xs text-muted-foreground">
									{new Date(data.trial_B.captured_at).toLocaleString()}
								</p>
							</CardContent>
						</Card>
					</div>
							...
							<Card className="h-full">
								...
								<CardContent className="prose prose-sm dark:prose-invert">
									...
									<ul className="mt-1 mb-4">
										{data.summary.improvements?.map((im, i) => (
											<li key={i}>{im}</li>
										))}
									</ul>
