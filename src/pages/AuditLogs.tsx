...
export default function AuditLogs() {
	...
	return (
		<PageLayout
			...
			subtitle="Logs de auditoria, segurança e backups"
			actions={
					...
					>
						...
						<TabsContent value="audit" className="space-y-4">
							...
							<div className="flex flex-wrap gap-2 mb-2">
								...
								<Button
									variant={
										quickFilter === "failures" ? "destructive" : "outline"
									}
									size="sm"
							...
							<Card>
								<CardHeader>
									<CardTitle>
										Eventos (
										{logs.filter((l) => !l.action.startsWith("LOGIN_")).length})
									</CardTitle>
								</CardHeader>
								<CardContent>
									{isLoading ? (
										<LoadingSkeleton type="table" rows={8} />
									) : logs.filter((l) => !l.action.startsWith("LOGIN_"))
											.length === 0 ? (
										<EmptyState
											icon={FileText}
											...
											<div>
												<p className="text-sm font-medium">
													{backupStats.lastBackup
														? format(
																new Date(backupStats.lastBackup.created_at),
																"dd/MM HH:mm",
																{
																	locale: ptBR,
																},
