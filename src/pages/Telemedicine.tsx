...
import {
	...
	useCreateTelemedicineRoom,
} from "@/hooks/useTelemedicine";
				...
				<div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
					...
					<Card className="rounded-[32px] border-brand-blue/10 bg-card shadow-premium dark:bg-slate-950/50">
						<CardHeader className="p-8">
							...
							<CardDescription>
								Gerencie suas consultas online e envie os links para seus
								pacientes.
							</CardDescription>
						</CardHeader>
							...
							{roomsLoading ? (
								...
								<div className="text-center py-8 text-muted-foreground">
									...
									<p className="text-xs sm:text-sm">
										Nenhuma teleconsulta criada ainda
									</p>
									<Button
										variant="link"
										className="mt-2"
											...
											<div className="flex items-center gap-4 min-w-0 flex-1">
												...
												<div className="min-w-0 flex-1">
													<p className="font-bold text-slate-900 dark:text-white truncate">
														{(room.patients as { name: string } | null)?.name ||
															"Paciente"}
													</p>
											...
											<div className="flex items-center gap-2 shrink-0">
												...
												{room.status !== "encerrado" && (
													<Button
														size="sm"
														onClick={() =>
															navigate(`/telemedicine-room/${room.id}`)
														}
