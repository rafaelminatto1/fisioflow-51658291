...
export default function PainMapHistoryPage() {
	...
	const { data: historyData, isLoading: historyLoading } = usePainMapHistory(
		patientId || "",
	);
	...
	const exportPDF = async () => {
		...
		doc.text("Relatório de Evolução de Dor", pageWidth / 2, 20, {
			align: "center",
		});
		...
		if (historyData.painMaps.length > 0) {
			...
			autoTable(doc, {
				...
				body: historyData.painMaps.slice(0, 10).map((pm) => [
					format(new Date(pm.recorded_at), "dd/MM/yyyy HH:mm", {
						locale: ptBR,
					}),
		...
		doc.save(
			`fisioflow_mapa_dor_${patientName.replace(/\s/g, "_")}_${format(new Date(), "yyyyMMdd")}.pdf`,
		);
	};
		...
		<PageLayout>
			<PageContainer>
				<PageHeader
					...
					subtitle={<>{PatientHelpers.getName(patient)}</>}
					actions={
						<>
					...
					<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
						<div className="flex gap-2">
							<Button
								...
								disabled={!historyData || historyData.painMaps.length < 2}
							>
							...
							<Button
								onClick={exportPDF}
								disabled={!historyData || historyData.painMaps.length === 0}
							>
								...
								<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
										...
										<CardContent className="p-4">
											<div className="flex items-center gap-3">
												...
												<div>
													...
													<p className="text-2xl font-bold">
														{historyData.statistics.avgPainLevel}/10
													</p>
												</div>
											</div>
										</CardContent>
									...
									>
										<CardContent className="p-4">
											<div className="flex items-center gap-3">
												<div
													className={`p-2 rounded-lg ${historyData.statistics.painReduction > 0 ? "bg-green-500/10" : "bg-red-500/10"}`}
												>
													{historyData.statistics.painReduction > 0 ? (
												...
												<div>
													...
													<p className="text-2xl font-bold">
														{historyData.statistics.painReduction}%
													</p>
												</div>
											</div>
										</CardContent>
											...
											<ScrollArea className="h-[400px]">
												<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
													{historyData.painMaps.slice(0, 10).map((painMap) => {
														const points =
															painMap.pain_points as PainMapPoint[];
														const mostAffected =
															points.length > 0
																? points.sort(
																...
																<div className="flex gap-4">
																	...
																	<div className="flex-1 space-y-2">
																		<div className="flex items-center justify-between">
																			...
																			<Badge
																				variant={
																					painMap.global_pain_level <= 3
																						? "secondary"
																						: painMap.global_pain_level <= 6
																							? "default"
																							: "destructive"
																				}
																			>
