...
const PatientProfileContent = () => {
	...
	const [isMedicalReportOpen, setIsMedicalReportOpen] =
		useState<boolean>(false);
	const [generatedMedicalReport, setGeneratedMedicalReport] =
		useState<any>(null);
	...
	const handleExportPremium = async () => {
		...
		try {
				...
				<RelatorioPremiumPDF
					data={{
						clinica: {
							...
							endereco:
								"Rua Manuel Vieira de Sousa, 166 - Mooca, São Paulo - SP",
						...
						metricas: (evolutionReportData.measurementEvolution || []).map(
							(m: any) => ({
								nome: m.name,
			...
			saveAs(
				blob,
				`RELATORIO-PREMIUM-${patientNameStr.replace(/\s/g, "-")}.pdf`,
			);
	...
	useEffect(() => {
		if (
			patient &&
			(patient as { incomplete_registration?: boolean }).incomplete_registration
		) {
								...
								>
									...
									>
										<PatientAnalyticsTab
											...
											condition={
												(patient as any).main_condition || "Não informada"
											}
										/>
