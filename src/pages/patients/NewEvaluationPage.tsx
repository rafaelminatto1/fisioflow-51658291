...
import { appointmentsApi, evaluationFormsApi, goalsApi, patientsApi } from "@/api/v2";
...
export default function NewEvaluationPage() {
  ...
  const lastCompletedEvaluation = useMemo(() => {
    ...
    return allEvaluations
      .filter(
        (ev) =>
          ev.id !== evaluationId && ev.status === "completed" && ev.form_id === selectedTemplate.id,
      )
      .sort(
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const [patientRes, goalsRes, pathologiesRes, surgeriesRes, appointmentsRes] =
        await Promise.all([
  ...
  const isReadOnlyEvaluation = isViewMode && evaluationResponse?.status === "completed";
                      ...
                      {hasRedFlags && (
                        ...
                        >
                          ...
                          <AlertDescription className="text-sm mt-1">
                            Foram detectados possíveis sinais de alerta (Red Flags) baseados na
                            avaliação:
                            <span className="font-bold ml-1">{detectedFlags.join(", ")}</span>
                            .
                            <br />
                            Considere encaminhamento médico ou investigação aprofundada antes de
