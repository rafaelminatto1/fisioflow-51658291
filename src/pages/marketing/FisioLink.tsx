...
export default function FisioLinkPage() {
	...
	const getFisioLinkUrl = () =>
		`${window.location.origin}/l/${slug || organizationId}`;
		...
		<PageLayout>
			<PageContainer>
				<PageHeader
					...
					subtitle="Crie sua página personalizada para o link do Instagram"
					actions={
						<>
							<Button
								variant="outline"
						...
						{!previewMode && (
							<Card>
								<CardHeader>
									...
									<CardDescription>
										Personalize sua página FisioLink
									</CardDescription>
								</CardHeader>
									...
									<div className="space-y-3">
										...
										<div className="grid grid-cols-3 gap-2">
											{THEME_OPTIONS.map((theme) => (
												...
												>
													<div
														className={cn("h-8 rounded mb-2", theme.preview)}
													/>
										...
										<div className="flex flex-wrap gap-2">
											{COLOR_OPTIONS.map((color) => (
												<button
													key={color.value}
													onClick={() =>
														setConfig({ ...config, primary_color: color.value })
													}
													className={cn(
