...
export default function ContentGeneratorPage() {
	...
	const [contentType, setContentType] = useState<
		"educational" | "motivational" | "promotional" | "transformation"
	>("educational");
	...
	const handleAIGenerate = async () => {
		...
		try {
			...
			toast({
				...
				title: "Erro na IA",
				description:
					"Não foi possível gerar o conteúdo com IA. Usando template padrão.",
			});
			generateContent();
		} finally {
		...
		<PageLayout>
			<PageContainer>
				<PageHeader
					...
					subtitle="Crie posts otimizados para redes sociais"
				/>
								...
								<div className="grid grid-cols-2 gap-3">
									{PLATFORMS.map((p) => {
										...
										return (
											...
											>
												<div
													className={cn("p-2 rounded-lg text-white", p.color)}
												>
							...
							<div className="p-4 bg-muted/50 rounded-lg">
								<div className="flex items-center justify-between mb-2">
									<span className="text-sm font-medium">
										Template {templateIndex + 1} de {templates.length}
									</span>
							...
							<div className="flex items-center justify-between p-3 border rounded-lg">
								<div className="space-y-0.5">
									...
									<p className="text-xs text-muted-foreground">
										Adiciona emojis ao conteúdo gerado
									</p>
								</div>
								<Switch
									id="emojis"
							...
							<CardContent>
								...
								{isOverLimit && (
									<p className="text-sm text-destructive mt-2">
										⚠️ Conteúdo excede o limite de caracteres para{" "}
										{platform.name}
									</p>
								)}
							</CardContent>
