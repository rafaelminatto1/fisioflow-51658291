...
import {
	MessageSquare, Users, AlertTriangle, CheckCheck, Send, Loader2,
...
export default function ReactivationCampaign() {
	...
	const { data: patients = [], isLoading, refetch, isFetching } = useQuery({
					...
					<Card className="border shadow-sm">
						<CardContent className="p-4 flex items-center gap-3">
							...
							<div>
								...
								<p className="text-[10px] text-muted-foreground uppercase font-semibold">Em risco</p>
							</div>
						</CardContent>
					</Card>
					<Card className="border shadow-sm">
						<CardContent className="p-4 flex items-center gap-3">
							...
							<div>
								...
								<p className="text-[10px] text-muted-foreground uppercase font-semibold">Selecionados</p>
							</div>
						</CardContent>
					</Card>
					<Card className="border shadow-sm">
						<CardContent className="p-4 flex items-center gap-3">
							...
							<div>
								...
								<p className="text-[10px] text-muted-foreground uppercase font-semibold">Enviados</p>
							</div>
						</CardContent>
					</Card>
				</div>
				...
				<div className="grid lg:grid-cols-3 gap-4">
					...
					<div className="lg:col-span-2 space-y-3">
						<div className="flex items-center justify-between">
							...
							{patients.length > 0 && (
								<Button variant="ghost" size="sm" className="text-xs h-7 gap-1" onClick={handleSelectAll}>
												...
												<div className="flex-1 min-w-0">
													<div className="flex items-center gap-2">
														...
														{patient.missed_count >= 2 && !isSent && (
															<Badge variant="outline" className="rounded-lg text-[10px] border-red-200 text-red-600 shrink-0">
						...
						<Card className="border shadow-sm">
							...
							<CardContent className="px-4 pb-4 space-y-3">
								...
								>
									...
									Enviar para {unsentSelected.length || 0} paciente{unsentSelected.length !== 1 ? "s" : ""}
								</Button>
							</CardContent>
						</Card>
								...
								<CardContent className="px-4 pb-4 space-y-2">
									{campaigns.slice(0, 5).map((c: Record<string, unknown>) => (
										...
										>
											...
											<p className="text-[10px] text-muted-foreground truncate">{c.message_template as string}</p>
										</button>
									))}
								</CardContent>
							</Card>
