			...
			<CardContent>
				<div className="space-y-2">
								...
								{isEditing ? (
									<>
										...
										<Input
											...
											className="h-6 text-xs text-right px-1"
											onChange={(e) =>
												updateChannel(ch.id, "newPatients", e.target.value)
											}
										/>
										<button
					...
					<div className="grid grid-cols-[1fr_90px_90px_70px] gap-2 text-xs font-semibold border-t pt-2 px-1">
						...
						>
							{totalPatients > 0
								? `R$${(totalSpend / totalPatients).toFixed(0)}`
								: "-"}
						</span>
					</div>
				</div>
	...
	return (
		<PageLayout>
			<PageContainer>
				<PageHeader
					title="Marketing Hub"
					subtitle="Gestão centralizada de crescimento, autoridade e ROI"
					actions={
						<>
							<Button
								variant="outline"
				...
				<div className="w-full p-4 sm:p-6 space-y-6">
					<div className="gradient-brand-light rounded-3xl p-8 border border-primary/20 card-premium-hover"></div>
								...
								<CardContent className="space-y-4">
									<div className="p-4 rounded-lg bg-emerald-50 border border-emerald-100 dark:bg-emerald-950/20 dark:border-emerald-900">
										<p className="text-sm text-emerald-800 dark:text-emerald-300">
											Seu perfil local tem{" "}
											<strong>12% mais visualizações</strong> que a média da
											região. Recomendamos focar em{" "}
