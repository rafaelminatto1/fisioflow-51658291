...
export default function MarketingSettingsPage() {
	...
	const [birthdayConfig, setBirthdayConfig] =
		useState<BirthdayAutomationConfig>({
			organization_id: organizationId,
			enabled: false,
			message_template: "",
			send_whatsapp: true,
	...
	const handleGenerateReviewWithAI = async () => {
		...
		try {
			...
			setReviewConfig({
				...reviewConfig,
				message_template:
					"Olá {nome}! Gostaríamos de saber sua opinião sobre nosso atendimento: {review_link}",
			});
		} finally {
	...
	const handleGenerateBirthdayWithAI = async () => {
		...
		try {
			...
			setBirthdayConfig({
				...birthdayConfig,
				message_template:
					"Olá {nome}! Desejamos um feliz aniversário! 🎉 Que este novo ciclo traga muita saúde e felicidade.",
			});
		} finally {
	...
	const handleGenerateRecallWithAI = async () => {
		...
		try {
			...
			setNewCampaign({
				...newCampaign,
				message_template:
					"Olá {nome}! Já faz {dias} dias desde sua última visita. Que tal agendar um check-up?",
			});
		} finally {
		...
		<PageLayout>
			<PageContainer>
				<PageHeader
					...
					subtitle="Automatize suas ações de marketing e engajamento"
				/>
						...
						<TabsContent value="reviews" className="space-y-6">
							<Card>
								<CardHeader>
									...
									<CardDescription>
										Solicite avaliações automaticamente quando pacientes
										concluírem tratamento
									</CardDescription>
								</CardHeader>
									...
									<div className="flex items-center justify-between p-4 border rounded-lg">
										<div className="space-y-0.5">
											...
											<p className="text-sm text-muted-foreground">
												Envie mensagens automáticas solicitando avaliações do
												Google
											</p>
										</div>
										<Switch
										...
										<>
													...
													{["alta", "concluido", "melhorado"].map((status) => (
														<Badge
															...
															onClick={() => {
																const newStatuses =
																	reviewConfig.trigger_status?.includes(status)
																		? reviewConfig.trigger_status.filter(
																				(s) => s !== status,
																			)
																		: [
																				...(reviewConfig.trigger_status || []),
																				status,
																			];
																setReviewConfig({
																	...reviewConfig,
																	trigger_status: newStatuses,
																});
															}}
														>
											...
											<div className="space-y-2">
												...
												<p className="text-xs text-muted-foreground">
													Aguarde este período após a alta antes de solicitar a
													avaliação
												</p>
											</div>
											...
											<div className="space-y-2">
												...
												/>
												<p className="text-xs text-muted-foreground">
													O ID do local da sua clínica no Google Maps.
													Necessário para exibir reviews no site.
												</p>
											</div>
											...
											<div className="flex items-start gap-3">
												...
												<div className="text-sm">
													...
													<p className="text-emerald-800 dark:text-emerald-200">
														Use IA para criar uma mensagem personalizada e
														persuasiva.
													</p>
												</div>
											</div>
									...
									<div className="flex items-start gap-3">
										...
										<div className="text-sm">
											...
											<p className="text-blue-800 dark:text-blue-200 mt-1">
												O momento ideal para solicitar uma avaliação é quando o
												paciente relata melhora significativa ou completa o
												tratamento. Personalize a mensagem para soar genuína e
												não automatizada.
											</p>
										</div>
									</div>
												...
												>
													<div className="flex-1">
														...
														<p className="text-xs text-muted-foreground mt-1">
															Pacientes sem visita há{" "}
															{campaign.days_without_visit}+ dias
														</p>
													</div>
												</div>
											))}
							...
							<Card>
								...
								<CardContent className="space-y-6">
									<div className="flex items-center justify-between p-4 border rounded-lg">
										<div className="space-y-0.5">
											<Label className="text-base">
												Ativar Automação de Aniversários
											</Label>
											<p className="text-sm text-muted-foreground">
										...
										<CardContent className="p-4">
											<div className="flex items-start gap-3">
												...
												<div className="text-sm">
													...
													<p className="text-emerald-800 dark:text-emerald-200">
														Use IA para criar uma mensagem calorosa e
														personalizada.
													</p>
												</div>
											</div>
								...
								<CardContent className="space-y-4">
									<div className="text-center py-8 border rounded-lg border-dashed">
										...
										<h3 className="font-semibold mb-2">
											Configure seu FisioLink
										</h3>
										<p className="text-sm text-muted-foreground mb-4">
											Crie uma página personalizada com seus links importantes
											para usar no Instagram e outras redes sociais.
										</p>
										<Button asChild>
