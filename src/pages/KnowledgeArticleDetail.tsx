...
export default function KnowledgeArticleDetail() {
	...
	const { filteredKnowledge, handleUpdateArticle, curationMap, auditProfiles } =
	...
	const handleOpenStudyMode = (url?: string) => {
		...
		const finalUrl =
			url ||
			article.metadata?.attachments?.find((a) => a.type === "document")?.url ||
			article.url;
								...
								<div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center gap-3 text-amber-800 animate-in fade-in slide-in-from-top-4 duration-500">
									...
									<div className="flex-1">
										...
										<p className="text-xs opacity-80">
											Este conteúdo foi importado automaticamente e aguarda
											validação da equipe clínica.
										</p>
									</div>
									<Button
										size="sm"
									...
									<Card className="border-slate-200 shadow-sm overflow-hidden">
										<CardHeader className="bg-slate-50/50 border-b pb-4">
											<div className="flex items-center justify-between">
												...
												>
													{knowledgeEvidenceLabels[article.evidence] ||
														article.evidence}
												</Badge>
											</div>
										</CardHeader>
												...
												<CardContent className="p-6">
													<div className="flex items-center justify-between mb-6">
														...
														<Button
															...
															className="gap-2"
															onClick={() =>
																openEditorForUpload("presentation")
															}
														>
															<Upload className="h-4 w-4" />
															Subir Aula
														...
														article.metadata.attachments.filter(
															...
															<div className="text-center py-12 border border-dashed rounded-2xl bg-slate-50/50">
																...
																<p className="text-[10px] text-muted-foreground max-w-xs mx-auto mt-1">
																	Suba os slides da reunião clínica para que
																	outros fiquem por dentro da discussão.
																</p>
															</div>
														) : (
															article.metadata?.attachments
											...
											<div className="space-y-3">
												<div className="flex items-start gap-3">
													...
													<div className="space-y-1">
														...
														<p className="text-xs text-slate-300 leading-relaxed">
															Priorizar treino excêntrico de alta carga conforme
															tolerância. Monitorar EVA após 24h.
														</p>
													</div>
												</div>
												...
												<div className="flex items-start gap-3">
													...
													<div className="space-y-1">
														...
														<p className="text-xs text-slate-300 leading-relaxed">
															Melhora esperada de 40% em 6 semanas com protocolo
															de carga linear.
														</p>
													</div>
												</div>
											...
											<div className="text-xs text-slate-600 dark:text-slate-400 bg-white/50 dark:bg-black/20 p-3 rounded-lg border border-emerald-100/50">
												{curation?.notes ? (
													...
													<p className="italic opacity-60">
														Diretriz importada via IA. Aguardando parecer
														clínico oficial sobre a aplicabilidade nesta
														organização.
													</p>
												)}
											</div>
