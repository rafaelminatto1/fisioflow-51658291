...
export default function KnowledgeArticleDetail() {
	...
	const [activeAttachmentType, setActiveAttachmentType] = useState<"document" | "presentation" | null>(null);
	...
	const artifact = useMemo(() => {
		...
		return {
			...
			tags: article.tags,
			evidenceLevel: (article.evidence as any) === 'SystematicReview' ? 'SystematicReview' : (article.evidence as any),
			status: article.status === 'pending' ? 'pending' : article.status === 'review' ? 'draft' : 'verified',
			metadata: {
				...
				<div className="bg-background border-b px-4 py-3 flex items-center justify-between sticky top-0 z-10 shadow-sm">
					...
					<div className="flex items-center gap-2">
						<div className="hidden sm:flex items-center gap-1 mr-2 border-r pr-2">
							<Button variant="ghost" size="icon" onClick={() => handleShare('whatsapp')} className="h-8 w-8 text-emerald-600">
								<MessageCircle className="h-4 w-4" />
							</Button>
							<Button variant="ghost" size="icon" onClick={() => handleShare('clipboard')} className="h-8 w-8 text-slate-500">
						...
						{article.status !== 'verified' && (
							<div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center gap-3 text-amber-800 animate-in fade-in slide-in-from-top-4 duration-500">
								...
								<div className="flex-1">
									<p className="text-sm font-bold uppercase tracking-tight">Curadoria IA em Andamento</p>
									<p className="text-xs opacity-80">Este conteúdo foi importado automaticamente e aguarda validação da equipe clínica.</p>
								</div>
								<Button size="sm" variant="outline" className="border-amber-300 text-amber-700 bg-white" onClick={() => setIsEditDialogOpen(true)}>
									Validar Agora
								</Button>
							</div>
						)}
							...
							<div className="lg:col-span-2 space-y-6">
								<Card className="border-slate-200 shadow-sm overflow-hidden">
									<CardHeader className="bg-slate-50/50 border-b pb-4">
										<div className="flex items-center justify-between">
											...
											<Badge className={`${evidenceColorMap[article.evidence] || ""} border-0 font-bold`}>
									...
									<CardContent className="p-6 space-y-8">
										...
										<div className="space-y-4">
											<div className="flex items-center justify-between">
												...
												<Button variant="ghost" size="sm" className="h-7 text-[10px] font-bold uppercase gap-1" onClick={handleGenerateSummary}>
													<Sparkles className="h-3 w-3 text-amber-500" /> Refinar IA
												</Button>
											</div>
											<div className="grid gap-3">
												{article.highlights.map((h, i) => (
													<div key={i} className="flex gap-3 p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 group hover:border-primary/30 transition-colors">
								...
								<Tabs defaultValue="pdfs" className="w-full">
									<TabsList className="grid w-full grid-cols-2 h-11 p-1 bg-slate-100 dark:bg-slate-900 rounded-xl">
										<TabsTrigger value="pdfs" className="rounded-lg font-bold text-xs uppercase tracking-tight data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-sm">
											Documentos PDF
										</TabsTrigger>
										<TabsTrigger value="presentations" className="rounded-lg font-bold text-xs uppercase tracking-tight data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-sm">
											...
											<CardContent className="p-6">
												<div className="flex items-center justify-between mb-6">
													<div className="space-y-1">
														...
														<p className="text-xs text-muted-foreground">Documentos originais e anexos técnicos.</p>
													</div>
													<Button size="sm" variant="outline" className="gap-2" onClick={() => openEditorForUpload("document")}>
												...
												<div className="grid gap-3">
													{article.metadata?.attachments?.filter(a => a.type === "document").map((file, i) => (
														<div key={i} className="flex items-center justify-between p-4 rounded-xl border bg-background hover:bg-slate-50 transition-all group hover:shadow-sm">
															<div className="flex items-center gap-4">
																...
																<div>
																	...
																	<div className="flex items-center gap-2 mt-0.5">
																		<Badge variant="outline" className="text-[9px] h-4 py-0 bg-slate-50">PDF</Badge>
																		<span className="text-[10px] text-muted-foreground uppercase font-medium">{article.year || 'N/A'}</span>
																	</div>
																</div>
															</div>
															<div className="flex items-center gap-2">
																<Button variant="ghost" size="sm" className="h-8 px-2 gap-2 text-xs" asChild>
																	<a href={file.url} target="_blank" rel="noreferrer">
																		<Download className="h-4 w-4" />
																		Baixar
																	</a>
																</Button>
																<Button variant="outline" size="sm" className="h-8 text-[10px] uppercase font-bold" onClick={() => setIsStudyModeOpen(true)}>
																	Abrir com IA
																</Button>
															</div>
														</div>
														...
														<div className="text-center py-12 border border-dashed rounded-2xl bg-slate-50/50">
															...
															<p className="text-sm text-muted-foreground font-medium">Nenhum PDF anexado à esta diretriz.</p>
															<Button variant="link" size="sm" onClick={() => openEditorForUpload("document")}>Fazer o primeiro upload</Button>
														</div>
													)}
												</div>
											</CardContent>
									...
									<TabsContent value="presentations" className="mt-4 animate-in fade-in duration-300">
										<Card className="border-slate-200">
											<CardContent className="p-6">
												<div className="flex items-center justify-between mb-6">
													<div className="space-y-1">
														<h4 className="font-bold">Apresentações Internas</h4>
														<p className="text-xs text-muted-foreground">Materiais criados pela equipe (Aulas Clínicas).</p>
													</div>
													<Button size="sm" variant="outline" className="gap-2" onClick={() => openEditorForUpload("presentation")}>
														<Upload className="h-4 w-4" />
														Subir Aula
													</Button>
												</div>
												...
												<div className="grid gap-3">
													{(!article.metadata?.attachments || article.metadata.attachments.filter(a => a.type === "video" || a.name.toLowerCase().includes("aula")).length === 0) ? (
														<div className="text-center py-12 border border-dashed rounded-2xl bg-slate-50/50">
															<div className="h-12 w-12 rounded-full bg-slate-100 flex items-center justify-center mx-auto mb-3">
																<Layout className="h-6 w-6 text-slate-300" />
															</div>
															<p className="text-sm text-muted-foreground font-medium">Nenhuma aula ou apresentação vinculada.</p>
															<p className="text-[10px] text-muted-foreground max-w-xs mx-auto mt-1">Suba os slides da reunião clínica para que outros fiquem por dentro da discussão.</p>
														</div>
													) : (
														article.metadata?.attachments?.filter(a => a.type === "video" || a.name.toLowerCase().includes("aula")).map((file, i) => (
															<div key={i} className="flex items-center justify-between p-4 rounded-xl border bg-background hover:bg-slate-50 transition-all group hover:shadow-sm">
																<div className="flex items-center gap-4">
																	...
																	<div>
																		<p className="text-sm font-bold group-hover:text-amber-700 transition-colors">{file.name}</p>
																		<p className="text-[10px] text-muted-foreground uppercase font-medium mt-0.5">Apresentação • Colaborativo</p>
																	</div>
																</div>
									...
									<CardContent className="p-5 space-y-5">
										<div className="space-y-3">
											<div className="flex items-start gap-3">
												...
												<div className="space-y-1">
													<p className="text-[11px] font-bold text-amber-100 uppercase tracking-tight">Estratégia Clínica</p>
													<p className="text-xs text-slate-300 leading-relaxed">
														Priorizar treino excêntrico de alta carga conforme tolerância. Monitorar EVA após 24h.
													</p>
												</div>
											</div>
											...
											<div className="flex items-start gap-3">
												...
												<div className="space-y-1">
													<p className="text-[11px] font-bold text-emerald-100 uppercase tracking-tight">Prognóstico Estimado</p>
													<p className="text-xs text-slate-300 leading-relaxed">
										...
										<div className="grid grid-cols-2 gap-2">
											...
											>
												<Sparkles className="h-3 w-3 mr-1.5" /> {isGeneratingSummary ? "..." : "Resumo"}
											</Button>
										</div>
									</CardContent>
								</Card>
									...
									<CardContent className="p-5 space-y-4">
										<div className="grid gap-4">
											...
											<div className="flex items-center justify-between text-sm">
												...
												<span className="font-bold truncate max-w-[120px]">{article.source || "N/A"}</span>
											</div>
											<div className="flex items-center justify-between text-sm">
												<span className="text-muted-foreground flex items-center gap-2">
													<ShieldCheck className="h-4 w-4" /> Qualidade
												</span>
												<Badge variant={article.status === "verified" ? "default" : "secondary"} className="h-5 text-[10px]">
										...
										<div className="space-y-2">
											<span className="text-[10px] font-bold uppercase text-muted-foreground tracking-widest">Indexação</span>
											<div className="flex flex-wrap gap-1.5">
												{article.tags.map(tag => (
													<Badge key={tag} variant="secondary" className="text-[10px] px-2 py-0.5 bg-slate-100 border-0 hover:bg-primary/10 hover:text-primary transition-colors cursor-default">
														#{tag}
													</Badge>
												))}
											</div>
										...
										{article.url && (
											<Button asChild variant="outline" className="w-full mt-2 gap-2 text-xs font-bold uppercase h-10 border-slate-200 hover:bg-slate-50">
									...
									<CardContent className="p-4 pt-0 space-y-3">
										<div className="text-xs text-slate-600 dark:text-slate-400 bg-white/50 dark:bg-black/20 p-3 rounded-lg border border-emerald-100/50">
											{curation?.notes ? (
												...
												<p className="italic opacity-60">Diretriz importada via IA. Aguardando parecer clínico oficial sobre a aplicabilidade nesta organização.</p>
											)}
										</div>
										{curation?.assigned_to && (
											<div className="flex items-center gap-2 pt-1 text-[10px] font-bold uppercase text-emerald-600">
												...
												Validador: {auditProfiles?.[curation.assigned_to]?.full_name || curation.assigned_to}
											</div>
										)}
									</CardContent>
								</Card>
