...
export default function SimuladorReceitasPage() {
	...
	const [novaDespesa, setNovaDespesa] = useState({
		nome: "",
		valor: "",
		tipo: "mensal" as "mensal" | "trimestral" | "anual",
	});
	const [cenarioPersonalizado, setCenarioPersonalizado] = useState({
		aumento: 0,
		novosAlunos: 0,
	});
	...
	// Cálculos
	const totalReceitas = receitasFixas.reduce(
		(acc, r) => acc + r.valor * r.quantidade,
		0,
	);
	const totalDespesas = despesasFixas.reduce((acc, d) => {
		const fator =
			d.tipo === "mensal" ? 1 : d.tipo === "trimestral" ? 1 / 3 : 1 / 12;
		return acc + d.valor * fator;
	}, 0);
	const lucroAtual = totalReceitas - totalDespesas;
	const margemLucro =
		totalReceitas > 0 ? (lucroAtual / totalReceitas) * 100 : 0;
	...
	const totalAtendimentos = receitasFixas.reduce(
		(acc, r) => acc + r.quantidade,
		0,
	);
	const taxaOcupacao =
		horasDisponiveis > 0 ? (totalAtendimentos / horasDisponiveis) * 100 : 0;

	const adicionarReceita = () => {
		if (!novaReceita.nome || !novaReceita.valor || !novaReceita.quantidade)
			return;
		setReceitasFixas([
			...receitasFixas,
			{
		...
		<PageLayout>
			<PageContainer>
				<PageHeader
					...
					subtitle="Planeje e projeta suas finanças futuras"
					actions={
						<>
					...
					<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
						<Card>
							...
							<CardContent>
								...
								<p className="text-xs text-muted-foreground mt-1">
									{receitasFixas.length} fontes de receita
								</p>
							</CardContent>
						</Card>
						...
						<Card>
							...
							<CardContent>
								...
								<p className="text-xs text-muted-foreground mt-1">
									{despesasFixas.length} despesas cadastradas
								</p>
							</CardContent>
						</Card>
						...
						<Card>
							...
							<CardContent>
								<div
									className={`text-2xl font-bold ${lucroAtual >= 0 ? "text-green-600" : "text-red-600"}`}
								>
									R${" "}
						...
						<Card>
							...
							<CardContent>
								<div
									className={`text-2xl font-bold ${metaAtingida ? "text-green-600" : "text-orange-600"}`}
								>
									{percentualMeta.toFixed(0)}%
								</div>
								<p className="text-xs text-muted-foreground mt-1">
									{!metaAtingida
										? `Faltam R$ ${diferencaMeta.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`
										: "Meta atingida!"}
								</p>
							</CardContent>
						</Card>
													...
													<div className="flex items-center gap-3">
														<div className="text-right">
															<p className="font-bold text-green-600">
																R${" "}
																{(
																	receita.valor * receita.quantidade
																).toLocaleString("pt-BR", {
																	minimumFractionDigits: 2,
																})}
															</p>
															<p className="text-xs text-muted-foreground">
																R$ {receita.valor} cada
															</p>
														</div>
														<Button
															variant="ghost"
											...
											<div className="grid grid-cols-4 gap-4">
												...
												<div>
													<Select
														...
														onValueChange={(
															v: "mensal" | "trimestral" | "anual",
														) => setNovaDespesa({ ...novaDespesa, tipo: v })}
													>
														<SelectTrigger>
															<SelectValue />
														</SelectTrigger>
						...
						<div className="space-y-6">
								...
								<CardContent className="space-y-4">
									<div className="space-y-2">
										...
										<div className="flex items-center gap-2">
											...
											<Input
												...
												value={metaLucro}
												onChange={(e) =>
													setMetaLucro(parseFloat(e.target.value) || 0)
												}
											/>
										</div>
									</div>
									...
									<div className="space-y-2">
										...
										<Input
											...
											onChange={(e) =>
												setHorasDisponiveis(parseInt(e.target.value) || 0)
											}
										/>
										<p className="text-xs text-muted-foreground">
								...
								<CardContent className="space-y-4">
									...
									<div className="p-3 bg-green-50 dark:bg-green-950/30 rounded-lg">
										<div className="flex items-center justify-between mb-2">
											...
											<Badge className="bg-green-500">
												+
												{(
													((lucroOtimista - lucroAtual) /
														Math.abs(lucroAtual || 1)) *
													100
												).toFixed(0)}
												%
											</Badge>
										...
										<p className="text-lg font-bold text-green-600">
											R${" "}
											{lucroOtimista.toLocaleString("pt-BR", {
												minimumFractionDigits: 2,
											})}
										</p>
									</div>
									...
									<div className="p-3 bg-red-50 dark:bg-red-950/30 rounded-lg">
										...
										<p className="text-lg font-bold text-red-600">
											R${" "}
											{lucroPessimista.toLocaleString("pt-BR", {
												minimumFractionDigits: 2,
											})}
										</p>
									</div>
									...
									<div className="space-y-3">
										...
										<div className="space-y-3">
											<div className="space-y-2">
												<div className="flex items-center justify-between text-sm">
													...
													<span className="font-medium">
														{cenarioPersonalizado.aumento}%
													</span>
												</div>
												<Slider
													value={[cenarioPersonalizado.aumento]}
													onValueChange={(v) =>
														setCenarioPersonalizado({
															...cenarioPersonalizado,
															aumento: v[0],
														})
													}
													max={50}
											...
											<div className="space-y-2">
												<div className="flex items-center justify-between text-sm">
													...
													<span className="font-medium">
														{cenarioPersonalizado.novosAlunos}
													</span>
												</div>
												<Slider
										...
										<div className="p-3 bg-blue-50 dark:bg-blue-950/30 rounded-lg">
											...
											<p
												className={`text-lg font-bold ${lucroPersonalizado >= lucroAtual ? "text-green-600" : "text-red-600"}`}
											>
												R${" "}
												{lucroPersonalizado.toLocaleString("pt-BR", {
													minimumFractionDigits: 2,
												})}
											</p>
											<p className="text-xs text-muted-foreground">
							...
							<Card className="bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800">
								...
								<CardContent className="text-sm text-blue-700 dark:text-blue-400 space-y-2">
									{margemLucro < 30 && (
										<p>
											• Sua margem de lucro está baixa. Considere revisar preços
											ou reduzir custos.
										</p>
									)}
									{taxaOcupacao > 80 && (
										<p>
											• Alta ocupação! Considere aumentar preços ou expandir
											horários.
										</p>
									)}
									{taxaOcupacao < 50 && (
										<p>
											• Baixa ocupação. Invista em marketing para atrair mais
											pacientes.
										</p>
									)}
									{!metaAtingida && (
										<p>
											• Para atingir sua meta, você precisa aumentar R${" "}
											{diferencaMeta.toLocaleString("pt-BR", {
												minimumFractionDigits: 2,
											})}{" "}
											nas receitas.
										</p>
