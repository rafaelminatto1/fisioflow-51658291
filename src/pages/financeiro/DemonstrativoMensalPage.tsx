...
export default function DemonstrativoMensalPage() {
	...
	const [mesSelecionado, setMesSelecionado] = useState(
		format(new Date(), "yyyy-MM").split("-")[1],
	);
	...
	const { data: demoData, isLoading } = useQuery({
		...
		queryFn: async (): Promise<DemonstrativoData> => {
			...
			entradas.forEach((m) => {
				...
				entradasPorCategoria[cat] =
					(entradasPorCategoria[cat] || 0) + Number(m.valor);
				const metadata = (m.metadata ?? {}) as Record<string, unknown>;
				const forma =
					typeof metadata.forma_pagamento === "string"
			...
			saidas.forEach((m) => {
				const cat = m.categoria || "Outros";
				saidasPorCategoria[cat] =
					(saidasPorCategoria[cat] || 0) + Number(m.valor);
			});
	...
	const entradasCategoriaData = demoData
		? Object.entries(demoData.entradasPorCategoria).map(([cat, valor]) => ({
				name: cat,
				value: valor,
			}))
		: [];
	...
	const saidasCategoriaData = demoData
		? Object.entries(demoData.saidasPorCategoria).map(([cat, valor]) => ({
				name: cat,
				value: valor,
			}))
		: [];
	...
	const formasPagamentoData = demoData
		? Object.entries(demoData.entradasPorFormaPagamento).map(
				([forma, valor]) => ({ name: forma, value: valor }),
			)
		: [];
	...
	const crescimentoReceita =
		demoMesAnterior && demoData
			? ((demoData.entradas - demoMesAnterior.entradas) /
					demoMesAnterior.entradas) *
				100
			: 0;

	const crescimentoSaldo =
		demoMesAnterior && demoData
			? ((demoData.saldo - demoMesAnterior.saldo) /
					Math.abs(demoMesAnterior.saldo || 1)) *
				100
			: 0;
						...
						<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
							<Card className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-950 dark:to-green-900 border-green-200 dark:border-green-800">
								...
								<CardContent>
									...
									{crescimentoReceita !== 0 && (
										<div
											className={`flex items-center text-xs mt-1 ${crescimentoReceita > 0 ? "text-green-600" : "text-red-600"}`}
										>
											{crescimentoReceita > 0 ? (
											...
											{crescimentoReceita > 0 ? "+" : ""}
											{crescimentoReceita.toFixed(1)}% vs mês anterior
										</div>
									)}
								</CardContent>
							</Card>
							...
							<Card className="bg-gradient-to-br from-red-50 to-red-100 dark:from-red-950 dark:to-red-900 border-red-200 dark:border-red-800">
								...
								<CardContent>
									...
									<div className="text-xs text-muted-foreground mt-1">
										{(
											(demoData.saidas / (demoData.entradas || 1)) *
											100
										).toFixed(1)}
										% das entradas
									</div>
							...
							<Card className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-950 dark:to-blue-900 border-blue-200 dark:border-blue-800">
								...
								<CardContent>
									...
									{crescimentoSaldo !== 0 && (
										<div
											className={`flex items-center text-xs mt-1 ${crescimentoSaldo > 0 ? "text-green-600" : "text-red-600"}`}
										>
											{crescimentoSaldo > 0 ? (
												<TrendingUp className="h-3 w-3 mr-1" />
											) : (
											...
											{crescimentoSaldo > 0 ? "+" : ""}
											{crescimentoSaldo.toFixed(1)}% vs mês anterior
										</div>
									)}
								</CardContent>
							</Card>
								...
								<CardContent>
									<div className="grid grid-cols-2 gap-4">
										<div className="p-4 bg-green-50 dark:bg-green-950 rounded-lg">
											...
											<p className="text-xl font-bold text-green-600">
												R${" "}
												{demoData.contasReceber.toLocaleString("pt-BR", {
													minimumFractionDigits: 2,
												})}
											</p>
										</div>
										<div className="p-4 bg-red-50 dark:bg-red-950 rounded-lg">
											...
											<p className="text-xl font-bold text-red-600">
												R${" "}
												{demoData.contasPagar.toLocaleString("pt-BR", {
													minimumFractionDigits: 2,
												})}
											</p>
										</div>
									...
									<div className="p-4 bg-blue-50 dark:bg-blue-950 rounded-lg">
										<p className="text-sm text-muted-foreground">
											Saldo Projetado (considerando pendentes)
										</p>
										<p
											className={`text-xl font-bold ${(demoData.saldo + demoData.contasReceber - demoData.contasPagar) >= 0 ? "text-green-600" : "text-red-600"}`}
										>
											R${" "}
											{(
												demoData.saldo +
												demoData.contasReceber -
												demoData.contasPagar
							...
							<CardContent className="pt-6">
								<div className="flex items-center justify-between text-sm text-muted-foreground">
									<div>
										<p>
											Período:{" "}
											<span className="font-medium">{demoData.periodo}</span>
										</p>
										<p>
											Gerado em:{" "}
