...
import { TrendingUp, TrendingDown, DollarSign, Calendar, LineChart as LineChartIcon } from 'lucide-react';
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <Card className="rounded-2xl border-none shadow-sm bg-white dark:bg-slate-900">
                ...
                <CardContent>
                  <div className={`text-2xl font-black ${saldoAcumulado >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
            ...
            <Card className="rounded-2xl border-none shadow-sm bg-white dark:bg-slate-900 overflow-hidden">
              <CardHeader className="p-6 pb-0">
                <CardTitle className="text-sm font-black uppercase tracking-[0.1em] text-slate-400">Evolução Mensal</CardTitle>
              </CardHeader>
          ...
          <>
            ...
            <Card className="rounded-2xl border-none shadow-sm bg-white dark:bg-slate-900 p-4">
              <div className="flex items-center gap-4">
                <span className="text-xs font-black uppercase tracking-widest text-slate-400">Data do Caixa:</span>
                <input
                  type="date"
                  value={dataCaixa}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Card className="rounded-2xl border-none shadow-sm bg-white dark:bg-slate-900">
                <CardHeader className="pb-2">
                  <CardTitle className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Entradas</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-black text-emerald-600">
              ...
              <Card className="rounded-2xl border-none shadow-sm bg-white dark:bg-slate-900">
                <CardHeader className="pb-2">
                  <CardTitle className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Saídas</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-black text-red-600">
              ...
              <Card className="rounded-2xl border-none shadow-sm bg-white dark:bg-slate-900">
                <CardHeader className="pb-2">
                  <CardTitle className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Saldo Líquido</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className={`text-2xl font-black ${(caixaDiario?.saldo || 0) >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
            ...
            {caixaDiario?.porFormaPagamento && Object.keys(caixaDiario.porFormaPagamento).length > 0 && (
              <Card className="rounded-2xl border-none shadow-sm bg-white dark:bg-slate-900">
                <CardHeader>
                  <CardTitle className="text-sm font-black uppercase tracking-widest text-slate-400">Por Meio de Pagamento</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {Object.entries(caixaDiario.porFormaPagamento).map(([forma, valor]) => (
                      <div key={forma} className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">{forma}</p>
