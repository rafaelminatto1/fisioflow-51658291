...
export function RecibosContent({
	...
	const { currentOrganization, updateOrganization, isUpdating } =
		useOrganizations();
	...
	useEffect(() => {
		if (clinicaConfig?.org?.settings?.receipt_settings) {
			...
			setReceiptConfig({
				custom_issuer_name:
					settings.custom_issuer_name || currentOrganization?.name || "",
				custom_professional_name:
					settings.custom_professional_name ||
			...
			setReceiptConfig((prev) => ({
				...prev,
				custom_issuer_name:
					prev.custom_issuer_name || currentOrganization?.name || "",
				custom_professional_name:
					prev.custom_professional_name ||
					clinicaConfig.profile?.full_name ||
					"",
			}));
		}
	...
	const {
		...
		handleSubmit,
		isSubmitting,
	} = useReciboForm(pacientes, clinicaConfig, receiptConfig);
