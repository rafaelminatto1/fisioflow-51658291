...
export function NFSeStats({ nfses }: { nfses: NFSe[] }) {
	const stats = useMemo(() => {
		...
		const thisMonth = nfses.filter(
			(n) => new Date(n.data_emissao).getMonth() === month,
		);
		...
		const pending = nfses.filter(
			(n) =>
				n.status === "aguardando_prefeitura" ||
				n.status === "aguardando_internet",
		).length;
		...
		return {
			...
			successRate: (() => {
				...
				return Math.round(
					(nfses.filter((n) => n.status === "autorizado").length / sent) * 100,
				);
			})(),
		};
	}, [nfses]);
