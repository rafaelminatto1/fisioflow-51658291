...
export function DemonstrativoMensalContent() {
  ...
  const currentDate = new Date(parseInt(anoSelecionado, 10), parseInt(mesSelecionado, 10) - 1, 1);
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute("download", `demonstrativo-${anoSelecionado}-${mesSelecionado}.csv`);
  ...
  return (
    <div className="space-y-6">
        ...
        <Card className="xl:col-span-2 rounded-[32px] border-none shadow-sm bg-white dark:bg-slate-900 overflow-hidden">
          <CardContent className="p-8">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h3 className="text-lg font-black tracking-tight">Tendência de Performance</h3>
                <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider">
                  Histórico de Receitas vs Despesas (6 meses)
                </p>
              </div>
            </div>
            <FinancialCharts demoData={demoData} historicoData={historicoData} />
          </CardContent>
        </Card>
      ...
      <Card className="rounded-[32px] border-none shadow-sm bg-white dark:bg-slate-900 overflow-hidden">
        <CardContent className="p-8">
          <div className="mb-8">
            <h3 className="text-lg font-black tracking-tight">Detalhamento por Categoria</h3>
            <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider">
