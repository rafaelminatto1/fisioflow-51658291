...
import {
	SectionSkeleton,
	PageShellFallback,
} from "./components/FinancialUIUtils";
			...
			<PageHeader
				...
				actions={
					<div className="flex flex-wrap items-center gap-2">
						<Select
							value={state.period}
							onValueChange={actions.handlePeriodChange}
						>
				...
				>
					...
					<TabsContent value="summary" className="space-y-6">
						{!commandCenter && state.isLoading ? (
							...
							<FinancialCommandCenterSummary
								data={commandCenter}
								onNewTransaction={() =>
									actions.handleOpenTransactionModal("receita")
								}
							/>
						) : (
							<EmptyState
								...
								<div className="grid gap-4 md:grid-cols-3">
									...
									<MetricCard
										...
										value={formatCurrency(
											commandCenter?.summary.pendingReceivables ?? 0,
										)}
									/>
								</div>
					...
					<TabsContent value="billing" className="space-y-6">
						<Tabs
							...
							onValueChange={(v) =>
								actions.updateQueryParams({ tab: "billing", billing: v })
							}
							className="space-y-5"
						>
							<TabsList className="flex h-auto flex-wrap justify-start gap-2 rounded-[24px] bg-card p-2 dark:bg-slate-950/70">
					...
					<TabsContent value="documents" className="space-y-6">
						<div className="grid gap-4 md:grid-cols-3">
							<MetricCard
								title="Recibos"
								value={commandCenter?.documents.receiptsInPeriod ?? 0}
							/>
							<MetricCard
						...
						<Tabs
							...
							onValueChange={(v) =>
								actions.updateQueryParams({ tab: "documents", documents: v })
							}
							className="space-y-5"
						>
							<TabsList className="flex h-auto flex-wrap justify-start gap-2 rounded-[24px] bg-card p-2 dark:bg-slate-950/70">
							...
							<TabsContent value="receipts">
								<Suspense fallback={<PageShellFallback />}>
									<RecibosContent
										...
										onAutoOpenHandled={() =>
											actions.clearActionParams("receiptAction")
										}
									/>
								</Suspense>
							</TabsContent>
							<TabsContent value="nfse" className="space-y-4">
								...
								<Suspense fallback={<PageShellFallback />}>
									<NFSeContent
										...
										onAutoOpenHandled={() =>
											actions.clearActionParams("nfseAction")
										}
									/>
								</Suspense>
							</TabsContent>
							...
							<div className="space-y-6">
								<div className="rounded-[2rem] bg-gradient-to-br from-brand-blue to-indigo-600 p-8 text-white shadow-xl">
									...
									<p className="text-blue-100 max-w-2xl text-sm">
										Análise estratégica correlacionando desfechos clínicos (IA
										Studio & RTM) com performance financeira (LTV). Identifique
										os protocolos mais rentáveis e pacientes com risco de
										abandono.
									</p>
								</div>
