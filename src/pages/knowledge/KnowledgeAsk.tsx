              ...
              <div key={art.pmid} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm hover:border-emerald-300 transition">
                <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    {(art.studyType ?? art.study_type) && (
                      <span className="rounded-md bg-slate-100 px-2 py-0.5 text-xs font-semibold text-slate-600">{art.studyType ?? art.study_type}</span>
                    )}
                  </div>
                  {(art.pubDate ?? art.pub_date) && <span className="text-xs font-medium text-slate-400">{art.pubDate ?? art.pub_date}</span>}
                </div>
