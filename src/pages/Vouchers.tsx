...
import {
	usePurchaseVoucher,
	useVerifyVoucherPayment,
} from "@/hooks/usePurchaseVoucher";
...
export default function Vouchers() {
	...
	const { data: userVouchers, isLoading: userVouchersLoading } =
		useUserVouchers();
	...
	const activeVouchers =
		userVouchers?.filter(
			(v) => v.ativo && new Date(v.data_expiracao) > new Date(),
		) || [];
	...
	const stats = {
		...
		sessoesDisponiveis: activeVouchers.reduce(
			(acc, v) => acc + v.sessoes_restantes,
			0,
		),
	};
		...
		<PageLayout>
			<PageContainer>
				<PageHeader
					...
					subtitle="Gerencie seus pacotes e planos de sessões"
				/>
						...
						<TabsContent value="my-vouchers" className="mt-4 sm:mt-6">
							{userVouchersLoading ? (
								...
								<EmptyState
									...
									description="Você ainda não possui vouchers ativos."
								/>
							) : (
								<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
											...
											<CardContent className="space-y-3">
												<div className="h-2 bg-muted rounded-full overflow-hidden">
													<div
														...
														style={{
															width: `${(userVoucher.sessoes_restantes / userVoucher.sessoes_totais) * 100}%`,
														}}
													/>
												</div>
												<div className="flex items-center text-sm text-muted-foreground">
													...
													Expira em{" "}
													{format(
														new Date(userVoucher.data_expiracao),
														"dd 'de' MMMM",
														{
															locale: ptBR,
														},
							...
							<TabsContent value="manage" className="mt-4 sm:mt-6 space-y-4">
								...
								{allVouchersLoading ? (
									...
									<EmptyState
										...
										description="Crie seu primeiro voucher"
									/>
								) : (
									<div className="space-y-3">
													...
													<div className="flex items-center justify-between gap-4">
														<div className="flex-1 min-w-0">
															<div className="flex items-center gap-2 mb-1">
																...
																<Badge
																	variant={
																		voucher.ativo ? "default" : "secondary"
																	}
																>
															...
															<div className="flex items-center gap-4 mt-2 text-sm">
																{voucher.sessoes && (
																	<span>{voucher.sessoes} sessões</span>
																)}
				...
				>
					<AlertDialogContent>
						<AlertDialogHeader>
							...
							<AlertDialogDescription>
								Tem certeza que deseja excluir este voucher? Esta ação não pode
								ser desfeita.
							</AlertDialogDescription>
						</AlertDialogHeader>
						<AlertDialogFooter>
