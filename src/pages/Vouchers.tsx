...
export default function Vouchers() {
  ...
  const activeVouchers =
    userVouchers?.filter((v) => v.ativo && new Date(v.data_expiracao) > new Date()) || [];

  const filteredAllVouchers = (allVouchers || []).filter(
    (v) => accentIncludes(v.nome, searchTerm) || accentIncludes(v.descricao || "", searchTerm),
  );
                    ...
                    <Card key={userVoucher.id} className="hover:shadow-md transition-all">
                      ...
                      <CardContent className="space-y-3">
                        ...
                        <div className="flex items-center text-sm text-muted-foreground">
                          ...
                          Expira em{" "}
                          {format(new Date(userVoucher.data_expiracao), "dd 'de' MMMM", {
                            locale: ptBR,
                          })}
                        </div>
                        <div className="flex items-center text-sm text-muted-foreground">
