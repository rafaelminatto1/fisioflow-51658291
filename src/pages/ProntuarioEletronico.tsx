...
import {
	DigitalSignature,
	type SignatureData,
} from "@/components/signature/DigitalSignature";
...
function useSessionSignatures(sessionId: string) {
	return useQuery({
		...
		queryFn: () =>
			documentSignaturesApi.list(sessionId).then((r) => r.data ?? []),
		enabled: !!sessionId,
		staleTime: 60 * 1000,
	});
}
			...
			{isSigned && signatures[0] && (
				<div className="flex items-center gap-2 p-3 bg-emerald-50 rounded-xl text-xs text-emerald-700">
					...
					<span>
						Assinado por{" "}
						<strong>
							{(signatures[0] as Record<string, unknown>).signer_name as string}
						</strong>{" "}
						em{" "}
						{format(
							parseISO(
								(signatures[0] as Record<string, unknown>).signed_at as string,
							),
							"dd/MM/yyyy 'às' HH:mm",
						)}
					</span>
...
export default function ProntuarioEletronico() {
	...
	const selectedSession = sessions.find(
		(s) =>
			String((s as Record<string, unknown>).id ?? "") === selectedSessionId,
