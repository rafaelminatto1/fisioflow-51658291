...
function usePatient(patientId: string) {
  return useQuery({
    ...
    queryFn: () =>
      request<{ data: Record<string, unknown> }>(`/api/patients/${patientId}`).then((r) => r.data),
    enabled: !!patientId,
    staleTime: 5 * 60 * 1000,
  });
}
...
function SessionDetail({
  ...
  const handleExportPDF = () => {
    ...
    const evolution: SoapEvolution = {
      ...
      signed_at: sig
        ? format(parseISO(sig.signed_at as string), "dd/MM/yyyy 'às' HH:mm")
        : undefined,
    };
  ...
  const fields: Array<{
    ...
    {
      key: "subjective",
      sectionTitle: "S — Subjetivo",
      value: session.subjective as string | undefined,
    },
    {
      key: "objective",
      sectionTitle: "O — Objetivo",
      value: session.objective as string | undefined,
    },
    {
      key: "assessment",
      sectionTitle: "A — Avaliação/Diagnóstico",
      value: session.assessment as string | undefined,
    },
    {
      key: "plan",
      sectionTitle: "P — Plano",
      value: session.plan as string | undefined,
    },
  ];
    ...
    <div className="space-y-4">
      ...
      {isSigned && signatures[0] && (
        <div className="flex items-center gap-2 p-3 bg-emerald-50 rounded-xl text-xs text-emerald-700">
          ...
          <span>
            Assinado por{" "}
            <strong>{(signatures[0] as Record<string, unknown>).signer_name as string}</strong> em{" "}
            {format(
              parseISO((signatures[0] as Record<string, unknown>).signed_at as string),
              "dd/MM/yyyy 'às' HH:mm",
            )}
          </span>
        </div>
...
export default function ProntuarioEletronico() {
  ...
  const firstSessionId =
    sessions.length > 0 ? String((sessions[0] as Record<string, unknown>).id ?? "") : null;
  ...
  useEffect(() => {
    ...
    const hasRequestedSession = requestedSessionId
      ? sessions.some(
          (session) => String((session as Record<string, unknown>).id ?? "") === requestedSessionId,
        )
      : false;
  ...
  const selectedSession = sessions.find(
    (s) => String((s as Record<string, unknown>).id ?? "") === selectedSessionId,
                ...
                <div className="flex flex-col items-center justify-center py-16 gap-3 text-center text-muted-foreground">
                  ...
                  <div>
                    ...
                    <p className="text-xs mt-1">
                      Clique em uma entrada à esquerda para ver os detalhes
                    </p>
                  </div>
                </div>
              ) : (
