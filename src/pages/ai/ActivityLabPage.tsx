...
function mapToActivityLabPatient(
	p: Patient | ActivityLabPatient,
): ActivityLabPatient {
...
export default function ActivityLabPage() {
	const [selectedPatient, setSelectedPatient] =
		useState<ActivityLabPatient | null>(null);
	const [selectedSession, setSelectedSession] =
		useState<ActivityLabSession | null>(null);
	const [compareSession, setCompareSession] =
		useState<ActivityLabSession | null>(null);
	const [comparisonMode, setComparisonMode] = useState(false);
	const [activeTab, setActiveTab] = useState("patients");

	const { data: activityLabPatients = [], isLoading: loadingLabPatients } =
		useActivityLabPatients();
	const { data: allPatients = [], isLoading: loadingAllPatients } =
		useActivePatients();
	const { data: sessions = [], isLoading: loadingSessions } =
		useActivityLabSessions(selectedPatient?.id);
	const { data: clinic } = useActivityLabClinic();

	const isInitialLoading =
		(loadingLabPatients && activityLabPatients.length === 0) ||
													...
													>
														<div className="flex items-center space-x-3">
															...
															<div>
																...
																<p className="text-[11px] text-gray-500 mt-1 uppercase font-medium tracking-tighter">
																	CPF: {patient.cpf || "---"}
																</p>
															</div>
														</div>
													...
													<div className="p-12 text-center text-gray-400">
														{loadingLabPatients ? (
															...
															<>
																...
																<p className="text-sm">
																	Nenhum paciente com testes vinculados.
																</p>
																<p className="text-xs mt-1">
																	Use a busca acima para encontrar qualquer
																	paciente.
																</p>
															</>
														)}
												...
												<div className="flex items-center justify-between">
													<div>
														...
														<CardDescription className="flex items-center gap-2 mt-1">
															<Badge
																variant="outline"
																className="text-[10px] uppercase font-bold tracking-widest border-indigo-200 text-indigo-600"
															>
																Activity Lab Source
															</Badge>
														</CardDescription>
										...
										<Card className="h-full flex items-center justify-center border-dashed border-2 bg-slate-50/30">
											<div className="text-center p-12">
												...
												<p className="text-slate-500 max-w-xs mx-auto">
													Selecione um paciente na lista ao lado para visualizar
													seu perfil e histórico de força.
												</p>
											</div>
										</Card>
									)}
										...
										<CardHeader className="bg-slate-50/50 pb-4">
											<div className="flex items-center gap-2 text-indigo-600 mb-2">
												<Button
													...
													className="p-0 h-auto hover:bg-transparent font-bold text-xs uppercase tracking-tighter"
												>
													...
													>
														<div className="flex items-center space-x-3">
															...
															<div>
																...
																<div className="flex items-center text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-tighter">
																	<Badge
																		variant="outline"
																		className={`mr-2 px-1 py-0 h-4 text-[9px] ${
																			session.side === "LEFT"
														...
														<div className="flex flex-col items-end">
															<span className="font-bold text-slate-900">
																{session.peak_force.toFixed(1)}kg
															</span>
															{comparisonMode && (
																<Badge
									...
									{selectedSession && compareSession && comparisonMode ? (
										...
										<div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
											{comparisonMode && !compareSession && (
												<div className="bg-indigo-50 border border-indigo-100 p-3 rounded-xl text-center text-xs font-bold text-indigo-600 animate-pulse">
													Selecione a segunda sessão na lista ao lado para
													comparar
												</div>
											)}
										...
										<Card className="h-full flex items-center justify-center border-dashed border-2 bg-slate-50/30">
											<div className="text-center p-12">
												...
												<p className="text-slate-500 max-w-xs mx-auto">
													Escolha um teste no histórico ao lado para analisar a
													curva de força e métricas biomecânicas.
												</p>
											</div>
										</Card>
									)}
