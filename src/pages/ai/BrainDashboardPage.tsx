          ...
          <div className="md:col-span-1 space-y-6">
            <Card className="relative overflow-hidden border-none shadow-md bg-gradient-to-br from-indigo-500/10 via-purple-500/5 to-transparent dark:from-indigo-900/20 dark:via-purple-900/10">
              ...
              <CardHeader className="relative z-10">
                ...
                <CardDescription>
                  Busque em todo o seu histórico clínico e base de conhecimento simultaneamente.
                </CardDescription>
              </CardHeader>
              <CardContent className="relative z-10">
                <form onSubmit={handleSearch} className="flex flex-col gap-4">
                  ...
                  <Button
                    type="submit"
                    className="w-full h-12 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white shadow-md transition-all hover:shadow-lg"
                    disabled={isLoading || isLoadingUnified}
                  >
            ...
            <Card className="bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm border-border/50">
              ...
              <CardContent className="text-sm text-muted-foreground space-y-4">
                <p>
                  O FisioFlow Brain usa <strong>pgvector</strong> e embeddings para entender o
                  significado clínico das suas anotações, não apenas palavras-chave exatas. Isso
                  permite agrupar pacientes com perfis de recuperação semelhantes para otimizar suas
                  condutas.
                </p>
              </CardContent>
            </Card>
                  ...
                  {isLoadingUnified ? (
                    ...
                    >
                      ...
                      <p className="text-sm font-medium animate-pulse">
                        Consultando biblioteca de protocolos...
                      </p>
                    </motion.div>
                  ) : unifiedData?.data && unifiedData.data.length > 0 ? (
                    <motion.div
                    ...
                    >
                      {unifiedData.data.map((item: any, index: number) => {
                        ...
                        if (item.type === "exercise") {
                          ...
                          route = `/exercises/${item.id}`;
                        } else if (item.type === "protocol") {
                          Icon = ClipboardList;
                          iconColor = "text-amber-500";
                          route = `/protocols/${item.id}`;
                        } else if (item.type === "wiki") {
                          Icon = BookOpen;
                          iconColor = "text-blue-500";
                          route = `/wiki?id=${item.id}`;
                        ...
                        return (
                          ...
                          >
                            <div
                              className={cn(
                                "p-2.5 rounded-lg bg-slate-100 dark:bg-slate-800 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300",
                                iconColor,
                              )}
                            >
                    ...
                    >
                      ...
                      <p className="text-sm">
                        Aguardando busca para consultar protocolos e exercícios.
                      </p>
                    </motion.div>
                  )}
                </AnimatePresence>
                ...
                <AnimatePresence mode="wait">
                  {isLoading ? (
                    ...
                    >
                      ...
                      <p className="text-sm font-medium animate-pulse">
                        Minerando milhares de evoluções e prontuários...
                      </p>
                    </motion.div>
                  ) : insightsData?.data && insightsData.data.length > 0 ? (
                    <motion.div
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                  {format(new Date(match.sessionDate), "dd 'de' MMM, yyyy", {
                                    locale: ptBR,
                                  })}
                                </p>
                              </div>
                            ...
                            <div className="flex flex-col items-end gap-1.5">
                              <Badge
                                variant={match.painScale > 6 ? "destructive" : "default"}
                                className={cn(
                                  "text-[10px]",
                                  match.painScale <= 6
                    ...
                    >
                      <Search className="w-8 h-8 mb-3 opacity-50" />
                      <p className="text-sm">
                        Nenhum padrão clínico correspondente encontrado nas evoluções.
                      </p>
                    </motion.div>
                  ) : (
                    <motion.div
