...
const TABS = [
	...
	},
	{
		...
		icon: Gauge,
		Component: CapacidadeTab,
	},
	{
		value: "status",
		label: "Status",
		description: "Cores e estados",
		icon: Palette,
		Component: StatusTab,
	},
	{
	...
	},
	{
		value: "bloqueios",
		label: "Bloqueios",
		description: "Feriados e folgas",
		icon: CalendarOff,
		Component: BloqueiosTab,
	},
	{
		value: "politicas",
		label: "Políticas",
	...
	return (
		<nav className="flex flex-col gap-1">
			{TABS.map((tab) => {
				...
				const badge =
					tab.value === "tipos" && typesCount > 0 ? typesCount : null;
				return (
					<button
						key={tab.value}
						type="button"
...
export default function ScheduleSettings() {
	...
	const activeTab: TabValue = useMemo(() => {
		...
		return (VALID_TAB_IDS as readonly string[]).includes(rawTab)
			? (rawTab as TabValue)
			: "overview";
	}, [rawTab]);
		...
		<PageLayout>
			<PageContainer>
				<PageHeader
					...
					subtitle="Horários, capacidade, status, tipos, bloqueios e políticas de atendimento."
					actions={
						<>
							<Button
								asChild
						...
						{isMobile ? (
							<Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
								<SheetTrigger asChild>
									<Button
										variant="outline"
										className="h-11 justify-between rounded-lg"
									>
