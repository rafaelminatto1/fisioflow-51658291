...
const PatientPortal = () => {
  ...
  const [selectedPrescription, setSelectedPrescription] = useState<PrescriptionExercise | null>(
    null,
  );
  ...
  const { data: painMapsData = [] } = useQuery({
    ...
    queryFn: () =>
      patient?.id ? PainMapService.getPainMapsByPatientId(patient.id) : Promise.resolve([]),
    enabled: !!patient?.id,
  });
  ...
  const generateHealthSummary = async () => {
    ...
    try {
      const painLevels = painRecords
        .slice(0, 5)
        .map((r) => `${r.intensity ?? r.pain_level}/10 (${r.location ?? r.body_part ?? "geral"})`)
        .join(", ");
      const prompt = `
          ...
          <div className="flex items-center justify-center h-[60vh]">
            <Card className="max-w-md text-center">
              <CardHeader>
                ...
                <CardDescription>
                  Seu perfil ainda não está vinculado a um registro de paciente. Entre em contato
                  com sua clínica.
                </CardDescription>
              </CardHeader>
              <CardContent>
  ...
  return (
    <PageLayout>
      <PageContainer>
        <PageHeader title={<>Olá, {PatientHelpers.getName(patient).split(" ")[0]}! 👋</>} />
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-lg font-bold">{painRecords?.[0]?.pain_level ?? "---"}/10</p>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <Card className="shadow-lg border-primary/20 bg-primary/5">
                  <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    >
                      <Plus
                        className={cn("h-5 w-5 transition-transform", showPainReg && "rotate-45")}
                      />
                    </Button>
                  </CardHeader>
                  <CardContent>
                        ...
                        {painRecords?.[0] ? (
                          <div className="flex items-center gap-4 p-4 bg-white rounded-lg shadow-sm">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">
                                {format(parseISO(painRecords[0].created_at!), "dd/MM 'às' HH:mm", {
                                  locale: ptBR,
                                })}
                              </p>
                            </div>
