...
export default function Exercises() {
	...
	const shouldLoadPatientsForAI =
		activeTab === "ai" || Boolean(selectedPatientId);
	...
	const _videoPercentage = useMemo(
		() =>
			exercises.length > 0
				? Math.round((exercisesWithVideo.length / exercises.length) * 100)
				: 0,
					...
					<div className="flex items-center gap-4 mt-2 overflow-x-auto pb-1 no-scrollbar">
						<div className="flex items-center gap-1.5 whitespace-nowrap">
							...
							<span className="text-xs font-medium text-slate-600">
								<span className="text-slate-900">{exercises.length}</span>{" "}
								Exercícios
							</span>
						</div>
						<div className="flex items-center gap-1.5 whitespace-nowrap">
						...
						<div className="flex items-center gap-1.5 whitespace-nowrap">
							...
							<span className="text-xs font-medium text-slate-600">
								<span className="text-slate-900">{templates.length}</span>{" "}
								Templates
							</span>
						</div>
						<div className="flex items-center gap-1.5 whitespace-nowrap">
							...
							<span className="text-xs font-medium text-slate-600">
								<span className="text-slate-900">{protocols.length}</span>{" "}
								Protocolos
							</span>
						</div>
					</div>
