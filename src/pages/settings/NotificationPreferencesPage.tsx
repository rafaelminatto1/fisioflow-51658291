...
export default function NotificationPreferencesPage() {
	...
	const { subscribe, unsubscribe, isSubscribed, permission } =
	...
	const [preferences, setPreferences] =
		useState<NotificationPreferencesForm>(defaultPreferences);

	const mappedWorkerPrefs = useMemo<NotificationPreferencesForm>(() => {
		const localPrefs = readLocalPrefs();
		return {
			appointmentReminders:
				workerPreferences?.appointment_reminders ??
				defaultPreferences.appointmentReminders,
			exerciseReminders:
				workerPreferences?.exercise_reminders ??
				defaultPreferences.exerciseReminders,
			progressUpdates:
				workerPreferences?.progress_updates ??
				defaultPreferences.progressUpdates,
			systemAlerts:
				workerPreferences?.system_alerts ?? defaultPreferences.systemAlerts,
			therapistMessages:
				workerPreferences?.therapist_messages ??
				defaultPreferences.therapistMessages,
			paymentReminders:
				workerPreferences?.payment_reminders ??
				defaultPreferences.paymentReminders,
			weekendNotifications:
				workerPreferences?.weekend_notifications ??
				defaultPreferences.weekendNotifications,
			emailNotifications:
				localPrefs.emailNotifications ?? defaultPreferences.emailNotifications,
			smsNotifications:
				localPrefs.smsNotifications ?? defaultPreferences.smsNotifications,
			quietHours: {
				enabled: localPrefs.quietHours?.enabled ?? false,
				start:
					workerPreferences?.quiet_hours_start ??
					defaultPreferences.quietHours.start,
				end:
					workerPreferences?.quiet_hours_end ??
					defaultPreferences.quietHours.end,
			},
		};
	}, [workerPreferences]);
		...
		<PageLayout>
			<PageContainer>
				...
				<div className="min-h-screen bg-card pb-20">
					<div className="sticky top-0 z-30 bg-card border-b px-6 py-3"></div>
					...
					<div className="container max-w-3xl mx-auto pt-6 px-4 space-y-6">
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Receba alertas diretamente no seu navegador, mesmo quando o
									app estiver fechado
								</CardDescription>
							</CardHeader>
						...
						<Card>
							...
							<CardContent className="space-y-4">
								...
								<div className="flex items-center justify-between">
									<div className="space-y-0.5">
										<Label htmlFor="therapist-messages">
							...
							<CardContent>
								<div className="flex items-center justify-between">
									<Label htmlFor="weekend-notifications">
										Receber notificações no fim de semana
									</Label>
									<Switch
