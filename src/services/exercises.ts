...
export const exerciseService = {
  async getExercises(filters?: ExerciseFilters): Promise<Exercise[]> {
    ...
    if (equipment && equipment.includes(NO_EQUIPMENT_GROUP_ID)) {
      const homeGroupLabels = EQUIPMENT.filter((e) => HOME_EQUIPMENT_GROUP.includes(e.value)).map(
        (e) => e.label,
      );
    ...
    if (equipment?.length) {
      items = items.filter(
        (ex) => !ex.equipment?.length || ex.equipment.some((e) => equipment!.includes(e)),
      );
    }
    if (filters?.bodyParts?.length) {
      items = items.filter((ex) => ex.body_parts?.some((bp) => filters.bodyParts!.includes(bp)));
    }
  ...
  async mergeExercises(
    ...
    try {
      ...
      logger.error(
        "Falha ao unir exercícios de forma inteligente",
        error as Error,
        "exerciseService",
      );
