...
export const SessionService = {
	async saveSession(session: ExerciseSession) {
		try {
			const result = await exerciseSessionsApi.create({
				...
				posture_issues_summary: session.postureIssues.reduce(
					(acc, issue) => {
			...
			logger.error(
				"Erro ao salvar sessão de exercício",
				error,
				"SessionService",
			);
			throw error;
	...
	async getPatientHistory(patientId: string, exerciseId: string) {
		try {
			const result = await exerciseSessionsApi.list({
				patientId,
				exerciseId,
				limit: 20,
			});
	...
	async getPatientStats(patientId: string) {
		try {
			...
			logger.error(
				"Erro ao buscar estatísticas do paciente",
				error,
				"SessionService",
			);
			return null;
