...
export class PoseAnalytics {
  ...
  completeSession(session: ExerciseSession): void {
    ...
    logger.info(
      "[PoseAnalytics] Sessão completada",
      {
        sessionId: session.id,
        duration,
        repetitions: session.repetitions,
  ...
  abandonSession(session: ExerciseSession): void {
    ...
    logger.info(
      "[PoseAnalytics] Sessão abandonada",
      {
        sessionId: session.id,
        duration,
      },
 ...
 * Logar repetição completada
 */
export function logRepetitionCompleted(exerciseId: string, repNumber: number, quality: any): void {
  const rep: any = {
    duration: 0,
    quality,
    endPhase: "completed",
