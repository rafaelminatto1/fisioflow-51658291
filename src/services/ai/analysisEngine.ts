...
export class AnalysisEngine {
  ...
  constructor(exerciseType: ExerciseType = ExerciseType.SQUAT) {
    ...
    this.exerciseTemplate =
      getExerciseTemplate(exerciseType) || getExerciseTemplate(ExerciseType.SQUAT);
  ...
  private calculateRangeOfMotion(jointAngles: Record<MainJoint, any>): RomData {
    ...
    for (const triad of this.exerciseTemplate.angleTriads || []) {
      const angleData =
        jointAngles[this.exerciseTemplate.primaryKeypoints?.[triad.pivot] as MainJoint];
    ...
    if (romData.length > 0) {
      ...
      const avgPercentage =
        romData.reduce((sum, r) => sum + r.percentageOfNormal, 0) / romData.length;
 ...
 */
export function calculateCombinedScore(formScore: number, stabilityScore: number): number {
