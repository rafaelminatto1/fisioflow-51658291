...
export class AnalysisEngine {
	...
	constructor(exerciseType: ExerciseType = ExerciseType.SQUAT) {
		...
		this.exerciseTemplate =
			getExerciseTemplate(exerciseType) ||
	...
	processFrame(poseDetection: PoseDetection): AnalysisResult {
		if (
			!this.isActive ||
			!poseDetection.landmarks ||
			poseDetection.landmarks.length === 0
		) {
		...
		const postureIssues = this.detectPostureIssues(
			smoothedLandmarks,
			jointAngles,
		);
		...
		return {
			pose: poseDetection,
			jointAngles: new Map(
				Object.entries(jointAngles).map(([k, v]) => [k, v] as [MainJoint, any]),
			),
			postureIssues,
	...
	private smoothLandmarks(landmarks: PoseLandmark[]): PoseLandmark[] {
		...
		return landmarks.map((lm, index) => {
			...
			return {
				x:
					lm.x * (1 - DEFAULT_CONFIG.smoothingFactor) +
					prev.x * DEFAULT_CONFIG.smoothingFactor,
				y:
					lm.y * (1 - DEFAULT_CONFIG.smoothingFactor) +
					prev.y * DEFAULT_CONFIG.smoothingFactor,
				z: lm.z
					? lm.z! * (1 - DEFAULT_CONFIG.smoothingFactor) +
						prev.z! * DEFAULT_CONFIG.smoothingFactor
					: undefined,
				visibility:
					(lm.visibility || 0) * (1 - DEFAULT_CONFIG.smoothingFactor) +
					(prev.visibility || 0) * DEFAULT_CONFIG.smoothingFactor,
			};
		});
	}
	...
	private calculateJointAngles(
		...
		for (const triad of this.exerciseTemplate.angleTriads || []) {
			...
			if (
				...
				angles[
					this.exerciseTemplate.primaryKeypoints?.[triad.pivot] as MainJoint
				] = {
					current: angle,
					...
					target: this.exerciseTemplate.thresholds?.[triad.pivot as string] || {
						min: 0,
						max: 180,
					},
		...
		for (const [joint, angleData] of Object.entries(jointAngles)) {
			if (angleData && typeof angleData === "object") {
				...
				if (!angle.inRange && angle.target) {
					...
					const deviation = this.calculateAngleDeviation(
						angle.current,
						target.min,
						target.max,
					);
	 ...
	 */
	private calculateAngleDeviation(
		current: number,
		min: number,
		max: number,
	): number {
	...
	private calculateRangeOfMotion(jointAngles: Record<MainJoint, any>): RomData {
		...
		for (const triad of this.exerciseTemplate.angleTriads || []) {
			...
			if (angleData && typeof angleData === "object") {
				...
				romData.push({
					joint: this.exerciseTemplate.primaryKeypoints?.[
						triad.pivot
					] as MainJoint,
		...
		if (romData.length > 0) {
			const avgRom =
				romData.reduce((sum, r) => sum + r.rom, 0) / romData.length;
			const avgPercentage =
				romData.reduce((sum, r) => sum + r.percentageOfNormal, 0) /
				romData.length;
	...
	private updateState(
		...
		const now = Date.now();
		if (now - this.state.lastScoreUpdate > 500) {
			// A cada 500ms
	...
	private notifyCallbacks(data: {
		...
		if (this.state.callbacks.onPoseDetected) {
			this.state.callbacks.onPoseDetected({
				pose: data.pose,
				jointAngles: new Map(
					Object.entries(data.jointAngles) as [MainJoint, any],
				),
