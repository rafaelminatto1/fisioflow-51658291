...
export class RepetitionCounter {
  ...
  private processPhaseUpDown(_landmarks: PoseLandmark[]): {
    ...
    const phaseChanged = this.state.currentPhase !== null && newPhase !== this.state.currentPhase;
    ...
    if (phaseChanged) {
      ...
      const shouldCount =
        (previousPhase === MovementPhase.UP && newPhase === MovementPhase.DOWN) ||
  ...
  private processPhaseLeftRight(_landmarks: PoseLandmark[]): {
    ...
    if (phaseChanged) {
      ...
      const shouldCount =
        (previousPhase === MovementPhase.LEFT && newPhase === MovementPhase.RIGHT) ||
  ...
  private processPhaseFlexionExtension(_landmarks: PoseLandmark[]): {
    ...
    if (phaseChanged) {
      ...
      const shouldCount =
        (previousPhase === MovementPhase.FLEXION && newPhase === MovementPhase.EXTENSION) ||
  ...
  private completeRepetition(): Repetition {
    ...
    const duration = this.state.phaseStartTime ? now - this.state.phaseStartTime : 0;
