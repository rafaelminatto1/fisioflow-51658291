...
export class RepetitionCounter {
	...
	constructor(exerciseType: ExerciseType = ExerciseType.SQUAT) {
		this.state = {
			...
			thresholds:
				...
				DEFAULT_THRESHOLDS[ExerciseType.SQUAT],
	...
	setExerciseType(type: ExerciseType): void {
		...
		this.state.thresholds =
			DEFAULT_THRESHOLDS[type] || DEFAULT_THRESHOLDS[ExerciseType.SQUAT];
		this.reset();
	}
	...
	private processPhaseUpDown(landmarks: PoseLandmark[]): {
		...
		const phaseChanged =
			this.state.currentPhase !== null && newPhase !== this.state.currentPhase;
		...
		if (phaseChanged) {
			...
			const shouldCount =
				(previousPhase === MovementPhase.UP &&
					newPhase === MovementPhase.DOWN) ||
	...
	private processPhaseLeftRight(landmarks: PoseLandmark[]): {
		...
		if (phaseChanged) {
			...
			const shouldCount =
				(previousPhase === MovementPhase.LEFT &&
					newPhase === MovementPhase.RIGHT) ||
				(previousPhase === MovementPhase.RIGHT &&
					newPhase === MovementPhase.LEFT);
	...
	private processPhaseFlexionExtension(_landmarks: PoseLandmark[]): {
		...
		if (phaseChanged) {
			...
			const shouldCount =
				(previousPhase === MovementPhase.FLEXION &&
					newPhase === MovementPhase.EXTENSION) ||
				(previousPhase === MovementPhase.EXTENSION &&
					newPhase === MovementPhase.FLEXION);
	...
	private processHoldPosition(_landmarks: PoseLandmark[]): {
		...
		const now = Date.now();
		const timeInPhase = this.state.phaseStartTime
