...
export async function analyzePainEvolution(
  ...
  return {
    overallTrend:
      currentPainLevel <= 3 ? "improving" : currentPainLevel >= 7 ? "worsening" : "stable",
...
export async function evaluateTreatmentResponse(
  ...
  return {
    responseRating:
      currentPainLevel <= 3 ? "positive" : currentPainLevel >= 7 ? "negative" : "neutral",
    summary: response,
    recommendations: [],
    nextSteps: [],
  };
...
export async function findSimilarClinicalRecords(
  ...
  const result = await callAIService<{ query: string; limit: number }, SemanticSearchResponse>(
    "semanticSearch",
    { query, limit },
  );
  return result;
...
export async function generateClinicalReport(input: ClinicalReportInput): Promise<string> {
  const sessionsText = input.sessions
    .map((s) => {
      const exercisesText =
        s.exercises?.map((e) => e.name + (e.notes ? " (" + e.notes + ")" : "")).join(", ") || "N/A";
      return `Data: ${s.date}
