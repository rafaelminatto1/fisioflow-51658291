...
const SOCIAL_CAPTION_PROMPT_TEMPLATE = (
	type: string,
	context: Record<string, any>,
) => `
...
const DEFAULT_REVIEW_TEMPLATES = {
	professional:
		"Olá {nome}! 🌟 Gostaríamos de saber sua opinião sobre nosso atendimento. Sua avaliação ajuda muito! Agradeçemos por seu tempo. ⭐ {review_link}",
	friendly:
		"Oi {nome}! 😊 Como foi sua experiência? Se puder, deixe uma avaliaçãozinha pra gente! Faz toda a diferença! ❤️ {review_link}",
	casual:
		"E aí {nome}! 😄 Conta pra gente como foi? Deixa aquela estrela aí que a gente fica muito feliz! ⭐⭐⭐⭐⭐ {review_link}",
};

const DEFAULT_BIRTHDAY_TEMPLATES = {
	formal:
		"Estimado(a) {nome}, a equipe da clínica deseja a você um feliz aniversário repleto de saúde, paz e realizações. 🎂",
	friendly:
		"Olá {nome}! 🎉 Tudo de bom? Hoje é seu dia especial! Que este novo ciclo traga muita saúde, alegria e conquistas! Parabéns! 🎂🥳",
...
const DEFAULT_RECALL_TEMPLATES = {
	health:
		"Olá {nome}! Sentimos sua falta. Já faz {dias} dias desde sua última visita. Que tal agendar um check-up? Sua saúde é nossa prioridade! 💪",
	followup:
		"Oi {nome}! 👋 Estamos pensando em você! Faz {dias} dias que não vimos por aqui. Como você está? Vamos marcar uma sessão?",
	promotion:
		"Olá {nome}! 🎁 Estamos com uma promoção especial pra você! Faz {dias} dias que nos visitou. Agende agora e ganhe 10% OFF! 📞",
};
...
export async function generateReviewTemplate(
	input: GenerateReviewTemplateInput,
	...
	try {
		...
		const fallbackTemplate =
			DEFAULT_REVIEW_TEMPLATES[tone as keyof typeof DEFAULT_REVIEW_TEMPLATES];
		...
		console.error(
			"[MarketingAIService] Error generating review template:",
			error,
		);
		// Fallback para templates pré-definidos
		const tone = input.tone || "professional";
 ...
 */
export async function generateBirthdayTemplate(
	input: GenerateBirthdayTemplateInput,
): Promise<GeneratedTemplateResponse> {
	try {
		const result = await requestMarketingJson<{
			template?: string;
		...
		const fallbackTemplate =
			DEFAULT_BIRTHDAY_TEMPLATES[
				relationship as keyof typeof DEFAULT_BIRTHDAY_TEMPLATES
			];
		...
		console.error(
			"[MarketingAIService] Error generating birthday template:",
			error,
		);
		// Fallback para templates pré-definidos
		const relationship = input.relationship || "friendly";
 ...
 */
export async function generateRecallTemplate(
	input: GenerateRecallTemplateInput,
): Promise<GeneratedTemplateResponse> {
	try {
		const result = await requestMarketingJson<{
			template?: string;
		...
		const fallbackTemplate =
			DEFAULT_RECALL_TEMPLATES[
				motivationType as keyof typeof DEFAULT_RECALL_TEMPLATES
			];
		...
		console.error(
			"[MarketingAIService] Error generating recall template:",
			error,
		);
		// Fallback para templates pré-definidos
		const motivationType = input.motivationType || "health";
 ...
 */
export async function generateFisiolinkTemplates(
	input: GenerateFisiolinkTemplateInput,
): Promise<{
	success: boolean;
	templates?: {
		short: string;
	...
	try {
		...
		console.error(
			"[MarketingAIService] Error generating FisioLink templates:",
			error,
		);
		// Fallback para templates pré-definidos
		const clinicName = input.clinicName || "Nossa Clínica";
...
export async function generateSocialCaption(
	...
	try {
		...
		console.error(
			"[MarketingAIService] Error generating social caption:",
			error,
		);
		// Fallback para templates pré-definidos
		return {
 ...
 */
export async function generateMythVsTruth(
	input: GenerateMythVsTruthInput,
): Promise<{
	success: boolean;
	myth?: string;
	truth?: string;
	...
	try {
		...
		console.error(
			"[MarketingAIService] Error generating myth vs truth:",
			error,
		);
		return {
			success: true,
			...
			explanation:
				"Consulte seu fisioterapeuta para uma orientação baseada em evidências.",
...
export async function generateMarketingContent(
	...
	switch (input.type) {
		case "review":
			return generateReviewTemplate(
				input.context as GenerateReviewTemplateInput,
			);
		case "birthday":
			return generateBirthdayTemplate(
				input.context as GenerateBirthdayTemplateInput,
			);
		case "recall":
			return generateRecallTemplate(
				input.context as GenerateRecallTemplateInput,
			);
		case "fisiolink": {
			const fisiolinkResult = await generateFisiolinkTemplates(
				input.context as GenerateFisiolinkTemplateInput,
			);
			return {
...
export async function generateSOAPTemplate(
	input: GenerateSOAPTemplateInput,
): Promise<SOAPTemplateResponse> {
	try {
		const result = await requestMarketingJson<{
			soap?: {
				subjective?: string;
		...
		return {
			...
			soap: {
				subjective:
					"Paciente relata melhora gradual nos sintomas. Discreta ausência de dor em repouso.",
				objective:
					"Amplitude de movimento preservada. Força muscular dentro dos limites esperados para fase do tratamento.",
				assessment:
					"Resposta positiva ao tratamento progressivo. Continuar com protocolo atual.",
		...
		console.error(
			"[MarketingAIService] Error generating SOAP template:",
			error,
		);
		// Fallback para template SOAP básico
		return {
