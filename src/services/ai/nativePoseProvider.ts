...
class NativePoseProvider implements PoseProvider {
	...
	async initialize(): Promise<void> {
		if (!this.isMobile) {
			logger.info(
				"[NativePoseProvider] Plataforma não nativa, ignorando inicialização.",
				null,
				"NativePoseProvider",
			);
			return;
		...
		try {
			logger.info(
				"[NativePoseProvider] Inicializando provedor nativo no Capacitor...",
				null,
				"NativePoseProvider",
			);
			...
			this.isLoaded = true;
			logger.info(
				"[NativePoseProvider] Provedor nativo inicializado com sucesso.",
				null,
				"NativePoseProvider",
			);
		} catch (err) {
			logger.error(
				"Erro ao inicializar provedor nativo",
				err,
				"NativePoseProvider",
			);
			this.isLoaded = false;
	...
	stopStream(): void {
		if (!this.isLoaded) return;
		logger.info(
			"[NativePoseProvider] Parando stream nativo.",
			null,
			"NativePoseProvider",
		);
	}
