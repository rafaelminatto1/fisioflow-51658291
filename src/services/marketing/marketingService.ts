...
export const checkMarketingConsent = async (
  ...
  try {
    ...
    if (consent.expires_at && new Date(consent.expires_at).getTime() < Date.now()) return false;
    if (consentType === 'any') {
  ...
  try {
    ...
    if (!hasConsent && !params.isAnonymized) {
      return {
        success: false,
        url: '',
        error: 'Paciente não possui consentimento de marketing para esta exportação',
      };
    }

    const finalFileName = fileName || `marketing_${params.patientId}_${Date.now()}.mp4`;
    const file = new File([blob], finalFileName, { type: blob.type || 'video/mp4' });
    const upload = await uploadToR2(file, `marketing-exports/${params.organizationId}`);

    const record = (await marketingApi.exports.create({
    ...
    logger.info('[MarketingService] Export record created', { id: record.id }, 'marketingService');
  ...
  try {
    ...
    if (result.data?.file_path) {
      await deleteFromR2(result.data.file_path).catch((err) => {
        logger.warn('[MarketingService] Could not delete file from R2', err, 'marketingService');
      });
    }
  } catch (error) {
...
export const getPatientMarketingExports = async (patientId: string): Promise<MarketingExportRecord[]> => {
  try {
...
export const getOrganizationMarketingExports = async (): Promise<MarketingExportRecord[]> => {
  try {
    return (await marketingApi.exports.list()).data ?? [];
  } catch (error) {
    logger.error('[MarketingService] Error getting organization exports', error, 'marketingService');
    return [];
  }
};

export const getReviewAutomationConfig = async (): Promise<ReviewAutomationConfig | null> => {
  try {
...
export const getRecallCampaigns = async (_organizationId: string): Promise<RecallCampaign[]> => {
  try {
...
export const createReferralCode = async (
  ...
  try {
    const created = (await marketingApi.referrals.create({
...
export const redeemReferralCode = async (
  ...
  try {
    return (await marketingApi.referrals.redeem({ code, new_patient_id: newPatientId })).data;
  } catch (error) {
...
export const getPatientReferralCode = async (patientId: string): Promise<ReferralCode | null> => {
  try {
...
export const getReferralStats = async (_organizationId: string): Promise<{
  totalCodes: number;
  activeCodes: number;
  ...
  try {
    ...
    return { totalCodes: 0, activeCodes: 0, totalRedemptions: 0, pendingRewards: 0, topReferrers: [] };
  }
};
...
export const getFisioLinkByOrganization = async (_organizationId: string): Promise<FisioLinkConfig | null> => {
  try {
...
export const trackFisioLinkClick = async (slug: string, button: string): Promise<void> => {
  try {
...
export const getBirthdayAutomationConfig = async (_organizationId: string): Promise<BirthdayAutomationConfig> => {
  try {
...
export const calculateMarketingROI = async (
  ...
  try {
    const metrics = (await marketingApi.roi({
