...
class OfflineSyncService {
	...
	private async init(): Promise<void> {
		...
		try {
			...
			logger.warn(
				"Background sync not available, using polling fallback",
				error,
				"offlineSync",
			);
			this.startPeriodicSync();
		...
		this.syncTimer = setInterval(() => {
			if (this.config.enabled && navigator.onLine && !this.isSyncing) {
				this.syncNow().catch((err) =>
					logger.error("Periodic sync failed", err, "offlineSync"),
				);
			}
		}, interval);
	}
	...
	public async cacheCriticalData(): Promise<void> {
		if (typeof navigator !== "undefined" && !navigator.onLine) {
			logger.warn(
				"[OfflineSyncService] Cannot cache data while offline",
				undefined,
				"offlineSync",
			);
			return;
		...
		try {
			if (!_loggedOfflineSyncStart) {
				...
				logger.info(
					"[OfflineSyncService] Starting critical data caching",
					undefined,
					"offlineSync",
				);
			}
			...
			try {
				...
				if ((error as { code?: string })?.code === "permission-denied") {
					logger.debug(
						"[OfflineSyncService] Permission denied for appointments (User might be patient/estagiario). Skipping.",
						undefined,
						"offlineSync",
					);
					appointments = [];
					...
					logger.warn(
						"[OfflineSyncService] Error fetching appointments",
			...
			if (appointments && appointments.length > 0) {
				...
				await tx.done;
				logger.debug(
					`[OfflineSyncService] Cached ${appointments.length} appointments`,
					{ count: appointments.length },
					"offlineSync",
				);
			} else {
				logger.debug(
					"[OfflineSyncService] No appointments to cache for today",
					undefined,
					"offlineSync",
				);
			}
			...
			try {
				...
				logger.warn(
					"[OfflineSyncService] Error fetching exercises from Workers API",
					{ error },
					"offlineSync",
				);
				exercises = [];
			...
			if (exercises && exercises.length > 0) {
				...
				await tx.done;
				logger.debug(
					`[OfflineSyncService] Cached ${exercises.length} exercises`,
			...
			if (!_loggedOfflineSyncComplete) {
				...
				logger.debug(
					"[OfflineSyncService] Critical data caching complete",
					undefined,
					"offlineSync",
				);
			}
			...
			logger.error(
				"[OfflineSyncService] Error caching critical data",
				error,
				"offlineSync",
			);
		}
		...
		try {
			...
			for (const action of allActions) {
				...
				if (action.synced) {
					...
					action.retryCount >= (this.config.maxRetries || DEFAULT_MAX_RETRIES)
				) {
					failed++;
				} else {
					pending++;
			...
			logger.error(
				"[OfflineSyncService] Error getting sync stats",
				error,
				"offlineSync",
			);
			return {
	...
	private emit(event: SyncEvent): void {
		this.listeners.forEach((callback) => {
			try {
				...
				logger.error(
					"[OfflineSyncService] Listener error",
					error,
					"offlineSync",
				);
			}
	...
	useEffect(() => {
		...
		const unsubscribe = service.subscribe((event) => {
			...
			if (
				...
				event.data?.stats?.pendingActions === 0
			) {
