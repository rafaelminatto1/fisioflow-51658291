...
export class AppointmentService {
				...
				"Appointments API response received",
				{
					...
					sampleItem: response.data?.[0]
						? {
								...
								hasDate: !!(
									response.data[0].date || response.data[0].appointment_date
								),
								hasTime: !!(
									response.data[0].start_time ||
									response.data[0].appointment_time
								),
							}
						: null,
				},
				...
				if (validation.success) {
					...
					validAppointments.push({
						...
						createdAt: validData.created_at
							...
							: new Date(),
						updatedAt: validData.updated_at
							? new Date(validData.updated_at)
							: new Date(),
					...
					const maskedName = item.patient_name
					...
					logger.error(
						`Appointment validation failed for ID ${item.id}`,
						{
							...
							validationError: JSON.stringify(
								validation.error?.issues || "Unknown error",
								null,
								2,
							),
						},
	...
	static async createAppointment(
		...
		try {
			...
			// Validation
			if (!data.patient_id)
				throw AppError.badRequest("ID do paciente é obrigatório");
			...
			if (!dateValidation.success)
				throw AppError.badRequest(`Data inválida: ${rawDate}`);

			const timeValidation = timeSchema.safeParse(rawTime);
			if (!timeValidation.success)
				throw AppError.badRequest(`Horário inválido: ${rawTime}`);

			const endTime = calculateEndTime(rawTime, data.duration || 60);
			const sessionType =
				data.type === "Fisioterapia" || data.type === "fisioterapia"
					? "individual"
					: "group";
			...
			const appointment: AppointmentBase = {
				...
				date: parseResponseDate(
					newAppointment.date || newAppointment.appointment_date,
				),
				time:
					...
					"00:00",
				duration:
					newAppointment.duration_minutes || newAppointment.duration || 60,
			...
			if (newAppointment.payment_status === "paid") {
				await AppointmentService.syncFinancialTransaction({
					...
					payment_amount: (
						data as AppointmentFormData & { payment_amount?: number }
					).payment_amount,
					payment_method: (
						data as AppointmentFormData & { payment_method?: string }
					).payment_method,
				});
			}
	...
	static async updateAppointment(
		...
		try {
			...
			if (updates.therapist_id !== undefined)
				updateData.therapist_id = updates.therapist_id;
			...
			if (updates.payment_amount !== undefined)
				updateData.payment_amount = updates.payment_amount;
			if (updates.ignoreCapacity !== undefined)
				updateData.ignoreCapacity = updates.ignoreCapacity;
			...
			if (updateDate) {
				...
				if (!dateValidation.success)
					throw AppError.badRequest(`Data inválida: ${updateDate}`);
			...
			if (updateTime) {
				...
				if (!timeValidation.success)
					throw AppError.badRequest(`Horário inválido: ${updateTime}`);
			...
			if (updateEndTime) {
				...
				if (!timeValidation.success)
					throw AppError.badRequest(
						`Horário de término inválido: ${updateEndTime}`,
					);
			...
			logAgentEvent({
				location:
					"src/services/appointmentService.ts:updateAppointment:preparedData",
			...
			logAgentEvent({
				location:
					"src/services/appointmentService.ts:updateAppointment:response",
				message: "Received update response",
				data: {
					appointmentId: id,
					responseTime:
						fetchedUpdatedAppointment?.start_time ??
						fetchedUpdatedAppointment?.appointment_time,
			...
			const updatedAppointment: AppointmentBase = {
				...
				date: parseResponseDate(
					fetchedUpdatedAppointment.date ||
						fetchedUpdatedAppointment.appointment_date,
				),
				time:
					fetchedUpdatedAppointment.start_time ||
					fetchedUpdatedAppointment.appointment_time,
		...
		for (const appt of data) {
			try {
				await appointmentsApi.cancel(
					appt.id,
					"Cancelados em lote (todos de hoje)",
				);
				cancelled++;
			} catch {
				errors++;
	...
	private static async syncFinancialTransaction(
		...
		if (
			...
			appointment.session_package_id
		) {
			return;
		}
		...
		try {
			...
			const existing = await FinancialService.findTransactionByAppointmentId(
				appointment.id!,
			);
			...
			logger.error(
				"Failed to sync financial transaction",
				error,
				"AppointmentService",
			);
		}
