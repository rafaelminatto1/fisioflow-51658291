...
export const logExportEvent = async (options: ExportOptions) => {
  try {
    ...
    logger.info(
      `[AUDIT] Export logged via Workers: ${options.professionalName} → ${options.patientName} (${options.format}/${options.recipientType})`,
      undefined,
      "reportExportService",
    );
  } catch (error) {
    ...
    logger.info(
      `[AUDIT] Export generated by ${options.professionalName} for ${options.patientName} (${options.format}/${options.recipientType})`,
      undefined,
      "reportExportService",
    );
  }
...
export const generateDoctorLetterText = (
  patientName: string,
  professionalName: string,
  findings: string[],
  evolution: string,
