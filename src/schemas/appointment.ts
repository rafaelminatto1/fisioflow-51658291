 ...
 */
function getPatientName(patient: { full_name?: string | null } | null | undefined): string {
...
export const VerifiedAppointmentSchema = AppointmentSchema.transform((data) => {
  ...
  if (dateSource) {
    ...
    if (parsedDate && !isNaN(parsedDate.getTime())) {
      ...
      fisioLogger.warn(
        "Invalid date format, using today as fallback",
        { dateSource },
        "appointment-schema",
      );
      finalDate = new Date();
    ...
    fisioLogger.warn(
      "Date field is missing or null, using today as fallback",
      undefined,
      "appointment-schema",
    );
    finalDate = new Date();
  ...
  if (isNaN(finalDate.getTime())) {
    fisioLogger.warn(
      "Parsed date is invalid, using today as fallback",
      { dateSource },
      "appointment-schema",
    );
    finalDate = new Date();
