...
export const clinicalRules: AssessmentRule[] = [
  // REGRAS DE JOELHO
  {
    ...
    suggestion: {
      ...
      description:
        "Lachman positivo indica alta probabilidade de ruptura do LCA. Considere realizar o teste de Pivot Shift para avaliar instabilidade rotacional.",
      type: "warning",
  ...
  },
  {
    ...
    suggestion: {
      ...
      description:
        "McMurray positivo sugere lesão de menisco. Realize o teste de Thessaly para confirmar o achado funcional.",
      type: "info",
  ...
  // REGRAS DE QUADRIL
  {
    ...
    suggestion: {
      ...
      description:
        "Teste de Thomas positivo indica tensão no Ilio-psoas ou Reto Femoral. Integre mobilidade de quadril no plano de tratamento.",
      type: "info",
  ...
  // REGRAS DE COLUNA
  {
    ...
    suggestion: {
      ...
      description:
        "Lasègue positivo entre 30-70° sugere compressão radicular. Realize o teste de Slump para confirmar a neurodinâmica.",
      type: "warning",
      testToPerform: ["slump"],
...
export const getClinicalSuggestions = (results: Record<string, any>): ClinicalSuggestion[] => {
  return clinicalRules.filter((rule) => rule.condition(results)).map((rule) => rule.suggestion);
};
