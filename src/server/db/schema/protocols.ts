...
// ===== EXERCISE PROTOCOLS =====
export const exerciseProtocols = pgTable(
	"exercise_protocols",
	{
	...
	(table) => ({
		...
		organizationIdIdx: index("idx_exercise_protocols_organization_id").on(
			table.organizationId,
		),
		protocolTypeIdx: index("idx_exercise_protocols_protocol_type").on(
			table.protocolType,
		),
	}),
);

export const exerciseProtocolsRelations = relations(
	exerciseProtocols,
	({ many }) => ({
...
export const protocolExercises = pgTable(
	"protocol_exercises",
	{
		id: uuid("id").primaryKey().defaultRandom(),
	...
	(table) => ({
		...
		exerciseIdIdx: index("idx_protocol_exercises_exercise_id").on(
			table.exerciseId,
		),
	}),
);

export const protocolExercisesRelations = relations(
	protocolExercises,
	({ one }) => ({
