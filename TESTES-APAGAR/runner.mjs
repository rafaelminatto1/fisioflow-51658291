...
const nomes = [
  "Teste Evolução Silva",
  "Teste Evolução Costa",
  "Teste Evolução Santos",
  "Teste Evolução Oliveira",
  "Teste Evolução Souza",
  "Teste Evolução Rodrigues",
  "Teste Evolução Ferreira",
  "Teste Evolução Alves",
  "Teste Evolução Pereira",
  "Teste Evolução Lima",
];
...
(async () => {
  ...
  for (let i = 0; i < nomes.length; i++) {
    ...
    await page.evaluate(async (nomePaciente) => {
      ...
      const token =
        localStorage.getItem("auth-token") || localStorage.getItem("supabase.auth.token");
