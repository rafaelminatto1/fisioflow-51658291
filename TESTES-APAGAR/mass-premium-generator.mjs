...
(async () => {
  ...
  const nomes = [
    "Dr. Robot - Paciente Alpha",
    "Dr. Robot - Paciente Beta",
    "Dr. Robot - Paciente Gamma",
    "Dr. Robot - Paciente Delta",
    "Dr. Robot - Paciente Epsilon",
    "Dr. Robot - Paciente Zeta",
    "Dr. Robot - Paciente Eta",
    "Dr. Robot - Paciente Theta",
    "Dr. Robot - Paciente Iota",
  ...
  for (let i = 0; i < nomes.length; i++) {
    ...
    try {
      ...
      const token = await page.evaluate(() => {
        ...
        for (let i = 0; i < localStorage.length; i++) {
          ...
          if (key && (key.includes("auth-token") || key.includes("supabase.auth.token"))) {
            try {
              t = JSON.parse(localStorage.getItem(key)).access_token;
              break;
            } catch (e) {
      ...
      if (token) {
        ...
        const patientId =
          urlParts[urlParts.length - 1] === "patients" ? null : urlParts[urlParts.length - 1];
        ...
        if (patientId && patientId.length > 20) {
          ...
          const downloadPath = path.join(
            TEST_DIR,
            `Relatorio_Premium_${pName.replace(/ /g, "_")}.pdf`,
          );
