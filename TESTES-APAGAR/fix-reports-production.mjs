...
(async () => {
  ...
  const patients = await page.evaluate(() => {
    return Array.from(document.querySelectorAll('[data-patient-id], button, a'))
      ...
      .map(el => {
          const id = el.getAttribute('data-patient-id') || el.getAttribute('data-id') || (el.href ? el.href.split('/').pop() : null);
  ...
  for (const p of uniquePatients) {
    ...
    try {
      ...
      const downloadPath = path.join(TEST_DIR, `RELATORIO_PREMIUM_OFICIAL_${p.name.replace(/ /g, '_')}.pdf`);
