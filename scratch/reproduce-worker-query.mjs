...
async function reproduce() {
  ...
  try {
    ...
    if (result.rows.length > 0) {
      ...
      const largeFields = Object.entries(sample).filter(
        ([k, v]) => v && typeof v === "string" && v.length > 1000,
      );
      if (largeFields.length > 0) {
        console.log(
          "Large fields found:",
          largeFields.map(([k, v]) => `${k} (${v.length} chars)`),
        );
      }
    }
    ...
    const countResults = await baseSql.transaction([
      baseSql.query(`SELECT set_config('app.org_id', $1, true)`, [orgId]),
      baseSql.query(
        `SELECT count(*) FROM "exercises" LEFT JOIN "exercise_categories" ON "exercises"."category_id" = "exercise_categories"."id" WHERE ("exercises"."is_active" = true AND "exercises"."is_public" = true)`,
        [],
      ),
    ]);
