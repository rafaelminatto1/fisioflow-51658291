...
test.describe("Agenda - CRUD Completo", () => {
  ...
  test("deve exibir agenda corretamente", async ({ page }) => {
    const viewButtons = page.locator(
      'button:has-text("D"), button:has-text("S"), button:has-text("Dia"), button:has-text("Semana")',
    );
    await expect(viewButtons.first()).toBeVisible({ timeout: 10000 });
  });

  test("deve expor ação de novo agendamento", async ({ page }) => {
    const novoButton = page
      .locator('[data-testid="new-appointment"], button[aria-label*="Novo Agendamento" i]')
      .first();
  ...
  test("deve permitir abrir filtros quando disponíveis", async ({ page }) => {
    const filterButton = page
      .locator(
        'button:has-text("Filtrar"), button:has([data-lucide="filter"]), [aria-label*="filt" i]',
      )
      .first();
