...
test.describe("Validação de Produção - MediaPipe", () => {
  test("deve carregar ativos do MediaPipe sem erro 404", async ({ page }) => {
    ...
    const baseUrl =
      process.env.PLAYWRIGHT_TEST_BASE_URL || "https://fisioflow-professional.web.app";
    ...
    expect(
      mediapipeErrors,
      `Encontrados erros de carregamento no MediaPipe: ${mediapipeErrors.join(", ")}`,
    ).toHaveLength(0);
    ...
    expect(
      hasWrongVersion,
      "A aplicação ainda está tentando carregar a versão antiga 0.10.22",
    ).toBe(false);
