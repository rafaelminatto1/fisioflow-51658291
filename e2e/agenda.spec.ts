...
test.describe("Agenda de Fisioterapia - E2E Tests", () => {
  ...
  test("deve carregar a página de agenda com sucesso", async ({ page }) => {
    ...
    const novoAgendamentoButton = page.locator(
      'button:has-text("Novo Agendamento"), button:has-text("Novo")',
    );
    const calendarElement = page.locator('.calendar, [class*="calendar"], [class*="Calendar"]');
    const agendaText = page
      .locator(
        'h1:has-text("Agenda"), h2:has-text("Agenda"), text=/Agenda|Agendamentos|Calendário/i',
      )
      .first();
    ...
    await expect
      .poll(
        async () => {
          const [hasNovoButton, hasCalendar, hasAgendaText, bodyVisible, onSchedule] =
            await Promise.all([
  ...
  test("deve criar novo agendamento com sucesso", async ({ page }) => {
    ...
    const novoAgendamentoButton = page
  ...
  test("deve filtrar agendamentos por status", async ({ page }) => {
    ...
    if (hasFilterButton) {
      ...
      console.log(
        hasAnyFilter ? "✅ Elementos de filtro encontrados" : "⚠ Filtros não encontrados",
      );
    }
  });
  ...
  test("deve validar campos obrigatórios no formulário", async ({ page }) => {
    ...
    if (await novoAgendamentoButton.isVisible({ timeout: 5000 })) {
      ...
      const submitButton = page
        .locator('button[type="submit"], button:has-text(/Salvar|Criar/)')
        .first();
  ...
  test("deve permitir criar paciente rápido durante agendamento", async ({ page }) => {
    ...
    if (await novoAgendamentoButton.isVisible({ timeout: 5000 })) {
      ...
      if (await patientCombobox.isVisible({ timeout: 2000 })) {
        ...
        console.log(
          hasCreateOption ? "✅ Opção de criar paciente encontrada" : "⚠ Opção não encontrada",
        );
