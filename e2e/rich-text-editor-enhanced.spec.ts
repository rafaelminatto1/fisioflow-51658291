...
test.describe("Enhanced Rich Text Editor Validation", () => {
  test.beforeEach(async ({ page }) => {
    ...
    const appointmentCard = page
      .locator(
        '[role="button"][aria-label*="às"], .calendar-appointment-card, .appointment-card, [data-testid="appointment-card"]',
      )
      .first();
    ...
    const startButton = page
      .locator('button:has-text("Iniciar atendimento"), button:has-text("Iniciar Avaliação")')
      .first();
