...
test.describe("Agendamento — alteração de status em produção", () => {
  ...
  test("inspecionar cards de agendamento e estrutura de status", async ({ page }) => {
    ...
    const cardInfo = await page.evaluate(() => {
      ...
      const statusButtons = Array.from(document.querySelectorAll("button"))
        .filter((b) =>
          /aguardando|confirmado|em atendimento|concluído|cancelado|cancelar/i.test(
            b.textContent || "",
          ),
        )
  ...
  test("alterar status de agendamento via context menu (right-click)", async ({ page }) => {
    ...
    const cardSelectors = ['[data-week-appointment="true"]', "[data-appointment-popover-anchor]"];
    ...
    for (const sel of cardSelectors) {
      ...
      if (await card.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        const statusSubTrigger = page
          .locator('[role="menuitem"]:has-text("status"), [role="menuitem"]:has-text("Status")')
          .first();
    ...
    if (!cardFound) {
      console.log(
        "⚠️ Nenhum card de agendamento encontrado — agenda pode estar vazia para a data atual",
      );
    ...
    console.log(
      "Requests capturadas:",
      statusRequests.length > 0 ? statusRequests : "(nenhum PATCH)",
    );
