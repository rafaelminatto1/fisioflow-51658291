...
test.describe("Payment Flow - New Package", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(
      page.locator('button:has-text("Novo Agendamento"), button:has-text("Novo")').first(),
    ).toBeVisible({ timeout: 20000 });
    console.log("Agenda loaded (Novo button visible)");
  });

  test("should create a new package with custom options during appointment scheduling", async ({
    page,
  }) => {
    test.slow();
    ...
    await expect(
      page.getByRole("heading", { name: /Novo Agendamento|Novo Atendimento/i }).first(),
    ).toBeVisible();
    ...
    const option = page
      ...
      .filter({ hasText: "Test Patient Playwright" })
      .first();
    ...
    const popoverContent = page
      .locator('[role="dialog"]')
      .filter({ hasText: "Vender Novo Pacote" });
    const templateTrigger = popoverContent.locator('button[role="combobox"]').first();
    await templateTrigger.click();
    const templateUnavailable = popoverContent
      .getByText(/Erro ao carregar modelos|Nenhum modelo encontrado/i)
      .first();
