...
test.describe("Módulo Marketing", () => {
  ...
  test("Deve exibir link para FisioLink correto", async ({ page }) => {
    ...
    const fisiolinkLink = page.getByRole("link", { name: /FisioLink|Fisiolink/i }).first();
  ...
  test("Deve navegar entre as plataformas do gerador de conteúdo", async ({ page }) => {
    await page.goto("/marketing/content-generator");
    await page.waitForLoadState("domcontentloaded");
  ...
  test("Deve exibir botão de gerar conteúdo", async ({ page }) => {
    ...
    const generateButton = page.getByRole("button", { name: /Gerar|Generate|Criar/i }).first();
    await expect(generateButton).toBeVisible({ timeout: 10000 });
  });

  test("Deve exibir área de texto para geração de conteúdo", async ({ page }) => {
    await page.goto("/marketing/content-generator");
    await page.waitForLoadState("domcontentloaded");

    const textArea = page.locator('textarea, .editor, [contenteditable="true"]').first();
    await expect(textArea).toBeVisible({ timeout: 10000 });
  });

  test("Deve exibir botões de ação (copiar, baixar, etc.)", async ({ page }) => {
    await page.goto("/marketing/content-generator");
    await page.waitForLoadState("domcontentloaded");
  ...
  test("Deve acessar a página de ROI", async ({ page }) => {
    ...
    await expect(page.locator("h1, h2").filter({ hasText: /ROI/i }).first()).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("Deve exibir estados vazios quando não há conteúdo", async ({ page }) => {
    await page.goto("/marketing/content-generator");
    await page.waitForLoadState("domcontentloaded");
