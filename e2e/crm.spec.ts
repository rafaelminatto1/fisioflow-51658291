...
test.describe("Módulo CRM", () => {
  ...
  test("Deve exibir abas de navegação (leads, campanhas, tarefas, automações)", async ({
    ...
    const leadsTab = page.locator('[role="tab"]').filter({ hasText: /Leads/i }).first();
  ...
  test("Deve exibir o botão de importar leads na aba de leads", async ({ page }) => {
    await page.goto("/crm");
    await page.waitForLoadState("domcontentloaded");
    ...
    const importButton = page.getByRole("button", { name: /Importar|Upload|Adicionar/i }).first();
  ...
  test("Deve exibir filtros de busca em leads", async ({ page }) => {
    ...
    const leadsTab = page.locator('button, [role="tab"]').filter({ hasText: /Leads/i }).first();
    if (await leadsTab.isVisible({ timeout: 5000 }).catch(() => false)) {
      await leadsTab.click();
      await page.waitForTimeout(300);
    }
  ...
  test("Deve exibir botões de ação em campanhas", async ({ page }) => {
    ...
    const actionButton = page.getByRole("button", { name: /Criar|Nova|Adicionar/i }).first();
  ...
  test("Deve exibir lista de automações na aba de automações", async ({ page }) => {
    await page.goto("/crm");
    await page.waitForLoadState("domcontentloaded");
    ...
    const automationsList = page
      .locator('[data-testid*="automation"], [data-testid*="automação"], .automation-card')
      .first();
