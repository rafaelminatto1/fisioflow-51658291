...
test.describe("Protocol Modal Theme Validation", () => {
  ...
  test("NewProtocolModal should use system theme colors (no hardcoded teal)", async ({ page }) => {
    ...
    const newProtocolBtn = page
      .locator('button:has-text("Novo Protocolo"), button:has-text("Criar Protocolo")')
      .first();
    ...
    const bgColor = await firstStepCircle.evaluate(
  ...
  test("ApplyProtocolModal should follow system theme", async ({ page }) => {
    ...
    if (await firstProtocolCard.isVisible()) {
      const applyBtn = firstProtocolCard
        .locator('button:has-text("Aplicar"), button[title*="Aplicar"]')
        .first();
