...
test.describe("Módulo Admin", () => {
  ...
  test("Deve exibir cards de profissionais", async ({ page }) => {
    ...
    const professionalCards = page
      .locator('[data-testid*="professional"], [data-testid*="profissional"], .professional-card')
      .first();
    if (await professionalCards.isVisible({ timeout: 5000 }).catch(() => false)) {
  ...
  test("Deve exibir botão de adicionar profissional", async ({ page }) => {
    ...
    const addButton = page.getByRole("button", { name: /Adicionar|Novo|Criar|Convidar/i }).first();
  ...
  test("Deve exibir badges de status de profissionais", async ({ page }) => {
    ...
    const statusBadges = page.locator('.badge, [role="status"], [data-testid*="status"]');
    if ((await statusBadges.count()) > 0) {
      await expect(statusBadges.first()).toBeVisible({ timeout: 10000 });
    }
  });

  test("Deve exibir menus de ação em profissionais (editar, excluir, etc.)", async ({ page }) => {
    await page.goto("/admin/professionals");
    await page.waitForLoadState("domcontentloaded");

    const firstCard = page
      .locator('[data-testid*="professional"], [data-testid*="profissional"], tr')
      .first();
    if (await firstCard.isVisible({ timeout: 5000 }).catch(() => false)) {
      const actionButton = firstCard.getByRole("button").filter({ hasText: "" }).first();
  ...
  test("Deve exibir estados vazios quando não há profissionais", async ({ page }) => {
    await page.goto("/admin/professionals");
    await page.waitForLoadState("domcontentloaded");
  ...
  test("Deve ter loading state durante carregamento", async ({ page }) => {
    ...
    const loader = page.locator('.loading, [data-testid*="loading"], .skeleton').first();
    if (await loader.isVisible({ timeout: 2000 }).catch(() => false)) {
      await expect(loader).toBeVisible();
    }
  });

  test("Deve exibir informações de métricas (uptime, taxa de erro, etc.)", async ({ page }) => {
    ...
    if ((await metricCards.count()) > 0) {
      ...
      const hasContent = (await firstCard.locator("h1, h2, h3, .value, .metric").count()) > 0;
