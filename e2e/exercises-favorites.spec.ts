...
test.describe("Sistema de Favoritos de Exercícios", () => {
  ...
  test.beforeEach(async ({ page }) => {
    await authenticateBrowserContext(page.context(), TEST_USER.email, TEST_USER.password);
  });
  ...
  test("deve favoritar um exercício pela biblioteca", async ({ page }) => {
    ...
    const heartButton = firstCard.locator('button:has(svg[data-lucide="heart"])').first();
  ...
  test("deve manter favoritos após recarregar a página", async ({ page }) => {
    ...
    const reloadedButton = reloadedCard.locator('button:has(svg[data-lucide="heart"])').first();
  ...
  test("deve filtrar exercícios por favoritos", async ({ page }) => {
    ...
    const button1 = card1.locator('button:has(svg[data-lucide="heart"])').first();
    const button2 = card2.locator('button:has(svg[data-lucide="heart"])').first();
  ...
  test("deve favoritar pelo modal de detalhes", async ({ page }) => {
    ...
    const firstCard = page.locator("div.group\\/overflow-hidden").first();
    const cardHeartButton = firstCard.locator('button:has(svg[data-lucide="heart"])').first();
  ...
  test("deve permitir favoritar múltiplos exercícios", async ({ page }) => {
    ...
    if (cardCount < 3) {
      test.skip("Não há exercícios suficientes para teste de múltiplos favoritos");
    }
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const button = card.locator('button:has(svg[data-lucide="heart"])').first();
