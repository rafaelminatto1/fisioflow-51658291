...
async function isCardVisibleInColumn(
  ...
  if (pageId) {
    const byId = await triageColumn
      .locator(`[data-testid="triage-card-${pageId}"]`)
      .first()
      .isVisible()
      .catch(() => false);
    if (byId) return true;
  ...
  return triageColumn
    .locator('[data-testid^="triage-card-"]', { hasText: cardTitle })
    .first()
    .isVisible()
    .catch(() => false);
}
...
async function waitForStableBoardState(
  ...
  throw new Error(`Board não estabilizou com os cards criados. cardA=${cardA} cardB=${cardB}`);
}
...
async function createTriagePageFromTemplate(page: Page, title: string, _capture: SaveCapture) {
  ...
  await page
    .locator('input[placeholder="tag1, tag2, tag3"]')
    .first()
    .fill("triage, triage-backlog, prd");
...
async function dragCardToColumn(
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    if (!cardBox || !destinationBox)
      throw new Error("Não foi possível obter bounding box para drag-and-drop");
...
async function moveCardToColumnByMenu(
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    await page.waitForTimeout(250);
    const movedTo =
      destinationLabel === "Backlog"
        ? "backlog"
        : destinationLabel === "Em execução"
          ? "in-progress"
          : "done";
  ...
  throw new Error(
    `Não foi possível mover "${cardTitle}" para "${destinationLabel}" via menu rápido.`,
  );
}
...
test.describe("Wiki triage board", () => {
  test("create template pages, move between columns, reorder and persist on reload", async ({
    page,
  }) => {
    ...
    page.on("console", (msg) => {
      ...
      if ((msg.type() === "info" || msg.type() === "log") && msg.text().includes("[E2E][")) {
        ...
        const createdPayload = tryParseTaggedJson(
          msg.text(),
          "[E2E][WikiService][savePage:created]",
        );
        if (createdPayload) {
          const traceId =
            typeof createdPayload.traceId === "string" ? createdPayload.traceId : null;
    ...
    if (!cardASource)
      throw new Error(`Card "${cardA}" não encontrado em nenhuma coluna antes da movimentação.`);

    if (cardASource !== "in-progress") {
      const movedByDrag = await dragCardToColumn(
        page,
        cardA,
        cardASource,
        "in-progress",
    ...
    if (!cardBSource)
      throw new Error(`Card "${cardB}" não encontrado em nenhuma coluna antes do reorder.`);
    ...
    await expect
      .poll(async () => isCardVisibleInColumn(page, "in-progress", cardA, saveCapture), {
        timeout: 15000,
        intervals: [500, 1000, 2000],
      })
      .toBe(true);
    await expect
      .poll(async () => isCardVisibleInColumn(page, "backlog", cardB, saveCapture), {
        timeout: 15000,
        intervals: [500, 1000, 2000],
      })
      .toBe(true);
