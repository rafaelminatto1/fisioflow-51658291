...
test.describe("Debug Produção - moocafisio.com.br", () => {
  ...
  test("deve tentar login e verificar erros de IA", async ({ page }) => {
    const targetBaseUrl =
      process.env.E2E_PROD_BASE_URL || process.env.BASE_URL || "https://moocafisio.com.br";
    ...
    if (hasAuthForbidden) {
      console.log(
        "⚠️ O login falhou com 403 Forbidden. Provavelmente o domínio moocafisio.com.br não está na ALLOWED_ORIGINS do Neon Auth.",
      );
    }
    ...
    if (hasOldVersionError) {
      console.log(
        "⚠️ O erro de versão antiga ainda aparece. O bundle JS servido para moocafisio.com.br ainda contém o código antigo.",
      );
    } else {
