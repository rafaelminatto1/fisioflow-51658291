...
test("FisioFlow Production Data Population Mission", async ({ page }) => {
  ...
  const emailInput = page
    .locator('[data-testid="auth-email-input"], input[name="email"], input[type="email"]')
    .first();
  const passwordInput = page
    .locator('[data-testid="auth-password-input"], input[name="password"], input[type="password"]')
    .first();
  const submitBtn = page
    .locator(
      '[data-testid="auth-submit-button"], button[type="submit"], button:has-text("Acessar"), button:has-text("Entrar")',
    )
    .first();
  ...
  for (let p = 1; p <= 10; p++) {
    ...
    const nameField = page
      .locator('input[name="full_name"], input[name="name"], #full_name')
      .first();
    ...
    const savePatientBtn = page
      .locator('button[type="submit"], button:has-text("Salvar"), button:has-text("Criar")')
      .first();
    ...
    for (let a = 1; a <= 10; a++) {
      ...
      const newApptBtn = page
        .locator(
          'button:has-text("Novo Agendamento"), button:has-text("Novo"), button[aria-label*="novo" i]',
        )
        .first();
      ...
      const patientSearch = page
        .locator('input[placeholder*="Buscar"], input[placeholder*="paciente"], input[cmdk-input]')
        .first();
      if (await patientSearch.isVisible()) {
        ...
        await page
          .locator(
            `[role="option"]:has-text("${patientName}"), [cmdk-item]:has-text("${patientName}"), text=${patientName}`,
          )
          .first()
          .click();
      } else {
      ...
      // Confirm Appointment
      const confirmApptBtn = page
        .locator(
          'button:has-text("Confirmar"), button:has-text("Salvar"), button:has-text("Agendar")',
        )
        .first();
      ...
      const startAttendanceBtn = page
        .locator(
          'button:has-text("Iniciar Atendimento"), button:has-text("Evoluir"), button:has-text("Ver Evolução")',
        )
        .first();
      ...
      await page.waitForURL(
        (url) =>
          url.pathname.includes("patient-evolution") || url.pathname.includes("session-evolution"),
        { timeout: 20000 },
      );
      ...
      for (const field of soapFields) {
        if (!(await field.loc.isVisible())) {
          const fallback = page
            .locator(
              `textarea[placeholder*="${field.placeholder}" i], [data-testid*="${field.placeholder.toLowerCase()}"]`,
            )
            .first();
      ...
      const finishBtn = page
        .locator(
          'button:has-text("Finalizar"), button:has-text("Salvar Evolução"), button:has-text("Concluir")',
        )
        .first();
