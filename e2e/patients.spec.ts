...
test.describe("Pacientes - CRUD Completo", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="patients-page-header"]')).toBeVisible({
      timeout: 25000,
    });
  });
  ...
  test("deve filtrar pacientes por status", async ({ page }) => {
    ...
    if (await statusFilter.isVisible({ timeout: 10000 })) {
      ...
      const option = page
        .locator(
          '[role="option"]:has-text("Em Tratamento"), [role="menuitem"]:has-text("Em Tratamento")',
        )
        .first();
  ...
  test("deve visualizar detalhes do paciente", async ({ page }) => {
    ...
    const isEmpty = await page
      .locator("text=/Nenhum paciente|Comece adicionando/i")
      .isVisible()
      .catch(() => false);
    ...
    const patientCard = page
      .locator('[data-testid="patient-list"] > div, .card, [class*="PatientCard"]')
      .first();
  ...
  test("deve editar paciente", async ({ page }) => {
    ...
    if (await patientCard.isVisible({ timeout: 5000 })) {
      ...
      const editOption = page
        .locator('div[role="menuitem"]:has-text("Editar"), [role="option"]:has-text("Editar")')
        .first();
  ...
  test("deve exportar lista de pacientes", async ({ page }) => {
    const exportButton = page
      .locator(
        'button[title*="Exportar"], button:has(svg[class*="download"]), button:has(svg[class*="Download"])',
      )
      .first();
