...
async function main() {
  console.log("Conectando ao Chrome aberto do usuário na porta 9222 via CDP (DevTools Protocol)...");
  ...
  for (let p = 1; p <= NUM_PATIENTS; p++) {
    ...
    for (let a = 1; a <= NUM_APPTS; a++) {
      ...
      await page.locator('.rbc-time-slot').nth(10 + a).click();
      ...
      const eventCards = page.locator('.rbc-event, [data-testid="event-card"], .cursor-pointer').filter({ hasText: 'Paciente DevTools' });
      ...
      await editor.type(`Validando AUTOSAVE do Paciente ${p} no agendamento ${a} via DevTools Protocol!`);
  ...
  console.log("✅ TESTE DEVTOOLS COMPLETO: Pacientes e Agendamentos criados e evoluídos com sucesso usando protocolo DevTools na sua máquina.");
  await browser.close();
}
