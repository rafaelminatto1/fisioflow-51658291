...
test.describe("Patient Evolution V3 - Templates SOAP", () => {
  test("deve abrir templates, aplicar template e exibir ação Editar Template", async ({ page }) => {
    ...
    if (new URL(page.url()).pathname.startsWith("/auth")) {
      ...
      await page
        .getByRole("button", { name: /Acessar Minha Conta|Entrar na Plataforma/i })
        .first()
        .click();
    ...
    await expect(patientReportEditor).toContainText("Paciente refere dor moderada", {
      timeout: 10000,
    });
    await expect(evolutionEditor).toContainText("Sinais vitais estáveis", { timeout: 10000 });
    await expect(evolutionEditor).toContainText("Paciente em pós-operatório imediato", {
      timeout: 10000,
    });
