...
async function mockEventosBootstrap(page: Page) {
  ...
  await page.route("**/api/activities**", async (route) => {
    ...
    if (method === "POST") {
      ...
      const created: EventoRecord = {
        ...
        valor_padrao_prestador:
          typeof body.valor_padrao_prestador === "number" ? body.valor_padrao_prestador : 0,
      };
  ...
  await page.route("**/api/activities/*", async (route) => {
    ...
    if (method === "PUT") {
      const body = await route.request().postDataJSON();
      eventos = eventos.map((evento) => (evento.id === id ? { ...evento, ...body } : evento));
...
async function hasEventosInteractiveContent(page: Page) {
  const novoEventoButton = page
    .locator("button")
    .filter({ hasText: /Novo Evento|Novo/i })
    .last();
...
test.describe("Gestão de Eventos", () => {
  ...
  test("deve editar evento existente", async ({ page }) => {
    ...
    await page
      .getByRole("button", { name: /Salvar|Atualizar/i })
      .first()
      .click();

    await expect(page.locator("text=/editado|atualizado|sucesso/i").first()).toBeVisible({
      timeout: 5000,
    });
  });
