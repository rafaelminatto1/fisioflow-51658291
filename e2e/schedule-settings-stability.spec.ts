...
test.describe("Schedule Settings Stability", () => {
  ...
  test("deve carregar /schedule/settings sem travar e sem erros críticos de console", async ({
    page,
  }) => {
    ...
    const shouldIgnore = (text: string) =>
      !text || text.includes("Download the React DevTools") || text.includes("[vite]");
