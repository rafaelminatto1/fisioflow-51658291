...
test.describe("Produção - Validação Completa CRUD (Pacientes e Agendamentos)", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForURL(
      (url) => url.pathname.includes("/agenda") || url.pathname.includes("/dashboard"),
      { timeout: 30000 },
    );
  ...
  test("Deve realizar o CRUD completo de Paciente", async ({ page }) => {
    ...
    await page.fill(
      'input[placeholder*="Nome"], input[name="name"], input[name="full_name"]',
      testPatientName,
    );
    await page.fill('input[placeholder*="E-mail"], input[name="email"]', "test@playwright.com");
    await page.fill(
      'input[type="date"], input[name="birth_date"], input[name="birthDate"]',
      "1990-01-01",
    );
