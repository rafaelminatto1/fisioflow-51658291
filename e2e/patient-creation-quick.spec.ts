...
test("criação rápida de paciente e autocomplete no modal de agendamento", async ({ page }) => {
  ...
  const newAppointmentButton = page
    .locator(
      'button:has-text("Novo"), button:has-text("Agendar"), button[aria-label*="novo" i], button[aria-label*="agendar" i]',
    )
  ...
  if (await comboboxTrigger.isVisible()) {
    ...
    const createOption = page
      .locator("[cmdk-item]")
      .filter({ hasText: `Cadastrar "${patientName}"` })
      .first();
    ...
    if (await createOption.isVisible()) {
      ...
      const createButtonEmpty = page
        .locator("button")
        .filter({ hasText: /Cadastrar/i })
        .first();
      if (await createButtonEmpty.isVisible()) {
        ...
        const comboboxPopover = page
          .locator("[data-radix-popover-content]")
          .filter({ has: page.locator('input[placeholder*="Buscar"]') })
          .first();
    ...
    const quickRegisterButton = page
      .locator('button:has-text("Cadastro Rápido"), button:has-text("Novo Paciente")')
      .first();
  ...
  const phoneInput = page
    .locator('input[name="phone"], input[placeholder*="telefone"], input[type="tel"]')
    .last();
  ...
  const modalVisible = page
    .locator('[role="dialog"], .dialog-content, [data-radix-dialog-content]')
    .first();
  if ((await modalVisible.count()) === 0) {
    ...
    const newAppointmentButton = page
      .locator('button:has-text("Novo"), button:has-text("Agendar")')
      .first();
  ...
  if ((await comboboxButton.count()) > 0) {
    ...
    await page
      .waitForSelector('[data-state="open"], [role="dialog"]', { timeout: 3000 })
      .catch(() => {});
    ...
    const searchInput = page
      .locator(
        'input[placeholder*="nome"], input[placeholder*="CPF"], input[placeholder*="telefone"], input.cmdk-input, input[data-radix-collection-item]',
      )
      .first();
    ...
    if ((await searchInput.count()) > 0) {
      ...
      await page.screenshot({
        path: "/tmp/patient-creation-08-autocomplete-resultados.png",
        fullPage: true,
      });
      ...
      if (listVisible) {
        ...
        if (itemCount > 0) {
          ...
          if (
            ...
            console.log(
              "ℹ️  Items appear to be tabs, not patients. PatientCombobox dropdown may not have opened.",
            );
          } else {
            console.log("✅ Autocomplete funcionando com opções de paciente!");

            // Verify if the created patient is in the list
            const createdPatientItem = patientList
              .locator(`[cmdk-item]:has-text("${patientName}")`)
              .first();
      ...
      const emptyMessage = page.locator(
        "text=/Nenhum paciente encontrado/i, text=/No patients found/i, text=/Selecione o paciente/i",
      );
