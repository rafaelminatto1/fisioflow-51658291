...
test("fluxo completo: criar paciente e verificar autocomplete", async ({ page }) => {
  ...
  if ((await createButton.count()) > 0) {
    ...
    const dialogTitle = page
      .locator('h2:has-text("Cadastro Rápido"), [role="dialog"]:has-text("Cadastro")')
      .first();
    ...
    if (isModalVisible) {
      ...
      if ((await nameInput.count()) > 0) {
        ...
        const dialogLocator = page
          .locator('[role="dialog"]:has-text("Cadastro"), h2:has-text("Cadastro Rápido")')
          .first();
        ...
        const errorElements = await page
          .locator('.destructive, .error, [role="alert"], [data-state="open"]:has-text("erro")')
          .all();
        let hasError = false;
        ...
        const modalStillOpen = await page
          .locator('h2:has-text("Cadastro Rápido")')
          .isVisible()
          .catch(() => false);
