...
test.describe("Módulo Telemedicine", () => {
  ...
  test("Deve exibir informações de salas (nome, paciente, status)", async ({ page }) => {
    await page.goto("/telemedicine");
    await page.waitForLoadState("domcontentloaded");
    ...
    for (const card of await roomCards) {
      if (await card.isVisible().catch(() => false)) {
        const hasName = (await card.locator("h1, h2, h3, .name, .title").count()) > 0;
        const hasStatus = (await card.locator('.badge, [role="status"], .status').count()) > 0;
        expect(hasName || hasStatus).toBeTruthy();
      }
    }
  });

  test("Deve exibir botões de ação nas salas (entrar, copiar link, excluir)", async ({ page }) => {
    await page.goto("/telemedicine");
    await page.waitForLoadState("domcontentloaded");
