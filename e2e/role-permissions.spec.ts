...
test.describe("Permissões por Role", () => {
  ...
  test("fisioterapeuta deve ter acesso a rotas permitidas", async ({ page }) => {
    ...
    const fisioRoutes = ["/schedule", "/patients", "/exercises"];
  ...
  test("estagiário deve ter acesso limitado", async ({ page }) => {
    ...
    const estagiarioRoutes = ["/schedule", "/patients"];
    ...
    for (const route of restrictedRoutes) {
      ...
      const isRestricted =
        ...
        (await page
          .locator("text=/sem permissão|acesso negado|não autorizado/i")
  ...
  test("deve verificar role no perfil do usuário", async ({ page }) => {
    ...
    if (!hasProfileInfo) {
      ...
      const userMenu = page
        .locator('[data-testid="user-menu"], button[aria-label*="menu"]')
        .first();
  ...
  test("deve bloquear acesso a rotas protegidas sem autenticação", async ({ page }) => {
    ...
    const protectedRoutes = ["/schedule", "/patients", "/exercises", "/reports", "/settings"];
