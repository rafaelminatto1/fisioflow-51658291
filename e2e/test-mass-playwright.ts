  ...
  try {
    ...
    for (let p = 1; p <= NUM_PATIENTS; p++) {
      ...
      for (let a = 1; a <= NUM_APPTS; a++) {
        ...
        await page
          .locator('input[placeholder*="Buscar paciente"], input[placeholder*="paciente"]')
          .fill("Paciente Mass Playwright");
        ...
        const eventCards = page
          .locator('.fc-event, .rbc-event, [data-testid="event-card"], .cursor-pointer')
          .filter({ hasText: "Paciente Mass Playwright" });
        ...
        await page
          ...
          .click();
        await page
          .getByRole("button", { name: /Iniciar Atendimento|Continuar Atendimento/i })
          .click();
        ...
        await page.screenshot({
          path: path.join(process.cwd(), "e2e", "success-screenshot.png"),
          fullPage: true,
        });

        if (savedHtml.includes("BUG DA TELA BRANCA")) {
          console.log(
            `  ✅ SUCESSO: O texto persistiu após recarregar sem F5! (Tamanho: ${savedHtml.length} chars)`,
          );
        } else {
    ...
    await page.screenshot({
      path: path.join(process.cwd(), "e2e", "error-screenshot.png"),
      fullPage: true,
    });
  } finally {
    await browser.close();
