...
test.describe("Evolução SOAP (Mocked)", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    page.on("response", (resp) => {
      if (resp.status() >= 400 && resp.status() !== 401) {
        // 401 might happen for non-essential assets
    ...
    await page.route(/appointmentservicehttp/i, async (route) => {
      ...
      await route.fulfill({
        status: 200,
        json: { data: { ...mockAppointment, status: "Realizado" } },
      });
    });
    ...
    await page.route("**/rest/v1/organizations*", async (route) =>
      route.fulfill({ json: [{ id: validOrgId, name: "Org Mock" }] }),
    );
    await page.route("**/rest/v1/organization_members*", async (route) =>
      route.fulfill({ json: [{ organization_id: validOrgId, role: "admin" }] }),
    );
  ...
  test("deve carregar dados do agendamento e salvar evolução", async ({ page }) => {
    ...
    if (await appointmentNotFound.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      await expect(page.getByRole("link", { name: /Evolução do Paciente/i }).first()).toBeVisible({
        timeout: 10000,
      });
      return;
    }
