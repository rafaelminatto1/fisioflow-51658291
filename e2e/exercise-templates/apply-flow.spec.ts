 ...
 */
async function setupApplyFlowMocks(
  page: Page,
  templates: ExerciseTemplate[] = [MOCK_TEMPLATE_ORTOPEDICO],
) {
  // Profile / auth
...
test.describe("Template Apply Flow — Fluxo completo de aplicação", () => {
  ...
  test('deve abrir o sheet de aplicação ao clicar em "Aplicar a Paciente" (Req 3.1)', async ({
    page,
  }) => {
    // Select the template
    const templateCard = page.getByText("Protocolo Lombalgia Crônica").first();
    ...
    await expect(
      page.getByRole("dialog").filter({ hasText: /Aplicar template a paciente/i }),
  ...
  test("fluxo completo: selecionar template → buscar paciente → definir data → criar plano (Req 3.1, 3.5, 3.6)", async ({
    page,
  }) => {
  ...
  test("deve exibir etapa de vinculação a cirurgia para template pós-operatório (Req 3.4)", async ({
    page,
  }) => {
    ...
    await expect(sheet.getByText(/Reconstrução LCA Joelho Direito/i)).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test('deve exibir badge "Sistema" no template card e no painel de detalhes', async ({ page }) => {
    ...
    const cardContainer = page
      .locator('[data-testid="template-card"], .cursor-pointer')
      .filter({ hasText: "Protocolo Lombalgia Crônica" })
      .first();
