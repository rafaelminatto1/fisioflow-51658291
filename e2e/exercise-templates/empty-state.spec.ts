...
const MIXED_TEMPLATES = [
  MOCK_SYSTEM_TEMPLATE_ORTOPEDICO,
  MOCK_SYSTEM_TEMPLATE_ESPORTIVO,
...
test.describe("Estado Vazio — Organização sem Custom_Templates (Req 7.1, 7.2, 7.3, 7.4)", () => {
  ...
  test("deve exibir banner de estado vazio quando não há Custom_Templates (Req 7.1)", async ({
    page,
  }) => {
  ...
  test("deve exibir System_Templates na sidebar mesmo sem Custom_Templates (Req 7.2)", async ({
    page,
  }) => {
  ...
  test('deve exibir botão "Explorar Templates do Sistema" no estado vazio (Req 7.3)', async ({
    page,
  }) => {
    await expect(page.getByText("Protocolo Lombalgia Crônica")).toBeVisible({ timeout: 15000 });
  ...
  test('deve exibir botão "Criar Meu Primeiro Template" no estado vazio (Req 7.4)', async ({
    page,
  }) => {
    await expect(page.getByText("Protocolo Lombalgia Crônica")).toBeVisible({ timeout: 15000 });

    await expect(page.getByRole("button", { name: /Criar Meu Primeiro Template/i })).toBeVisible();
  });

  test('clicar em "Explorar Templates do Sistema" mantém System_Templates visíveis (Req 7.2, 7.3)', async ({
    page,
  }) => {
    await expect(page.getByText("Protocolo Lombalgia Crônica")).toBeVisible({ timeout: 15000 });
  ...
  test('clicar em "Criar Meu Primeiro Template" abre o TemplateCreateFlow (Req 7.4)', async ({
    page,
  }) => {
    await expect(page.getByText("Protocolo Lombalgia Crônica")).toBeVisible({ timeout: 15000 });
...
test.describe("Estado com Custom_Templates — Banner de estado vazio ausente", () => {
  ...
  test("não deve exibir banner de estado vazio quando há Custom_Templates (Req 7.1)", async ({
    page,
  }) => {
  ...
  test("deve exibir tanto System_Templates quanto Custom_Templates na sidebar (Req 7.2)", async ({
    page,
  }) => {
  ...
  test("não deve exibir botões de CTA do estado vazio quando há Custom_Templates (Req 7.3, 7.4)", async ({
    page,
  }) => {
    await expect(page.getByText("Meu Protocolo Personalizado")).toBeVisible({ timeout: 15000 });
...
test.describe("Estado completamente vazio — Sem nenhum template", () => {
  ...
  test("deve exibir banner de estado vazio quando não há nenhum template (Req 7.1)", async ({
    page,
  }) => {
  ...
  test("deve exibir botões de CTA mesmo sem nenhum template (Req 7.3, 7.4)", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: /Explorar Templates do Sistema/i })).toBeVisible({
      timeout: 10000,
    });
