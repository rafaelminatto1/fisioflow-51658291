...
test("login e logout devem invalidar sessão no servidor", async ({ page }) => {
  ...
  page.on("response", (resp) => {
    if (
      resp.url().includes("neonauth") ||
      resp.url().includes("sign-out") ||
      resp.url().includes("signout")
    ) {
  ...
  console.log(
    "🍪 Session token antes do logout:",
    sessionToken ? `${sessionToken.slice(0, 20)}...` : "NENHUM",
  );
  ...
  console.log(
    "📡 Respostas recebidas no logout:",
    allResponses.length > 0 ? allResponses : "NENHUMA",
  );
  ...
  const sessionCookieAfter = cookiesAfter.find(
    (c) => c.name === "__Secure-neon-auth.session_token",
  );
  console.log(
    "🍪 Session cookie após logout:",
    sessionCookieAfter
