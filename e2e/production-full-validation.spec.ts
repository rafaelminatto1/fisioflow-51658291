...
test.describe("Produção - Validação Completa de Agendamentos", () => {
  test("deve criar e editar um agendamento com sucesso", async ({ page }) => {
    ...
    const patientInput = page
      .locator('input[placeholder*="paciente"], input[role="combobox"]')
      .first();
    ...
    const notesField = page
      .locator('textarea[placeholder*="Observações"], textarea[name="notes"]')
      .first();
    ...
    await page.click(
      'button[type="submit"]:has-text("Agendar"), button:has-text("Confirmar Agendamento")',
    );
    ...
    const editButton = page
      .locator('button:has-text("Editar"), [data-testid="edit-button"]')
      .first();
