...
async function confirmVisibleRescheduleDialog(page: Page) {
  const dialog = page
    ...
    .filter({ hasText: "Confirmar Reagendamento" })
    .last();
...
async function openAgendaUntilAppointmentIsVisible(
  page: Page,
  appointmentId: string,
  date: string,
): Promise<Locator> {
  const sourceCard = page
    .locator(`[data-appointment-popover-anchor="${appointmentId}"]:visible`)
    .first();
...
test.describe("Agenda - Reagendamento otimista", () => {
  ...
  test("move o card imediatamente apos o sucesso do reagendamento mesmo com a lista ainda stale", async ({
    page,
  }, testInfo) => {
    const mocks = await installAgendaOptimisticMocks(page);

    const sourceCard = await openAgendaUntilAppointmentIsVisible(
      page,
      APPOINTMENT_ID,
      APPOINTMENT_DATE,
    );
    ...
    const targetSlot = page
      .locator(`[data-testid="time-slot-${APPOINTMENT_DATE}-${TARGET_TIME}"]:visible`)
      .first();
    ...
    await expect(page.getByText(`Atendimento de ${PATIENT_NAME}`, { exact: false })).toBeVisible({
      timeout: 10000,
    });
    ...
    await expect
      .poll(
        async () => {
          ...
          return (
            Math.abs(currentBox.y - originalBox.y) > 8 || Math.abs(currentBox.x - originalBox.x) > 8
          );
        },
        {
          timeout: 10000,
    ...
    expect(
      movedBox,
      "Nao foi possivel medir a posicao do card apos o reagendamento.",
    ).not.toBeNull();

    const movedImmediately =
      !!originalBox &&
      !!movedBox &&
    ...
    expect(
      movedImmediately,
      "O card nao mudou de posicao imediatamente apos o sucesso do reagendamento.",
    ).toBe(true);
    expect(mocks.getAppointmentsListCalls()).toBeGreaterThan(0);
    expect(mocks.updatedPayloads).toHaveLength(1);
    expect(
      String(
        mocks.updatedPayloads[0]?.start_time || mocks.updatedPayloads[0]?.appointment_time || "",
      ),
    ).toContain(TARGET_TIME);
