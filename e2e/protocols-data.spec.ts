...
test.describe("Validação de Dados de Protocolos", () => {
  test("Deve carregar protocolos e exibir contagem correta de marcos e restrições", async ({
    page,
  }) => {
  ...
  test("Deve abrir detalhes do protocolo e exibir dados clínicos preenchidos", async ({ page }) => {
    ...
    await expect(page.locator("h1").filter({ hasNotText: "Protocolos Clínicos" })).toBeVisible({
      timeout: 15000,
    });
