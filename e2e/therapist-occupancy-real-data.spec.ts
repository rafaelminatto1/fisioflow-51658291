...
async function getActiveTherapistsCount(page: Page): Promise<number> {
  const text =
    (await page.getByTestId("occupancy-stat-active-therapists-value").textContent()) || "";
...
test.describe("Therapist Occupancy - Real Data", () => {
  test("loads organization real data and stays consistent across periods", async ({ page }) => {
    ...
    await expect
      .poll(async () => getActiveTherapistsCount(page), { timeout: 30000 })
      .toBeGreaterThan(0);
    const activeTherapists = await getActiveTherapistsCount(page);

    await expect
      .poll(async () => page.getByTestId("therapist-occupancy-row").count(), { timeout: 30000 })
      .toBeGreaterThan(0);
