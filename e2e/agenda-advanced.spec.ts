...
test.describe("Agenda Avançada - E2E Tests", () => {
  ...
  test("deve criar um agendamento recorrente semanal", async ({ page }) => {
    ...
    const recurringSwitch = page
      .getByRole("switch", { name: /Agendamento Recorrente/i })
      .or(page.locator('button[role="switch"]:has-text("Agendamento Recorrente")'))
      .or(page.locator('label:has-text("Agendamento Recorrente")'))
      .first();
    ...
    const segButton = page
      .locator("button")
      .filter({ hasText: /^Seg$|^S$/i })
      .first();
    const quaButton = page
      .locator("button")
      .filter({ hasText: /^Qua$|^Q$/i })
      .first();
    const sexButton = page
      .locator("button")
      .filter({ hasText: /^Sex$|^S$/i })
      .last(); // Last to avoid Seg/Sex conflict if 'S'
  ...
  test("deve cancelar um agendamento com sucesso", async ({ page }) => {
    ...
    const appointment = page
      .locator('[class*="appointment-card"], [class*="calendar-card"]')
      .first();
