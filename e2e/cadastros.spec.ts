...
test.describe("Módulo Cadastros", () => {
  ...
  for (const pageInfo of CADASTROS_PAGES) {
    test(`Deve acessar a página de ${pageInfo.title.source}`, async ({ page }) => {
      await page.goto(pageInfo.path);
      await page.waitForLoadState("domcontentloaded");

      await expect(page.locator("h1, h2").filter({ hasText: pageInfo.name }).first()).toBeVisible({
        timeout: 10000,
      });
    });
  }
  ...
  test("Deve exibir botão de criar novo em serviços", async ({ page }) => {
    ...
    const addButton = page.getByRole("button", { name: /Novo|Criar|Adicionar/i }).first();
  ...
  test("Deve exibir cards de templates de evolução", async ({ page }) => {
    ...
    const cards = page
      .locator('[data-testid*="template"], [data-testid*="card"], .template-card')
      .first();
  ...
  test("Deve exibir cards de fichas de avaliação", async ({ page }) => {
    ...
    const cards = page.locator('[data-testid*="form"], [data-testid*="card"], .form-card').first();
    if (await cards.isVisible({ timeout: 5000 }).catch(() => false)) {
      await expect(cards).toBeVisible();
    }
  });

  test("Deve exibir estados vazios quando não há serviços", async ({ page }) => {
    await page.goto("/cadastros/servicos");
    await page.waitForLoadState("domcontentloaded");
  ...
  test("Deve exibir estados vazios quando não há convênios", async ({ page }) => {
    await page.goto("/cadastros/convenios");
    await page.waitForLoadState("domcontentloaded");
  ...
  test("Deve exibir menus de ação em serviços", async ({ page }) => {
    ...
    if (await firstRow.isVisible({ timeout: 5000 }).catch(() => false)) {
      const actionButton = firstRow.getByRole("button").filter({ hasText: "" }).first();
  ...
  test("Deve ter consistência de UX entre páginas de cadastros", async ({ page }) => {
