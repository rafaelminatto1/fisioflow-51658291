...
test.describe("Validação em Produção - Cadastro de Paciente", () => {
  ...
  test("Deve realizar login e cadastrar um novo paciente com sucesso", async ({ page }) => {
    ...
    const patientForm = page
      .locator("form")
      .filter({ has: page.locator('[data-testid="patient-name"]') })
      .first();
    ...
    if (await medicoTab.isVisible()) {
      ...
      await page
        .locator('input#main_condition, [name="main_condition"]')
        .fill("Teste de Produção - Cadastro");
    }
    ...
    const submitButton = page
      .locator(
        'button[type="submit"]:has-text("Finalizar"), button[type="submit"]:has-text("Salvar")',
      )
      .first();
    ...
    await expect(page.locator("h1, h2").filter({ hasText: newPatientName }).first()).toBeVisible({
      timeout: 15000,
    });
