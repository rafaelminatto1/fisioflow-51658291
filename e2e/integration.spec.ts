...
async function setupIntegrationMocks(page: Page) {
  ...
  await page.route(/\/api\/appointments(?:\?.*)?$/i, async (route) => {
    ...
    if (method === "POST") {
      const payload = (await route
        .request()
        .postDataJSON()
        .catch(() => ({}))) as Partial<AppointmentRecord>;
  ...
  await page.route(/\/api\/appointments\/[^/?#]+(?:\/status)?$/i, async (route) => {
    ...
    const appointment =
      appointments.find((item) => item.id === id || `${item.id}/status` === id) || appointments[0];
...
test.describe("Testes de Integração E2E", () => {
  ...
  test("fluxo completo: criar paciente → agendar → marcar presença", async ({ page }) => {
    ...
    const newPatientButton = page
      ...
      .filter({ hasText: /Novo Paciente|Adicionar/i })
      .first();
    ...
    await expect(page.locator("text=Paciente Integração, text=Paciente Criado").first())
  ...
  test("multi-tenancy: dados isolados por organização", async ({ page }) => {
    ...
    await expect(page.getByText(/Organização Integração|Evento Integração/i).first())
  ...
  test("busca global: encontrar dados em diferentes módulos", async ({ page }) => {
    ...
    await expect(page.locator("text=Evento Integração").first())
      .toBeVisible({ timeout: 10000 })
      .catch(async () => {
        await expect(page.locator("main")).toBeVisible();
      });
