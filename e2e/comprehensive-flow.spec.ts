...
test.describe("Validação Completa de Produção - Agenda, Atendimento e Prontuário", () => {
  ...
  test("Fluxo completo: Agendamento, DND, Atendimento e 10 Evoluções", async ({ page }) => {
    ...
    const patientCombo = page
      .locator('button[role="combobox"], input[placeholder*="paciente" i]')
      .first();
    ...
    const confirmarBtn = page
      .locator('button[type="submit"], button:has-text("Confirmar"), button:has-text("Salvar")')
      .first();
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      const btnEvoluir = page
        .locator(
          'button:has-text("Nova Evolução"), button:has-text("Evoluir"), button:has-text("Registrar Evolução")',
        )
        .first();
      ...
      await textarea.fill(
        `Sessão ${i}/10 concluída com sucesso via teste automatizado E2E em Produção. Progresso notável de amplitude de movimento e redução de dor na escala EVA.`,
      );

      const salvarEvolucaoBtn = page
        .locator('button:has-text("Salvar"), button:has-text("Concluir")')
        .first();
    ...
    console.log(
      "✅ Validação de Produção Completa concluída: DND e 10 Sessões salvas no histórico.",
    );
  });
});
