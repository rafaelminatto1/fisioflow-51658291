...
test.describe("FisioFlow Resilience & Recovery", () => {
  ...
  const routes = ["/", "/agenda", "/patients", "/exercises", "/patient-evolution", "/auth/login"];
  ...
  for (const route of routes) {
    test(`Route "${route}" should load without Component crashes`, async ({ page }) => {
      ...
      const errorBoundaryTitle = page
        .locator("h1, h2, h3")
        .filter({ hasText: /Ops!|Gatilho|Cãibra|Lesão|Postura/i });
      ...
      if (criticalConsoleErrors.length > 0) {
        throw new Error(
          `CRITICAL: Runtime ReferenceErrors detected in console on route ${route}: ${criticalConsoleErrors[0]}`,
        );
      }

      if (pageErrors.length > 0) {
        throw new Error(
          `CRITICAL: Uncaught exception detected on route ${route}: ${pageErrors[0].message}`,
        );
      }
    });
  }
