...
const TEST_ORG_ID =
  testUsers.fisio.expectedOrganizationId || "00000000-0000-0000-0000-000000000001";
...
async function openVideoPlayerModal(page: Page, title = "Rotação de Ombro") {
  ...
  const playButton = page
    .getByRole("button", { name: new RegExp(`Reproduzir ${title}`, "i") })
    .first();
...
async function openDeleteDialog(page: Page, title = "Rotação de Ombro") {
  ...
  const deleteButton = page
    .getByRole("button", { name: new RegExp(`Excluir ${title}`, "i") })
    .first();
...
test.describe("Exercise Video Library", () => {
  ...
  test("should display video library header", async ({ page }) => {
    ...
    await expect(page.getByText("Biblioteca de Vídeos"))
      .toBeVisible({ timeout: 5000 })
      .catch(() => {
        // Header text might be different
      });
    await expect(page.getByRole("button", { name: /upload|vídeo/i }))
      .toBeVisible({ timeout: 5000 })
      .catch(() => {
  ...
  test("should display video count", async ({ page }) => {
    ...
    await expect(page.getByText(/\d+\s*vídeos?/i))
      .toBeVisible({ timeout: 5000 })
      .catch(() => {
  ...
  test("should filter videos by search term", async ({ page }) => {
    ...
    await expect(page.getByText("Rotação de Ombro")).toBeVisible({ timeout: 5000 });
    await expect(page.getByText("Agachamento Profundo"))
  ...
  test("should filter videos by category", async ({ page }) => {
    ...
    await page
      .getByRole("combobox", { name: /categoria|filtrar/i })
      .or(page.getByRole("button").filter({ hasText: /categoria/i }))
      .first()
      .click()
      .catch(() => {});
    ...
    await page
      .getByRole("option", { name: /fortalecimento|strength/i })
      .click()
      .catch(() => {});
  ...
  test("should toggle between grid and list view", async ({ page }) => {
    ...
    await expect(page.locator('[class*="Video"], [class*="video"]').first()).toBeVisible({
      timeout: 3000,
    });
  });
  ...
  test("should enter bulk selection mode", async ({ page }) => {
    ...
    await page
      .getByRole("button", { name: /selecao|seleção|modo/i })
      .or(page.locator('button[title*="Modo de seleção"]'))
      .first()
      .click()
      .catch(async () => {
        ...
        await page
          .locator("button")
          .filter({ has: page.locator(".lucide-square") })
          .first()
          .click();
      });
    ...
    await expect(
      page
        .getByRole("button", { name: /selecionar todos|desmarcar todos/i })
        .or(page.getByText(/Selecionar|Desmarcar/))
        .first(),
    )
      .toBeVisible({ timeout: 3000 })
      .catch(() => {});
  ...
  test("should select videos in bulk mode", async ({ page }) => {
    ...
    await page
      .locator(".group.border.bg-card")
      .filter({ hasText: "Rotação de Ombro" })
      .first()
      .click();
    ...
    await expect(
      page
        .getByRole("button", { name: /excluir.*1/i })
        .or(page.getByText(/\(1\)/))
        .first(),
    )
      .toBeVisible({ timeout: 3000 })
    ...
    await page
      .locator(".group.border.bg-card")
      .filter({ hasText: "Agachamento Profundo" })
      .first()
      .click();
    ...
    await expect(
      page
        .getByRole("button", { name: /excluir.*2/i })
        .or(page.getByText(/\(2\)/))
        .first(),
    )
      .toBeVisible({ timeout: 3000 })
...
test.describe("Video Player Modal", () => {
  ...
  test("should show action menu in modal", async ({ page }) => {
    ...
    await page
      .getByRole("dialog")
      .locator("button")
      .filter({ has: page.locator(".lucide-more-vertical") })
      .first()
      .click();
    ...
    await expect(
      page
        ...
        .or(page.getByRole("menuitem", { name: /eliminar/i })),
    ).toBeVisible({ timeout: 3000 });
  });
  ...
  test("should display keyboard shortcuts hint", async ({ page }) => {
    ...
    await expect(
      page
        .locator(
          '.dialog-content button[title*="Atalhos"], [class*="DialogContent"] button[title*="atalhos"]',
        )
        .or(page.locator(".lucide-keyboard"))
        .first(),
    )
      .toBeVisible({ timeout: 3000 })
      .catch(() => {
...
test.describe("Video Edit Modal", () => {
  ...
  test("should open edit modal from video card", async ({ page }) => {
    ...
    await expect(page.getByText("Editar Vídeo").or(page.getByText("Editar"))).toBeVisible({
      timeout: 5000,
    });
    await expect(
      page.getByText("Atualize as informações").or(page.getByText("informações do vídeo")),
    ).toBeVisible({ timeout: 3000 });
  });
  ...
  test("should populate edit form with video data", async ({ page }) => {
    ...
    // Check form fields
    await expect(page.getByPlaceholder("Ex: Rotação de Ombro com Bastão")).toHaveValue(
      "Rotação de Ombro",
      { timeout: 3000 },
    );
    await expect(
      page.getByPlaceholder("Descreva o exercício, objetivo e cuidados importantes..."),
    ).toHaveValue("Exercício para mobilidade de ombro", { timeout: 3000 });
  });
...
test.describe("Delete Confirmation", () => {
  ...
  test("should show delete confirmation dialog", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /Excluir vídeo\?/i })).toBeVisible({
      timeout: 5000,
    });
    await expect(page.getByText(/Esta ação não pode ser desfeita/i).first()).toBeVisible({
      timeout: 3000,
    });
  });
...
test.describe("Empty States", () => {
  test("should show empty state when no videos", async ({ page }) => {
    ...
    await expect(
      page.getByText("Comece adicionando vídeos demonstrativos de exercícios"),
    ).toBeVisible();
  });
