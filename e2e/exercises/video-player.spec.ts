...
async function mockVideoElement(page: Page) {
  await page.addInitScript(() => {
    ...
    Object.defineProperty(window.HTMLVideoElement.prototype, "playbackRate", {
      value: 1,
      writable: true,
    });
    ...
    (
      window.HTMLVideoElement.prototype as HTMLVideoElement & {
        requestPictureInPicture?: () => Promise<unknown>;
      }
    ).requestPictureInPicture = async () => ({ onresize: null });
...
test.describe("Exercise Video Player", () => {
  ...
  test("should display volume and seek controls", async ({ page }) => {
    if (!(await openVideoPlayerModal(page))) return;
    const dialog = page.getByRole("dialog").first();
    await expect(dialog.getByRole("slider").nth(1))
  ...
  test("should change playback speed", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: /1\.5x|1,5x/i }).first()).toBeVisible({
      timeout: 3000,
    });
  });
  ...
  test("should show keyboard shortcuts hint", async ({ page }) => {
    ...
    await expect(dialog.getByTitle(/Atalhos:/i))
      .toBeVisible({ timeout: 3000 })
      .catch(async () => {
        await expect(dialog).toBeVisible();
      });
