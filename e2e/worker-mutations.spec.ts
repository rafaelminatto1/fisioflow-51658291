  ...
  await page.route("**/api/templates/*", async (route) => {
    ...
    if (request.method() === "PUT") {
      ...
      template.templateVariant =
        body.template_variant ?? body.templateVariant ?? template.templateVariant;
...
test.describe("Worker Mutations - CRUD Flows", () => {
  ...
  test("Exercise Protocol CRUD Flow", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: "NOVO PROTOCOLO" })).toBeVisible({
      timeout: 15000,
    });

    await page.getByRole("button", { name: "NOVO PROTOCOLO" }).click();
    const dialog = page.getByRole("dialog").last();
    await expect(dialog.getByRole("heading", { name: "Criar Novo Protocolo" }).nth(1)).toBeVisible({
      timeout: 10000,
    });
