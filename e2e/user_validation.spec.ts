...
test.describe("Validação de Usuário e Fluxo de Paciente", () => {
  ...
  test("Deve criar paciente, validar abas e agendar/evoluir 10 sessões", async ({
    page,
    baseURL,
  }) => {
    ...
    const genderTrigger = dialog
      .locator("button")
      .filter({ hasText: /Selecione o gênero|masculino|feminino/i });
    ...
    for (let i = 0; i < 10; i++) {
      ...
      // Selecionar Horário
      const timeTrigger = aptModal
        .locator("button")
        .filter({ hasText: /Hora/i })
        .or(aptModal.locator('button:has-text("07:00")'))
        .first();
      ...
      const capacityDialog = page
        .getByRole("alertdialog")
        .or(page.locator("text=Capacidade Atingida"));
      try {
        ...
        const confirmBtn = page
          .locator('button:has-text("Agendar Mesmo Assim")')
    ...
    const criticalErrors = consoleErrors.filter(
      (e) => e.includes("Minified React error #306") || e.includes("toUpperCase"),
    );
