...
export const waConversations = pgTable(
  ...
  (table) => [
    ...
    index("idx_wa_conv_org_assigned").on(table.organizationId, table.assignedTo),
...
export const waMessages = pgTable(
  ...
  (table) => [
    ...
    index("idx_wa_msgs_conv_created").on(table.conversationId, table.createdAt),
...
export const waRawEvents = pgTable(
  "wa_raw_events",
  {
    ...
    processingState: varchar("processing_state", { length: 40 }).default(
      "received",
    ),
...
export const waConversationTags = pgTable(
  ...
  (table) => [
    uniqueIndex("idx_wa_conv_tags_unique").on(table.conversationId, table.tagId),
...
export const whatsappContactsRelations = relations(whatsappContacts, ({ many }) => ({
  messages: many(waMessages),
  conversations: many(waConversations),
}));

export const waConversationsRelations = relations(waConversations, ({ one, many }) => ({
...
export const waInternalNotesRelations = relations(waInternalNotes, ({ one }) => ({
  conversation: one(waConversations, {
