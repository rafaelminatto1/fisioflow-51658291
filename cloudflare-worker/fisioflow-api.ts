...
export default {
	async fetch(request: Request, env: Env): Promise<Response> {
        ...
        `;
				...
				const notifications = await queryNeon(
					sql,
        ...
        `;
			...
			if (
				path.match(/^\/api\/notifications\/[\w-]+\/read$/) &&
				request.method === "PUT"
			) {
				const user = verifyToken(
					request.headers.get("Authorization"),
        ...
        `;
			...
			if (
				path.match(/^\/api\/notifications\/[\w-]+$/) &&
        ...
        `;
				const revenueResult = await queryNeon(
					revenueSql,
					[startDate, endDate],
					env.DATABASE_URL,
				);
        ...
        `;
          ...
          WHERE organization_id IS NOT NULL
            ...
            AND date <= $2
        `;
				const sessionsResult = await queryNeon(
					sessionsSql,
					[startDate, endDate],
					env.DATABASE_URL,
				);
        ...
        `;
				const patientsResult = await queryNeon(
					patientsSql,
					[],
					env.DATABASE_URL,
				);
          ...
          WHERE is_active = true
            ...
            AND created_at <= $2
        `;
				const newPatientsResult = await queryNeon(
					newPatientsSql,
					[startDate, endDate],
					env.DATABASE_URL,
				);
				const newPatientsThisMonth = parseInt(
					newPatientsResult[0]?.count || "0",
				);
          ...
          ORDER BY date
        `;
				const revenueByDay = await queryNeon(
					revenueByDaySql,
					[startDate, endDate],
					env.DATABASE_URL,
				);
        ...
        `;
			...
			if (
				path.match(/^\/api\/settings\/notifications\/[\w-]+$/) &&
				request.method === "GET"
			) {
				const user = verifyToken(
					request.headers.get("Authorization"),
        ...
        `;
			...
			if (
				path === "/api/ai/exercise-suggestions" &&
				request.method === "POST"
			) {
				const user = verifyToken(
					request.headers.get("Authorization"),
				...
				const suggestions = await generateAIExerciseSuggestions(
					body,
					env.OPENAI_API_KEY,
					env.GEMINI_API_KEY,
					env.DATABASE_URL,
			...
			if (
				path.match(/^\/api\/ai\/patient-insights\/[\w-]+$/) &&
				request.method === "GET"
			) {
				const user = verifyToken(
					request.headers.get("Authorization"),
          ...
          GROUP BY p.id
        `;
				const patientData = await queryNeon(
					patientSql,
					[patientId],
					env.DATABASE_URL,
				);
        ...
        `;
				...
				return new Response(
					JSON.stringify({
						...
						exportId,
						message:
							"Export request received. Processing may take up to 48 hours.",
        ...
        `;
			...
			if (
				path.match(/^\/api\/financial\/card-mapping\/[\d]+$/) &&
				request.method === "GET"
			) {
        ...
        `;
				const result = await queryNeon(
					sql,
					[body.patientId, body.cardLastDigits],
					env.DATABASE_URL,
				);
			...
			if (
				path === "/api/marketing/analysis/extract-receipt" &&
				request.method === "POST"
			) {
          ...
          `;
					...
					const result = await queryNeon(query, params, env.DATABASE_URL);
					...
					return new Response(
						JSON.stringify({ error: "Erro ao buscar comunicados" }),
						{
							status: 500,
							headers: { ...corsHeaders, "Content-Type": "application/json" },
						},
			...
			// =====================
			// INSIGHTS API (Analytics alias)
			// =====================

			// GET /api/insights/dashboard - Dashboard insights
			if (path === "/api/insights/dashboard" && request.method === "GET") {
				const user = verifyToken(
					request.headers.get("Authorization"),
					env.JWT_SECRET,
				);
				...
				try {
					const [
						...
						queryNeon(
							`SELECT COUNT(*) as total FROM patients WHERE organization_id = $1 AND is_active = true`,
							[user.orgId],
							env.DATABASE_URL,
						),
						queryNeon(
               ...
               WHERE organization_id = $1 AND date = $2`,
                ...
                WHERE organization_id = $1
                  AND started_at BETWEEN $2 AND $3`,
							[user.orgId, `${startDate}T00:00:00`, `${endDate}T23:59:59`],
							env.DATABASE_URL,
						),
					]);

					const activePatients = parseInt(activePatientsRes[0]?.total || "0");
					const appts = appointmentsRes[0] || {};
					const totalAppointments = parseInt(appts.total || "0");
					const completedAppointments = parseInt(appts.total_completed || "0");
					const noShowAppointments = parseInt(appts.no_show || "0");
					const confirmedAppointments = parseInt(appts.confirmed || "0");
					const occupancyRate =
						totalAppointments > 0
					...
					const avgSessionDuration = parseFloat(
						sessionStatsRes[0]?.avg_session_duration || "0",
					);
					const engagementScore = Math.max(
						...
						Math.min(
							100,
							Math.round(
								((100 - absenceRate) * 0.6 + confirmationRate * 0.4) * 10,
							) / 10,
						),
					);
					...
					const dashboard: Record<string, unknown> = {
						...
						occupancyRate: Number(
							(Math.round(occupancyRate * 10) / 10).toFixed(1),
						),
						noShowRate: Number((Math.round(absenceRate * 10) / 10).toFixed(1)),
						confirmationRate: Number(
							(Math.round(confirmationRate * 10) / 10).toFixed(1),
						),
						avgSessionDuration: Number(
							(Math.round(avgSessionDuration * 10) / 10).toFixed(1),
						),
						engagementScore,
						topPainRegions: [],
						npsScore: 0,
						appointmentsToday: parseInt(todayRes[0]?.count || "0"),
						revenueChart: [],
					};
					...
					console.error("[Insights] Error fetching dashboard:", error);
					return new Response(
						JSON.stringify({ error: "Erro ao carregar dashboard" }),
						{
							status: 500,
							headers: { ...corsHeaders, "Content-Type": "application/json" },
						},
...
export class PatientAgent {
	...
	async fetch(request: Request): Promise<Response> {
		...
		if (method === "POST" && url.pathname.endsWith("/updateClinicalStatus")) {
			const body = await request.json();
			// Simple implementation - in production use Agent SDK
			return new Response(
				JSON.stringify({
					success: true,
					patientId: url.pathname.split("/")[2],
				}),
				{
					headers: { ...corsHeaders, "Content-Type": "application/json" },
				},
		...
		if (method === "POST" && url.pathname.endsWith("/dismissAction")) {
			return new Response(JSON.stringify({ success: true }), {
				headers: { ...corsHeaders, "Content-Type": "application/json" },
			});
		}
...
async function generateAIExerciseSuggestions(
	...
	if (openaiApiKey) {
		try {
			const suggestions = await generateOpenAISuggestions(
				context,
				openaiApiKey,
			);
			// Cache the result
...
function generateCacheKey(context: any): string {
	const str = JSON.stringify({
		conditions: context.conditions
			?.map((c: any) => c.bodyPart || c.body_part || "general")
			.sort(),
		painLevel:
			context.painLevel > 7 ? "high" : context.painLevel > 4 ? "medium" : "low",
	});
	let hash = 0;
...
async function getCachedSuggestion(
    ...
    `;
		...
		if (results.length > 0) {
			// Update hit count
			await queryNeon(
				"UPDATE ai_cache SET hit_count = hit_count + 1 WHERE query_hash = $1",
				[cacheKey],
				databaseUrl,
			);
			...
			messages: [
				{
					role: "system",
					content:
						"Você é um assistente especializado em fisioterapia. Responda apenas com JSON válido.",
				},
...
function generatePatientInsights(patientData: any): {
	...
	if (evolutionCount === 0) {
		insights.push(
			"Paciente sem evoluções registradas. Inicie o acompanhamento.",
		);
	...
	if (avgPain > 7) {
		...
		recommendations.push(
			"Priorizar manejo da dor antes de exercícios intensos.",
		);
	} else if (avgPain < 3) {
