...
for (const file of project.getSourceFiles()) {
  ...
  for (const importDecl of file.getImportDeclarations()) {
    if (importDecl.getModuleSpecifierValue().includes("MainLayout")) {
      importDecl.replaceWithText(`import { PageLayout, PageContainer, PageHeader } from "@/components/layout/PageLayout";`);
      hasMainLayout = true;
    }
  }
  ...
  const mainLayouts = file.getDescendantsOfKind(SyntaxKind.JsxElement)
  ...
  for (const layout of mainLayouts) {
    ...
    const h1s = layout.getDescendantsOfKind(SyntaxKind.JsxElement)
    ...
    if (h1s.length > 0) {
      ...
      titleStr = h1.getJsxChildren().map(c => c.getText()).join("").trim();
      ...
      if (parent) {
        const pTags = parent.getDescendantsOfKind(SyntaxKind.JsxElement)
        ...
        for (const p of pTags) {
          ...
          if (classNameAttr && classNameAttr.getText().includes("text-muted-foreground")) {
            subtitleStr = p.getJsxChildren().map(c => c.getText()).join("").trim();
            fallbackPToRemove = p;
            break;
          }
        }
        ...
        const buttons = headerContainer.getDescendantsOfKind(SyntaxKind.JsxElement)
          .filter(jsx => jsx.getOpeningElement().getTagNameNode().getText() === "Button");
          
        if (buttons.length > 0) {
           const children = headerContainer.getJsxChildren().filter(c => c.getKind() === SyntaxKind.JsxElement);
           if (children.length > 1) {
             const actionChild = children.find(c => c !== parent && c.getText().includes("Button"));
             if (actionChild) {
                if (actionChild.getKind() === SyntaxKind.JsxElement && (actionChild as JsxElement).getOpeningElement().getTagNameNode().getText() === "div") {
                   actionsStr = (actionChild as JsxElement).getJsxChildren().map(c => c.getText()).join("").trim();
                } else {
    ...
    let layoutChildrenText = layout.getJsxChildren().map(c => c.getText()).join("");
    
    if (headerNodeToRemove) {
      layoutChildrenText = layoutChildrenText.replace(headerNodeToRemove.getText(), "");
    } else {
      if (fallbackH1ToRemove) layoutChildrenText = layoutChildrenText.replace(fallbackH1ToRemove.getText(), "");
      if (fallbackPToRemove) layoutChildrenText = layoutChildrenText.replace(fallbackPToRemove.getText(), "");
    }
