    ...
    try {
      ...
      if (APPLY) {
        ...
        if (existing.rows.length) {
          await client.query(`update medical_records set chief_complaint=$1, current_history=$2, past_history=$3, previous_surgeries=$4, physical_activity=$5, medical_history=$6, current_medications=$7, allergies=$8::jsonb, medications=$9::jsonb, physical_exam=$10::jsonb, updated_at=now() where id=$11`, [fieldText(ev, s => /queixa|main_complaint/i.test(s)) || null, diagnosisText || null, fieldText(ev, s => /previous_medical_history|hist[oó]ria/i.test(s)) || null, surgeryText || null, sportText || null, diagnosisText || null, medsText || null, JSON.stringify(allergies.map(allergen => ({ allergen }))), JSON.stringify(medications.map(medication => ({ name: medication }))), JSON.stringify(physicalExam), existing.rows[0].id]);
        } else {
          await client.query(`insert into medical_records (patient_id, organization_id, record_date, chief_complaint, current_history, past_history, previous_surgeries, physical_activity, medical_history, current_medications, allergies, medications, physical_exam, created_by, created_at, updated_at) values ($1,$2,$3::date,$4,$5,$6,$7,$8,$9,$10,$11::jsonb,$12::jsonb,$13::jsonb,$14,now(),now())`, [patientId, organizationId, date, fieldText(ev, s => /queixa|main_complaint/i.test(s)) || null, diagnosisText || null, fieldText(ev, s => /previous_medical_history|hist[oó]ria/i.test(s)) || null, surgeryText || null, sportText || null, diagnosisText || null, medsText || null, JSON.stringify(allergies.map(allergen => ({ allergen }))), JSON.stringify(medications.map(medication => ({ name: medication }))), JSON.stringify(physicalExam), therapistId]);
        }
