...
const url = process.env.DATABASE_URL || "postgresql://neondb_owner:REDACTED@ep-wandering-bonus-acj4zwvo-pooler.sa-east-1.aws.neon.tech/neondb?sslmode=require";
const sql = neon(url);
...
async function run() {
  ...
  try {
    ...
    const baseData = await sql`SELECT organization_id, profile_id FROM patients WHERE organization_id IS NOT NULL LIMIT 1`;
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      for (let j = 1; j <= 10; j++) {
        ...
        if (j === 1 || j === 10) {
           metrics = { eva: j === 1 ? 8 : 2, rom_flexao: j === 1 ? 40 : 85, rom_extensao: j === 1 ? 10 : 30 };
