...
function generateProceduralContent(ex) {
  ...
  if (name.includes("alongamento") || name.includes("stretch")) {
    ...
    tipsText =
      "A respiração profunda e lenta ajuda no relaxamento do músculo alvo. Evite realizar 'insistências' (movimentos curtos de ir e vir).";
    ben =
      "Diminuição do tônus muscular, aumento da amplitude de movimento (ADM) e alívio de sobrecarga articular.";
  } else if (
    ...
    tipsText =
      "Foque na fluidez articular. O movimento deve ser como um 'óleo' nas engrenagens, nunca forçado com impacto.";
    ...
    tipsText =
      "Garanta que seus joelhos acompanhem a direção da ponta dos pés. Evite que eles desabem para dentro (valgo dinâmico). O calcanhar deve permanecer firme no chão na fase de descida.";
    ben =
      "Fortalecimento integrado de quadríceps, glúteos e estabilizadores, além de simular movimentos fundamentais do dia a dia.";
    prec =
      "Atenção ao aumento de pressão patelofemoral. Caso sinta dor no joelho anterior, reduza a amplitude da descida.";
  } else if (
    name.includes("prancha") ||
    name.includes("plank") ||
    name.includes("isometria") ||
    name.includes("ponte") ||
    name.includes("bridge") ||
    name.includes("estabilização")
  ) {
    ...
    tipsText =
      "Imagine que seu corpo é uma prancha rígida. Não permita que a lombar curve para baixo (hiperlordose) durante o exercício.";
    ...
    tipsText =
      "A escápula deve ser o pilar do movimento. Fixe-a suavemente no gradil costal para isolar a movimentação correta do braço e não tensionar excessivamente o trapézio superior.";
  ...
  if (ex.equipment && Array.isArray(ex.equipment)) {
    ...
    if (eq.includes("elástico") || eq.includes("band") || eq.includes("theraband")) {
      tipsText +=
        " Controle o retorno elástico! O tempo de volta à posição inicial (fase excêntrica) é o momento onde mais há ganho de controle motor. Não deixe a borracha puxar você de forma solta.";
      prec +=
        " Verifique sempre as extremidades do elástico para garantir que estão presas e não irão soltar de forma repentina durante a tensão.";
    }
    if (eq.includes("halteres") || eq.includes("peso") || eq.includes("kettlebell")) {
      tipsText +=
        " Movimente as sobrecargas de forma suave, sem impulsos corporais ou balanço (efeito pêndulo).";
    }
    if (eq.includes("bola") || eq.includes("bola suíça") || eq.includes("swiss ball")) {
      tipsText +=
        " A instabilidade da bola requer constante ativação do core. O foco principal deve ser sempre manter-se equilibrado antes de realizar o movimento do exercício.";
    }
  }
  ...
  const isGenericInstructions =
    !ex.instructions ||
    ex.instructions.includes(`Instruções para`) ||
    ex.instructions === "Nenhuma instrução adicional.";
...
async function run() {
  ...
  for (const ex of exercises) {
    ...
    const needsEnrichment =
      isGenericDescription || isGenericInstructions || !ex.tips || !ex.precautions || !ex.benefits;
  ...
  console.log(
    `Finalizado! Total de exercícios revisados com Algoritmo Procedural Avançado: ${updatedCount}`,
  );
}
