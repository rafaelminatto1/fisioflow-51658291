...
function protocolSection(title: string, items: string[], intro?: string): string {
  return [`## ${title}`, intro ?? "", ...items.map((item) => `- ${item}`)]
    .filter(Boolean)
    .join("\n");
}
...
async function upsertPage(
  ...
  await client.query(
    `
		...
		`,
    [id, page.title, page.content, "Criação automática das trilhas de evidência", userId],
  );
...
async function runOnce(
  ...
  try {
    ...
    const [lcaArticles, arthroplastyArticles, shoulderArticles, ankleArticles] = await Promise.all([
...
async function main() {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt += 1) {
    try {
      ...
      console.warn(`Tentativa ${attempt} falhou com erro transitorio; repetindo em ${attempt}s...`);
