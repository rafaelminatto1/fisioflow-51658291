...
const remoteExercises = [
  ...
  {"name": "Farmer's Carry (Caminhada do Fazendeiro)", "slug": "farmers-carry-caminhada-do-fazendeiro"},
  ...
  {"name": "Mobilização de Coluna Thorácica com Foam Roller", "slug": "mobilizacao-de-coluna-thoracica-com-foam-roller"},
  ...
  {"name": "Respiração com Labios Franzidos (Pursed Lip)", "slug": "respiracao-com-labios-franzidos-pursed-lip"},
  ...
  {"name": "Single Leg Stance com Movimento de Braço", "slug": "single-leg-stance-com-movimento-de-braco"},
...
console.log(`Auditing ${remoteExercises.length} remote exercises against ${localFiles.length} local files...\n`);
...
matches.forEach(m => {
    console.log(`UPDATE exercises SET image_url = '${m.newUrl}', thumbnail_url = '${m.newUrl}' WHERE slug = '${m.slug}' AND image_url LIKE 'http%';`);
});
