...
async function audit() {
  ...
  try {
    ...
    const res = await client.query(
      "SELECT id, name, slug, image_url FROM exercises WHERE image_url LIKE 'http%'",
    );
    const remoteExercises = res.rows;

    console.log(
      `Checking ${remoteExercises.length} remote exercises against ${files.length} local files...`,
    );
    ...
    for (const ex of remoteExercises) {
      ...
      const filenameMatch = files.find((f) => {
        ...
        if (ex.slug.includes("prancha-abdominal-plank") && f.includes("prancha-abdominal"))
          return true;
