...
async function main() {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        return (
          window.location.href.includes("science/journal") ||
    ...
    console.log(
      "Timeout waiting for login. Will proceed anyway, maybe it's not strictly required or you are already logged in.",
    );
  }
  ...
  for (const paper of papersInfo) {
    ...
    try {
      ...
      console.log(
        `Please manually download the PDF for ${paper.doi} into 'public/clinical-tests/papers/${paper.file}' if not done automatically.`,
      );

      // Try to click typical PDF download buttons
      const pdfButton = await page
        .locator(
          'text=Download PDF, text=View PDF, a:has-text("PDF"), .pdf-download-btn, a.pdf-link',
        )
        .first();
      if (await pdfButton.isVisible().catch(() => false)) {
        ...
        console.log(
          "Could not find a clear PDF download button. You have 20 seconds to interact and download it manually.",
        );
        await page.waitForTimeout(20000);
      }
    } catch (e) {
      console.log(
        `Error interacting with page for ${paper.doi}: ${e instanceof Error ? e.message : String(e)}. Waiting 15 seconds for manual action...`,
      );
