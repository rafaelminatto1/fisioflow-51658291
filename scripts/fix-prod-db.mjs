  ...
  try {
    ...
    for (const table of tables) {
      ...
      if (check.length === 0) {
        ...
        if (table === "patients")
          await sql`ALTER TABLE "patients" ADD COLUMN "deleted_at" timestamp`;
        else if (table === "appointments")
          await sql`ALTER TABLE "appointments" ADD COLUMN "deleted_at" timestamp`;
        else if (table === "sessions")
          await sql`ALTER TABLE "sessions" ADD COLUMN "deleted_at" timestamp`;
        else if (table === "transacoes")
          await sql`ALTER TABLE "transacoes" ADD COLUMN "deleted_at" timestamp`;
        else if (table === "pagamentos")
          await sql`ALTER TABLE "pagamentos" ADD COLUMN "deleted_at" timestamp`;
        else if (table === "medical_records")
          await sql`ALTER TABLE "medical_records" ADD COLUMN "deleted_at" timestamp`;
