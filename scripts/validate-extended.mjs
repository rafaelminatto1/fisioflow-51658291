...
const assertList = (r, label) =>
  r.ok
    ? ok(`${label} LIST → ${r.status} (${r.json?.data?.length ?? 0})`)
...
const assertOk = (r, label) =>
  r.ok
    ? ok(`${label} → ${r.status}`)
    : fail(`${label} → ${r.status}: ${JSON.stringify(r.json).slice(0, 120)}`);
...
async function main() {
  ...
  if (!patient) {
    fail("Nenhum paciente disponível — abortando");
    return;
  }
  ...
  await suite("Sessions SOAP — CRUD", async () => {
    ...
    const c = await req("POST", "/api/sessions", token, {
      ...
      subjective: "Dor lombar leve",
      objective: "Mobilidade 80%",
      assessment: "Melhora progressiva",
    ...
    if (sid) {
      ...
      assertOk(
        await req("PUT", `/api/sessions/${sid}`, token, {
          subjective: "Dor lombar reduzida",
          status: "draft",
        }),
        "Sessions UPDATE",
      );
      ...
      assertOk(
        await req("DELETE", `/api/sessions/${sid}`, token),
        "Sessions DELETE (finalizada→409 ok)",
      );
    }
  });
  ...
  await suite("Clinical — Conduct Library CRUD", async () => {
    ...
    if (cid) {
      assertOk(
        await req("PUT", `/api/clinical/conduct-library/${cid}`, token, {
      ...
      assertOk(
        await req("DELETE", `/api/clinical/conduct-library/${cid}`, token),
        "ConductLib DELETE",
      );
    }
  });
  ...
  await suite("Clinical — Evolution Templates CRUD", async () => {
    ...
    const c = await req("POST", "/api/clinical/evolution-templates", token, {
      name: `Template ${ts}`,
      description: "Template de validação",
    ...
    if (tid) {
      assertOk(
        await req("GET", `/api/clinical/evolution-templates/${tid}`, token),
        "EvoTemplate GET/:id",
      );
      assertOk(
        await req("PUT", `/api/clinical/evolution-templates/${tid}`, token, {
          name: `Template ${ts} EDIT`,
        }),
        "EvoTemplate UPDATE",
      );
      assertOk(
        await req("DELETE", `/api/clinical/evolution-templates/${tid}`, token),
        "EvoTemplate DELETE",
      );
    }
  });
  ...
  await suite("Clinical — Pain Maps CRUD", async () => {
    assertList(
      await req("GET", `/api/clinical/pain-maps?patientId=${patient.id}`, token),
      "PainMap",
    );
    ...
    if (pmid) {
      assertOk(
        await req("PUT", `/api/clinical/pain-maps/${pmid}`, token, { pain_level: 3 }),
        "PainMap UPDATE",
      );
  ...
  await suite("CRM — Leads CRUD", async () => {
    ...
    if (lid) {
      ...
      assertOk(
        await req("PUT", `/api/crm/leads/${lid}`, token, {
          estagio: "em_contato",
          observacoes: "Validação automática",
        }),
        "Leads UPDATE",
      );
      ...
      assertOk(
        await req("POST", `/api/crm/leads/${lid}/historico`, token, {
  ...
  await suite("CRM — Campanhas CRUD", async () => {
    ...
    const c = await req("POST", "/api/crm/campanhas", token, {
      nome: `Campanha ${ts}`,
      tipo: "whatsapp",
      conteudo: "Campanha de validação",
      status: "concluida",
    });
    ...
    if (cmpid) {
      assertOk(
        await req("PUT", `/api/crm/campanhas/${cmpid}`, token, { status: "concluida" }),
        "Campanhas UPDATE",
      );
  ...
  await suite("Eventos — CRUD", async () => {
    ...
    if (evid) {
      ...
      assertOk(
        await req("PUT", `/api/activities/${evid}`, token, { nome: `Evento ${ts} EDIT` }),
        "Eventos UPDATE",
      );
  ...
  await suite("Time Entries — CRUD + Stats", async () => {
    ...
    if (teid) {
      assertOk(
        await req("PATCH", `/api/time-entries/${teid}`, token, {
          description: `Sessão ${ts} EDIT`,
        }),
        "TimeEntries UPDATE",
      );
  ...
  await suite("Treatment Cycles — CRUD", async () => {
    ...
    if (tcid) {
      assertOk(
        await req("PATCH", `/api/treatment-cycles/${tcid}`, token, { status: "completed" }),
        "TreatCycles UPDATE",
      );
  ...
  await suite("Satisfaction Surveys — CRUD + Stats", async () => {
    ...
    if (svid) {
      assertOk(
        await req("PATCH", `/api/satisfaction-surveys/${svid}`, token, { comments: "Editado" }),
        "Surveys UPDATE",
      );
  ...
  await suite("Invitations — CRUD", async () => {
    ...
    if (iid) {
      assertOk(
        await req("PATCH", `/api/invitations/${iid}`, token, { role: "admin" }),
        "Invitations UPDATE",
      );
  ...
  await suite("Scheduling — settings", async () => {
    assertOk(
      await req("GET", "/api/scheduling/settings/business-hours", token),
      "Scheduling BUSINESS-HOURS",
    );
    assertOk(
      await req("GET", "/api/scheduling/settings/blocked-times", token),
      "Scheduling BLOCKED-TIMES",
    );
  ...
  await suite("Wiki — Categories + Comments", async () => {
    ...
    const cc = await req("POST", "/api/wiki/categories", token, {
      name: `Cat ${ts}`,
      color: "#3B82F6",
    });
  ...
  await suite("Exercise Plans — CRUD", async () => {
    ...
    if (epid) {
      assertOk(
        await req("PATCH", `/api/exercise-plans/${epid}`, token, { name: `Plano ${ts} EDIT` }),
        "ExercisePlans UPDATE",
      );
  ...
  await suite("Evolution Versions — list", async () => {
    ...
    const r = await req(
      "GET",
      "/api/evolution-versions?soapRecordId=00000000-0000-0000-0000-000000000000",
      token,
    );
  ...
  await suite("Goal Profiles — CRUD", async () => {
    ...
    if (gpid) {
      assertOk(
        await req("PUT", `/api/goal-profiles/${gpid}`, token, { name: `Perfil ${ts} EDIT` }),
        "GoalProfiles UPDATE",
      );
  ...
  await suite("Medical Requests — CRUD", async () => {
    assertList(
      await req("GET", `/api/medical-requests?patientId=${patient.id}`, token),
      "MedicalReqs",
    );
  ...
  await suite("Financeiro — Convênios + Formas de Pagamento", async () => {
    ...
    if (convid) {
      assertOk(
        await req("PUT", `/api/financial/convenios/${convid}`, token, {
          nome: `Convênio ${ts} EDIT`,
        }),
        "Convenios UPDATE",
      );
      assertOk(
        await req("DELETE", `/api/financial/convenios/${convid}`, token),
        "Convenios DELETE",
      );
    }
    ...
    if (fpid) {
      assertOk(
        await req("DELETE", `/api/financial/formas-pagamento/${fpid}`, token),
  ...
  await suite("Exercise Templates — CRUD", async () => {
    ...
    if (etid) {
      assertOk(
        await req("PUT", `/api/templates/${etid}`, token, { name: `Template ${ts} EDIT` }),
        "ExerciseTemplates UPDATE",
      );
