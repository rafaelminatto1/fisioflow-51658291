...
function checkUrl(
	url: string | null,
): Promise<{ status: number; format: string | null }> {
...
async function main() {
	...
	for (const exercise of exercises) {
		...
		if (exercise.imageUrl || exercise.thumbnailUrl) {
			if (isWorking) {
				...
				console.log(
					`      Imagem: ${imageCheck.status} (${imageCheck.format || "N/A"})`,
				);
				console.log(
	...
	if (brokenImages.length > 0) {
		...
		brokenImages.slice(0, 10).forEach((r) => {
			...
			if (r.imageUrl)
				console.log(`     Imagem: ${r.imageUrl} (${r.imageStatus})`);
			if (r.thumbnailUrl)
				console.log(`     Thumbnail: ${r.thumbnailUrl} (${r.thumbnailStatus})`);
		});
	...
	const REPORT_FILE = path.join(
		OUTPUT_DIR,
		`validation-report-${Date.now()}.txt`,
	);
	...
	if (brokenImages.length > 0) {
		...
		brokenImages.forEach((r) => {
			...
			if (r.imageUrl)
				report += `  Imagem: ${r.imageUrl} - Status: ${r.imageStatus}\n`;
			if (r.thumbnailUrl)
