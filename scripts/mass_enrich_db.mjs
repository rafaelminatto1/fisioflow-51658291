...
const clinicalData = {
    "alongamento-tensor-fascia-lata": {
        indicated: ["Síndrome do Trato Iliotibial", "Dor Lateral no Quadril", "Tensão na Cadeia Lateral"],
        contraindicated: ["Bursite Trocanteriana Aguda", "Lesão Muscular Aguda no TFL"],
        refs: { title: "Clinical Orthopaedic Rehabilitation", authors: "Brotzman & Manske", year: 2017 }
    },
    "avanco-isometrico": {
...
for (const ex of exercises) {
    ...
    if (!ex.reps_recommended && !ex.duration_seconds) {
        const isStretch = ex.name.toLowerCase().includes('alongamento') || ex.name.toLowerCase().includes('isometr');
        if (isStretch) {
    ...
    if (!ex.pathologies_indicated || ex.pathologies_indicated.length === 0) {
        ...
        if (clinicalData[slug]) {
            ...
            updateFields.pathologies_contraindicated = ["Fase Aguda de Inflamação", "Dor Intensa e Limitante"];
            updateFields.references = JSON.stringify({ title: "Protocolos Clínicos FisioFlow", authors: "Equipe Técnica FisioFlow", year: 2026 });
        }
        needsUpdate = true;
    }
...
for (const update of updates) {
    const sets = Object.entries(update.fields).map(([k, v]) => {
