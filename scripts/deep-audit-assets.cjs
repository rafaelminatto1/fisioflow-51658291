...
async function run() {
    try {
        ...
        const res = await client.query('SELECT id, slug, name, image_url FROM exercises WHERE is_active = true');
        const exercises = res.rows;
        
        const files = fs.readdirSync(ILLUSTRATIONS_DIR).filter(f => f.endsWith('.avif'));
        
        console.log(`🔍 Deep Audit: Checking ${exercises.length} exercises against ${files.length} assets.`);
