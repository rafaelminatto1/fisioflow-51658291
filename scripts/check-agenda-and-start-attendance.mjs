...
(async () => {
  ...
  try {
    ...
    if ((await page.locator(emailSel).count()) === 0) {
      ...
      await page.goto(BASE + "/?view=week&date=2026-02-04", {
    ...
    const corsErrors = consoleErrors.filter(
      (t) => t.includes("CORS") || t.includes("Access-Control"),
    );
    ...
    if (consoleErrors.length === 0) console.log("   Nenhum erro no console.");
      ...
      if (corsErrors.length)
        console.log("   CORS:", corsErrors.length, "–", corsErrors[0]?.slice(0, 120));
      if (otherErrors.length)
        console.log("   Outros:", otherErrors.length, "–", otherErrors[0]?.slice(0, 120));
    }
    ...
    if (startCount > 0) {
      console.log(
        "   Encontrado(s)",
        startCount,
        'botão(ões) "Iniciar atendimento" visível(is). Clicando no primeiro.',
      );
      ...
      if (clicked) {
        ...
        if ((await startBtn.count()) > 0) {
          ...
          const inDrawer = page
            ...
            .getByRole("button", { name: /iniciar atendimento/i });
        ...
        console.log(
          '   Nenhum card clicável encontrado. Página contém "agenda/paciente":',
          hasAgenda,
        );
      }
    }
    ...
    if (
      urlFinal.includes("evolution") ||
      urlFinal.includes("atendimento") ||
      urlFinal.includes("patient-evolution")
    ) {
