...
async function findDuplicates() {
  ...
  exercises.forEach(ex => {
    ...
    if (words.length > 0) {
      ...
      if (signature.length > 4) { // Evita signatures muito curtas como "com a"
  ...
  for (const [signature, group] of wordMatches.entries()) {
    ...
    if (group.length > 1) {
      ...
      if (uniqueNames.size > 1 && group.length <= 4) { // Filtra grupos gigantes como "alongamento" (terá 30+)
        susDupesCount++;
  ...
  console.log(`Resumo: ${exactDupesCount} grupos de nomes idênticos | ${susDupesCount} grupos de nomes suspeitamente similares.`);
}
