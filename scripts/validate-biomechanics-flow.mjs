...
function getPasswordLocator(page) {
	return page
		.locator(
			"[data-testid=auth-password-input], input[type='password'], input[name='password']",
		)
		.first();
}
...
function readVersionMarkers(html) {
	return {
		appVersion:
			html.match(/<meta name="app-version" content="([^"]+)"/)?.[1] ?? null,
		buildTime:
			html.match(/<meta name="build-time" content="([^"]+)"/)?.[1] ?? null,
	};
}
