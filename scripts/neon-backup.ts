...
async function runBackup() {
  ...
  try {
    ...
    const parentBranch = branches.find(
      (branch) => branch.id === PARENT_BRANCH || branch.name === PARENT_BRANCH,
    );

    if (!parentBranch) {
      throw new Error(
        `Branch pai "${PARENT_BRANCH}" não encontrada no projeto Neon ${NEON_PROJECT_ID}.`,
      );
    }

    // 1. Criar novo branch (Snapshot)
    console.log(
      `📦 Criando snapshot branch: ${backupName} a partir de ${parentBranch.name} (${parentBranch.id})...`,
    );
    ...
    const backupBranches = refreshedBranches.filter(
      (b: NeonBranch) =>
        b.name.startsWith("backup-") &&
        (createRes.data.branch ? b.id !== createRes.data.branch.id : true),
    );
