...
async function main() {
  ...
  if (h.ok) { ok(`Health → ${h.status}`); } else { fail(`Health → ${h.status}`); }
  ...
  await suite('Pacientes — CRUD completo', async () => {
    ...
    if (create.status === 201 && pid) { ok(`CREATE → 201 id=${pid}`); } else { fail(`CREATE → ${create.status}: ${JSON.stringify(create.json)}`); }

    if (pid) {
      const read = await req('GET', `/api/patients/${pid}`, token);
      if (read.ok) { ok(`READ → ${read.status}`); } else { fail(`READ → ${read.status}`); }

      const upd = await req('PATCH', `/api/patients/${pid}`, token, { full_name: `Paciente ${ts} EDIT`, status: 'Em tratamento' });
      if (upd.ok) { ok(`UPDATE → ${upd.status} nome="${upd.json?.data?.name}"`); } else { fail(`UPDATE → ${upd.status}: ${JSON.stringify(upd.json)}`); }

      const del = await req('DELETE', `/api/patients/${pid}`, token);
      if (del.ok) { ok(`DELETE → ${del.status} (soft)`); } else { fail(`DELETE → ${del.status}`); }
    }
  });
  ...
  await suite('Agendamentos — CRUD completo', async () => {
    ...
    if (aid) {
      ...
      if (read.ok) { ok(`READ → ${read.status} data="${read.json?.data?.date?.substring(0,10)}"`); } else { fail(`READ → ${read.status}`); }

      const upd = await req('PATCH', `/api/appointments/${aid}`, token, { status: 'confirmed', notes: 'Validado' });
      if (upd.ok) { ok(`UPDATE → ${upd.status} status="${upd.json?.data?.status}"`); } else { fail(`UPDATE → ${upd.status}: ${JSON.stringify(upd.json)}`); }

      const del = await req('DELETE', `/api/appointments/${aid}`, token);
      if (del.ok) { ok(`DELETE → ${del.status}`); } else { fail(`DELETE → ${del.status}`); }
    }
  });
  ...
  await suite('Financeiro — Transações CRUD', async () => {
    ...
    if (list.ok) { ok(`LIST → ${list.status} — ${list.json?.data?.length ?? 0} registros`); } else { fail(`LIST → ${list.status}: ${JSON.stringify(list.json)}`); }
    ...
    if (create.status === 201 && tid) { ok(`CREATE → 201 id=${tid}`); } else { fail(`CREATE → ${create.status}: ${JSON.stringify(create.json)}`); }
    ...
    if (tid) {
      ...
      if (upd.ok) { ok(`UPDATE → ${upd.status} valor="${upd.json?.data?.valor}"`); } else { fail(`UPDATE → ${upd.status}: ${JSON.stringify(upd.json)}`); }
  ...
  await suite('Financeiro — Contas CRUD', async () => {
    ...
    if (create.status === 201 && cid) { ok(`CREATE → 201 id=${cid}`); } else { fail(`CREATE → ${create.status}: ${JSON.stringify(create.json)}`); }
    ...
    if (cid) {
      ...
      if (upd.ok) { ok(`UPDATE → ${upd.status}`); } else { fail(`UPDATE → ${upd.status}: ${JSON.stringify(upd.json)}`); }
  ...
  await suite('Financeiro — Pagamentos (list)', async () => {
    ...
    if (r.ok) { ok(`LIST → ${r.status} — ${r.json?.data?.length ?? 0} registros`); } else { fail(`LIST → ${r.status}: ${JSON.stringify(r.json)}`); }
  });
  ...
  await suite('Exercícios — CRUD', async () => {
    ...
    if (cats.ok) { ok(`Categories → ${cats.status} — ${cats.json?.data?.length ?? 0}`); } else { fail(`Categories → ${cats.status}`); }

    const list = await req('GET', '/api/exercises', token);
    if (list.ok) { ok(`LIST → ${list.status} — ${list.json?.data?.length ?? 0}`); } else { fail(`LIST → ${list.status}`); }
    ...
    if (eid) { ok(`CREATE → ${create.status} id=${eid}`); } else { fail(`CREATE → ${create.status}: ${JSON.stringify(create.json)}`); }
  ...
  await suite('Metas — CRUD', async () => {
    ...
    if (list.ok) { ok(`LIST → ${list.status} — ${list.json?.data?.length ?? 0}`); } else { fail(`LIST → ${list.status}: ${JSON.stringify(list.json)}`); }
    ...
    if (create.status === 201 && gid) { ok(`CREATE → 201 id=${gid}`); } else { fail(`CREATE → ${create.status}: ${JSON.stringify(create.json)}`); }
  ...
  await suite('Wiki — CRUD', async () => {
    ...
    if (wslug || wid) { ok(`CREATE → ${create.status} slug="${wslug}"`); } else { fail(`CREATE → ${create.status}: ${JSON.stringify(create.json)}`); }
  ...
  await suite('Tarefas — CRUD', async () => {
    ...
    if (create.status === 201 && tarid) { ok(`CREATE → 201 id=${tarid}`); } else { fail(`CREATE → ${create.status}: ${JSON.stringify(create.json)}`); }
  ...
  await suite('Gamificação — list', async () => {
    ...
    if (r.ok) { ok(`Leaderboard → ${r.status}`); } else { fail(`Leaderboard → ${r.status}: ${JSON.stringify(r.json)}`); }

    const plist = await req('GET', '/api/patients?limit=1', token);
    const patient = plist.json?.data?.[0];
    if (patient) {
      const rp = await req('GET', `/api/gamification/profile/${patient.id}`, token);
      if (rp.ok) { ok(`Profile → ${rp.status}`); } else { fail(`Profile → ${rp.status}: ${JSON.stringify(rp.json)}`); }
    }
  });
  ...
  await suite('Documentos — list (com patientId)', async () => {
    ...
    if (r.ok) { ok(`LIST → ${r.status} — ${r.json?.data?.length ?? 0}`); } else { fail(`LIST → ${r.status}: ${JSON.stringify(r.json)}`); }
  });
  ...
  await suite('Perfil e Organização', async () => {
    ...
    if (profile.ok) { ok(`Profile → ${profile.status} uid="${uid}"`); } else { fail(`Profile → ${profile.status}: ${JSON.stringify(profile.json)}`); }

    const org = await req('GET', '/api/organizations/current', token);
    const nome = org.json?.data?.name || org.json?.name;
    if (org.ok) { ok(`Organização → ${org.status} nome="${nome}"`); } else { fail(`Org → ${org.status}: ${JSON.stringify(org.json)}`); }
  });
