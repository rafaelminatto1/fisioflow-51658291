...
# Script para converter exercícios restantes para AVIF
# Usa API direta do Cloudflare para baixar imagens

CDN_BASE="https://media.moocafisio.com.br"
BUCKET="fisioflow-media"
TEMP_DIR="/tmp/avif_batch"
...
# UUIDs remaining (from previous API call)
UUIDS=(
  "56b512a2-f095-4c5f-94db-c7a31ecb9e18" "577b7d6e-b3c3-431a-9d85-9addd133f3c9"
  "59933cf5-7599-43de-8f69-606575f91b4f" "5b42e888-789f-43e5-8145-5d41b325acc6"
  "5bc20d55-ceed-4fd1-8274-2eede587cb33" "5ce76038-76ef-43d6-8455-60290a618ff0"
  "5d1ccf64-73d9-46e1-a95b-c535dc634b1d" "5eca0c6e-41ba-4ed5-93c1-fb5ae4660db0"
  "61ab52c6-cdf5-406b-b940-d7dbbfcecc05" "61dcaaf9-92ed-4224-b242-d2406a00e8ea"
  "68efc336-a02f-4fb8-9f05-00afb97207c3" "69f345ad-dfbe-42e7-b476-fae3c48b12e6"
  "6b3ea758-f4cd-4c6a-bb1b-7f9732d3510d" "6b574243-5a72-44bd-b143-bdf5c8976940"
  "6bc8caa0-645b-469e-b7e8-4aa18d336761" "6e6fa606-3248-4a8f-a015-ae0026fae84a"
  "705de1c3-7aa2-4b32-bf9f-2bb903e79450" "70751514-d6d8-4ffc-9037-226b7433c590"
...
for UUID in "${UUIDS[@]}"; do
  for type in "image" "thumbnail"; do
    ...
    if ! file "${local_png}" | grep -q "PNG\|image"; then
      rm -f "${local_png}" "${local_avif}"
      continue
    fi
  ...
  if [ $((PROGRESS % 20)) -eq 0 ]; then
    log "Progress: $PROGRESS / $((TOTAL * 2)) - Success: $SUCCESS, Failed: $FAILED"
  fi
done
