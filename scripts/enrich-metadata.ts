...
const enriched = exerciseDictionary.map((ex) => {
  ...
  if (!meta.suggested_reps) {
    if (meta.subcategory === "Alongamento" || meta.pt.includes("Alongamento"))
      meta.suggested_reps = 30; // seconds
    else if (meta.subcategory === "Mobilidade" || meta.pt.includes("Mobilização"))
      meta.suggested_reps = 15;
    else if (meta.subcategory === "Propriocepção")
      meta.suggested_reps = 30; // seconds
    else if (meta.subcategory === "Vestibular")
      meta.suggested_reps = 60; // seconds
  ...
  if (!meta.suggested_rpe) {
    ...
    else if (meta.intensity_level === 1 || meta.subcategory === "Alongamento")
      meta.suggested_rpe = "4-5";
    else if (meta.subcategory === "Neurodinâmica") meta.suggested_rpe = "3-4";
    else if (meta.subcategory === "Respiratório" || meta.subcategory === "Vestibular")
      meta.suggested_rpe = "Baixo";
  ...
  if (!meta.required_equipment || meta.required_equipment.length === 0) {
    ...
    if (
      ...
      text.includes("miniband") ||
      text.includes("theraband")
    )
      eq.push("Faixa Elástica");
