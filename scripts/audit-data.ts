...
async function audit() {
    ...
    builtinClinicalTestsCatalog.forEach(test => {
        const missingFields = [];
        if (!test.evidence_summary || test.evidence_summary.trim() === '' || test.evidence_summary === 'Pendente') missingFields.push('evidence_summary');
        if (!test.execution || test.execution.trim() === '' || test.execution === 'Pendente') missingFields.push('execution/instructions');
        if (!test.reference || test.reference.trim() === '' || test.reference === 'Pendente') missingFields.push('reference');
        if (!test.purpose || test.purpose.trim() === '') missingFields.push('purpose');
        if (!test.positive_sign || test.positive_sign.trim() === '') missingFields.push('positive_sign');
        if (!test.image_url || test.image_url.startsWith('data:image/svg+xml')) missingFields.push('image_url (fallback SVG)');
