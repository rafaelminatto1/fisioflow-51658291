...
async function main() {
	...
	const r2Status: R2Status = JSON.parse(
		fs.readFileSync(R2_STATUS_FILE, "utf-8"),
	);
	...
	console.log(
		`📚 ${exercises.length} exercícios encontrados no banco de dados`,
	);
	console.log("");
	...
	const BACKUP_FILE = path.join(
		BACKUP_DIR,
		`exercises-urls-backup-${TIMESTAMP}.json`,
	);
	...
	const ROLLBACK_FILE = path.join(
		__dirname,
		"migration/rollback-exercises-urls.ts",
	);
...
`;
	...
	migrations
		...
		.forEach((m) => {
			...
			if (m.old_thumbnail_url)
				detailsText += `  thumbnail_url: ${m.old_thumbnail_url}\n`;
			detailsText += `\n`;
		});
	...
	console.log(
		"   2. Validar mudanças (npm run tsx scripts/validate-migration.ts)",
	);
