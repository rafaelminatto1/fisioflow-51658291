...
const LOGIN_EMAIL = process.env.E2E_LOGIN_EMAIL || "REDACTED_EMAIL";
const LOGIN_PASSWORD = process.env.E2E_LOGIN_PASSWORD || "REDACTED";

// Páginas públicas (sem login)
const PUBLIC_ROUTES = ["/welcome", "/auth", "/pre-cadastro", "/install"];
...
function setupConsoleListeners(page, errors, warnings) {
  const shouldIgnoreError = (text, _locUrl) => {
    ...
    if (text.includes("WebChannelConnection RPC") && text.includes("transport errored"))
      return true;
    if (text.includes("RunAggregationQuery") && text.includes("failed-precondition")) return true;
    if (
      text.includes("Invalid collection reference") &&
      text.includes("organizations/time_entries")
    )
      return true;
    ...
    if (
      (text.includes("brotli") || text.includes("crypto-js")) &&
  ...
  if (!loggedIn) {
    ...
    for (const route of PROTECTED_ROUTES) {
      ...
      if (result.loadError) {
        ...
        console.log(
          errCount > 0
            ? `${errCount} erros, ${warnCount} warns`
            : warnCount > 0
              ? `${warnCount} warns`
              : "ok",
        );
