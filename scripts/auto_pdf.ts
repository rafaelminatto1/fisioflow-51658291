...
const remainingPapers = [
  { file: 'hip-fai-combined-tests-palsson-2020.pdf', doi: '10.1016/j.math.2014.11.006', publisher: 'Elsevier' },
  { file: 'shoulder-impingement-calis-2000.pdf', doi: '10.1136/ard.59.1.24', publisher: 'BMJ' },
  { file: 'spurling-cervical-review-rubinstein-2007.pdf', doi: '10.1007/s00586-006-0224-4', publisher: 'Springer' }
];
...
async function autoLoginIfNeeded(page: Page) {
    try {
        ...
        const userInput = page.locator('input[type="email"], input[name*="user"], input[id*="user"], input[name*="login"], input[id*="identificacao"]').first();
        ...
        if (await passInput.isVisible({ timeout: 3000 }).catch(() => false)) {
            ...
            const submitBtn = page.locator('button[type="submit"], input[type="submit"], button:has-text("Login"), button:has-text("Entrar"), button:has-text("Sign in")').first();
...
async function main() {
  ...
  for (const paper of remainingPapers) {
      ...
      try {
          ...
          console.log("If the site requires you to click 'Sign in via Institution', please do it NOW via VNC.");
