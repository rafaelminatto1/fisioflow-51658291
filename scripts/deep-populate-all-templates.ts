...
async function main() {
  ...
  for (const t of allTemplates) {
    ...
    if (t.name.includes("DTM") || t.name.includes("ATM")) {
      notes =
        "Foco em relaxamento de masseter e temporal. Posição de descanso da língua (N). Biofeedback visual para evitar desvios.";
      ...
      notes =
        "Princípio da micro-progressão. Educação em dor (neurociência). Sessões curtas no início.";
      ...
      notes =
        "Extensão total passiva é PRIORIDADE. Ativação de quadríceps (VMO) para evitar inibição artrogênica.";
      ...
      t.name.includes("Lombar") ||
      t.name.includes("Disco") ||
      t.name.includes("Espondilo")
    ) {
      notes =
        "Baseado em McKenzie. Buscar centralização da dor. Fortalecimento de Transverso e Multífidos.";
      ...
      t.name.includes("Ombro") ||
      t.name.includes("Manguito") ||
      t.name.includes("Subacromial")
    ) {
      ...
      t.name.includes("Tornozelo") ||
      t.name.includes("Aquiles") ||
      t.name.includes("Fascite")
    ) {
      notes =
        "Treino de equilíbrio unipodal. Fortalecimento de sóleo e gastrocnêmio. Mobilidade de dorsiflexão.";
    ...
    await db
      ...
      .set({
        description:
          t.description || `Protocolo clínico especializado para ${t.conditionName || t.name}.`,
        clinicalNotes: notes,
