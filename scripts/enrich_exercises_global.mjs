...
const fallbackMap = {
    ...
    "Neural": {
        ...
        contraindicated: '["Neuropatia Aguda Progressiva", "Sinais de Alerta Neurológicos (Red Flags)"]',
...
while (i < content.length) {
    ...
    if (!exBody.includes('indicated_pathologies:')) {
        ...
        let matchCat = "Global";
        if (lowerBody.includes("ombro") || lowerBody.includes("escápula") || lowerBody.includes("peito") || lowerBody.includes("latíssimo")) matchCat = "Ombro";
        else if (lowerBody.includes("joelho") || lowerBody.includes("coxa") || lowerBody.includes("quadríceps") || lowerBody.includes("isquiotibiais") || lowerBody.includes("panturrilha") || lowerBody.includes("sóleo") || lowerBody.includes("gastrocnêmio")) matchCat = "Joelho";
        else if (lowerBody.includes("tornozelo") || lowerBody.includes("pé")) matchCat = "Tornozelo";
        else if (lowerBody.includes("coluna") || lowerBody.includes("lombar") || lowerBody.includes("torácica") || lowerBody.includes("core") || lowerBody.includes("abdômen")) matchCat = "Coluna";
        else if (lowerBody.includes("quadril") || lowerBody.includes("glúteo") || lowerBody.includes("piriforme")) matchCat = "Quadril";
        else if (lowerBody.includes("cotovelo") || lowerBody.includes("braço")) matchCat = "Cotovelo";
        else if (lowerBody.includes("punho") || lowerBody.includes("mão") || lowerBody.includes("antebraço")) matchCat = "Punho";
        ...
        else if (lowerBody.includes("mobilidade") || lowerBody.includes("liberação")) matchCat = "Mobilidade";
        else if (lowerBody.includes("equilíbrio") || lowerBody.includes("bosu")) matchCat = "Equilíbrio";
...
console.log(`Exercises enriched with global fallback successfully. Total modified: ${modifiedCount}`);
