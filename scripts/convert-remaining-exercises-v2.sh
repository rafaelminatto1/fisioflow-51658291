...
# Script to convert remaining exercises - handles both PNG and JPEG sources
...
UUIDS=(
  ...
  "5bc20d55-ceed-4fd1-8274-2eede587cb33"
)
...
echo "=== Starting conversion of $TOTAL exercises at $(date) ==="

for UUID in "${UUIDS[@]}"; do
  for type in "image" "thumbnail"; do
    src_url="${CDN_BASE}/exercises/${UUID}/${type}.png"
    local_file="${TEMP_DIR}/${UUID}_${type}"
    local_avif="${local_file}.avif"
    
    # Download source (PNG or JPEG)
    curl -s -o "${local_file}" "${src_url}" 2>/dev/null
    
    # Verify it's an image (not HTML error)
    if [ ! -f "${local_file}" ] || [ ! -s "${local_file}" ]; then
      rm -f "${local_file}" "${local_avif}"
      continue
    fi
    
    # Check if it's actually an image (not HTML)
    file_type=$(file "${local_file}" 2>/dev/null)
  ...
  # Progress every 20
  if [ $(( (SUCCESS + FAILED) % 20 )) -eq 0 ]; then
    echo "Progress: $((SUCCESS + FAILED)) / $((TOTAL * 2)) - Success: $SUCCESS, Failed: $FAILED"
  fi
done
