...
async function screenshotWithLogin() {
  ...
  try {
    ...
    const expandBtn = painScale.locator(
      'button[aria-label*="Expandir"], button:has(.lucide-chevron-down), button:has(svg.lucide-chevron-down)',
    );
    ...
    if (hasExpandBtn) {
      ...
      // Collapse back
      const collapseBtn = painScale.locator(
        'button[aria-label*="Recolher"], button:has(.lucide-chevron-up), button:has(svg.lucide-chevron-up)',
      );
