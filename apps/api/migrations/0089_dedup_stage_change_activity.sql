-- 0089_dedup_stage_change_activity.sql
--
-- Evita activity duplicada na criação de lead. Antes:
--   INSERT lead → worker grava `lead_created` + trigger grava `stage_change`
-- Agora: trigger só grava `stage_change` em UPDATE real (TG_OP='UPDATE').
...
BEGIN
  ...
  -- Activity APENAS em UPDATE (no INSERT o worker já gravou 'lead_created')
  IF TG_OP = 'UPDATE' THEN
    ...
    VALUES (
      ...
      'Estágio: ' || NEW.estagio,
      NEW.id,
      jsonb_build_object('from', OLD.estagio, 'to', NEW.estagio),
      NEW.responsavel_id
    );
  END IF;
