...
function getCachedResolvedAuthUser(candidate: CandidateAuthContext): AuthUser | null {
  ...
  const byEmail = getCachedAuthUserByKey(authUserCacheByEmail, normalizeEmailKey(candidate.email));
  ...
  try {
    ...
    if (!row && candidate.email) {
      ...
      if (row) {
        ...
        sql("UPDATE profiles SET user_id = $1, updated_at = NOW() WHERE email = $2", [
          candidate.uid,
          candidate.email,
        ]).catch((err) => console.error("[Auth] Erro ao auto-sincronizar user_id:", err));
...
function getSessionCandidate(sessionData: any): CandidateAuthContext | null {
  ...
  return {
    ...
    organizationId: sessionData.user?.organizationId || sessionData.user?.orgId || null,
...
async function resolveJwtCandidate(env: Env, token: string): Promise<AuthUser | null> {
  ...
  return resolveAuthContext(env, {
    ...
    organizationId: (payload as any).orgId || (payload as any).organizationId || null,
...
export async function verifyToken<E extends { Bindings: Env }>(
  ...
  try {
    ...
    if (!looksLikeJwt(token)) {
      console.log("[Auth] Token de sessão opaco detectado, usando fallback de validacao");
      ...
      try {
        ...
        if (res.rows && res.rows.length > 0) {
          ...
          return cacheVerifiedToken(
            token,
            await resolveAuthContext(env, {
              uid: row.userId,
              email: row.email,
              organizationId: row.organization_id,
              role: row.role,
