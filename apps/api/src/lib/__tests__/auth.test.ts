...
function makeContext(token?: string): any {
	return {
		req: {
			header: (name: string) => (name === 'Authorization' ? `Bearer ${token ?? makeFakeJwt('user-1', 'test@example.com')}` : undefined),
			query: () => undefined,
...
describe('verifyToken', () => {
	...
	it('permite token valido sem org explicita usando fallback padrão', async () => {
		...
		const user = await verifyToken(makeContext(makeFakeJwt('user-2', 'missing-org@example.com')), env);
	...
	it('usa organizationId explicito do token quando a consulta de perfil falha', async () => {
		...
		const user = await verifyToken(makeContext(makeFakeJwt('user-3', 'fallback@example.com', { organizationId: 'org-from-token', role: 'fisioterapeuta' })), env);
