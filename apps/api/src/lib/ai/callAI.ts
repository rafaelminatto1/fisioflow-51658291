...
function taskToConfigKey(
  task: AITask,
): keyof Pick<AIModelConfig, "chatModel" | "analysisModel" | "transcriptionModel"> {
  switch (task) {
    case "analysis":
...
export async function callAIStructured<T>(
  ...
  const schemaHint =
    ...
    "Respond with valid JSON matching the expected schema. No markdown, no explanation.";
