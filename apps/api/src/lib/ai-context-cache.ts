...
import { createContextCache, deleteContextCache, type GeminiModel } from "./ai-gemini-v2";
import {
  buildPatientContext,
...
async function deletePatientCacheEntry(env: Env, patientId: string): Promise<void> {
  if (!env.FISIOFLOW_CONFIG) return;
...
export async function invalidatePatientCache(env: Env, patientId: string): Promise<void> {
  ...
  try {
    ...
    console.warn(`[ai-context-cache] failed to delete remote cache ${entry.cacheName}:`, error);
  }
}
