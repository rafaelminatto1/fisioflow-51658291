...
export async function findOrCreateConversation(pool: Pool, orgId: string, contactId: string) {
  try {
    ...
    console.error("[whatsapp-conversations] findOrCreateConversation error:", error);
    throw error;
  }
}
...
export async function assignConversation(
  ...
  try {
    ...
    const targetUuid = orgId ? await resolveProfileUuid(pool, assignedTo, orgId) : assignedTo;
...
export async function transferConversation(
  ...
  try {
    ...
    const targetUuid = orgId ? await resolveProfileUuid(pool, newAssignee, orgId) : newAssignee;
    ...
    const transfer = await pool.query(
      `INSERT INTO wa_assignments (conversation_id, organization_id, assigned_to, assigned_by, assigned_team, reason)
	       ...
	       WHERE id = $1
	       RETURNING *`,
      [conversationId, targetUuid, transferredBy, team ?? null, reason ?? null],
    );

    return transfer.rows[0];
  } catch (error) {
    console.error("[whatsapp-conversations] transferConversation error:", error);
    throw error;
  }
}
...
export async function updateConversationStatus(pool: Pool, conversationId: string, status: string) {
  try {
    ...
    console.error("[whatsapp-conversations] updateConversationStatus error:", error);
    throw error;
  }
}
...
export async function getConversationWithMessages(
  ...
  try {
    ...
    console.error("[whatsapp-conversations] getConversationWithMessages error:", error);
    throw error;
  }
}
...
export async function updateConversationFields(
  ...
  try {
    ...
    console.error("[whatsapp-conversations] updateConversationFields error:", error);
    throw error;
  }
}
...
export async function markConversationDeleted(
  ...
  try {
    ...
    console.error("[whatsapp-conversations] markConversationDeleted error:", error);
    throw error;
  }
}
...
export async function restoreConversation(
  ...
  try {
    ...
    console.error("[whatsapp-conversations] restoreConversation error:", error);
    throw error;
  }
}
...
export async function editMessageContent(
  ...
  try {
    const existing = await getMessageForMutation(pool, conversationId, messageId, orgId);
    ...
    const editHistory = Array.isArray(metadata.edit_history) ? metadata.edit_history : [];
    const nextMetadata = {
      ...metadata,
      edited_at: now,
      edited_by: editedBy,
    ...
    const result = await pool.query(
      `UPDATE wa_messages
			 ...
			 RETURNING *`,
      [messageId, conversationId, orgId, JSON.stringify(content), JSON.stringify(nextMetadata)],
    );
      ...
      if (!canDeleteForEveryone(existing)) {
        return {
          ...
          deleteForEveryoneExpiresAt:
            getDeleteForEveryoneDeadline(existing.created_at)?.toISOString() ?? null,
        };
      }
    }
...
export async function getInboxConversations(
  ...
  try {
    ...
    console.error("[whatsapp-conversations] getInboxConversations error:", error);
    throw error;
  }
}
...
export async function getConversationMetrics(pool: Pool, orgId: string) {
  try {
    ...
    console.error("[whatsapp-conversations] getConversationMetrics error:", error);
    throw error;
  }
}
