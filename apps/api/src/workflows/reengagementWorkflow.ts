 ...
 */
export class PatientReengagementWorkflow extends WorkflowEntrypoint<
  Env,
...
> {
  async run(event: WorkflowEvent<PatientReengagementParams>, step: WorkflowStep) {
    ...
    let reengaged = false;
    try {
      await step.waitForEvent("patient-responded-1", {
    ...
    try {
      await step.waitForEvent("patient-responded-2", {
        type: "appointment-booked",
        timeout: "8 days",
      });
      reengaged = true;
