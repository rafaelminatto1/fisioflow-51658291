...
app.post("/login", loginRateLimit, async (c) => {
  ...
  try {
    const neonRes = await forwardToNeonAuth(neonAuthUrl, "/sign-in/email", "POST", {
      email,
      password,
    });
    ...
    const rawToken: string | null =
      neonRes.headers.get("set-auth-jwt") || neonData.token || neonData.access_token || null;
...
app.post("/signup", signupRateLimit, turnstileVerify, async (c) => {
  ...
  try {
    const neonRes = await forwardToNeonAuth(neonAuthUrl, "/sign-up/email", "POST", {
      email,
      password,
      name,
    });
    ...
    if (!neonRes.ok) {
      ...
      return c.json({ error: neonData.message || neonData.error || "Erro ao criar conta" }, status);
    }

    const token = neonRes.headers.get("set-auth-jwt") || neonData.token || neonData.access_token;
    return c.json({
      token,
      user: neonData.user || { email, name },
...
app.post("/reset-password", passwordResetRateLimit, turnstileVerify, async (c) => {
  ...
  try {
    const neonRes = await forwardToNeonAuth(neonAuthUrl, "/reset-password", "POST", {
      token,
      password,
    });
    if (!neonRes.ok) {
      ...
      return c.json({ error: neonData.message || "Erro ao redefinir senha" }, statusRP);
    }
