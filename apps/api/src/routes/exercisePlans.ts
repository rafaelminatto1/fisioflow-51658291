...
app.get('/:id/compliance', requireAuth, async (c) => {
  ...
  const completedDays = new Set(
    sessions.filter((s: any) => s.completed).map((s: any) => s.session_date?.toString?.()?.slice(0, 10) ?? ''),
  ).size;
  ...
  const last14 = Array.from({ length: 14 }, (_, i) => {
    ...
    const hasSession = sessions.some((s: any) => s.completed && s.session_date?.toString?.()?.slice(0, 10) === dateStr);
