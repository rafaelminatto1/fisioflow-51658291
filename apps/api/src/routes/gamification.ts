...
app.put("/quests/:patientId/complete", requireAuth, async (c) => {
  ...
  return c.json({
    data: { ...record, quests_data: updatedQuests, completed_count: completedCount },
    xpAwarded: quest.xp,
  });
});
...
app.get("/achievements", requireAuth, async (c) => {
  ...
  const [allResult, unlockedResult] = await Promise.all([
    ...
    pool.query("SELECT * FROM achievements_log WHERE patient_id = $1 ORDER BY unlocked_at DESC", [
      patientId,
    ]),
  ]);
...
app.post("/buy", requireAuth, async (c) => {
  ...
  // Get profile
  const profResult = await pool.query("SELECT * FROM patient_gamification WHERE patient_id = $1", [
    patientId,
  ]);
  ...
  const updatedProf = await pool.query("SELECT * FROM patient_gamification WHERE patient_id = $1", [
    patientId,
  ]);
...
app.get("/admin/stats", requireAuth, async (c) => {
  ...
  const activeLast30Days = rows.filter(
    (row) => row.last_activity_date && row.last_activity_date >= startDate,
  ).length;
  const activeLast7Days = rows.filter(
    (row) => row.last_activity_date && row.last_activity_date >= sevenDaysAgo,
  ).length;
  const atRiskPatients = rows.filter(
    (row) => !row.last_activity_date || row.last_activity_date < sevenDaysAgo,
  ).length;
  const totalXpAwarded = xpRows.rows.reduce((sum, row) => sum + (row.amount ?? 0), 0);
  const averageLevel = rows.length
    ? rows.reduce((sum, row) => sum + (row.level ?? 0), 0) / rows.length
    : 0;
  const averageStreak = rows.length
    ? rows.reduce((sum, row) => sum + (row.current_streak ?? 0), 0) / rows.length
    : 0;
...
app.get("/admin/at-risk", requireAuth, async (c) => {
  ...
  const inactiveSince = new Date(
    Date.now() - (Number.parseInt(days, 10) || 7) * 86400000,
  ).toISOString();
  const result = await pool.query(
    `SELECT
        pg.patient_id,
...
app.get("/quest-definitions", requireAuth, async (c) => {
  const pool = await createPool(c.env);
  const result = await pool.query(
    "SELECT * FROM quest_definitions ORDER BY created_at DESC NULLS LAST, title ASC",
  );
  try {
    return c.json({ data: result.rows || result });
  } catch {
...
app.get("/weekly-challenges", requireAuth, async (c) => {
  ...
  if (active === "true") {
    ...
    conditions.push(
      `is_active = true AND start_date <= $${params.length} AND end_date >= $${params.length}`,
    );
  }
  ...
  if (patientId) {
    const progress = await pool.query("SELECT * FROM patient_challenges WHERE patient_id = $1", [
      patientId,
    ]);
...
app.post("/weekly-challenges", requireAuth, async (c) => {
  ...
  return c.json(
    { data: { ...result.rows[0], target: parseJsonField(result.rows[0]?.target, {}) } },
    201,
  );
});
...
app.get("/shop-items", requireAuth, async (c) => {
  const pool = await createPool(c.env);
  const result = await pool.query(
    "SELECT * FROM shop_items ORDER BY created_at DESC NULLS LAST, cost ASC",
  );
  try {
    return c.json({ data: result.rows || result });
  } catch {
