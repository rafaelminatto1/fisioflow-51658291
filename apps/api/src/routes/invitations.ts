...
app.post('/', requireAuth, async (c) => {
  ...
  const result = await db.query(
    ...
    [user.organizationId, body.email.trim().toLowerCase(), body.role ?? 'fisioterapeuta', token, user.uid, expiresAt.toISOString()]
  );
...
app.patch('/:id', requireAuth, async (c) => {
  ...
  const result = await db.query(
    ...
    [body.email ? body.email.trim().toLowerCase() : null, body.role ?? null, body.expires_at ?? null, id, user.organizationId]
  );
