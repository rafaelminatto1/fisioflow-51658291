...
function normalizeStoredTemplateForResponse(template: Record<string, unknown>) {
...
async function loadOrganizationSettings(
  ...
  try {
    const result = await pool.query(`SELECT settings FROM organizations WHERE id = $1 LIMIT 1`, [
      organizationId,
    ]);
...
app.get("/templates", requireAuth, async (c) => {
  ...
  const pool = await createPool(c.env);
  const settings = await loadOrganizationSettings(pool, user.organizationId);
  const templates = getStoredTemplates(settings).map(normalizeStoredTemplateForResponse);
...
app.put("/templates/:id", requireAuth, async (c) => {
  ...
  existingTemplates[currentIndex] = {
    ...
    ...(body.template_key !== undefined ? { template_key: body.template_key.trim() } : {}),
    ...(body.variables !== undefined
...
app.delete("/templates/:id", requireAuth, async (c) => {
  ...
  const settings = await loadOrganizationSettings(pool, user.organizationId);
  const existingTemplates = getStoredTemplates(settings);
  const nextTemplates = existingTemplates.filter((template) => String(template.id) !== id);
...
app.put("/automations/:id", requireAuth, async (c) => {
  ...
  try {
    ...
    if (result.rows.length === 0) return c.json({ error: "Automation not found" }, 404);
    const r = result.rows[0];
    return c.json({
      data: {
        id: r.id,
...
app.post("/templates", requireAuth, async (c) => {
  ...
  if (body.localOnly || body.content !== undefined) {
    ...
    if (existingTemplates.some((template) => String(template.template_key) === templateKey)) {
  ...
  try {
    const pool = await createPool(c.env);
    const settings = await pool.query(`SELECT settings FROM organizations WHERE id = $1 LIMIT 1`, [
      user.organizationId,
    ]);
    const wabaId = (settings.rows[0]?.settings as Record<string, unknown> | null)
      ?.whatsapp_business_account_id as string | undefined;

    const targetId = wabaId || phoneId;

    const metaRes = await fetch(`https://graph.facebook.com/v21.0/${targetId}/message_templates`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
    ...
    if (!metaRes.ok) {
      const metaError = metaData.error as { message?: string } | undefined;
      return c.json(
        {
          error: metaError?.message || "Failed to create template on Meta",
          details: metaData,
        },
...
app.post("/templates/sync", requireAuth, async (c) => {
  ...
  try {
    ...
    await pool.query(`UPDATE organizations SET settings = $1::jsonb WHERE id = $2`, [
      JSON.stringify({ ...settings, whatsapp_templates: updatedTemplates }),
      user.organizationId,
    ]);
