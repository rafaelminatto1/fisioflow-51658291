...
describe('create package input validation', () => {
  ...
  it('aceita input válido', () => {
    expect(validateCreateInput({ name: 'Pacote 10 sessões', total_sessions: 10, price: 600 })).toBeNull();
  });
  ...
  it('rejeita total_sessions zero', () => {
    expect(validateCreateInput({ name: 'X', total_sessions: 0, price: 100 })).toMatch(/total_sessions/);
  });
...
describe('sell package validation', () => {
  ...
  it('rejeita patient_id inválido', () => {
    expect(validateSell({ patient_id: 'bad', package_id: validId, amount_paid: 500 })).toMatch(/patient_id/);
  });

  it('rejeita amount_paid zero', () => {
    expect(validateSell({ patient_id: validId, package_id: validId, amount_paid: 0 })).toMatch(/amount_paid/);
  });
});
