...
describe("media routes", () => {
  ...
  it("POST /api/media/upload-url sanitizes folder and returns signed upload metadata", async () => {
    ...
    expect(json.data.key).toMatch(
      /^patient-examsunsafepath\/\d{4}-\d{2}-\d{2}\/user-media-001\/uuid-fixed-123\.png$/,
    );
  ...
  it("DELETE /api/media/:key blocks deletion for files owned by another user", async () => {
    const app = await buildApp();
    const res = await app.fetch(
      makeRequest("DELETE", "/api/media/patient-exams/2026-04-05/user-other-001/file.png"),
      BASE_ENV as any,
    );
