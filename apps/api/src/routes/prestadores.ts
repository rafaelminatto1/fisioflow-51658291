...
async function ensureEventBelongsToOrg(pool: any, eventId: string, organizationId: string) {
  const result = await pool.query("SELECT organization_id FROM eventos WHERE id = $1 LIMIT 1", [
    eventId,
  ]);
...
async function getPrestadorWithEvent(pool: any, id: string) {
...
app.put("/:id", requireAuth, async (c) => {
  ...
  if (prestador.organization_id !== user.organizationId)
...
app.put("/:id/status", requireAuth, async (c) => {
  ...
  const current = await pool.query(
    "SELECT status_pagamento FROM prestadores WHERE id = $1 LIMIT 1",
    [id],
  );
