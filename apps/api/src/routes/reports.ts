...
app.get("/medical-templates", requireAuth, async (c) => {
  ...
  return c.json({
    data: result.rows.map((row) => normalizeMedicalTemplate(row as Record<string, unknown>)),
  });
});
...
app.delete("/medical-templates/:id", requireAuth, async (c) => {
  ...
  await pool.query("DELETE FROM medical_report_templates WHERE id = $1 AND organization_id = $2", [
    id,
    user.organizationId,
  ]);
...
app.get("/medical", requireAuth, async (c) => {
  ...
  return c.json({
    data: result.rows.map((row) => normalizeMedicalReport(row as Record<string, unknown>)),
  });
});
...
app.delete("/medical/:id", requireAuth, async (c) => {
  ...
  await pool.query("DELETE FROM medical_reports WHERE id = $1 AND organization_id = $2", [
    id,
    user.organizationId,
  ]);
  return c.json({ ok: true });
...
app.get("/convenio", requireAuth, async (c) => {
  ...
  return c.json({
    data: result.rows.map((row) => normalizeConvenioReport(row as Record<string, unknown>)),
  });
});
...
app.delete("/convenio/:id", requireAuth, async (c) => {
  ...
  await pool.query("DELETE FROM convenio_reports WHERE id = $1 AND organization_id = $2", [
    id,
    user.organizationId,
  ]);
  return c.json({ ok: true });
