...
const VALID_CONVERSATION_STATUSES = ["open", "pending", "assigned", "resolved", "closed"];
...
function mapMessageRow(row: any): any {
  ...
  const isInternalNote =
    row.is_internal_note === true || messageType === "note" || row.direction === "internal";
  const content =
  ...
  return {
    ...
    deleteForEveryoneExpiresAt: deleteForEveryoneDeadline?.toISOString() || undefined,
...
app.get("/conversations", requireAuth, async (c) => {
  ...
  try {
    ...
    const { data: conversations, total } = await getInboxConversations(pool, user.organizationId, {
...
app.patch("/conversations/:id", requireAuth, async (c) => {
  ...
  const hasField = (key: keyof typeof body) => Object.prototype.hasOwnProperty.call(body, key);
  ...
  if (body.status !== undefined && !VALID_CONVERSATION_STATUSES.includes(body.status)) {
    return c.json(
      {
  ...
  if (body.priority !== undefined && !VALID_CONVERSATION_PRIORITIES.includes(body.priority)) {
    return c.json(
      {
  ...
  try {
    const existing = await getConversationWithMessages(pool, id, user.organizationId, 0);
    if (!existing) {
      return c.json({ error: "Conversation not found" }, 404);
    }
    ...
    const updated = await getConversationWithMessages(pool, id, user.organizationId, 0);
...
app.post("/conversations/:id/restore", requireAuth, async (c) => {
  ...
  try {
    const result = await restoreConversation(pool, id, user.organizationId, user.uid);
...
app.post("/conversations/:id/messages", requireAuth, async (c) => {
  ...
  try {
    ...
    const messageType = body.messageType || body.type || (body.attachmentUrl ? "image" : "text");
    const logContext = {
      requestId,
      conversationId: id,
    ...
    if (!to) {
      ...
      try {
        ...
        if (!metaRes.ok) {
          ...
          sendError = {
            kind: "meta_api_error",
            message: metaData?.error?.message ?? `Meta Graph API returned HTTP ${metaRes.status}`,
...
app.patch("/conversations/:id/messages/:messageId", requireAuth, async (c) => {
  ...
  try {
    ...
    console.error("[WhatsApp Inbox] PATCH /conversations/:id/messages/:messageId error:", err);
...
app.delete("/conversations/:id/messages/:messageId", requireAuth, async (c) => {
  ...
  try {
    ...
    if (result.error === "not_outbound") {
      return c.json({ error: "Only outbound messages can be deleted for everyone" }, 400);
    }
    ...
    await broadcastToOrg(c.env, user.organizationId, {
      type:
        scope === "everyone" ? "whatsapp_message_deleted_for_everyone" : "whatsapp_message_deleted",
      conversationId: id,
      messageId,
      deletedBy: user.uid,
      scope,
    ...
    console.error("[WhatsApp Inbox] DELETE /conversations/:id/messages/:messageId error:", err);
...
app.post("/conversations/:id/interactive", requireAuth, async (c) => {
  ...
  try {
    ...
    if (body.type === "button" && body.buttons) {
      metaResult = await sendReplyButtons(c.env, to, body.bodyText, body.buttons, {
        header: body.header,
        footer: body.footer,
        bsuid: conv.bsuid,
      });
      ...
      return c.json({ error: "Invalid interactive message type or missing parameters" }, 400);
    }
    ...
    console.error("[WhatsApp Inbox] POST /conversations/:id/interactive error:", err);
...
app.post("/conversations/:id/assign", requireAuth, async (c) => {
  ...
  try {
    ...
    console.error("[WhatsApp Inbox] POST /conversations/:id/assign error:", err);
...
app.post("/conversations/:id/transfer", requireAuth, async (c) => {
  ...
  try {
    ...
    console.error("[WhatsApp Inbox] POST /conversations/:id/transfer error:", err);
...
app.post("/conversations/:id/notes", requireAuth, async (c) => {
  ...
  try {
    const result = await addInternalNote(pool, id, user.organizationId, user.uid, body.content);
...
app.get("/tags", requireAuth, async (c) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      ...
      result = await pool.query(`SELECT * FROM wa_tags WHERE organization_id = $1 ORDER BY name`, [
        user.organizationId,
      ]);
    }
...
app.delete("/conversations/:id/tags/:tagId", requireAuth, async (c) => {
  ...
  try {
    ...
    console.error("[WhatsApp Inbox] DELETE /conversations/:id/tags/:tagId error:", err);
...
app.post("/broadcast", requireAuth, async (c) => {
  ...
  for (const contactId of body.contactIds) {
    try {
      ...
      const conversation = await findOrCreateConversationSvc(pool, user.organizationId, contactId);
  ...
  try {
    ...
    const activities = [
      ...
      ...messages.rows.map((r) => ({
        ...
        timestamp: r.created_at,
        description: r.message_type === "template" ? "Template enviado" : "Nota interna adicionada",
