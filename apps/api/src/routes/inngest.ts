  ...
  async ({ event, step, env }: { event: any; step: any; env: Env }) => {
    ...
    await step.run("send-whatsapp-reminder", async () => {
      ...
      try {
        ...
        return await whatsapp.sendSmartTemplate(phone, "lembrete_consulta_v1", [
          name,
          date,
          startTime,
          calendarUrl,
    ...
    await step.run("send-welcome-whatsapp", async () => {
      ...
      if (hasApp) {
        ...
        return await whatsapp.sendTextMessage(
          phone,
          `Seja bem-vindo(a), ${name}! 🚀 Suas consultas já estão sincronizadas com a agenda do seu celular através do nosso App.`,
        );
      } else {
        ...
        return await whatsapp.sendTextMessage(
          phone,
          `Seja bem-vindo(a), ${name}! 🚀 Clique no link abaixo para adicionar suas consultas automaticamente à agenda do seu celular:\n\n${calendarUrl}`,
        );
      }
    });
