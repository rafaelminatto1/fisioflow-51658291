...
app.post("/", requireAuth, async (c) => {
	...
	if (!type || !messageBody) {
		return c.json({ error: "type e body são obrigatórios" }, 400);
	}

	// Support for bulk messaging
	const targetPatientIds = Array.isArray(patient_ids)
		? patient_ids
		: patient_id
			? [patient_id]
			: [];
	...
	if (targetRecipients.length === 0) {
		return c.json({ error: "Ao menos um destinatário é obrigatório" }, 400);
	}

	const results = [];

	// We'll process them in a loop for now.
	// In a real high-scale scenario, we'd use a background queue or a single multi-row INSERT.
	for (let i = 0; i < targetRecipients.length; i++) {
		const currentRecipient = targetRecipients[i];
		const currentPatientId = targetPatientIds[i] || null;
...
app.delete("/:id", requireAuth, async (c) => {
	...
	await pool.query(
		"DELETE FROM communication_logs WHERE id = $1 AND organization_id = $2",
...
app.post("/:id/resend", requireAuth, async (c) => {
	...
	if (!result.rows.length)
		return c.json({ error: "Comunicação não encontrada" }, 404);
