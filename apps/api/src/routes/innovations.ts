...
app.put("/inventory/:id", requireAuth, async (c) => {
  ...
  if (body.current_quantity !== undefined)
    mapField("current_quantity", Number(body.current_quantity));
  if (body.minimum_quantity !== undefined)
    mapField("minimum_quantity", Number(body.minimum_quantity));
  if (body.unit !== undefined) mapField("unit", body.unit);
  if (body.cost_per_unit !== undefined)
    mapField("cost_per_unit", body.cost_per_unit != null ? Number(body.cost_per_unit) : null);
...
app.post("/inventory-movements", requireAuth, async (c) => {
  ...
  if (!Number.isFinite(quantity) || quantity <= 0)
    return c.json({ error: "quantity inválido" }, 400);
...
app.get("/inventory/forecast", requireAuth, async (c) => {
  ...
  try {
    ...
    const forecasts = inventory.rows.map((item: any) => {
      ...
      const estimatedAvg =
        avgConsumption || (item.item_name.toLowerCase().includes("gel") ? 0.05 : 0.1);

      const predictedMonthlyNeed = sessionCount * estimatedAvg;
      const daysRemaining =
        estimatedAvg > 0 ? item.current_quantity / (estimatedAvg * (sessionCount / 30)) : 999;
