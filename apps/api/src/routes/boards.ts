...
app.post("/", requireAuth, async (c) => {
  try {
    ...
    const backgroundImage = cleanString(body.background_image) ?? cleanString(body.backgroundImage);
    ...
    const result = await db.query(
      `WITH new_board AS (
       ...
       FROM new_board`,
      [user.organizationId, user.uid, name, description, backgroundColor, backgroundImage, icon],
    );
...
app.delete("/columns/:colId", requireAuth, async (c) => {
  try {
    ...
    await db.query(`UPDATE tarefas SET column_id = NULL WHERE column_id = $1`, [colId]);
    await db.query(
...
app.delete("/checklist-templates/:templateId", requireAuth, async (c) => {
  try {
    ...
    await db.query(`DELETE FROM board_checklist_templates WHERE id = $1 AND organization_id = $2`, [
      templateId,
      user.organizationId,
    ]);
    return c.json({ ok: true });
...
app.delete("/automations/:automationId", requireAuth, async (c) => {
  try {
    ...
    await db.query(`DELETE FROM board_automations WHERE id = $1 AND organization_id = $2`, [
      automationId,
      user.organizationId,
    ]);
    return c.json({ ok: true });
