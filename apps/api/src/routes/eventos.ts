...
app.post("/activities", requireAuth, async (c) => {
  ...
  const result = await pool.query(
    ...
    [
      ...
      b.hora_fim ?? null,
      b.gratuito ?? false,
      b.link_whatsapp ?? null,
      b.valor_padrao_prestador ?? null,
      b.status ?? "ativo",
      b.participantes_previstos ?? 0,
      b.minimo_colaboradores ?? 0,
    ],
  );
...
app.put("/activities/:id", requireAuth, async (c) => {
  ...
  const result = await pool.query(
    ...
    [
      ...
      b.valor_padrao_prestador ?? null,
      b.status ?? null,
      b.participantes_previstos ?? null,
      b.minimo_colaboradores ?? null,
    ],
  );
  if (!result.rows.length) return c.json({ error: "Não encontrado" }, 404);
  return c.json({ data: result.rows[0] });
...
app.delete("/activities/:id", requireAuth, async (c) => {
  ...
  const pool = await createPool(c.env);
  const { id } = c.req.param();
  await pool.query("DELETE FROM eventos WHERE id=$1 AND organization_id=$2", [
    id,
    user.organizationId,
  ]);
...
app.get("/salas", requireAuth, async (c) => {
  const user = c.get("user");
  const pool = await createPool(c.env);
  const result = await pool.query("SELECT * FROM salas WHERE organization_id=$1 ORDER BY nome", [
    user.organizationId,
  ]);
  try {
    return c.json({ data: result.rows || result });
...
app.delete("/salas/:id", requireAuth, async (c) => {
  ...
  const pool = await createPool(c.env);
  const { id } = c.req.param();
  await pool.query("DELETE FROM salas WHERE id=$1 AND organization_id=$2", [
    id,
    user.organizationId,
  ]);
  return c.json({ ok: true });
...
app.get("/servicos", requireAuth, async (c) => {
  const user = c.get("user");
  const pool = await createPool(c.env);
  const result = await pool.query("SELECT * FROM servicos WHERE organization_id=$1 ORDER BY nome", [
    user.organizationId,
  ]);
  try {
    return c.json({ data: result.rows || result });
...
app.delete("/servicos/:id", requireAuth, async (c) => {
  ...
  const pool = await createPool(c.env);
  const { id } = c.req.param();
  await pool.query("DELETE FROM servicos WHERE id=$1 AND organization_id=$2", [
    id,
    user.organizationId,
  ]);
  return c.json({ ok: true });
...
app.get("/contratados", requireAuth, async (c) => {
  ...
  const result = hasOrg
    ? await pool.query("SELECT * FROM contratados WHERE organization_id = $1 ORDER BY nome ASC", [
        user.organizationId,
      ])
...
app.put("/contratados/:id", requireAuth, async (c) => {
  ...
  if (body.contato !== undefined) {
    params.push(body.contato);
    sets.push(`contato = $${params.length}`);
  }
  if (body.cpf_cnpj !== undefined) {
    params.push(body.cpf_cnpj);
    sets.push(`cpf_cnpj = $${params.length}`);
  }
  if (body.especialidade !== undefined) {
    params.push(body.especialidade);
    sets.push(`especialidade = $${params.length}`);
  }
...
app.put("/checklist/:id", requireAuth, async (c) => {
  ...
  if (body.quantidade !== undefined) {
    params.push(Number(body.quantidade));
    sets.push(`quantidade = $${params.length}`);
  }
  if (body.custo_unitario !== undefined) {
    params.push(Number(body.custo_unitario));
    sets.push(`custo_unitario = $${params.length}`);
  }
...
app.delete("/contratados/:id", requireAuth, async (c) => {
  ...
  if (hasOrg) {
    await pool.query("DELETE FROM contratados WHERE id = $1 AND organization_id = $2", [
      id,
      user.organizationId,
    ]);
  } else {
...
app.get("/activity-contractors", requireAuth, async (c) => {
  ...
  if (eventoId) {
    ...
    conditions.push(`ec.evento_id = $${params.length}`);
  }
  if (contratadoId) {
    params.push(contratadoId);
    conditions.push(`ec.contratado_id = $${params.length}`);
  }
...
app.put("/activity-contractors/:id", requireAuth, async (c) => {
  ...
  if (body.funcao !== undefined) {
    params.push(body.funcao);
    sets.push(`funcao = $${params.length}`);
  }
  if (body.valor_acordado !== undefined) {
    params.push(Number(body.valor_acordado));
    sets.push(`valor_acordado = $${params.length}`);
  }
  if (body.horario_inicio !== undefined) {
    params.push(body.horario_inicio);
    sets.push(`horario_inicio = $${params.length}`);
  }
  if (body.horario_fim !== undefined) {
    params.push(body.horario_fim);
    sets.push(`horario_fim = $${params.length}`);
  }
  if (body.status_pagamento !== undefined) {
    params.push(body.status_pagamento);
    sets.push(`status_pagamento = $${params.length}`);
  }
...
app.get("/participantes", requireAuth, async (c) => {
  ...
  if (eventoId) {
    params.push(eventoId);
    conditions.push(`p.evento_id = $${params.length}`);
  }
  params.push(Number(limit), Number(offset));
...
app.post("/participantes", requireAuth, async (c) => {
  ...
  if (!body.evento_id || !body.nome)
    return c.json({ error: "evento_id e nome são obrigatórios" }, 400);
...
app.put("/participantes/:id", requireAuth, async (c) => {
  ...
  if (body.instagram !== undefined) {
    params.push(body.instagram);
    sets.push(`instagram = $${params.length}`);
  }
  if (body.segue_perfil !== undefined) {
    params.push(body.segue_perfil);
    sets.push(`segue_perfil = $${params.length}`);
  }
  if (body.observacoes !== undefined) {
    params.push(body.observacoes);
    sets.push(`observacoes = $${params.length}`);
...
app.delete("/activity-templates/:id", requireAuth, async (c) => {
  ...
  await pool.query("DELETE FROM evento_templates WHERE id = $1 AND organization_id = $2", [
    id,
    user.organizationId,
  ]);
