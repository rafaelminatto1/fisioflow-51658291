...
function normalizeTextArray(value: unknown): string[] {
  if (Array.isArray(value)) {
    return value.map((item) => trimmedString(item)).filter((item): item is string => Boolean(item));
  }
  ...
  if (typeof value === "string") {
    ...
    if (normalized.startsWith("[") || normalized.startsWith("{")) {
      try {
        const parsed = JSON.parse(normalized.replace(/^\{/, "[").replace(/\}$/, "]")) as unknown;
...
function deriveIsActive(body: PatientPayload): boolean {
  ...
  if (
    ["inactive", "inativo", "archived", "arquivado", "deleted", "excluido", "excluído"].includes(
      status,
    )
  ) {
    return false;
...
function normalizePatientRow(row: DbRow) {
  ...
  const incompleteReg = row.incompleteRegistration ?? row.incomplete_registration;
...
function buildPatientWritePayload(
  ...
  if (["address", "city", "state", "zip_code"].some((key) => body[key] !== undefined)) {
    payload.address = buildAddressPayload(body);
  }
  if (
    ["emergency_contact", "emergency_phone", "emergency_contact_relationship"].some(
  ...
  if (
    ["health_insurance", "insurance_plan", "insurance_number", "insurance_validity"].some(
      (key) => body[key] !== undefined,
    )
  ) {
  ...
  if (body.care_profiles !== undefined || body.careProfiles !== undefined) {
    payload.careProfiles = parseStringArrayInput(body.care_profiles ?? body.careProfiles);
  }
  if (body.sports_practiced !== undefined || body.sportsPracticed !== undefined) {
    payload.sportsPracticed = parseStringArrayInput(body.sports_practiced ?? body.sportsPracticed);
  }
  if (body.therapy_focuses !== undefined || body.therapyFocuses !== undefined) {
    payload.therapyFocuses = parseStringArrayInput(body.therapy_focuses ?? body.therapyFocuses);
  }
  ...
  if (body.partner_company_name !== undefined || body.partnerCompanyName !== undefined) {
...
function normalizePatientDirectoryRow(row: DbRow) {
  ...
  const pathologyNames = normalizeTextArray(row.pathologyNames ?? row.pathology_names);
  const activePathologyNames = normalizeTextArray(
    row.activePathologyNames ?? row.active_pathology_names,
  );
  const pathologyStatuses = normalizeTextArray(row.pathologyStatuses ?? row.pathology_statuses);
  const careProfiles = normalizeTextArray(row.careProfiles ?? row.care_profiles);
  const sportsPracticed = normalizeTextArray(row.sportsPracticed ?? row.sports_practiced);
  const therapyFocuses = normalizeTextArray(row.therapyFocuses ?? row.therapy_focuses);
  const classification = trimmedString(row.classification as string) ?? null;
  const financialStatus =
    trimmedString((row.financialStatus ?? row.financial_status) as string) ?? null;
  const lastAppointmentDate = row.lastAppointmentDate ?? row.last_appointment_date;
  const nextAppointmentDate = row.nextAppointmentDate ?? row.next_appointment_date;
  const sessionsCompleted = nullableNumber(row.sessionsCompleted ?? row.sessions_completed);
  const totalAppointments = nullableNumber(row.totalAppointments ?? row.total_appointments);
  ...
  const payerModel = trimmedString((row.payerModel ?? row.payer_model) as string) ?? null;
  const partnerCompanyName =
    trimmedString((row.partnerCompanyName ?? row.partner_company_name) as string) ?? null;
  ...
  return {
    ...
    last_appointment_date: lastAppointmentDate ? String(lastAppointmentDate) : null,
    lastAppointmentDate: lastAppointmentDate ? String(lastAppointmentDate) : null,
    next_appointment_date: nextAppointmentDate ? String(nextAppointmentDate) : null,
...
app.get("/", async (c) => {
  ...
  try {
    ...
    const params: Array<string | number | boolean | string[]> = [user.organizationId];
    let paramIndex = 2;
    const pushCondition = (clause: string, value?: string | number | boolean | string[]) => {
    ...
    if (pathologies.length > 0) {
      pushCondition(`directory."pathologyNames" && $${paramIndex}::text[]`, pathologies);
      paramIndex += 1;
    }
    ...
    if (pathologyStatus && pathologyStatus !== "all") {
      if (pathologyStatus === "active") {
        ...
        baseConditions.push(`CARDINALITY(directory."pathologyNames") > 0`);
      }
    }

    if (careProfiles.length > 0) {
      pushCondition(`directory."careProfiles" && $${paramIndex}::text[]`, careProfiles);
      paramIndex += 1;
    }

    if (sports.length > 0) {
      pushCondition(`directory."sportsPracticed" && $${paramIndex}::text[]`, sports);
      paramIndex += 1;
    }

    if (therapyFocuses.length > 0) {
      pushCondition(`directory."therapyFocuses" && $${paramIndex}::text[]`, therapyFocuses);
      paramIndex += 1;
    }
    ...
    if (financialStatus && financialStatus !== "all") {
      pushCondition(`directory."financialStatus" = $${paramIndex++}`, financialStatus);
    }
    ...
    if (partnerCompany && partnerCompany !== "all") {
      pushCondition(`directory."partnerCompanyName" = $${paramIndex++}`, partnerCompany);
    }
    ...
    try {
      ...
      console.log(
        `[Patients/List] Data query took ${duration}ms (Limit: ${safeLimit}, Offset: ${safeOffset})`,
      );

      if (duration > 5000) {
        console.warn(
          `[Patients/List] SLOW QUERY DETECTED: ${duration}ms for org ${user.organizationId}`,
        );
      }
    } catch (e: any) {
    ...
    try {
      ...
      summaryParams = params.slice(
        0,
        params.length - (classification && classification !== "all" ? 1 : 0),
      );
...
app.get("/by-profile/:profileId", async (c) => {
  ...
  try {
    const result = await db
      .select()
      .from(patients)
      .where(withTenant(patients, user.organizationId, eq(patients.profileId, profileId)))
      .limit(1);
...
app.post("/", async (c) => {
  ...
  try {
    ...
    console.error("[Patients/Create] Stack:", error instanceof Error ? error.stack : "N/A");
...
app.delete("/:id", async (c) => {
  ...
  const isAdmin = user.role === "admin" || user.email === "REDACTED_EMAIL";

  try {
    if (isAdmin && mode === "hard") {
      console.log(
        `[Patients/Delete] Hard delete requested by admin ${user.email} for patient ${id}`,
      );
      const result = await db
        .delete(patients)
        .where(withTenant(patients, user.organizationId, eq(patients.id, id)))
...
app.get("/:id/export", async (c) => {
  ...
  try {
    ...
    const exportData = {
      ...
      financialHistory,
      legalNotice:
        "Este arquivo contém dados sensíveis protegidos pela LGPD. O uso indevido destas informações é de responsabilidade do portador.",
    };
...
app.post("/:id/forget", async (c) => {
  ...
  try {
    ...
    return c.json({
      ...
      anonSuffix,
      message:
        "Dados pessoais anonimizados. Histórico clínico mantido sob pseudônimo para fins estatísticos.",
    });
  } catch (error) {
