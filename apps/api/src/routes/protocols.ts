...
async function kvGet(env: Env, key: string): Promise<unknown | null> {
  ...
  try {
    return await env.FISIOFLOW_CONFIG.get(key, "json");
  } catch {
    return null;
  }
}
...
// ===== LISTA DE PROTOCOLOS =====
app.get("/", async (c) => {
  const { q, type, evidenceLevel, icd10, page = "1", limit = "20", professionalId } = c.req.query();
  ...
  if (evidenceLevel) {
    conditions.push(eq(exerciseProtocols.evidenceLevel, evidenceLevel as "A" | "B" | "C" | "D"));
  }
