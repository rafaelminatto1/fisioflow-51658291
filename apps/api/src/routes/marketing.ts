...
app.delete("/content-calendar/:id", requireAuth, async (c) => {
  ...
  await pool.query("DELETE FROM content_calendar WHERE id = $1 AND organization_id = $2", [
    id,
    user.organizationId,
  ]);
  return c.json({ ok: true });
...
app.post("/public/fisiolink/:slug/click", async (c) => {
  ...
  const orgRes = await pool.query("SELECT organization_id FROM fisio_links WHERE slug = $1", [
    slug,
  ]);
