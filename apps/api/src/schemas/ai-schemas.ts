...
export const SoapSchema = z.object({
  subjective: z.string().describe("Queixa principal, histórico, relato do paciente"),
  objective: z.string().describe("Achados de exame físico, testes, mensurações objetivas"),
  assessment: z.string().describe("Diagnóstico fisioterapêutico, raciocínio clínico"),
  ...
  exercises: z
    .array(
      z.object({
        ...
        targetArea: z.string().describe("Área/articulação alvo (ex: ombro, lombar)"),
...
export const ReceiptOcrSchema = z.object({
  valor: z.number().describe("Valor numérico em reais, sem símbolo (ex: 150.00)"),
...
export const ClinicalReportSchema = z.object({
  ...
  trend: z.enum(["positive", "neutral", "negative"]).describe("Tendência evolutiva do paciente"),
  keyFindings: z.array(z.string()).min(1).describe("Principais achados clínicos objetivos"),
  clinicalReasoning: z.string().describe("Raciocínio clínico estruturado baseado nas métricas"),
  recommendations: z.array(z.string()).min(1).describe("Recomendações de conduta, próximos passos"),
  riskFactors: z.array(z.string()).describe("Fatores de risco identificados (vazio se nenhum)"),
});
  ...
  suggestions: z
    .array(
      z.object({
        ...
        reason: z.string().describe("Por que este campo/teste é relevante para o contexto"),
        category: z.enum([
          "escala_dor",
          "amplitude_movimento",
          "forca_muscular",
...
export const TreatmentAdherenceSchema = z.object({
  riskLevel: z.enum(["low", "medium", "high"]).describe("Nível de risco de não-aderência"),
  confidenceScore: z.number().min(0).max(1).describe("Confiança da predição (0-1)"),
  primaryFactors: z
    .array(z.string())
    .min(1)
