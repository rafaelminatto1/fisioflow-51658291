...
export async function deleteContactData(
	...
	try {
		...
		if (conversationIds.length > 0) {
			await pool.query(
				`DELETE FROM wa_messages WHERE conversation_id = ANY($1)`,
				[conversationIds],
			);

			await pool.query(
				`DELETE FROM wa_assignments WHERE conversation_id = ANY($1)`,
				[conversationIds],
			);

			await pool.query(
				`DELETE FROM wa_sla_tracking WHERE conversation_id = ANY($1)`,
				[conversationIds],
			);

			await pool.query(`DELETE FROM wa_conversations WHERE id = ANY($1)`, [
				conversationIds,
			]);
		}

		await pool.query(
			`DELETE FROM wa_opt_in_out WHERE contact_id = $1 AND org_id = $2`,
			[contactId, orgId],
		);

		await pool.query(
			`DELETE FROM wa_access_log WHERE contact_id = $1 AND org_id = $2`,
			[contactId, orgId],
		);
