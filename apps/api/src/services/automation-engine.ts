...
export async function processAutomationTriggers(
  ...
  try {
    ...
    for (const row of rulesResult.rows) {
      ...
      for (const action of rule.actions) {
        try {
          ...
          console.error("[automation-engine] action execution error:", actionError);
        }
      }
    ...
    console.error("[automation-engine] processAutomationTriggers error:", error);
  }
}

export function evaluateCondition(condition: AutomationCondition, data: TriggerData): boolean {
...
export async function executeAction(
  ...
  switch (action.type) {
    ...
    case "send_interactive": {
      ...
      const bodyText = interpolateTemplate(action.config.bodyText ?? "", context);

      if (interactiveType === "button") {
        await sendReplyButtons(env, waId, bodyText, action.config.buttons ?? [], {
    ...
    case "assign_to": {
      ...
      if (assigneeId) {
        await assignConversation(pool, context.conversationId, assigneeId, "automation", team);
      }
      break;
    }
    ...
    case "notify": {
      await broadcastToOrg(env, context.orgId, {
        ...
        message: interpolateTemplate(action.config.notificationMessage ?? "", context),
...
function interpolateTemplate(template: string, context: ActionContext): string {
  return template.replace(/\{\{(\w+)\}\}/g, (match, key) => {
    ...
    if (context.message && key in context.message) return String(context.message[key]);
    return match;
  });
}
