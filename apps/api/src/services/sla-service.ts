  ...
  try {
    ...
    await pool.query(
      ...
      ON CONFLICT (conversation_id) DO UPDATE SET
        ...
        resolved_at = NULL`,
      [conversationId, orgId, priority, firstResponseTarget, nextResponseTarget, resolutionTarget],
    );
  } catch (error) {
...
export async function markFirstResponse(pool: Pool, conversationId: string): Promise<void> {
  try {
    await pool.query(
...
export async function markNextResponse(pool: Pool, conversationId: string): Promise<void> {
  try {
    await pool.query(
...
export async function markResolved(pool: Pool, conversationId: string): Promise<void> {
  try {
    await pool.query(`UPDATE wa_sla_tracking SET resolved_at = now() WHERE conversation_id = $1`, [
      conversationId,
    ]);
  } catch (error) {
