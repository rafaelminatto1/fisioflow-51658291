      ...
      try {
        ...
        for (const source of aiResults.sources || []) {
          ...
          if (types.includes(resType) || types.includes("wiki")) {
            ...
            const booster =
              detectedRegion && source.content?.toLowerCase().includes(detectedRegion) ? 0.1 : 0;
            allResults.push({
              id: source.id,
              type: resType as any,
    ...
    try {
      await runWithOrg(organizationId, async () => {
        ...
        if (types.includes("exercise")) {
          const internalEx = await db
            .select()
            .from(exercises)
            .where(
              or(searchFilter(exercises.name, query), searchFilter(exercises.description, query)),
            )
            .limit(4);
        ...
        if (types.includes("test")) {
          const internalTests = await db
            .select()
            .from(clinicalTestTemplates)
            .where(
              or(
                searchFilter(clinicalTestTemplates.name, query),
                searchFilter(clinicalTestTemplates.targetJoint, query),
              ),
            )
            .limit(3);
  ...
  public async saveSuggestion(
    ...
    return await runWithOrg(orgId, async () => {
      return await db.insert(clinicalResourceSuggestions).values({
        ...
        userId: userId,
        query: query,
        suggestedType: (resource.type === "external_suggestion"
          ? "exercise"
