...
export async function buildPatientContext(
  ...
  const [
    ...
    db
      .select()
      .from(medicalRecords)
      .where(and(eq(medicalRecords.patientId, patientId), isNull(medicalRecords.deletedAt)))
      .orderBy(desc(medicalRecords.createdAt))
      .limit(1),
    db
      .select()
      .from(patientPathologies)
      .where(and(eq(patientPathologies.patientId, patientId), isNull(patientPathologies.deletedAt)))
      .orderBy(desc(patientPathologies.isPrimary), desc(patientPathologies.diagnosedAt)),
    db
      .select()
      .from(patientGoals)
      .where(and(eq(patientGoals.patientId, patientId), isNull(patientGoals.deletedAt)))
    ...
    db
      .select()
      .from(appointments)
      .where(and(eq(appointments.patientId, patientId), sql`${appointments.date} < ${today}`))
...
function renderPatientContextXml(input: RenderInput): string {
  ...
  if (patient.bloodType) lines.push(`    <blood_type>${xmlEscape(patient.bloodType)}</blood_type>`);
  if (patient.weightKg)
  ...
  if (patient.mainCondition)
    lines.push(`    <main_condition>${xmlEscape(patient.mainCondition)}</main_condition>`);
  ...
  if (input.medical) {
    ...
    lines.push("  <anamnese>");
    if (m.chiefComplaint)
      lines.push(`    <chief_complaint>${xmlEscape(m.chiefComplaint)}</chief_complaint>`);
    if (m.currentHistory)
      lines.push(`    <current_history>${xmlEscape(m.currentHistory)}</current_history>`);
    if (m.pastHistory) lines.push(`    <past_history>${xmlEscape(m.pastHistory)}</past_history>`);
    if (m.medicalHistory)
      lines.push(`    <medical_history>${xmlEscape(m.medicalHistory)}</medical_history>`);
    if (m.familyHistory)
      lines.push(`    <family_history>${xmlEscape(m.familyHistory)}</family_history>`);
    if (m.currentMedications)
      lines.push(`    <medications>${xmlEscape(m.currentMedications)}</medications>`);
    if (m.previousSurgeries)
      lines.push(`    <previous_surgeries>${xmlEscape(m.previousSurgeries)}</previous_surgeries>`);
    if (m.lifestyleHabits) lines.push(`    <lifestyle>${xmlEscape(m.lifestyleHabits)}</lifestyle>`);
    if (m.diagnosis) lines.push(`    <diagnosis>${xmlEscape(m.diagnosis)}</diagnosis>`);
    if (Array.isArray(m.icd10Codes) && m.icd10Codes.length > 0) {
      lines.push(`    <icd10>${xmlEscape(m.icd10Codes.join(", "))}</icd10>`);
    }
  ...
  if (input.goals.length > 0) {
    ...
    for (const g of input.goals) {
      const attrs = [
        ...
        .filter(Boolean)
        .join(" ");
      lines.push(`    <goal ${attrs}>${xmlEscape(g.description ?? "")}</goal>`);
    }
    lines.push("  </treatment_goals>");
  }
  ...
  if (input.testResults.length > 0) {
    ...
    for (const t of input.testResults) {
      const attrs = [
        ...
        t.maxScore != null ? `max="${xmlEscape(String(t.maxScore))}"` : null,
      ]
        .filter(Boolean)
        .join(" ");
      lines.push(`    <test ${attrs}>${xmlEscape(t.interpretation ?? "")}</test>`);
    }
    lines.push("  </standardized_tests>");
  }
