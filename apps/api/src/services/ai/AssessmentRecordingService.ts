...
import { AssessmentFormSchema, type AssessmentForm } from "../../schemas/assessment-schema";
import {
...
export class AssessmentRecordingService {
  ...
  async processRecording(input: ProcessRecordingInput): Promise<AssessmentResult> {
    const transcript = await transcribeAudio(this.env, input.audioBase64);
    if (!transcript || transcript.trim().length < 10) {
      throw new Error("Transcrição vazia ou muito curta. Verifique a qualidade do áudio.");
    }
  ...
  async processTranscript(input: ProcessTranscriptInput): Promise<AssessmentResult> {
  ...
  private async loadPatientContext(
    ...
    try {
      ...
      console.warn("[AssessmentRecordingService] Falha ao carregar contexto do paciente:", error);
      return undefined;
    }
  }
}
