...
export class AssessmentLiveSession implements DurableObject {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.pathname === "/ws") {
      ...
      const organizationId = url.searchParams.get("organizationId") ?? "unknown";
      ...
      server.addEventListener("message", (evt) => this.handleClientMessage(evt.data));
  ...
  private async handleClientMessage(raw: string | ArrayBuffer): Promise<void> {
    ...
    try {
      ...
      console.error("[AssessmentLiveSession] Error forwarding to Gemini:", err);
      this.sendToClient({
        type: "error",
