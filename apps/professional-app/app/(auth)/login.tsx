...
export default function LoginScreen() {
  ...
  const { authenticate, enable, isAvailable, isEnabled, biometricTypeName, checkAvailability } =
    useBiometricAuth();
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Save email for biometrics (password NEVER stored)
  ...
  const isEmailUnverified =
    error?.toLowerCase().includes("not verified") || error?.toLowerCase().includes("verificado");
        ...
        >
          ...
          <View style={styles.form}>
              ...
              <View style={[styles.errorBox, { backgroundColor: colors.errorLight }]}>
                ...
                {isEmailUnverified && (
                  <TouchableOpacity onPress={handleResendVerification} style={styles.resendButton}>
                    <Text
                      style={[styles.resendText, { color: colors.primary, fontWeight: "bold" }]}
                    >
            ...
            {isAvailable && isEnabled && (
              ...
              >
                <Ionicons name={getBiometricIcon() as any} size={24} color={colors.textSecondary} />
