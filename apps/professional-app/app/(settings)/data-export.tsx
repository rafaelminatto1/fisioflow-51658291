...
export default function DataExportScreen() {
  ...
  const handleRequestExport = async () => {
    ...
    if (!password || password.length < 8) {
      Alert.alert(
        "Erro",
        "Por favor, defina uma senha de no mínimo 8 caracteres para criptografar seu arquivo de exportação.",
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        "Erro",
        "Não foi possível processar sua solicitação de exportação. Tente novamente.",
      );
    } finally {
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.warningContainer}>
          ...
          <Text style={[styles.warningText, { color: colors.textSecondary }]}>
            O processamento pode levar até 48 horas. Você receberá um link de download válido por 7
            dias.
          </Text>
        </View>
