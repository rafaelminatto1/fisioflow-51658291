...
export default function WorkingHoursScreen() {
  ...
  const openPicker = (
    ...
    const currentTime =
      day?.[field] ?? (field.includes("break") ? DEFAULT_BREAK_START : DEFAULT_OPEN);
  ...
  const applyPicker = (date: Date) => {
    ...
    setHours((prev) =>
      prev.map((d) =>
        d.day_of_week === pickerTarget.day_of_week ? { ...d, [pickerTarget.field]: timeStr } : d,
      ),
    );
  };
  ...
  const validateHours = () => {
    for (const day of hours) {
      ...
      if (day.open_time >= day.close_time) {
        ...
        Alert.alert(
          "Erro",
          `Em ${label}, o horário de abertura deve ser menor que o de fechamento.`,
        );
        return false;
      }
      if (day.has_break) {
        ...
        if (day.break_start <= day.open_time || day.break_end >= day.close_time) {
          ...
          Alert.alert(
            "Erro",
            `Em ${label}, o intervalo de pausa deve estar dentro do horário de atendimento.`,
          );
          return false;
        }
      }
                ...
                {day.is_open && (
                  <>
                    ...
                    <View style={styles.breakToggleRow}>
                      <Text
                        style={[styles.timeLabel, { color: colors.textSecondary, marginBottom: 0 }]}
                      >
                      ...
                      <Switch
                        ...
                        onValueChange={() => {
                          ...
                          setHours((prev) =>
                            prev.map((d) =>
                              d.day_of_week === key ? { ...d, has_break: !d.has_break } : d,
                            ),
                          );
                        }}
                        trackColor={{ false: colors.border, true: colors.primary + "66" }}
                    ...
                    {key >= 1 && key <= 5 && (
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          ...
                          Alert.alert(
                            "Copiado",
                            "Horários copiados para todos os dias úteis (Seg-Sex).",
                          );
                        }}
                      >
            ...
            >
              <View style={styles.modalHeader}>
                ...
                <Text style={[styles.modalTitle, { color: colors.text }]}>
                  {pickerLabel} —{" "}
                  {currentDay && DAYS.find((d) => d.key === pickerTarget!.day_of_week)?.label}
                </Text>
                <TouchableOpacity
                  onPress={() => {
