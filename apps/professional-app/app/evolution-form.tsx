...
import {
	FillingStyleToggle,
	FillingMode,
} from "@/components/evolution/FillingStyleToggle";
...
export default function EvolutionFormScreen() {
	...
	>("idle");
	const [autoSaveErrorDetail, setAutoSaveErrorDetail] = useState<string | null>(
		null,
	);
	...
	const triggerAutoSave = useCallback(() => {
		...
		autoSaveTimer.current = setTimeout(async () => {
			...
			try {
				...
				const res = await fetchApi<{ data: { id: string } }>(
	...
	useEffect(() => {
		...
		const load = async () => {
			try {
				...
				if (session.pain_level != null)
					setPainLevel(Number(session.pain_level));
			} catch {
