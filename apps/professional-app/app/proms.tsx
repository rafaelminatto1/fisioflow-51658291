...
export default function PROMs() {
  ...
  const filteredScales = PROM_SCALES.filter((s) =>
    search === '' ||
    s.name.toLowerCase().includes(search.toLowerCase()) ||
    s.shortName.toLowerCase().includes(search.toLowerCase())
  );

  const lastResultForScale = (scaleId: string) =>
    results.find((r) => r.scale_name === scaleId);
            ...
            {results.length > 0 && (
              <View style={styles.section}>
                ...
                {results.slice(0, 3).map((r) => {
                  ...
                  return (
                    <View key={r.id} style={[styles.historyCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                      <View style={styles.historyCardLeft}>
                        ...
                        {r.interpretation && (
                          <Text style={[styles.historyInterpretation, { color: colors.textSecondary }]}>
                            {r.interpretation}
                          </Text>
                        )}
                      </View>
            ...
            <View style={styles.section}>
              ...
              {filteredScales.map((scale) => {
                ...
                return (
                  <TouchableOpacity
                    key={scale.id}
                    style={[styles.scaleCard, { backgroundColor: colors.surface, borderColor: colors.border }]}
                    onPress={() => {
                      medium();
                      router.push({
                        pathname: '/prom-form',
                  ...
                  >
                    <View style={styles.scaleCardContent}>
                      ...
                      <View style={styles.scaleInfo}>
                        <Text style={[styles.scaleName, { color: colors.text }]}>{scale.shortName}</Text>
                        <Text style={[styles.scaleFullName, { color: colors.textSecondary }]} numberOfLines={1}>
                          {scale.name}
                        </Text>
                        {last && (
...
const styles = StyleSheet.create({
  ...
  scaleIcon: { width: 40, height: 40, borderRadius: 20, alignItems: 'center', justifyContent: 'center' },
  scaleInfo: { flex: 1 },
