...
import { findExactNormalizedMatch, findSimilarItems } from "@/lib/utils/similarity";
...
function AutocompleteMultiSelectModal({
  ...
  const noResults = searchQuery.trim().length > 0 && filteredOptions.length === 0;

  const handleToggle = useCallback(
    (value: string) => {
      medium();
      const isSelected = selectedValues.includes(value);
      onSelect(isSelected ? selectedValues.filter((v) => v !== value) : [...selectedValues, value]);
    },
  ...
  const handleConfirmNew = useCallback(() => {
    ...
    const alreadySelected = selectedValues.find((s) => normalizeText(s) === normalizeText(trimmed));
    if (alreadySelected) {
    ...
    const exactInAll = allLabels.find((l) => normalizeText(l) === normalizeText(trimmed));
    if (exactInAll && exactInAll !== trimmed) {
      Alert.alert("Autocorreção", `Corrigido para "${exactInAll}". Deseja adicionar?`, [
  ...
  const handleSubmitEditing = useCallback(() => {
    ...
    const exactMatch = options.find((o) => normalizeText(o.label) === normalizeText(trimmed));
    if (exactMatch) {
      if (!selectedValues.includes(exactMatch.label)) {
        onSelect([...selectedValues, exactMatch.label]);
      }
  ...
  }, [searchQuery, options, selectedValues, onSelect, noResults, handleConfirmNew, medium]);
        ...
        <View style={msStyles.content}>
          ...
          {noResults && (
            ...
            >
              ...
              <Text style={msStyles.noResultText}>Criar "{searchQuery.trim()}"</Text>
              <Text style={msStyles.noResultSub}>Toque para adicionar</Text>
            </TouchableOpacity>
          )}

          <ScrollView style={msStyles.scroll} keyboardShouldPersistTaps="handled">
            {filteredOptions.map((option) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  activeOpacity={0.6}
                >
                  <Text style={[msStyles.itemText, isSelected && msStyles.itemTextSelected]}>
                    {option.label}
                  </Text>
                  {isSelected ? (
                    <Ionicons name="checkmark-circle" size={22} color="#4ade80" />
                  ) : (
                    <Ionicons name="ellipse-outline" size={22} color="#475569" />
                  )}
                </TouchableOpacity>
              );
            })}
  ...
  useEffect(() => {
    if (exercise) {
      setFormData({
        ...
        scientific_references: parseScientificReferences(exercise.scientific_references),
      });
  ...
  const handleSave = async () => {
    ...
    try {
      const dataToSave = {
        ...
        scientific_references: JSON.stringify(formData.scientific_references ?? []),
      };
      if (isEditing && exerciseId) {
        ...
        await createExerciseAsync(dataToSave as Omit<Exercise, "id" | "createdAt" | "updatedAt">);
        success();
        ...
        <ScrollView style={styles.form}>
          ...
          >
            <Text
              ...
              numberOfLines={1}
            >
              {categories.length > 0 ? categories.join(", ") : "Selecione categorias..."}
            </Text>
          ...
          <Text style={[styles.label, { color: colors.text }]}>Partes do Corpo</Text>
          <TouchableOpacity
            style={[
              styles.multiSelectBtn,
              { borderColor: colors.border, backgroundColor: colors.surface },
          ...
          >
            <Text
              ...
              numberOfLines={1}
            >
              {bodyParts.length > 0 ? bodyParts.join(", ") : "Selecione partes do corpo..."}
            </Text>
            <Ionicons name="chevron-down" size={20} color={colors.textSecondary} />
          </TouchableOpacity>
          {bodyParts.length > 0 && (
            <View style={styles.chipsContainer}>
              {bodyParts.map((part) => (
                <TouchableOpacity
                  ...
                  onPress={() => setBodyParts(bodyParts.filter((p) => p !== part))}
                >
                  <Text style={[styles.chipText, { color: colors.textMuted }]}>{part}</Text>
                  <Ionicons name="close" size={11} color={colors.textMuted} />
                </TouchableOpacity>
              ))}
            </View>
          ...
          <Text style={[styles.label, { color: colors.text }]}>Equipamentos</Text>
          <TouchableOpacity
            style={[
              styles.multiSelectBtn,
              { borderColor: colors.border, backgroundColor: colors.surface },
          ...
          >
            <Text
              ...
              numberOfLines={1}
            >
              {equipment.length > 0 ? equipment.join(", ") : "Selecione equipamentos..."}
            </Text>
            <Ionicons name="chevron-down" size={20} color={colors.textSecondary} />
          </TouchableOpacity>
          {equipment.length > 0 && (
            <View style={styles.chipsContainer}>
              {equipment.map((eq) => (
                <TouchableOpacity
                  ...
                  onPress={() => setEquipment(equipment.filter((e) => e !== eq))}
                >
                  <Text style={[styles.chipText, { color: colors.textMuted }]}>{eq}</Text>
                  <Ionicons name="close" size={11} color={colors.textMuted} />
                </TouchableOpacity>
              ))}
            </View>
          ...
          {tags.length > 0 && (
            <View style={styles.chipsContainer}>
              {tags.map((tag) => (
                ...
                >
                  <Text style={[styles.chipText, { color: colors.textMuted }]}>{tag}</Text>
                  <Ionicons name="close" size={11} color={colors.textMuted} />
                </TouchableOpacity>
              ))}
            </View>
          ...
          >
            <Text
              style={{
                color: (formData.pathologiesIndicated || []).length
                  ? colors.text
                  : colors.textMuted,
                flex: 1,
              }}
          ...
          >
            <Text
              style={{
                color: (formData.pathologiesContraindicated || []).length
                  ? colors.text
                  : colors.textMuted,
                flex: 1,
              }}
          ...
          <SegmentedPicker
            ...
            onChange={(v) =>
              updateField("precaution_level", v as ExerciseFormData["precaution_level"])
            }
            primaryColor={colors.primary}
            textMuted={colors.textMuted}
          />
