...
const REGION: Record<
  Region,
  { tint: string; stroke: string; badgeBg: string; badgeFg: string; label: string }
> = {
  knee: {
    tint: "hsl(211, 55%, 93%)",
    stroke: "hsl(211, 50%, 45%)",
    badgeBg: "hsl(211, 100%, 93%)",
    badgeFg: "hsl(211, 100%, 38%)",
    label: "JOELHO",
  },
  gait: {
    tint: "hsl(264, 47%, 93%)",
    stroke: "hsl(264, 45%, 50%)",
    badgeBg: "hsl(264, 60%, 93%)",
    badgeFg: "hsl(264, 50%, 45%)",
    label: "MARCHA",
  },
  shoulder: {
    tint: "hsl(28, 77%, 93%)",
    stroke: "hsl(28, 70%, 48%)",
    badgeBg: "hsl(28, 92%, 92%)",
    badgeFg: "hsl(25, 70%, 38%)",
    label: "OMBRO",
  },
  trunk: {
    tint: "hsl(158, 47%, 92%)",
    stroke: "hsl(158, 45%, 38%)",
    badgeBg: "hsl(158, 60%, 92%)",
    badgeFg: "hsl(158, 55%, 30%)",
    label: "POSTURA",
  },
};
  ...
  const visibleTests = protocols.length
    ? protocols.map((protocol) => {
        ...
        const metrics = Array.isArray(protocol.metricDefinitions)
          ? protocol.metricDefinitions
              .slice(0, 3)
              .map((metric: any) => String(metric.key ?? metric.label ?? "Métrica"))
          : ["ROM", "Simetria"];
        return {
          id: protocol.id,
          name: protocol.name,
          ...
          meta: [
            {
              icon: Clock,
              t: requirements.minDurationMs
                ? `~${Math.ceil(Number(requirements.minDurationMs) / 1000)}s`
                : "~2 min",
            },
            {
              icon: Repeat,
              t: requirements.attempts ? `${requirements.attempts} tent.` : "2 tent.",
            },
          ],
          protocol,
        };
        ...
        >
          {FILTERS.map((f) => {
            ...
            return (
              <Pressable
                key={f.label}
                onPress={() => setSel(f.label)}
                style={[styles.chip, active && styles.chipSel]}
              >
                {!active && f.dot ? (
