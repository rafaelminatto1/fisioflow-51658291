          ...
          {[1, 2].map((card) => (
            <View key={card} style={{ paddingHorizontal: 8, marginBottom: 8 }}>
              <Card style={{ borderRadius: 10, padding: 10 }}>
                ...
                <Skeleton width="50%" height={10} variant="text" style={{ marginTop: 6 }} />
              </Card>
            </View>
          ))}
        </View>
...
export default function TarefasScreen() {
  ...
  const filteredData = useMemo(
    () => (filterStatus === "TODAS" ? data : data.filter((t) => t.status === filterStatus)),
    [data, filterStatus],
  );
    ...
    return (
      <SafeAreaView style={[styles.center, { backgroundColor: colors.background }]} edges={["top"]}>
        <View style={styles.errorContainer}>
          <View style={[styles.errorIconContainer, { backgroundColor: colors.errorLight + "30" }]}>
        ...
        <View style={styles.headerLeft}>
          ...
          {!isLoading && (
            <View style={[styles.totalBadge, { backgroundColor: colors.primary + "15" }]}>
              <Text style={[styles.totalBadgeText, { color: colors.primary }]}>{data.length}</Text>
            </View>
          )}
        </View>
          ...
          {isLoading && !refreshing ? (
            ...
            <View style={styles.emptyState}>
              <View style={[styles.emptyIconContainer, { backgroundColor: colors.primary + "10" }]}>
                <Ionicons
                  name={filterStatus === "TODAS" ? "clipboard-outline" : "filter-outline"}
                  size={48}
                  color={colors.primary}
                />
              </View>
              <Text style={[styles.emptyTitle, { color: colors.text }]}>
                {filterStatus === "TODAS" ? "Tudo em dia!" : "Nenhuma tarefa encontrada"}
              </Text>
