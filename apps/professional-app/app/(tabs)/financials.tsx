...
import { useAllFinancialRecords, useDeleteFinancialRecord } from "@/hooks/usePatientFinancial";
...
export default function FinancialsScreen() {
  ...
  const { data: records, isLoading, refetch } = useAllFinancialRecords(queryOptions);
  ...
  const totalRevenue =
    records?.reduce(
      (acc, record) => acc + (record.payment_status === "paid" ? record.final_value : 0),
      0,
    ) || 0;
  const totalPending =
    records?.reduce(
      (acc, record) => acc + (record.payment_status === "pending" ? record.final_value : 0),
      0,
    ) || 0;
        ...
        >
          <Text
            style={[
              styles.tabText,
              {
                color: activeTab === "nfse" ? colors.primary : colors.textSecondary,
              },
          ...
          <>
            ...
            <View style={styles.listSection}>
              ...
              {isLoading && !records ? (
                ...
                <EmptyStateFinancial
                  title={
                    hasActiveFilters ? "Nenhum registro encontrado" : "Nenhuma transação encontrada"
                  }
                  description={
                    hasActiveFilters
              ...
              nfseRecords.map((nfse) => (
                ...
                >
                  <View style={styles.nfseHeader}>
                    <View style={{ flex: 1 }}>
                      <Text style={[styles.nfseTitle, { color: colors.text }]} numberOfLines={1}>
                  ...
                  <View style={[styles.nfseFooter, { borderTopColor: colors.border + "50" }]}>
                    ...
                    {nfse.link_nfse ? (
                      <TouchableOpacity onPress={() => light()} style={styles.openNfseBtn}>
                        <Ionicons name="open-outline" size={18} color={colors.primary} />
                        <Text style={[styles.openNfseText, { color: colors.primary }]}>
                          Ver PDF
                        </Text>
                      </TouchableOpacity>
                    ) : null}
        ...
        recordInfo={
          recordToDelete
            ? {
                ...
                date: format(new Date(recordToDelete.session_date), "dd/MM/yyyy"),
              }
            : undefined
        }
      />
