          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filters} contentContainerStyle={{ paddingRight: 20 }}>
            ...
            {[0, 3, 5, 7, 9].map((val) => (
              <TouchableOpacity
                ...
                style={[
                  styles.filterChip,
                  { backgroundColor: minPain === val ? colors.primary : colors.background, borderColor: minPain === val ? colors.primary : colors.border }
                ]}
              >
                <Text style={[styles.filterText, { color: minPain === val ? "#fff" : colors.textSecondary }]}>
                  {val === 0 ? "Todas" : `> ${val}`}
                </Text>
              </TouchableOpacity>
            ))}
      ...
      <ScrollView contentContainerStyle={styles.resultsScroll} showsVerticalScrollIndicator={false}>
        {isLoading ? (
          <View style={styles.centerContainer}>
            ...
            <Text style={[styles.loadingText, { color: colors.textSecondary }]}>Analisando base clínica...</Text>
          </View>
          ...
          data.data.map((match: any) => (
            <TouchableOpacity 
              ...
              onPress={() => router.push(`/evolution-detail?evolutionId=${match.evolutionId}&patientId=${match.patientId}&patientName=${encodeURIComponent(match.patientName)}` as any)}
            >
              <View style={styles.cardHeader}>
                <View style={[styles.avatar, { backgroundColor: colors.primary + "20" }]}>
                  <Text style={[styles.avatarText, { color: colors.primary }]}>{match.patientName.charAt(0)}</Text>
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={[styles.patientName, { color: colors.text }]}>{match.patientName}</Text>
                  <Text style={[styles.date, { color: colors.textSecondary }]}>
                    {format(new Date(match.sessionDate), "dd 'de' MMM, yyyy", { locale: ptBR })}
                  </Text>
                </View>
                <View style={[styles.painBadge, { backgroundColor: match.painScale > 7 ? "#EF444420" : colors.primary + "20" }]}>
                  <Text style={[styles.painText, { color: match.painScale > 7 ? "#EF4444" : colors.primary }]}>Dor {match.painScale}/10</Text>
                </View>
              </View>
          ...
          <View style={styles.centerContainer}>
            ...
            <Text style={[styles.emptyText, { color: colors.textSecondary }]}>Nenhum padrão clínico correspondente encontrado.</Text>
          </View>
        ) : (
          <View style={styles.centerContainer}>
            <Ionicons name="bulb-outline" size={48} color={colors.textMuted} />
            <Text style={[styles.emptyText, { color: colors.textSecondary }]}>Busque por sintomas, tratamentos ou diagnósticos para encontrar casos similares.</Text>
          </View>
        )}
      </ScrollView>
    </View>
...
const styles = StyleSheet.create({
  ...
  searchBar: { flexDirection: "row", alignItems: "center", paddingHorizontal: 12, borderRadius: 12, borderWidth: 1, height: 50 },
  ...
  emptyText: { marginTop: 12, fontSize: 14, textAlign: "center", paddingHorizontal: 40, lineHeight: 20 },
  matchCard: { padding: 16, borderRadius: 16, borderWidth: 1, marginBottom: 12, shadowColor: "#000", shadowOpacity: 0.05, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, elevation: 2 },
