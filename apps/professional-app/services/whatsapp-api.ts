...
function normalizeMessage(row: any): WaMessage {
  ...
  return {
    ...
    isInternalNote: Boolean(row.isInternalNote ?? row.is_internal_note) || messageType === "note",
    createdAt,
  };
}
...
function normalizeConversation(row: any): WaConversation {
  const lastContent =
    ...
    (typeof row.lastMessage === "object" ? row.lastMessage?.content : row.lastMessage);
  const lastMessageType =
    row.lastMessageType ??
    row.last_message_type ??
    (typeof row.lastMessage === "object" ? row.lastMessage?.messageType : undefined);
...
export async function fetchConversations(
  ...
  const response = await fetchApi<any>(`${BASE}/conversations`, { params });
  ...
  return {
    ...result,
    data: Array.isArray(result?.data) ? result.data.map(normalizeConversation) : [],
  } as ConversationsResponse;
}

export async function fetchConversationDetail(id: string): Promise<WaConversationDetailResponse> {
  const response = await fetchApi<any>(`${BASE}/conversations/${id}`, {
...
export async function resolveContact(input: ResolveContactInput): Promise<WaContact> {
...
export async function sendMessage(conversationId: string, content: string): Promise<WaMessage> {
  const response = await fetchApi<any>(`${BASE}/conversations/${conversationId}/messages`, {
    method: "POST",
...
export async function addNote(conversationId: string, content: string): Promise<WaMessage> {
  const response = await fetchApi<any>(`${BASE}/conversations/${conversationId}/notes`, {
    method: "POST",
    data: { content },
  });
  return normalizeMessage(unwrapResponse(response));
...
export async function updateConversationStatus(
  ...
  const response = await fetchApi<any>(`${BASE}/conversations/${conversationId}/status`, {
    method: "PUT",
    data: { status },
  });
  return normalizeConversation(unwrapResponse(response));
...
export async function assignConversation(
  ...
  const response = await fetchApi<any>(`${BASE}/conversations/${conversationId}/assign`, {
    method: "POST",
...
export async function addTags(conversationId: string, tagIds: string[]): Promise<void> {
