...
try {
  ...
  console.warn("[NativePoseProvider] expo-vision-pose-detector não disponível neste ambiente");
}
...
export class NativePoseProvider implements PoseProvider {
  ...
  async detect(image: any): Promise<PoseDetection> {
    if (!this.isLoaded || !VisionPoseDetector) {
      throw new Error("NativePoseProvider não inicializado ou módulo não disponível");
    }
    ...
    if (Platform.OS === "ios") {
      try {
        ...
        console.error("[NativePoseProvider] Erro na detecção nativa iOS:", error);
      }
    }
