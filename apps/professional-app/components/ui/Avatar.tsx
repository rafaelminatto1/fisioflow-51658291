...
export function Avatar({ source, alt = "Avatar", size = "md", fallback, style }: AvatarProps) {
  ...
  return (
    ...
    >
      {source ? (
        <Image source={source} style={[styles.image, { borderRadius: sizeStyles.width / 2 }]} />
      ) : (
        <View style={[styles.fallback, { backgroundColor: colors.primary + "20" }]}>
          <Text
            style={[styles.fallbackText, { color: colors.primary, fontSize: sizeStyles.fontSize }]}
          >
            {initials}
          </Text>
        </View>
