...
function topToTime(snappedTop: number, startHour: number, hourHeight: number): string {
  "worklet";
  const totalMinutes = Math.round((snappedTop / hourHeight) * 60) + startHour * 60;
...
export const DraggableAptCard = ({
  ...
  const handleReschedule = (newColumnIndex: number, newTime: string) => {
    ...
    if (hasChangedDay || hasChangedTime) {
      if (onRescheduleRequest) {
        onRescheduleRequest(apt.id, newDay, newTime, (confirm: boolean) =>
          handleConfirm(confirm, newDay, newTime),
        );
  ...
  const panGesture = Gesture.Pan()
    ...
    .onUpdate((e) => {
      ...
      const rawColumn = currentColumnIndex + Math.round(e.translationX / columnWidth);
