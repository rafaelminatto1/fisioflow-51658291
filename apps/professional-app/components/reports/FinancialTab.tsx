...
export function FinancialTab({ selectedPeriod }: FinancialTabProps) {
  ...
  if (isLoading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={{ marginTop: 12, color: colors.textSecondary }}>Carregando dados financeiros...</Text>
      </View>
    );
  }
    ...
    return (
      <Card style={styles.infoCard} padding="md">
        <View style={styles.infoContent}>
          ...
          <Text style={{ marginLeft: 8, color: colors.textSecondary }}>Erro ao carregar dados financeiros.</Text>
        </View>
      </Card>
    );
  }
  ...
  const financialStats = [
    { label: 'Faturamento', value: formatCurrency(metrics?.totalRevenue || 0), color: colors.success },
    { label: 'Sessões', value: metrics?.sessionsCount || 0, color: colors.primary },
    { label: 'Pendentes', value: formatCurrency(metrics?.pendingRevenue || 0), color: colors.warning },
    { label: 'Ticket Médio', value: formatCurrency(metrics?.sessionsCount ? (metrics.totalRevenue / metrics.sessionsCount) : 0), color: colors.info },
  ];
      ...
      <Card style={styles.infoCard} padding="md">
        ...
        <View style={styles.metricsRow}>
          <View style={styles.metricItem}>
            <Text style={[styles.metricValue, { color: colors.text }]}>{formatNumber(metrics?.patientsCount || 0)}</Text>
            <Text style={[styles.metricLabel, { color: colors.textSecondary }]}>Ativos</Text>
          </View>
          <View style={[styles.metricDivider, { backgroundColor: colors.border }]} />
          <View style={styles.metricItem}>
            <Text style={[styles.metricValue, { color: colors.success }]}>+{formatNumber(metrics?.newPatientsThisMonth || 0)}</Text>
