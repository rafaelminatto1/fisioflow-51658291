...
import { ApiError, cleanRequestData, fetchApi, type FetchOptions } from "./http";
import {
  ApiPatient,
  ApiAppointment,
  ApiExercise,
...
export { ApiError, cleanRequestData, fetchApi };
...
function normalizeAppointmentPayload(data: Partial<ApiAppointment>): Record<string, unknown> {
...
function normalizeConversation(rawConversation: any): ApiConversation {
  ...
  const unreadCount =
    typeof rawConversation.unreadCount === "number"
      ...
      : (rawConversation.unreadCount?.[participantId] ?? rawConversation.unread_count ?? 0);
