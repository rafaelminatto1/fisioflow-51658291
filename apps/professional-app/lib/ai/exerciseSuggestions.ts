...
export function analyzeProgress(evolutions: any[]): {
  ...
  const avgPain =
    recentEvolutions.reduce((sum, e) => sum + (e.painLevel || 0), 0) / recentEvolutions.length;
  const avgMobility =
    recentEvolutions.reduce((sum, e) => sum + (e.mobilityScore || 0), 0) / recentEvolutions.length;
  ...
  const firstHalfPain =
    firstHalf.reduce((sum, e) => sum + (e.painLevel || 0), 0) / firstHalf.length;
  const secondHalfPain =
    secondHalf.reduce((sum, e) => sum + (e.painLevel || 0), 0) / secondHalf.length;
