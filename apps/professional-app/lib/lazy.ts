...
export function useLazyLocation() {
  return useLazyModule(() => import("expo-location"), "expo-location");
}
...
export function useLazyMediaLibrary() {
  return useLazyModule(() => import("expo-media-library"), "expo-media-library");
}

/**
 * Hook para carregar o calendário local sob demanda
 */
export function useLazyCalendars() {
  return useLazyModule(() => import("@/components/calendar/MonthView"), "local-month-view");
}
...
export function useLazyChartKit() {
  return useLazyModule(
    () =>
      import("@/components/charts/evolution-chart").then((m) => ({
        default: m.EvolutionChart,
      })),
    "evolution-chart",
  );
}
...
export async function preloadPostLoginModules(): Promise<void> {
  ...
  const modules = [{ name: "date-fns", importer: () => import("date-fns") }];
