...
export function useTelemedicine() {
  ...
  const startMutation = useMutation({
    ...
    onMutate: async (id) => {
      ...
      queryClient.setQueryData<ApiTelemedicineRoom[]>(["telemedicine", "rooms"], (old) =>
        old?.map((room) => (room.id === id ? { ...room, status: "active" as const } : room)),
      );
      return { previousRooms };
    },
    onError: (_err, _id, context) => {
      if (context?.previousRooms) {
        queryClient.setQueryData(["telemedicine", "rooms"], context.previousRooms);
      }
    },
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: ["telemedicine", "rooms"] });
