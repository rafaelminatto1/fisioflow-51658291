 ...
 */
export async function toggleSetting(key: keyof AppSettings): Promise<Result<boolean>> {
  return asyncResult(async () => {
    const settings = await AppStorage.getSettings();
...
export async function resetSettings(): Promise<Result<void>> {
  return asyncResult(async () => {
    ...
    await AppStorage.setSetting("@fisioflow_settings_notifications", defaults.notifications);
    ...
    await AppStorage.setSetting("@fisioflow_settings_auto_play_videos", defaults.autoPlayVideos);
    await AppStorage.setSetting("@fisioflow_settings_haptic_feedback", defaults.hapticFeedback);
